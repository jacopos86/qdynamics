...
def test_project_runtime_theta_preserves_bit_identical_block_values() -> None:
    layout = build_parameter_layout(_sample_terms())
    theta_runtime = np.asarray([-0.0, -0.0, 0.125], dtype=float)

    theta_logical = project_runtime_theta_block_mean(theta_runtime, layout)

    assert theta_logical.view(np.uint64).tolist() == [
        theta_runtime[0].view(np.uint64).item(),
        theta_runtime[2].view(np.uint64).item(),
    ]
...
def test_serialize_deserialize_layout_preserves_generator_execution_mode() -> None:
    grouped_term = AnsatzTerm(
        label="grouped",
        polynomial=PauliPolynomial(
            "JW",
            [PauliTerm(2, ps="xx", pc=1.0), PauliTerm(2, ps="yy", pc=0.5)],
        ),
        execution_mode="grouped_exact",
    )

    payload = serialize_layout(build_parameter_layout([grouped_term]))
    restored = deserialize_layout(payload)

    assert payload["blocks"][0]["execution_mode"] == "grouped_exact"
    assert restored.blocks[0].execution_mode == "grouped_exact"


def test_deserialize_legacy_layout_defaults_missing_execution_mode() -> None:
    payload = serialize_layout(build_parameter_layout(_sample_terms()))
    for block in payload["blocks"]:
        block.pop("execution_mode")

    restored = deserialize_layout(payload)

    assert [block.execution_mode for block in restored.blocks] == [
        "termwise_product",
        "termwise_product",
    ]
