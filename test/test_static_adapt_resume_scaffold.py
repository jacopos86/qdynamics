...
from pipelines.static_adapt.cli_config import _build_run_hardcoded_adapt_vqe_kwargs
...
def test_resume_cli_flags_mutual_exclusion_and_secret_guard() -> None:
    args = parse_args(
        [
    ...
    assert args.adapt_resume_mode == "scaffold_v1"
    assert args.adapt_resume_smoke_backend == "FakeMarrakesh"
    assert args.adapt_segment_target_depth == 3
    assert args.adapt_segment_target_controller_round == 45
    assert args.adapt_resume_boundary_refit_policy == "verified_checkpoint_no_refit_v1"
    assert args.adapt_scipy_maxfev == 1234
    ...
    assert kwargs["adapt_resume_mode"] == "scaffold_v1"
    assert kwargs["adapt_segment_target_depth"] == 3
    assert kwargs["adapt_segment_target_controller_round"] == 45
    assert kwargs["adapt_resume_boundary_refit_policy"] == (
        "verified_checkpoint_no_refit_v1"
    )
    assert kwargs["adapt_resume_compile_smoke"] == "off"
    assert kwargs["adapt_scipy_maxfev"] == 1234

    defaults = parse_args(["--adapt-resume-compile-smoke", "off"])
    assert defaults.adapt_resume_boundary_refit_policy == "required"
    assert defaults.adapt_segment_target_controller_round is None

    with pytest.raises(SystemExit):
        parse_args(["--adapt-ref-json", "a.json", "--adapt-resume-scaffold-json", "b.json"])

    with pytest.raises(SystemExit):
        parse_args(["--adapt-segment-id", "token=abc"])
...
def test_phase3_batch_order_defaults_to_finite_step_and_legacy_alias() -> None:
    def _kwargs_for(cli_tokens: list[str]) -> dict[str, object]:
        args = parse_args(["--adapt-resume-compile-smoke", "off", *cli_tokens])
    ...
    kwargs = _kwargs_for([])
    assert kwargs["phase3_batch_order_selection_mode"] == "finite_step_v1"
    assert kwargs["phase3_batch_order_max_permutations"] == 24

    kwargs = _kwargs_for(["--phase2-batch-order-selection-mode", "score_sorted"])
    assert kwargs["phase3_batch_order_selection_mode"] == "score_sorted"

    kwargs = _kwargs_for(["--phase3-batch-order-selection-mode", "score_sorted"])
    assert kwargs["phase3_batch_order_selection_mode"] == "score_sorted"
...
def test_output_payload_copies_resume_segment_and_credential_audit() -> None:
    args = parse_args(
        [
            "--adapt-resume-compile-smoke",
            "off",
    ...
    assert payload["adapt_resume_import"]["no_credentials_serialized"] is True
    assert payload["adapt_segment"]["segment_id"] == "seg1"
    assert payload["credential_audit"]["runtime_credentials_serialized"] is False
    assert payload["settings"]["adapt_resume_boundary_refit_policy"] == (
        "verified_checkpoint_no_refit_v1"
    )
    assert payload["settings"]["adapt_segment_target_controller_round"] == 45


def test_output_payload_serializes_disabled_batch_fields_as_null() -> None:
    args = parse_args(["--phase2-no-batching", "--adapt-resume-compile-smoke", "off"])
    adapt_payload = {
        "success": True,
        "energy": -1.0,
        "exact_gs_energy": -1.0,
    ...
    assert settings["phase2_batch_target_size"] is None
    assert settings["phase2_batch_size_cap"] is None
    assert settings["phase2_batch_near_degenerate_ratio"] is None
    assert settings["phase2_batch_rank_rel_tol"] is None
    assert settings["phase2_batch_additivity_tol"] is None


def test_output_payload_serializes_sr_route_profile_identity() -> None:
    args = parse_args(
        [
    ...
    assert not hasattr(args, "sr_controller_ablation_contract")
    adapt_payload = {
        "success": True,
        "energy": -1.0,
        "exact_gs_energy": -1.0,
    ...
    settings = payload["settings"]
    assert settings["sr_escape_mode"] == "saddle_only"
    assert settings["historical_singleton_coordinate_solve_policy"] == (
        "supported_metric_global_trust_eigh_v2"
    )
    assert settings["historical_singleton_coordinate_solve_scope"] == (
        "phase3_only_v1"
    )
    assert settings["sr_controller_ablation_contract"] == "off"
    assert settings["route_family"] == "singleton_response_snake"
    assert settings["route_profile"] == (
        "supported_whitened_adaptive_trust_saddle_escape_v2"
    )
    assert settings["sr_powell_coordinate_chart_policy_requested"] == (
        "logical_shared_reduced_v1"
    )
    assert settings["sr_powell_coordinate_chart_policy"] == (
        "logical_shared_reduced_v1"
    )
...
def test_output_payload_materializes_canonical_sr_chart_from_auto_request() -> None:
    args = parse_args(
        [
            "--historical-singleton-coordinate-solve-policy",
    ...
    adapt_payload = {
        ...
        "static_route_identity": {
            "route_family": "singleton_response_snake",
            "route_profile": "supported_whitened_adaptive_trust_v1",
            "powell_coordinate_chart_policy": canonical_chart,
        },
    ...
    settings = round_trip["settings"]

    assert settings["sr_powell_coordinate_chart_policy_requested"] == "auto"
    assert settings["sr_powell_coordinate_chart_policy"] == canonical_chart
    assert settings["sr_powell_coordinate_chart_policy_source"] == (
