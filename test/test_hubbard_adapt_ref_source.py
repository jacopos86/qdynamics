...
class TestInternalHHAdaptTerminationSemantics:
    def test_internal_hh_phase3_disables_eps_energy_hard_stop(
        ...
        assert bool(adapt_internal.get("adapt_drop_policy_enabled")) is False
        assert adapt_internal.get("adapt_drop_floor_resolved") == pytest.approx(-1.0)
        assert int(adapt_internal.get("adapt_drop_patience_resolved")) == 0
        assert int(adapt_internal.get("adapt_drop_min_depth_resolved")) == 0
        assert adapt_internal.get("adapt_grad_floor_resolved") == pytest.approx(-1.0)
        assert adapt_internal.get("adapt_drop_policy_source") == "auto_staged"
