...
class TestHHBeamRuntimeFallbackRegression:
    def test_beam_finite_angle_fallback_splices_runtime_block(self):
        ...
        assert final_choice["benchmark_target_hit_success"] is False
        assert final_choice["benchmark_target_classification"]["required_stop_reason"] == "benchmark_abs_delta_e_target"
        assert summary["benchmark_target_classification"] == final_choice["benchmark_target_classification"]
        assert audit["benchmark_target_classification"] == final_choice["benchmark_target_classification"]
        assert beam_diag["winner_target_hit_success"] is False
        assert beam_diag["winner_target_hit_classification"]["required_stop_reason"] == "benchmark_abs_delta_e_target"
        assert beam_diag["final_checkpoint_relationship"]["schema_version"] == "static_adapt_beam_final_checkpoint_relationship_v1"
        ...
        assert beam_diag["admission_surface_version"] == "beam_phase3_shortlist_no_stop_archive_v2"
        assert beam_diag["terminal_archive_enabled"] is False
        assert beam_diag["rounds"]
        assert int(beam_diag["terminal_final_count"]) == 0
        assert all(
            int(row["stop_children_count"]) == 0
            and int(row["terminal_kept_count"]) == 0
        ...
        assert payload["history"][-1]["beam_structural_mode"] == "single_admission"
        ...
        assert all(
            set(row["post_admission_prune"]["probe_indices"]).issubset(
                set(row["post_admission_prune"]["recoverability_eligible_indices"])
            )
            for row in payload["history"]
        )
    ...
    def test_beam_shared_seed_policy_is_plumbed_for_multi_admission_spsa(self):
        h_poly = build_hubbard_holstein_hamiltonian(
            dims=2,
            J=1.0,
            U=4.0,
        ...
        assert int(first_row["beam_parent_proposal_family_size"]) == 2
        assert str(first_row["beam_local_optimizer_seed_policy"]) == "parent_shared"
        assert isinstance(first_row["beam_local_optimizer_seed"], int)
        assert first_row["beam_local_optimizer_seed_proposal_fingerprint"] is None
    ...
    def test_recoverability_prune_final_checkpoint_records_safe_ladder(self):
        h_poly = build_hubbard_holstein_hamiltonian(
            dims=2,
            J=1.0,
            U=0.5,
        ...
        payload, _psi = _run_hardcoded_adapt_vqe(
            ...
            phase1_prune_min_candidates=1,
            phase1_prune_max_candidates=4,
            phase1_prune_max_regression=0.0,
            phase1_prune_retained_gain_ratio=0.0,
            phase1_prune_protect_steps=0,
            phase1_shortlist_size=16,
            phase1_probe_max_positions=16,
            phase1_trough_margin_ratio=1.0,
            phase2_shortlist_fraction=1.0,
        ...
        assert prune["prune_policy"] == "recoverability_ladder_v1"
        assert prune["recoverability_ladder_active"] is True
        assert prune["recoverability_rung_policy"] == "final_local_then_full_refit_v1"
        schur_nomination = prune["schur_surrogate_nomination"]
        assert schur_nomination["active"] is True
        assert schur_nomination["used_for_nomination"] is True
        assert schur_nomination["used_for_acceptance"] is False
        assert schur_nomination["coordinate_mode"] in {"logical_shared", "runtime_block"}
        assert int(schur_nomination["score_count"]) > 0
        assert any(
            "schur_surrogate" in source.get("lanes", [])
            for source in prune.get("candidate_nomination_sources", [])
        )
        assert prune["permission_reason"] == "final_checkpoint"
        assert int(prune["candidate_count"]) > 0
        assert prune["recoverability_ladder_rows"]
        assert all(
            row["acceptance_source"] == "remove_refit_energy_safety"
            and row["surrogate_used_for_acceptance"] is False
        ...
        accepted = [row for row in prune["recoverability_ladder_rows"] if row["accepted"]]
        assert len(accepted) == int(prune["accepted_count"])
        if accepted:
            assert accepted[0]["regression"] <= accepted[0]["regression_threshold"] + 1e-12
            assert prune["selected_label"] == accepted[0]["candidate_label"]
            assert prune["trial"]["accepted"] is True
            assert prune["trial"]["rollback_reason"] is None
        else:
            assert float(prune["energy_after_prune"]) == pytest.approx(float(prune["energy_before"]))
    ...
    def test_both_prune_mode_records_live_and_final_surface_metadata(self):
        ...
        payload, _psi = _run_hardcoded_adapt_vqe(
            ...
            phase1_prune_checkpoint_period=1,
            phase1_prune_live_min_depth=99,
            phase1_prune_maturity_threshold=0.0,
        ...
        assert all(int(row["post_admission_prune"]["live_min_depth"]) == 99 for row in payload["history"])
        assert all(
            row["post_admission_prune"].get("live_depth_open") is False
            for row in payload["history"]
            if "live_depth_open" in row["post_admission_prune"]
        )
...
class TestGenericPhase3ContinuationParity:
    ...
    def test_beam_current_checkpoint_includes_replay_telemetry(self, tmp_path, monkeypatch):
        ...
        defaults = {
            **self._hubbard_defaults(),
            ...
            "phase2_shortlist_size": 8,
            "phase2_batch_target_size": 2,
            "phase2_batch_size_cap": 2,
            "adapt_full_refit_every": 0,
            "adapt_final_full_refit": False,
            "finite_angle_fallback": False,
        }
        ...
        payload, _psi = _run_hardcoded_adapt_vqe(
            **defaults,
            adapt_current_json=current_json,
            adapt_current_json_every_depth=1,
            adapt_current_json_keep_history_tail=3,
            benchmark_target_abs_delta_e=1.0e-15,
            phase3_enable_rescue=False,
        ...
        assert checkpoint["schema_version"] == "static_adapt_current_checkpoint_v1"
        assert checkpoint["no_credentials_serialized"] is True
        assert checkpoint["credential_audit"]["no_credentials_serialized"] is True
        assert checkpoint["checkpoint"]["reason"] == "beam_round_done"
        assert checkpoint["checkpoint"]["beam_enabled"] is True
        assert checkpoint["checkpoint"]["checkpoint_branch_policy"] in {
            "best_frontier_branch",
            "best_terminal_or_root_branch",
        }
        assert "frontier_summary" in checkpoint["checkpoint"]
        assert "terminal_summary" in checkpoint["checkpoint"]
        assert checkpoint["checkpoint"]["target_hit_classification"]["target_hit_success"] is False
        adapt_payload = checkpoint["adapt_vqe"]
        assert adapt_payload["partial_checkpoint"] is True
        assert adapt_payload["adapt_beam_enabled"] is True
        assert adapt_payload["benchmark_target_abs_delta_e_current"] is not None
        assert adapt_payload["benchmark_target_classification"]["target_hit_success"] is False
        assert adapt_payload["benchmark_target_hit_success"] is False
        assert adapt_payload["history_tail_count"] >= 1
        assert all("branch_id" in row and "parent_branch_id" in row for row in adapt_payload["history_tail"])
        assert adapt_payload["history_checkpoint_complete"] is True
        assert adapt_payload["history"] == adapt_payload["history_tail"]
        assert int(adapt_payload["nfev_total"]) > 0
        assert adapt_payload["controller_measurement_work_summary"][
            "beam_run_scope"
        ] == "winner_lineage_only"
        assert adapt_payload["resume_boundary_refit"]["nfev"] == 0
        assert adapt_payload["final_full_refit"]["nfev"] == 0
        algorithmic_work, query_audit = snake_algorithmic_work_from_payload(
            checkpoint,
            scope="terminal",
            source_label="beam_current_checkpoint_test",
        )
        assert algorithmic_work["S_alg_status"] == "ok", query_audit
        assert float(algorithmic_work["S_alg"]) > 0.0
        assert isinstance(checkpoint.get("initial_state"), dict)
        assert isinstance(checkpoint.get("ansatz_input_state"), dict)
        assert checkpoint["initial_state"]["handoff_state_kind"] == "prepared_state"
        ...
        replay = adapt_payload["beam_replay_telemetry"]
        assert replay["schema_version"] == "static_adapt_beam_replay_telemetry_v1"
        round_payload = replay["current_round"]
        assert round_payload["schema_version"] == "static_adapt_beam_replay_round_v1"
        assert "frontier_summary" in round_payload
        assert "terminal_summary" in round_payload
        assert replay["checkpoint_branch"]["branch_id"] == adapt_payload["branch_id"]
        assert replay["checkpoint_branch"]["ansatz_depth"] == adapt_payload["ansatz_depth"]
        if round_payload["frontier"]["branches"]:
            assert replay["checkpoint_branch_policy"] == "best_frontier_branch"
            assert adapt_payload["stop_reason"] is None
        else:
            assert replay["checkpoint_branch_policy"] == "best_terminal_or_root_branch"
        branches = (
            round_payload["frontier"]["branches"]
            + round_payload["terminal"]["branches"]
            + round_payload["terminal"]["round_branches"]
        )
        assert branches
        branch = next((row for row in branches if row.get("history_tail")), branches[0])
        assert "branch_id" in branch
        assert "parent_branch_id" in branch
        assert "energy" in branch
        assert branch["benchmark_target_abs_delta_current"] is not None
        assert "stop_reason" in branch
        assert "history_tail" in branch
        selected_records = list(branch.get("last_selected_records") or branch["history_tail"][-1]["selected_records"])
        assert selected_records
        selected = selected_records[0]
        assert selected["operator_label"]
        assert selected["generator_label"]
        assert "generator_id" in selected
        assert selected["position_id"] is not None

        assert all("branch_id" in row and "parent_branch_id" in row for row in payload["history"])

    def test_hubbard_phase3_beam_parallel_gradient_matches_serial(self, monkeypatch):
        defaults = {
            **self._hubbard_defaults(),
            "max_depth": 1,
            "maxiter": 20,
        ...
        assert payload_serial["operators"] == payload_parallel["operators"]
        assert [int(row["pool_index"]) for row in payload_serial.get("history", [])] == [
            int(row["pool_index"]) for row in payload_parallel.get("history", [])
        ]
        assert float(payload_parallel["energy"]) == pytest.approx(float(payload_serial["energy"]), abs=1e-12)
        assert all(not bool(row.get("gradient_parallel_enabled")) for row in payload_serial.get("history", []))
        assert any(bool(row.get("gradient_parallel_enabled")) for row in payload_parallel.get("history", []))

    def test_hubbard_phase3_beam_parent_parallel_matches_serial(self, monkeypatch):
        defaults = {
            **self._hubbard_defaults(),
            "max_depth": 2,
            "maxiter": 8,
        ...
        def _capture_ai_log(event: str, **kw: Any) -> None:
            events.append({"event": str(event), **dict(kw)})

        monkeypatch.setattr(_adapt_mod, "_ai_log", _capture_ai_log)
        ...
        assert payload_parallel["operators"] == payload_serial["operators"]
        assert [int(row["pool_index"]) for row in payload_parallel.get("history", [])] == [
            int(row["pool_index"]) for row in payload_serial.get("history", [])
        ]
        assert [str(row["selected_op"]) for row in payload_parallel.get("history", [])] == [
            str(row["selected_op"]) for row in payload_serial.get("history", [])
        ]
        assert float(payload_parallel["energy"]) == pytest.approx(float(payload_serial["energy"]), abs=1e-12)
        assert int(payload_parallel["nfev_total"]) == int(payload_serial["nfev_total"])

        serial_beam = payload_serial["continuation"]["beam_search"]
        parallel_beam = payload_parallel["continuation"]["beam_search"]
        for key in ("winner_branch_id", "winner_fingerprint", "winner_prune_key"):
            assert parallel_beam[key] == serial_beam[key]
        assert payload_serial["adapt_beam_parent_workers_requested"] == 1
        assert payload_parallel["adapt_beam_parent_workers_requested"] == 2
        assert payload_parallel["adapt_beam_parent_parallel_enabled_final"] is True
        assert any(
            bool(row.get("beam_parent_parallel_enabled"))
            for row in parallel_beam.get("rounds", [])
        )
        assert all(
            str(row.get("beam_parent_parallel_merge_order")) == "frontier_order"
            for row in parallel_beam.get("rounds", [])
            if bool(row.get("beam_parent_parallel_enabled"))
        )
        round_done_events = [
            row for row in events if row.get("event") == "hardcoded_adapt_beam_round_done"
        ]
        assert any(bool(row.get("beam_parent_parallel_enabled")) for row in round_done_events)
        assert any(int(row.get("beam_parent_workers_effective", 1)) == 2 for row in round_done_events)

    def test_hubbard_phase3_formula_true_beam_runs_on_generic_surface(self):
        defaults = {
            **self._hubbard_defaults(),
        ...
        continuation = payload["continuation"]
        assert continuation["mode"] == "phase3_v1"
        assert continuation["phase3_selector_policy"] == "hardware_resolvable_v1"
        assert continuation["hardware_resolution"]["schema"] == "gradient_resolution_v1"
        assert continuation["hardware_resolution"]["mode"] == "ideal"
        assert continuation["beam_search"]["beam_enabled"] is True
        assert continuation["beam_search"]["winner_branch_id"] is not None
        assert continuation["phase0_pilot"]["enabled"] is True
        assert continuation["phase0_pilot"]["max_records"] == 0
        assert continuation["phase0_last_pilot_rows"]
        assert continuation["selected_scaffold_summary"]["selection_source"] == "beam_winner"
        assert continuation["selected_scaffold_final_choice"]["beam_enabled"] is True
        assert continuation["selected_scaffold_branch_state"]["branch_id"] == int(
            continuation["beam_search"]["winner_branch_id"]
        )
        phase2_cfg = continuation["phase2"]
        assert phase2_cfg["canonical_score_formula"] == "DeltaE_TR * N3 / (1 + K3)"
        assert phase2_cfg["hardware_resolution_mode"] == "ideal"
        assert phase2_cfg["primary_selector_score_key"] == "full_v2_score"
        assert phase2_cfg["selector_tie_break_score_key"] == "phase3_tie_break_score"
        assert phase2_cfg["auxiliary_terms_primary_mode"] == "tie_break_only"
        surface = continuation["active_phase3_surface_summary"]
        assert surface["score_config"]["canonical_score_formula"] == phase2_cfg["canonical_score_formula"]
        assert surface["score_config"]["batch_target_size_requested"] == 8
        assert surface["score_config"]["batch_target_size_effective"] == 3
        assert surface["score_config"]["batch_size_cap_effective"] == 3
        scored_rows = continuation["phase2_scored_rows"]
        assert scored_rows
        assert scored_rows[0]["phase3_canonical_score_formula"] == phase2_cfg["canonical_score_formula"]
        assert "g_hw_lcb" in scored_rows[0]
        assert "epsilon_g_res" in scored_rows[0]
        assert scored_rows[0]["phase3_auxiliary_score_mode"] == "tie_break_only"
        assert "phase3_primary_score" in scored_rows[0]
        assert "phase3_tie_break_score" in scored_rows[0]

    def test_legacy_beam_archive_materializes_stop_child_for_every_parent(self):
        payload, _psi = _run_hardcoded_adapt_vqe(
            **{
                **self._hubbard_defaults(),
                "max_depth": 2,
        ...
        assert beam["terminal_archive_enabled"] is True
        assert beam["terminal_archive_semantics"] == (
            "materialize_parent_stop_and_accumulate_capped_v1"
        )
        assert beam["rounds"]
        assert all(
            int(row["stop_children_count"]) == int(row["parents_expanded_count"])
            for row in beam["rounds"]
        )
        assert all(
            0 < int(row["terminal_kept_count"]) <= 3 for row in beam["rounds"]
        )
        assert int(beam["terminal_final_count"]) <= 3
        assert payload["history"]
        assert all(
            row["beam_structural_mode"] == "stop_or_single_admission"
            and row["beam_parent_stop_terminal_also_materialized"] is True
            for row in payload["history"]
        )
