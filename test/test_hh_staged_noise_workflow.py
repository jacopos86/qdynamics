...
def test_resolve_noise_defaults_and_retagged_artifacts() -> None:
    ...
    assert str(cfg.noise.value_noise_model) == "off"
    assert float(cfg.noise.value_noise_std) == pytest.approx(0.0)
    assert cfg.noise.value_noise_seed is None
    assert str(cfg.noise.controller_value_noise_model) == "off"
    assert float(cfg.noise.controller_value_noise_std) == pytest.approx(0.0)
    assert cfg.noise.controller_value_noise_seed is None
...
def test_resolve_oracle_v1_accepts_aer_density_matrix_noise_mode() -> None:
    cfg = noise_wf.resolve_staged_hh_noise_config(
        parse_args(
            [
                "--L",
    ...
    assert cfg.noise.modes == ("aer_density_matrix",)
    assert cfg.noise.audit_modes == ("aer_density_matrix",)
    assert str(cfg.noise.controller_noise_mode) == "aer_density_matrix"


def test_resolve_value_noise_pass_through_for_staged_phase3_noise_and_controller() -> None:
    cfg = noise_wf.resolve_staged_hh_noise_config(
        parse_args(
            [
                "--L",
    ...
    assert cfg.noise.modes == ("aer_density_matrix",)
    assert str(cfg.noise.value_noise_model) == "gaussian_iid_v1"
    assert float(cfg.noise.value_noise_std) == pytest.approx(1.0e-4)
    assert cfg.noise.value_noise_seed == 20260514
    assert str(cfg.noise.controller_value_noise_model) == "gaussian_iid_v1"
    assert float(cfg.noise.controller_value_noise_std) == pytest.approx(2.0e-4)
    assert cfg.noise.controller_value_noise_seed == 20260515
    assert cfg.staged.adapt.phase3_oracle_gradient_config is not None
    phase3_cfg = cfg.staged.adapt.phase3_oracle_gradient_config
    assert str(phase3_cfg.value_noise_model) == "gaussian_iid_v1"
    assert float(phase3_cfg.value_noise_std) == pytest.approx(3.0e-4)
    assert phase3_cfg.value_noise_seed == 20260516

    controller_cfg = noise_wf._controller_oracle_config_from_noise_cfg(cfg.noise)
    assert str(controller_cfg.value_noise_model) == "gaussian_iid_v1"
    assert float(controller_cfg.value_noise_std) == pytest.approx(2.0e-4)
    assert controller_cfg.value_noise_seed == 20260515


def test_staged_value_noise_forwards_to_imported_audit_kwargs(
    ...
    artifact_json = tmp_path / "import.json"
    artifact_json.write_text("{}", encoding="utf-8")
    source_cfg = noise_wf.StagedNoiseSourceConfig(
        mode="imported_artifact",
        requested_json=artifact_json,
        resolved_json=artifact_json,
        source_kind="unit_test",
    )
    seen: dict[str, dict[str, object]] = {}

    def _capture(label: str):
        def _impl(*, kwargs: dict[str, object], timeout_s: int) -> dict[str, object]:
            seen[label] = dict(kwargs)
            return {"success": True, "label": label, "timeout_s": int(timeout_s)}

        return _impl

    monkeypatch.setattr(
        noise_wf.noise_report,
        "_run_imported_prepared_state_audit_mode_isolated",
        _capture("prepared"),
    )
    monkeypatch.setattr(
        noise_wf.noise_report,
        "_run_imported_ansatz_input_state_audit_mode_isolated",
        _capture("ansatz_input"),
    )
    monkeypatch.setattr(
        noise_wf.noise_report,
        "_run_imported_full_circuit_audit_mode_isolated",
        _capture("full_circuit"),
    )
    monkeypatch.setattr(
        noise_wf.noise_report,
        "_run_imported_fixed_lean_noisy_replay_mode_isolated",
        _capture("fixed_lean"),
    )

    noise_wf._run_imported_prepared_state_audit_mode(
        source_cfg=source_cfg,
        noise_cfg=cfg.noise,
        mode="aer_density_matrix",
    )
    noise_wf._run_imported_ansatz_input_state_audit_mode(
        source_cfg=source_cfg,
        noise_cfg=cfg.noise,
        mode="aer_density_matrix",
    )
    noise_wf._run_imported_full_circuit_audit_mode(
        source_cfg=source_cfg,
        noise_cfg=cfg.noise,
        mode="aer_density_matrix",
    )
    noise_wf._run_fixed_lean_noisy_replay_mode(
        source_cfg=source_cfg,
        noise_cfg=cfg.noise,
        fixed_cfg=replace(cfg.fixed_lean_replay, enabled=True),
    )

    assert set(seen) == {"prepared", "ansatz_input", "full_circuit", "fixed_lean"}
    for kwargs in seen.values():
        assert kwargs["value_noise_model"] == "gaussian_iid_v1"
        assert kwargs["value_noise_std"] == pytest.approx(1.0e-4)
        assert kwargs["value_noise_seed"] == 20260514


def test_resolve_value_noise_rejects_phase3_without_phase3_oracle_mode() -> None:
    with pytest.raises(ValueError, match="phase3-oracle-value-noise-model"):
        noise_wf.resolve_staged_hh_noise_config(
            parse_args(
                [
                    "--L",
