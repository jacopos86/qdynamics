...
def test_failed_append_reuse_skips_unchanged_failed_search() -> None:
    runtime_input = _runtime_input_with_candidates(
        AnsatzTerm(label="candidate_x", polynomial=_poly("x")),
        AnsatzTerm(label="candidate_y", polynomial=_poly("y")),
        hamiltonian=_poly_sum("x", "y"),
    ...
    payload = run_append_ap_mclachlan_from_runtime_input(
        ...
        support_patch_config=SupportPatchControllerConfig(
            ...
            residual_ratio_threshold=0.0,
            cost_required_for_decisions=False,
            failed_append_reuse_enabled=True,
        ),
    )

    assert payload["summary"]["accepted_append_count"] == 0
    assert payload["summary"]["failed_append_reuse_full_search_count"] == 1
    assert payload["summary"]["failed_append_reuse_skip_count"] == 1
    assert payload["summary"]["failed_append_reuse_certificate_created_count"] == 1
    assert payload["summary"]["failed_append_reuse_route_counts"] == {
        "no_certificate": 1,
        "skipped": 1,
    }
    first, second = payload["plot_rows"][:2]
    assert first["patch_reason"] == "append_gain_below_threshold"
    assert first["patch_scored_count"] == 3
    assert first["failed_append_reuse_certificate_created"] is True
    assert second["patch_reason"] == "failed_append_reuse_skip"
    assert second["patch_scored_count"] == 0
    assert second["failed_append_reuse_full_search_run"] is False
    assert second["failed_append_reuse_reopen_route"] == "skipped"


def test_failed_append_reuse_direct_threshold_reopens_search() -> None:
    runtime_input = _runtime_input_with_candidates(
        AnsatzTerm(label="candidate_x", polynomial=_poly("x")),
        AnsatzTerm(label="candidate_y", polynomial=_poly("y")),
        hamiltonian=_poly_sum("x", "y"),
    ...
    payload = run_append_ap_mclachlan_from_runtime_input(
        ...
        support_patch_config=SupportPatchControllerConfig(
            ...
            cost_required_for_decisions=False,
            failed_append_reuse_enabled=True,
            failed_append_reuse_tau_min=0.0,
            failed_append_reuse_tau_max=0.0,
        ),
    )

    assert payload["summary"]["failed_append_reuse_full_search_count"] == 2
    assert payload["summary"]["failed_append_reuse_skip_count"] == 0
    assert payload["summary"]["failed_append_reuse_route_counts"] == {
        "no_certificate": 1,
        "direct": 1,
    }
    assert payload["plot_rows"][1]["patch_reason"] == "append_gain_below_threshold"
    assert payload["plot_rows"][1]["patch_scored_count"] == 3
    assert payload["plot_rows"][1]["failed_append_reuse_reopen_route"] == "direct"


def test_failed_append_reuse_model_change_falls_back_to_direct_when_no_scale() -> None:
    runtime_input = _runtime_input_with_candidates(
        AnsatzTerm(label="candidate_x", polynomial=_poly("x")),
        AnsatzTerm(label="candidate_y", polynomial=_poly("y")),
        hamiltonian=_poly_sum("x", "y"),
    ...
    payload = run_append_ap_mclachlan_from_runtime_input(
        ...
        support_patch_config=SupportPatchControllerConfig(
            ...
            cost_required_for_decisions=False,
            failed_append_reuse_enabled=True,
            failed_append_reuse_reopen_mode=FAILED_APPEND_REOPEN_MODEL_CHANGE,
            failed_append_reuse_tau_min=0.0,
            failed_append_reuse_tau_max=0.0,
        ),
    )
    ...
    assert payload["plot_rows"][1]["failed_append_reuse_reopen_route"] == "fallback_direct"
    assert payload["plot_rows"][1]["failed_append_reuse_full_search_run"] is True


def test_failed_append_reuse_does_not_cap_successive_appends() -> None:
    runtime_input = _runtime_input_with_candidates(
        AnsatzTerm(label="candidate_x", polynomial=_poly("x")),
        AnsatzTerm(label="candidate_y", polynomial=_poly("y")),
        AnsatzTerm(label="candidate_z", polynomial=_poly("z")),
        hamiltonian=_poly_sum("x", "y", "z"),
    )
    ...
    assert payload["summary"]["accepted_append_count"] == 3
    assert payload["summary"]["runtime_parameter_count_final"] == 3
    assert payload["summary"]["failed_append_reuse_skip_count"] == 0
    assert [row["patch_accepted"] for row in payload["plot_rows"][:3]] == [
        True,
        True,
        True,
    ]
