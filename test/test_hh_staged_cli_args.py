...
def test_staged_parser_default_inner_optimizer_stays_spsa() -> None:
    args = parse_args(["--L", "2", "--skip-pdf"])
    assert str(args.adapt_inner_optimizer) == "SPSA"


def test_staged_parser_accepts_qnspsa_inner_optimizer() -> None:
    args = parse_args(["--L", "2", "--skip-pdf", "--adapt-inner-optimizer", "QNSPSA"])
    assert str(args.adapt_inner_optimizer) == "QNSPSA"


def test_staged_parser_accepts_rotosolve_inner_optimizer() -> None:
    args = parse_args(["--L", "2", "--skip-pdf", "--adapt-inner-optimizer", "ROTOSOLVE"])
    assert str(args.adapt_inner_optimizer) == "ROTOSOLVE"


def test_staged_noise_parser_accepts_qnspsa_inner_optimizer() -> None:
    parser = build_staged_hh_parser(description="test staged noise parser", include_noise=True)
    args = parser.parse_args(["--L", "2", "--skip-pdf", "--adapt-inner-optimizer", "QNSPSA"])
    assert str(args.adapt_inner_optimizer) == "QNSPSA"
...
def test_staged_parser_accepts_phase3_oracle_density_matrix_value_noise_args() -> None:
    args = parse_args(
        [
            "--L",
            "2",
    ...
    assert str(args.phase3_oracle_gradient_mode) == "aer_density_matrix"
    assert str(args.phase3_oracle_value_noise_model) == "gaussian_iid_v1"
    assert float(args.phase3_oracle_value_noise_std) == pytest.approx(0.125)
    assert int(args.phase3_oracle_value_noise_seed) == 19


def test_staged_noise_parser_accepts_value_noise_args() -> None:
    parser = build_staged_hh_parser(description="test staged noise parser", include_noise=True)
    ...
    assert str(args.noise_value_noise_model) == "gaussian_iid_v1"
    assert float(args.noise_value_noise_std) == pytest.approx(0.25)
    assert int(args.noise_value_noise_seed) == 23
    assert str(args.checkpoint_controller_noise_mode) == "aer_density_matrix"
    assert str(args.checkpoint_controller_value_noise_model) == "gaussian_iid_v1"
    assert float(args.checkpoint_controller_value_noise_std) == pytest.approx(0.375)
    assert int(args.checkpoint_controller_value_noise_seed) == 29
...
def test_checkpoint_controller_prune_mode_accepts_schur_projected_shadow_v1() -> None:
    args = parse_args(
        [
            "--L", "2", "--skip-pdf",
            "--checkpoint-controller-prune-mode", "schur_projected_shadow_v1",
            "--checkpoint-controller-prune-differential-miss-tol", "0.02",
            "--checkpoint-controller-prune-projection-rounds", "2",
            "--checkpoint-controller-prune-persistence-window", "3",
            "--checkpoint-controller-prune-persistence-required", "2",
        ]
    )
    assert str(args.checkpoint_controller_prune_mode) == "schur_projected_shadow_v1"
    assert float(args.checkpoint_controller_prune_differential_miss_tol) == 0.02
    assert int(args.checkpoint_controller_prune_projection_rounds) == 2
    assert int(args.checkpoint_controller_prune_persistence_window) == 3
    assert int(args.checkpoint_controller_prune_persistence_required) == 2
