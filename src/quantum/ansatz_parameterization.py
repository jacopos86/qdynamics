...
_SUPPORTED_EXECUTION_MODES = frozenset({"termwise_product", "grouped_exact"})


def _normalize_execution_mode(value: Any) -> str:
    mode = "termwise_product" if value is None else str(value).strip().lower()
    if mode not in _SUPPORTED_EXECUTION_MODES:
        raise ValueError(f"Unsupported ansatz generator execution mode: {value!r}.")
    return mode
...
def project_runtime_theta_block_mean(
    ...
    for block in layout.blocks:
        ...
        value_bits = vals.view(np.uint64)
        out[block.logical_index] = float(
            vals[0]
            if np.all(value_bits == value_bits[0])
            else np.mean(vals)
        )
    return out
...
def deserialize_layout(payload: Mapping[str, Any]) -> AnsatzParameterLayout:
    ...
    for logical_index, raw_block in enumerate(blocks_raw):
        ...
        block = GeneratorParameterBlock(
            ...
            terms=tuple(terms),
            execution_mode=_normalize_execution_mode(
                raw_block.get("execution_mode", "termwise_product")
            ),
        )
