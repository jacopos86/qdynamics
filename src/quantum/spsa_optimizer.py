...
def _aggregate_values(values: Sequence[float], *, eval_agg: str) -> float:
...
def _aggregate_eval(
    ...
    *,
    ...
    for rep in range(int(eval_repeats)):
        ...
        nfev_counter[0] += 1
    return _aggregate_values(vals, eval_agg=str(eval_agg))


def _aggregate_eval_batch(
    fun: Callable[[np.ndarray], float],
    ...
    *,
    ...
    if executor is None or (len(point_arrays) * repeats) <= 1:
        return [
            _aggregate_eval(
                fun,
                point,
    ...
    for idx, point in enumerate(point_arrays):
        for _rep in range(repeats):
            tasks.append((int(idx), np.array(point, copy=True)))

    def _eval_task(task: tuple[int, np.ndarray]) -> tuple[int, float]:
        idx, point = task
        return int(idx), float(fun(np.asarray(point, dtype=float)))

    values_by_point: list[list[float]] = [[] for _ in point_arrays]
    for idx, value in executor.map(_eval_task, tasks):
        values_by_point[int(idx)].append(float(value))
    nfev_counter[0] += int(len(tasks))
    return [_aggregate_values(values, eval_agg=str(eval_agg)) for values in values_by_point]
...
def spsa_minimize(
    ...
    *,
    ...
    parallel_eval_workers = int(max(1, int(parallel_evaluations)))
    parallel_eval_executor = (
        ThreadPoolExecutor(max_workers=int(parallel_eval_workers))
        if int(parallel_eval_workers) > 1
        else None
    )
    ...
    best_y_observed = _aggregate_eval_batch(
        fun,
        [x_current],
        eval_repeats=int(eval_repeats),
        eval_agg=str(eval_agg),
        nfev_counter=nfev_counter,
        executor=parallel_eval_executor,
    )[0]
    nit = 0
    ...
    try:
        for k in range(int(maxiter)):
            ...
            y_plus, y_minus = _aggregate_eval_batch(
                fun,
                [x_plus, x_minus],
                eval_repeats=int(eval_repeats),
                eval_agg=str(eval_agg),
        ...
        return SPSAResult(
            ...
            message=(
                ...
                f"parallel_evaluations={int(parallel_eval_workers)})"
            ),
            history=history,
            optimizer_memory={
                ...
                "parallel_evaluations": int(parallel_eval_workers),
        ...
        if parallel_eval_executor is not None:
            parallel_eval_executor.shutdown(wait=True)
