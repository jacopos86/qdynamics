"""
Reference exact diagonalization (ED) for the Hubbard-Holstein model.

This module builds the HH Hamiltonian directly in the occupation-number
basis, without Pauli algebra. It is intended as an independent oracle for
unit tests that validate the Pauli-polynomial qubit encoding.
"""
...
from src.qubitization.hubbard_holstein_pauli_builder import (
    Dims,
    SPIN_DN,
    SPIN_UP,
...
class HHPhysicalState:
    """A physical basis state: fermion occupation plus phonon occupations."""
...
@dataclass
class HHEDBasis:
    """Sector-restricted basis for Hubbard-Holstein exact diagonalization."""
...
def _boson_qubits_per_site(n_ph_max: int, encoding: str = "binary") -> int:
    n_ph_i = int(n_ph_max)
    if n_ph_i < 0:
    ...
    if encoding == "unary":
        return n_ph_i + 1
    raise ValueError(f"Unknown boson encoding '{encoding}'")
...
def _spin_orbital_index_sets(
    ...
    up = [
        mode_index(i, SPIN_UP, indexing=indexing, n_sites=n_sites_i)
        for i in range(n_sites_i)
    ]
    dn = [
        mode_index(i, SPIN_DN, indexing=indexing, n_sites=n_sites_i)
        for i in range(n_sites_i)
    ]
    return up, dn
...
def encode_state_to_qubit_index(
    *,
    ...
    """Map a physical state to a computational-basis index."""
...
def build_hh_sector_basis(
    *,
    ...
    qpb = int(_boson_qubits_per_site(n_ph_i, enc))
    ...
    for f_bits in fermion_bits:
        for ph in phonon_configs:
            ...
            packed.append(
                (comp_idx, HHPhysicalState(fermion_bits=int(f_bits), phonons=ph_t))
            )
...
def _apply_cdag_c(
    ...
    """Apply c_p^dagger c_q to a fermion bitstring."""
...
def build_hh_sector_hamiltonian_ed(
    *,
    ...
    """Build the HH Hamiltonian directly in the occupation-number basis."""
