...
class OracleConfig:
    noise_mode: str = "ideal"  # ideal | shots | aer_noise | aer_density_matrix | aer_density_matrix_synthetic_depolarizing | aer_density_matrix_synthetic_coherent | runtime | backend_scheduled
    shots: int = 2048
    seed: int = 7
    ...
    value_noise_semantic: str = "post_expectation_value_noise_not_physical_shots"
...
class NormalizedOracleSpec:
    ...
    def to_oracle_config(self) -> OracleConfig:
        return OracleConfig(
            ...
            value_noise_model=str(self.value_noise_model),
            value_noise_std=float(self.value_noise_std),
            value_noise_seed=(None if self.value_noise_seed is None else int(self.value_noise_seed)),
            value_noise_sigma0_abs=(
                None if self.value_noise_sigma0_abs is None else float(self.value_noise_sigma0_abs)
            ),
            value_noise_n_eff=(
                None if self.value_noise_n_eff is None else float(self.value_noise_n_eff)
            ),
            value_noise_semantic=str(self.value_noise_semantic),
            value_noise_std_source=str(self.value_noise_std_source),
            synthetic_depolarizing_1q_error=float(self.synthetic_depolarizing_1q_error),
            synthetic_depolarizing_2q_error=float(self.synthetic_depolarizing_2q_error),
            synthetic_depolarizing_1q_gates=tuple(
                str(g) for g in tuple(self.synthetic_depolarizing_1q_gates)
            ),
            synthetic_depolarizing_2q_gates=tuple(
                str(g) for g in tuple(self.synthetic_depolarizing_2q_gates)
            ),
            synthetic_coherent_1q_angle_std=float(self.synthetic_coherent_1q_angle_std),
            synthetic_coherent_2q_angle_std=float(self.synthetic_coherent_2q_angle_std),
            synthetic_coherent_seed=(
                None if self.synthetic_coherent_seed is None else int(self.synthetic_coherent_seed)
            ),
            synthetic_coherent_generator_mode=str(self.synthetic_coherent_generator_mode),
            synthetic_coherent_1q_gates=tuple(str(g) for g in tuple(self.synthetic_coherent_1q_gates)),
            synthetic_coherent_2q_gates=tuple(str(g) for g in tuple(self.synthetic_coherent_2q_gates)),
        )
...
def _normalize_gate_name_tuple(
    ...
    *,
    default: tuple[str, ...],
    field_name: str = "oracle gate list",
) -> tuple[str, ...]:
    return normalize_gate_name_tuple(value, default=default, field_name=field_name)
...
def _normalized_oracle_request_payload(spec: NormalizedOracleSpec) -> dict[str, Any]:
    return {
        ...
        "value_noise_model": str(spec.value_noise_model),
        "value_noise_std": float(spec.value_noise_std),
        "value_noise_seed": (None if spec.value_noise_seed is None else int(spec.value_noise_seed)),
        "value_noise_sigma0_abs": (
            None if spec.value_noise_sigma0_abs is None else float(spec.value_noise_sigma0_abs)
        ),
        "value_noise_n_eff": (
            None if spec.value_noise_n_eff is None else float(spec.value_noise_n_eff)
        ),
        "value_noise_semantic": str(spec.value_noise_semantic),
        "value_noise_std_source": str(spec.value_noise_std_source),
        "synthetic_depolarizing_1q_error": float(spec.synthetic_depolarizing_1q_error),
        "synthetic_depolarizing_2q_error": float(spec.synthetic_depolarizing_2q_error),
        "synthetic_depolarizing_1q_gates": [
            str(g) for g in tuple(spec.synthetic_depolarizing_1q_gates)
        ],
        "synthetic_depolarizing_2q_gates": [
            str(g) for g in tuple(spec.synthetic_depolarizing_2q_gates)
        ],
        "synthetic_coherent_1q_angle_std": float(spec.synthetic_coherent_1q_angle_std),
        "synthetic_coherent_2q_angle_std": float(spec.synthetic_coherent_2q_angle_std),
        "synthetic_coherent_seed": (
            None if spec.synthetic_coherent_seed is None else int(spec.synthetic_coherent_seed)
        ),
        "synthetic_coherent_generator_mode": str(spec.synthetic_coherent_generator_mode),
        "synthetic_coherent_1q_gates": [str(g) for g in tuple(spec.synthetic_coherent_1q_gates)],
        "synthetic_coherent_2q_gates": [str(g) for g in tuple(spec.synthetic_coherent_2q_gates)],
    }
...
def normalize_oracle_execution_request(config: Any) -> dict[str, Any]:
    spec = NormalizedOracleSpec(
        ...
        value_noise_seed=(
            None
            if getattr(config, "value_noise_seed", None) in {None, ""}
            else int(getattr(config, "value_noise_seed"))
        ),
        value_noise_sigma0_abs=(
            None
            if getattr(config, "value_noise_sigma0_abs", None) in {None, ""}
            else float(getattr(config, "value_noise_sigma0_abs"))
        ),
        value_noise_n_eff=(
            None
            if getattr(config, "value_noise_n_eff", None) in {None, ""}
            else float(getattr(config, "value_noise_n_eff"))
        ),
        value_noise_semantic=str(
            getattr(config, "value_noise_semantic", _VALUE_NOISE_SEMANTIC)
        ),
        ...
        synthetic_depolarizing_1q_gates=_normalize_gate_name_tuple(
            getattr(
                config,
                "synthetic_depolarizing_1q_gates",
                SYNTHETIC_DEPOLARIZING_1Q_GATES_DEFAULT,
        ...
        synthetic_depolarizing_2q_gates=_normalize_gate_name_tuple(
            getattr(config, "synthetic_depolarizing_2q_gates", SYNTHETIC_DEPOLARIZING_2Q_GATES_DEFAULT),
        ...
        synthetic_coherent_seed=(
            None
            if getattr(config, "synthetic_coherent_seed", None) in {None, ""}
        ...
        or SYNTHETIC_COHERENT_GENERATOR_MODE_DEFAULT,
        synthetic_coherent_1q_gates=_normalize_gate_name_tuple(
        ...
        synthetic_coherent_2q_gates=_normalize_gate_name_tuple(
...
def assess_oracle_execution_capability(config: Any) -> dict[str, Any]:
    ...
    if spec.noise_mode not in {
        ...
        return _report(
            ...
            reason=(
                ...
                "{'ideal','shots','aer_noise','aer_density_matrix',"
                "'aer_density_matrix_synthetic_depolarizing','aer_density_matrix_synthetic_coherent',"
                "'runtime','backend_scheduled'}."
            ),
        )
    if int(spec.shots) < 1:
        return _report(
    ...
    value_noise_model = str(spec.value_noise_model).strip().lower() or "off"
    value_noise_std = float(spec.value_noise_std)
    if value_noise_model not in _VALUE_NOISE_MODELS:
        return _report(
            supported=False,
            reason_code="unsupported_value_noise_model",
            reason=(
                f"Unsupported value_noise_model {value_noise_model!r}; "
                f"expected one of {sorted(_VALUE_NOISE_MODELS)}."
            ),
        )
    if value_noise_model == "off" and float(value_noise_std) != 0.0:
        return _report(
            supported=False,
            reason_code="invalid_value_noise_off_std",
            reason="value_noise_model='off' requires value_noise_std == 0.",
        )
    if value_noise_model == "gaussian_iid_v1":
        if (not np.isfinite(value_noise_std)) or float(value_noise_std) <= 0.0:
            return _report(
                supported=False,
                reason_code="invalid_value_noise_std",
                reason="value_noise_model='gaussian_iid_v1' requires finite value_noise_std > 0.",
            )
        if str(spec.execution_surface) == "raw_measurement_v1":
            return _report(
                supported=False,
                reason_code="value_noise_raw_measurement_unsupported",
                reason="value_noise_raw_measurement_unsupported: value_noise_model='gaussian_iid_v1' supports only execution_surface='expectation_v1'.",
            )
    ...
    if str(spec.execution_surface) == "raw_measurement_v1":
        ...
        if str(spec.noise_mode) == "runtime":
            ...
            for raw_profile_cfg in (spec.runtime_profile, spec.runtime_raw_profile):
                try:
                    normalize_runtime_raw_sampler_profile_config(raw_profile_cfg)
    ...
    if str(spec.noise_mode) in {
        ...
        if (
            ...
            return _report(
                supported=False,
                reason_code="aer_density_matrix_mitigation_unsupported",
                reason=(
                    "aer_density_matrix is a shotless density-matrix expectation mode and "
                    "requires mitigation_mode='none' with local ZNE/twirling/DD disabled."
                ),
            )
        if symmetry_mode not in {"off", "verify_only"}:
            return _report(
                supported=False,
                reason_code="aer_density_matrix_symmetry_unsupported",
            reason="aer_density_matrix supports symmetry_mitigation only in {'off','verify_only'}.",
            )
        if str(spec.noise_mode) in {
            "aer_density_matrix_synthetic_depolarizing",
            "aer_density_matrix_synthetic_coherent",
        }:
            p1q = float(spec.synthetic_depolarizing_1q_error)
            p2q = float(spec.synthetic_depolarizing_2q_error)
            if (
                not np.isfinite(p1q)
                or not np.isfinite(p2q)
                or p1q < 0.0
                or p1q > 1.0
                or p2q < 0.0
                or p2q > 1.0
                ...
                return _report(
                    supported=False,
                    reason_code="invalid_synthetic_depolarizing_error",
                    reason=(
                        "synthetic density-matrix noise requires "
                        "0 <= synthetic_depolarizing_{1q,2q}_error <= 1."
                    ),
                )
        if str(spec.noise_mode) == "aer_density_matrix_synthetic_coherent":
            s1q = float(spec.synthetic_coherent_1q_angle_std)
            s2q = float(spec.synthetic_coherent_2q_angle_std)
            if (not np.isfinite(s1q)) or (not np.isfinite(s2q)) or s1q < 0.0 or s2q < 0.0:
                return _report(
                    supported=False,
                    reason_code="invalid_synthetic_coherent_angle_std",
                    reason="aer_density_matrix_synthetic_coherent requires nonnegative finite coherent angle std values.",
                )
            if s1q > 0.0 and not tuple(spec.synthetic_coherent_1q_gates):
                return _report(
                    supported=False,
                    reason_code="missing_synthetic_coherent_1q_gates",
                    reason="synthetic_coherent_1q_gates must be non-empty when 1q angle std > 0.",
                )
            if s2q > 0.0 and not tuple(spec.synthetic_coherent_2q_gates):
                return _report(
                    supported=False,
                    reason_code="missing_synthetic_coherent_2q_gates",
                    reason="synthetic_coherent_2q_gates must be non-empty when 2q angle std > 0.",
                )
...
def normalize_sampler_raw_runtime_config(config: OracleConfig) -> OracleConfig:
    normalized = OracleConfig(
        ...
        value_noise_semantic=str(getattr(config, "value_noise_semantic", "post_expectation_value_noise_not_physical_shots")),
        value_noise_std_source=str(getattr(config, "value_noise_std_source", "explicit_std")),
    )
...
_AER_DENSITY_MATRIX_MAX_QUBITS = 12
_AER_DENSITY_MATRIX_IMAG_TOL = 1e-8
_AER_DENSITY_MATRIX_PREFLIGHT_OK_CACHE: set[tuple[int | None, bool]] = set()
...
def _validate_aer_density_matrix_config(cfg: OracleConfig) -> None:
    ...
    noise_mode = str(getattr(cfg, "noise_mode", "aer_density_matrix")).strip().lower()
    execution_surface = str(getattr(cfg, "execution_surface", "expectation_v1")).strip().lower()
    if execution_surface != "expectation_v1":
        raise ValueError("aer_density_matrix requires execution_surface='expectation_v1'.")
    if (
        str(mitigation_cfg.get("mode", "none")) != "none"
        or list(mitigation_cfg.get("zne_scales", []) or [])
        or mitigation_cfg.get("dd_sequence", None) not in {None, "", "none"}
        or bool(mitigation_cfg.get("local_gate_twirling", False))
    ):
        raise ValueError(
            "aer_density_matrix requires mitigation_mode='none' with local ZNE/twirling/DD disabled."
        )
    if str(symmetry_cfg.get("mode", "off")) not in {"off", "verify_only"}:
        raise ValueError(
            "aer_density_matrix supports symmetry_mitigation only in {'off','verify_only'}."
        )
    if noise_mode in {
        "aer_density_matrix_synthetic_depolarizing",
        "aer_density_matrix_synthetic_coherent",
    }:
        _validate_synthetic_depolarizing_config(cfg)
    if noise_mode == "aer_density_matrix_synthetic_coherent":
        _validate_synthetic_coherent_config(cfg)


def _validate_synthetic_depolarizing_config(cfg: OracleConfig) -> None:
    p1q = float(getattr(cfg, "synthetic_depolarizing_1q_error", 0.0))
    p2q = float(getattr(cfg, "synthetic_depolarizing_2q_error", 0.0))
    if not np.isfinite(p1q) or not np.isfinite(p2q):
        raise ValueError("synthetic depolarizing errors must be finite.")
    if p1q < 0.0 or p1q > 1.0 or p2q < 0.0 or p2q > 1.0:
        raise ValueError("synthetic depolarizing errors must satisfy 0 <= p <= 1.")
    oneq_gates = tuple(getattr(cfg, "synthetic_depolarizing_1q_gates", ()) or ())
    twoq_gates = tuple(getattr(cfg, "synthetic_depolarizing_2q_gates", ()) or ())
    if p1q > 0.0 and not oneq_gates:
        raise ValueError("synthetic_depolarizing_1q_gates must be non-empty when p1q > 0.")
    if p2q > 0.0 and not twoq_gates:
        raise ValueError("synthetic_depolarizing_2q_gates must be non-empty when p2q > 0.")


def _synthetic_depolarizing_config_payload(cfg: OracleConfig) -> dict[str, Any]:
    return {
        "model": "depolarizing_v1",
        "one_qubit_error": float(getattr(cfg, "synthetic_depolarizing_1q_error", 0.0)),
        "two_qubit_error": float(getattr(cfg, "synthetic_depolarizing_2q_error", 0.0)),
        "one_qubit_gates": [
            str(g) for g in tuple(getattr(cfg, "synthetic_depolarizing_1q_gates", ()) or ())
        ],
        "two_qubit_gates": [
            str(g) for g in tuple(getattr(cfg, "synthetic_depolarizing_2q_gates", ()) or ())
        ],
        "implementation": "qiskit_aer.noise.depolarizing_error",
    }


def _build_synthetic_depolarizing_noise_model(cfg: OracleConfig) -> Any:
    _validate_synthetic_depolarizing_config(cfg)
    try:
        from qiskit_aer.noise import NoiseModel, depolarizing_error
    except Exception as exc:  # pragma: no cover - import error path
        raise RuntimeError(
            "aer_density_matrix_synthetic_depolarizing requires qiskit-aer.noise support."
        ) from exc
    ...
    if p1q > 0.0:
        noise_model.add_all_qubit_quantum_error(depolarizing_error(p1q, 1), oneq_gates)
    if p2q > 0.0:
        noise_model.add_all_qubit_quantum_error(depolarizing_error(p2q, 2), twoq_gates)
    return noise_model


def _validate_synthetic_coherent_config(cfg: OracleConfig) -> None:
    s1q = float(getattr(cfg, "synthetic_coherent_1q_angle_std", 0.0))
    s2q = float(getattr(cfg, "synthetic_coherent_2q_angle_std", 0.0))
    if (not np.isfinite(s1q)) or (not np.isfinite(s2q)) or s1q < 0.0 or s2q < 0.0:
        raise ValueError("synthetic coherent angle std values must be nonnegative and finite.")
    oneq_gates = tuple(getattr(cfg, "synthetic_coherent_1q_gates", ()) or ())
    twoq_gates = tuple(getattr(cfg, "synthetic_coherent_2q_gates", ()) or ())
    if s1q > 0.0 and not oneq_gates:
        raise ValueError("synthetic_coherent_1q_gates must be non-empty when 1q angle std > 0.")
    if s2q > 0.0 and not twoq_gates:
        raise ValueError("synthetic_coherent_2q_gates must be non-empty when 2q angle std > 0.")
    mode = str(getattr(cfg, "synthetic_coherent_generator_mode", SYNTHETIC_COHERENT_GENERATOR_MODE_DEFAULT))
    if mode.strip().lower() != SYNTHETIC_COHERENT_GENERATOR_MODE_DEFAULT:
        raise ValueError(
            "Only synthetic coherent generator mode "
            f"{SYNTHETIC_COHERENT_GENERATOR_MODE_DEFAULT!r} is currently supported."
        )


def _single_qubit_pauli_matrix(label: str) -> np.ndarray:
    key = str(label).upper()
    if key == "I":
...
def _pauli_rotation_unitary(generator_ixyz: str, delta: float) -> np.ndarray:
    label = str(generator_ixyz).upper()
    mat = np.array([[1.0 + 0.0j]])
    for ch in label:
        mat = np.kron(mat, _single_qubit_pauli_matrix(ch))
    dim = int(mat.shape[0])
    return np.cos(float(delta) / 2.0) * np.eye(dim, dtype=complex) - 1.0j * np.sin(float(delta) / 2.0) * mat


def _synthetic_coherent_config_payload(cfg: OracleConfig) -> dict[str, Any]:
    return {
        "model": "frozen_gate_local_pauli_overrotation_v1",
        "one_qubit_angle_std": float(getattr(cfg, "synthetic_coherent_1q_angle_std", 0.0)),
        "two_qubit_angle_std": float(getattr(cfg, "synthetic_coherent_2q_angle_std", 0.0)),
        "seed": (
            None if getattr(cfg, "synthetic_coherent_seed", None) is None else int(getattr(cfg, "synthetic_coherent_seed"))
        ),
        "generator_mode": str(getattr(cfg, "synthetic_coherent_generator_mode", SYNTHETIC_COHERENT_GENERATOR_MODE_DEFAULT)),
        "one_qubit_gates": [str(g) for g in tuple(getattr(cfg, "synthetic_coherent_1q_gates", ()) or ())],
        "two_qubit_gates": [str(g) for g in tuple(getattr(cfg, "synthetic_coherent_2q_gates", ()) or ())],
        "implementation": "insert_unitary_gate_after_configured_gate_locations",
    }


def _insert_synthetic_coherent_overrotations(
    circuit: QuantumCircuit,
    cfg: OracleConfig,
    ...
    seed = int(
        getattr(cfg, "seed", 7)
        if getattr(cfg, "synthetic_coherent_seed", None) is None
        else getattr(cfg, "synthetic_coherent_seed")
    )
    rng = np.random.default_rng(seed)
    oneq_gates = {str(g).strip().lower() for g in tuple(getattr(cfg, "synthetic_coherent_1q_gates", ()) or ())}
    twoq_gates = {str(g).strip().lower() for g in tuple(getattr(cfg, "synthetic_coherent_2q_gates", ()) or ())}
    oneq_generators = ("X", "Y", "Z")
    twoq_generators = tuple(
        a + b for a in ("I", "X", "Y", "Z") for b in ("I", "X", "Y", "Z") if a + b != "II"
    )
    out = QuantumCircuit(int(circuit.num_qubits), int(circuit.num_clbits), name=f"{circuit.name}_coh" if circuit.name else None)
    inserted: list[dict[str, Any]] = []
    seen_gate_counts: dict[str, int] = {}
    seen_gate_arity_counts: dict[str, int] = {}
    location_index = 0
    for instruction in circuit.data:
        operation = getattr(instruction, "operation", None)
        qargs = list(getattr(instruction, "qubits", ()))
        cargs = list(getattr(instruction, "clbits", ()))
        if operation is None and isinstance(instruction, tuple) and instruction:
            operation = instruction[0]
            qargs = list(instruction[1])
            cargs = list(instruction[2]) if len(instruction) > 2 else []
        q_indices = [int(circuit.find_bit(q).index) for q in qargs]
        c_indices = [int(circuit.find_bit(c).index) for c in cargs]
        out.append(operation, q_indices, c_indices)
        name = str(getattr(operation, "name", "")).strip().lower()
        arity = int(len(q_indices))
        if name:
            seen_gate_counts[name] = int(seen_gate_counts.get(name, 0)) + 1
            seen_gate_arity_counts[f"{arity}q:{name}"] = int(seen_gate_arity_counts.get(f"{arity}q:{name}", 0)) + 1
        angle_std = 0.0
        generator = None
        if arity == 1 and name in oneq_gates and s1q > 0.0:
            angle_std = s1q
            generator = str(rng.choice(oneq_generators))
        elif arity == 2 and name in twoq_gates and s2q > 0.0:
            angle_std = s2q
            generator = str(rng.choice(twoq_generators))
        if generator is None:
            continue
        delta = float(rng.normal(loc=0.0, scale=float(angle_std)))
        unitary = _pauli_rotation_unitary(generator, delta)
        gate = UnitaryGate(unitary, label=f"coh_{generator}_{location_index}")
        out.append(gate, q_indices)
        inserted.append(
            {
                "location_index": int(location_index),
                "after_gate": name,
                "qubits": [int(q) for q in q_indices],
                "generator_ixyz": str(generator),
                "delta": float(delta),
    ...
    return out, {
        ...
        "seen_gate_counts": dict(sorted(seen_gate_counts.items())),
        "seen_gate_arity_counts": dict(sorted(seen_gate_arity_counts.items())),
        "one_qubit_angle_std": float(s1q),
        "two_qubit_angle_std": float(s2q),
        "one_qubit_gates": sorted(oneq_gates),
        "two_qubit_gates": sorted(twoq_gates),
    }


def _synthetic_coherent_enabled(cfg: OracleConfig) -> bool:
    return (
        float(getattr(cfg, "synthetic_coherent_1q_angle_std", 0.0)) > 0.0
        or float(getattr(cfg, "synthetic_coherent_2q_angle_std", 0.0)) > 0.0
    )


def _require_synthetic_coherent_insertions(
    *,
    compiled: QuantumCircuit,
    cfg: OracleConfig,
    coherent_details: Mapping[str, Any],
) -> None:
    if not _synthetic_coherent_enabled(cfg):
        return
    if int(getattr(compiled, "size", lambda: 0)()) <= 0:
        return
    inserted_count = int(coherent_details.get("inserted_count", 0))
    if inserted_count > 0:
        return
    raise RuntimeError(
        "Synthetic coherent overrotation was configured but inserted zero gate errors. "
        "This is not a valid full-noise circuit-channel diagnostic. "
        f"Configured 1q gates={tuple(getattr(cfg, 'synthetic_coherent_1q_gates', ()) or ())}, "
        f"configured 2q gates={tuple(getattr(cfg, 'synthetic_coherent_2q_gates', ()) or ())}, "
        f"compiled gates seen={dict(coherent_details.get('seen_gate_arity_counts', {}))}."
    )



def _preflight_aer_density_matrix_environment(cfg: OracleConfig) -> None:
    key = ((None if cfg.seed is None else int(cfg.seed)), bool(cfg.omp_shm_workaround))
    if key in _AER_DENSITY_MATRIX_PREFLIGHT_OK_CACHE:
        return
...
try:
    ev_circuit = qc.copy()
    ev_circuit.save_expectation_value(observable, [0], label="ev")
    _ = sim.run(ev_circuit, shots=1).result()["ev"]
except Exception:
    try:
        _ = sim.run(ev_circuit, shots=1).result().data(0)["ev"]
    except Exception:
        rho_circuit = qc.copy()
        rho_circuit.save_expectation_value(observable, [0], label="ev")
        _ = sim.run(rho_circuit, shots=1).result().data(0)["ev"]
print("AER_DENSITY_MATRIX_OK")
"""
    env = None
    if bool(cfg.omp_shm_workaround):
        env = dict(os.environ)
    ...
    if int(result.returncode) != 0:
        ...
        if _looks_like_openmp_shm_abort(combined):
            raise RuntimeError(
                "Aer density-matrix preflight failed due to OpenMP shared-memory restrictions "
                "in this environment (detected OMP/SHM2 failure). "
                f"Preflight stderr/stdout tail:\n{detail_tail}"
            )
        raise RuntimeError(
            "Aer density-matrix preflight failed before noisy expectation execution started. "
            f"Preflight stderr/stdout tail:\n{detail_tail}"
        )
    _AER_DENSITY_MATRIX_PREFLIGHT_OK_CACHE.add(key)

def _circuit_has_measurements(circuit: QuantumCircuit) -> bool:
    for instruction in getattr(circuit, "data", ()):
        operation = getattr(instruction, "operation", None)
        if operation is None and isinstance(instruction, tuple) and instruction:
            operation = instruction[0]
        if str(getattr(operation, "name", "")).lower() == "measure":
            return True
    return False


def _compile_circuit_for_aer_density_matrix(
    circuit: QuantumCircuit,
    *,
    simulator: Any,
    noise_model: Any,
    ...
    if _circuit_has_measurements(circuit):
        raise ValueError("aer_density_matrix expects circuits without terminal measurements.")
    if int(getattr(circuit, "num_qubits", 0)) > int(_AER_DENSITY_MATRIX_MAX_QUBITS):
        raise ValueError(
            "aer_density_matrix is capped at "
            f"{_AER_DENSITY_MATRIX_MAX_QUBITS} qubits because density-matrix memory scales as O(4^n)."
        )
    basis_gates = tuple(str(g) for g in getattr(noise_model, "basis_gates", ()) or ())
    ...
    if basis_gates:
        # AerSimulator can execute broad custom instructions directly, but synthetic
        # gate-noise diagnostics must expose configured 1q/2q gate locations to the
        # NoiseModel and coherent-overrotation insertion pass.  Passing the
        # NoiseModel basis forces the transpiler to decompose supported evolution
        # instructions instead of silently executing them as opaque un-noised gates.
        transpile_kwargs["basis_gates"] = list(basis_gates)
    compiled = transpile(circuit, **transpile_kwargs)
    if _circuit_has_measurements(compiled):
        raise ValueError("aer_density_matrix transpilation produced measurement instructions.")
    if int(compiled.num_qubits) != int(circuit.num_qubits):
        raise RuntimeError(
            "aer_density_matrix compilation changed circuit width "
            f"from {circuit.num_qubits} to {compiled.num_qubits}; refusing density-matrix execution."
        )
    if int(compiled.num_qubits) > int(_AER_DENSITY_MATRIX_MAX_QUBITS):
        raise ValueError(
            "aer_density_matrix compiled circuit exceeds "
            f"{_AER_DENSITY_MATRIX_MAX_QUBITS} qubits."
        )
    return {
        "compiled": compiled,
        "logical_to_physical": tuple(range(int(circuit.num_qubits))),
        "compiled_num_qubits": int(compiled.num_qubits),
        "compile_signature": {
            "mode": "aer_density_matrix_logical_width",
            "seed_transpiler": int(seed_transpiler),
            "optimization_level": int(optimization_level),
            "noise_model_basis_gates": list(basis_gates),
            "coupling_map_enforced": False,
        },
        "noise_model_basis_gates": list(basis_gates),
    }


def _coerce_real_expectation_value(value: Any, *, label: str) -> float:
    z = complex(np.asarray(value).reshape(-1)[0])
    if abs(float(np.imag(z))) > float(_AER_DENSITY_MATRIX_IMAG_TOL):
        raise RuntimeError(
            f"aer_density_matrix produced non-real {label} expectation {z!r}; "
            f"imag tolerance is {_AER_DENSITY_MATRIX_IMAG_TOL}."
        )
    return float(np.real(z))


def _run_aer_density_matrix_expectation(
    *,
    simulator: Any,
    compiled_circuit: QuantumCircuit,
    observable: SparsePauliOp,
) -> tuple[float, dict[str, Any]]:
    if _circuit_has_measurements(compiled_circuit):
        raise ValueError("aer_density_matrix expects compiled circuits without measurements.")
    if tuple(getattr(compiled_circuit, "parameters", ())) != ():
        raise ValueError("aer_density_matrix cannot execute an unbound parameterized circuit.")
    if int(getattr(compiled_circuit, "num_qubits", 0)) > int(_AER_DENSITY_MATRIX_MAX_QUBITS):
        raise ValueError(
            "aer_density_matrix execution is capped at "
            f"{_AER_DENSITY_MATRIX_MAX_QUBITS} qubits because density-matrix memory scales as O(4^n)."
        )
    if int(observable.num_qubits) != int(compiled_circuit.num_qubits):
        raise ValueError(
            "aer_density_matrix observable width mismatch: "
            f"observable has {observable.num_qubits} qubits, circuit has {compiled_circuit.num_qubits}."
        )

    execution_details = {
        "shotless": True,
        "aer_method": "density_matrix",
        "expectation_extraction": "save_expectation_value",
        "simulator_shots": 1,
    }
    try:
        ev_circuit = compiled_circuit.copy()
        ev_circuit.save_expectation_value(
            observable,
            list(range(int(ev_circuit.num_qubits))),
            label="aer_density_matrix_ev",
        )
        result = simulator.run(ev_circuit, shots=1).result()
        value = result.data(0)["aer_density_matrix_ev"]
        return _coerce_real_expectation_value(value, label="save_expectation_value"), execution_details
    except Exception as exc:
        try:
            from qiskit.quantum_info import DensityMatrix
        except Exception as import_exc:  # pragma: no cover - qiskit version/environment path
            raise RuntimeError(
                "aer_density_matrix save_density_matrix fallback requires qiskit.quantum_info.DensityMatrix."
            ) from import_exc
        rho_circuit = compiled_circuit.copy()
        rho_circuit.save_density_matrix(label="aer_density_matrix_rho")
        result = simulator.run(rho_circuit, shots=1).result()
        rho = DensityMatrix(result.data(0)["aer_density_matrix_rho"])
        value = rho.expectation_value(observable)
        fallback_details = dict(execution_details)
        fallback_details.update(
            {
                "expectation_extraction": "save_density_matrix",
                "save_expectation_value_error": f"{type(exc).__name__}: {exc}",
            }
        )
        return _coerce_real_expectation_value(value, label="save_density_matrix"), fallback_details
...
class RawMeasurementOracle:
    ...
    def __init__(self, config: OracleConfig):
        ...
        self.config = OracleConfig(
            ...
            value_noise_semantic=str(getattr(config, "value_noise_semantic", _VALUE_NOISE_SEMANTIC)),
            value_noise_std_source=str(getattr(config, "value_noise_std_source", "explicit_std")),
        )
        value_noise_model = str(self.config.value_noise_model).strip().lower() or "off"
        value_noise_std = float(self.config.value_noise_std)
        if value_noise_model == "off" and value_noise_std != 0.0:
            raise ValueError("value_noise_model='off' requires value_noise_std == 0.")
        if value_noise_model != "off":
            raise ValueError(
                "value_noise_raw_measurement_unsupported: value noise is not supported on raw_measurement_v1."
            )
...
class ExpectationOracle:
    ...
    def __init__(self, config: OracleConfig):
        ...
        self.config = OracleConfig(
            ...
            value_noise_std_source=str(
                getattr(config, "value_noise_std_source", "explicit_std")
            ),
            synthetic_depolarizing_1q_error=float(
                getattr(config, "synthetic_depolarizing_1q_error", 0.0)
            ),
        ...
        self._aer_density_matrix_simulator = None
        self._aer_density_matrix_noise_model = None
        self._aer_density_matrix_backend_obj = None
        self._runtime_sampler = None
        self._runtime_sampler_configured_details: dict[str, Any] | None = None
        ...
        if value_noise_model not in _VALUE_NOISE_MODELS:
            raise ValueError(
                f"Unsupported value_noise_model={value_noise_model!r}; expected one of {sorted(_VALUE_NOISE_MODELS)}."
            )
        if value_noise_model == "off" and value_noise_std != 0.0:
            raise ValueError("value_noise_model='off' requires value_noise_std == 0.")
        if value_noise_model == "gaussian_iid_v1":
            if (not np.isfinite(value_noise_std)) or value_noise_std <= 0.0:
                raise ValueError("value_noise_model='gaussian_iid_v1' requires finite value_noise_std > 0.")
            if str(self.config.execution_surface) != "expectation_v1":
                raise ValueError(
                    "value_noise_raw_measurement_unsupported: value noise supports only execution_surface='expectation_v1'."
                )
        self._value_noise_effective_seed = (
            int(self.config.value_noise_seed)
            if self.config.value_noise_seed is not None
            else int(self.config.seed)
        ) if value_noise_model == "gaussian_iid_v1" else None
        self._value_noise_seed_source = (
            "value_noise_seed"
            if value_noise_model == "gaussian_iid_v1" and self.config.value_noise_seed is not None
            else ("oracle_config_seed" if value_noise_model == "gaussian_iid_v1" else None)
        )
        self._value_noise_rng = (
            np.random.default_rng(int(self._value_noise_effective_seed))
            if value_noise_model == "gaussian_iid_v1"
            else None
        )
        self._value_noise_draw_count = 0
        if self.config.oracle_aggregate not in {"mean", "median"}:
            raise ValueError(
                f"Unsupported oracle_aggregate={self.config.oracle_aggregate}; use mean or median."
            )
        ...
        if self.config.noise_mode == "backend_scheduled":
        ...
        elif self.config.noise_mode in {
            "aer_density_matrix",
            "aer_density_matrix_synthetic_depolarizing",
            "aer_density_matrix_synthetic_coherent",
        }:
            _validate_aer_density_matrix_config(self.config)
            _apply_omp_env_workaround(self.config)
            _preflight_aer_density_matrix_environment(self.config)
            try:
                from qiskit_aer import AerSimulator
            except Exception as exc:  # pragma: no cover - import error path
                raise RuntimeError(
                    "aer_density_matrix requires qiskit-aer with AerSimulator density_matrix support."
                ) from exc

            backend_obj = None
            backend_name: str | None = None
            using_fake = False
            if self.config.noise_mode == "aer_density_matrix_synthetic_depolarizing":
                noise_model = _build_synthetic_depolarizing_noise_model(self.config)
                backend_name = "synthetic_depolarizing"
            elif self.config.noise_mode == "aer_density_matrix_synthetic_coherent":
                _validate_synthetic_depolarizing_config(self.config)
                _validate_synthetic_coherent_config(self.config)
                noise_model = _build_synthetic_depolarizing_noise_model(self.config)
                backend_name = "synthetic_coherent_overrotation_plus_depolarizing"
            else:
                try:
                    from qiskit_aer.noise import NoiseModel
                except Exception as exc:  # pragma: no cover - import error path
                    raise RuntimeError(
                        "aer_density_matrix requires qiskit-aer noise model support."
                    ) from exc
                backend_obj, backend_name, using_fake = _resolve_noise_backend(self.config)
                noise_model = NoiseModel.from_backend(backend_obj)
            ...
            if self.config.seed is not None:
                simulator_kwargs["seed_simulator"] = int(self.config.seed)
            self._aer_density_matrix_backend_obj = backend_obj
            self._aer_density_matrix_noise_model = noise_model
            self._aer_density_matrix_simulator = AerSimulator(**simulator_kwargs)
            density_details: dict[str, Any] = {
                "shots": None,
                "shots_configured_ignored": int(self.config.shots),
                "shotless": True,
                "aer_method": "density_matrix",
                "compiled_mode": "aer_density_matrix_logical_width",
                "layout_policy": "logical_prefix_qubits_no_backend_scheduling",
                "coupling_map_enforced": False,
                "readout_errors_applied": False,
                "readout_error_note": (
                    "No measurement instructions are inserted; readout errors in a noise model "
                    "do not affect save_expectation_value/save_density_matrix."
                ),
                "max_qubits": int(_AER_DENSITY_MATRIX_MAX_QUBITS),
                "transpile_optimization_level": int(self.config.transpile_optimization_level),
                "transpile_seed": int(
                    self.config.seed
                ...
                "mitigation": dict(mitigation_cfg),
                "symmetry_mitigation": dict(symmetry_cfg),
                "noise_model_basis_gates": [
                    str(g) for g in getattr(noise_model, "basis_gates", ()) or ()
                ],
            }
            if self.config.noise_mode == "aer_density_matrix_synthetic_depolarizing":
                density_details["synthetic_depolarizing"] = dict(
                    _synthetic_depolarizing_config_payload(self.config)
                )
                density_details["backend_snapshot"] = None
            elif self.config.noise_mode == "aer_density_matrix_synthetic_coherent":
                density_details["synthetic_depolarizing"] = dict(
                    _synthetic_depolarizing_config_payload(self.config)
                )
                density_details["synthetic_coherent"] = dict(
                    _synthetic_coherent_config_payload(self.config)
                )
                density_details["backend_snapshot"] = None
            else:
                density_details["backend_snapshot"] = dict(_snapshot_backend_target_shared(backend_obj))
            self.backend_info = NoiseBackendInfo(
                noise_mode=str(self.config.noise_mode),
                estimator_kind="qiskit_aer.AerSimulator(method='density_matrix')",
                backend_name=(None if backend_name in {None, ""} else str(backend_name)),
                using_fake_backend=bool(using_fake),
    ...
    def _apply_value_noise_to_estimate(self, estimate: OracleEstimate) -> OracleEstimate:
        model = str(getattr(self.config, "value_noise_model", "off")).strip().lower() or "off"
        if model == "off":
            return estimate
        metadata = dict(getattr(estimate, "metadata", {}) or {})
        existing = metadata.get("value_noise", {})
        if isinstance(existing, Mapping) and bool(existing.get("enabled", False)):
            return estimate
        raw_values = [float(x) for x in list(getattr(estimate, "raw_values", []) or [])]
        if not raw_values:
            raw_values = [float(estimate.mean)]
        if self._value_noise_rng is None:
            self._value_noise_rng = np.random.default_rng(int(self._value_noise_effective_seed))
        draw_index_start = int(self._value_noise_draw_count)
        noise = self._value_noise_rng.normal(
            loc=0.0,
            scale=float(self.config.value_noise_std),
            size=int(len(raw_values)),
        )
        noisy_values = [float(base + delta) for base, delta in zip(raw_values, noise)]
        self._value_noise_draw_count = draw_index_start + int(len(noisy_values))
        draw_index_stop = int(self._value_noise_draw_count)
        noisy_estimate = _oracle_estimate_from_samples(
            noisy_values,
            aggregate=str(estimate.aggregate),
        )
        summary = {
            "enabled": True,
            "model": str(model),
            "std": float(self.config.value_noise_std),
            "std_units": "absolute_observable_value",
            "semantic": str(getattr(self.config, "value_noise_semantic", _VALUE_NOISE_SEMANTIC)),
            "sigma0_abs": (
                None
                if getattr(self.config, "value_noise_sigma0_abs", None) is None
                else float(getattr(self.config, "value_noise_sigma0_abs"))
            ),
            "N_eff": (
                None
                if getattr(self.config, "value_noise_n_eff", None) is None
                else float(getattr(self.config, "value_noise_n_eff"))
            ),
            "derived_std": (
                None
                if getattr(self.config, "value_noise_sigma0_abs", None) is None
                or getattr(self.config, "value_noise_n_eff", None) is None
                else float(getattr(self.config, "value_noise_sigma0_abs"))
                / math.sqrt(float(getattr(self.config, "value_noise_n_eff")))
            ),
            "std_source": str(getattr(self.config, "value_noise_std_source", "explicit_std")),
            ...
            "draw_index_stop": int(draw_index_stop),
            "n_draws": int(len(noisy_values)),
            "applied_at": "ExpectationOracle.public_expectation_exit",
            "base_mean": float(estimate.mean),
            "base_std": float(estimate.std),
            "base_stderr": float(estimate.stderr),
            "base_aggregate": str(estimate.aggregate),
        }
        metadata["value_noise"] = dict(summary)
        self._update_backend_details(value_noise=dict(summary))
        return OracleEstimate(
            mean=float(noisy_estimate.mean),
            std=float(noisy_estimate.std),
            stdev=float(noisy_estimate.stdev),
            stderr=float(noisy_estimate.stderr),
            n_samples=int(noisy_estimate.n_samples),
            raw_values=[float(x) for x in noisy_estimate.raw_values],
            aggregate=str(noisy_estimate.aggregate),
            metadata=metadata,
        )
    ...
    def _get_parameterized_aer_density_matrix_base(self, plan: Any) -> dict[str, Any]:
        cache_key = self._parameterized_plan_cache_key(plan)
        cached = self._compiled_plan_base_cache.get(cache_key, None)
        if cached is not None:
            if cache_key not in self._compiled_plan_base_cache_hit_logged:
                self._compiled_plan_base_cache_hit_logged.add(cache_key)
                _oracle_compile_heartbeat(
                    "noise_oracle_aer_density_matrix_parameterized_compile_cache_hit",
                    noise_mode=str(self.config.noise_mode),
                    backend_name=self.backend_info.backend_name,
                    transpile_seed=int(
                        self.config.seed if self.config.seed_transpiler is None else self.config.seed_transpiler
            ...
            return cached
        if self._aer_density_matrix_simulator is None or self._aer_density_matrix_noise_model is None:
            raise RuntimeError("aer_density_matrix simulator/noise model is unavailable.")
        transpile_seed = int(self.config.seed if self.config.seed_transpiler is None else self.config.seed_transpiler)
        optimization_level = int(self.config.transpile_optimization_level)
        compile_t0 = time.perf_counter()
        _oracle_compile_heartbeat(
            "noise_oracle_aer_density_matrix_parameterized_compile_start",
            noise_mode=str(self.config.noise_mode),
            backend_name=self.backend_info.backend_name,
            transpile_seed=int(transpile_seed),
            optimization_level=int(optimization_level),
        ...
        cached = _compile_circuit_for_aer_density_matrix(
            plan.circuit,
            simulator=self._aer_density_matrix_simulator,
            noise_model=self._aer_density_matrix_noise_model,
            seed_transpiler=int(transpile_seed),
            optimization_level=int(optimization_level),
        )
        if str(self.config.noise_mode) == "aer_density_matrix_synthetic_coherent":
            compiled_with_coherent, coherent_details = _insert_synthetic_coherent_overrotations(
                cached["compiled"], self.config
            )
            _require_synthetic_coherent_insertions(
                compiled=cached["compiled"], cfg=self.config, coherent_details=coherent_details
            )
            cached = dict(cached)
            cached["compiled"] = compiled_with_coherent
            cached["synthetic_coherent"] = dict(coherent_details)
            cached["compile_signature"] = {
                **dict(cached.get("compile_signature", {})),
                "synthetic_coherent_inserted_after_transpile": True,
            }
        _oracle_compile_heartbeat(
            "noise_oracle_aer_density_matrix_parameterized_compile_done",
            noise_mode=str(self.config.noise_mode),
            backend_name=self.backend_info.backend_name,
            transpile_seed=int(transpile_seed),
            optimization_level=int(optimization_level),
        ...
        self._update_backend_details(
            ...
            compiled_num_qubits=int(cached["compiled_num_qubits"]),
            parameterized_plan_digest=str(getattr(plan, "plan_digest", "")),
            compile_signature=dict(cached.get("compile_signature", {})),
            **_compiled_metrics_payload(compiled),
        )
        return cached

    def _get_aer_density_matrix_base(self, circuit: QuantumCircuit) -> dict[str, Any]:
        ...
        if cached is not None:
            if cache_key not in self._compiled_base_cache_hit_logged:
                ...
                _oracle_compile_heartbeat(
                    "noise_oracle_aer_density_matrix_compile_cache_hit",
                    noise_mode=str(self.config.noise_mode),
                    backend_name=self.backend_info.backend_name,
                    transpile_seed=int(
                        self.config.seed if self.config.seed_transpiler is None else self.config.seed_transpiler
        ...
        compile_t0 = time.perf_counter()
        _oracle_compile_heartbeat(
            "noise_oracle_aer_density_matrix_compile_start",
            noise_mode=str(self.config.noise_mode),
            backend_name=self.backend_info.backend_name,
            transpile_seed=int(transpile_seed),
            optimization_level=int(optimization_level),
        ...
        _oracle_compile_heartbeat(
            "noise_oracle_aer_density_matrix_compile_done",
            noise_mode=str(self.config.noise_mode),
            backend_name=self.backend_info.backend_name,
            transpile_seed=int(transpile_seed),
            optimization_level=int(optimization_level),
    ...
    def _evaluate_aer_density_matrix_compiled(
        self,
        compiled_circuit: QuantumCircuit,
        observable: SparsePauliOp,
        *,
        ...
        if self._aer_density_matrix_simulator is None:
            raise RuntimeError("aer_density_matrix simulator is unavailable.")
        compiled_observable = _apply_observable_layout_for_compiled_circuit(
            observable,
            compiled_circuit,
        )
        value, execution_details = _run_aer_density_matrix_expectation(
            simulator=self._aer_density_matrix_simulator,
            compiled_circuit=compiled_circuit,
            observable=compiled_observable,
        )
        repeats = max(1, int(self.config.oracle_repeats))
        vals = [float(value) for _ in range(repeats)]
        details: dict[str, Any] = {
            "shotless": True,
            "aer_method": "density_matrix",
            "readout_errors_applied": False,
            "readout_error_note": (
                "No measurement instructions are inserted; readout errors in NoiseModel.from_backend "
                "do not affect save_expectation_value/save_density_matrix."
            ),
            "simulations_run": 1,
            "raw_values_replicated_for_aggregate": bool(repeats > 1),
            "aer_density_matrix_execution": dict(execution_details),
            "isa_observable_num_qubits": int(compiled_observable.num_qubits),
        }
        if base is not None:
            details.update(
                {
                    "layout_physical_qubits": [
                        int(x) for x in tuple(base.get("logical_to_physical", ()))
                    ],
                    "compiled_num_qubits": int(
                        base.get("compiled_num_qubits", getattr(compiled_circuit, "num_qubits", 0))
                    ),
                    "compile_signature": dict(base.get("compile_signature", {})),
                    **_compiled_metrics_payload(compiled_circuit),
                }
            )
            if "synthetic_coherent" in base:
                details["synthetic_coherent"] = dict(base["synthetic_coherent"])
        self._update_backend_details(**details)
        return _oracle_estimate_from_samples(vals, aggregate=str(self.config.oracle_aggregate))

    def _evaluate_aer_density_matrix(
        self,
        circuit: QuantumCircuit,
        observable: SparsePauliOp,
    ) -> OracleEstimate:
        base = self._get_aer_density_matrix_base(circuit)
        return self._evaluate_aer_density_matrix_compiled(
            base["compiled"],
            observable,
            base=base,
        )
    ...
    def evaluate_parameterized(
        ...
        *,
        ...
        if symmetry_est is not None:
            return self._apply_value_noise_to_estimate(symmetry_est)
        if str(self.config.noise_mode) == "backend_scheduled":
            ...
            self._update_backend_details(**details)
            return self._apply_value_noise_to_estimate(estimate)
        if str(self.config.noise_mode) in {
            "aer_density_matrix",
            "aer_density_matrix_synthetic_depolarizing",
            "aer_density_matrix_synthetic_coherent",
        }:
            base = self._get_parameterized_aer_density_matrix_base(plan)
            bound_compiled = self._bind_compiled_parameterized_template(
                base["compiled"],
                plan,
                theta_arr,
            )
            return self._apply_value_noise_to_estimate(
                self._evaluate_aer_density_matrix_compiled(
                    bound_compiled,
                    observable,
                    base=base,
                )
        ...
        if str(self.config.noise_mode) == "runtime":
            ...
            return self._apply_value_noise_to_estimate(
                self._evaluate_runtime_isa_bound(
                    bound_compiled,
                    observable,
    ...
    def evaluate(
        ...
        *,
        ...
        if symmetry_est is not None:
            return self._apply_value_noise_to_estimate(symmetry_est)
        if str(self.config.noise_mode) == "backend_scheduled":
            return self._apply_value_noise_to_estimate(self._evaluate_backend_scheduled(circuit, observable))
        if str(self.config.noise_mode) in {
            "aer_density_matrix",
            "aer_density_matrix_synthetic_depolarizing",
            "aer_density_matrix_synthetic_coherent",
        }:
            return self._apply_value_noise_to_estimate(self._evaluate_aer_density_matrix(circuit, observable))
        if str(self.config.noise_mode) == "runtime":
            return self._apply_value_noise_to_estimate(
        ...
        for _ in range(repeats):
            ...
            try:
                ...
                if self._can_fallback_from_error(exc):
                    ...
                    raise
        return self._apply_value_noise_to_estimate(
            _oracle_estimate_from_samples(vals, aggregate=str(self.config.oracle_aggregate))
        )
