...
SCHEMA_VERSION = "hh_bench_metrics_v5"
...
class ProxyMetricRow:
    ...
    avqite_steps_completed: int | None = None
    avqite_stop_reason: str = ""
    imaginary_time_total: float | None = None
...
def extract_proxy_metric_row(
    ...
    *,
    ...
    hamiltonian_id = str(
        _first_nonempty(merged.get("hamiltonian_id"), merged.get("case_id"), "") or ""
    )
    ...
    shots_total = _to_int(merged.get("shots_total"))
    nit = _to_int(merged.get("nit"))
    num_parameters = _to_int(_first_nonempty(merged.get("num_parameters"), merged.get("num_params")))
    selected_operator_count = _to_int(merged.get("selected_operator_count"))
    subspace_dimension = _to_int(merged.get("subspace_dimension"))
    avqite_steps_completed = _to_int(merged.get("avqite_steps_completed"))
    avqite_stop_reason = str(_first_nonempty(merged.get("avqite_stop_reason"), "") or "")
    imaginary_time_total = _to_float(merged.get("imaginary_time_total"))
    ...
    return ProxyMetricRow(
        ...
        avqite_steps_completed=avqite_steps_completed,
        avqite_stop_reason=avqite_stop_reason,
        imaginary_time_total=imaginary_time_total,
        vqe_reps=vqe_reps,
...
def summarize_proxy_rows(
    ...
    *,
    ...
    shot_vals = [int(r.shots_total) for r in rows if r.shots_total is not None]
    ...
    summary: dict[str, Any] = {
        ...
        "shots_total": {
            "sum": int(sum(shot_vals)) if shot_vals else None,
            "median": float(statistics.median(shot_vals)) if shot_vals else None,
            "max": int(max(shot_vals)) if shot_vals else None,
        },
        "delta_E_abs": {
