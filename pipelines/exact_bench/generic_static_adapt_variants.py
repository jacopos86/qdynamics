...
singles/doubles pool; visible ``static_geo_adapt_vqe`` uses the problem-local
``full_meta`` pool.  ``static_geo_qeb_adapt_vqe`` is retained
...
from pipelines.static_adapt.builders import hh_pool_presets as _hh_pool_presets
...
PAPER_I_COMPARATOR_ESTIMATOR_ACCOUNTING_SCHEMA = (
    "paper_i_comparator_estimator_accounting_v2"
)
PAPER_I_COMPARATOR_ROUND_RECEIPT_SCHEMA = (
    "paper_i_comparator_estimator_round_receipt_v2"
)
PAPER_I_COMPARATOR_ACTIVE_PREFIX_CHECKPOINT_SCHEMA = (
    "paper_i_comparator_active_prefix_checkpoint_v1"
)
PAPER_I_COMPARATOR_COMPACT_LEDGER_SCHEMA = (
    "paper_i_comparator_compact_estimator_ledger_v2"
)
PAPER_I_COMPARATOR_LEDGER_CURSOR_SCHEMA = (
    "paper_i_comparator_estimator_ledger_cursor_v1"
)
PAPER_I_COMPARATOR_ACCOUNTING_SNAPSHOT_SCHEMA = (
    "paper_i_comparator_estimator_accounting_snapshot_v1"
)
# A full symmetric Geo metric scan contains n(n+1)/2 logical estimator
# primitives.  Materializing a Python identity/occurrence object for every
# member caused the 1,622-candidate Paper-I smoke to exceed memory after the
# science had already completed.  Small families retain the historical
# byte-for-byte payload; larger families use exact counts plus deterministic,
# order-independent 256-bit XOR/sum set receipts.
_COMPARATOR_COMPACT_FAMILY_MEMBER_THRESHOLD = 10_000
...
PAPER_I_APPEND_COMPARISON_EXECUTION_OWNER = (
    "pipelines.static_adapt.ra_adapt.run_append_adapt"
)
_PAPER_I_APPEND_COMPARISON_CASE_IDS = frozenset(
    str(value)
    for value in TABLE_I_PAPER_I_HH_COMPLETION_REGIME_CASE_IDS.values()
)
...
def _primitive_set_accumulator_payload(
    *, count: int, xor_value: int, sum_value: int
) -> dict[str, Any]:
    """Return a bounded deterministic receipt for a set of SHA-256 IDs."""

    return {
        "schema": "sha256_id_set_xor_sum_v1",
        "count": int(count),
        "xor_sha256": f"{int(xor_value) % _DIGEST_MODULUS:064x}",
        "sum_mod_2_256_sha256": (
            f"{int(sum_value) % _DIGEST_MODULUS:064x}"
        ),
    }


def _accumulator_add(
    accumulator: tuple[int, int, int], primitive_id: str
) -> tuple[int, int, int]:
    value = int(str(primitive_id), 16)
    return (
        int(accumulator[0]) + 1,
        int(accumulator[1]) ^ value,
        (int(accumulator[2]) + value) % _DIGEST_MODULUS,
    )


def _accumulator_merge(
    left: tuple[int, int, int], right: tuple[int, int, int]
) -> tuple[int, int, int]:
    return (
        int(left[0]) + int(right[0]),
        int(left[1]) ^ int(right[1]),
        (int(left[2]) + int(right[2])) % _DIGEST_MODULUS,
    )


def _accumulator_from_payload(payload: Mapping[str, Any]) -> tuple[int, int, int]:
    row = dict(payload)
    if row.get("schema") != "sha256_id_set_xor_sum_v1":
        raise ValueError("unsupported primitive-set accumulator schema")
    return (
        int(row.get("count", 0)),
        int(str(row.get("xor_sha256", "0")), 16),
        int(str(row.get("sum_mod_2_256_sha256", "0")), 16),
    )


def _accumulator_delta(
    after: tuple[int, int, int], before: tuple[int, int, int]
) -> tuple[int, int, int]:
    count = int(after[0]) - int(before[0])
    if count < 0:
        raise RuntimeError("estimator primitive-set accumulator count regressed")
    return (
        count,
        int(after[1]) ^ int(before[1]),
        (int(after[2]) - int(before[2])) % _DIGEST_MODULUS,
    )


def _summary_primitive_accumulator(
    summary: Mapping[str, Any]
) -> tuple[int, int, int]:
    row = dict(summary)
    serialized = row.get("primitive_id_set_accumulator")
    if isinstance(serialized, Mapping):
        return _accumulator_from_payload(serialized)
    accumulator = (0, 0, 0)
    for primitive_id in row.get("primitive_ids", []):
        accumulator = _accumulator_add(accumulator, str(primitive_id))
    if accumulator[0] != int(row.get("unique_primitive_count", -1)):
        raise RuntimeError("materialized estimator primitive set does not close")
    return accumulator


def _bounded_estimator_summary(summary: Mapping[str, Any]) -> dict[str, Any]:
    """Drop replay-redundant primitive arrays above the receipt threshold."""

    row = dict(summary)
    unique_count = int(row.get("unique_primitive_count", 0))
    already_compact = not bool(row.get("primitive_ids_complete", True))
    if (
        not already_compact
        and unique_count <= _COMPARATOR_COMPACT_FAMILY_MEMBER_THRESHOLD
    ):
        return row
    accumulator = _summary_primitive_accumulator(row)
    row.update(
        {
            "primitive_ids": [],
            "primitive_ids_complete": False,
            "primitive_id_set_accumulator": _primitive_set_accumulator_payload(
                count=accumulator[0],
                xor_value=accumulator[1],
                sum_value=accumulator[2],
            ),
            "component_by_primitive_id": {},
            "component_by_primitive_id_complete": False,
            "cross_component_reuse_primitive_ids": [],
            "cross_component_reuse_primitive_ids_complete": False,
        }
    )
    return row


def _bounded_occurrence_summary(summary: Mapping[str, Any]) -> dict[str, Any]:
    row = dict(summary)
    occurrence_count = int(row.get("total_call_occurrences", 0))
    already_compact = not bool(row.get("occurrence_sequences_complete", True))
    if (
        not already_compact
        and occurrence_count <= _COMPARATOR_COMPACT_FAMILY_MEMBER_THRESHOLD
    ):
        return row
    row.update(
        {
            ...
            "occurrence_sequences_complete": False,
        }
    )
    return row
...
def _occurrence_semantic_stream_sha256(
    occurrences: Iterable[Mapping[str, Any]]
) -> str:
    """Hash occurrence semantics in execution order without array copies."""

    digest = hashlib.sha256()
    for raw_row in occurrences:
        row = dict(raw_row)
        semantic = {
...
class _PaperIComparatorEstimatorLedger:
    """Bounded exact ledger for the Paper-I Append/Geo comparators.

    Ordinary calls retain the historical :class:`EstimatorCallLedger` payload.
    A large complete symmetric-pair family is represented by its coordinate-set
    contract, exact member count, and commutative SHA-256 ID receipts rather
    than millions of Python records.  State fingerprints remain part of every
    member identity, so the ``S_unique`` diagnostic is still state keyed.
    """

    def __init__(self) -> None:
        self._base = EstimatorCallLedger()
        self._next_sequence = 1
        self._explicit_occurrences: list[dict[str, Any]] = []
        self._compact_blocks: list[dict[str, Any]] = []
        self._context_coordinate_sets: dict[str, list[frozenset[str]]] = {}
        self._scope_context_coordinate_sets: dict[
            tuple[str, str], list[frozenset[str]]
        ] = {}
        self._branch_context_coordinate_sets: dict[
            tuple[str, str], list[frozenset[str]]
        ] = {}
        self._compact_unique_count = 0
        self._compact_unique_components = {
            name: 0 for name in ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
        }
        self._compact_unique_accumulator = (0, 0, 0)
        self._compact_component_accumulators = {
            name: (0, 0, 0)
            for name in ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
        }
        self._compact_unique_by_scope: dict[str, int] = {}
        self._compact_unique_by_branch: dict[str, int] = {}

    @staticmethod
    def _context_payload(identity: EstimatorCallKey) -> dict[str, Any]:
        if identity.symmetric_pair is not None:
            raise ValueError("symmetric family template must not carry a pair")
        return identity.as_dict()

    @staticmethod
    def _pair_primitive_id(
        identity: EstimatorCallKey, left: str, right: str
    ) -> str:
        return replace(identity, symmetric_pair=(left, right)).primitive_id

    @staticmethod
    def _pair_seen(
        left: str,
        right: str,
        prior_sets: Sequence[frozenset[str]],
    ) -> bool:
        return any(left in prior and right in prior for prior in prior_sets)
    ...
    def record_call(
        ...
        *,
        ...
        receipt = self._base.record_call(
            identity,
        ...
        self._next_sequence += 1
        self._explicit_occurrences.append(
            {
                "sequence": sequence,
        ...
        # The wrapped materialized ledger has its own dense local sequence.
        # Once a compact block reserves a global sequence range, callers must
        # never observe that local counter as the execution-order receipt.
        return replace(receipt, occurrence_sequence=sequence)

    def record_symmetric_pair_family(
        self,
        identity: EstimatorCallKey,
        *,
        ...
        coordinate_rows = tuple(sorted({str(value) for value in coordinates}))
        if not coordinate_rows:
            return {
                "member_count": 0,
                "new_unique_primitive_count": 0,
                "serialization_mode": "empty_family_v1",
            }
        member_count = len(coordinate_rows) * (len(coordinate_rows) + 1) // 2
        if member_count <= _COMPARATOR_COMPACT_FAMILY_MEMBER_THRESHOLD:
            new_count = 0
            for left_offset, left in enumerate(coordinate_rows):
                for right in coordinate_rows[left_offset:]:
                    receipt = self.record_call(
                        replace(identity, symmetric_pair=(left, right)),
                        component=str(component),
                        consumer_scope=str(consumer_scope),
                        branch_id=branch_id,
                    )
                    new_count += int(bool(receipt.charged))
            return {
                "member_count": int(member_count),
                "new_unique_primitive_count": int(new_count),
                "serialization_mode": "materialized_v1",
            }

        if str(component) != "N_metric":
            raise ValueError(
                "compact symmetric-family accounting is restricted to N_metric"
            )
        context_payload = self._context_payload(identity)
        context_sha = _canonical_payload_sha256(context_payload)
        coordinate_set = frozenset(coordinate_rows)
        coordinate_sha = _canonical_payload_sha256(
            {"coordinates": list(coordinate_rows)}
        )
        prior_global = self._context_coordinate_sets.setdefault(context_sha, [])
        scope_key = (str(consumer_scope), context_sha)
        prior_scope = self._scope_context_coordinate_sets.setdefault(scope_key, [])
        branch_label = "__unbranched__" if branch_id is None else str(branch_id)
        branch_key = (branch_label, context_sha)
        prior_branch = self._branch_context_coordinate_sets.setdefault(branch_key, [])
        base_ids = set(str(value) for value in self._base.summary()["primitive_ids"])
        base_scope_ids = set(
            str(value)
            for value in self._base.summary(
                consumer_scopes=[str(consumer_scope)]
            )["primitive_ids"]
        )
        base_branch_ids = set(
            str(value)
            for value in self._base.summary(
                branch_ids=([] if branch_id is None else [str(branch_id)]),
                include_unbranched=bool(branch_id is None),
            )["primitive_ids"]
        )

        all_accumulator = (0, 0, 0)
        new_accumulator = (0, 0, 0)
        scope_new = 0
        branch_new = 0
        for left_offset, left in enumerate(coordinate_rows):
            for right in coordinate_rows[left_offset:]:
                primitive_id = self._pair_primitive_id(identity, left, right)
                all_accumulator = _accumulator_add(
                    all_accumulator, primitive_id
                )
                already_global = (
                    primitive_id in base_ids
                    or self._pair_seen(left, right, prior_global)
                )
                if not already_global:
                    new_accumulator = _accumulator_add(
                        new_accumulator, primitive_id
                    )
                if (
                    primitive_id not in base_scope_ids
                    and not self._pair_seen(left, right, prior_scope)
                ):
                    scope_new += 1
                if (
                    primitive_id not in base_branch_ids
                    and not self._pair_seen(left, right, prior_branch)
                ):
                    branch_new += 1

        start = int(self._next_sequence)
        stop = int(start + member_count - 1)
        self._next_sequence = stop + 1
        self._compact_unique_count += int(new_accumulator[0])
        self._compact_unique_components["N_metric"] += int(new_accumulator[0])
        self._compact_unique_accumulator = _accumulator_merge(
            self._compact_unique_accumulator, new_accumulator
        )
        self._compact_component_accumulators["N_metric"] = _accumulator_merge(
            self._compact_component_accumulators["N_metric"], new_accumulator
        )
        self._compact_unique_by_scope[str(consumer_scope)] = int(
            self._compact_unique_by_scope.get(str(consumer_scope), 0) + scope_new
        )
        self._compact_unique_by_branch[branch_label] = int(
            self._compact_unique_by_branch.get(branch_label, 0) + branch_new
        )
        prior_global.append(coordinate_set)
        prior_scope.append(coordinate_set)
        prior_branch.append(coordinate_set)
        block = {
            "schema": "paper_i_comparator_compact_symmetric_family_v2",
            "context_sha256": context_sha,
            "identity_template": dict(context_payload),
            "coordinate_set_sha256": coordinate_sha,
            "coordinate_count": int(len(coordinate_rows)),
            "coordinates": list(coordinate_rows),
            "member_count": int(member_count),
            "new_unique_primitive_count": int(new_accumulator[0]),
            "component": str(component),
            "consumer_scope": str(consumer_scope),
            "branch_id": branch_id,
            "occurrence_sequence_start": start,
            "occurrence_sequence_stop": stop,
            "member_primitive_set_accumulator": _primitive_set_accumulator_payload(
                count=all_accumulator[0],
                xor_value=all_accumulator[1],
                sum_value=all_accumulator[2],
            ),
            "new_unique_primitive_set_accumulator": (
                _primitive_set_accumulator_payload(
                    count=new_accumulator[0],
                    xor_value=new_accumulator[1],
                    sum_value=new_accumulator[2],
                )
            ),
        }
        block["block_sha256"] = _canonical_payload_sha256(block)
        previous_chain = (
            "0" * 64
            if not self._compact_blocks
            else str(self._compact_blocks[-1]["block_chain_sha256"])
        )
        block["previous_block_chain_sha256"] = previous_chain
        block["block_chain_sha256"] = _canonical_payload_sha256(
            {
                "previous_block_chain_sha256": previous_chain,
                "block_sha256": str(block["block_sha256"]),
            }
        )
        self._compact_blocks.append(block)
        return {
            "member_count": int(member_count),
            "new_unique_primitive_count": int(new_accumulator[0]),
            "serialization_mode": "compact_exact_symmetric_family_v2",
            "block_sha256": str(block["block_sha256"]),
        }

    def _base_unique_accumulator(self) -> tuple[int, int, int]:
        accumulator = (0, 0, 0)
        for primitive_id in self._base.summary()["primitive_ids"]:
            accumulator = _accumulator_add(accumulator, str(primitive_id))
        return accumulator

    def _unique_accumulator(self) -> tuple[int, int, int]:
        return _accumulator_merge(
            self._base_unique_accumulator(), self._compact_unique_accumulator
        )

    def _raw_occurrence_count(self) -> int:
        return int(
            self._base.occurrence_summary()["total_call_occurrences"]
            + sum(int(block["member_count"]) for block in self._compact_blocks)
        )

    def _unique_primitive_count(self) -> int:
        return int(
            self._base.summary()["unique_primitive_count"]
            + self._compact_unique_count
        )

    def summary(self) -> dict[str, Any]:
        base = dict(self._base.summary())
        if not self._compact_blocks:
            return base
        components = {
            name: int(dict(base.get("components", {})).get(name, 0))
            + int(self._compact_unique_components.get(name, 0))
            for name in ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
        }
        unique_count = int(sum(components.values()))
        raw_occurrence_count = self._raw_occurrence_count()
        accumulator = self._unique_accumulator()
        by_scope = {
            str(key): int(value)
            for key, value in dict(
                base.get("unique_primitive_count_by_consumer_scope", {})
            ).items()
        }
        for key, value in self._compact_unique_by_scope.items():
            by_scope[key] = int(by_scope.get(key, 0) + int(value))
        by_branch = {
            str(key): int(value)
            for key, value in dict(
                base.get("unique_primitive_count_by_consumer_branch", {})
            ).items()
        }
        for key, value in self._compact_unique_by_branch.items():
            by_branch[key] = int(by_branch.get(key, 0) + int(value))
        return {
            "schema": str(base.get("schema")),
            "selection": dict(base.get("selection", {})),
            "components": dict(components),
            **components,
            "S_unique": unique_count,
            "unique_primitive_count": unique_count,
            "selected_call_occurrence_count": int(raw_occurrence_count),
            "deduplicated_reuse_occurrence_count": int(
                raw_occurrence_count - unique_count
            ),
            "primitive_ids": [],
            "primitive_ids_complete": False,
            "primitive_id_set_accumulator": _primitive_set_accumulator_payload(
            ...
            "cross_component_reuse_primitive_ids": [],
            "cross_component_reuse_primitive_ids_complete": False,
            "unique_primitive_count_by_consumer_scope": dict(sorted(by_scope.items())),
            "unique_primitive_count_by_consumer_branch": dict(sorted(by_branch.items())),
            "compact_symmetric_family_block_count": int(len(self._compact_blocks)),
        }

    def occurrence_summary(self) -> dict[str, Any]:
        base = dict(self._base.occurrence_summary())
        if not self._compact_blocks:
            return base
        components = {
            name: int(dict(base.get("component_occurrence_counts", {})).get(name, 0))
            for name in ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
        }
        by_scope = {
            str(key): int(value)
            for key, value in dict(
                base.get("occurrence_count_by_consumer_scope", {})
            ).items()
        }
        by_branch = {
            str(key): int(value)
            for key, value in dict(
                base.get("occurrence_count_by_consumer_branch", {})
            ).items()
        }
        for block in self._compact_blocks:
            count = int(block["member_count"])
            components[str(block["component"])] += count
            scope = str(block["consumer_scope"])
            by_scope[scope] = int(by_scope.get(scope, 0) + count)
            branch = (
                "__unbranched__"
                if block.get("branch_id") is None
                else str(block["branch_id"])
            )
            by_branch[branch] = int(by_branch.get(branch, 0) + count)
        total = int(sum(components.values()))
        unique_count = self._unique_primitive_count()
        return {
            "schema": "estimator_call_occurrence_summary_v1",
            "selection": {
                "branch_ids": None,
                "include_unbranched": True,
                "consumer_scopes": None,
                "components": None,
            },
            "component_occurrence_counts": dict(components),
            "components": dict(components),
            **components,
            "S_alg": total,
            "unit": "executed_logical_scalar_estimator_invocation",
            "total_call_occurrences": total,
            "unique_primitive_count": unique_count,
            "same_identity_reuse_occurrence_count": int(total - unique_count),
            ...
            "occurrence_sequences": [],
            "occurrence_sequences_complete": False,
            "occurrence_sequence_ranges": [
                {
                    "start": int(block["occurrence_sequence_start"]),
                    "stop": int(block["occurrence_sequence_stop"]),
                    "count": int(block["member_count"]),
                    "block_sha256": str(block["block_sha256"]),
                }
                for block in self._compact_blocks
    ...
    def accounting_cursor(self) -> dict[str, Any]:
        full_summary = self.summary()
        full_occurrence_summary = self.occurrence_summary()
        summary = _bounded_estimator_summary(full_summary)
        occurrence_summary = _bounded_occurrence_summary(
            full_occurrence_summary
        )
        accumulator = self._unique_accumulator()
        primitive_complete = bool(summary.get("primitive_ids_complete", True))
        occurrence_complete = bool(
            occurrence_summary.get("occurrence_sequences_complete", True)
        )
        cursor = {
            "schema": PAPER_I_COMPARATOR_LEDGER_CURSOR_SCHEMA,
            "occurrence_sequence_stop": int(self._next_sequence - 1),
            "summary": dict(summary),
            "occurrence_summary": dict(occurrence_summary),
            "unique_primitive_set_accumulator": (
                _primitive_set_accumulator_payload(
                    count=accumulator[0],
                    xor_value=accumulator[1],
                    sum_value=accumulator[2],
            ...
            "primitive_ids_complete": bool(primitive_complete),
            "primitive_ids": (
                list(summary.get("primitive_ids", []))
                if primitive_complete
                else []
            ),
            "occurrences_complete": bool(occurrence_complete),
            "occurrences": (
                [dict(row) for row in self._explicit_occurrences]
                if occurrence_complete
                else []
            ),
        }
        cursor["cursor_sha256"] = _canonical_payload_sha256(cursor)
        return cursor

    def accounting_snapshot(self) -> dict[str, Any]:
        cursor = self.accounting_cursor()
        snapshot = {
            "schema": PAPER_I_COMPARATOR_ACCOUNTING_SNAPSHOT_SCHEMA,
            "summary": dict(cursor["summary"]),
            "occurrence_summary": dict(cursor["occurrence_summary"]),
            "unique_primitive_set_accumulator": dict(
                cursor["unique_primitive_set_accumulator"]
            ),
            "occurrence_sequence_stop": int(cursor["occurrence_sequence_stop"]),
            "cursor_sha256": str(cursor["cursor_sha256"]),
        }
        snapshot["snapshot_sha256"] = _canonical_payload_sha256(snapshot)
        return snapshot

    def to_payload(self) -> dict[str, Any]:
        if not self._compact_blocks:
            return self._base.to_payload()
        base_payload = self._base.to_payload()
        summary = self.summary()
        occurrence_summary = self.occurrence_summary()
        payload = {
            "schema": PAPER_I_COMPARATOR_COMPACT_LEDGER_SCHEMA,
            "serialization_mode": "bounded_exact_symmetric_family_v2",
            "compact_family_member_threshold": int(
                _COMPARATOR_COMPACT_FAMILY_MEMBER_THRESHOLD
            ),
            "base_materialized_ledger": base_payload,
            "materialized_occurrence_count": int(
                len(self._explicit_occurrences)
            ),
            "materialized_occurrence_semantic_stream_sha256": (
                _occurrence_semantic_stream_sha256(
                    self._explicit_occurrences
                )
            ),
            "global_occurrence_sequence_stop": int(self._next_sequence - 1),
            "compact_symmetric_family_blocks": [
                dict(block) for block in self._compact_blocks
            ],
            "summary": dict(summary),
            "occurrence_summary": dict(occurrence_summary),
        }
        fingerprint_source = dict(payload)
        payload["ledger_fingerprint"] = _canonical_payload_sha256(
            fingerprint_source
        )
        return payload
    ...
    def validate_payload(payload: Mapping[str, Any]) -> bool:
        data = dict(payload)
        if data.get("schema") != PAPER_I_COMPARATOR_COMPACT_LEDGER_SCHEMA:
            EstimatorCallLedger.from_payload(data)
            return True
        fingerprint = str(data.pop("ledger_fingerprint", ""))
        if fingerprint != _canonical_payload_sha256(data):
            raise ValueError("compact comparator ledger fingerprint mismatch")
        base_payload = data.get("base_materialized_ledger")
        if not isinstance(base_payload, Mapping):
            raise ValueError("compact comparator ledger lacks its base ledger")
        base = EstimatorCallLedger.from_payload(base_payload)
        base_occurrences = list(base_payload.get("occurrences", []))
        if int(data.get("materialized_occurrence_count", -1)) != len(
            base_occurrences
        ):
            raise ValueError("compact comparator materialized count does not close")
        if str(
            data.get("materialized_occurrence_semantic_stream_sha256", "")
        ) != _occurrence_semantic_stream_sha256(base_occurrences):
            raise ValueError(
                "compact comparator materialized occurrence stream hash mismatch"
            )
        blocks = data.get("compact_symmetric_family_blocks", [])
        if not isinstance(blocks, list):
            raise ValueError("compact symmetric-family blocks must be a list")
        compact_unique = 0
        compact_occurrences = 0
        previous_chain = "0" * 64
        previous_stop = 0
        validation_context_sets: dict[str, list[frozenset[str]]] = {}
        base_ids = set(str(value) for value in base.summary()["primitive_ids"])
        for raw_block in blocks:
            if not isinstance(raw_block, Mapping):
                raise ValueError("compact symmetric-family block must be an object")
            block = dict(raw_block)
            observed_chain = str(block.pop("block_chain_sha256", ""))
            observed_previous = str(
                block.pop("previous_block_chain_sha256", "")
            )
            if observed_previous != previous_chain:
                raise ValueError("compact symmetric-family hash chain is broken")
            block_sha = str(block.pop("block_sha256", ""))
            if block_sha != _canonical_payload_sha256(block):
                raise ValueError("compact symmetric-family block hash mismatch")
            expected_chain = _canonical_payload_sha256(
                {
                    "previous_block_chain_sha256": previous_chain,
            ...
            if observed_chain != expected_chain:
                raise ValueError("compact symmetric-family block chain mismatch")
            previous_chain = observed_chain
            member_count = int(block.get("member_count", -1))
            new_count = int(block.get("new_unique_primitive_count", -1))
            if member_count < 0 or new_count < 0 or new_count > member_count:
                raise ValueError("compact symmetric-family counts are invalid")
            member_acc = _accumulator_from_payload(
                block.get("member_primitive_set_accumulator", {})
            )
            new_acc = _accumulator_from_payload(
                block.get("new_unique_primitive_set_accumulator", {})
            )
            if member_acc[0] != member_count or new_acc[0] != new_count:
                raise ValueError("compact symmetric-family accumulators do not close")
            identity_payload = block.get("identity_template")
            coordinates = block.get("coordinates")
            if not isinstance(identity_payload, Mapping) or not isinstance(
                coordinates, list
            ):
                raise ValueError(
                    "compact symmetric-family replay inputs are malformed"
                )
            identity = EstimatorCallKey.from_dict(identity_payload)
            coordinate_rows = tuple(sorted({str(value) for value in coordinates}))
            if len(coordinate_rows) != int(block.get("coordinate_count", -1)):
                raise ValueError(
                    "compact symmetric-family coordinate count does not close"
                )
            if _canonical_payload_sha256(
                {"coordinates": list(coordinate_rows)}
            ) != str(block.get("coordinate_set_sha256", "")):
                raise ValueError(
                    "compact symmetric-family coordinate hash mismatch"
                )
            context_payload = _PaperIComparatorEstimatorLedger._context_payload(
                identity
            )
            context_sha = _canonical_payload_sha256(context_payload)
            if context_sha != str(block.get("context_sha256", "")):
                raise ValueError(
                    "compact symmetric-family identity context hash mismatch"
                )
            prior_sets = validation_context_sets.setdefault(context_sha, [])
            replay_all = (0, 0, 0)
            replay_new = (0, 0, 0)
            for left_offset, left in enumerate(coordinate_rows):
                for right in coordinate_rows[left_offset:]:
                    primitive_id = (
                        _PaperIComparatorEstimatorLedger._pair_primitive_id(
                            identity, left, right
                        )
                    )
                    replay_all = _accumulator_add(replay_all, primitive_id)
                    if (
                        primitive_id not in base_ids
                        and not _PaperIComparatorEstimatorLedger._pair_seen(
                            left, right, prior_sets
                        )
                    ):
                        replay_new = _accumulator_add(
                            replay_new, primitive_id
                        )
            if replay_all != member_acc or replay_new != new_acc:
                raise ValueError(
                    "compact symmetric-family replay receipt mismatch"
                )
            prior_sets.append(frozenset(coordinate_rows))
            start = int(block.get("occurrence_sequence_start", 0))
            stop = int(block.get("occurrence_sequence_stop", 0))
            if (
                start <= previous_stop
                or stop < start
                or stop - start + 1 != member_count
            ):
                raise ValueError(
                    "compact symmetric-family occurrence range is invalid"
                )
            previous_stop = stop
            compact_occurrences += member_count
            compact_unique += new_count
        summary = data.get("summary", {})
        occurrence_summary = data.get("occurrence_summary", {})
        if not isinstance(summary, Mapping) or not isinstance(
            occurrence_summary, Mapping
        ):
            raise ValueError("compact comparator summaries are malformed")
        if int(summary.get("unique_primitive_count", -1)) != int(
            base.summary()["unique_primitive_count"] + compact_unique
        ):
            raise ValueError("compact comparator unique count does not close")
        if int(occurrence_summary.get("total_call_occurrences", -1)) != int(
            base.occurrence_summary()["total_call_occurrences"]
            + compact_occurrences
        ):
            raise ValueError("compact comparator occurrence count does not close")
        if int(data.get("global_occurrence_sequence_stop", -1)) != int(
            occurrence_summary.get("total_call_occurrences", -2)
        ):
            raise ValueError(
                "compact comparator global occurrence sequence does not close"
            )
        return True
...
class _FullMetaCandidatePoolResult:
    ...
    pool_cache_policy: dict[str, Any] | None = None


def _historical_hh_seed_candidates(
    ...
    *,
    ...
    """Resolve the compact quadrature seed used by the successful FM route."""

    boson_bits = int(num_sites) * int(
        boson_qubits_per_site(int(n_ph_max), str(boson_encoding))
    )
    selected: list[_PoolCandidate] = []
    for candidate in pool:
        if not str(candidate.label).startswith(
            "hh_termwise_ham_quadrature_term("
        ):
            continue
        terms = list(candidate.polynomial.return_polynomial())
        if not terms:
            continue
        word = str(terms[0].pw2strng()).lower()
        has_boson_y = any(ch == "y" for ch in word[:boson_bits])
        has_electron_z = "z" in word[boson_bits:]
    ...
    *,
    ...
    if requested not in {
        ...
        raise ValueError(
            ...
            "{bfgs, geo_qngd, powell, rotosolve, spsa}; "
            f"got {requested!r}."
        )
    ...
    spsa_seed_base = _nonnegative_int(
...
def _spec_by_case_id(family: str, case_id: str, algorithm_id: str) -> HamiltonianBenchmarkSpec:
    ...
    if (
        family_key == "hh"
        and case_key
        in set(TABLE_I_PAPER_I_HH_COMPLETION_REGIME_CASE_IDS.values())
    ):
...
def _write_json(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    # Stream through a same-directory temporary file so a large validated
    # result never exists simultaneously as both a Python object graph and one
    # monolithic ``json.dumps`` string.  ``os.replace`` preserves the previous
    # artifact until the new JSON has been flushed completely.
...
def _expand_pool_with_shared_pauli_children(
    *,
    ...
    result = build_shared_pauli_child_pool(
        ...
        n_ph_max=(
            int(context.request.n_ph_max)
            if getattr(context.request, "n_ph_max", None) is not None
            else None
        ),
        boson_encoding=(
            str(context.request.boson_encoding)
            if getattr(context.request, "boson_encoding", None) not in {None, ""}
            else None
        ),
        total_register_width=int(context.layout.total_qubits),
        fixed_num_particles=getattr(
            getattr(context, "sector", None), "num_particles", None
        ),
        max_terms=max_terms,
    )
    converted = tuple(
    ...
    if (
        ...
        and len(converted) != len(result.candidates)
    ):
        raise RuntimeError(
            f"{mode_key} conversion dropped a candidate."
        )
...
def _hh_pool_cache_policy_receipt(
    *,
    context: ResolvedProblemContext,
    pool_key: str,
    filter_resolution: Any,
) -> dict[str, Any]:
    """Serialize the exact requested/effective HH pool-cache policy."""

    requested_mode = str(os.environ.get(_HH_POOL_CACHE_ENV, "disk"))
    requested_scope_raw = str(os.environ.get(_HH_POOL_CACHE_SCOPE_ENV, "exact"))
    effective_mode = str(_hh_pool_presets._hh_pool_cache_mode())
    requested_scope = str(_hh_pool_presets._hh_pool_cache_requested_scope())
    effective_scope = str(
        _hh_pool_presets._hh_pool_cache_effective_scope(
            key=str(pool_key),
            n_ph_max=int(getattr(context.request, "n_ph_max", 0) or 0),
            full_meta_class_filter_spec=getattr(
                filter_resolution, "full_meta_class_filter_spec", None
            ),
        )
    )
    requested_dir_raw = os.environ.get(_HH_POOL_CACHE_DIR_ENV)
    effective_dir = _repo_relative_or_abs(_hh_pool_presets._hh_pool_cache_dir())
    return {
        "schema": "generic_static_hh_pool_cache_policy_receipt_v1",
        "requested_mode": requested_mode,
        "effective_mode": effective_mode,
        "requested_scope_raw": requested_scope_raw,
        "requested_scope": requested_scope,
        "effective_scope": effective_scope,
        "requested_dir": (
            None
            if requested_dir_raw in {None, ""}
            else _repo_relative_or_abs(str(requested_dir_raw))
        ),
...
def _build_full_meta_candidate_pool_with_meta(
    ...
    *,
    ...
    return _FullMetaCandidatePoolResult(
        ...
        pool_cache_policy=_hh_pool_cache_policy_receipt(
            context=context,
            pool_key=str(plan.pool_key),
            filter_resolution=filter_resolution,
        ),
...
def _ground_space_physical_bases_for_context(
    context: ResolvedProblemContext,
) -> tuple[tuple[int, ...], tuple[int, ...], str, str]:
    """Resolve explicit physical bases for post-run projector fidelity only."""

    request = context.request
    family = str(
        getattr(context, "family_key", getattr(request, "problem_key", ""))
    ).strip().lower()
    nq = int(context.layout.total_qubits)
    dimension = 1 << nq
    legal_basis: tuple[int, ...] = tuple(range(dimension))
    ...
    if family in {"hh", "spin_boson", "bose_hubbard", "harmonic_kerr_chain"}:
        ...
        legal = legal_subspace_basis_for_problem(
            problem_key=family,
        ...
        legal_basis = tuple(int(value) for value in legal["legal_indices"])
        legal_label = str(legal["legal_subspace_scope"])

    particles = getattr(context.sector, "num_particles", None)
    if family in {"hubbard", "extended_hubbard", "ttprime_hubbard", "hh"} or (
        particles is not None
        and int(context.layout.fermion_qubits) == nq
        and int(context.layout.fermion_qubits) % 2 == 0
    ):
        if particles is None or len(particles) != 2:
            raise ValueError("fixed_spin_sector_particles_missing")
        ...
        fixed_basis = tuple(
            _spinful_sector_basis_indices(
                n_qubits=nq,
                num_sites=int(context.layout.fermion_qubits // 2),
        ...
        default_particles = getattr(context, "default_num_particles", None)
        if not default_particles:
            raise ValueError("fixed_particle_sector_missing")
        fixed_count = int(default_particles[0])
        fixed_basis = tuple(
            index for index in range(dimension) if int(index).bit_count() == fixed_count
        )
        ...
        fixed_basis = legal_basis
    else:
        raise ValueError(f"unsupported_physical_sector_family:{family}")
    return (
        tuple(fixed_basis),
        tuple(legal_basis),
        str(getattr(context.sector, "label", "physical_sector")),
        str(legal_label),
    )
...
def _dense_exact_state_fidelity_for_selected(
    *,
    ...
    try:
        fixed_basis, legal_basis, fixed_label, legal_label = (
            _ground_space_physical_bases_for_context(context)
        )
        working_cutoff = int(context.request.n_ph_max)
        receipt = evaluate_ground_space_fidelity(
            hamiltonian=hmat,
            variational_state=psi_adapt,
            working_cutoff=working_cutoff,
            reference_cutoff=working_cutoff,
            fixed_sector_basis_indices=fixed_basis,
            legal_binary_basis_indices=legal_basis,
            fixed_sector_label=fixed_label,
            legal_binary_basis_label=legal_label,
        )
    except (GroundSpaceFidelityError, ValueError) as exc:
        blocker = (
            str(exc.code)
            if isinstance(exc, GroundSpaceFidelityError)
            else type(exc).__name__
        )
        return {
            "infidelity_exact": None,
            "exact_state_fidelity": None,
            "infidelity_status": "blocked_ground_space_projector_fidelity",
            "exact_state_fidelity_source": (
                "physical_sector_ground_space_projector_same_cutoff_blocked"
            ),
            "exact_state_fidelity_blocker": blocker,
            "exact_state_fidelity_error": str(exc),
            "exact_state_fidelity_qubit_cap": int(max_qubits),
            "ground_space_fidelity": {
                "schema": "ground_space_projector_fidelity_v1",
                "status": "blocked",
    ...
    ground_energy = float(receipt["ground_energy"])
    return {
        "infidelity_exact": float(max(0.0, 1.0 - fidelity)),
        "exact_state_fidelity": float(fidelity),
        "infidelity_status": "computed_ground_space_projector",
        "exact_state_fidelity_source": (
            "physical_sector_ground_space_projector_same_cutoff"
        ),
        "exact_state_fidelity_reference_convention": receipt.get(
            "reference_convention"
        ),
        "exact_state_fidelity_energy": ground_energy,
        ...
        else float(abs(ground_energy - float(exact_energy))),
        "exact_state_fidelity_s_alg_charged": False,
        "ground_space_fidelity": receipt,
    }
...
def _optimize_selected(
    *,
    ...
    estimator_h_call_recorder: Callable[[np.ndarray, str], None] | None = None,
    ...
    def exact_energy_for(
        ...
        *,
        ...
        if estimator_h_call_recorder is not None:
            estimator_h_call_recorder(psi, str(consumer_scope))
        return float(energy_val)

    eval_count = 0
    exact_objective_energy_by_theta: dict[tuple[int, bytes], float] = {}

    def _theta_result_key(theta_vec: np.ndarray) -> tuple[int, bytes]:
        values = np.ascontiguousarray(
            np.asarray(theta_vec, dtype=np.float64).reshape(-1)
        )
        return int(values.size), values.tobytes()

    def objective(theta_vec: np.ndarray) -> float:
        nonlocal eval_count
        eval_count += 1
        theta_values = np.asarray(theta_vec, dtype=float).reshape(-1)
        exact_energy = exact_energy_for(
            theta_values,
            consumer_scope=f"{method_surface}:objective",
        )
        exact_objective_energy_by_theta[_theta_result_key(theta_values)] = float(
            exact_energy
        )
    ...
    decision_noise_enabled = bool(
        decision_noise_recorder is not None
        and bool(getattr(decision_noise_recorder.config, "enabled", False))
    )
    # Reuse the exact objective already acquired at result.x for every route,
    # including the noiseless comparator.  This avoids the historical
    # unconditional post-Powell verifier and also refuses to trust a pluggable
    # optimizer's stale result.fun.  A genuinely unevaluated endpoint requires
    # one counted verification and remains part of clean algorithmic work.
    post_optimizer_exact_verification_count = 0
    cached_final_exact_energy = exact_objective_energy_by_theta.get(
        _theta_result_key(theta)
    )
    if cached_final_exact_energy is None:
        energy = exact_energy_for(
            theta,
            consumer_scope=(
                f"{method_surface}:required_endpoint_exact_verification"
            ),
        )
        post_optimizer_exact_verification_count = 1
        optimizer_energy_source = "required_endpoint_exact_verification"
    else:
        energy = float(cached_final_exact_energy)
        optimizer_energy_source = "optimizer_objective_exact_value"
    ...
    info = {
        ...
        "optimizer_objective_hamiltonian_occurrence_count": int(eval_count),
        "post_optimizer_exact_verification_hamiltonian_occurrence_count": int(
            post_optimizer_exact_verification_count
        ),
        "optimizer_total_hamiltonian_occurrence_count": int(
            eval_count + post_optimizer_exact_verification_count
        ),
        "optimizer_energy_source": str(optimizer_energy_source),
        "optimizer_decision_surface": method_surface
        if decision_noise_enabled
        else None,
...
def _geo_selector_tangent_span_solve(
    tangents: Sequence[np.ndarray],
    force: np.ndarray,
    *,
    metric_floor: float,
) -> dict[str, Any]:
    """Solve the full candidate Gram pseudoinverse in its real tangent span.

    With real candidate coefficients, realifying the projected complex tangent
    columns gives ``S = A.T @ A``.  If ``A = U diag(s) V.T``, then the exact
    Moore--Penrose selector step is

    ``S^+ f = V_K diag(s_K^-2) V_K.T f``,

    where ``K`` uses the same relative cutoff as ``np.linalg.pinv(S)``.  This
    avoids allocating the candidate-by-candidate Gram matrix while retaining
    the complete candidate coordinate vector and its null-space semantics.
    Rows that are identically zero across every tangent are removed exactly;
    no approximate row, candidate, or singular-mode screening is performed.
    """

    tangent_arrays = [
        np.asarray(tangent, dtype=complex).reshape(-1) for tangent in tangents
    ]
    candidate_count = int(len(tangent_arrays))
    force_arr = np.asarray(force, dtype=float).reshape(-1)
    if force_arr.size != candidate_count:
        raise ValueError(
            "Geo tangent-span force size must equal the candidate count: "
            f"force={force_arr.size}, candidates={candidate_count}."
        )
    if candidate_count <= 0:
        return {
            "step": np.zeros(0, dtype=float),
            "metric_diagonal": np.zeros(0, dtype=float),
            "regularization": float(metric_floor),
            "rank": 0,
            "pinv_supported_rank": 0,
            "condition": None,
            "offdiag_norm": 0.0,
            "scale": 1.0,
            "fs_norm": 0.0,
            ...
            "max_abs_step": 0.0,
            "pinv_rcond": float(max(1e-10, float(metric_floor))),
            "complex_row_count_before_exact_zero_compression": 0,
            "complex_row_count_after_exact_zero_compression": 0,
            "real_row_count_after_exact_zero_compression": 0,
        }

    state_dimension = int(tangent_arrays[0].size)
    if state_dimension <= 0:
        raise ValueError("Geo tangent-span solve received empty tangent vectors.")
    for index, tangent in enumerate(tangent_arrays):
        if int(tangent.size) != state_dimension:
            raise ValueError(
                "Geo tangent vectors must have a common state dimension: "
                f"candidate 0 has {state_dimension}, candidate {index} has "
                f"{tangent.size}."
            )
        if not (
            np.all(np.isfinite(tangent.real))
            and np.all(np.isfinite(tangent.imag))
        ):
            raise ValueError(
                f"Geo tangent vector {index} contains nonfinite values."
            )
    if not np.all(np.isfinite(force_arr)):
        raise ValueError("Geo tangent-span force contains nonfinite values.")

    # The symmetry- and padding-valid pools leave most computational-basis
    # rows exactly zero.  Removing only exact all-zero rows preserves A.T @ A
    # identically while reducing the n_ph=7 selector to its physical support.
    complex_row_mask = np.zeros(state_dimension, dtype=bool)
    for tangent in tangent_arrays:
        complex_row_mask |= (tangent.real != 0.0) | (tangent.imag != 0.0)
    complex_rows = np.flatnonzero(complex_row_mask)
    realified = np.empty(
        (2 * int(complex_rows.size), candidate_count), dtype=float
    )
    for column, tangent in enumerate(tangent_arrays):
        restricted = tangent[complex_rows]
        realified[: complex_rows.size, column] = restricted.real
        realified[complex_rows.size :, column] = restricted.imag
    if realified.size:
        real_row_mask = np.any(realified != 0.0, axis=1)
        tangent_matrix = np.asarray(realified[real_row_mask, :], dtype=float)
    else:
        tangent_matrix = np.zeros((0, candidate_count), dtype=float)

    metric_diagonal = np.einsum(
        "ij,ij->j", tangent_matrix, tangent_matrix, optimize=True
    )
    scale = float(
        max(
            1.0,
            float(np.max(np.abs(metric_diagonal)))
            if metric_diagonal.size
            else 1.0,
        )
    )
    regularization = float(max(metric_floor, metric_floor * scale))
    ...
    if tangent_matrix.shape[0] <= 0:
        step_arr = np.zeros(candidate_count, dtype=float)
        singular_values = np.zeros(0, dtype=float)
        singular_value_sq = np.zeros(0, dtype=float)
        diagnostic_rank = 0
        supported_rank = 0
        condition = 1.0
    else:
        ...
        singular_value_sq = singular_values * singular_values
        diagnostic_tol = float(max(1e-12, regularization))
        diagnostic_rank = int(np.count_nonzero(singular_value_sq > diagnostic_tol))
        if singular_value_sq.size:
            pinv_cutoff = float(pinv_rcond * singular_value_sq[0])
            supported = singular_value_sq > pinv_cutoff
        else:
            supported = np.zeros(0, dtype=bool)
        supported_rank = int(np.count_nonzero(supported))
        if supported_rank:
            supported_right = np.asarray(right_vectors_h[supported, :], dtype=float)
            projected_force = supported_right @ force_arr
            step_arr = supported_right.T @ (
                projected_force / singular_value_sq[supported]
            )
        else:
            step_arr = np.zeros(candidate_count, dtype=float)

        largest_metric_eigenvalue = (
            float(singular_value_sq[0]) if singular_value_sq.size else 0.0
        )
        if candidate_count > int(singular_value_sq.size):
            smallest_metric_eigenvalue = 0.0
        elif singular_value_sq.size:
            smallest_metric_eigenvalue = float(singular_value_sq[-1])
        else:
            smallest_metric_eigenvalue = 0.0
        condition = float(
            (largest_metric_eigenvalue + regularization)
            / (smallest_metric_eigenvalue + regularization)
        )
        if not math.isfinite(condition):
            condition = None

    metric_frobenius_sq = float(np.sum(singular_value_sq * singular_value_sq))
    diagonal_frobenius_sq = float(np.sum(metric_diagonal * metric_diagonal))
    offdiag_norm = float(
        math.sqrt(max(0.0, metric_frobenius_sq - diagonal_frobenius_sq))
    )
    fs_norm = (
        float(np.linalg.norm(tangent_matrix @ step_arr))
        if step_arr.size
        else 0.0
    )
    l2_norm = float(np.linalg.norm(step_arr)) if step_arr.size else 0.0
    max_abs_step = (
        float(np.max(np.abs(step_arr))) if step_arr.size else 0.0
    )
    return {
        "step": np.asarray(step_arr, dtype=float).reshape(-1),
        "metric_diagonal": np.asarray(metric_diagonal, dtype=float).reshape(-1),
        "regularization": regularization,
        "rank": int(diagnostic_rank),
        "pinv_supported_rank": int(supported_rank),
        "condition": condition,
        ...
        "complex_row_count_before_exact_zero_compression": state_dimension,
        "complex_row_count_after_exact_zero_compression": int(
            complex_rows.size
        ),
        "real_row_count_after_exact_zero_compression": int(
            tangent_matrix.shape[0]
        ),
    }
...
def _score_candidates(
    *,
    ...
    if _is_geo_config(config):
        ...
        span_solve = _geo_selector_tangent_span_solve(
            tangents,
            force,
            metric_floor=float(metric_floor),
        )
        step_arr = np.asarray(span_solve["step"], dtype=float).reshape(-1)
        regularization = float(span_solve["regularization"])
        pinv_rcond = float(span_solve["pinv_rcond"])
        rank_deficient = int(span_solve["rank"]) < int(len(candidates))
        selector_scores = [float(abs(x)) for x in step_arr]
        metric_variances = [
            float(value)
            for value in np.asarray(
                span_solve["metric_diagonal"], dtype=float
            ).reshape(-1)
        ]
        geo_steps = [float(x) for x in step_arr]
        geo_forces = [float(x) for x in force.reshape(-1)]
        geo_step_fs_norm = float(span_solve["fs_norm"])
        geo_step_l2_norm = float(span_solve["l2_norm"])
        geo_max_abs_step = float(span_solve["max_abs_step"])
        # Equivalent projected singleton generators can differ only by an
        # operator sign or provenance label.  Algebraically their absolute
        # Moore--Penrose components are tied, but different exact
        # factorizations may separate those values by a few floating-point
        # ulps.  Keep the recorded scores untouched and stabilize only the
        # noise-free ordering at a machine-precision scale.
        geo_numerical_tie_abs_tol = float(
            64.0
            * np.finfo(float).eps
            * max(1.0, float(np.max(np.abs(step_arr))) if step_arr.size else 0.0)
        )
        geo_diag = {
            "geo_selector_mode": _geo_selector_mode_for_config(config),
            **geo_screen_diag,
            "geo_metric_rank": int(span_solve["rank"]),
            "geo_metric_condition": span_solve["condition"],
            ...
            "geo_metric_pinv_supported_rank": int(
                span_solve["pinv_supported_rank"]
            ),
            "geo_metric_offdiag_norm": float(span_solve["offdiag_norm"]),
            "geo_metric_rank_deficient": bool(rank_deficient),
            "geo_metric_diagonal_fallback": False,
            "geo_selector_numerical_tie_policy": "machine_precision_collinear_alias_then_label_v1",
            "geo_selector_numerical_tie_abs_tol": geo_numerical_tie_abs_tol,
            "geo_tangent_complex_row_count_before_exact_zero_compression": int(
                span_solve[
                    "complex_row_count_before_exact_zero_compression"
                ]
            ),
            "geo_tangent_complex_row_count_after_exact_zero_compression": int(
                span_solve[
                    "complex_row_count_after_exact_zero_compression"
                ]
            ),
            "geo_tangent_real_row_count_after_exact_zero_compression": int(
                span_solve["real_row_count_after_exact_zero_compression"]
            ),
    ...
    for idx, candidate in enumerate(candidates):
        ...
        scored.append(row)
    score_value = lambda row: float(  # noqa: E731
        row.get("selector_score_decision", row.get("selector_score", 0.0))
    )
    scored.sort(key=lambda row: (-score_value(row), str(row["label"])))
    if _is_geo_config(config) and not decision_enabled and scored:
        tie_tol = float(
            scored[0].get("geo_selector_numerical_tie_abs_tol", 0.0) or 0.0
        )
        top_score = score_value(scored[0])
        top_stop = 1
        while (
            top_stop < len(scored)
            and abs(score_value(scored[top_stop]) - top_score) <= tie_tol
        ):
            top_stop += 1
        if top_stop > 1:
            tangent_by_label = {
                str(candidate.label): np.asarray(tangent, dtype=complex).reshape(-1)
                for candidate, tangent in zip(candidates, tangents, strict=True)
            }
            alias_clusters: list[list[dict[str, Any]]] = []
            alias_representatives: list[np.ndarray] = []
            for row in scored[:top_stop]:
                tangent = tangent_by_label[str(row["label"])]
                tangent_norm = float(np.linalg.norm(tangent))
                assigned = False
                for cluster_index, representative in enumerate(alias_representatives):
                    representative_norm = float(np.linalg.norm(representative))
                    norm_tol = float(
                        256.0
                        * np.finfo(float).eps
                        * max(1.0, tangent_norm, representative_norm)
                    )
                    inner = complex(np.vdot(representative, tangent))
                    product = float(representative_norm * tangent_norm)
                    direction_tol = float(
                        256.0 * np.finfo(float).eps * max(1.0, product)
                    )
                    same_real_direction = (
                        abs(tangent_norm - representative_norm) <= norm_tol
                        and abs(inner.imag) <= direction_tol
                        and abs(abs(inner.real) - product) <= direction_tol
                    )
                    if same_real_direction:
                        alias_clusters[cluster_index].append(row)
                        assigned = True
                        break
                if not assigned:
                    alias_representatives.append(tangent)
                    alias_clusters.append([row])
            scored[:top_stop] = [
                row
                for cluster in alias_clusters
                for row in sorted(cluster, key=lambda item: str(item["label"]))
            ]
    return scored
...
def _diagnostic_proxy_stats(
    ...
    return {"diagnostic_pauli_rotation_proxy_stats": _compiled_proxy_stats(selected)}
...
def _qiskit_compiled_stats_for_selected(
    *,
    ...
    diagnostic = _diagnostic_proxy_stats(selected)
...
def _paper_i_executed_s_alg_work(
    occurrence_summary: Mapping[str, Any],
    *,
    scope: str,
) -> dict[str, Any]:
    """Normalize executed estimator occurrences into the Paper-I S_alg unit."""

    component_names = ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
    serialized_components = occurrence_summary.get(
        "component_occurrence_counts",
        occurrence_summary.get("components", {}),
    )
    if not isinstance(serialized_components, Mapping):
        raise RuntimeError("Paper-I comparator occurrence components are malformed.")
    components = {
        component: int(serialized_components.get(component, 0))
        for component in component_names
    }
    if any(value < 0 for value in components.values()):
        raise RuntimeError(
            "Paper-I comparator executed estimator counts must be nonnegative."
        )
    s_alg = int(occurrence_summary.get("total_call_occurrences", -1))
    if s_alg < 0 or s_alg != int(sum(components.values())):
        raise RuntimeError(
            "Paper-I comparator executed estimator occurrences do not close."
        )
    s_unique = int(occurrence_summary.get("unique_primitive_count", -1))
    if s_unique < 0 or s_unique > s_alg:
        raise RuntimeError(
            "Paper-I comparator unique-identity diagnostic is inconsistent."
        )
    return {
...
def _paper_i_comparator_round_receipt(
    *,
    ledger: _PaperIComparatorEstimatorLedger,
    before_payload: Mapping[str, Any],
    iteration: int,
    outcome: str,
    receipt_kind: str = "adaptive_round",
) -> dict[str, Any]:
    """Close one comparator round against the cumulative state-keyed ledger."""

    before = dict(before_payload)
    if before.get("schema") != PAPER_I_COMPARATOR_LEDGER_CURSOR_SCHEMA:
        raise RuntimeError("Paper-I comparator estimator prefix is not a cursor.")
    before_hash_source = dict(before)
    before_hash = str(before_hash_source.pop("cursor_sha256", ""))
    if before_hash != _canonical_payload_sha256(before_hash_source):
        raise RuntimeError("Paper-I comparator estimator prefix hash is invalid.")
    after = ledger.accounting_cursor()
    before_occurrence_count = int(
        dict(before.get("occurrence_summary", {})).get(
            "total_call_occurrences", 0
        )
    )
    after_occurrence_count = int(
        dict(after.get("occurrence_summary", {})).get(
            "total_call_occurrences", 0
        )
    )
    if after_occurrence_count < before_occurrence_count:
        raise RuntimeError("Paper-I comparator estimator occurrence count regressed.")
    occurrences_complete = bool(before.get("occurrences_complete")) and bool(
        after.get("occurrences_complete")
    )
    before_occurrences = before.get("occurrences", [])
    after_occurrences = after.get("occurrences", [])
    if not isinstance(before_occurrences, list) or not isinstance(after_occurrences, list):
        raise RuntimeError("Paper-I comparator estimator occurrences are malformed.")
    if occurrences_complete:
        if after_occurrences[: len(before_occurrences)] != before_occurrences:
            raise RuntimeError("Paper-I comparator estimator ledger lost its round prefix.")
        round_occurrences = [
            dict(row) for row in after_occurrences[len(before_occurrences) :]
        ]
    else:
        round_occurrences = []
    expected_sequences = list(
        range(before_occurrence_count + 1, after_occurrence_count + 1)
    )
    observed_sequences = [
        int(row.get("sequence", 0)) for row in round_occurrences
    ]
    if occurrences_complete and observed_sequences != expected_sequences:
        raise RuntimeError("Paper-I comparator round estimator sequences are not contiguous.")

    before_summary = before.get("summary", {})
    after_summary = after.get("summary", {})
    if not isinstance(before_summary, Mapping) or not isinstance(after_summary, Mapping):
        raise RuntimeError("Paper-I comparator estimator summaries are malformed.")
    component_names = ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
    unique_components = {
        component: int(dict(after_summary.get("components", {})).get(component, 0))
        - int(dict(before_summary.get("components", {})).get(component, 0))
        for component in component_names
    }
    if any(value < 0 for value in unique_components.values()):
        raise RuntimeError("Paper-I comparator component totals regressed.")
    primitive_ids_complete = bool(before.get("primitive_ids_complete")) and bool(
        after.get("primitive_ids_complete")
    )
    if primitive_ids_complete:
        before_unique_ids = {
            str(value) for value in before.get("primitive_ids", [])
        }
        after_unique_ids = {
            str(value) for value in after.get("primitive_ids", [])
        }
        if not before_unique_ids.issubset(after_unique_ids):
            raise RuntimeError(
                "Paper-I comparator estimator ledger lost a charged primitive."
            )
        new_unique_ids = sorted(after_unique_ids.difference(before_unique_ids))
    else:
        new_unique_ids = []
    before_accumulator = _accumulator_from_payload(
        before.get("unique_primitive_set_accumulator", {})
    )
    after_accumulator = _accumulator_from_payload(
        after.get("unique_primitive_set_accumulator", {})
    )
    new_accumulator = _accumulator_delta(
        after_accumulator, before_accumulator
    )
    new_unique_count = int(sum(unique_components.values()))
    if new_accumulator[0] != new_unique_count:
        raise RuntimeError("Paper-I comparator round primitive digest count does not close.")
    if primitive_ids_complete and len(new_unique_ids) != new_unique_count:
        raise RuntimeError("Paper-I comparator materialized primitive delta does not close.")
    raw_components = {
        component: int(
            dict(after.get("occurrence_summary", {})).get(
                "component_occurrence_counts", {}
    ...
    if any(value < 0 for value in raw_components.values()):
        raise RuntimeError("Paper-I comparator raw occurrence totals regressed.")
    raw_occurrence_count = int(after_occurrence_count - before_occurrence_count)
    if sum(raw_components.values()) != raw_occurrence_count:
        raise RuntimeError("Paper-I comparator raw round components do not close.")

    receipt = {
        "schema": PAPER_I_COMPARATOR_ROUND_RECEIPT_SCHEMA,
        "receipt_kind": str(receipt_kind),
        "iteration": int(iteration),
        "outcome": str(outcome),
        "occurrence_sequence_start": (
            None if raw_occurrence_count == 0 else int(before_occurrence_count + 1)
        ),
        "occurrence_sequence_stop": (
            None if raw_occurrence_count == 0 else int(after_occurrence_count)
        ),
        "raw_occurrence_count": raw_occurrence_count,
        "raw_occurrence_components": raw_components,
        "executed_S_alg_delta": int(raw_occurrence_count),
        "new_unique_primitive_count": new_unique_count,
        "new_unique_primitive_ids": new_unique_ids,
        "unique_component_delta": unique_components,
        "S_alg_delta": int(raw_occurrence_count),
        "S_unique_delta": int(sum(unique_components.values())),
        "cumulative_unique_summary": dict(after_summary),
        "cumulative_occurrence_summary": dict(
            after.get("occurrence_summary", {})
        ),
        "cumulative_executed_summary": {
            "components": {
                component: int(
                    dict(after.get("occurrence_summary", {}))
                    .get("component_occurrence_counts", {})
                    .get(component, 0)
                )
                for component in component_names
            },
            "S_alg": int(after_occurrence_count),
            "unit": "executed_logical_scalar_estimator_invocation",
        },
        ...
        "prefix_occurrence_count_before": before_occurrence_count,
        "prefix_occurrence_count_after": after_occurrence_count,
        "prefix_closed": True,
    }
    if not primitive_ids_complete or not occurrences_complete:
        receipt.update(
            {
                "serialization_mode": "compact_exact_receipt_v2",
                "new_unique_primitive_ids_complete": bool(
                    primitive_ids_complete
                ),
                "new_unique_primitive_set_accumulator": (
                    _primitive_set_accumulator_payload(
                        count=new_accumulator[0],
                ...
                "round_occurrences_complete": bool(occurrences_complete),
                "round_occurrence_contract_sha256": _canonical_payload_sha256(
                    {
                        "receipt_kind": str(receipt_kind),
                        "iteration": int(iteration),
                        "outcome": str(outcome),
                        "occurrence_sequence_start": receipt[
                            "occurrence_sequence_start"
                        ],
                        "occurrence_sequence_stop": receipt[
                            "occurrence_sequence_stop"
                        ],
                        "raw_occurrence_count": raw_occurrence_count,
                        "raw_occurrence_components": raw_components,
...
def _paper_i_comparator_estimator_accounting(
    *,
    ledger: _PaperIComparatorEstimatorLedger | None = None,
    accounting_snapshot: Mapping[str, Any] | None = None,
    round_receipts: Sequence[Mapping[str, Any]],
    legacy_component_fields: Mapping[str, Any],
    require_terminal_receipt: bool = False,
    terminal_diagnostic_queries_excluded: bool = False,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Build canonical comparator accounting and compatibility component fields."""

    if (ledger is None) == (accounting_snapshot is None):
        raise ValueError(
            "exactly one of ledger or accounting_snapshot must be supplied"
        )
    if ledger is not None:
        full_ledger = ledger.to_payload()
    else:
        full_ledger = dict(accounting_snapshot or {})
        if (
            full_ledger.get("schema")
            != PAPER_I_COMPARATOR_ACCOUNTING_SNAPSHOT_SCHEMA
        ):
            raise RuntimeError("unsupported comparator accounting snapshot")
        snapshot_hash_source = dict(full_ledger)
        snapshot_hash = str(
            snapshot_hash_source.pop("snapshot_sha256", "")
        )
        if snapshot_hash != _canonical_payload_sha256(snapshot_hash_source):
            raise RuntimeError("comparator accounting snapshot hash mismatch")
    summary = dict(full_ledger["summary"])
    occurrence_summary = dict(full_ledger["occurrence_summary"])
    component_names = ("N_H_outer", "N_H_refit", "N_grad", "N_metric")
    round_rows = [dict(row) for row in round_receipts]
    terminal_receipts = [
        row
        for row in round_rows
        if str(row.get("receipt_kind", "adaptive_round"))
        == "terminal_diagnostic"
    ]
    if bool(require_terminal_receipt) and len(terminal_receipts) != 1:
        raise RuntimeError(
            "Paper-I comparator terminal accounting requires exactly one "
            "terminal diagnostic receipt."
        )
    if bool(terminal_diagnostic_queries_excluded) and terminal_receipts:
        raise RuntimeError(
            "Paper-I comparator cannot both exclude and record terminal diagnostics."
        )
    round_unique_ids_complete = bool(
        summary.get("primitive_ids_complete", True)
        and all(
            bool(row.get("new_unique_primitive_ids_complete", True))
            for row in round_rows
        )
    )
    round_unique_ids = [
        str(primitive_id)
        for row in round_rows
        for primitive_id in row.get("new_unique_primitive_ids", [])
    ]
    if round_unique_ids_complete:
        if len(round_unique_ids) != len(set(round_unique_ids)):
            raise RuntimeError("Paper-I comparator per-round unique primitive sets overlap.")
        if set(round_unique_ids) != set(
            str(value) for value in summary.get("primitive_ids", [])
        ):
            raise RuntimeError("Paper-I comparator per-round unique primitives do not close.")
    else:
        round_accumulator = (0, 0, 0)
        for row in round_rows:
            serialized = row.get("new_unique_primitive_set_accumulator")
            if isinstance(serialized, Mapping):
                receipt_accumulator = _accumulator_from_payload(serialized)
            else:
                receipt_accumulator = (0, 0, 0)
                for primitive_id in row.get("new_unique_primitive_ids", []):
                    receipt_accumulator = _accumulator_add(
                        receipt_accumulator, str(primitive_id)
                    )
                if receipt_accumulator[0] != int(
                    row.get("new_unique_primitive_count", -1)
                ):
                    raise RuntimeError(
                        "compact comparator round lacks its primitive-set receipt"
                    )
            round_accumulator = _accumulator_merge(
                round_accumulator,
                receipt_accumulator,
            )
        if round_accumulator != _summary_primitive_accumulator(summary):
            raise RuntimeError(
                "Paper-I comparator compact per-round primitive receipts do not close."
            )
    raw_round_total = int(sum(int(row.get("raw_occurrence_count", 0)) for row in round_rows))
    if raw_round_total != int(occurrence_summary.get("total_call_occurrences", 0)):
        raise RuntimeError("Paper-I comparator per-round raw occurrences do not close.")
    unique_component_sum = {
        component: int(
            sum(
                int(dict(row.get("unique_component_delta", {})).get(component, 0))
                for row in round_rows
            )
        )
        for component in component_names
    }
    if unique_component_sum != {
        component: int(dict(summary.get("components", {})).get(component, 0))
        for component in component_names
    }:
        raise RuntimeError("Paper-I comparator per-round component totals do not close.")

    serialized_unique_summary = _bounded_estimator_summary(summary)
    serialized_occurrence_summary = _bounded_occurrence_summary(
        occurrence_summary
    )
    executed_work = _paper_i_executed_s_alg_work(
        occurrence_summary,
        scope="current_execution_segment_all_branch_search_work",
    )
    components = dict(executed_work["components"])
    s_unique = int(
        summary.get("S_unique", summary.get("unique_primitive_count", -1))
    )
    if s_unique != int(summary.get("unique_primitive_count", -2)):
        raise RuntimeError(
            "Paper-I comparator S_unique does not close to its identity set."
        )
    compatibility_components = {
        "N_H_outer_eval": float(components.get("N_H_outer", 0)),
        "N_H_refit_eval": float(components.get("N_H_refit", 0)),
        "N_grad_probe": float(components.get("N_grad", 0)),
        "N_metric_probe": float(components.get("N_metric", 0)),
        "N_other_quantum": 0.0,
    }
    s_alg = float(executed_work["S_alg"])
    canonical_component_fields = {
        "S_alg": s_alg,
        "S_unique": float(s_unique),
        "S_alg_N_H_outer_eval": compatibility_components["N_H_outer_eval"],
        "S_alg_N_H_refit_eval": compatibility_components["N_H_refit_eval"],
        "S_alg_N_grad_probe": compatibility_components["N_grad_probe"],
        "S_alg_N_metric_probe": compatibility_components["N_metric_probe"],
        "S_alg_N_other_quantum": 0.0,
        "algorithmic_measurement_work_N_H_outer_eval": compatibility_components[
            "N_H_outer_eval"
        ],
        "algorithmic_measurement_work_N_H_refit_eval": compatibility_components[
            "N_H_refit_eval"
        ],
        "algorithmic_measurement_work_N_grad_probe": compatibility_components[
            "N_grad_probe"
        ],
        "algorithmic_measurement_work_N_metric_probe": compatibility_components[
            "N_metric_probe"
        ],
        "algorithmic_measurement_work_N_other_quantum": 0.0,
        "S_norm": s_alg,
        "S_norm_N_H_outer_eval": compatibility_components["N_H_outer_eval"],
        "S_norm_N_H_refit_eval": compatibility_components["N_H_refit_eval"],
        "S_norm_N_grad": compatibility_components["N_grad_probe"],
        "S_norm_N_metric": compatibility_components["N_metric_probe"],
        "S_norm_N_H_eval": compatibility_components["N_H_outer_eval"],
        "S_norm_N_refit_eval": compatibility_components["N_H_refit_eval"],
        "S_norm_N_other_quantum": 0.0,
        "N_H_outer_eval": compatibility_components["N_H_outer_eval"],
        "N_H_refit_eval": compatibility_components["N_H_refit_eval"],
        "N_grad": compatibility_components["N_grad_probe"],
        "N_metric": compatibility_components["N_metric_probe"],
        "N_other_quantum": 0.0,
        "metric_fraction": (
            float(components.get("N_metric", 0)) / s_alg if s_alg > 0.0 else 0.0
        ),
        "table_i_measurement_event_ledger": {
            "schema": TABLE_I_EVENT_LEDGER_SCHEMA,
            "status": "ok",
            "source_kind": "state_keyed_estimator_call_ledger_v1",
            "component_totals": compatibility_components,
            "event_count_convention": (
                "required_executed_logical_scalar_estimator_invocations_v1"
            ),
            "cache_policy": (
                "no_persistent_cache_deduction_with_already_acquired_"
                "optimizer_endpoint_reuse_v1"
            ),
            "persistent_or_prior_run_cache_reductions_allowed": False,
            "already_acquired_optimizer_endpoint_reuse_allowed": True,
            "measurement_model_id": "noiseless_estimator_schedule_count_v1",
            "executed_summary": dict(executed_work),
            "unique_identity_diagnostic": dict(serialized_unique_summary),
            "raw_occurrence_summary": dict(serialized_occurrence_summary),
            "N_other_quantum": 0.0,
        },
    }
    for compatibility_key in (
        "N_metric_selector_probe",
        "N_metric_qngd_refit_probe",
        "N_metric_position_trial_probe",
        "N_metric_residual_probe",
        "N_metric_split_status",
        "N_grad_selector_probe",
        "N_grad_qngd_refit_probe",
        "N_grad_position_trial_probe",
        "N_grad_residual_probe",
        "N_grad_split_status",
    ):
        if compatibility_key in legacy_component_fields:
            canonical_component_fields[compatibility_key] = legacy_component_fields[
                compatibility_key
            ]
    canonical_component_fields["table_i_measurement_event_ledger"].update(
        {
            "legacy_metric_split_totals": {
                key: legacy_component_fields.get(key)
                for key in (
                    "N_metric_selector_probe",
                    "N_metric_qngd_refit_probe",
                    "N_metric_position_trial_probe",
    ...
    accounting = {
        "schema": PAPER_I_COMPARATOR_ESTIMATOR_ACCOUNTING_SCHEMA,
        ...
        "accounting_scope": "current_execution_segment",
        "components": dict(components),
        "S_alg": int(executed_work["S_alg"]),
        "S_unique": int(s_unique),
        "all_branch_search_work": dict(executed_work),
        "winning_lineage": dict(executed_work),
        "all_branch_unique_primitive_diagnostic": dict(
            serialized_unique_summary
        ),
        "winning_lineage_unique_primitive_diagnostic": dict(
            serialized_unique_summary
        ),
        "discarded_branch_only_by_unique_set_difference": {
            "components": {component: 0 for component in component_names},
            "S_unique": 0,
            "unique_primitive_count": 0,
            "primitive_ids": [],
        },
        "single_lineage_partition": {
            "winning_equals_total": True,
            "discarded_work_is_zero": True,
            "position_trial_branches_present": False,
        },
        "executed_occurrence_accounting": {
            ...
            "all_execution": dict(serialized_occurrence_summary),
        },
        "per_round_receipts": round_rows,
        "per_round_unique_primitive_count_sum": int(
            sum(
                int(row.get("new_unique_primitive_count", 0))
                for row in round_rows
            )
        ),
        "per_round_raw_occurrence_count_sum": int(raw_round_total),
        "per_round_receipts_close_to_terminal": True,
        "per_round_primitive_ids_materialized": bool(
            round_unique_ids_complete
        ),
        "per_round_compact_exact_receipts_v2": bool(
            not round_unique_ids_complete
        ),
        "terminal_diagnostic_queries_excluded": bool(
            terminal_diagnostic_queries_excluded
        ),
        "terminal_diagnostic_queries_included": bool(terminal_receipts),
        "terminal_diagnostic_receipt_count": int(len(terminal_receipts)),
        "terminal_diagnostic_receipt_required": bool(require_terminal_receipt),
        "terminal_diagnostic_query_policy": (
            "recorded_as_executed_logical_scalar_estimator_invocations_v1"
            if terminal_receipts
            else "excluded_as_post_prefix_dead_work_for_fixed_horizon_v1"
            if terminal_diagnostic_queries_excluded
            else "not_present_in_prefix_accounting_scope"
        ),
        "exact_diagonalization_and_fidelity_queries_excluded": True,
        "legacy_aggregate_measurement_work_proxy": dict(legacy_component_fields),
        "full_ledger": full_ledger,
    }
    return canonical_component_fields, accounting
...
def _resolve_cutoff_provenance(
    *,
    spec: HamiltonianBenchmarkSpec,
    context: ResolvedProblemContext,
    exact_reference_n_ph_max: int | str | None,
) -> dict[str, Any]:
    """Return one normalized working/reference phonon-cutoff receipt."""
    ...
    raw_reference = exact_reference_n_ph_max
    if raw_reference in {None, ""}:
        raw_reference = getattr(spec, "exact_reference_n_ph_max", None)
    n_ph_reference = (
        int(float(str(raw_reference))) if raw_reference not in {None, ""} else None
    )
    same_cutoff_reference = bool(
        n_ph_work is not None
        and n_ph_reference is not None
        and int(n_ph_work) == int(n_ph_reference)
    )
    if same_cutoff_reference:
        reference_role = "same_cutoff_exact_reference"
    elif n_ph_reference is not None:
        reference_role = "different_cutoff_exact_reference"
    else:
        reference_role = None
    return {
        "resolved_exact_reference_n_ph_max": n_ph_reference,
        "n_ph_work": n_ph_work,
        "n_ph_reference": n_ph_reference,
        "exact_reference_n_ph_max": n_ph_reference,
        "same_cutoff_reference": same_cutoff_reference,
        "cutoff_pair": {
            "n_ph_work": n_ph_work,
            "n_ph_ref": n_ph_reference,
...
def _classify_powell_maxiter_cap(
    *,
    ...
    out = dict(opt_info)
    raw_success = bool(out.get("raw_success", out.get("success", False)))
    ...
    effective_success = bool(
        (raw_success or optimizer_capped_accepted)
        if is_powell
        else out.get("success", False)
    )
...
def _paper_i_comparator_active_prefix_checkpoint(
    *,
    algorithm_id: str,
    iteration: int,
    selected: Sequence[_PoolCandidate],
    ...
    parameterization_layout: AnsatzParameterLayout,
    psi_ref: np.ndarray,
    psi_prepared: np.ndarray,
    sector_padding_audit: Mapping[str, Any],
) -> dict[str, Any]:
    """Serialize replay-complete accepted-prefix metadata without a statevector."""

    theta_arr = np.asarray(theta, dtype=float).reshape(-1)
    mode = str(parameterization_mode).strip().lower()
    if mode.startswith("per_pauli"):
        if int(theta_arr.size) != int(parameterization_layout.runtime_parameter_count):
            raise ValueError(
                "Paper-I comparator active-prefix runtime theta/layout mismatch."
            )
        runtime_theta = np.asarray(theta_arr, dtype=float)
        logical_theta = np.asarray(
            project_runtime_theta_block_mean(runtime_theta, parameterization_layout),
            dtype=float,
        )
    else:
        if int(theta_arr.size) != int(parameterization_layout.logical_parameter_count):
            raise ValueError(
                "Paper-I comparator active-prefix logical theta/layout mismatch."
            )
        logical_theta = np.asarray(theta_arr, dtype=float)
        runtime_theta = np.asarray(
            expand_legacy_logical_theta(logical_theta, parameterization_layout),
            dtype=float,
        )
    ...
    payload: dict[str, Any] = {
        "schema": PAPER_I_COMPARATOR_ACTIVE_PREFIX_CHECKPOINT_SCHEMA,
        "algorithm_id": str(algorithm_id),
        "iteration": int(iteration),
        "outer_iteration": int(iteration) + 1,
        "active_ansatz_depth": int(len(selected)),
        "active_logical_coordinate_count": int(len(selected)),
        "active_operator_order": [str(candidate.label) for candidate in selected],
        "active_operators": operators,
        "parameterization_mode": str(parameterization_mode),
        "parameterization_layout": serialize_layout(parameterization_layout),
        "logical_parameter_count": int(parameterization_layout.logical_parameter_count),
        "runtime_parameter_count": int(parameterization_layout.runtime_parameter_count),
        "logical_theta": [float(value) for value in logical_theta.tolist()],
        "runtime_theta": [float(value) for value in runtime_theta.tolist()],
        "ansatz_input_state_projective_fingerprint": (
            estimator_projective_state_fingerprint(
                np.asarray(psi_ref, dtype=complex).reshape(-1)
            )
        ),
        "prepared_state_projective_fingerprint": (
            estimator_projective_state_fingerprint(
                np.asarray(psi_prepared, dtype=complex).reshape(-1)
            )
        ),
        "sector_padding_audit": dict(sector_padding_audit),
        "reporting_replay_metadata_only": True,
        "statevector_serialized": False,
        "refit_performed_for_checkpoint": False,
    }
...
def _run_impl(
    *,
    ...
    adapt_spsa_big_a: float | str | None = None,
    optimizer_overlay_source: str | None = None,
    hh_seed_preopt_enabled: bool = False,
    hh_adaptive_pool_profile: str | None = None,
    hh_full_meta_class_filter_json: str | Path | None = None,
) -> dict[str, Any]:
    family_key = str(family).strip()
    ...
    minimize_fn = None
    if _requires_scipy_minimize(config) or bool(hh_seed_preopt_enabled):
    ...
    num_qubits = int(context.layout.total_qubits)
    cutoff_provenance = _resolve_cutoff_provenance(
        spec=spec,
        context=context,
        exact_reference_n_ph_max=exact_reference_n_ph_max,
    )
    n_ph_work = cutoff_provenance["n_ph_work"]
    n_ph_reference = cutoff_provenance["n_ph_reference"]
    resolved_exact_reference_n_ph_max = cutoff_provenance[
        "resolved_exact_reference_n_ph_max"
    ]
    same_cutoff_reference = bool(cutoff_provenance["same_cutoff_reference"])
    cutoff_pair = dict(cutoff_provenance["cutoff_pair"])
    ...
    if shared_pool_mode == SHARED_PAULI_POOL_MODE_PROJECTED_SINGLETON_CHILDREN_ONLY_V1:
        if selected_logical_requested:
            raise ValueError(
                "projected_singleton_children_only_v1 requires the complete "
                "unfiltered full_meta parent pool; selected-logical filtering is forbidden."
            )
        if family_key == "hh" and (
            effective_hh_adaptive_pool_profile
            != _HH_ADAPTIVE_POOL_PROFILE_FULL_META_UNFILTERED
            or hh_full_meta_class_filter_json is not None
        ):
            raise ValueError(
                "projected_singleton_children_only_v1 requires "
                "hh_adaptive_pool_profile=full_meta_unfiltered with no class filter."
            )
    try:
        if config.pool_kind == "full_meta":
            if selected_logical_requested:
                ...
                if (
                    hh_full_meta_class_filter_json is None
                    and not _is_hh_context(context)
                ):
                    # Preserve the historical generic Hubbard/test hook.  The
                    # cache-policy receipt is an HH-only contract and must not
                    # reroute other full_meta builders through HH internals.
                    pool = build_full_meta_candidate_pool(
                        context,
                        max_terms=pool_term_cap,
                    )
                    pool_result = None
                elif hh_full_meta_class_filter_json is None:
            ...
            if pool_result is not None:
                ...
                pool_cache_policy = (
                    dict(pool_result.pool_cache_policy)
                    if isinstance(pool_result.pool_cache_policy, Mapping)
                    else None
                )
        else:
    ...
    paper_i_powell_comparator_accounting_enabled = bool(
        config.algorithm_id
        in {STATIC_GEO_ADAPT_VQE, STATIC_FULL_META_APPEND_ADAPT_VQE}
        and str(config.optimizer_kind) == "powell"
    )
    if (
        paper_i_powell_comparator_accounting_enabled
        and str(config.position_policy) != "append"
    ):
        raise ValueError(
            "Paper-I comparator state-keyed estimator accounting currently "
            "requires a single append lineage; position-trial branch accounting "
            "must be implemented before enabling a non-append position policy."
        )
    estimator_call_ledger = (
        _PaperIComparatorEstimatorLedger()
        if paper_i_powell_comparator_accounting_enabled
        else None
    )
    ...
    def _record_outer_hamiltonian(
        ...
        *,
        ...
        _record_estimator_call(
            ...
            component="N_H_outer",
            consumer_scope=(
                f"round:{int(iteration)}:outer_hamiltonian"
                if consumer_scope is None
                else str(consumer_scope)
            ),
            primitive_kind="hamiltonian_expectation",
            observable_or_formula_identity="hamiltonian_expectation_v1",
        )

    def _record_selector_primitives(
        *,
        state: np.ndarray,
        scored_rows: Sequence[Mapping[str, Any]],
        iteration: int,
        consumer_scope_prefix: str | None = None,
    ) -> None:
        if estimator_call_ledger is None:
            return
        scope_prefix = (
            f"round:{int(iteration)}:selector"
            if consumer_scope_prefix is None
            else str(consumer_scope_prefix)
        )
        coordinates = [
            f"candidate:{str(row.get('label', ''))}" for row in scored_rows
        ]
        for coordinate in coordinates:
            _record_estimator_call(
                state=np.asarray(state, dtype=complex),
                component="N_grad",
                consumer_scope=f"{scope_prefix}_gradient",
        ...
        if _is_geo_config(config):
            state_fingerprint = estimator_projective_state_fingerprint(
                np.asarray(state, dtype=complex).reshape(-1)
            )
            estimator_call_ledger.record_symmetric_pair_family(
                EstimatorCallKey(
                ...
                component="N_metric",
                consumer_scope=f"{scope_prefix}_metric",
                branch_id=None,
            )

    def _record_refit_hamiltonian(
        state: np.ndarray,
        consumer_scope: str,
        *,
        iteration: int,
        ...
        _record_estimator_call(
            state=np.asarray(state, dtype=complex),
            component="N_H_refit",
            consumer_scope=f"round:{int(iteration)}:{str(consumer_scope)}",
            primitive_kind="hamiltonian_expectation",
            observable_or_formula_identity="hamiltonian_expectation_v1",
        )
    ...
    reference_metrics = _normalize_reference_metric_fields(
        ...
        exact_reference_n_ph_max=resolved_exact_reference_n_ph_max,
    ...
    estimator_round_receipts: list[dict[str, Any]] = []
    hh_seed_preopt: dict[str, Any] = {
        "enabled": bool(hh_seed_preopt_enabled),
        "executed": False,
        "source": "historical_fm_hh_quadrature_seed_preopt_v1",
    }

    def _close_estimator_round(
        *,
        ...
        if before_payload is None:
            raise RuntimeError(
                "Paper-I comparator estimator round lacks its prefix snapshot."
            )
        receipt = _paper_i_comparator_round_receipt(
            ledger=estimator_call_ledger,
            before_payload=before_payload,
            iteration=int(iteration),
            outcome=str(outcome),
            receipt_kind=str(receipt_kind),
        )
        estimator_round_receipts.append(receipt)
        return receipt

    if bool(hh_seed_preopt_enabled):
        if family_key != "hh":
            raise ValueError("historical HH seed preoptimization is HH-only")
        if warm_start_continuation_enabled:
            raise ValueError(
                "historical HH seed preoptimization cannot be combined with an initial scaffold"
            )
        if minimize_fn is None:
            raise RuntimeError("historical HH seed preoptimization requires SciPy Powell")
        seed_candidates = _historical_hh_seed_candidates(
            pool,
            num_sites=int(context.request.num_sites),
            n_ph_max=int(context.request.n_ph_max),
            boson_encoding=str(context.request.boson_encoding),
        )
        if not seed_candidates:
            raise RuntimeError(
                "historical HH seed preoptimization found no quadrature candidates"
            )
        seed_prefix = (
            None
            if estimator_call_ledger is None
            else estimator_call_ledger.accounting_cursor()
        )
        seed_layout = _selected_parameterization_layout(seed_candidates)
        seed_x0 = np.zeros(int(seed_layout.logical_parameter_count), dtype=float)
        seed_theta, seed_energy, seed_info = _optimize_selected(
            ...
            pauli_action_cache=pauli_action_cache,
            optimizer_maxiter=int(max(100, min(int(optimizer_maxiter), 600))),
            optimizer_method="POWELL",
            parameterization_mode="logical_shared",
            parameterization_layout=seed_layout,
            estimator_h_call_recorder=(
                None
                if estimator_call_ledger is None
                else lambda state, scope: _record_refit_hamiltonian(
                    state,
                    f"hh_seed_preopt:{scope}",
                    iteration=-1,
                )
            ),
        )
        if not np.all(np.isfinite(np.asarray(seed_theta, dtype=float))) or not math.isfinite(
            float(seed_energy)
        ):
            raise RuntimeError("historical HH seed preoptimization returned non-finite state")
        selected = list(seed_candidates)
        selected_batches = [[candidate] for candidate in seed_candidates]
        selected_labels = {str(candidate.label) for candidate in seed_candidates}
        theta = np.asarray(seed_theta, dtype=float).reshape(-1)
        seed_nfev = int(seed_info.get("nfev") or 0)
        seed_nit = int(seed_info.get("nit") or 0)
        nfev_total += seed_nfev
        nit_total += seed_nit
        seed_receipt = _close_estimator_round(
            before_payload=seed_prefix,
            iteration=-1,
            outcome="seed_complete",
            receipt_kind="hh_seed_preopt",
        )
        hh_seed_preopt = {
            "enabled": True,
            "executed": True,
            ...
            "operator_count": int(len(seed_candidates)),
            "operator_labels": [str(candidate.label) for candidate in seed_candidates],
            "energy": float(seed_energy),
            "nfev": seed_nfev,
            "nit": seed_nit,
            "success": bool(seed_info.get("success", False)),
            "message": str(seed_info.get("message", "")),
            "estimator_call_round_receipt": seed_receipt,
        }
        _progress(
            "hh_seed_preopt_complete",
            seed_operator_count=int(len(seed_candidates)),
            seed_energy=float(seed_energy),
            seed_nfev=seed_nfev,
            seed_nit=seed_nit,
            seed_success=bool(seed_info.get("success", False)),
        )

    # The first selector scan computes the first algorithmic Hamiltonian
    # application.  Avoid a redundant uncounted energy call here.  A completed
    # historical HH seed already supplies a measured endpoint energy, including
    # for the useful zero-outer-round bootstrap diagnostic.
    energy = (
        float(hh_seed_preopt["energy"])
        if bool(hh_seed_preopt.get("executed", False))
        else float("nan")
    )

    for iteration in range(int(len(adapt_history)), int(max_adapt_iterations)):
        round_estimator_prefix = (
            None
            if estimator_call_ledger is None
            else estimator_call_ledger.accounting_cursor()
        )
        ...
        _record_outer_hamiltonian(psi_current, int(iteration))
        ...
        if not scored:
            ...
            _close_estimator_round(
                before_payload=round_estimator_prefix,
                iteration=int(iteration),
        ...
        if config.stop_rule == "geo_natural_gradient_norm":
            ...
            break
        if not batch:
            stop_reason = "no_candidate_selected"
            energy = float(energy_current)
            _close_estimator_round(
                before_payload=round_estimator_prefix,
                iteration=int(iteration),
                    ...
                    trial_theta, trial_energy, trial_info = _optimize_selected(
                        ...
                        **_rotosolve_parameterization_kwargs(positioned_selected, positioned_layout),
                        ...
                        estimator_h_call_recorder=(
                            None
                            if estimator_call_ledger is None
                            else lambda state, scope, iteration=iteration, pos=pos: _record_refit_hamiltonian(
                                state,
                                f"position_trial:{int(pos)}:{scope}",
                                iteration=int(iteration),
                            )
                        ),
                    )
                ...
                theta, energy, opt_info = _optimize_selected(
                    ...
                    **_rotosolve_parameterization_kwargs(selected, selected_layout_for_refit),
                    ...
                    estimator_h_call_recorder=(
                        None
                        if estimator_call_ledger is None
                        else lambda state, scope, iteration=iteration: _record_refit_hamiltonian(
                            state,
                            scope,
                            iteration=int(iteration),
                        )
        ...
        if estimator_call_ledger is not None and bool(opt_info.get("success", False)):
            checkpoint_layout = _selected_parameterization_layout(selected)
            checkpoint_state = _prepare_selected_state(
                selected=selected,
                theta=theta,
                psi_ref=psi_ref,
                pauli_action_cache=pauli_action_cache,
                parameterization_mode="logical_shared",
                parameterization_layout=checkpoint_layout,
            )
            adapt_history[-1]["active_prefix_checkpoint"] = (
                _paper_i_comparator_active_prefix_checkpoint(
                    algorithm_id=str(config.algorithm_id),
                    iteration=int(iteration),
                    selected=selected,
                    theta=np.asarray(theta, dtype=float),
                    ...
                    parameterization_layout=checkpoint_layout,
                    psi_ref=psi_ref,
                    psi_prepared=checkpoint_state,
                    sector_padding_audit=_sector_or_unavailable(
                        context,
                        checkpoint_state,
                    ),
                )
            )
        round_receipt = _close_estimator_round(
            before_payload=round_estimator_prefix,
            iteration=int(iteration),
            outcome=(
                "iteration_complete"
                if bool(opt_info.get("success", False))
                else "optimizer_failure"
            ),
        )
        if round_receipt is not None:
            adapt_history[-1]["estimator_call_round_receipt"] = round_receipt
        ...
        first_hit_candidates.append(
            {
                ...
                "estimator_call_accounting_snapshot": (
                    None
                    if estimator_call_ledger is None
                    else estimator_call_ledger.accounting_snapshot()
                ),
            }
        )
    ...
    fixed_horizon_reuses_acquired_terminal_energy = bool(
        generic_adapt_stop_policy_label == _FIXED_HORIZON_NO_TARGET_STOP_POLICY
        and math.isfinite(float(energy))
    )
    terminal_diagnostic_performed = bool(
        str(stop_reason) != "benchmark_abs_delta_e_target"
        and not fixed_horizon_reuses_acquired_terminal_energy
    )
    terminal_diagnostic_excluded_as_dead_work = bool(
        generic_adapt_stop_policy_label == _FIXED_HORIZON_NO_TARGET_STOP_POLICY
        and not terminal_diagnostic_performed
    )
    terminal_estimator_prefix = (
        estimator_call_ledger.accounting_cursor()
        if estimator_call_ledger is not None and terminal_diagnostic_performed
        else None
    )
    ...
    terminal_estimator_call_receipt: dict[str, Any] | None = None
    terminal_scored: list[dict[str, Any]] = []
    if terminal_diagnostic_performed:
        energy, hpsi_final = energy_via_one_apply(psi_final, h_compiled)
        _record_outer_hamiltonian(
            psi_final,
            int(len(adapt_history)),
            consumer_scope="terminal:hamiltonian_verification",
        )
        ...
        _record_selector_primitives(
            state=psi_final,
            scored_rows=terminal_scored,
            iteration=int(len(adapt_history)),
            consumer_scope_prefix="terminal:selector",
        )
        terminal_estimator_call_receipt = _close_estimator_round(
            before_payload=terminal_estimator_prefix,
            iteration=int(len(adapt_history)),
        ...
        for candidate in first_hit_candidates:
            ...
            for threshold in benchmark_first_hit_thresholds:
                ...
                if threshold_value not in first_hits and float(abs_delta_after) <= threshold_value:
                    first_hit_record = _first_hit_record(
                    ...
                    first_hit_accounting: dict[str, Any] | None = None
                    accounting_snapshot = candidate.get(
                        "estimator_call_accounting_snapshot"
                    )
                    if isinstance(accounting_snapshot, Mapping):
                        snapshot_receipts = [
                            dict(receipt)
                            for receipt in estimator_round_receipts
                            if int(receipt.get("iteration", -1))
                            <= int(iteration_index)
                        ]
                        legacy_first_hit_component_fields = {
                            key: value
                            for key, value in first_hit_record.items()
                            if key.startswith("S_alg")
                            or key.startswith("S_norm")
                            or key.startswith("N_H_")
                            or key.startswith("N_grad")
                            or key.startswith("N_metric")
                            or key.startswith("algorithmic_measurement_work_")
                            or key
                            in {
                                "N_other_quantum",
                                "metric_fraction",
                        ...
                        (
                            canonical_first_hit_fields,
                            first_hit_accounting,
                        ) = _paper_i_comparator_estimator_accounting(
                            accounting_snapshot=accounting_snapshot,
                            round_receipts=snapshot_receipts,
                            legacy_component_fields=(
                                legacy_first_hit_component_fields
                            ),
                        )
                        first_hit_record.update(canonical_first_hit_fields)
                        first_hit_record[
                            "legacy_aggregate_measurement_work_proxy"
                        ] = legacy_first_hit_component_fields
                        first_hit_record[
                            "estimator_call_accounting"
                        ] = first_hit_accounting
                        first_hit_record[
                            "estimator_call_round_receipts"
                        ] = snapshot_receipts
                    first_hits[threshold_value] = first_hit_record
    ...
    legacy_aggregate_measurement_work_proxy: dict[str, Any] | None = None
    estimator_call_accounting: dict[str, Any] | None = None
    if estimator_call_ledger is not None:
        legacy_aggregate_measurement_work_proxy = dict(component_fields)
        component_fields, estimator_call_accounting = (
            _paper_i_comparator_estimator_accounting(
                ledger=estimator_call_ledger,
                round_receipts=estimator_round_receipts,
                legacy_component_fields=legacy_aggregate_measurement_work_proxy,
                require_terminal_receipt=bool(terminal_diagnostic_performed),
                terminal_diagnostic_queries_excluded=bool(
                    terminal_diagnostic_excluded_as_dead_work
                ),
            )
        )
    ...
    row.update(
        {
            ...
            "exact_state_fidelity_reference_convention": fidelity_fields.get(
                "exact_state_fidelity_reference_convention"
            ),
            "exact_state_fidelity_s_alg_charged": fidelity_fields.get(
                "exact_state_fidelity_s_alg_charged"
            ),
            "exact_state_fidelity_blocker": fidelity_fields.get(
                "exact_state_fidelity_blocker"
            ),
            "ground_space_fidelity": fidelity_fields.get(
                "ground_space_fidelity"
            ),
            ...
            "selected_operator_count": int(len(selected)),
            "hh_seed_preopt": dict(hh_seed_preopt),
            ...
            "shared_pauli_pool_ordered_pool_hash": shared_pauli_pool_meta.get("ordered_pool_hash"),
            "shared_pauli_pool_source_parent_ordered_label_hash": shared_pauli_pool_meta.get(
                "source_parent_ordered_label_hash"
            ),
            "shared_pauli_pool_source_parent_ordered_pool_hash": shared_pauli_pool_meta.get(
                "source_parent_ordered_pool_hash"
            ),
            "shared_pauli_pool_candidate_representation_counts": shared_pauli_pool_meta.get(
                "candidate_representation_counts"
            ),
            "shared_pauli_pool_projected_singleton_candidate_count": shared_pauli_pool_meta.get(
                "projected_singleton_candidate_count"
            ),
            ...
            "hh_pool_cache_mode": (
                pool_cache_policy.get("requested_mode")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_mode_requested": (
                pool_cache_policy.get("requested_mode")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_mode_effective": (
                pool_cache_policy.get("effective_mode")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_scope": (
                pool_cache_policy.get("requested_scope")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_scope_requested": (
                pool_cache_policy.get("requested_scope")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_scope_requested_raw": (
                pool_cache_policy.get("requested_scope_raw")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_scope_effective": (
                pool_cache_policy.get("effective_scope")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_dir": (
                pool_cache_policy.get("effective_dir")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_dir_requested": (
                pool_cache_policy.get("requested_dir")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_dir_effective": (
                pool_cache_policy.get("effective_dir")
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            "hh_pool_cache_policy": (
                dict(pool_cache_policy)
                if isinstance(pool_cache_policy, Mapping)
                else None
            ),
            ...
            "adapt_terminal_diagnostic_performed": bool(
                terminal_diagnostic_performed
            ),
            "adapt_terminal_diagnostic_excluded_as_dead_work": bool(
                terminal_diagnostic_excluded_as_dead_work
            ),
            "adapt_terminal_diagnostic_hamiltonian_eval_count": int(
                terminal_diagnostic_performed
            ),
            ...
            "adapt_terminal_diagnostic_queries_in_S_alg": bool(
                terminal_estimator_call_receipt is not None
            ),
            "adapt_terminal_estimator_call_receipt": (
                None
                if terminal_estimator_call_receipt is None
                else dict(terminal_estimator_call_receipt)
            ),
            "adapt_terminal_diagnostic_query_policy": (
                "recorded_as_executed_logical_scalar_estimator_invocations_v1"
                if terminal_estimator_call_receipt is not None
                else "excluded_as_post_prefix_dead_work_for_fixed_horizon_v1"
                if terminal_diagnostic_excluded_as_dead_work
                else "skipped_after_benchmark_target_hit"
                if str(stop_reason) == "benchmark_abs_delta_e_target"
                else "legacy_aggregate_accounting_not_instrumented"
            ),
            "paper_i_estimator_call_ledger_enabled": bool(
                estimator_call_ledger is not None
            ),
            "estimator_call_round_receipts": [
                dict(receipt) for receipt in estimator_round_receipts
            ],
            "estimator_call_accounting": estimator_call_accounting,
            "paper_i_estimator_call_accounting": (
                None
                if estimator_call_accounting is None
                else {
                    "schema": "paper_i_comparator_estimator_accounting_alias_v1",
                    "source_field": "estimator_call_accounting",
                    "accounting_schema": str(
                        estimator_call_accounting.get("schema", "")
                    ),
                    "S_alg": float(
                        dict(
                            estimator_call_accounting.get(
                                "all_branch_search_work", {}
                            )
                        ).get("S_alg", 0.0)
                    ),
                    "full_ledger_fingerprint": str(
                        dict(
                            estimator_call_accounting.get("full_ledger", {})
...
def _paper_i_append_facade_migration_blockers(
    *,
    family: str,
    case_id: str,
    algorithm_id: str,
    ...
    gradient_threshold: float,
    seed: int,
    energy_stop_target: float | None,
    benchmark_decision_noise_config: BenchmarkDecisionNoiseConfig | Mapping[str, Any] | None,
    selected_logical_route: str | None,
    selected_logical_source_json: str | Path | None,
    allow_repeats: bool | None,
    progress_jsonl_path: str | Path | None,
    generic_adapt_stop_policy: str | None,
    powell_maxiter_cap_policy: str | None,
    generic_adapt_runtime_split_mode: str | None,
    shared_pauli_pool_mode: str | None,
    initial_selected_operator_labels: Sequence[str] | None,
    initial_selected_operator_batches: Sequence[Sequence[str]] | None,
    initial_theta: Sequence[float] | None,
    initial_adapt_history: Sequence[Mapping[str, Any]] | None,
    optimizer_profile: str | None,
    adapt_optimizer_kind: str | None,
    adapt_spsa_maxiter: int | None,
    adapt_spsa_seed: int | None,
    adapt_spsa_a: float | str | None,
    adapt_spsa_c: float | str | None,
    adapt_spsa_alpha: float | str | None,
    adapt_spsa_gamma: float | str | None,
    adapt_spsa_big_a: float | str | None,
    optimizer_overlay_source: str | None,
    hh_seed_preopt_enabled: bool,
    hh_adaptive_pool_profile: str | None,
    hh_full_meta_class_filter_json: str | Path | None,
) -> tuple[str, ...]:
    """Classify whether the old executor is needed for a frozen identity.

    The retained Paper-I comparison has one deliberately narrow identity:
    L=2 completion rows, macro candidates, Powell-200, seed 7, a fixed
    gradient-independent outer horizon, and no replay/overlay controls.
    Existing Geo routes and append diagnostics/replays keep their historical
    implementation until their own archival decisions are applied.
    """

    if str(algorithm_id) != STATIC_FULL_META_APPEND_ADAPT_VQE:
        return ("not_append_comparison",)
    blockers: list[str] = []
    if str(family).strip().lower() != "hh":
        blockers.append("non_paper_i_hh_family")
    if str(case_id).strip() not in _PAPER_I_APPEND_COMPARISON_CASE_IDS:
        blockers.append("non_completion_matrix_case")
    if int(max_adapt_iterations) < 1:
        blockers.append("zero_horizon_diagnostic")
    if int(optimizer_maxiter) != 200:
        blockers.append("noncanonical_optimizer_budget")
    if float(gradient_threshold) != 0.0:
        blockers.append("gradient_stopping_requested")
    if int(seed) != 7:
        blockers.append("noncanonical_seed")
    if energy_stop_target is not None:
        blockers.append("reference_assisted_stop_requested")
    noise_enabled = bool(
        benchmark_decision_noise_config
        and (
            benchmark_decision_noise_config.get("enabled", False)
            if isinstance(benchmark_decision_noise_config, Mapping)
            else getattr(benchmark_decision_noise_config, "enabled", False)
        )
    )
    if noise_enabled:
        blockers.append("decision_noise_overlay")
    if str(selected_logical_route or "").strip() not in {"", "off"}:
        blockers.append("selected_logical_replay")
    if selected_logical_source_json not in {None, ""}:
        blockers.append("selected_logical_source_replay")
    if allow_repeats is False:
        blockers.append("without_replacement_ablation")
    if progress_jsonl_path not in {None, ""}:
        blockers.append("legacy_progress_stream")
    if str(generic_adapt_stop_policy or "").strip() != (
        _FIXED_HORIZON_NO_TARGET_STOP_POLICY
    ):
        blockers.append("noncanonical_stop_policy")
    if str(powell_maxiter_cap_policy or "").strip() not in {
        "",
        _POWELL_MAXITER_CAP_POLICY_STRICT,
    }:
        blockers.append("legacy_powell_cap_policy")
    if _normalize_generic_adapt_runtime_split_mode(
        generic_adapt_runtime_split_mode
    ) != _GENERIC_ADAPT_RUNTIME_SPLIT_MODE_OFF:
        blockers.append("runtime_split_replay")
    if normalize_shared_pauli_pool_mode(
        shared_pauli_pool_mode
    ) != SHARED_PAULI_POOL_MODE_OFF:
        blockers.append("shared_pauli_child_replay")
    if (
        initial_selected_operator_labels is not None
        and len(initial_selected_operator_labels) > 0
    ):
        blockers.append("initial_operator_replay")
    if (
        initial_selected_operator_batches is not None
        and len(initial_selected_operator_batches) > 0
    ):
        blockers.append("initial_batch_replay")
    if initial_theta is not None and len(initial_theta) > 0:
        blockers.append("initial_theta_replay")
    if initial_adapt_history is not None and len(initial_adapt_history) > 0:
        blockers.append("initial_history_replay")
    if str(optimizer_profile or "").strip():
        blockers.append("optimizer_profile_overlay")
    if str(adapt_optimizer_kind or "").strip().lower() not in {"", "powell"}:
        blockers.append("non_powell_optimizer_overlay")
    if any(
        value not in {None, ""}
        for value in (
            adapt_spsa_maxiter,
        ...
        blockers.append("spsa_overlay")
    if str(optimizer_overlay_source or "").strip():
        blockers.append("optimizer_overlay_source")
    if bool(hh_seed_preopt_enabled):
        blockers.append("seed_preoptimization")
    if str(hh_adaptive_pool_profile or "").strip() not in {
        "",
        _HH_ADAPTIVE_POOL_PROFILE_LEGACY_AUTO,
        _HH_ADAPTIVE_POOL_PROFILE_FULL_META_MINUS_HVA,
    }:
        blockers.append("noncanonical_hh_pool_profile")
    if hh_full_meta_class_filter_json not in {None, ""}:
        requested_filter = Path(
            str(hh_full_meta_class_filter_json)
        ).expanduser()
        if not requested_filter.is_absolute():
            requested_filter = _REPO_ROOT / requested_filter
        if requested_filter.resolve() != (
            _HH_FULL_META_MINUS_HVA_CLASS_FILTER_JSON.resolve()
        ):
            blockers.append("explicit_legacy_pool_filter")
    return tuple(blockers)


def _validated_canonical_append_compile_receipt(
    value: Any,
) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(
            "Canonical Append result omitted its compiled-resource receipt."
        )
    ...
    missing_fields = [
        field for field in required_fields if field not in resources
    ]
    if missing_fields:
        raise ValueError(
            "Canonical Append compiled-resource receipt omitted fields: "
            + ", ".join(missing_fields)
        )
    if (
        str(resources["compile_convention"])
        != TABLE_I_QISKIT_COMPILE_CONVENTION
    ):
        raise ValueError(
            "Canonical Append compiled-resource convention drifted."
        )
    if not isinstance(
        resources["compiled_resource_qiskit_validated"], bool
    ):
        raise ValueError(
            "Canonical Append compiled-resource validation flag must be bool."
        )
    if (
        not resources["compiled_resource_qiskit_validated"]
        and not str(resources.get("reason", "")).strip()
    ):
        raise ValueError(
            "Unavailable canonical Append compile receipt omitted its reason."
        )
    return resources


def _run_paper_i_append_comparison_via_facade(
    *,
    family: str,
    case_id: str,
    output_dir: Path,
    max_adapt_iterations: int,
    shots_per_pauli_term_proxy: int,
    first_hit_thresholds: Sequence[float] | str | None,
    same_cutoff_exact_gs_energy: float | str | None,
    exact_reference_energy: float | str | None,
    exact_reference_n_ph_max: int | str | None,
    primary_energy_metric: str | None,
    same_cutoff_error_role: str | None,
) -> dict[str, Any]:
    """Execute the retained Paper-I macro comparison through Append-ADAPT."""

    config = _get_config(STATIC_FULL_META_APPEND_ADAPT_VQE)
    output = Path(output_dir)
    started_utc = _utc_now()
    t0 = time.perf_counter()
    spec = _spec_by_case_id(
        str(family).strip(),
        str(case_id).strip(),
        STATIC_FULL_META_APPEND_ADAPT_VQE,
    )
    context = _resolve_context_from_spec(spec)
    if (
        str(context.family_key).strip().lower() != "hh"
        or int(context.request.num_sites) != 2
        or int(context.request.n_ph_max) not in {3, 7}
    ):
        raise ValueError(
            "The retained Paper-I Append comparison requires the source-locked "
            "Hubbard--Holstein L=2 nph=3/7 completion matrix."
        )
    horizon = _positive_int(
        max_adapt_iterations, field="max_adapt_iterations"
    )
    request = AppendAdaptRequest(
        ...
        observation=SRObservationPolicy(
            checkpoint=CheckpointObservation(
                path=output / "canonical_append_checkpoint.json",
                every_controller_rounds=1,
                keep_history_tail=100,
            ),
            estimator_ledger=EstimatorLedgerObservation(
                path=output / "canonical_append_estimator_ledger.json"
            ),
        ),
    )
    facade_result = run_append_adapt(context, request)
    execution = dict(facade_result.result_payload)
    protocol_payload = facade_result.protocol.to_dict()
    history = [
        dict(item)
        for item in execution.get("history", [])
        if isinstance(item, Mapping)
    ]
    selected = [
        str(label)
        for label in execution.get("accepted_operator_labels", [])
    ]
    selected_counts = dict(Counter(selected))
    logical_theta = [
        float(value) for value in execution.get("logical_theta", [])
    ]
    runtime_theta = [
        float(value) for value in execution.get("runtime_theta", [])
    ]
    energy = float(execution["final_energy"])
    cutoff_provenance = _resolve_cutoff_provenance(
        spec=spec,
        context=context,
        exact_reference_n_ph_max=exact_reference_n_ph_max,
    )
    reference = _normalize_reference_metric_fields(
        same_cutoff_exact_gs_energy=same_cutoff_exact_gs_energy,
        exact_reference_energy=exact_reference_energy,
        exact_reference_n_ph_max=(
            cutoff_provenance["resolved_exact_reference_n_ph_max"]
        ),
        primary_energy_metric=primary_energy_metric,
        same_cutoff_error_role=same_cutoff_error_role,
        fallback_same_cutoff_energy=_safe_exact_energy(context),
    )
    error_fields = _energy_error_fields(energy, reference)
    accounting = execution.get("estimator_accounting", {})
    if not isinstance(accounting, Mapping):
        raise ValueError(
            "Canonical Append result omitted estimator accounting."
        )
    components = accounting.get("components", {})
    if not isinstance(components, Mapping):
        raise ValueError(
            "Canonical Append estimator accounting omitted components."
        )
    n_h_outer = int(components.get("N_H_outer", 0))
    n_h_refit = int(components.get("N_H_refit", 0))
    n_grad = int(components.get("N_grad", 0))
    n_metric = int(components.get("N_metric", 0))
    s_alg = int(accounting.get("S_alg", -1))
    if s_alg != n_h_outer + n_h_refit + n_grad + n_metric:
        raise ValueError(
            "Canonical Append estimator accounting does not close."
        )
    resources = _validated_canonical_append_compile_receipt(
        execution.get("compiled_resources")
    )
    finished_utc = _utc_now()
    row = _base_row(
    ...
    row.update(
        {
            "algorithm_origin": "canonical_ra_adapt_package",
            "execution_owner": PAPER_I_APPEND_COMPARISON_EXECUTION_OWNER,
            "pool_source": "canonical_ra_adapt_macro_executable_pool",
            "pool_construction": (
                "full_meta_parent_inventory_then_fixed_sector_safe_macro_"
                "filter_v1"
            ),
            "historical_comparator_algorithm_id": (
                STATIC_FULL_META_APPEND_ADAPT_VQE
            ),
            "canonical_append_algorithm_id": execution.get("algorithm_id"),
            "canonical_append_protocol_sha256": (
                facade_result.protocol.sha256
            ),
            "canonical_append_protocol": protocol_payload,
            "selector_identity": facade_result.selector_identity,
            "selector_source_id": execution.get("selector_source_id"),
            "ra_staged_funnel_invoked": False,
            "candidate_representation": (
                facade_result.protocol.candidate_representation
            ),
            "parent_inventory": (
                facade_result.parent_inventory.to_dict()
            ),
            "executable_pool": (
                facade_result.executable_pool.to_dict()
            ),
            "parent_inventory_count": int(
                facade_result.parent_inventory.count
            ),
            "pool_term_count": int(
                facade_result.executable_pool.count
            ),
            "parent_inventory_sha256": (
                facade_result.parent_inventory.ordered_pool_sha256
            ),
            "executable_pool_sha256": (
                facade_result.executable_pool.ordered_pool_sha256
            ),
            "energy": energy,
            "exact_energy": reference.get("primary_reference_energy"),
            **reference,
            **error_fields,
            "num_qubits": int(context.layout.total_qubits),
            "num_parameters": int(len(runtime_theta)),
            "logical_parameter_count": int(len(logical_theta)),
            "runtime_parameter_count": int(len(runtime_theta)),
            "parameterization_mode": "per_pauli_term",
            "selected_operator_count": int(len(selected)),
            "selected_operators": selected,
            "selected_label_counts": selected_counts,
            "selected_operator_batches": [[label] for label in selected],
            "theta": runtime_theta,
            "logical_theta": logical_theta,
            "adapt_history": history,
            "adapt_num_iterations": int(
                execution.get("controller_rounds_completed", len(history))
            ),
            "adapt_depth_reached": int(len(selected)),
            "adapt_stop_reason": str(execution.get("stop_reason", "")),
            "max_adapt_iterations": horizon,
            "optimizer_kind": "powell",
            "optimizer": "scipy.optimize.minimize:Powell",
            "optimizer_maxiter": int(
                facade_result.protocol.optimizer_maxiter
            ),
            "optimizer_success_all": all(
                bool(item.get("optimizer", {}).get("success", False))
                for item in history
            ),
            "N_H_outer": float(n_h_outer),
            "N_H_refit": float(n_h_refit),
            "N_grad": float(n_grad),
            "N_metric": float(n_metric),
            "N_H_outer_eval": float(n_h_outer),
            "N_H_refit_eval": float(n_h_refit),
            "N_grad_selector_probe": float(
                sum(
                    int(item.get("candidate_count_scored", 0))
                    for item in history
                )
            ),
            "N_grad_refit_probe": float(
                max(
                    0,
                    n_grad
                    - sum(
            ...
            "N_grad_qngd_refit_probe": 0.0,
            "N_metric_selector_probe": 0.0,
            "N_metric_refit_probe": float(n_metric),
            "N_metric_qngd_refit_probe": 0.0,
            "N_grad_split_status": "canonical_append_refit_explicit",
            "N_metric_split_status": "canonical_append_refit_explicit",
            "S_alg": float(s_alg),
            "paper_i_estimator_call_accounting": dict(accounting),
            "estimator_call_accounting": dict(accounting),
            "compiled_resources": dict(resources),
            **dict(resources),
            "shots_per_pauli_term_proxy": int(
                shots_per_pauli_term_proxy
            ),
            "candidate_geometry_chart": execution.get(
                "candidate_geometry_chart"
            ),
            "accepted_refit_scope": execution.get(
                "accepted_refit_scope"
            ),
            "accepted_refit_coordinate_chart": execution.get(
                "accepted_refit_coordinate_chart"
            ),
            "runtime_s": float(time.perf_counter() - t0),
            "runtime_seed_status": (
                "not_emitted_canonical_append_result_does_not_serialize_"
                "final_statevector"
            ),
        }
    )
    thresholds = _normalize_first_hit_thresholds(first_hit_thresholds)
    first_hits: dict[str, Any] = {}
    primary_reference = _finite_float_or_none(
        reference.get("primary_reference_energy")
    )
    if primary_reference is not None:
        for threshold in thresholds:
            for item in history:
                item_energy = _finite_float_or_none(
                    item.get("energy_after")
                )
                if (
                    item_energy is not None
                    and abs(item_energy - primary_reference)
                    <= float(threshold)
                ):
                    first_hits[f"{float(threshold):.0e}"] = {
                        "source": "canonical_append_history",
                        "iteration": int(
                            item.get("controller_round", 0)
                        ),
                        "energy": item_energy,
                        "delta_E_abs": float(
                            abs(item_energy - primary_reference)
                        ),
                        "selected_operator_count": int(
                            item.get("controller_round", 0)
                        ),
                    ...
                    break
    row["benchmark_first_hits"] = first_hits
    guardrails = _guardrails(
        config,
        exact_reference_usage="reporting_only_after_optimization",
    )
    guardrails.update(
        {
            "runner_boundary": PAPER_I_APPEND_COMPARISON_EXECUTION_OWNER,
            "canonical_append_facade_called": True,
            "generic_append_executor_called": False,
            "ra_staged_funnel_invoked": False,
            "common_macro_pool_required": True,
            "pool_source": "canonical_ra_adapt_macro_executable_pool",
        }
    )
    comparator_source = _source_fields(
    ...
    comparator_source.update(
        {
            "execution_owner": PAPER_I_APPEND_COMPARISON_EXECUTION_OWNER,
            "canonical_protocol_sha256": facade_result.protocol.sha256,
        }
    )
    payload = {
        "schema": SCHEMA_VERSION,
        ...
        "status": "completed",
        "runner": (
            "pipelines.exact_bench.generic_static_adapt_variants."
            "run_generic_static_adapt_variant_single"
        ),
        "execution_owner": PAPER_I_APPEND_COMPARISON_EXECUTION_OWNER,
        "canonical_append_facade": facade_result.to_dict(),
        "table_i": {
            "tex_label": "tab:benchmark_suite",
            "table_i_canonical_suite": True,
            "first_slice": False,
        ...
        "n_ph_work": cutoff_provenance["n_ph_work"],
        "n_ph_reference": cutoff_provenance["n_ph_reference"],
        "exact_reference_n_ph_max": cutoff_provenance[
            "exact_reference_n_ph_max"
        ],
        "same_cutoff_reference": cutoff_provenance[
...
def run_generic_static_adapt_variant_single(
    *,
    ...
    try:
        append_migration_blockers = (
            _paper_i_append_facade_migration_blockers(
                family=family,
                case_id=case_id,
                ...
                adapt_spsa_big_a=adapt_spsa_big_a,
                optimizer_overlay_source=optimizer_overlay_source,
                hh_seed_preopt_enabled=hh_seed_preopt_enabled,
                hh_adaptive_pool_profile=hh_adaptive_pool_profile,
                hh_full_meta_class_filter_json=(
                    hh_full_meta_class_filter_json
                ),
            )
        )
        if (
            algorithm_id == STATIC_FULL_META_APPEND_ADAPT_VQE
            and not append_migration_blockers
        ):
            return _run_paper_i_append_comparison_via_facade(
                family=family,
                case_id=case_id,
