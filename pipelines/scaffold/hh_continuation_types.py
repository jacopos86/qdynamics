...
class CandidateFeatures:
    ...
    hardware_cost_policy: str = "unresolved"
    hardware_cost_signed_components: dict[str, float] = field(default_factory=dict)
    hardware_cost_signed_index: float = 0.0
    hardware_cost_score_factor: float = 1.0
    hardware_cost_population_hash: str | None = None
    ...
    # Retained for historical payload parsing only; ordinary Phase-II
    # novelty scoring is retired and these fields never carry authority.
    phase2_novelty_mode: str = "ordinary_novelty_scoring_retired_v1"
    phase2_novelty_source: str = "ordinary_novelty_scoring_retired_v1"
    ...
    phase2_raw_score_formula: str = "DeltaE_TR_raw / (1 + K2)"
    ...
    phase3_active_only_trust_gain: float | None = None
    phase3_incremental_candidate_gain: float | None = None
    phase3_candidate_gain_policy: str = "joint_total_gain_v1"
    phase3_candidate_gain_receipt: dict[str, Any] = field(
        default_factory=dict
    )
    ...
    phase3_response_coordinate_indices: list[int] = field(default_factory=list)
    phase3_response_pre_support_count: int = 0
    phase3_response_supported_rank: int | None = None
    phase3_active_logical_coordinate_count: int = 0
    phase3_accepted_refit_coordinate_count: int | None = None
...
class OrderedInsertionGeometryOracleProtocol(Protocol):
    """Interface for exact ordered-insertion derivative geometry."""
