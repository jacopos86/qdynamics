...
def _ansatz_term_from_layout_block(block: Any) -> AnsatzTerm:
    ...
    return AnsatzTerm(
        ...
        polynomial=poly,
        execution_mode=str(
            getattr(block, "execution_mode", "termwise_product")
            or "termwise_product"
        ).strip().lower(),
    )
...
_LEGACY_EXECUTION_MODE_MODEL_KEYS = (
    "vibronic_h2_model",
    "vibronic_h2o_model",
    "vibronic_h2o_linear_fd_model",
)


def _legacy_execution_mode_recovery_pool(
    resolved_problem: Any,
) -> tuple[tuple[AnsatzTerm, ...], tuple[str, ...]]:
    ...
    for key in _LEGACY_EXECUTION_MODE_MODEL_KEYS:
        model = runtime_data.get(key)
        model_pool = getattr(model, "pool", None)
        if model_pool is None:
            continue
        source_keys.append(str(key))
        for term in model_pool:
            if id(term) in seen_ids:
                continue
            seen_ids.add(id(term))
            terms.append(term)
    return tuple(terms), tuple(source_keys)


def _recover_legacy_layout_execution_modes(
    *,
    parameterization: Mapping[str, Any],
    layout: AnsatzParameterLayout,
    selected_terms: Sequence[AnsatzTerm],
    resolved_problem: Any,
) -> tuple[AnsatzParameterLayout, tuple[AnsatzTerm, ...], dict[str, Any]]:
    """Restore execution modes omitted by legacy parameter-block payloads.

    Recovery is intentionally narrow: a block is rebound only when its label
    and canonical coefficient-bearing polynomial match the resolved model pool.
    An explicit serialized execution mode is always authoritative.
    """
    ...
    if len(raw_blocks) != len(layout.blocks) or len(selected_terms) != len(layout.blocks):
        raise ValueError(
            "Legacy execution-mode recovery requires one serialized block per "
            "selected generator."
        )
    missing_indices = [
        int(index)
    ...
    recovery_pool, source_keys = _legacy_execution_mode_recovery_pool(resolved_problem)
    pool_by_label: dict[str, list[AnsatzTerm]] = {}
    for term in recovery_pool:
        pool_by_label.setdefault(str(term.label), []).append(term)

    rebound_terms = list(selected_terms)
    records: list[dict[str, Any]] = []
    unresolved_labels: list[str] = []
    changed_count = 0
    for index in missing_indices:
        serialized_term = rebound_terms[index]
        label = str(serialized_term.label)
        signature = _polynomial_signature(serialized_term.polynomial)
        matches = [
            term
            for term in pool_by_label.get(label, ())
            if _polynomial_signature(term.polynomial) == signature
        ]
        if not matches:
            ...
            records.append(
                {
                    ...
                    "label": label,
                    "status": "unresolved_preserved_legacy_default",
                    "execution_mode": str(serialized_term.execution_mode),
                }
            )
            continue
        matching_modes = {
        ...
        if len(matching_modes) != 1:
            raise ValueError(
                "Legacy parameterization execution mode is ambiguous for exact "
                f"same-label polynomial match {label!r}: {sorted(matching_modes)}."
            )
        recovered_mode = next(iter(matching_modes))
        matching_terms = [
            term
            for term in matches
            if str(
        ...
        rebound_terms[index] = matching_terms[0]
        prior_mode = str(serialized_term.execution_mode).strip().lower()
        changed = prior_mode != recovered_mode
        changed_count += int(changed)
        records.append(
            {
                "logical_index": int(index),
                "label": label,
                "status": "rebound_by_exact_label_and_polynomial",
                "legacy_default_execution_mode": prior_mode,
                "recovered_execution_mode": recovered_mode,
                "execution_mode_changed": bool(changed),
                "equivalent_pool_match_count": int(len(matching_terms)),
            }
        )

    recovered_layout = layout
    if any(record["status"] == "rebound_by_exact_label_and_polynomial" for record in records):
        sort_terms = str(layout.term_order).strip().lower() == "sorted"
        if str(layout.term_order).strip().lower() not in {"sorted", "native"}:
            raise ValueError(
                "Legacy execution-mode recovery cannot rebuild unsupported term order "
                f"{layout.term_order!r}."
            )
        recovered_layout = build_parameter_layout(
            rebound_terms,
        ...
        if int(recovered_layout.runtime_parameter_count) != int(
            layout.runtime_parameter_count
        ):
            raise ValueError(
                "Legacy execution-mode recovery changed runtime parameter count: "
                f"before={layout.runtime_parameter_count}, "
                f"after={recovered_layout.runtime_parameter_count}."
            )

    metadata = {
        "schema_version": "legacy_parameter_block_execution_mode_recovery_v1",
        "legacy_missing_execution_mode": bool(missing_indices),
        "missing_execution_mode_block_count": int(len(missing_indices)),
        "model_pool_source_keys": [str(key) for key in source_keys],
        "model_pool_term_count": int(len(recovery_pool)),
        "exact_pool_rebind_count": int(
            sum(
                record["status"] == "rebound_by_exact_label_and_polynomial"
                for record in records
            )
        ),
        "execution_mode_changed_count": int(changed_count),
        "unresolved_block_count": int(len(unresolved_labels)),
        "unresolved_labels": [str(label) for label in unresolved_labels],
        "records": records,
    }
    return recovered_layout, tuple(rebound_terms), metadata
...
def _load_generic_runtime_input_from_payload(
    ...
    *,
    ...
    legacy_execution_mode_recovery: dict[str, Any] = {
        "schema_version": "legacy_parameter_block_execution_mode_recovery_v1",
        "legacy_missing_execution_mode": False,
        "missing_execution_mode_block_count": 0,
        "model_pool_source_keys": [],
        "model_pool_term_count": 0,
        "exact_pool_rebind_count": 0,
        "execution_mode_changed_count": 0,
    ...
    if isinstance(layout_parameterization, Mapping):
        ...
        (
            layout,
            selected_terms,
            legacy_execution_mode_recovery,
        ) = _recover_legacy_layout_execution_modes(
            parameterization=layout_parameterization,
            layout=layout,
    ...
    provenance = {
        ...
        "legacy_parameter_block_execution_mode_recovery": dict(
            legacy_execution_mode_recovery
        ),
    }
    extensions = {
...
def _reconstruct_prepared_state(
    ...
    *,
    ...
    if bool(use_logical_alias):
        ...
        theta_input = np.asarray(theta_logical, dtype=float).reshape(-1)
    executor = CompiledAnsatzExecutor(
