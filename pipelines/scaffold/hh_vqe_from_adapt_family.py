...
from src.quantum.operator_pools.hh_paop import _make_paop_core
...
from pipelines.scaffold.hh_continuation_replay import (
...
def _ansatz_term_from_parameterization_block(block: Any) -> AnsatzTerm:
    ...
    return AnsatzTerm(
        ...
        polynomial=poly,
        execution_mode=str(getattr(block, "execution_mode", "termwise_product")),
    )


def _replay_terms_from_parameterization_layout(
    layout: AnsatzParameterLayout,
) -> list[AnsatzTerm]:
    return [_ansatz_term_from_parameterization_block(block) for block in layout.blocks]


def _replay_terms_from_serialized_selected_pauli_terms(
    payload: Mapping[str, Any],
) -> list[AnsatzTerm] | None:
    adapt = payload.get("adapt_vqe", None)
    if not isinstance(adapt, Mapping):
        return None
    labels_raw = adapt.get("operators", None)
    terms_raw = adapt.get("selected_operator_pauli_terms", None)
    ...
    if not isinstance(labels_raw, Sequence) or isinstance(labels_raw, (str, bytes)):
        raise ValueError(
            "Serialized selected Pauli terms require adapt_vqe.operators."
        )
    labels = [str(label).strip() for label in labels_raw]
    if len(labels) != len(terms_raw):
        raise ValueError(
            "adapt_vqe.selected_operator_pauli_terms length does not match "
            f"adapt_vqe.operators: {len(terms_raw)} vs {len(labels)}."
        )
    modes_raw = adapt.get("selected_operator_execution_modes", None)
    if modes_raw is None:
        modes = ["termwise_product"] * len(labels)
    elif isinstance(modes_raw, Sequence) and not isinstance(modes_raw, (str, bytes)):
        modes = [str(mode).strip().lower() or "termwise_product" for mode in modes_raw]
        if len(modes) != len(labels):
            raise ValueError(
                "adapt_vqe.selected_operator_execution_modes length does not match "
                f"adapt_vqe.operators: {len(modes)} vs {len(labels)}."
            )
    else:
        raise ValueError("adapt_vqe.selected_operator_execution_modes must be a sequence.")

    replay_terms: list[AnsatzTerm] = []
    for logical_index, (label, raw_specs, execution_mode) in enumerate(
        zip(labels, terms_raw, modes, strict=True)
    ):
        if label == "":
            raise ValueError(f"Empty serialized operator label at logical index {logical_index}.")
        if not isinstance(raw_specs, Sequence) or isinstance(raw_specs, (str, bytes)):
            raise ValueError(
                "Serialized selected operator Pauli terms must be a sequence at "
                f"logical index {logical_index}."
            )
        poly = PauliPolynomial("JW")
        for term_index, raw_spec in enumerate(raw_specs):
            if not isinstance(raw_spec, Mapping):
                raise ValueError(
                    f"Invalid serialized Pauli term at logical index {logical_index}, term {term_index}."
                )
            pauli_exyz = str(raw_spec.get("pauli_exyz", "")).strip().lower()
            if pauli_exyz == "":
                raise ValueError(
                    f"Missing pauli_exyz at logical index {logical_index}, term {term_index}."
                )
            coeff_real = float(raw_spec.get("coeff_re", 0.0))
            coeff_imag = float(raw_spec.get("coeff_im", 0.0))
            if (not np.isfinite(coeff_real)) or (not np.isfinite(coeff_imag)):
                raise ValueError(
                    f"Non-finite serialized coefficient at logical index {logical_index}, term {term_index}."
                )
            if abs(coeff_imag) > 1.0e-12:
                raise ValueError(
                    "Serialized ansatz generator has a non-real Pauli coefficient at "
                    f"logical index {logical_index}, term {term_index}: {coeff_imag}."
                )
            poly.add_term(
                PauliTerm(
                    int(len(pauli_exyz)),
        ...
        poly._reduce()
        if len(tuple(poly.return_polynomial())) == 0:
            raise ValueError(f"Serialized operator {label!r} has no nonzero Pauli terms.")
        replay_terms.append(
            AnsatzTerm(
                label=label,
                polynomial=poly,
...
def _payload_pauli_label_map(payload: Mapping[str, Any] | None) -> dict[str, list[str]]:
    if not isinstance(payload, Mapping):
        return {}
    out: dict[str, list[str]] = {}

    def _merge_pool_map(raw: Any) -> None:
        if not isinstance(raw, Mapping):
            return
        for key, labels_raw in raw.items():
            if not isinstance(labels_raw, Sequence) or isinstance(labels_raw, (str, bytes)):
                continue
            labels = [str(x).strip() for x in labels_raw if str(x).strip()]
            if labels:
                out.setdefault(str(key), labels)

    def _merge_selected_lists(container: Mapping[str, Any]) -> None:
        labels_raw = container.get("operators", container.get("selected_operators", None))
        paulis_raw = container.get("selected_operator_pauli_labels_exyz", None)
        if (
            not isinstance(labels_raw, Sequence)
            or isinstance(labels_raw, (str, bytes))
            or not isinstance(paulis_raw, Sequence)
            or isinstance(paulis_raw, (str, bytes))
        ):
            return
        for label_raw, paulis_for_label in zip(labels_raw, paulis_raw):
            if not isinstance(paulis_for_label, Sequence) or isinstance(paulis_for_label, (str, bytes)):
                continue
            labels = [str(x).strip() for x in paulis_for_label if str(x).strip()]
            if labels:
                out.setdefault(str(label_raw), labels)

    for container in (payload, payload.get("adapt_vqe", None), payload.get("result", None)):
        if isinstance(container, Mapping):
            _merge_pool_map(container.get("pool_pauli_labels_exyz", None))
            _merge_selected_lists(container)
    return out


def _parent_label_for_child_set(label: str) -> str | None:
    return label.split("::child_set", 1)[0] if "::child_set" in str(label) else None


def _term_from_parent_payload_pauli_subset(
    *,
    label: str,
    parent_term: AnsatzTerm,
    payload_pauli_labels: Mapping[str, list[str]],
) -> AnsatzTerm | None:
    requested = payload_pauli_labels.get(str(label), None)
    if not requested:
        return None
    requested_set = {str(x) for x in requested}
    parent_terms = list(parent_term.polynomial.return_polynomial())
    parent_by_label = {str(term.pw2strng()): term for term in parent_terms}
    if not requested_set.issubset(set(parent_by_label)):
        return None
    poly = PauliPolynomial("JW")
    for pauli_label in requested:
        term = parent_by_label[str(pauli_label)]
        poly.add_term(
            PauliTerm(
                int(term.nqubit()),
...
def _fill_payload_child_set_terms(
    selected: dict[str, AnsatzTerm],
    labels: Sequence[str],
    payload: Mapping[str, Any] | None,
) -> None:
    payload_pauli_labels = _payload_pauli_label_map(payload)
    if not payload_pauli_labels:
        return
    for raw_label in labels:
        label = str(raw_label)
        if label in selected:
            continue
        parent_label = _parent_label_for_child_set(label)
        if parent_label is None:
            continue
        parent_term = selected.get(str(parent_label), None)
        if parent_term is None:
            continue
        child_term = _term_from_parent_payload_pauli_subset(
            label=label,
            parent_term=parent_term,
            payload_pauli_labels=payload_pauli_labels,
        )
        if child_term is not None:
            selected[label] = child_term
...
def _inject_replay_terms_from_payload(
    ...
    while stack:
        ...
        if isinstance(cur, Mapping):
            lbl = str(cur.get("candidate_label", "")).strip()
            if lbl != "" and lbl not in label_to_term:
                compile_meta = cur.get("compile_metadata", {})
                if (not isinstance(compile_meta, Mapping)) or ("serialized_terms_exyz" not in compile_meta):
                    generator_meta = cur.get("generator_metadata", {})
                    compile_meta = (
                        generator_meta.get("compile_metadata", {})
                        if isinstance(generator_meta, Mapping)
                        else {}
                    )
                serialized_terms = (
                ...
                if isinstance(serialized_terms, Sequence):
                    try:
...
def _build_replay_terms_from_adapt_labels(
    ...
    _fill_payload_child_set_terms(label_to_term, adapt_labels, payload)
...
def _build_full_meta_replay_terms_sparse(
    ...
    *,
    ...
    needed_lookup_order = list(needed_order)
    for lbl in needed_order:
        parent_label = _parent_label_for_child_set(lbl)
        if parent_label is not None and parent_label not in needed_lookup_order:
            needed_lookup_order.append(parent_label)
    needed_set = set(needed_lookup_order)
    selected: dict[str, AnsatzTerm] = {}
    ...
    def _build_paop_subset(pool_name: str) -> tuple[list[AnsatzTerm], int]:
        ...
        include_lf3 = any(lbl.startswith("paop_lf3(") for lbl in local_needed)
        include_lf4 = any(lbl.startswith("paop_lf4(") for lbl in local_needed)
        ...
        include_local_squeeze = any(lbl.startswith("paop_sq_local(") for lbl in local_needed)
        include_density_squeeze = any(lbl.startswith("paop_sq_density(") for lbl in local_needed)
        include_bond_disp = any(lbl.startswith("paop_bond_disp(") for lbl in local_needed)
        include_hop_squeeze = any(lbl.startswith("paop_hop_sq(") for lbl in local_needed)
        include_pair_squeeze = any(lbl.startswith("paop_pair_sq(") for lbl in local_needed)
        ...
        specs = _make_paop_core(
            ...
            include_lf3=bool(include_lf3),
            include_lf4=bool(include_lf4),
            drop_hop2_phonon_identity=bool(include_hop2 or include_lf3 or include_lf4),
            ...
            include_local_squeeze=bool(include_local_squeeze),
            include_density_squeeze=bool(include_density_squeeze),
            include_bond_disp=bool(include_bond_disp),
            include_hop_squeeze=bool(include_hop_squeeze),
            include_pair_squeeze=bool(include_pair_squeeze),
    ...
    _fill_payload_child_set_terms(selected, needed_order, payload)

    missing_after_sparse = [lbl for lbl in needed_order if lbl not in selected]
    if missing_after_sparse:
        full_meta_t0 = time.perf_counter()
        full_meta_pool, full_meta_info = _build_hh_full_meta_pool(
            h_poly=h_poly,
            num_sites=n_sites,
            t=float(cfg.t),
            u=float(cfg.u),
        ...
        raw_sizes["raw_full_meta_fallback"] = int(len(full_meta_pool))
        for term in full_meta_pool:
            lbl = str(term.label)
            if lbl in needed_set and lbl not in selected:
                selected[lbl] = term
        raw_sizes["raw_full_meta_fallback_elapsed_ms"] = int(1000.0 * (time.perf_counter() - full_meta_t0))
        if isinstance(full_meta_info, Mapping):
            raw_sizes["raw_full_meta_fallback_required_components"] = int(
                len(full_meta_info.get("required_component_keys", []) or [])
            )
            raw_sizes["raw_full_meta_fallback_optional_components"] = int(
                len(full_meta_info.get("optional_component_keys", []) or [])
            )
        del full_meta_pool
        gc.collect()
        _fill_payload_child_set_terms(selected, needed_order, payload)
...
_APPEND_POOL_MATCH_REPLAY_ALIASES = {
    "",
    "match_replay",
    "same_as_replay",
    "replay_family",
...
def _resolve_append_family(
    cfg: RunConfig,
    payload: Mapping[str, Any],
    replay_family_info: Mapping[str, Any],
) -> dict[str, Any]:
    """Resolve the append/candidate-pool family independently of replay scaffolding.

    ``match_replay`` preserves legacy behavior: the append pool is the same pool
    reconstructed for the replay/scaffold family.  Explicit families (for
    example ``full_meta``) opt into a separate problem-local candidate surface.
    """
    requested = str(getattr(cfg, "append_pool_family", "match_replay")).strip().lower()
    replay_resolved = str(replay_family_info.get("resolved", "")).strip().lower()
    fallback = _canonical_family(cfg.fallback_family)
    if fallback is None:
        raise ValueError(f"Invalid --fallback-family '{cfg.fallback_family}'.")

    if requested in _APPEND_POOL_MATCH_REPLAY_ALIASES:
        return {
            "requested": "match_replay" if requested == "" else requested,
    ...
    if requested == "match_adapt":
        ...
        if from_meta is not None:
            ...
            source = "fallback_family"
            warning = (
                "Could not resolve append pool family from input metadata; using "
                f"fallback family '{resolved}'."
            )
        return {
            "requested": requested,
            "resolved": resolved,
    ...
    canonical = _canonical_family(requested)
    if canonical is None:
        allowed = ", ".join(sorted(EXPLICIT_FAMILIES | {"match_replay", "match_adapt"}))
        raise ValueError(
            f"Unsupported append pool family '{requested}'. Expected one of: {allowed}."
        )
    return {
        "requested": requested,
        "resolved": canonical,
...
def build_replay_scaffold_context(
    ...
    *,
    ...
    replay_terms_list: list[AnsatzTerm] | None = None
    if payload_layout is not None:
        replay_terms_list = _replay_terms_from_parameterization_layout(payload_layout)
    else:
        replay_terms_list = _replay_terms_from_serialized_selected_pauli_terms(payload_map)
    ...
    sparse_full_meta_default = family_resolved == "full_meta" and int(cfg.n_ph_max) >= 2
    complete_pool_requested = str(
        payload_map.get("replay_candidate_pool_mode", payload_map.get("candidate_pool_replay_mode", ""))
    ).strip().lower() in {
        "complete",
        "full",
        "full_meta_complete",
    ...
    if family_resolved == "full_meta" and int(cfg.n_ph_max) >= 2 and not complete_pool_requested:
        if replay_terms_list is None:
            ...
            pool_meta = {
                "family": "full_meta",
                "selection_mode": (
                    "serialized_parameterization"
                    if payload_layout is not None
                    else "serialized_selected_pauli_terms"
                ),
                "raw_total": int(len(replay_terms_list)),
                "selected_unique_labels": int(len({str(x) for x in adapt_labels})),
                "replay_terms": int(len(replay_terms_list)),
            }
        ...
        if replay_terms_list is None:
            replay_terms_list = _build_replay_terms_from_adapt_labels(
                family_pool,
                adapt_labels,
                payload=payload_map,
            )
        ...
        if complete_pool_requested:
            pool_meta["candidate_pool_complete_requested_by_payload"] = True
            pool_meta["candidate_pool_complete_before_payload_request"] = (
                not bool(sparse_full_meta_default)
            )
            if bool(sparse_full_meta_default):
                pool_meta["selection_mode_before_payload_request"] = "sparse_label_lookup"
        family_terms_count = int(len(family_pool))

    if replay_terms_list is None:
        raise ValueError("Internal replay error: selected replay terms were not resolved.")

    append_family_info = _resolve_append_family(cfg, payload_map, family_info)
    append_resolved = str(append_family_info["resolved"])
    append_pool: list[AnsatzTerm]
    append_pool_meta: dict[str, Any]
    append_family_terms_count: int
    if bool(append_family_info.get("uses_replay_pool", False)):
        append_pool = list(family_pool)
        append_pool_meta = dict(pool_meta)
        append_pool_meta.update(
            {
                "append_pool_source": "replay_family_pool",
                "replay_family": str(family_resolved),
                "append_family": str(append_resolved),
            }
        )
        append_family_terms_count = int(family_terms_count)
    elif append_resolved == family_resolved and bool(pool_meta.get("candidate_pool_complete", True)):
        append_pool = list(family_pool)
        append_pool_meta = dict(pool_meta)
        append_pool_meta.update(
            {
...
def run(cfg: RunConfig) -> dict[str, Any]:
    ...
    if replay_context.append_family_info is not None:
        append_info = dict(replay_context.append_family_info)
        logger.log(
            f"Append pool family resolved: requested={append_info.get('requested')} "
            f"resolved={append_info.get('resolved')} source={append_info.get('resolution_source')}"
        )
    ...
    result = {
        ...
        "settings": {
            ...
            "append_pool_family": str(getattr(cfg, "append_pool_family", "match_replay")),
        },
        "generator_family": family_info,
        "pool": pool_meta,
        "append_generator_family": (
            {} if replay_context.append_family_info is None else dict(replay_context.append_family_info)
        ),
        "append_pool": (
            {} if replay_context.append_pool_meta is None else dict(replay_context.append_pool_meta)
        ),
        "replay_contract": {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "artifact-resolved replay family; explicit values such as 'full_meta' "
            "opt into a separate problem-local append surface."
        ),
    )
