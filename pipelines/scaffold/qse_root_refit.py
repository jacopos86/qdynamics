...
def _validate_qse_manifest(payload: Mapping[str, Any], *, state_index: int, allow_ground_state: bool) -> tuple[int, int, Mapping[str, Any]]:
    ...
    policy = _basis_vector_policy_from_manifest(payload)
    q0_projected = str(policy.reference_projection) == "q0"
    if int(state_index) == 0 and not q0_projected and not bool(allow_ground_state):
...
def reconstruct_qse_root_target(
    ...
    *,
    ...
    if int(state_index) == 0 and str(policy.reference_projection) == "q0":
        warnings.append("q0_root_zero_is_lowest_orthogonal_ritz_root_not_ground_state")
    elif int(state_index) == 0:
    ...
    *,
    ...
    artifact = {
        ...
        "qse_ritz_diagnostics": {
            "state_index": int(config.state_index),
            "root_role": (
                "lowest_orthogonal_ritz_root"
                if int(config.state_index) == 0
                and str(target.basis_vector_policy.reference_projection) == "q0"
                else "qse_ritz_root"
            ),
            "energy": float(target.qse_energy),
            "energy_relative_to_reference": selected.get("energy_relative_to_reference"),
        ...
        "visibility": {
            ...
            "forbidden_to_controller_refs": [
                ...
                "qse_ritz_diagnostics.energy_relative_to_reference",
