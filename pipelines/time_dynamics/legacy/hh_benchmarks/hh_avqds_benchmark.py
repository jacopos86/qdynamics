#!/usr/bin/env python3
"""Benchmark-local AVQDS row for the HH L=2 t=8 anchor.

This module implements a benchmark-local plain AVQDS RHS-tangent row inside
the benchmark surface only.  It reuses the fixed-pVQD benchmark's anchor
loading and exact reporting seams, plus the adaptive-pVQD replay-pool and
...
QPU-faithful; exact/reference values are reporting-only diagnostics.
"""
...
SCHEMA_VERSION = "hh_avqds_benchmark_v1"
RUN_SCHEMA_VERSION = "hh_avqds_run_v1"
DEFAULT_CASE_ID = base.DEFAULT_CASE_ID
METHOD_ID = "hh_td_avqds_pareto_lean_l2_rhsv1"
METHOD_KIND = "avqds"
...
class AVQDSBenchmarkCase:
    ...
    append_rhs_residual_ratio_threshold: float = 1.0e-3
    append_min_residual_ratio_gain: float = 1.0e-5
...
@dataclass(frozen=True)
class AVQDSTangentStepResult:
    theta_runtime: np.ndarray
    final_state: np.ndarray
    theta_dot: np.ndarray
    rhs_norm: float
    rhs_residual_norm: float
    rhs_residual_ratio: float
    projected_rhs_norm: float
    delta_norm: float
...
class AVQDSBenchmarkRow:
    ...
    avqds_linear_solve_total: int | None
    avqds_step_count: int | None
    avqds_state_prep_total: int | None
    rhs_residual_ratio_final: float | None
    rhs_residual_ratio_max: float | None
    ...
    exact_assisted_contract: str = "exact_fields_reporting_only_plain_avqds_not_qpu_faithful"


@dataclass(frozen=True)
class _CaseRunRecord:
    case: AVQDSBenchmarkCase
...
"Built Math: AVQDS row = local Hamiltonian RHS projection from the current prepared scaffold; append tests use residual-ratio improvement and horizon cost is a serial sum over active scaffold epochs."
def _now_utc() -> str:
...
def _append_candidate_pool(replay_context: Any) -> tuple[list[Any], dict[str, Any]]:
    try:
        ...
        raise ValueError(str(exc).replace("adaptive pVQD", "AVQDS")) from exc
...
def default_cases() -> tuple[AVQDSBenchmarkCase, ...]:
    return (
        AVQDSBenchmarkCase(
...
def _case_by_id(case_id: str) -> AVQDSBenchmarkCase:
    for case in default_cases():
        if case.case_id == case_id:
            return case
    known = ", ".join(case.case_id for case in default_cases())
    raise ValueError(f"unknown AVQDS benchmark case_id={case_id!r}; known cases: {known}")


def _run_spec_for_case(case: AVQDSBenchmarkCase) -> fixed.FixedManifoldRunSpec:
...
def _solve_rhs_tangent_step(
    *,
    prepare_state: Callable[[np.ndarray], np.ndarray],
    theta_start: np.ndarray | Sequence[float],
    hmat_total: np.ndarray | Sequence[Sequence[complex]],
    dt: float,
    finite_difference_epsilon: float,
    regularization_lambda: float,
    pinv_relative_cutoff: float,
) -> AVQDSTangentStepResult:
    """Apply one deterministic local-Hamiltonian AVQDS RHS tangent update."""
    ...
    hmat = np.asarray(hmat_total, dtype=complex)
    if hmat.ndim != 2 or hmat.shape[0] != hmat.shape[1] or hmat.shape[0] != int(current.size):
        raise ValueError(
            f"hmat_total shape {hmat.shape!r} is incompatible with state dimension {int(current.size)}"
        )
    if not np.all(np.isfinite(hmat)):
        raise ValueError("non-finite local Hamiltonian supplied to AVQDS RHS solve")

    hpsi = np.asarray(hmat @ current, dtype=complex).reshape(-1)
    energy = complex(np.vdot(current, hpsi))
    rhs = np.asarray(-1.0j * (hpsi - current * energy), dtype=complex).reshape(-1)
    rhs = rhs - current * np.vdot(current, rhs)
    rhs_norm = float(np.linalg.norm(rhs))
    parameter_count = int(theta_init.size)
    if not math.isfinite(rhs_norm):
        raise ValueError("AVQDS RHS norm is non-finite")

    if parameter_count == 0:
        residual_norm = float(rhs_norm)
        residual_ratio = float(residual_norm / max(rhs_norm, 1.0e-15))
        return AVQDSTangentStepResult(
            theta_runtime=np.asarray(theta_init, dtype=float),
            final_state=np.asarray(current, dtype=complex),
            theta_dot=np.zeros_like(theta_init),
            rhs_norm=float(rhs_norm),
            rhs_residual_norm=float(residual_norm),
            rhs_residual_ratio=float(residual_ratio),
            ...
            success=True,
            message="scaffold has no runtime parameters; RHS tangent update skipped",
        )

    gram = np.real(np.conjugate(tangents).T @ tangents)
    gram = 0.5 * (gram + gram.T)
    force = np.real(np.conjugate(tangents).T @ rhs)
    if not np.all(np.isfinite(gram)) or not np.all(np.isfinite(force)):
        raise ValueError("non-finite finite-difference RHS tangent system")
    ...
    if max_eval <= 0.0:
        ...
        if not bool(np.any(retained)):
            ...
            svals = np.asarray(evals[retained], dtype=float)
            projected_force = np.asarray(v.T @ force, dtype=float).reshape(-1)
            theta_dot = np.asarray(v @ (projected_force / (svals + float(reg))), dtype=float).reshape(-1)
            retained_rank = int(svals.size)
            condition_estimate = float(max_eval / max(float(np.min(svals)), 1.0e-300))
            status = "regularized_spectral_solve" if reg > 0.0 else "spectral_pseudoinverse_solve"

    if not np.all(np.isfinite(theta_dot)):
        raise ValueError(f"AVQDS RHS tangent solve produced non-finite theta_dot; status={status}")
    projected_rhs = np.asarray(tangents @ theta_dot, dtype=complex).reshape(-1)
    residual = np.asarray(rhs - projected_rhs, dtype=complex).reshape(-1)
    residual_norm = float(np.linalg.norm(residual))
    projected_rhs_norm = float(np.linalg.norm(projected_rhs))
    residual_ratio = float(residual_norm / max(rhs_norm, 1.0e-15))
    if not all(math.isfinite(x) for x in (residual_norm, projected_rhs_norm, residual_ratio)):
        raise ValueError("AVQDS RHS residual diagnostics are non-finite")

    delta = np.asarray(step_dt * theta_dot, dtype=float).reshape(-1)
    theta_next = np.asarray(theta_init + delta, dtype=float).reshape(-1)
    final_state = base._normalize_state(prepare_state(theta_next))
    state_prep_count += 1
    success = bool(np.all(np.isfinite(final_state)) and math.isfinite(residual_ratio))
    if not success:
        raise ValueError("AVQDS RHS tangent step produced non-finite state/residual diagnostics")
    ...
    return AVQDSTangentStepResult(
        ...
        rhs_residual_norm=float(residual_norm),
        rhs_residual_ratio=float(residual_ratio),
        projected_rhs_norm=float(projected_rhs_norm),
        ...
        message="one finite-difference local-Hamiltonian RHS tangent linear solve",
    )


def _evaluate_append_candidates_for_avqds(
    *,
    current_terms: Sequence[Any],
    current_layout: Any,
    current_theta_runtime: np.ndarray,
    psi_ref: np.ndarray,
    hmat_total: np.ndarray,
    dt: float,
...
def _select_append_candidate(
    *,
    base_fit: AVQDSTangentStepResult,
    candidate_fits: Sequence[AppendCandidateTangentFit],
    min_residual_ratio_gain: float,
) -> tuple[AppendCandidateTangentFit | None, float]:
    if not candidate_fits:
        return None, 0.0
    best = min(candidate_fits, key=lambda item: float(item.fit.rhs_residual_ratio))
    gain = float(base_fit.rhs_residual_ratio) - float(best.fit.rhs_residual_ratio)
    if gain >= float(min_residual_ratio_gain):
        return best, float(gain)
    return None, float(gain)

def _summarize_trajectory(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    if not rows:
        raise ValueError("cannot summarize empty AVQDS trajectory")
    ...
    solve_total = sum(int(row.get("avqds_linear_solve_count", 0) or 0) for row in rows)
    state_prep_total = sum(int(row.get("avqds_state_prep_count", 0) or 0) for row in rows)
    ...
    return {
        ...
        "avqds_linear_solve_total": int(solve_total),
        "avqds_step_count": int(step_count),
...
def _simulate_avqds(
    *,
...
) -> AVQDSSimulationResult:
    times_arr = np.asarray(times, dtype=float).reshape(-1)
    if int(times_arr.size) < 2:
        raise ValueError("AVQDS requires at least two time points")
    ...
    def _append_row(
        ...
        state: np.ndarray,
        *,
        fit: AVQDSTangentStepResult | None,
        ...
        row: dict[str, Any] = {
            ...
            "avqds_linear_solve_count": int(interval_linear_solve_count),
            "avqds_state_prep_count": int(interval_state_prep_count),
        }
        if fit is None:
            row.update(
                {
                    ...
                    "rhs_norm": None,
                    "rhs_projected_norm": None,
                    "rhs_residual_norm": None,
            ...
            row.update(
                {
                    "avqds_step_index": int(idx) - 1,
                    "rhs_norm": float(fit.rhs_norm),
                    "rhs_projected_norm": float(fit.projected_rhs_norm),
    ...
    for k in range(int(times_arr.size) - 1):
        ...
        rhs_physical_time = base._micro_sample_time(
            t_start=float(times_arr[int(k)]),
            dt_micro=float(dt),
            micro_index=0,
            drive_t0=float(drive_t0),
            drive_time_sampling=str(drive_sampling),
        )
        hmat_step = overlay._hmat_total_at_observation(
            hmat_static=np.asarray(context.hmat, dtype=complex),
            drive_coeff_provider_exyz=context.drive_coeff_provider_exyz,
            physical_time=float(rhs_physical_time),
            nq=int(context.nq),
        )
        ...
        base_fit = _solve_rhs_tangent_step(
            prepare_state=prepare_current,
            theta_start=theta_start,
            hmat_total=np.asarray(hmat_step, dtype=complex),
            dt=float(dt),
        ...
        append_triggered = bool(
            float(base_fit.rhs_residual_ratio) > float(case.append_rhs_residual_ratio_threshold)
        )
        ...
        if append_triggered:
            ...
            candidate_fits = _evaluate_append_candidates_for_avqds(
                current_terms=current_terms,
                current_layout=current_layout,
            ...
            accepted_append, append_gain = _select_append_candidate(
                ...
                min_residual_ratio_gain=float(case.append_min_residual_ratio_gain),
            )
            ...
            append_candidate_evaluations.append(
                {
                    ...
                    "rhs_norm": float(item.fit.rhs_norm),
                    "rhs_residual_norm": float(item.fit.rhs_residual_norm),
                    "rhs_residual_ratio": float(item.fit.rhs_residual_ratio),
                    "residual_ratio_gain_vs_base": float(base_fit.rhs_residual_ratio) - float(item.fit.rhs_residual_ratio),
        ...
        if append_accepted and accepted_append is not None:
            event = {
                ...
                "rhs_physical_time": float(rhs_physical_time),
                "candidate_pool_index": int(accepted_append.candidate_pool_index),
                "candidate_label": str(accepted_append.candidate_label),
                "rhs_residual_ratio_before_append": float(base_fit.rhs_residual_ratio),
                "rhs_residual_ratio_after_append": float(accepted_append.fit.rhs_residual_ratio),
                "rhs_residual_ratio_gain": float(append_gain),
        ...
        step_payload = {
            ...
            "regularization_lambda": float(case.regularization_lambda),
            "pinv_relative_cutoff": float(case.pinv_relative_cutoff),
            "append_rhs_residual_ratio_threshold": float(case.append_rhs_residual_ratio_threshold),
            "append_min_residual_ratio_gain": float(case.append_min_residual_ratio_gain),
            ...
            "append_best_residual_ratio_gain": float(append_gain),
            ...
            "rhs_norm": float(accepted_fit.rhs_norm),
            "rhs_projected_norm": float(accepted_fit.projected_rhs_norm),
            "rhs_residual_norm": float(accepted_fit.rhs_residual_norm),
            "rhs_residual_ratio": float(accepted_fit.rhs_residual_ratio),
            "base_rhs_residual_ratio": float(base_fit.rhs_residual_ratio),
    ...
    finite_residual_ratios = [
        float(step["rhs_residual_ratio"])
        for step in avqds_steps
        if step.get("rhs_residual_ratio") is not None and math.isfinite(float(step["rhs_residual_ratio"]))
    ]
    summary.update(
        {
            "append_events_total": int(len(append_events)),
            "append_candidate_evaluations_total": int(
                sum(int(step.get("append_candidate_evaluations", 0) or 0) for step in avqds_steps)
            ),
            ...
            "rhs_residual_ratio_final": None if not finite_residual_ratios else float(finite_residual_ratios[-1]),
            "rhs_residual_ratio_max": None if not finite_residual_ratios else float(max(finite_residual_ratios)),
    ...
    return AVQDSSimulationResult(
        ...
        exact_reference_summary={
            ...
            "reference_policy": "benchmark-local exact interval propagation for reporting-only fidelity diagnostics",
...
def _compile_scaffold(
    *,
    ...
    base._require_finite_cost(cost, label=f"AVQDS scaffold compile row scope={scope}")
...
def _compile_epoch_scaffolds(
    *,
    context: overlay.RebuiltOverlayContext,
    simulation: AVQDSSimulationResult,
    compile_defaults: Mapping[str, Any],
    ...
    *,
    ...
    return [
        ...
        },
        {
            ...
            "group": "horizon",
            "scope": HORIZON_SCOPE,
            "basis": f"serial sum of {len(epoch_records)} active AVQDS scaffold epochs over {int(intervals)} intervals; each epoch uses latest-active theta for its scaffold signature",
...
def _parameter_manifest(
    *,
    ...
    return {
        ...
        "ansatz_types": "ADAPT seed prep; AVQDS one-step RHS-tangent update with benchmark-local one-generator append tests",
...
def _row_from_run_artifact(
    ...
    *,
    ...
    row = AVQDSBenchmarkRow(
        ...
        avqds_linear_solve_total=_required_int(summary.get("avqds_linear_solve_total"), field="summary.avqds_linear_solve_total"),
        avqds_step_count=_required_int(summary.get("avqds_step_count"), field="summary.avqds_step_count"),
        avqds_state_prep_total=_required_int(summary.get("avqds_state_prep_total"), field="summary.avqds_state_prep_total"),
        rhs_residual_ratio_final=_required_finite_float(summary.get("rhs_residual_ratio_final"), field="summary.rhs_residual_ratio_final"),
        rhs_residual_ratio_max=_required_finite_float(summary.get("rhs_residual_ratio_max"), field="summary.rhs_residual_ratio_max"),
    ...
    out["append_rhs_residual_ratio_threshold"] = _maybe_float(manifest.get("append_rhs_residual_ratio_threshold"))
    out["append_min_residual_ratio_gain"] = _maybe_float(manifest.get("append_min_residual_ratio_gain"))
    return out
    ...
    return _jsonable(
        {
            ...
            "config": {
                ...
                "append_min_residual_ratio_gain": float(case.append_min_residual_ratio_gain),
                "append_candidate_limit": case.append_candidate_limit,
                "rhs_policy": "local_hamiltonian_rhs_from_current_prepared_scaffold_state",
                "step_policy": "one finite-difference local-Hamiltonian RHS tangent linear solve; no exact next-step target and no iterative pVQD projection optimizer",
                "append_policy": "one replay-pool operator appended at end; run the same RHS tangent update; accept only fixed minimum residual-ratio improvement",
            ...
            "contract": {
                ...
                "exact_fields_reporting_only": True,
                "exact_next_step_targets_used_for_decisions": False,
            ...
            "summary": summary,
            "avqds_steps": simulation.avqds_steps,
            ...
            "raw_compile_rows": {
                "avqds_final_state_scaffold": list(final_raw_compile_rows),
                "avqds_epoch_scaffolds": [
                    {
...
def _manifest_payload(
    *,
    ...
    return {
        ...
        "generated_utc": _now_utc(),
        "benchmark": "hh_avqds_time_dynamics",
        "method_contract": {
            "method_id": METHOD_ID,
            "method_kind": METHOD_KIND,
            "decision_mode": DECISION_MODE,
            ...
            "seed_family": SEED_FAMILY,
            "rhs_policy": "local_hamiltonian_rhs_from_current_prepared_scaffold_state",
            "step_policy": "single finite-difference local-Hamiltonian RHS tangent linear solve per interval",
            "append_policy": "one replay-pool operator append/RHS-tangent update under residual-ratio threshold and fixed minimum residual-ratio gain",
        ...
        "key_metrics": [
            {
                ...
                "avqds_linear_solve_total": row.get("avqds_linear_solve_total"),
                "avqds_step_count": row.get("avqds_step_count"),
                "rhs_residual_ratio_final": row.get("rhs_residual_ratio_final"),
                "rhs_residual_ratio_max": row.get("rhs_residual_ratio_max"),
...
def run_benchmark(
    *,
    cases: Sequence[AVQDSBenchmarkCase],
    output_dir: Path,
    command: str = "",
) -> dict[str, Any]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the benchmark-local HH L2 t=8 AVQDS row."
    )
    ...
    parser.add_argument("--append-rhs-residual-ratio-threshold", type=float, default=None)
    parser.add_argument("--append-min-residual-ratio-gain", type=float, default=None)
...
def _case_from_args(args: argparse.Namespace) -> AVQDSBenchmarkCase:
    ...
    return replace(
        ...
        append_rhs_residual_ratio_threshold=(
            float(args.append_rhs_residual_ratio_threshold)
            if args.append_rhs_residual_ratio_threshold is not None
            else case.append_rhs_residual_ratio_threshold
        ),
        append_min_residual_ratio_gain=(
            float(args.append_min_residual_ratio_gain)
            if args.append_min_residual_ratio_gain is not None
            else case.append_min_residual_ratio_gain
        ),
...
def _command_from_argv(argv: Sequence[str] | None) -> str:
    if argv is None:
        return " ".join([sys.executable, "-m", "pipelines.time_dynamics.legacy.hh_benchmarks.hh_avqds_benchmark", *sys.argv[1:]])
    return " ".join(["python", "-m", "pipelines.time_dynamics.legacy.hh_benchmarks.hh_avqds_benchmark", *map(str, argv)])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"avqds_linear_solve_total={row.get('avqds_linear_solve_total')}")
    print(f"avqds_step_count={row.get('avqds_step_count')}")
    print(f"rhs_residual_ratio_final={row.get('rhs_residual_ratio_final')}")
    print(f"rhs_residual_ratio_max={row.get('rhs_residual_ratio_max')}")
