...
from pipelines.time_dynamics.hh_realtime_checkpoint_types import (
...
class RealtimeCheckpointController:
    ...
    def _exact_forecast_primary_density_target_mode(self) -> str:
        ...
        if mode == "auto":
            return "pair_difference" if int(max(1, self._num_sites)) == 2 else "staggered"
        return mode

    def _primary_density_value_from_snapshot(self, snapshot: Mapping[str, Any]) -> float:
        mode = self._exact_forecast_primary_density_target_mode()
        ...
        return float(snapshot["staggered"])
    ...
    def _exact_step_forecast(
        ...
        *,
        ...
        primary_density_controller_next = float(self._primary_density_value_from_snapshot(pred_obs))
        primary_density_exact_next = float(self._primary_density_value_from_snapshot(exact_obs))
        return {
            "fidelity_exact_next": float(abs(np.vdot(psi_exact, psi_pred)) ** 2),
            "primary_density_mode": str(self._exact_forecast_primary_density_target_mode()),
            "primary_density_controller_next": float(primary_density_controller_next),
            "primary_density_exact_next": float(primary_density_exact_next),
            "site_occupations_controller_next": [float(x) for x in pred_site.tolist()],
            "site_occupations_exact_next": [float(x) for x in exact_site.tolist()],
            "doublon_controller_next": float(pred_obs["doublon"]),
            "doublon_exact_next": float(exact_obs["doublon"]),
            "staggered_controller_next": float(pred_obs["staggered"]),
            "staggered_exact_next": float(exact_obs["staggered"]),
            ...
            "abs_primary_density_error_next": float(
                abs(float(primary_density_controller_next) - float(primary_density_exact_next))
            ),
    ...
    def _exact_forecast_normalize_terms(
        ...
        *,
        ...
        if anchor is None:
            anchor = normalized[0]
        anchor_primary_density_exact = float(anchor.get("primary_density_exact_next", normalized[0]["primary_density_exact_next"]))
        anchor_doublon_exact = float(anchor.get("doublon_exact_next", normalized[0]["doublon_exact_next"]))
        anchor_site_exact = np.asarray(
            anchor.get("site_occupations_exact_next", normalized[0]["site_occupations_exact_next"]),
            dtype=float,
        ).reshape(-1)
        anchor_energy_exact = float(anchor.get("energy_total_exact_next", normalized[0]["energy_total_exact_next"]))
        primary_density_exact = np.asarray(
            [float(item["primary_density_exact_next"]) for item in normalized],
            dtype=float,
        )
        doublon_exact = np.asarray(
            [float(item["doublon_exact_next"]) for item in normalized],
            dtype=float,
        )
        ...
        doublon_scale = max(
            self._exact_forecast_scale_floor("exact_forecast_doublon_scale_floor"),
        ...
        for item in normalized:
            item["normalized_primary_density_error_next"] = float(
                float(item.get("abs_primary_density_error_next", item.get("abs_staggered_error_next", 0.0)))
                / float(primary_density_scale)
            )
            item["normalized_doublon_error_next"] = float(
                float(item["abs_doublon_error_next"]) / float(doublon_scale)
            )
            item["normalized_site_occupations_abs_error_max_next"] = float(
                float(item["site_occupations_abs_error_max_next"]) / float(site_occupations_scale)
            )
            item["normalized_energy_total_error_next"] = float(
                float(item["abs_energy_total_error_next"]) / float(energy_total_scale)
            )
    ...
    def _forecast_tracking_score(
        ...
        *,
        ...
        for weight, item in zip(weights, forecasts):
            ...
            doublon_error = float(
                item.get("normalized_doublon_error_next", item.get("abs_doublon_error_next", 0.0))
            )
            site_occupations_error = float(
                item.get(
                    "normalized_site_occupations_abs_error_max_next",
                    item.get("site_occupations_abs_error_max_next", 0.0),
                )
            )
            energy_total_error = float(
                item.get("normalized_energy_total_error_next", item.get("abs_energy_total_error_next", 0.0))
            )
            score += float(weight) * float(
                float(fidelity_defect_weight) * fidelity_defect
                + float(primary_density_error_weight) * primary_density_error
                + float(doublon_error_weight) * doublon_error
                + float(site_occupations_error_weight) * site_occupations_error
                + float(energy_total_error_weight) * energy_total_error
            )
        total = float(score / total_weight)
        density_slope_weight = self._exact_forecast_density_slope_weight()
        if float(density_slope_weight) > 0.0:
            if isinstance(forecast, Mapping) and "tracking_primary_density_slope_error_mean" in forecast:
                density_slope_error_mean = float(forecast["tracking_primary_density_slope_error_mean"])
                ...
                density_slope_error_mean = float(normalized_terms["primary_density_slope_error_mean"])
                total += float(density_slope_weight) * float(density_slope_error_mean)
        slope_weight, curvature_weight = self._exact_forecast_energy_shape_weights()
    ...
    def _exact_v1_componentwise_aspiration_result(
        ...
        *,
        ...
        target_gain_floor = 3.0e-2 if bool(below_floor_probe) else 2.0e-2
        ...
        if not (material_density_gain or material_density_slope_gain or material_site_gain):
    ...
    def _exact_v1_nonimproving_score_allows_density_first_append(
        ...
        *,
        ...
        material_density_gain = float(primary_density_gain) > 2.0e-2
        material_density_slope_gain = float(primary_density_slope_gain) > 2.0e-2
        if not (material_density_gain or material_density_slope_gain):
            return False
        fidelity_regression = float(selected_fidelity_defect) - float(stay_fidelity_defect)
        energy_regression = float(selected_energy_error) - float(stay_energy_error)
        if float(fidelity_regression) > 5.0e-2 or float(energy_regression) > 2.5e-1:
            return False
        if (
    ...
    def _stay_forecast_within_exact_v1_bounded_defect(
        ...
        *,
        ...
        primary_density_error = float(
            forecast.get("abs_primary_density_error_next", forecast.get("abs_staggered_error_next", float("inf")))
        )
        slope_required = bool(
            ("abs_primary_density_slope_error_next" in forecast)
            or float(self._exact_forecast_density_slope_weight()) > 0.0
        )
        primary_density_slope_error = float(
            forecast.get("abs_primary_density_slope_error_next", float("inf"))
        )
        return bool(
            fidelity_defect <= 1.0e-3
            and float(primary_density_error) <= 2.0e-2
            and (not slope_required or float(primary_density_slope_error) <= 2.0e-2)
    ...
    def _exact_v1_below_floor_probe_limit(self) -> int:
        if int(self._exact_v1_append_lane_stall_streak) < int(self._exact_v1_below_floor_probe_stall_threshold()):
            return 0
        return 1

    def _exact_v1_below_floor_probe_stall_threshold(self) -> int:
        return 3
    ...
    def _select_action_exact_v1(
        ...
        *,
        ...
        if not confirmed:
            ...
            return "stay", None
        stay_forecast_payload = None if stay_forecast is None else dict(stay_forecast)
        ...
        for rank_index, record in enumerate(self._sorted_confirmed_by_gain(confirmed)):
            ...
            if not passes_exact_gate:
                near_miss_gate = self._exact_v1_near_miss_confirm_thresholds(record)
                if not near_miss_gate:
                    below_floor_probe = int(rank_index) < int(self._exact_v1_below_floor_probe_limit())
                    if not below_floor_probe:
                        rejection_counts["below_near_miss_floor"] = (
                            int(rejection_counts.get("below_near_miss_floor", 0)) + 1
                        )
                        continue
            ...
            admission_reason = "exact_confirm_pass"
            if near_miss_gate or below_floor_probe:
                if stay_forecast_payload is None:
                    rejection_counts["missing_stay_forecast"] = (
                        int(rejection_counts.get("missing_stay_forecast", 0)) + 1
                    )
                    continue
                aspiration_pass, aspiration_reason = self._exact_v1_componentwise_aspiration_result(
                    stay_forecast=stay_forecast_payload,
                    selected_forecast=forecast,
                    below_floor_probe=bool(below_floor_probe),
                )
                if not aspiration_pass:
                    reason_key = str(aspiration_reason or "fails_componentwise_aspiration")
                    rejection_counts[reason_key] = int(rejection_counts.get(reason_key, 0)) + 1
                    continue
                admission_reason = (
                    "near_miss_componentwise_aspiration"
                    if near_miss_gate
                    else "below_floor_componentwise_aspiration"
                )
            if best_record is None or best_score is None or float(score) < float(best_score) - 1.0e-12:
                best_record = dict(scaled_record)
                best_record["exact_confirm_passed"] = bool(passes_exact_gate)
                best_record["exact_confirm_near_miss_admitted"] = bool(near_miss_gate)
                best_record["exact_confirm_below_floor_probed"] = bool(below_floor_probe)
                best_record["exact_v1_admission_reason"] = str(admission_reason)
                best_score = float(score)
        if best_record is None:
            if rejection_counts:
                self._last_exact_v1_selection_reason = max(
                    rejection_counts.items(), key=lambda item: (int(item[1]), str(item[0]))
                )[0]
            else:
                self._last_exact_v1_selection_reason = "no_admitted_candidates"
            return "stay", None
        self._last_exact_v1_selection_reason = str(best_record.get("exact_v1_admission_reason", "append_selected"))
    ...
    def run(self) -> ControllerRunArtifacts:
        ...
        try:
            ...
            for checkpoint_index, time_value in enumerate(self.times):
                ...
                primary_density_exact = float(self._primary_density_value_from_snapshot(exact_obs))
                abs_primary_density_error = float(
                    abs(float(primary_density_controller) - float(primary_density_exact))
                )
                ...
                self._trajectory.append(
                    {
                        ...
                        "forecast_stay_abs_primary_density_error_next": (
                            ...
                            if forecast_stay is None
                                ...
                                forecast_stay.get(
                                    ...
                                    forecast_stay.get("abs_staggered_error_next", float("nan")),
                                )
                            )
                        ),
                        "forecast_selected_abs_primary_density_error_next": (
                            ...
                            if forecast_selected is None
                                ...
                                forecast_selected.get(
                                    "abs_primary_density_error_next",
                                    forecast_selected.get("abs_staggered_error_next", float("nan")),
                                )
                            )
                        ),
                        "forecast_stay_abs_primary_density_slope_error_next": (
                            None
                            if forecast_stay is None
                            else float(forecast_stay.get("abs_primary_density_slope_error_next", float("nan")))
                        ),
                        "forecast_selected_abs_primary_density_slope_error_next": (
                            None
                            if forecast_selected is None
                            else float(
                                forecast_selected.get("abs_primary_density_slope_error_next", float("nan"))
                            )
                        ),
                        ...
                        "primary_density_exact": float(primary_density_exact),
                        "abs_primary_density_error": float(abs_primary_density_error),
