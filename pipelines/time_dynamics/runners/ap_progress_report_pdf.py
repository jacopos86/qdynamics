...
def build_ap_progress_report_pdf(
    *,
    ...
    summaries = [
        _summarize_run(
            path,
            run_index=int(run_index_offset) + run_count - index,
        )
    ...
    if summaries:
        compact_panel_pngs = _write_compact_panels(
            ...
            run_index_offset=int(run_index_offset),
...
def _draw_qiskit_cost_strip(ax: Any, run: Mapping[str, Any]) -> None:
    ...
    values = [
        ...
        _fmt_count(qiskit_cost.get("N2q")),
        _fmt_count(qiskit_cost.get("D2q")),
        _fmt_count(qiskit_cost.get("Dc")),
    ]
    headers = [
        "resource",
        "logical",
    ...
    *,
    ...
    if summaries:
        for summary in summaries:
            lines.append(rf"% event_audit: {_event_sentence((summary,))}")
            lines.append(rf"% threshold_audit: {_threshold_sentence((summary,))}")
    ...
    for row in rows:
        ...
        line = " & ".join(
            [
                ...
                _fmt_count(row.get("N2q")),
                _fmt_count(row.get("D2q")),
                _fmt_count(row.get("Dc")),
            ]
        )
        lines.append(line + r" \\")
    lines.extend(
...
def _run_label(path: Path, payload: Mapping[str, Any]) -> str:
    summary_label_overrides = {
        ...
        "apm_run117_k10_A0p8_t5_n1001": (
            "t=5 canonical APM support patch"
        ),
        "avqds_method1_k10_A0p8_t5_n1001_results_adapter": (
            "t=5 AVQDS Method 1 singleton"
        ),
        "avqds_tetris_method3_k10_A0p8_t5_n1001_results_adapter": (
            "t=5 AVQDS(T) Method 3 TETRIS"
        ),
        "apm_h32": (
            "normalized H32 pool, APM unique support"
        ),
        "apm_h32_layer_reuse_report_slim": (
            "normalized H32 pool, APM layer reuse"
        ),
        "avqds_singleton_results_adapter": (
            "normalized H32 pool, AVQDS singleton reuse"
        ),
        "avqds_tetris_results_adapter": (
            "normalized H32 pool, AVQDS(T) TETRIS reuse"
        ),
        "apm_layered_no_repair_no_prune_report_slim": (
            "redundant ANZATS, no stabilization or prune"
        ),
        "apm_layered_repair_no_prune_report_slim": (
            "stabilization enabled"
        ),
        "apm_layered_repair_prune_report_slim": (
            "stabilization plus prune"
        ),
        "avqds_tetris_layered_redundancy_t5_report": (
            "matched redundant ANZATS, AVQDS(T)"
        ),
        "run119_append_macro_k23_cached_exchange_nomacroscout_A0p8_t3_n601_dualref_workers16_report_slim": (
            "k=23 canonical support patch, 16 scoring workers"
        ),
        "run120_run119_prunehist10_append_macro_k23_cached_exchange_nomacroscout_A0p8_t3_n601_dualref_workers16_report_slim": (
            "prune history window 3 -> 10"
        ),
        "run121_paperI_sr_v3_k23_fixed_no_repair_A0p8_t3_n601_dualref_report_slim": (
            "new SR-v3 k=23 fixed support, no stabilization"
        ),
        "run122_paperI_sr_v3_k23_supportpatch_prunehist10_no_repair_A0p8_t3_n601_dualref_workers16_report_slim": (
            "new SR-v3 k=23 support patch, no stabilization"
        ),
        "run123_paperI_sr_v3_k23_supportpatch_stabilized_A0p8_t3_n601_dualref_workers16_report_slim": (
            "stabilization enabled"
        ),
        "run124_paperI_sr_v3_k23_fixed_stabilized_A0p8_t3_n601_dualref_report_slim": (
            "fixed support, stabilization enabled"
        ),
        "run125_paperI_sr_v3_k23_prune_only_stabilized_A0p8_t3_n601_dualref_workers16_report_slim": (
            "prune only, stabilization enabled"
        ),
        "run126_paperI_sr_v3_k23_prune_only_no_repair_A0p8_t3_n601_dualref_workers16_report_slim": (
            "prune enabled, stabilization disabled"
        ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        build_ap_progress_report_pdf(
            ...
            run_index_offset=int(args.run_index_offset),
