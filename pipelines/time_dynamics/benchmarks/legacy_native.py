...
def _build_avqds_payload(
    *,
    ...
    current_terms, layout, theta, psi_ref, executor, drive_aligned_scaffold = (
        _runtime_variational_bundle(
            runtime_input,
...
"""Historical product-formula-target adaptive tangent benchmark.

The target tangent is built from a product-formula circuit step applied to the
current prepared state.  It is not published AVQDS(T), where ``T`` means
TETRIS. Exact ED/reference trajectories are diagnostics only.
"""
