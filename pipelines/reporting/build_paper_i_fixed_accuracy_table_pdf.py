...
CUTOFF_STRENGTH_BY_LABEL = {
    ...
    "Bose-Hubbard": r"$(2,4);\,2/6$",
    "harmonic/Kerr chain": r"$(2,4);\,1/0.75$",
    "spin-boson": r"$(2,4);\,0.25/1.0$",
    "Hubbard-Holstein": r"$(2,4);\, (2,0.25)/(8,1.0)$",
    "molecular-vibronic H$_2$": r"$(1,4);\,0.25/1.0$",
}
...
DEFERRED_PLACEHOLDER_FAMILIES = {
    "molecular_vibronic_h2",
}
DEFERRED_PLACEHOLDER_REASON = "molecular_vibronic_h2_weak_strong_points_not_locked_or_wired"
...
class TableCell:
    ...
    def as_latex_cells(self) -> list[str]:
        return [self.delta_e, self.infidelity, self.n_2q, self.d_2q, self.d_circ, self.s_norm]
...
def format_infidelity(row: dict[str, Any]) -> str:
    for key in ("infidelity", "one_minus_fidelity", "one_minus_F", "1-F"):
        if key in row:
...
def _resource_display_policy(row: Mapping[str, Any], status: str | None = None) -> tuple[bool, str]:
    ...
    for name, value in (
        ...
        ("depth_2q", first_finite_number(row.get("depth_2q"), row.get("D_2q"), row.get("d_2q"))),
        ("circuit_depth", first_finite_number(row.get("circuit_depth"), row.get("D_circ"), row.get("d_circ"))),
        ("S_norm", first_finite_number(row.get("S_norm"), row.get("s_norm"), row.get("S_alg"))),
    ):
...
def cost_cells_from_row(row: dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    return (
        ...
        format_number(first_finite_number(row.get("S_norm"), row.get("s_norm"), row.get("S_alg"))),
    )


def cell_from_row(row: dict[str, Any], threshold: float) -> TableCell:
    n_2q, d_2q, d_circ, s_norm = cost_cells_from_row(row)
    return TableCell(
...
def _supplemental_remote_roots(summary_json: Path) -> list[Path]:
    roots: list[Path] = []
...
def _supplemental_row_from_result_path(
    *,
    ...
    return {
        ...
        "count_2q": _first_number_or_none(cost.get("count_2q"), cost.get("compiled_count_2q_total")) if cost_included else None,
        "depth_2q": _first_number_or_none(cost.get("depth_2q"), cost.get("compiled_depth_2q_total")) if cost_included else None,
        "circuit_depth": _first_number_or_none(cost.get("circuit_depth"), cost.get("compiled_depth_total")) if cost_included else None,
...
def _load_supplemental_result_rows(
    ...
    *,
    ...
    roots = _supplemental_remote_roots(summary_json)
    if not roots:
        return []
    ...
    for spec in _expected_cell_specs():
        if spec["canonical_family"] in DEFERRED_PLACEHOLDER_FAMILIES:
            continue
        if (spec["canonical_family"], spec["method"], spec["regime"]) in existing_keys:
            continue
...
def _augment_snake_row_from_live_overlay(
    ...
    *,
    ...
    if _lane_for_row(out, normalize_method(out.get("method"))) != "snake":
        return out
    if _row_is_historical_pareto_display(out):
        return out
    case_keys = [
...
def _row_priority(row: Mapping[str, Any], *, threshold_matches: bool) -> tuple[int, int, int]:
    status = str(row.get("threshold_status") or "").lower()
    if not threshold_matches:
        status_score = 0
    elif _row_is_historical_pareto_display(row) and _resource_display_policy(row, status)[0]:
        status_score = 90
    elif status in ELIGIBLE_COST_STATUSES and _resource_display_policy(row, status)[0]:
        status_score = 70
        ...
        status_score = 10
    return (1 if threshold_matches else 0, status_score, 1 if row.get("cost_included") is True else 0)


def _cell_resources_displayed(cell: TableCell) -> bool:
    return any(value != "--" for value in (cell.n_2q, cell.d_2q, cell.d_circ, cell.s_norm))


def _cell_numeric_resources_displayed(cell: TableCell) -> bool:
    return any(value not in {"--", "UB"} for value in (cell.n_2q, cell.d_2q, cell.d_circ, cell.s_norm))
...
def _build_expected_cell_audits(
    *,
    ...
    for spec in _expected_cell_specs():
        ...
        resource_allowed, resource_reason = (
            (False, "missing_source")
            if raw is None
            else _resource_display_policy(raw, raw_threshold_status)
        )
        ...
        audits.append(
            {
                ...
                **_snake_audit_fields(raw, status=status, method=spec["method"]),
                ...
                "display_s_norm": None if cell is None else cell.s_norm,
...
def load_rows_with_diagnostics(
    ...
    supplemental_rows = (
        []
        if summary_json is None or not summary_json.exists()
        else _load_supplemental_result_rows(summary_json, threshold, existing_keys=existing_keys)
    )
    rows = [*rows, *supplemental_rows, *historical_rows]
    selected: dict[tuple[str, str, str], tuple[TableCell, dict[str, Any], tuple[int, int, int]]] = {}
    for index, row in enumerate(rows):
...
def render_diagnostics(diagnostics: LoadDiagnostics | None) -> str:
    ...
    if diagnostics.historical_pareto_metadata:
        ...
        lines.append(
            rf"\item SNAKE historical Route-A/SPSA Pareto overlay supplied {count} display representative row(s) from {latex_escape(str(path))}."
        )
    ...
    return "\n".join(
        [
            ...
            *lines,
            r"\item Resource columns are rendered only when the source row explicitly validates Qiskit-compiled ansatz-circuit costs at the first accepted target hit (or an explicitly terminal-only fixed-ansatz method); SNAKE live/current-best overlays provide status and \(\delta E\) only until a hash-verified first-crossing compiled-cost sidecar exists.",
            r"\end{itemize}",
        ]
    )
...
def _cost_semantics_audit(diagnostics: LoadDiagnostics | None) -> dict[str, Any]:
    ...
    forbidden_numeric = [
        ...
        or _forbidden_resource_source_reason(item) is not None
    ]
    synthetic_numeric = [
        item
        for item in numeric
    ...
    adaptive_terminal_numeric = [
        ...
        and (
            ...
            or "terminal" in str(item.get("first_hit_cost_source_kind") or item.get("source") or "").lower()
...
Each entry reports $(n_{{\rm ph}}^{{\rm work}},n_{{\rm ph}}^{{\rm ED}});\lambda_{{w}}/\lambda_{{s}}$.
...
$n_{{\rm ph}}^{{\rm algorithm}}={latex_escape(n_ph_algorithm)}$,
$n_{{\rm ph}}^{{\rm ED}}={latex_escape(n_ph_ed)}$,
...
\caption{{Paper-I fixed-accuracy run-report table at $\tau_{{\rm phys}}={format_number(threshold)}$. Rows are grouped by Hamiltonian family, but every data row lists the individual Hamiltonian rather than a class-level aggregate. The cutoff/strength column reports $(n_{{\rm ph}}^{{\rm work}},n_{{\rm ph}}^{{\rm ED}});\lambda_w/\lambda_s$, with $\lambda$ defined row-wise in the preceding note. Each Hamiltonian repeats the full method set. Weak and strong regime blocks retain the approved quality/resource columns: target-excess energy $\delta E$, infidelity diagnostic, two-qubit count, two-qubit depth, circuit depth, and normalized work/shot proxy. \(N_{{2q}}\), \(D_{{2q}}\), and \(D_{{\rm circ}}\) are Qiskit-compiled ansatz-circuit metrics at the first accepted target hit where $\delta E=0$; \(S_{{\rm norm}}\) is the normalized estimator/work proxy at the same first-hit record. Terminal circuits and selector-stage proxies are diagnostics, not promoted table resources.}}\label{{tab:paper_i_approved_fixed_accuracy}}\\
\toprule
...
&
&
& $\delta E$ & $1-F$ & $N_{{2q}}$ & $D_{{2q}}$ & $D_{{\rm circ}}$ & $S_{{\rm norm}}$
& $\delta E$ & $1-F$ & $N_{{2q}}$ & $D_{{2q}}$ & $D_{{\rm circ}}$ & $S_{{\rm norm}}$\\
\midrule
\endfirsthead
\toprule
