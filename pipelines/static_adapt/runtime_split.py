...
def build_projected_child_records_for_retained_phase1_parent(
    *,
    parent_label: str,
    ...
    """Expand one retained Phase-I parent into projected child records.

    The caller supplies the child-stage evaluator.  Paper I uses Phase II here,
    after the Phase-I parent shortlist and before the child shortlist.
    """

    if not bool(split_eligible):
        record = dict(
            evaluate_candidate(
        ...
        return [record], {
            "schema": "paper_i_projected_phase1_parent_expansion_v2",
            "parent_label": str(parent_label),
            "parent_position_id": int(parent_position_id),
            "split_parent": False,
            "raw_child_atom_count": 0,
            "admissible_child_set_count": 0,
            "evaluated_child_candidate_count": 1,
            "phase_order": (
    ...
    hard_guard_required = bool(
        str(child_set_symmetry_policy).strip().lower() == "hard_guard"
    )
    ...
    child_sets = [
        {**dict(child_set), "position_id": int(parent_position_id)}
        for child_set in child_sets
    ]
    projection_telemetry: dict[str, Any] = {
        "schema": "route_a_runtime_split_child_set_padding_v1",
        "policy": (
        ...
        "input_candidate_count": int(len(child_sets)),
        "retained_candidate_count": int(len(child_sets)),
    ...
    if (
        ...
        child_sets, projection_telemetry = (
            project_and_deduplicate_runtime_split_child_sets(
                child_sets,
    ...
    for child_set in child_sets:
        ...
        if not isinstance(polynomial, PauliPolynomial) or not isinstance(
            metadata, Mapping
        ):
            raise ValueError(
                "Projected Phase-III child set lacks its polynomial or "
                "generator metadata."
            )
        ...
        record = dict(
            evaluate_candidate(
                ...
                runtime_split_child_indices_value=[
                    int(value)
                    for value in child_set.get("child_indices", [])
                ],
                runtime_split_child_labels_value=[
                    str(value)
                    for value in child_set.get("child_labels", [])
                ],
                runtime_split_child_generator_ids_value=[
                    str(value)
                    for value in child_set.get("child_generator_ids", [])
                ],
            )
        )
        record["candidate_label"] = str(label)
        record["generator_metadata"] = dict(metadata)
        record["route_a_child_padding_projection"] = dict(
            child_set.get("route_a_child_padding_projection", {})
        )
    ...
    return records, {
        ...
        "parent_position_id": int(parent_position_id),
        "split_parent": True,
        "raw_child_atom_count": int(len(children)),
        "rejected_child_atom_count_symmetry": int(
            sum(
                1
        ...
        "evaluated_child_candidate_count": int(len(records)),
        "subset_sizes": [int(size) for size in subset_sizes],
        "child_padding_projection": dict(projection_telemetry),
        "phase_order": (
            "phase1_parent_shortlist_then_split_then_"
            "phase2_children_then_phase3"
        ),
