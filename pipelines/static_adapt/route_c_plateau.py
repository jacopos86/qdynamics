...
# Frozen only for the quarantined historical Route-C compatibility path.
# Ordinary RA scoring no longer exposes or consumes a novelty ridge.
ROUTE_C_PLATEAU_HISTORICAL_NOVELTY_EPS = 1.0e-6
    ...
    *,
    ...
    try:
        ...
        novelty_info = context.novelty_oracle.estimate(
            ...
            pauli_action_cache=context.pauli_action_cache,
            novelty_eps=ROUTE_C_PLATEAU_HISTORICAL_NOVELTY_EPS,
        )
...
def run_route_c_sp_qngd_trial_optimizer(
    *,
    ...
    energy_evaluation_observer: (
        Callable[[np.ndarray, Mapping[str, Any]], None] | None
    ) = None,
) -> SimpleNamespace:
    ...
    When supplied, ``energy_evaluation_observer`` receives an isolated copy of
    every state passed to ``energy_via_one_apply`` and is called exactly once
    per optimizer ``nfev``; finite-difference state probes are not energy calls.
    """
    ...
    def _exact_energy_hpsi_for_x(
        ...
        *,
        ...
        nonlocal nfev, energy_observer_call_count
        psi_val = _state_for_x(np.asarray(x_vec, dtype=float))
        energy_val, hpsi_val = energy_via_one_apply(psi_val, h_compiled)
        nfev += 1
        if energy_evaluation_observer is not None:
            energy_evaluation_observer(
                np.asarray(psi_val, dtype=complex).reshape(-1).copy(),
                {
                    "schema": "route_c_sp_qngd_energy_evaluation_v1",
                    "optimizer": "route_c_sp_qngd_v1",
                    "evaluation_index": int(nfev),
                    "evaluation_stage": str(evaluation_stage),
                    "iteration": None if iteration is None else int(iteration),
                    "backtrack": None if backtrack is None else int(backtrack),
                    "alpha": None if alpha is None else float(alpha),
                    "energy": float(energy_val),
                    "active_parameter_count": int(x.size),
                },
            )
            energy_observer_call_count += 1
    ...
    current_energy, psi_current, hpsi_current = _exact_energy_hpsi_for_x(
        x,
    ...
    if int(x.size) == 0:
        return SimpleNamespace(
            ...
            qngd_info={
                ...
                "qngd_energy_observer_call_count": int(
                    energy_observer_call_count
                ),
            },
        )
    if int(maxiter) <= 0:
    ...
    for iteration in range(int(maxiter)):
        ...
        for backtrack in range(int(max_backtracks) + 1):
            ...
            candidate_energy, candidate_psi, candidate_hpsi = (
                _exact_energy_hpsi_for_x(
                    candidate_x,
                    evaluation_stage="line_search",
