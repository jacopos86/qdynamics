...
STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION = "global_single_population"
STATIC_LANE_ROUTE_CHOICES = (
    STATIC_LANE_ROUTE_PHYSICAL_OPERATOR_TYPE,
    STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION,
)
GLOBAL_SINGLE_POPULATION_LANE = "global_single_population"
...
def normalize_static_lane_route(
    value: Any,
    *,
    default: str = STATIC_LANE_ROUTE_PHYSICAL_OPERATOR_TYPE,
) -> str:
    ...
    aliases = {
        ...
        "global": STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION,
        "none": STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION,
        "off": STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION,
        "no_lanes": STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION,
        "lanes_off": STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION,
    }
...
def resolve_static_shortlist_lane_spec(
    ...
    *,
    ...
    if route_key == STATIC_LANE_ROUTE_GLOBAL_SINGLE_POPULATION:
        # Lanes-off arm: one lane holding every candidate, so the Phase-I
        # shortlist degenerates to a flat global ranking. Every downstream
        # per-lane cap, quota, and health key then applies to a single
        # population and is a no-op relative to lane-structured shortlisting.
        return StaticShortlistLaneSpec(
            route=route_key,
            lane_key="physical_operator_lane",
            lanes=(GLOBAL_SINGLE_POPULATION_LANE,),
            fallback_lane=GLOBAL_SINGLE_POPULATION_LANE,
            # Reuse the retained health field names: the bookkeeping shape is
            # identical, only the lane partition differs (one bucket, not nine).
...
def clamp_controller_cap_pair_for_lane_route(
    *,
    ...
    """Use the explicit physical-family shortlist cap at every iteration."""

    normalize_static_lane_route(route)
