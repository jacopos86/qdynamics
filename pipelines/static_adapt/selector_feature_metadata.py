...
class SelectorFeatureMetadataContext:
    ...
    def phase3_schur_indices(self, feat_base: Any) -> list[int]:
        ...
        if raw_schur is not None and (
            ...
            or response_scope != "legacy_reopt_coupled_v1"
        ):
            return schur
    ...
    def inherited_selector_updates(self, feat_base: Any) -> dict[str, Any]:
        ...
        return {
            ...
            "phase3_response_coordinate_scope": str(
                feat_base.phase3_response_coordinate_scope
            ),
            "phase3_response_coordinate_indices": [
                int(x) for x in feat_base.phase3_response_coordinate_indices
            ],
            "phase3_response_pre_support_count": int(
                feat_base.phase3_response_pre_support_count
            ),
            "phase3_response_supported_rank": (
                None
                if feat_base.phase3_response_supported_rank is None
                else int(feat_base.phase3_response_supported_rank)
            ),
            "phase3_active_logical_coordinate_count": int(
                feat_base.phase3_active_logical_coordinate_count
            ),
            "phase3_accepted_refit_coordinate_count": (
                None
                if feat_base.phase3_accepted_refit_coordinate_count is None
                else int(feat_base.phase3_accepted_refit_coordinate_count)
            ),
