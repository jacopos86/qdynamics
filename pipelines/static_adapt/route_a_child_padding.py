...
class RouteAChildPaddingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if policy not in active_policies:
            if policy == ROUTE_A_CHILD_PADDING_FULL_BINARY_CODE_SPACE_V1:
                if str(self.boson_encoding).strip().lower() != "binary":
                    raise ValueError(
                        "full_binary_code_space_v1 requires binary encoding."
                    )
                n_ph_max = int(
                    self.n_ph_max if self.n_ph_max is not None else -1
                )
                level_count = n_ph_max + 1
                if level_count < 1 or level_count & (level_count - 1):
                    raise ValueError(
                        "full_binary_code_space_v1 requires n_ph_max + 1 "
