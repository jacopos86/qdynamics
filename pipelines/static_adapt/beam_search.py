...
def _beam_base_branch_from_parent_scratch(
    ...
    *,
    ...
    base_branch.nfev_total_local += int(scratch.nfev_delta)
    return base_branch
...
def _beam_replay_payload_sha256(payload: Mapping[str, Any]) -> str:
    """Return a stable provenance digest for a JSON-compatible replay payload."""
...
def _compact_beam_replay_branch_payload(
    branch_payload: Mapping[str, Any],
) -> dict[str, Any]:
    """Keep historical branch identity without nesting full resumable state.

    The complete live branch remains available through ``current_round`` and
    ``checkpoint_branch``.  Historical rounds need immutable replay evidence,
    but not another copy of the full history, formal-manifold runtime, and
    curvature/whitening checkpoint for every frontier member.
    """

    branch = dict(branch_payload)
    compact = {
        key: copy.deepcopy(branch.get(key))
        for key in (
            "branch_id",
            "parent_branch_id",
            "status",
            ...
            "benchmark_target_error_within_threshold",
            "benchmark_target_hit",
            "history_count",
            "history_tail_count",
            "operator_labels",
            "route_a_trust_region_state",
    ...
    compact.update(
        {
            "payload_storage_policy": "historical_branch_digest_v1",
            "full_payload_sha256": _beam_replay_payload_sha256(branch),
        }
    )
    for field in (
        "last_selected_records",
        ...
        if value is not None:
            compact[f"{field}_sha256"] = _beam_replay_payload_sha256(
                {field: value}
            )
    return compact


def _compact_beam_replay_round_payload(
    round_payload: Mapping[str, Any],
) -> dict[str, Any]:
    """Compact a historical round while retaining branch-level provenance."""

    round_full = copy.deepcopy(dict(round_payload))
    compact = {
        key: copy.deepcopy(value)
        for key, value in round_full.items()
        if key not in {"frontier", "terminal"}
    }

    frontier = round_full.get("frontier")
    frontier_branches = (
        frontier.get("branches", []) if isinstance(frontier, Mapping) else []
    )
    terminal = round_full.get("terminal")
    terminal_branches = (
        terminal.get("branches", []) if isinstance(terminal, Mapping) else []
    )
    round_terminal_branches = (
        terminal.get("round_branches", []) if isinstance(terminal, Mapping) else []
    )
    compact.update(
        {
            "payload_storage_policy": "historical_round_compact_v1",
            "full_payload_sha256": _beam_replay_payload_sha256(round_full),
            "frontier": {
                "branches": [
                    _compact_beam_replay_branch_payload(branch)
            ...
            "terminal": {
                ...
                "round_branches": [
                    _compact_beam_replay_branch_payload(branch)
                    for branch in round_terminal_branches
                    if isinstance(branch, Mapping)
                ],
            },
        }
...
def _beam_replay_telemetry_payload(
    *,
    ...
    return {
        ...
        "rounds_storage_policy": "historical_round_compact_v1",
