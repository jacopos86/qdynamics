...
class ProblemRequest:
    ...
    @classmethod
    def from_namespace(cls, args: Any) -> "ProblemRequest":
...
class ReferenceStateSpec:
    ...
    source_label: str
    state_kind: str
    build_state: Callable[[], np.ndarray]
...
class ExactTargetSpec:
    ...
    build_fallback_anchor_state: Callable[[], np.ndarray]
    fallback_policy: str
...
class HamiltonianFamilyCapabilities:
    ...
    The registry stays import-light: these flags describe what a family exposes,
    while consumer layers such as realtime observables/drives decide how to call
    their local implementations.  This avoids making static ADAPT import
    time-dynamics modules just to answer "what does this family support?".
    """
...
def _resolve_molecular_vibronic_h2_context(
    ...
    *,
    ...
    if not bool(request.include_zero_point):
        raise ValueError("molecular_vibronic_h2 fixture includes zero-point energy; include_zero_point must be true.")
    fixture = load_cached_vibronic_h2_fixture()
    model = build_cached_vibronic_h2_model(
        n_ph_max=int(request.n_ph_max),
        boson_encoding=str(request.boson_encoding),
        coupling_scale=float(request.g_ep),
