...
def legal_subspace_basis_for_problem(
    *,
    ...
    if problem in {"bose_hubbard", "harmonic_kerr_chain"}:
        ...
        if n_sites != 6:
            raise ValueError(
                "molecular_vibronic_h2o_linear_fd legal-subspace filtering "
                "requires the production six-spatial-orbital active space."
            )
        boson_site_count = 3
        non_boson_register_width = 2 * n_sites
        legal_subspace_scope = (
            "three_normal_mode_codewords_with_full_active_fermion_register"
        )
    else:
