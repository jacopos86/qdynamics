...
def _sr_projected_source_metric_trust_transaction(
    summary: Mapping[str, Any],
    *,
    realized_joint_step: Sequence[float] | None,
    radius_before: float,
) -> dict[str, Any] | None:
    """Measure predicted and realized motion in the supported source Gram metric.

    The projected Phase-III solver already diagonalizes the raw source-point
    Gram matrix to identify its supported tangent space.  Reuse that certified
    support here, without constructing an inverse square root and without an
    endpoint-overlap evaluation.
    """
    ...
    if realized_joint_step is None:
        raise ValueError(
            "Projected source-metric trust requires the accepted realized "
            "joint coordinate step."
        )

    G_AA = np.asarray(summary.get("G_AA_raw", []), dtype=float)
    G_AB = np.asarray(summary.get("G_AB_raw", []), dtype=float)
    G_BB = np.asarray(summary.get("G_BB_raw", []), dtype=float)
    # JSON has no shape metadata for an empty two-dimensional block.  At the
    # depth-zero admission boundary, the legitimate (0, 0) active block and
    # (0, n) cross block therefore both serialize as ``[]``.  Recover only
    # those unambiguous empty shapes; nonempty malformed blocks still fail
    # closed below.
    if G_AA.size == 0:
        G_AA = np.zeros((0, 0), dtype=float)
    if G_AA.ndim != 2 or G_AA.shape[0] != G_AA.shape[1]:
        raise ValueError("Projected trust transaction is missing square G_AA_raw.")
    if G_BB.ndim != 2 or G_BB.shape[0] != G_BB.shape[1]:
        raise ValueError("Projected trust transaction is missing square G_BB_raw.")
    active_count = int(G_AA.shape[0])
    candidate_count = int(G_BB.shape[0])
    if G_AB.size == 0 and active_count == 0:
        G_AB = np.zeros((0, candidate_count), dtype=float)
    if G_AB.shape != (active_count, candidate_count):
        raise ValueError("Projected trust transaction has inconsistent G_AB_raw.")
    raw_metric = np.block([[G_AA, G_AB], [G_AB.T, G_BB]])
    raw_metric = 0.5 * (raw_metric + raw_metric.T)
    dimension = int(raw_metric.shape[0])
    if dimension <= 0 or not np.all(np.isfinite(raw_metric)):
        raise ValueError("Projected trust transaction has an invalid raw metric.")

    retained_mask = np.asarray(
        summary.get("metric_retained_mask", []), dtype=bool
    ).reshape(-1)
    certified_eigenvalues = np.asarray(
        summary.get("raw_metric_eigenvalues", []), dtype=float
    ).reshape(-1)
    provenance_id = str(
        summary.get("supported_metric_projection_provenance_id", "")
    )
    if (
        retained_mask.size != dimension
        or certified_eigenvalues.size != dimension
        or not provenance_id
    ):
        raise ValueError(
            "Projected trust transaction is missing its certified support receipt."
        )
    raw_canonical_support = summary.get("retained_support_receipt")
    canonical_support = None
    if isinstance(raw_canonical_support, Mapping):
        from pipelines.static_adapt.ra_adapt.support import (
            validate_retained_support_receipt,
        )

        canonical_support = validate_retained_support_receipt(
            raw_canonical_support
        )
        if (
            not canonical_support.feasible
            or canonical_support.dimension != dimension
            or canonical_support.metric_regularization != 0.0
            or canonical_support.factorization_provenance_id
            != provenance_id
            or canonical_support.retained_mask
            != tuple(bool(value) for value in retained_mask.tolist())
            or canonical_support.raw_metric_eigenvalues
            != tuple(float(value) for value in certified_eigenvalues.tolist())
        ):
            raise ValueError(
                "Projected trust transaction disagrees with its canonical "
                "selector-support receipt."
            )
    eigen_scale = float(
        max(
            1.0,
            (
                np.max(np.abs(certified_eigenvalues))
                if certified_eigenvalues.size
                else 0.0
            ),
        )
    )
    reconstruction_tolerance = float(
        max(
            _finite_float(summary.get("metric_support_threshold")) or 0.0,
            4096.0
            * np.finfo(float).eps
            * max(1, dimension)
            * eigen_scale,
    ...
    if canonical_support is None:
        ...
        eigenvalue_residual = float(
            np.max(np.abs(eigenvalues - certified_eigenvalues))
        )
        retained_values = np.asarray(
            eigenvalues[retained_mask],
            dtype=float,
        )
        retained_vectors = np.asarray(
            eigenvectors[:, retained_mask],
            dtype=float,
        )
    else:
        retained_values = np.asarray(
            canonical_support.retained_metric_eigenvalues,
            dtype=float,
        )
        retained_vectors = np.asarray(
            canonical_support.retained_eigenvectors,
            dtype=float,
        )
        eigenpair_residual = np.asarray(
            raw_metric @ retained_vectors
            - retained_vectors * retained_values[np.newaxis, :],
            dtype=float,
        )
        eigenvalue_residual = float(
            np.max(np.abs(eigenpair_residual))
            if eigenpair_residual.size
            else 0.0
        )
    if eigenvalue_residual > reconstruction_tolerance:
        raise ValueError(
            "Projected trust transaction does not reproduce the certified "
            "raw Gram support."
        )
    if not retained_values.size or np.any(retained_values <= 0.0):
        raise ValueError("Projected trust transaction has no positive support.")
    supported_metric = np.asarray(
    ...
    if (
        ...
        raise ValueError(
            "Projected trust coordinate steps do not match the certified "
            "source Gram dimension."
        )
    predicted_sq = float(
        max(0.0, predicted_step.T @ supported_metric @ predicted_step)
    )
    realized_sq = float(
        max(0.0, realized_step.T @ supported_metric @ realized_step)
    )
    certified_predicted_sq = _finite_float(
    ...
    crosscheck_tolerance = float(
        ...
        * np.finfo(float).eps
        * max(1, dimension)
        * max(1.0, predicted_sq, certified_predicted_sq or 0.0)
    )
    if (
        certified_predicted_sq is None
        or certified_predicted_sq < 0.0
        or abs(predicted_sq - certified_predicted_sq) > crosscheck_tolerance
    ):
        raise ValueError(
            "Projected trust predicted displacement disagrees with the "
            "certified Phase-III receipt."
        )

    certified_radius_sq = _finite_float(summary.get("trust_radius_sq"))
    live_radius_sq = float(radius_before) ** 2
    radius_tolerance_sq = float(
        max(
            _finite_float(summary.get("trust_radius_binding_tolerance_sq"))
            or 0.0,
            1e-14,
            4096.0
            * np.finfo(float).eps
            * max(1.0, live_radius_sq, certified_radius_sq or 0.0),
        )
    )
    if (
        certified_radius_sq is None
        or certified_radius_sq <= 0.0
        or abs(certified_radius_sq - live_radius_sq) > radius_tolerance_sq
    ):
        raise ValueError(
            "Projected trust receipt radius does not match the branch-local radius."
        )

    return {
        "schema": "sr_projected_source_metric_accepted_path_transaction_v1",
        "joint_linear_solve_policy": str(policy),
        "supported_metric_projection_provenance_id": str(provenance_id),
        "metric_retained_mask": [bool(value) for value in retained_mask.tolist()],
        "supported_rank": int(retained_values.size),
        "raw_metric_eigenvalue_reconstruction_residual": float(
            eigenvalue_residual
        ),
        "raw_metric_eigenvalue_reconstruction_tolerance": float(
        ...
        "predicted_source_metric_displacement": float(math.sqrt(predicted_sq)),
        ...
        "predicted_joint_step": [float(value) for value in predicted_step.tolist()],
        "accepted_realized_joint_step": [
            float(value) for value in realized_step.tolist()
        ],
        "certified_trust_radius_sq": float(certified_radius_sq),
        "branch_trust_radius_before": float(radius_before),
        "trust_radius_sq_match_tolerance": float(radius_tolerance_sq),
        "adaptive_radius_rescale_authority": (
...
def update_trust_region_state(
    ...
    *,
    ...
    precomputed_source_metric_trust_transaction: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """Update one branch after full refit and before any prune mutation."""

    summary = dict(selector_summary or {})
    policy = str(config.policy)
    no_overlap_policy = bool(
        policy
        == ROUTE_A_TRUST_REGION_SOURCE_METRIC_INVERSE_SQRT_NO_OVERLAP_V1
    )
    endpoint_overlap_policy = bool(
        policy
        == ROUTE_A_TRUST_REGION_DISPLACEMENT_CALIBRATED_UNBOUNDED_V2
    )
    ...
    realized_fs_displacement_exact: float | None = None
    if not no_overlap_policy:
        try:
    ...
    source_metric_trust_transaction: dict[str, Any] | None = None
    source_metric_trust_transaction_failure: str | None = None
    if precomputed_source_metric_trust_transaction is None:
        try:
        ...
        try:
            transaction = dict(precomputed_source_metric_trust_transaction)
            expected_schema = (
                "sr_material_window_full_source_metric_accepted_path_transaction_v1"
            )
            if str(transaction.get("schema", "")) != expected_schema:
                raise ValueError(
                    "Precomputed source-metric trust transaction has the wrong schema."
                )
            if not no_overlap_policy:
                raise ValueError(
                    "Precomputed source-metric trust transaction requires the "
                    "no-overlap trust policy."
                )
            if str(
                ...
                raise ValueError(
                    "Precomputed source-metric trust transaction requires the "
                    "projected generalized Phase-III solve."
                )
            predicted = _finite_float(
                transaction.get("predicted_source_metric_displacement")
            )
            predicted_sq = _finite_float(
                transaction.get("predicted_source_metric_displacement_sq")
            )
            realized = _finite_float(
                transaction.get("realized_source_metric_displacement")
            )
            realized_sq = _finite_float(
                transaction.get("realized_source_metric_displacement_sq")
            )
            if (
                predicted is None
                or predicted_sq is None
                or realized is None
                or realized_sq is None
                or min(predicted, predicted_sq, realized, realized_sq) < 0.0
            ):
                raise ValueError(
                    "Precomputed source-metric trust transaction has invalid "
                    "displacements."
                )
            displacement_tolerance = float(
                4096.0
                * np.finfo(float).eps
                * max(1.0, predicted, realized, predicted_sq, realized_sq)
            )
            if (
                abs(predicted * predicted - predicted_sq)
                > displacement_tolerance
                or abs(realized * realized - realized_sq)
                > displacement_tolerance
            ):
                raise ValueError(
                    "Precomputed source-metric displacement squares are inconsistent."
                )
            certified_radius = _finite_float(
                transaction.get("branch_trust_radius_before")
            )
            if certified_radius is None or abs(
                certified_radius - float(radius_before)
            ) > max(
                1.0e-14,
                4096.0
                * np.finfo(float).eps
                * max(1.0, abs(float(radius_before))),
            ):
                raise ValueError(
                    "Precomputed source-metric trust transaction radius mismatch."
                )
            if not bool(transaction.get("source_metric_reused_from_accepted_refit")):
                raise ValueError(
                    "Precomputed source-metric trust transaction lacks accepted-refit "
                    "reuse authority."
                )
            if int(transaction.get("incremental_quantum_query_charge", -1)) != 0:
                raise ValueError(
                    "Precomputed source-metric trust transaction must be zero-query."
                )
            if bool(transaction.get("endpoint_overlap_required", True)):
                raise ValueError(
                    "Precomputed source-metric trust transaction cannot require overlap."
                )
            if not bool(transaction.get("transaction_complete", False)):
                raise ValueError(
                    "Precomputed source-metric trust transaction is incomplete."
                )
            source_metric_trust_transaction = transaction
        except (TypeError, ValueError, FloatingPointError) as exc:
            source_metric_trust_transaction_failure = (
                f"{exc.__class__.__name__}:{str(exc)}"
            )
    if endpoint_overlap_policy:
        # This named policy is the overlap arm: the local supported model
        # predicts Fubini--Study motion and the measured endpoint overlap is
        # the sole realized-displacement authority.  A source-Gram
        # transaction may still be retained as diagnostic telemetry, but it
        # must not silently replace the overlap in the radius update.
        predicted_displacement = (
            None
    ...
    elif source_metric_trust_transaction is not None:
    ...
    if (
        (
            ...
            or source_metric_trust_transaction is not None
        )
    ...
    material_window_full_source_context = bool(
        source_metric_trust_transaction is not None
        and str(source_metric_trust_transaction.get("schema", ""))
        == "sr_material_window_full_source_metric_accepted_path_transaction_v1"
        and str(context_mode) == "active_window_v1"
    ...
    context_supported = bool(
        ...
        or material_window_full_source_context
    )

    if adaptive_policy and no_overlap_policy:
        if not context_supported:
            update_reason = "context_mode_not_supported"
        elif source_metric_trust_transaction_failure is not None:
            update_reason = "source_metric_transaction_failure_hold"
        elif source_metric_trust_transaction is None:
            update_reason = "source_metric_transaction_unavailable_hold"
        elif (
            predicted_displacement is None
            or predicted_displacement <= eps
            or realized_displacement is None
            or not math.isfinite(realized_displacement)
        ):
            update_reason = "source_metric_displacement_unresolved_hold"
        else:
            ratio = float(realized_displacement / (predicted_displacement + eps))
            if (
                predicted_energy_reduction is not None
                and predicted_energy_reduction > eps
            ):
                model_agreement_ratio = float(
                    realized_energy_reduction / predicted_energy_reduction
                )
                model_agreement_authority = (
                    "phase3_prediction_vs_post_refit_energy_decrease_v1"
                )
            if ratio > 1.0:
                update_factor = float(1.0 / math.sqrt(ratio))
                radius_after = float(
                    max(
                        numerical_radius_floor,
                        radius_before * update_factor,
                    )
                )
                numerical_floor_applied = bool(
                    radius_before * update_factor < numerical_radius_floor
                )
                update_reason = "realized_source_metric_motion_larger_contract"
            elif ratio > eps and ratio < 1.0 and trust_radius_binding:
                ...
                update_reason = "realized_source_metric_motion_zero_hold"
            else:
                update_reason = "radius_inactive_hold"
    elif adaptive_policy and sr_v2_policy:
        numerical_or_mapping_failure = bool(
    ...
    payload = {
        ...
        "predicted_fs_displacement": (
            ...
            if endpoint_overlap_policy
            or (
                stabilized_trust_transaction is None
                and source_metric_trust_transaction is None
            )
            else None
        ),
        ...
        "predicted_source_metric_displacement": (
            None
            if source_metric_trust_transaction is None
            else float(
                source_metric_trust_transaction[
                    "predicted_source_metric_displacement"
                ]
        ...
        "trust_radius_update_exponent": (
            -0.5
            if no_overlap_policy
            else 0.5
            if endpoint_overlap_policy
            else None
        ),
        "endpoint_overlap_measurement_required": bool(
            not no_overlap_policy
        ),
        "endpoint_overlap_measurement_performed": bool(
            not no_overlap_policy
            and realized_fs_displacement_exact is not None
        ),
        "endpoint_overlap_query_charge": 0 if no_overlap_policy else None,
        ...
        "source_metric_trust_transaction": (
            None
            if source_metric_trust_transaction is None
            else dict(source_metric_trust_transaction)
        ),
    }
    selector_support_receipt = summary.get("retained_support_receipt")
    if (
        isinstance(selector_support_receipt, Mapping)
        and isinstance(source_metric_trust_transaction, Mapping)
        and str(source_metric_trust_transaction.get("schema", ""))
        == "sr_projected_source_metric_accepted_path_transaction_v1"
    ):
        payload["retained_support_receipt"] = dict(
            selector_support_receipt
        )
        stabilization_keys = (
        ...
        if all(key in summary for key in stabilization_keys):
            payload["phase3_stabilization_receipt"] = {
                str(key): copy.deepcopy(summary[key])
                for key in stabilization_keys
            }
...
def update_geometry_expansion_trust_region_state(
    ...
    *,
    ...
    if no_overlap_policy:
        update_reason = GEOMETRY_EXPANSION_NO_OVERLAP_HOLD_REASON
    ...
    payload = {
        ...
        "model_agreement_ratio": None,
        "model_agreement_authority": "unavailable_without_coordinate_prediction",
        ...
        "endpoint_overlap_measurement_performed": bool(
            not no_overlap_policy and realized_displacement is not None
        ),
        "endpoint_overlap_query_charge": 0 if no_overlap_policy else None,
        "realized_energy_reduction_pre_prune": float(
            realized_energy_reduction
