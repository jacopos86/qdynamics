...
from pipelines.static_adapt.builders.hh_pool_presets import (
    ...
    _HH_FULL_META_CLASSIFIER_VERSION,
...
def _load_shadow_legacy_geometry_bridge(mode: str) -> Any:
    ...
    if mode_key == "off":
        return None
    cached = _SHADOW_LEGACY_BRIDGE_CACHE.get(mode_key)
    if cached is not None:
        return cached
    from pipelines.static_adapt import adapt_pipeline_legacy_20260322 as legacy_loader

    legacy_loader._load_legacy_helper_module(
        legacy_loader.LEGACY_SCORER_PRIVATE_MODULE,
        "pipelines/hardcoded/hh_continuation_scoring.py",
    )
    bridge = legacy_loader._load_local_helper_module(
        module_name=f"pipelines.static_adapt._shadow_hh_continuation_scoring_bridge_{mode_key}",
        rel_path="pipelines/static_adapt/hh_continuation_scoring_legacy_bridge.py",
        extra_globals={
            "LEGACY_SCORER_MODULE_NAME": legacy_loader.LEGACY_SCORER_PRIVATE_MODULE,
            "PHASE3_LEGACY_GEOMETRY_MODE": str(mode_key),
        },
    )
    _SHADOW_LEGACY_BRIDGE_CACHE[mode_key] = bridge
    return bridge
...
def _shadow_legacy_geometry_debug_payload(
    *,
    ...
    return {
        ...
        "current_F_metric": _shadow_float_or_none(getattr(current_feat, "F_metric", None)),
        "shadow_F_metric": _shadow_float_or_none(getattr(shadow_feat, "F_metric", None)),
    ...
    *,
    ...
    if phase3_runtime_split_selection_mode_key not in {
        ...
        raise ValueError(
            ...
            "{'proxy_child_set_preselection','full_child_set_scoring','parent_family_sum_top2_scoring'}."
        )
    ...
    if continuation_mode in {"phase1_v1", "phase2_v1", "phase3_v1"} and problem_key == "hh":
        if pool_key_input in ("full_meta", "pareto_lean", "pareto_lean_l2", "pareto_lean_gate_pruned"):
            ...
            pool, _pool_method, full_meta_class_filter_meta, full_meta_label_filter_meta = build_hh_pool_by_key(
            ...
            core_pool, _core_method, _core_class_filter_meta, _core_label_filter_meta = build_hh_pool_by_key(
            ...
            residual_pool, _residual_method, full_meta_class_filter_meta, full_meta_label_filter_meta = build_hh_pool_by_key(
        ...
        if problem_key == "hh":
            pool, method_name, full_meta_class_filter_meta, full_meta_label_filter_meta = build_hh_pool_by_key(
    ...
    def _shadow_legacy_geometry_active_for_depth(depth_one_based: int) -> bool:
        if not bool(phase3_shadow_legacy_geometry_enabled):
            return False
        if int(phase3_shadow_legacy_max_depth_val) <= 0:
            return True
        return bool(int(depth_one_based) <= int(phase3_shadow_legacy_max_depth_val))
    ...
    def _maybe_apply_phase3_proxy_selector_geometry(
        *,
        ...
        if not bool(phase3_proxy_selector_geometry_enabled):
            return feat_full
        try:
            proxy_feat = _build_phase3_proxy_selector_feature(
    ...
    def _maybe_attach_shadow_legacy_geometry(
        *,
        ...
        if not _shadow_legacy_geometry_active_for_depth(int(depth_one_based)):
            return feat_full
        try:
            if (
                bool(phase3_proxy_selector_geometry_enabled)
                and str(phase3_shadow_legacy_geometry_mode_key) == "proxy_reduced"
            ):
                shadow_feat = _build_phase3_proxy_selector_feature(
                    feat_candidate_base=feat_candidate_base,
                    candidate_term=candidate_term,
                    psi_state_shadow=np.asarray(psi_state_shadow, dtype=complex),
                    selected_ops_shadow=list(selected_ops_shadow),
                ...
                shadow_feat = phase3_shadow_legacy_bridge.build_full_candidate_features(
                    base_feature=feat_candidate_base,
                    psi_state=np.asarray(psi_state_shadow, dtype=complex),
                    candidate_term=candidate_term,
                    window_terms=list(selected_ops_shadow),
            ...
            payload = _shadow_legacy_geometry_debug_payload(
                ...
                shadow_mode=str(phase3_shadow_legacy_geometry_mode_key),
            )
            ...
            return _attach_shadow_legacy_geometry_debug(
                ...
                {
                    "status": "error",
                    "mode": str(phase3_shadow_legacy_geometry_mode_key),
    ...
    # Preserve backend parity: the ADAPT optimizer currently works in the
    # runtime-expanded theta space, so the compiled executor must honor the
    # same per-Pauli-term parameterization rather than collapsing blocks to a
    # logical shared mean for a subset of problems.
    ...
    def _phase3_record_sort_key(record: Mapping[str, Any]) -> tuple[float, float, int, int]:
                ...
                updated.update(
                    {
                        ...
                        "selector_score": float(updated["feature"].selector_score or updated.get("selector_score", float("-inf"))),
                    }
                )
            out.append(updated)
        return out
    ...
    def _positive_phase3_selector_records(records: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
        return [
            dict(rec)
            for rec in records
            if float(rec.get(_phase3_selector_score_key(), float("-inf"))) > 0.0
        ]
    ...
    def _selection_pool_from_shortlist(
        ...
        positive_full_records = _positive_phase3_selector_records(full_records_in)
        ...
        for rec in fallback_candidates[:1]:
            ...
            if rec_key not in seen:
                out.append(dict(rec))
                seen.add(rec_key)
        return sorted(out, key=_phase3_record_sort_key)
    ...
    try:
        ...
        if (not bool(disable_hh_seed)) and str(problem_key) == "hh" and abs(float(g_ep)) > 1e-15:
            n_sites = int(num_sites)
        ...
        def _selector_debug_rows(
            ...
            *,
            ...
            if int(topk) <= 0 or not rows:
                return []
            rows_sorted = sorted(rows, key=_phase3_record_sort_key)[: int(topk)]
            ...
            *,
            ...
            return {
                ...
                "score_config": {
                    ...
                    "selector_geometry_mode": str(phase3_selector_geometry_mode_key),
                    "shadow_legacy_geometry_mode": str(phase3_shadow_legacy_geometry_mode_key),
                    "shadow_legacy_max_depth": int(phase3_shadow_legacy_max_depth_val),
                    "parent_collapse_debug_max_depth": int(phase3_parent_collapse_debug_max_depth_val),
                            ...
                            parent_collapse_diagnostic = (
                                _build_parent_collapse_diagnostic(
                                    ...
                                    live_forwarded_representation=(
                                        "child_set" if split_wins else "parent"
                                    ),
                                    live_forwarded_score=(
                                        float(split_score) if split_wins else float(parent_score)
                                    ),
                                )
                ...
                if phase2_enabled:
                        ...
                        if (
                            ...
                            if child_set_candidates and split_child_records:
                                ...
                                for child_set in child_set_candidates:
                                    ...
                                    elif proxy_score > best_split_proxy:
                                        best_split_proxy = float(proxy_score)
                                        best_split_payload = dict(child_set)
                                    ...
                                    if isinstance(split_poly, PauliPolynomial) and isinstance(split_meta, Mapping):
                                                ...
                                                split_child_set_entries.append(
                                                    {
                                                        ...
                                                        "child_generator_ids": list(best_split_child_ids),
                                                        "proxy_score": float(best_split_proxy),
                                        ...
                                        parent_collapse_diagnostic = (
                                            _build_parent_collapse_diagnostic(
                                                ...
                                                live_best_child_set_proxy_score=float(best_split_proxy),
                                                live_forwarded_representation=(
                                                    "child_set" if split_wins else "parent"
                                                ),
                                                live_forwarded_score=(
                    ...
                    if backend_compile_oracle is not None:
                        finite_full_records = [
                            ...
                            if math.isfinite(float(rec.get(_phase3_selector_score_key(), float("-inf"))))
                        ]
                    ...
                    if full_records:
                        if bool(phase2_enable_batching) and str(stage_name) == "core":
                            ...
                            if phase3_enabled and phase3_shortlisted_records:
                                phase2_selected_records = [dict(phase3_shortlisted_records[0])]
                            else:
                                phase2_selected_records = [dict(full_records[0])]
                        phase2_selected_records = sorted(
                            phase2_selected_records,
                            key=(_phase3_record_sort_key if phase3_enabled else _phase2_record_sort_key),
                        )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "the archival child-set shortlist mode remains legacy-only and is not accepted on the current CLI."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        default="reduced",
        help=(
            "Phase-3 selector geometry mode: 'reduced' keeps the current exact-reduced selector, "
            "'proxy_reduced' uses the legacy-compatible proxy-reduced selector score on the live HH phase3 surface, "
