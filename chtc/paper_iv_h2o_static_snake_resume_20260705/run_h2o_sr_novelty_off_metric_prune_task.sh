...
RECORD_ID="${1:-paper_iv_h2o_sr_snake_novelty_off_metric_prune_powell_d15}"
ROOT="$PWD"
...
CODE_ARCHIVE="${H2O_CODE_ARCHIVE:-chtc/paper_iv_h2o_static_snake_resume_20260705/input/h2o_sr_snake_novelty_off_metric_prune_code_20260714.tgz}"
...
ADAPT_SEGMENT_ID="${H2O_ADAPT_SEGMENT_ID:-paper_iv_h2o_sr_snake_novelty_off_metric_prune_powell_d15}"
...
python3 - "$FIXTURE_JSON" "$EXPECTED_FIXTURE_SHA256" <<'PY'
...
python3 - "$MANIFEST_JSON" "$COMMAND_TXT" "$RECORD_ID" "$RESULT_JSON" \
  ...
  "$CODE_ARCHIVE" "$ADAPT_MAX_DEPTH" "$ADAPT_SEGMENT_MAX_NEW_ADMISSIONS" \
  "$ADAPT_SEGMENT_ID" "$PREFLIGHT_ONLY" <<'PY'
from __future__ import annotations
...
archive_path = Path(sys.argv[8])
payload = {
    "schema": "paper_iv_h2o_static_snake_chtc_submit_manifest_v3",
    "record_id": sys.argv[3],
    "run_class": "candidate_diagnostic_fresh_sr_snake",
    "preflight_only": sys.argv[12].strip().lower() in {"1", "true", "yes", "on"},
    ...
    "execution": {
        "starting_depth": 0,
        "max_controller_rounds": int(sys.argv[9]),
        "max_new_admissions": int(sys.argv[10]),
        "target_depth": None,
        "segment_id": sys.argv[11],
        "optimizer": "POWELL",
        "adapt_maxiter": 300,
        ...
        "resume_boundary_refit_policy": "not_applicable_fresh_run",
    },
    "inputs": {
