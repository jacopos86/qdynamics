...
def validate(repo_root: Path, *, require_image: bool) -> dict[str, Any]:
    ...
    for expected_name, raw in zip(CONTROL_FILES, controls, strict=True):
    ...
    if not all(isinstance(item, Mapping) for item in (source, resume, derived)):
    ...
    if (
        ...
        raise ActivationValidationError("Transferred archive binding drifted.")
