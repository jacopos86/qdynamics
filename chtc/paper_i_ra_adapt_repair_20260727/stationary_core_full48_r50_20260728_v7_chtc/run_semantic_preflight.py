#!/usr/bin/env python3
"""Run bounded semantic P3 checks for the stationary 48-cell core.

This is a local, non-paper preflight.  It exercises every selected route
family through the public facade, authenticates RA continuation and replay,
checks Append's reconstruction-only continuation boundary, and requires a
real scored-position witness for each insertion-capable route.  It never
executes the 50-round paper horizon and never authorizes submission.
"""
...
def _g5_witness(
    ...
    *,
    ...
    if scored <= 0 or (
        require_interior
        and (interior <= 0 or first_interior_round is None)
    ):
        raise SemanticPreflightError(
    ...
    if not require_interior and interior == 0:
        witness.update(
            {
                ...
                "limitation": (
                    "immutable_v9_full_insertion_population_receipt_"
                    "retains_selected_phase_records_but_not_the_"
                    "exhaustive_domain_population"
                ),
            }
        )
    else:
        witness["interior_witness_status"] = "observed"
    return witness
    ...
    *,
    ...
    if witness is not None:
        ...
        row["g5_scored_position_witness"] = {
            **_g5_witness(
                witness_payload,
                require_interior=not always_route,
            ),
            "execution_mode": (
