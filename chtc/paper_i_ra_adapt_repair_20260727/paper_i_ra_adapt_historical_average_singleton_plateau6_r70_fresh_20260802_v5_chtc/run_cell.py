#!/usr/bin/env python3
"""Preflight or execute one sealed fresh historical-average RA r70 row."""
...
def _prepare(job_path: Path) -> tuple[dict[str, Any], dict[str, Any], Any, Any, tempfile.TemporaryDirectory[str]]:
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix=f"paper-i-ra-r70-{job['execution_id']}."
    )
    try:
...
def preflight(job_path: Path) -> dict[str, Any]:
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r70_worker_preflight_v1",
            "status": "passed",
...
def smoke_one_round(job_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return digested(
            {
                "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r70_worker_smoke_v1",
    ...
    *, job_path: Path, authorization_path: Path, output_dir: Path, receipt_path: Path
    ...
    try:
        ...
        execution_manifest = digested(
            {
                "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r70_execution_manifest_v1",
                "status": "passed",
        ...
        receipt = digested(
            {
                "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r70_worker_receipt_v1",
                "status": "passed",
                "package_id": PACKAGE_ID,
                "campaign_id": CAMPAIGN_ID,
                "execution_id": job["execution_id"],
