#!/usr/bin/env python3
"""Fail-closed contract for the six-row cumulative-relative RA activation."""
...
BATCH_NAME = (
    "paper-i-ra-cumulative-relative-singleton-plateau6-r70-fresh-"
    "20260731-v1-held"
)
...
def _load_package_contract(package_dir: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "cumulative_relative_ra_r70_sealed_package_contract", path
    )
...
def validate_submit_text(text: str) -> None:
    ...
    if "max_materialize" in lowered or "max_idle" in lowered:
        raise ActivationContractError("Ordinary held activation cannot be a factory.")
...
def quota_release_contract() -> dict[str, Any]:
    return {
        ...
        "next_release_requires": [
            "external_collision_snapshot_clear_for_exact_execution_id",
            "current_hard_quota_headroom",
            "next_archive_estimate_plus_recorded_margin",
...
def validate_activation(repo_root: str | Path) -> dict[str, Any]:
    ...
    if manifest.get("pre_submit_requirements") != [
        ...
        "check_home_quota_and_record_release_margin",
    ]:
