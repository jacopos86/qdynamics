...
PACKAGE_ID = (
    ...
    "r100_fresh_20260808_v1_chtc"
)
ACTIVATION_ID = f"{PACKAGE_ID}_activation_ordinary_v2"
ACTIVATION_SCHEMA = (
    "paper_i_ra_adapt_historical_average_singleton_plateau6_r100_"
    "ordinary_activation_v4"
)
...
BATCH_NAME = (
    "paper-i-ra-historical-average-singleton-plateau6-r100-fresh-"
    "20260809-v2"
)
...
RA_R100_NPH3_MEMORY_FLOOR_MB = 40_960
RA_R100_NPH7_MEMORY_FLOOR_MB = 49_152
...
def activation_memory_mb(
    execution_id: str, package_request_memory_mb: int
) -> int:
    """Return the scientifically neutral scheduler-memory floor for one row."""
    if (
        isinstance(package_request_memory_mb, bool)
        or not isinstance(package_request_memory_mb, int)
        or package_request_memory_mb < 1
    ):
        raise ActivationContractError("Package memory request is invalid.")
    if "__nph3__" in execution_id:
        floor_mb = RA_R100_NPH3_MEMORY_FLOOR_MB
    elif "__nph7__" in execution_id:
        floor_mb = RA_R100_NPH7_MEMORY_FLOOR_MB
    else:
        raise ActivationContractError(
            f"R100 execution cutoff is unavailable: {execution_id}"
        )
    return max(package_request_memory_mb, floor_mb)
...
def _load_package_contract(package_dir: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "historical_average_ra_r100_sealed_package_contract", path
    )
...
def validate_sealed_package(repo_root: Path) -> dict[str, Any]:
    ...
    for index, (execution_id, row) in enumerate(
        ...
        if (
            ...
            or job.get("execution_mode") != "fresh_0_to_100"
            or job.get("target_horizon") != contract.TARGET_HORIZON
...
def validate_submit_text(text: str) -> None:
    ...
    required = (
        ...
        f"transfer_output_files = {archive}",
        (
            f'transfer_output_remaps = "{archive}={REMOTE_OUTPUT_DIRECTORY}/'
            '$(execution_id)__$(ClusterId)__$(ProcId).tar.gz"'
        ),
        "requirements = (TARGET.HasSIF == true) && "
    ...
    lowered = text.lower()
    if re.search(r"(?im)^\s*output_destination\s*=", text) or "osdf://" in lowered:
        raise ActivationContractError(
            "R100 output must use the explicit POSIX staging remap."
        )
    if len(re.findall(r"(?im)^\s*transfer_output_files\s*=", text)) != 1:
        raise ActivationContractError(
            "Attempt archive selection must be assigned exactly once."
        )
    if len(re.findall(r"(?im)^\s*transfer_output_remaps\s*=", text)) != 1:
        raise ActivationContractError(
            "Attempt archive remap must be assigned exactly once."
        )
...
def validate_activation(repo_root: str | Path) -> dict[str, Any]:
    ...
    for index, (queue_row, job_row) in enumerate(
        ...
        memory_mb = activation_memory_mb(
            job_row["execution_id"], resources["request_memory_mb"]
        )
        expected = [
    ...
    fixed_manifest = {
        ...
        "operational_mode": "ordinary_unheld_posix_transfer_v2",
        "execution_authorized": True,
        "submission_authorized": True,
        "submission_state": "authorized_pending_remote_preflight",
        "remote_stage": False,
