#!/usr/bin/env python3
"""Preflight or execute one macro-only Phase-0 proxy/no-lanes cell."""
...
def _load_closed_job(
    ...
    if (
        ...
        or job.get("structural_proxy_mode") != STRUCTURAL_PROXY_MODE
        or job.get("structural_proxy_cost_source")
        != STRUCTURAL_PROXY_COST_SOURCE
        or job.get("selector_qiskit_compile_cost_active") is not False
        ...
        raise PackageContractError(
            "Sealed macro-only Phase-0 proxy/no-lanes job drifted."
        )
...
def _validate_authorization(
    ...
    *,
    ...
    if (
        ...
        != STRUCTURAL_PROXY_MODE
        or runtime_probe.get("selector_qiskit_compile_cost_active") is not False
...
def _load_protocol(
    *,
    ...
    if (
        ...
        raise PackageContractError(
            "Typed macro-only Phase-0 proxy/no-lanes protocol drifted."
        )
...
def _prepare(
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix=f"paper-i-macro-phase0-proxy-{job['execution_id']}."
    )
    try:
