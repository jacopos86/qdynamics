...
JOB_SCHEMA = "paper_i_ra_always_commutation_reduced_job_v1"
SMOKE_SCHEMA = "paper_i_ra_always_commutation_reduced_smoke_v1"
PLAN_SCHEMA = "paper_i_ra_always_commutation_reduced_execution_plan_v1"
MANIFEST_SCHEMA = "paper_i_ra_always_commutation_reduced_package_v1"
ARCHIVE_SCHEMA = "paper_i_ra_always_commutation_reduced_source_archive_v1"
...
def direct_execution_rows() -> tuple[dict[str, Any], ...]:
    ...
    for regime_id, nph in REGIME_CUTOFF_PAIRS:
        for route_id in ROUTE_IDS:
            ...
            execution_id = f"core__{regime_id}__nph{nph}__{route_id}"
            rows.append(
                {
...
def validate_core_authority(repo_root: str | Path) -> dict[str, Any]:
    ...
    final_path = core_root / "final_publication_receipt.json"
    final = load_json(final_path, label="v13 final publication receipt")
    verify_self_digest(final, label="v13 final publication receipt")
    authorization = final.get("authorization")
    if (
        final.get("materialization_id")
        != "ra_adapt_stationary_late_core_v13"
        ...
        raise PackageContractError("v13 final authority is not inert.")

    bundle_root = core_root / CORE_BUNDLE_ID
    source_locks = load_json(
        bundle_root / "source_locks.json", label="v13 source locks"
    )
    verify_self_digest(source_locks, label="v13 source locks")
    implementation = source_locks.get("implementation_sources")
    if not isinstance(implementation, Mapping):
        raise PackageContractError(
            "v13 source locks have no implementation inventory."
        )
    verify_self_digest(implementation, label="implementation inventory")
    if implementation != final.get("implementation_source_inventory"):
        raise PackageContractError(
            "v13 implementation inventory binding drifted."
        )

    protocol_bindings: dict[str, dict[str, Any]] = {}
    rows = direct_execution_rows()
    for row in rows:
        path = bundle_root / "protocols" / f"{row['cell_id']}.json"
        protocol = load_json(path, label=f"{row['cell_id']} protocol")
        _validate_protocol(protocol, row=row)
        protocol_bindings[str(row["cell_id"])] = {
    ...
    route_delta_path = (
        ...
        / "always_route_delta_receipt.json"
    )
    route_delta = load_json(
        route_delta_path, label="v13 always route delta receipt"
    )
    verify_self_digest(route_delta, label="v13 always route delta receipt")
    if (
        route_delta.get("status") != "passed"
        or route_delta.get("repaired_source_lock_count") != 12
        or route_delta.get("changed_scalar_value_count") != 18
        or route_delta.get("target_insertion_policy")
        != ALWAYS_INSERTION_KIND
        or route_delta.get("all_archive_bindings_preserved") is not True
        or route_delta.get("all_member_bindings_preserved") is not True
        or route_delta.get("global_source_bindings_preserved") is not True
    ):
        raise PackageContractError(
            "v13 always route-delta provenance drifted."
        )
    ...
    return {
        ...
        "final_binding": {
            ...
            "canonical_sha256": str(final["sha256"]),
        ...
        "route_delta_binding": {
            "path": route_delta_path.relative_to(root).as_posix(),
            "sha256": sha256_file(route_delta_path),
            "canonical_sha256": str(route_delta["sha256"]),
            "size_bytes": route_delta_path.stat().st_size,
        },
        "rows": rows,
    }
...
def validate_smoke_receipt(
    ...
    if (
        ...
        or [row.get("route_id") for row in observations] != list(ROUTE_IDS)
    ):
