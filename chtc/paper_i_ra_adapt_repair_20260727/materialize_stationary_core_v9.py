...
seam.  It preserves v1-v8, reuses the 24 selected macro source locks byte for
byte, derives six conventional singleton locks from the canonical Append
registry, derives 18 RA singleton locks from the six immutable CHTC 9381198
plateau manifests, verifies every archive/member/global source exactly,
validates all 48 protocols, and publishes v9 with no-replace semantics.

The four CHTC 9381198 archives that were not previously locked by v8 are
honestly classified as first-lock acquisitions from the retrieved immutable
transfer bytes.  Their digests are not represented as independently
pre-existing expected values.  None of the 9381198 scientific results are
adopted as Paper-I evidence.
"""
...
REPO_ROOT = v8.REPO_ROOT
MATERIALIZATIONS_ROOT = v8.MATERIALIZATIONS_ROOT
support = v8.support
V8_ROOT = v8.V8_ROOT
V9_ROOT = (
    MATERIALIZATIONS_ROOT / "ra_adapt_stationary_late_core_v9"
)
V8_SOURCE_LOCKS_INPUT = (
    V8_ROOT / "source_materialization" / "source_locks_input.json"
)
V8_PROBLEM_BASELINES = (
    V8_ROOT / "source_materialization" / "problem_baselines.json"
)
V8_FINAL_RECEIPT = V8_ROOT / "final_materialization_receipt.json"
APPEND_REGISTRY = (
    REPO_ROOT
...
ICM_RECEIPT = (
    ...
    "materialize-stationary-core-v9.json"
)
FINAL_RECEIPT_NAME = "final_publication_receipt.json"

EXPECTED_V8_FILE_COUNT = 375
EXPECTED_V8_TOTAL_SIZE_BYTES = 13_597_637
EXPECTED_V8_RELATIVE_TREE_SHA256 = (
    "7c17616e7ffd93be15340b60c49511913211b6c1fbff3068d8a687d6e523b3c1"
)
EXPECTED_V8_FINAL_RECEIPT_FILE_SHA256 = (
    "03db7513e7a033f5ae629bbcf0a316fe90224ad8e8ac49c3e803ffd7325f1e26"
)
EXPECTED_V8_FINAL_RECEIPT_CANONICAL_SHA256 = (
    "890ed51bd15499c7d1ea70665b18843de3e20058b9782fca99266851375890e2"
)
EXPECTED_V8_SOURCE_LOCKS_FILE_SHA256 = (
    "bee791d98a008e604e053ff07a6ce55117c448d32ffec6c747b1771c2e7c4fba"
)
EXPECTED_V8_PROBLEM_BASELINES_FILE_SHA256 = (
...
V8_SOURCE_LOCKS_PATH = V8_SOURCE_LOCKS_INPUT.relative_to(REPO_ROOT).as_posix()
...
MaterializationAuditError = v8.MaterializationAuditError
...
def _load_mapping(path: Path, *, label: str) -> dict[str, Any]:
    return v8._load_mapping(path, label=label)


def _load_digested(path: Path, *, label: str) -> dict[str, Any]:
    return v8._load_canonical_digested(path, label=label)
...
def _write_receipt(path: Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    return v8._write_receipt(path, payload)
...
def _historical_snapshots() -> dict[str, dict[str, Any]]:
    snapshots = v8._historical_snapshots()
    snapshots["v8"] = support._snapshot_roots((V8_ROOT,))
    return snapshots
...
def _assert_historical_anchors(
    ...
    v8._assert_historical_anchors(snapshots)
    observed = snapshots.get("v8")
    if not isinstance(observed, Mapping):
        raise MaterializationAuditError("Missing immutable v8 snapshot.")
    relative = support._snapshot_roots((V8_ROOT,), relative_to=V8_ROOT)
    _assert_equal(
        int(relative["file_count"]),
        EXPECTED_V8_FILE_COUNT,
        label="v8 immutable file count",
    )
    _assert_equal(
        int(relative["total_size_bytes"]),
        EXPECTED_V8_TOTAL_SIZE_BYTES,
        label="v8 immutable total bytes",
    )
    _assert_equal(
        relative["tree_sha256"],
        EXPECTED_V8_RELATIVE_TREE_SHA256,
        label="v8 immutable relative tree SHA-256",
    )
    _assert_equal(
        support._hash_file(V8_FINAL_RECEIPT),
        EXPECTED_V8_FINAL_RECEIPT_FILE_SHA256,
        label="v8 final receipt file SHA-256",
    )
    final = _load_digested(
        V8_FINAL_RECEIPT, label="v8 final materialization receipt"
    )
    _assert_equal(
        final["sha256"],
        EXPECTED_V8_FINAL_RECEIPT_CANONICAL_SHA256,
        label="v8 final receipt canonical SHA-256",
    )
...
def _ra_singleton_locks(
    *,
    ...
    for regime_id, nph in CLAIM_FACING_REGIME_CUTOFF_PAIRS:
        ...
        archive_authority = (
            {
                "kind": "predecessor_v8_archive_lock_v1",
                "predecessor_source_locks_path": V8_SOURCE_LOCKS_PATH,
...
def _build_source_locks(
    *,
    v8_source_locks: Mapping[str, Any],
    registry: Mapping[str, Any],
    baselines: Mapping[str, Any],
) -> tuple[
    dict[str, Any],
    ...
    ed_by_regime = _same_cutoff_ed_by_regime()
    raw_v8_cells = v8_source_locks.get("cell_locks")
    if not isinstance(raw_v8_cells, Mapping):
        raise MaterializationAuditError(
            "v8 source locks have no cell mapping."
        )
    macro_locks: dict[str, Any] = {}
    for regime_id, nph in CLAIM_FACING_REGIME_CUTOFF_PAIRS:
        for route_id in MACRO_ROUTE_IDS:
            lock_id = source_lock_id(regime_id, int(nph), route_id)
            source = raw_v8_cells.get(lock_id)
            if not isinstance(source, Mapping):
                raise MaterializationAuditError(
                    f"v8 source locks are missing {lock_id}."
                )
            macro_locks[lock_id] = copy.deepcopy(source)
    append_locks, append_rows = _append_singleton_locks(
        registry=registry,
        ed_by_regime=ed_by_regime,
    )
    settings_by_regime, semantic_rows = (
        _read_and_validate_transfer_manifests(baselines=baselines)
    )
    ra_locks, ra_rows = _ra_singleton_locks(
        settings_by_regime=settings_by_regime,
        ed_by_regime=ed_by_regime,
    )
    cell_locks = {**macro_locks, **append_locks, **ra_locks}
    required_ids = {
    ...
    _assert_equal(
        set(cell_locks),
        required_ids,
        label="stationary-core source-lock id set",
    )
    source_locks = {
        "schema": SOURCE_LOCK_SCHEMA,
        "global_sources": copy.deepcopy(
            v8_source_locks["global_sources"]
        ),
    ...
    macro_receipt = _digested(
        {
            ...
            "objects_equal_to_predecessor": all(
                macro_locks[lock_id] == raw_v8_cells[lock_id]
    ...
    singleton_receipt = _digested(
        {
            ...
            "append_registry": {
                "path": APPEND_REGISTRY_PATH,
                "file_sha256": EXPECTED_APPEND_REGISTRY_FILE_SHA256,
                "record_count": len(append_rows),
            },
            "append_singleton_anchors": append_rows,
            "chtc_9381198_singleton_plateau_anchors": ra_rows,
            "append_source_lock_count": len(append_locks),
            "ra_source_lock_count": len(ra_locks),
            "singleton_source_lock_count": (
                len(append_locks) + len(ra_locks)
            ),
            "all_scientific_result_anchor_claims_false": True,
            "four_archive_first_lock_acquisitions": [
                row
                for row in ra_rows
                if row["proc_id"] in {0, 1, 4, 5}
            ],
            "first_lock_semantics": {
                "source": "retrieved_immutable_transfer_bytes",
                "independently_preexisting_expected_hash": False,
                "acquisition_date": "2026-07-28",
                "acquisition_tool": "shasum -a 256",
                "later_exact_verification_required": True,
            },
            "predecessor_locked_archive_proc_ids": [2, 3],
        }
    )
    semantic_receipt = _digested(
        {
            ...
            "status": "passed",
            "source_role": (
                "settings_and_route_derivation_only_no_result_adoption_v1"
            ),
            "cluster_id": 9381198,
    ...
    return _digested(
        {
            ...
            "predecessor_v8": {
                ...
                "file_count": EXPECTED_V8_FILE_COUNT,
                "total_size_bytes": (
                    EXPECTED_V8_TOTAL_SIZE_BYTES
                ),
            ...
            "materialization": {
                "path": V9_ROOT.relative_to(REPO_ROOT).as_posix(),
...
def _complete_missing_icm_receipt() -> int:
    """Safely complete the sole receipt outside an already-published v9."""

    if not V9_ROOT.is_dir() or ICM_RECEIPT.exists():
        raise MaterializationAuditError(
            "The missing-ICM recovery path requires published v9 and no "
            "existing ICM receipt."
        )
    support._darwin_renameatx_np()
    historical = _historical_snapshots()
    _assert_historical_anchors(historical)
    final_path = V9_ROOT / FINAL_RECEIPT_NAME
    ...
    source_locks = _load_digested(
        V9_ROOT / CORE_BUNDLE_ID / "source_locks.json",
    ...
    loader_receipt = _load_digested(
        V9_ROOT / "cross_file_loader_validation.json",
    ...
    persisted = _load_digested(
        ICM_RECEIPT,
        label="recovered stationary-core v9 ICM receipt",
    )
    _assert_equal(
        persisted,
        icm_payload,
        label="recovered stationary-core v9 ICM receipt payload",
    )
    print(
        json.dumps(
            {
                "destination": V9_ROOT.relative_to(
                    REPO_ROOT
                ).as_posix(),
                "status": "passed",
...
def main() -> int:
    if V9_ROOT.exists():
        ...
        raise FileExistsError(
            f"Refusing to overwrite immutable v9: {V9_ROOT}"
        )
    ...
    _assert_equal(
        support._hash_file(V8_SOURCE_LOCKS_INPUT),
        EXPECTED_V8_SOURCE_LOCKS_FILE_SHA256,
        label="v8 source-lock input file SHA-256",
    )
    _assert_equal(
        support._hash_file(V8_PROBLEM_BASELINES),
        EXPECTED_V8_PROBLEM_BASELINES_FILE_SHA256,
        label="v8 problem-baselines file SHA-256",
    )
    _assert_equal(
    ...
    v8_source_locks = _load_mapping(
        V8_SOURCE_LOCKS_INPUT, label="v8 source-lock input"
    )
    baselines = _load_mapping(
        V8_PROBLEM_BASELINES, label="v8 problem baselines"
    )
    ...
    staging_root = Path(
        tempfile.mkdtemp(
            prefix=".ra_adapt_stationary_late_core_v9.staging.",
    ...
    support._write_bytes_atomic_no_replace(
        ...
        V8_PROBLEM_BASELINES.read_bytes(),
    )
    _assert_equal(
        support._hash_file(source_root / "problem_baselines.json"),
        EXPECTED_V8_PROBLEM_BASELINES_FILE_SHA256,
        label="copied v9 problem-baselines SHA-256",
    )
    ...
    (
        ...
        v8_source_locks=v8_source_locks,
        registry=registry,
        baselines=baselines,
    )
    _write_plain_json(
    ...
    preflight_receipt = _write_receipt(
        ...
        {
            "schema": (
                "paper_i_ra_adapt_stationary_core_v9_preflight_v1"
            ),
            "status": "passed",
            ...
            "source_inputs": {
                ...
                "problem_baselines": {
                    "path": V8_PROBLEM_BASELINES.relative_to(
                        REPO_ROOT
                    ).as_posix(),
                    "file_sha256": (
                        EXPECTED_V8_PROBLEM_BASELINES_FILE_SHA256
    ...
    _assert_historical_unchanged(
        ...
        label="Staged stationary-core v9 materialization",
    )
    ...
    staged_receipt = _write_receipt(
        ...
        {
            "schema": (
                "paper_i_ra_adapt_stationary_core_v9_staged_"
                "materialization_receipt_v1"
            ),
            "status": "passed",
            "campaign_id": CORE_CAMPAIGN_ID,
    ...
    final_receipt = _write_receipt(
        staging_root / FINAL_RECEIPT_NAME,
        {
            ...
            "materialization_id": "ra_adapt_stationary_late_core_v9",
            "campaign_id": CORE_CAMPAIGN_ID,
    ...
    _assert_historical_unchanged(
        ...
        label="Complete staged stationary-core v9 materialization",
    )
    ...
    support._atomic_rename_no_replace(staging_root, V9_ROOT)

    published_final = _load_digested(
        V9_ROOT / FINAL_RECEIPT_NAME,
    ...
    _assert_historical_unchanged(
        historical_before,
        historical_after,
        label="Published stationary-core v9 materialization",
    )
    ...
    _assert_equal(
        ...
        label="staged-to-published complete v9 tree",
    )
    ...
    persisted_icm = _load_digested(
        ICM_RECEIPT,
        label="stationary-core v9 ICM stage receipt",
    )
    _assert_equal(
        persisted_icm,
        icm_payload,
        label="stationary-core v9 ICM receipt payload",
    )
    print(
        json.dumps(
            {
