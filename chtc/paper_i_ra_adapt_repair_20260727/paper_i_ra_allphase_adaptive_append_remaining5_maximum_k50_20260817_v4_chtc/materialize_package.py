...
def main() -> int:
    ...
    source_manifest = digested(
        {
            "schema": "paper_i_ra_allphase_adaptive_append_remaining5_source_archive_v1",
    ...
    if len(jobs) != 5 or len({row["execution_id"] for row in jobs}) != 5:
        raise RuntimeError("matrix is not exactly five unique cells")
    ...
    plan = digested(
        {
            ...
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "matrix_shape": {"arms": 1, "regimes": 5, "cells": 5},
            "arms": list(ARMS),
