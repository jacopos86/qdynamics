...
seam. It preserves v1-v9, reuses all 48 selected v9 scientific source locks
...
protocols, and publishes v10 with no-replace semantics. None of the CHTC
...
from chtc.paper_i_ra_adapt_repair_20260727 import (
    materialize_stationary_core_v9 as v9,
)
...
REPO_ROOT = v9.REPO_ROOT
MATERIALIZATIONS_ROOT = v9.MATERIALIZATIONS_ROOT
support = v9.support
V9_ROOT = v9.V9_ROOT
V10_ROOT = (
    MATERIALIZATIONS_ROOT / "ra_adapt_stationary_late_core_v10"
)
V9_SOURCE_LOCKS_INPUT = (
    V9_ROOT / "source_materialization" / "source_locks_input.json"
)
V9_PROBLEM_BASELINES = (
    V9_ROOT / "source_materialization" / "problem_baselines.json"
)
V9_FINAL_RECEIPT = V9_ROOT / "final_publication_receipt.json"
APPEND_REGISTRY = (
    REPO_ROOT
...
ICM_RECEIPT = (
    ...
    "materialize-stationary-core-v10.json"
)
FINAL_RECEIPT_NAME = "final_publication_receipt.json"

EXPECTED_V9_FILE_COUNT = 111
EXPECTED_V9_TOTAL_SIZE_BYTES = 4_253_443
EXPECTED_V9_RELATIVE_TREE_SHA256 = (
    "81ce37263961cec9b51720c5917a672b5158adb5211a747372216d3b13a004e9"
)
EXPECTED_V9_FINAL_RECEIPT_FILE_SHA256 = (
    "385fdc46d0969b7a5c8cc8add9c6ce8f51521fb0b22e836ca97f6f82a40b66ba"
)
EXPECTED_V9_FINAL_RECEIPT_CANONICAL_SHA256 = (
    "98df45352bfd5ebb8041f8a17a30ddb1c2870482f9d3269d5d0873f825664e58"
)
EXPECTED_V9_SOURCE_LOCKS_FILE_SHA256 = (
    "e31a6a0814cdee2d1b4bce7b7fefd18612e0f5d93fe5010f7e48a4e6679acccc"
)
EXPECTED_V9_PROBLEM_BASELINES_FILE_SHA256 = (
...
V9_SOURCE_LOCKS_PATH = V9_SOURCE_LOCKS_INPUT.relative_to(REPO_ROOT).as_posix()
...
MaterializationAuditError = v9.MaterializationAuditError
...
def _load_mapping(path: Path, *, label: str) -> dict[str, Any]:
    return v9._load_mapping(path, label=label)


def _load_digested(path: Path, *, label: str) -> dict[str, Any]:
    return v9._load_digested(path, label=label)
...
def _write_receipt(path: Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    return v9._write_receipt(path, payload)
...
def _historical_snapshots() -> dict[str, dict[str, Any]]:
    snapshots = v9._historical_snapshots()
    snapshots["v9"] = support._snapshot_roots((V9_ROOT,))
    return snapshots
...
def _assert_historical_anchors(
    ...
    v9._assert_historical_anchors(snapshots)
    observed = snapshots.get("v9")
    if not isinstance(observed, Mapping):
        raise MaterializationAuditError("Missing immutable v9 snapshot.")
    relative = support._snapshot_roots((V9_ROOT,), relative_to=V9_ROOT)
    _assert_equal(
        int(relative["file_count"]),
        EXPECTED_V9_FILE_COUNT,
        label="v9 immutable file count",
    )
    _assert_equal(
        int(relative["total_size_bytes"]),
        EXPECTED_V9_TOTAL_SIZE_BYTES,
        label="v9 immutable total bytes",
    )
    _assert_equal(
        relative["tree_sha256"],
        EXPECTED_V9_RELATIVE_TREE_SHA256,
        label="v9 immutable relative tree SHA-256",
    )
    _assert_equal(
        support._hash_file(V9_FINAL_RECEIPT),
        EXPECTED_V9_FINAL_RECEIPT_FILE_SHA256,
        label="v9 final receipt file SHA-256",
    )
    final = _load_digested(
        V9_FINAL_RECEIPT, label="v9 final materialization receipt"
    )
    _assert_equal(
        final["sha256"],
        EXPECTED_V9_FINAL_RECEIPT_CANONICAL_SHA256,
        label="v9 final receipt canonical SHA-256",
    )
...
def _ra_singleton_locks(
    *,
    ...
    for regime_id, nph in CLAIM_FACING_REGIME_CUTOFF_PAIRS:
        ...
        archive_authority = (
            {
                "kind": "predecessor_v9_archive_lock_v1",
                "predecessor_source_locks_path": V9_SOURCE_LOCKS_PATH,
...
def _build_source_locks(
    *,
    v9_source_locks: Mapping[str, Any],
    registry: Mapping[str, Any],
    baselines: Mapping[str, Any],
) -> tuple[
    dict[str, Any],
    ...
    _assert_equal(
        v9_source_locks.get("schema"),
        SOURCE_LOCK_SCHEMA,
        label="v9 source-lock schema",
    )
    raw_v9_cells = v9_source_locks.get("cell_locks")
    if not isinstance(raw_v9_cells, Mapping):
        raise MaterializationAuditError(
            "v9 source locks have no cell mapping."
        )
    required_ids = {
    ...
    _assert_equal(
        set(raw_v9_cells),
        required_ids,
        label="v9 stationary-core source-lock id set",
    )
    global_sources = v9_source_locks.get("global_sources")
    if not isinstance(global_sources, Mapping) or not global_sources:
        raise MaterializationAuditError(
            "v9 source locks have no global source mapping."
        )
    source_locks = copy.deepcopy(dict(v9_source_locks))
    macro_locks = {
        lock_id: copy.deepcopy(raw_v9_cells[lock_id])
        for lock_id in sorted(raw_v9_cells)
        if str(raw_v9_cells[lock_id]["route_id"]) in MACRO_ROUTE_IDS
    }
    singleton_locks = {
        lock_id: copy.deepcopy(raw_v9_cells[lock_id])
        for lock_id in sorted(raw_v9_cells)
        if str(raw_v9_cells[lock_id]["route_id"])
        in SINGLETON_CORE_ROUTE_IDS
    }
    _assert_equal(
        (len(macro_locks), len(singleton_locks)),
        (24, 24),
        label="v9 macro/singleton source-lock counts",
    )
    macro_receipt = _digested(
        {
            ...
            "objects_equal_to_predecessor": all(
                macro_locks[lock_id] == raw_v9_cells[lock_id]
    ...
    singleton_receipt = _digested(
        {
            ...
            "status": "passed",
            "derivation_mode": (
                "predecessor_v9_exact_source_lock_reuse_v1"
            ),
            "predecessor_source_locks": {
                "path": V9_SOURCE_LOCKS_PATH,
                "file_sha256": (
            ...
            "problem_baselines": {
                "path": V9_PROBLEM_BASELINES.relative_to(
                    REPO_ROOT
                ).as_posix(),
                "file_sha256": (
                    EXPECTED_V9_PROBLEM_BASELINES_FILE_SHA256
            ...
            "objects_equal_to_predecessor": all(
                singleton_locks[lock_id] == raw_v9_cells[lock_id]
    ...
    ra_singleton_rows = [
        {
            ...
            "predecessor_v9_object_reused_without_edit": True,
            "scientific_result_anchor_claimed": False,
        }
    ...
    semantic_receipt = _digested(
        {
            ...
            "status": "passed",
            "source_role": (
                "predecessor_v9_source_authority_reuse_no_result_"
                "adoption_v1"
            ),
            "cluster_id": 9381198,
    ...
    return _digested(
        {
            ...
            "predecessor_v9": {
                ...
                "file_count": EXPECTED_V9_FILE_COUNT,
                "total_size_bytes": (
                    EXPECTED_V9_TOTAL_SIZE_BYTES
                ),
...
def _complete_missing_icm_receipt() -> int:
    """Safely complete the sole receipt outside an already-published v10."""

    if not V10_ROOT.is_dir() or ICM_RECEIPT.exists():
        raise MaterializationAuditError(
            "The missing-ICM recovery path requires published v10 and no "
            "existing ICM receipt."
        )
    support._darwin_renameatx_np()
    historical = _historical_snapshots()
    _assert_historical_anchors(historical)
    final_path = V10_ROOT / FINAL_RECEIPT_NAME
    ...
    source_locks = _load_digested(
        V10_ROOT / CORE_BUNDLE_ID / "source_locks.json",
    ...
    loader_receipt = _load_digested(
        V10_ROOT / "cross_file_loader_validation.json",
    ...
    persisted = _load_digested(
        ICM_RECEIPT,
        label="recovered stationary-core v10 ICM receipt",
    )
    _assert_equal(
        persisted,
        icm_payload,
        label="recovered stationary-core v10 ICM receipt payload",
    )
    print(
        json.dumps(
            {
                "destination": V10_ROOT.relative_to(
                    REPO_ROOT
                ).as_posix(),
                "status": "passed",
...
def main() -> int:
    if V10_ROOT.exists():
        ...
        raise FileExistsError(
            f"Refusing to overwrite immutable v10: {V10_ROOT}"
        )
    ...
    _assert_equal(
        support._hash_file(V9_SOURCE_LOCKS_INPUT),
        EXPECTED_V9_SOURCE_LOCKS_FILE_SHA256,
        label="v9 source-lock input file SHA-256",
    )
    _assert_equal(
        support._hash_file(V9_PROBLEM_BASELINES),
        EXPECTED_V9_PROBLEM_BASELINES_FILE_SHA256,
        label="v9 problem-baselines file SHA-256",
    )
    _assert_equal(
    ...
    v9_source_locks = _load_mapping(
        V9_SOURCE_LOCKS_INPUT, label="v9 source-lock input"
    )
    baselines = _load_mapping(
        V9_PROBLEM_BASELINES, label="v9 problem baselines"
    )
    ...
    staging_root = Path(
        tempfile.mkdtemp(
            prefix=".ra_adapt_stationary_late_core_v10.staging.",
    ...
    support._write_bytes_atomic_no_replace(
        ...
        V9_PROBLEM_BASELINES.read_bytes(),
    )
    _assert_equal(
        support._hash_file(source_root / "problem_baselines.json"),
        EXPECTED_V9_PROBLEM_BASELINES_FILE_SHA256,
        label="copied v10 problem-baselines SHA-256",
    )
    ...
    (
        ...
        v9_source_locks=v9_source_locks,
        registry=registry,
        baselines=baselines,
    )
    _write_plain_json(
    ...
    preflight_receipt = _write_receipt(
        ...
        {
            "schema": (
                "paper_i_ra_adapt_stationary_core_v10_preflight_v1"
            ),
            "status": "passed",
    ...
    _assert_historical_unchanged(
        ...
        label="Staged stationary-core v10 materialization",
    )
    ...
    staged_receipt = _write_receipt(
        ...
        {
            "schema": (
                "paper_i_ra_adapt_stationary_core_v10_staged_"
                "materialization_receipt_v1"
            ),
            "status": "passed",
            "campaign_id": CORE_CAMPAIGN_ID,
    ...
    final_receipt = _write_receipt(
        staging_root / FINAL_RECEIPT_NAME,
        {
            ...
            "materialization_id": "ra_adapt_stationary_late_core_v10",
            "campaign_id": CORE_CAMPAIGN_ID,
            ...
            "atomic_publish": {
                ...
                "post_publish_v10_write_count": 0,
            },
            "tree": {
                "scope": (
                    "materialization_tree_excluding_"
    ...
    _assert_historical_unchanged(
        ...
        label="Complete staged stationary-core v10 materialization",
    )
    ...
    support._atomic_rename_no_replace(staging_root, V10_ROOT)

    published_final = _load_digested(
        V10_ROOT / FINAL_RECEIPT_NAME,
    ...
    _assert_historical_unchanged(
        historical_before,
        historical_after,
        label="Published stationary-core v10 materialization",
    )
    ...
    _assert_equal(
        ...
        label="staged-to-published complete v10 tree",
    )
    ...
    persisted_icm = _load_digested(
        ICM_RECEIPT,
        label="stationary-core v10 ICM stage receipt",
    )
    _assert_equal(
        persisted_icm,
        icm_payload,
        label="stationary-core v10 ICM receipt payload",
    )
    print(
        json.dumps(
            {
