...
def _validate_controller_overlay_binding(overlay: Mapping[str, Any]) -> None:
    after = _mapping(overlay.get("after"), label="controller overlay")
    expected = Path("source_overlay") / CONTROLLER_RELATIVE_PATH
    if safe_relative_path(
        after.get("path"), label="controller overlay path"
    ) != expected:
        raise PackageContractError("Controller overlay path drifted.")
    _verify_binding(after, label="controller overlay")
...
def validate(*, full_resume: bool, worker_preflight: bool) -> dict[str, Any]:
    ...
    overlay = _mapping(composition.get("operational_overlay"), label="overlay")
    _validate_controller_overlay_binding(overlay)
    if (
        ...
        or overlay.get("semantic_scope") != "accepted_energy_roundoff_only"
    ...
    return digested(
        {
            "schema": "paper_i_page12_strong_r70_local_validation_v1",
            "status": "passed",
