#!/usr/bin/env python3
"""Materialize operational-repair v3 from immutable worker-durable v2."""
...
def _load_source_package(repo_root: Path) -> dict[str, Any]:
    ...
    manifest = _load_bound_json(
        ...
        label="sealed v2 worker-durable predecessor package",
    )
...
def _apply_exact_source_patch(source_root: Path) -> list[dict[str, Any]]:
    ...
    if completed.returncode != 0:
        raise PackageContractError(
            "Exact v2-to-v3 source patch failed: "
...
def build() -> dict[str, Any]:
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix="paper-i-phase3-on-plateau-v3-source-"
    )
    source_root = (
    ...
    if (
        ...
        raise PackageContractError(
            "v2-derived v3 implementation/source-lock closure drifted."
        )
    ...
    if (
        ...
        raise PackageContractError(
            "v3 source inventory changed v2 archive membership."
        )
    members: list[dict[str, Any]] = []
    ...
    for regime_id, nph in REGIME_ROWS:
        ...
        if source_id not in source_rows:
            raise PackageContractError(
                f"Missing immutable-v2 source cell: {source_id}"
            )
        ...
        if (
            ...
            raise PackageContractError(
                f"Immutable-v2 source cell drifted: {source_id}"
            )
    ...
    for target_cell in target_cells:
        ...
        if not isinstance(source_payload, dict):
            raise PackageContractError(
                "Immutable-v2 protocol is not a mapping."
            )
        ...
        if source_protocol.sha256 != source_payload.get("sha256"):
            raise PackageContractError(
                "Immutable-v2 protocol digest drifted."
            )
