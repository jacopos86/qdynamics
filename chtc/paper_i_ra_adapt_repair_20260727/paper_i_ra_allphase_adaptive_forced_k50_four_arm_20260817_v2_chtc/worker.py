...
def _completion(
    *,
    ...
    native_contract = route.get("native_semantic_contract")
    execution_settings = route.get("execution_settings")
    if (
        not isinstance(native_contract, Mapping)
        or not isinstance(execution_settings, Mapping)
        or native_contract.get("phase3_no_positive_policy")
        != "forced_admission_exact_horizon_v1"
        or native_contract.get("controller_horizon_policy")
        != "exact_requested_accepted_controller_rounds_v1"
        or execution_settings.get("ra_phase3_no_positive_policy")
        != "forced_admission_exact_horizon_v1"
        or execution_settings.get("ra_controller_horizon_policy")
        != "exact_requested_accepted_controller_rounds_v1"
    ):
        raise RuntimeError("forced-k50 route contract drifted")
    ...
    if (
        ...
        raise RuntimeError("forced completion is not exact k50")
    ...
    return digested(
        {
            "schema": "paper_i_ra_allphase_adaptive_forced_k50_four_arm_cell_completion_v1",
