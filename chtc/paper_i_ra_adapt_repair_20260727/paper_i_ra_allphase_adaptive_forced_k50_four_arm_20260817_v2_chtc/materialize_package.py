...
def main() -> int:
    ...
    source_manifest = digested(
        {
            "schema": "paper_i_ra_allphase_adaptive_forced_k50_four_arm_source_archive_v1",
    ...
    if len(jobs) != 24 or len({row["execution_id"] for row in jobs}) != 24:
        raise RuntimeError("matrix is not exactly twenty-four unique cells")
    ...
    plan = digested(
        {
            ...
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "matrix_shape": {"arms": 4, "regimes": 6, "cells": 24},
            "arms": list(ARMS),
    ...
    manifest = digested(
        {
            ...
            "status": "authorized_twenty_four_cells",
            "created_at": created_at,
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "row_count": 24,
