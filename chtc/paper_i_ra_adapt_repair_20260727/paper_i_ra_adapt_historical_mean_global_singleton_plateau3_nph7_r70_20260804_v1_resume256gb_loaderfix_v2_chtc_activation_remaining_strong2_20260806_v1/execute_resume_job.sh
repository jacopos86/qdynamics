...
if [[ "$#" -ne 13 ]]; then
  echo "usage: $0 ACTIVATION PACKAGE JOB JOB_SHA AUTH AUTH_SHA SOURCE SOURCE_SHA IMAGE IMAGE_SHA RESUME_BASENAME RESUME_SHA OUTPUT_PREFIX" >&2
  exit 64
fi
...
expected_job_sha="$4"
authorization="$5"
expected_authorization_sha="$6"
source_archive="$7"
expected_source_sha="$8"
image_path="$9"
expected_image_sha="${10}"
resume_archive="${11}"
expected_resume_sha="${12}"
output_prefix="${13}"
...
case "$resume_archive" in
  */*|*".."*|"")
    echo "resume input must be the staged sandbox basename" >&2
    exit 64
    ;;
esac
case "$output_prefix" in
  *[!A-Za-z0-9_.-]*|"")
    echo "unsafe output prefix" >&2
    exit 64
    ;;
esac
...
for required in \
  ...
  "$activation_manifest" \
  "$archive_builder" \
  "${package_dir}/run_resume_cell.py"
do
...
if [[ "$(hash_file "$job_spec")" != "$expected_job_sha" \
   || "$(hash_file "$authorization")" != "$expected_authorization_sha" \
   || "$(hash_file "$source_archive")" != "$expected_source_sha" \
   || "$(hash_file "$resume_archive")" != "$expected_resume_sha" ]]; then
  echo "worker input SHA-256 mismatch" >&2
  exit 66
fi
...
if [[ -z "$job_ad_path" || ! -f "$job_ad_path" || -L "$job_ad_path" ]]; then
  echo "HTCondor job ad unavailable" >&2
  exit 70
fi
read_ad_integer() {
  local name="$1"
...
output_archive="${output_prefix}__${execution_id}__cluster_${cluster_id}__proc_${proc_id}__attempt_${attempt_ordinal}.tar.gz"
...
if [[ -e "$worker_root" || -L "$worker_root" ]]; then
  ...
  prior_output="${output_prefix}__${execution_id}__cluster_${cluster_id}__proc_${proc_id}__attempt_${prior_attempt}.tar.gz"
  rm -rf -- "$worker_root"
  if [[ -e "$prior_output" ]]; then
    [[ -f "$prior_output" && ! -L "$prior_output" ]] || exit 72
    rm -f -- "$prior_output"
  fi
elif [[ -e "$output_archive" || -L "$output_archive" ]]; then
  echo "orphaned prior attempt archive" >&2
  exit 72
fi
...
package_attempt() {
  ...
  "$runtime_bin" exec \
    ...
    python3 "$archive_builder" \
      ...
      --image-sha256 "$expected_image_sha" \
    || packaging_status=71
...
trap terminate_job TERM INT HUP
...
"$runtime_bin" exec \
  ...
  --pwd "$worker_abs" \
  "$image_abs" \
  python3 "${package_abs}/run_resume_cell.py" \
    --run \
    --job "$job_abs" \
    --execution-authorization "$authorization_abs" \
    --output-dir "${worker_abs}/artifacts" \
