#!/usr/bin/env python3
"""Capacity-audited all-six append-then-plateau Paper-I RA campaign."""
...
CAMPAIGN_ID = (
    "paper_i_ra_all6_adaptive_shortlist_append_then_plateau_k50_"
    "overnight_20260816_v2"
)
TARGET_HORIZON = 50
...
PLAN_SCHEMA = "paper_i_ra_all6_adaptive_overnight_plan_v1"
AUTH_SCHEMA = "paper_i_ra_all6_direct_execution_authorization_v1"
MANIFEST_SCHEMA = "paper_i_ra_all6_adaptive_execution_manifest_v1"
WORKER_SCHEMA = "paper_i_ra_all6_adaptive_worker_receipt_v1"
GUARD_SCHEMA = "paper_i_ra_all6_adaptive_guard_receipt_v1"
REPORT_SCHEMA = "paper_i_ra_all6_adaptive_comparison_v1"
TERMINAL_SCHEMA = "paper_i_ra_all6_adaptive_terminal_matrix_v1"
STATUS_SCHEMA = "paper_i_ra_all6_adaptive_status_v1"
SCHEDULER_SCHEMA = "paper_i_ra_all6_capacity_scheduler_decision_v1"
BATCH_RECEIPT_SCHEMA = "paper_i_ra_all6_archived_batch_closure_v1"
INITIAL_CAPACITY_SCHEMA = "paper_i_ra_all6_initial_capacity_receipt_v1"
COMPACT_CELL_SCHEMA = "paper_i_ra_all6_compact_cell_reporting_receipt_v1"
ARCHIVED_CELL_SCHEMA = "paper_i_ra_all6_archived_cell_closure_v1"
...
# This literal is deliberately non-executable.  It is replaced only after the
# production owner publishes the new versioned all-phase-adaptive route seam.
CORE_ROUTE_VARIANT_PLACEHOLDER = "__unresolved_all_phase_adaptive_core_route__"
CORE_ROUTE_CONSTANT = "PAPER_I_RA_ALL_PHASE_POSITION_ADAPTIVE_SHORTLIST_V1"
...
CORE_REQUEST_BUILDER = "build_paper_i_ra_all_phase_position_adaptive_request"
...
PLAN_RECEIPT_KEYS = frozenset(
    {
        "schema", "created_at", "campaign_id", "run_class", "target_horizon",
        ...
        for regime_id, nph in _REGIMES:
            ...
            rows.append(
                CellSpec(
                    ...
                    execution_id=(
                        "all_phase_adaptive__"
                        f"{regime_id}__nph{nph}__{insertion}__k50"
                    ),
                )
            )
    return tuple(rows)
    ...
    authorization = digested(
        {
            ...
            "created_at": serial_runtime.utc_now(),
            "campaign_id": CAMPAIGN_ID,
            "authorization_basis": "explicit_current_user_direct_all6_run_request",
...
def validate_authority(
    *, recompute_protocols: bool = False
    ...
    if (
        ...
        != "explicit_current_user_direct_all6_run_request"
...
def run_child(execution_id: str) -> int:
    ...
    if rounds != TARGET_HORIZON or result.run.paper_i_summary is None:
        raise RunnerError(
            f"Cell ended at {rounds}; exact k={TARGET_HORIZON} is required."
        )
    ...
    manifest = digested(
        {
            ...
            "execution_path_canary": {
                ...
                "accepted_round": 1 if cell.ordinal == 1 else None,
                "continued_same_trajectory_to_k50": cell.ordinal == 1,
            },
            "artifacts": artifacts,
            "submission_authorized": False,
            "paper_adoption_authorized": False,
...
def validate_worker_receipt(
    ...
    if (
        ...
        or worker.get("status") != "passed_k50"
...
def _reporting_selector_closure_rounds(
    ...
    if (
        ...
        or closure.get("validated_round_count") != TARGET_HORIZON
        or not isinstance(rounds, list)
        or len(rounds) != TARGET_HORIZON
...
def report_rows(
    ...
    if not (
        ...
        raise RunnerError("Cell report inputs do not close at exact k=50.")
    rows: list[dict[str, Any]] = []
...
def load_closed_cell(
    ...
    *,
    ...
    if (
        manifest.get("status") != "passed_k50"
    ...
    if len(report_rows(cell, result, summary)) != TARGET_HORIZON:
        raise RunnerError(f"Cell report does not contain exact k=50: {cell.execution_id}")
