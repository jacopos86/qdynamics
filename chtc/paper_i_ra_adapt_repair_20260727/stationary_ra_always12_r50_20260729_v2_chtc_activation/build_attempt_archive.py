#!/usr/bin/env python3
"""Create one authenticated, deterministic worker-attempt archive."""
...
def _safe_relative(value: str, *, label: str) -> PurePosixPath:
    ...
    if (
        ...
        raise AttemptArchiveError(f"Unsafe {label}: {value}")
    return path
...
def build_archive(args: argparse.Namespace) -> dict[str, Any]:
    ...
    worker_files = _validated_files(worker_root)
    worker_bindings = [
        _file_row(path, base=worker_root) for path in worker_files
    ]
    receipt: dict[str, Any] = {
        "schema": (
            "paper_i_ra_always_commutation_reduced_worker_attempt_v1"
        ),
        ...
        with temporary.open("xb") as raw:
            with gzip.GzipFile(
                ...
                with tarfile.open(
                    ...
                    for path, row in zip(
                        worker_files, worker_bindings, strict=True
                    ):
                        name = (
                            PurePosixPath("worker_outputs")
                            / _safe_relative(
                                row["path"], label="worker member"
                            )
                        ).as_posix()
                        with path.open("rb") as stream:
                            archive.addfile(
                    ...
                    archive.addfile(
                        ...
                        fileobj=__import__("io").BytesIO(receipt_bytes),
                    )
            raw.flush()
            os.fsync(raw.fileno())
