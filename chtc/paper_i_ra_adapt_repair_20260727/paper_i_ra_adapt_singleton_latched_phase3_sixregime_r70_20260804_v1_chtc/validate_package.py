#!/usr/bin/env python3
"""Fail-closed validation for the inert latched-Phase-III r70 package."""
...
def validate_package(
    *, deep: bool = False, smoke_one_round: bool = False
    ...
    for name in canonical_names:
        path, payload = _bound_file(
            manifest.get(name), label=name, canonical=True
        )
    ...
    if (
        ...
        or plan.get("row_count") != 6
        or plan.get("execution_mode") != "fresh_0_to_70"
        ...
        or plan.get("worker_owned_live_progress") is not True
        or plan.get("checkpoint_dynamic_sidecars_hash_bound") is not True
        or plan.get("same_filesystem_atomic_success_publication") is not True
        or plan.get("post_validation_failure_evidence_preserved") is not True
        or plan.get("execution_authorized") is not False
        or plan.get("submission_ready") is not False
        or audit.get("status") != "passed"
        or audit.get("cell_count") != 6
        or bundle.get("cells") is None
        or len(bundle["cells"]) != 6
        or bundle.get("execution_authorized") is not False
        or locks.get("implementation_sources", {}).get("sha256")
        != TARGET_IMPLEMENTATION_INVENTORY_SHA256
        or locks.get("sha256") != TARGET_SOURCE_LOCKS_CANONICAL_SHA256
        or sha256_file(document_paths["source_locks_snapshot"])
        != TARGET_SOURCE_LOCKS_FILE_SHA256
    ):
        raise PackageContractError("Plan/audit/protocol-bundle closure drifted.")

    expected_difference_paths = {
        tuple(path) for path in SOURCE_TO_TARGET_DIFFERENCE_PATHS
    }
    ...
    if (
        ...
        or len(member_rows) != SOURCE_ARCHIVE_MEMBER_COUNT
    ...
    if (
        ...
        or audit.get("operational_changes")
        != [
            "paper_i_summary_accepts_latched_phase3_route_identity_v1",
        ]
    ):
        raise PackageContractError(
            "Exact page-8-to-latched-r70 source patch audit drifted."
        )
    ...
    if queue_path.read_text(encoding="utf-8") != expected_queue:
        raise PackageContractError("Queue rows drifted from the six jobs.")
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_singleton_latched_phase3_sixregime_r70_package_validation_v1",
            "status": "passed",
