#!/usr/bin/env python3
"""Preflight or execute one sealed latched-Phase-III RA r70 row."""
...
def _load_closed_job(
    ...
    if (
        ...
        raise PackageContractError("Fresh r70 job contract drifted.")
...
def _load_protocol(
    *, job: Mapping[str, Any], payload: Mapping[str, Any], source_locks: Mapping[str, Any]
    ...
    protocol_lineage = protocol.lineage_authority
    if receipt is None:
    ...
    if (
        ...
        != PARENT_ROUTE_CONTRACT_SHA256
        or lineage.get("parent_route_profile") != PARENT_ROUTE_PROFILE
        or protocol_lineage.get("parent_contract_sha256")
        != PARENT_ROUTE_CONTRACT_SHA256
        or protocol_lineage.get("parent_route_profile")
        != PARENT_ROUTE_PROFILE
        ...
        or route_execution.get("ra_insertion_plateau_history_scope")
        ...
        or invariants.get("phase3_latch_retirement_policy") != "never_close_v1"
        ...
        raise PackageContractError("Typed latched-Phase-III protocol drifted.")
...
def _prepare(job_path: Path) -> tuple[dict[str, Any], dict[str, Any], Any, Any, tempfile.TemporaryDirectory[str]]:
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix=f"paper-i-ra-latched-phase3-r70-{job['execution_id']}."
    )
    try:
...
def preflight(job_path: Path) -> dict[str, Any]:
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_singleton_latched_phase3_sixregime_r70_worker_preflight_v1",
            "status": "passed",
    ...
    *, protocol: Any, problem: Any, staging: Path, maximum_rounds: int
    ...
    controls = RAAdaptOperationalControls(
        ...
        observation=SRObservationPolicy(
            ...
            resource_rounds=(int(maximum_rounds),),
        ),
    )
...
def smoke_one_round(job_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return digested(
            {
                "schema": "paper_i_ra_adapt_singleton_latched_phase3_sixregime_r70_worker_smoke_v1",
    ...
    *, job_path: Path, authorization_path: Path, output_dir: Path, receipt_path: Path
    ...
    try:
        ...
        execution_manifest = digested(
            {
                "schema": "paper_i_ra_adapt_singleton_latched_phase3_sixregime_r70_execution_manifest_v1",
                "status": "passed",
        ...
        receipt = digested(
            {
                "schema": "paper_i_ra_adapt_singleton_latched_phase3_sixregime_r70_worker_receipt_v1",
                "status": "passed",
                "package_id": PACKAGE_ID,
                "campaign_id": CAMPAIGN_ID,
                "execution_id": job["execution_id"],
