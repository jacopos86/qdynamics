#!/usr/bin/env python3
"""Materialize the fresh r70 latched-Phase-III package from page-8 v3."""
...
from package_contract import (  # noqa: E402
    ...
    SOURCE_TO_TARGET_DIFFERENCE_PATHS,
    ROUTE_ID,
    ROUTE_PROFILE,
...
def _load_source_package(repo_root: Path) -> dict[str, Any]:
    ...
    manifest = _load_bound_json(
        ...
        label="sealed page-8 v3 worker-durable predecessor package",
    )
    ...
    if (
        ...
        != SOURCE_IMPLEMENTATION_INVENTORY_SHA256
        or archive_manifest.get("member_count")
        != SOURCE_ARCHIVE_MEMBER_COUNT
    ):
    ...
    protocol_bundle_path, protocol_bundle = _package_bound_json(
        source_package,
        manifest.get("protocol_bundle_manifest"),
        label="predecessor protocol bundle manifest",
    )
    if (
        sha256_file(protocol_bundle_path)
        != SOURCE_PROTOCOL_BUNDLE_MANIFEST_FILE_SHA256
        or protocol_bundle.get("sha256")
        != SOURCE_PROTOCOL_BUNDLE_MANIFEST_CANONICAL_SHA256
    ):
        raise PackageContractError(
            "Predecessor protocol bundle exact authority drifted."
        )
    locks_path, source_locks = _package_bound_json(
        source_package,
        manifest.get("source_locks_snapshot"),
        label="predecessor source-lock snapshot",
    )
    if (
        sha256_file(locks_path) != SOURCE_LOCKS_SNAPSHOT_FILE_SHA256
        or source_locks.get("sha256")
        != SOURCE_LOCKS_SNAPSHOT_CANONICAL_SHA256
    ):
        raise PackageContractError(
            "Predecessor source-lock snapshot exact authority drifted."
        )
    return {
        "root": source_package,
        "manifest": manifest,
...
def _apply_exact_source_patch(source_root: Path) -> list[dict[str, Any]]:
    ...
    for relative, row in expected.items():
        ...
        if row["before_sha256"] == "<absent>":
            if path.exists() or path.is_symlink():
                raise PackageContractError(
                    f"Patch addition predecessor exists: {relative}"
                )
        elif (
            not path.is_file()
            or path.is_symlink()
            or sha256_file(path) != row["before_sha256"]
        ):
    ...
    if completed.returncode != 0:
        raise PackageContractError(
            "Exact page-8-to-latched-r70 source patch failed: "
...
def _target_cell(source_cell: Any, *, horizon: int) -> Any:
    return replace(
        ...
        stage="latched_phase3_separate_plateau_insertion_r70",
...
def _validate_source_to_target(
    *,
    ...
    paths = {path for path, _before, _after in differences}
    if paths != SOURCE_TO_TARGET_DIFFERENCE_PATHS:
        raise PackageContractError(
            "Source-to-target protocol changed unexpected fields: "
            f"{sorted(paths, key=str)}"
        )
    ...
    protocol_lineage = target_payload["lineage_authority"]
    if (
        ...
        != RESOURCE_WEIGHTING_SCOPE
        or target_payload["algorithm_id"] != ALGORITHM_ID
        or target_payload["route_contract"]["sha256"]
        != ROUTE_CONTRACT_SHA256
        or route_contract["route_profile"] != ROUTE_PROFILE
        or lineage["parent_contract_sha256"]
        != PARENT_ROUTE_CONTRACT_SHA256
        or lineage["parent_route_profile"]
        != PARENT_ROUTE_PROFILE
        or protocol_lineage["parent_contract_sha256"]
        != PARENT_ROUTE_CONTRACT_SHA256
        or protocol_lineage["parent_route_profile"]
        != PARENT_ROUTE_PROFILE
        or source_payload["route_contract"]["route_profile"]
        != PARENT_ROUTE_PROFILE
        or source_payload["route_contract"]["sha256"]
        != PARENT_ROUTE_CONTRACT_SHA256
        ...
        or route_execution["ra_phase3_population_activation_policy"]
        != "first_open_progress_plateau_predicate_latched_v1"
        or route_execution["ra_phase3_preplateau_materialization_policy"]
        != "phase2_winner_only_refit_geometry_v1"
        or route_execution["ra_insertion_plateau_history_scope"]
        != "prior_full_phase3_accepted_transition_global_prior_mean_v1"
        or invariants["phase3_competitive_population_activation"]
        != "first_open_progress_plateau_predicate_latched_v1"
        or invariants["phase3_activation_source"]
        != "first_authenticated_progress_plateau_domain_open_latched_v1"
        or invariants["phase3_preplateau_materialization_policy"]
        != "phase2_winner_only_refit_geometry_v1"
        or invariants["phase3_activation_independent_latch"] is not True
        or invariants["phase3_activation_hysteresis_active"] is not False
        or invariants["phase3_latch_retirement_policy"] != "never_close_v1"
        or invariants["insertion_plateau_history_scope"]
        != "prior_full_phase3_accepted_transition_global_prior_mean_v1"
        or invariants["insertion_activation_requires_prior_phase3_latch"]
        is not True
        or invariants["insertion_activation_changes_phase3_latch"] is not False
        or lineage["supersession_reason"]
        != (
            "paper_i_ra_adapt_singleton_latched_phase3_separate_"
            "plateau_insertion_20260804"
        )
        or lineage["only_intended_scientific_changes"]
        != [
        ...
        raise PackageContractError("Target latched-Phase-III protocol drifted.")
    return {
...
def build() -> dict[str, Any]:
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix="paper-i-latched-phase3-r70-source-"
    )
    source_root = (
    ...
    if len(predecessor_members) != SOURCE_ARCHIVE_MEMBER_COUNT:
        raise PackageContractError(
            "Page-8 predecessor archive member count drifted."
        )
    ...
    target_inventory = source_locks.get("implementation_sources")
    if (
        not isinstance(predecessor_inventory, Mapping)
        or predecessor_inventory.get("sha256")
        != SOURCE_IMPLEMENTATION_INVENTORY_SHA256
        or not isinstance(target_inventory, Mapping)
        or target_inventory.get("sha256")
        != TARGET_IMPLEMENTATION_INVENTORY_SHA256
        or implementation_inventory != target_inventory
        or source_locks.get("sha256")
        != TARGET_SOURCE_LOCKS_CANONICAL_SHA256
        or hashlib.sha256(
        ...
        raise PackageContractError(
            "Page-8-derived r70 implementation/source-lock closure drifted."
        )
    ...
    member_paths = set(predecessor_by_path) | implementation_paths
    if (
        implementation_paths - set(predecessor_by_path)
        or member_paths != set(predecessor_by_path)
        or len(member_paths) != SOURCE_ARCHIVE_MEMBER_COUNT
        or patch_paths
        != {
        ...
        raise PackageContractError(
            "Latched-r70 source inventory changed page-8 archive membership."
        )
    members: list[dict[str, Any]] = []
    for relative in sorted(member_paths):
        ...
        if (
            relative in predecessor_by_path
            and relative not in patch_paths
    ...
    for regime_id, nph in REGIME_ROWS:
        ...
        if source_id not in source_rows:
            raise PackageContractError(
                f"Missing immutable page-8 source cell: {source_id}"
            )
        source_cell = _cell_from_manifest_row(source_rows[source_id])
        if (
            source_cell.route_id != ROUTE_ID
            or source_cell.algorithm_id != SOURCE_ALGORITHM_ID
            ...
            raise PackageContractError(
                f"Immutable page-8 source cell drifted: {source_id}"
            )
    ...
    for target_cell in target_cells:
        ...
        if not isinstance(source_payload, dict):
            raise PackageContractError(
                "Immutable page-8 protocol is not a mapping."
            )
        ...
        if source_protocol.sha256 != source_payload.get("sha256"):
            raise PackageContractError(
                "Immutable page-8 protocol digest drifted."
            )
    ...
    source_lock_audit = digested(
        {
            ...
            "target_horizon": TARGET_HORIZON,
            "scientific_changes": [
                "phase3_competitive_population_first_open_progress_plateau_latched_v1",
                "commutation_reduced_insertion_requires_prior_full_phase3_plateau_transition_v1",
                "fresh_horizon_50_to_70_v1",
            ],
            "operational_changes": [
                "paper_i_summary_accepts_latched_phase3_route_identity_v1",
            ],
            "unchanged_scientific_settings": [
                "plateau_prior_mean_ratio_threshold_1e-4",
                "fresh_start",
                "full_meta_guarded_singleton_pool",
                "powell_200_seed_7",
                "stationary_source_gradients",
                "late_resource_weighting",
            ],
            "immutable_predecessor_authority": {
                "path": SOURCE_PACKAGE_RELATIVE.as_posix(),
                "package_manifest_sha256": (
                    source_package["manifest"]["sha256"]
                ),
                "source_archive_sha256": SOURCE_ARCHIVE_FILE_SHA256,
                "source_locks_snapshot_sha256": (
                    source_package["source_locks"]["sha256"]
                ),
                "implementation_source_inventory_sha256": (
                    SOURCE_IMPLEMENTATION_INVENTORY_SHA256
                ),
    ...
    execution_plan = digested(
        {
            "schema": (
                ...
                "sixregime_r70_execution_plan_v1"
            ),
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "run_class": "candidate",
            ...
            "checkpoint_dynamic_sidecars_hash_bound": True,
            "same_filesystem_atomic_success_publication": True,
            "post_validation_failure_evidence_preserved": True,
            "execution_authorized": False,
            "submission_authorized": False,
