...
def validate_package(
    *,
    ...
    overlay_rows = manifest.get("source_overlay_files")
    if (
        not isinstance(overlay_rows, list)
        or [row.get("path") for row in overlay_rows if isinstance(row, Mapping)]
        != [f"source_overlay/{relative}" for relative in SOURCE_OVERLAY_FILES]
    ):
        raise PackageContractError("Source-overlay binding closure drifted.")
    for row in overlay_rows:
        _binding(row, label="source overlay")
    ...
    return digested(
        {
            "schema": "phase3_qiskit_denominator_no_lanes_package_validation_v1",
