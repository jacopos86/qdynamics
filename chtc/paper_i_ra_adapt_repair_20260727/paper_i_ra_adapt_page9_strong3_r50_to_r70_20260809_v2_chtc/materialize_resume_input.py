...
def _lexical_absolute(path: Path) -> Path:
    """Normalize ``.``/``..`` without resolving symlinked mount ancestors."""

    return Path(os.path.abspath(os.fspath(path)))


def _validated_source_archive(
    source_archive: Path, remote: Mapping[str, Any]
) -> Path:
    candidate = _lexical_absolute(source_archive)
    declared = _lexical_absolute(Path(str(remote.get("path", ""))))
    if (
        candidate.as_posix() != declared.as_posix()
        or not candidate.is_file()
        or candidate.is_symlink()
        or candidate.stat().st_size != int(remote.get("size_bytes", -1))
        or sha256_file(candidate) != remote.get("sha256")
    ):
...
def materialize(
    *,
    ...
    source_archive = _validated_source_archive(source_archive, remote)
