#!/usr/bin/env python3
"""Seal the six-cell pure-Hubbard Page-12 full-noise package."""
...
def _application_sources() -> dict[str, dict[str, Any]]:
    ...
    if len(sources) != 6 or len({row["sha256"] for row in sources.values()}) != 6:
        raise PackageContractError("The six application sources are not unique.")
    return sources
...
def build() -> dict[str, Any]:
    ...
    for cell in cells:
        ...
        jobs.append(job)
    if set(route_digests_by_level) != {"low", "high", "extreme"} or any(
    ...
    audit = digested(
        {
            ...
            "status": "passed",
            "study_shape": "u2_by_full_noise3_v1",
            "fixed_fields": [
