#!/usr/bin/env python3
"""Run the six weak-Holstein Page-12 RA insertion routes locally.

The sealed twelve-cell package remains immutable.  This adapter binds exactly
the six already-closed CHTC rows to a new local-only activation, overlays only the
...
DEFAULT_PLANNING_DIR = REPAIR_ROOT / (
    "paper_i_page12_weak_holstein_ra6_priority_20260815_v1_planning"
)
DEFAULT_ACTIVATION_DIR = REPAIR_ROOT / (
    "paper_i_page12_weak_holstein_ra6_priority_20260815_v1_activation"
)
...
DEFAULT_REMOTE_HOLD_RECEIPT = REPAIR_ROOT / (
    "paper_i_page12_weak_holstein_ra6_prior_remote_closure_20260815.json"
)
PRIOR_REMOTE_CLOSURE_DIR = REPAIR_ROOT / (
...
CLOSURE_RECEIPT_MAX_AGE_SECONDS = 30 * 60
LOCAL_CHILD_TOKEN_ENV = "PAPER_I_PAGE12_WEAK_RA6_LOCAL_CHILD_TOKEN"
PARITY_CHILD_TOKEN_ENV = "PAPER_I_PAGE12_WEAK_RA6_CANARY_CHILD_TOKEN"
LOCAL_EXECUTION_TARGET = "local_mac_guarded_serial_weak_holstein_ra6_v1"
...
PARITY_JOB_SPEC_SHA256 = (
    "178eaacbac01c98a889f7f5c3f608a470bc516fb2128508409fc907aec01e123"
)
PARITY_PROTOCOL_SHA256 = (
    "03d1f6bd03d8bbada9452889e83a6e9cfee74fb2cd5ef8897cc40e27fd8d0d74"
)
PARITY_ROUTE_SHA256 = (
    "5344f39726b283fdba78d1c49616a530cc881557ef12d4245bdd9691fb753e45"
)
# Historical CHTC/local numerical equality is explicitly not required.  The
# canary compares the sealed and serializer-overlay branches within this one
# local runtime and deliberately does not pin a historical energy scalar.
PARITY_BASELINE_ENERGY: float | None = None
...
PLAN_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_execution_plan_v1"
PLANNING_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_planning_manifest_v1"
AUTHORIZATION_SCHEMA = (
    "paper_i_page12_weak_holstein_ra6_local_execution_authorization_v1"
)
ACTIVATION_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_activation_v1"
PREFLIGHT_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_preflight_v1"
RUNTIME_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_runtime_v1"
STATUS_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_status_v1"
GUARD_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_guard_receipt_v1"
TERMINAL_SCHEMA = "paper_i_page12_weak_holstein_ra6_local_terminal_receipt_v1"
SOURCE_OVERLAY_SCHEMA = (
    "paper_i_page12_weak_holstein_ra6_local_source_overlay_receipt_v1"
)
REMOTE_HOLD_SCHEMA = "paper_i_page12_weak_holstein_ra6_prior_remote_closure_v1"
PARITY_SPEC_SCHEMA = "paper_i_page12_weak_holstein_ra6_canary_spec_v1"
PARITY_BRANCH_SCHEMA = "paper_i_page12_weak_holstein_ra6_canary_branch_v1"
PARITY_SCHEMA = "paper_i_page12_weak_holstein_ra6_scientific_canary_v1"

OVERLAY_RELATIVE_PATHS = (
    Path("pipelines/static_adapt/current_checkpoint.py"),
)

CLOSURE_REASON = "prior authenticated CHTC closure; local rerun requested by user"


class LocalStrongSectorError(RuntimeError):
    """Raised when the weak-Holstein priority contract fails closed."""
...
class TargetCell:
    ...
    def execution_id(self) -> str:
        return (
            ...
            f"{self.regime_id}__nph3__ra_global_singleton_gradient_phase0_"
            f"phase123_qiskit_phase23_{self.policy}"
        )


# User-priority order: RA append-only across the three Hubbard regimes, then
# RA always-open. Proc numbers preserve the original cluster 9647385 identity.
TARGET_CELLS = (
    TargetCell(
        6,
        "weak_weak",
...
def _closed_inputs(
    ...
    if tuple(row["execution_id"] for row in selected) != TARGET_EXECUTION_IDS:
        raise LocalStrongSectorError("Weak-Holstein six-cell run order drifted.")
...
def _validate_prior_remote_closure(
    path: Path,
    *,
    require_closed_rows: bool,
) -> dict[str, Any]:
    rows: list[dict[str, Any]] = []
    created_values: list[str] = []
    for cell in TARGET_CELLS:
        closure_path = PRIOR_REMOTE_CLOSURE_DIR / (
            "paper_i_ra_adapt_page12_cluster9647385_"
            f"proc{cell.proc:02d}_{cell.execution_id}_closure_receipt_20260813.json"
        )
        closure = _load_digested(
            closure_path, label=f"authenticated prior closure proc {cell.proc}"
        )
        if (
            closure.get("schema")
            ...
            or int(closure.get("cluster_id", -1)) != 9647385
            or int(closure.get("proc_id", -1)) != cell.proc
            or closure.get("run_id") != cell.execution_id
            or closure.get("regime_id") != cell.regime_id
            or int(closure.get("nph", -1)) != 3
            or closure.get("comparator_policy") != cell.policy
            or int(closure.get("controller_rounds_completed", -1)) != 50
            or closure.get("authentication_checks", {}).get(
                "remote_local_identity_passed"
            )
            is not True
        ):
            raise LocalStrongSectorError(
                f"Authenticated prior closure drifted at proc {cell.proc}."
            )
        created_values.append(str(closure["created_at_utc"]))
        rows.append(
            {
                "proc": cell.proc,
                ...
                "closure_receipt": {
                    ...
                    "file_sha256": _sha256_file(closure_path),
                    "canonical_sha256": closure["sha256"],
    ...
    expected = _digested(
        {
            "schema": REMOTE_HOLD_SCHEMA,
            "status": "passed_authenticated_exact_prior_closures",
            "observed_at_utc": max(created_values),
            "scheduler": "chtc_condor",
            "cluster_id": 9647385,
            "authentication_kind": "persisted_authenticated_closure_receipts_v1",
            "authenticated_remote_query": False,
            "rows": rows,
            "closed_procs": sorted(TARGET_PROCS),
            "closed_execution_ids": sorted(TARGET_EXECUTION_IDS),
            "remote_active_execution_ids": [],
            "late_materialization_factory_active": False,
            "remote_rows_sha256": _canonical_sha256({"rows": rows}),
            "rerun_reason": CLOSURE_REASON,
            "scientific_execution_performed": False,
        }
    )
    if path.exists() or path.is_symlink():
        supplied = _load_digested(path, label="authenticated prior closure receipt")
        if supplied != expected:
            raise LocalStrongSectorError(
                "Authenticated prior closure aggregate drifted."
            )
    if require_closed_rows and len(rows) != len(TARGET_CELLS):
        raise LocalStrongSectorError("Prior closure row inventory is incomplete.")
    return expected
...
def _overlay_protocol_preflight(execution_id: str) -> dict[str, Any]:
    ...
    if execution_id not in TARGET_BY_ID:
        raise LocalStrongSectorError("Overlay preflight is outside the six-cell scope.")
    worker = _load_worker()
...
def _capacity(runtime_dir: Path) -> dict[str, Any]:
    ...
    return _digested(
        {
            "schema": "paper_i_page12_weak_holstein_ra6_local_capacity_v1",
...
def _local_numerical_runtime_policy() -> dict[str, Any]:
    return {
        ...
        "same_runtime_required_across_all_six_cells": True,
        ...
        "historical_local_chtc_bitwise_parity_required": False,
    }


def _parity_canary_spec() -> dict[str, Any]:
    if TARGET_CELLS[0].proc != 6:
        raise LocalStrongSectorError("Canary is not pinned to proc 6.")
...
def prepare_planning(
    *,
    planning_dir: Path,
    runtime_dir: Path,
    prior_remote_closure: Path = DEFAULT_REMOTE_HOLD_RECEIPT,
) -> dict[str, Any]:
    worker = _load_worker()
    manifest, rows = _closed_inputs(worker)
    prior_closure = _validate_prior_remote_closure(
        prior_remote_closure, require_closed_rows=True
    )
    ...
    try:
        ...
        copied_hold_path = temporary / "prior_remote_closure.json"
        _write_json_exclusive(copied_hold_path, prior_closure)
        ...
        plan = _digested(
            {
                ...
                "scope": "six_weak_holstein_ra_insertion_cells_priority_rerun",
                ...
                "remote_overlap_control": {
                    "prior_remote_closure": hold_binding,
                    "prior_remote_closure_sha256": prior_closure["sha256"],
                    "scheduler": prior_closure["scheduler"],
                    "cluster_id": prior_closure["cluster_id"],
                    "closed_procs": prior_closure["closed_procs"],
                    "observed_at_utc": prior_closure["observed_at_utc"],
                    "authenticated_remote_query": False,
...
def _validate_planning(
    ...
    *,
    ...
    require_closed_rows: bool = False,
    ...
    prior_closure = _verify_binding(
        planning_dir,
        planning.get("prior_remote_closure"),
        expected_path="prior_remote_closure.json",
        label="authenticated prior closure receipt",
        canonical=True,
    )
    assert (
        plan is not None
    ...
    prior_closure = _validate_prior_remote_closure(
        planning_dir / "prior_remote_closure.json",
        require_closed_rows=require_closed_rows,
    )
    ...
    if (
        ...
        or plan.get("remote_overlap_control", {}).get(
            "prior_remote_closure"
        )
        != planning.get("prior_remote_closure")
        or plan.get("remote_overlap_control", {}).get(
            "prior_remote_closure_sha256"
        )
        != prior_closure.get("sha256")
...
def _run_parity_branch(
    *, variant: str, activation_dir: Path
    ...
    # Keep the inherited deterministic temp prefix because the audited
    # scientific projection normalizes that path-only failure witness.
...
def _materialize_scientific_parity_canary(
    *,
    ...
    if not isinstance(energy, (int, float)):
        raise LocalStrongSectorError("Local canary energy witness is absent.")
    parity = _digested(
        {
            "schema": PARITY_SCHEMA,
            "status": "passed_exact_scientific_parity",
...
def authorize_activation(
    *,
    ...
    planning, plan, _preflight, prior_closure = _validate_planning(
        planning_dir, manifest=manifest, require_closed_rows=True
    )
        ...
        authorization = _digested(
            {
                ...
                "user_scope_confirmation": (
                    "six Paper-I weak-Holstein RA insertion-policy reruns"
                ),
        ...
        activation = _digested(
            {
                ...
                "prior_remote_closure": planning["prior_remote_closure"],
                "execution_ids": list(TARGET_EXECUTION_IDS),
                "execution_target": LOCAL_EXECUTION_TARGET,
                "execution_authorized": True,
                "submission_authorized": False,
...
def _validate_activation(
    ...
    *,
    ...
    planning, plan, preflight, prior_closure = _validate_planning(
        activation_dir,
        manifest=manifest,
        require_closed_rows=require_closed_rows,
        reject_authority_files=False,
    ...
    if (
        ...
        or activation.get("prior_remote_closure")
        != planning.get("prior_remote_closure")
        ...
        or authorization.get("prior_remote_closure_sha256")
        != prior_closure.get("sha256")
    ...
    return activation, plan, authorization, prior_closure
...
def _run_local_cell(
    *,
    ...
    if execution_id not in TARGET_BY_ID:
        raise LocalStrongSectorError("Internal child is outside the six-cell scope.")
    worker = _load_worker()
    manifest, rows = _closed_inputs(worker)
    activation, plan, authorization, _prior_closure = _validate_activation(
    ...
    return _digested(
        {
            ...
            "status": "passed_all_six_cells_immutable_closure",
    ...
    *, planning_dir: Path, activation_dir: Path, runtime_dir: Path
    ...
    if planning_dir.exists() or planning_dir.is_symlink():
        external_planning, _plan, _host, _hold = _validate_planning(
            planning_dir, manifest=manifest, require_closed_rows=False
        )
        planning_status = "validated_not_authorized"
    activation_status = "absent"
    if activation_dir.exists() or activation_dir.is_symlink():
        _validate_activation(
            activation_dir, manifest=manifest, require_closed_rows=False
        )
    ...
    if runtime_dir.exists() or runtime_dir.is_symlink():
        ...
        activation, _plan, _authority, _prior_closure = _validate_activation(
            activation_dir, manifest=manifest, require_closed_rows=False
        )
    ...
    *, activation_dir: Path, runtime_dir: Path
    ...
    with lock_path.open("a+", encoding="utf-8") as lock_stream:
        try:
            ...
            raise LocalStrongSectorError("Another weak-sector supervisor is active.") from exc
        ...
        fresh_activation = _validate_activation(
            activation_dir, manifest=manifest, require_closed_rows=True
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Guarded local Paper-I Page-12 weak-Holstein RA six-cell runner"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prior-remote-closure",
        dest="prior_remote_closure",
        type=Path,
    ...
    try:
        if args.prepare:
            ...
            payload = prepare_planning(
                ...
                runtime_dir=runtime_dir,
                prior_remote_closure=args.prior_remote_closure.resolve(),
            )
        elif args.authorize:
