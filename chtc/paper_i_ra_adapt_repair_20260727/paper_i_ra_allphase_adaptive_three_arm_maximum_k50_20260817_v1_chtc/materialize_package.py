...
def _source_paths() -> list[Path]:
    rows: list[Path] = []
    for root_name in ("pipelines", "src"):
...
def main() -> int:
    ...
    source_manifest = digested(
        {
            "schema": "paper_i_ra_allphase_adaptive_three_arm_source_archive_v1",
    ...
    if len(jobs) != 18 or len({row["execution_id"] for row in jobs}) != 18:
        raise RuntimeError("matrix is not exactly eighteen unique cells")
    ...
    plan = digested(
        {
            ...
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "matrix_shape": {"arms": 3, "regimes": 6, "cells": 18},
            "arms": list(ARMS),
    ...
    manifest = digested(
        {
            ...
            "status": "authorized_eighteen_cells",
            "created_at": created_at,
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "row_count": 18,
