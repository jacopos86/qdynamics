#!/usr/bin/env python3
"""Preflight or execute one Page-16 macro Qiskit beam/prune cell."""
...
def _load_closed_job(
    ...
    if (
        ...
        or job.get("phase_i_compile_cost_source") != "structural_proxy_v1"
        ...
        raise PackageContractError("Sealed Page-16 macro Qiskit job drifted.")
...
def _validate_authorization(
    ...
    *,
    ...
    runtime_probe = probe.get("probe") if isinstance(probe, Mapping) else None
    if (
        ...
        or not isinstance(runtime_probe, Mapping)
        or runtime_probe.get("sealed_source_imported") is not True
        or runtime_probe.get("algorithm_id") != ALGORITHM_ID
        or runtime_probe.get("candidate_adapter_id") != CANDIDATE_ADAPTER_ID
        or runtime_probe.get("structural_proxy_mode")
        != STRUCTURAL_PROXY_MODE
        or runtime_probe.get("selector_qiskit_compile_cost_active") is not True
        or runtime_probe.get("selector_compile_cost_scope")
        != BACKEND_COMPILE_SCOPE
        or runtime_probe.get("resolved_backend_name") != "FakeMarrakesh"
        or runtime_probe.get("backend_resolution_kind") != "fake_exact"
        or runtime_probe.get("optimization_level") != 1
        or runtime_probe.get("seed_transpiler") != 7
        or runtime_probe.get("structure_theta_value") != 1.0
        or runtime_probe.get("allow_preferred_fallback") is not False
        or runtime_probe.get("source_activation_policy")
        != SOURCE_ACTIVATION_POLICY
        or runtime_probe.get("source_cwd_isolated") is not True
        or runtime_probe.get("sealed_module_paths_verified") is not True
        or int(runtime_probe.get("loaded_source_module_count", 0)) < 4
        or not isinstance(runtime_probe.get("sealed_module_paths"), Mapping)
    ):
        raise PackageContractError(
...
def _load_protocol(
    *,
    ...
    if (
        ...
        or protocol.request.method.pruning.kind != "metric"
        or protocol.request.method.beam.kind != "fork_local"
        or protocol.request.method.beam.live_parent_branches
        != BEAM_LIVE_BRANCHES
        or protocol.request.method.beam.admission_children_per_parent
        != BEAM_CHILDREN_PER_PARENT
        or protocol.request.method.beam.maximum_admission_children_per_round
        != BEAM_MAXIMUM_CHILDREN_PER_ROUND
        or protocol.request.method.beam.s_alg_weight != BEAM_S_ALG_WEIGHT
        ...
        or execution.get("phase3_backend_cost_mode") != STRUCTURAL_PROXY_MODE
        ...
        or invariants.get("selector_qiskit_compile_cost_active") is not True
        ...
        != EXPECTED_CANDIDATE_FUNNEL
        or "post_exposure_singleton_phase_i_policy" in invariants
        or not isinstance(lineage, Mapping)
        or lineage.get("parent_route_profile") != TARGET_PARENT_ROUTE_PROFILE
        or lineage.get("parent_contract_sha256")
        != TARGET_PARENT_ROUTE_CONTRACT_SHA256
    ):
        raise PackageContractError(
            "Typed Page-16 macro Qiskit beam/prune protocol drifted."
        )
...
def _prepare(
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix=f"paper-i-page16-macro-qiskit-{job['execution_id']}."
    )
    try:
