#!/usr/bin/env python3
"""Seal six L=2 Page-16 macro Qiskit beam/prune cells through round 20."""
...
def _source_locks(cells: tuple[BundleCellSpec, ...]) -> dict[str, Any]:
    ...
    payload["derivation_overlay"] = {
        "schema": "page16_macro_phase23_qiskit_beam_metric_source_overlay_v1",
...
def build() -> dict[str, Any]:
    ...
    if (
        ...
        or verify_self_digest(
            baseline_manifest,
            label="Page-16 macro Qiskit baseline package manifest",
        )
        != BASELINE_PACKAGE_MANIFEST_CANONICAL_SHA256
    ):
    ...
    bundle_manifest = digested(
        {
            "schema": "page16_macro_phase23_qiskit_beam_metric_bundle_manifest_v1",
            "bundle_id": BUNDLE_ID,
    ...
    if len(route_digests) != 1:
        raise PackageContractError("Weak-Holstein route digest is not common.")

    validation = digested(
        {
            "schema": "page16_macro_phase23_qiskit_beam_metric_validation_v1",
            "status": "passed",
            "bundle_id": BUNDLE_ID,
            "protocol_count": 6,
    ...
    audit = digested(
        {
            "schema": "page16_macro_phase23_qiskit_beam_metric_source_audit_v1",
            "status": "passed",
            "source_route_profile": SOURCE_ROUTE_PROFILE,
            "source_route_contract_sha256": SOURCE_ROUTE_CONTRACT_SHA256,
            "page16_parent_route_profile": BASELINE_ROUTE_PROFILE,
            "page16_parent_route_contract_sha256": (
                BASELINE_ROUTE_CONTRACT_SHA256
            ),
            ...
            "physical_operator_lanes_active": False,
            "structural_proxy_mode": STRUCTURAL_PROXY_MODE,
            "approved_settings_changes_from_page16": {
                "pruning": {"from": "off", "to": PRUNING_POLICY},
                "beam": {
                    "from": "off",
                    "to": {
                        "kind": "fork_local",
                ...
                "target_horizon": {
                    "from": {"weak_holstein": 50, "strong_holstein": 30},
                    "to": HORIZON,
                },
                "scheduler_resources": {
                    "nph3": {
