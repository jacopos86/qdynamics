...
for path in "$job_path" "$protocol_path" "$authorization_path" "$source_archive" "$image_path" "${package_dir}/worker.py" "continuation_fppssu8/continuation_worker.py" "continuation_fppssu8/current.json.gz"; do
...
gunzip -c continuation_fppssu8/current.json.gz > "${attempt}/resume/current.json"
for ledger_gz in continuation_fppssu8/current.estimator_call_ledger_checkpoint.*.json.gz; do
...
"$runtime_bin" exec --cleanenv \
  ...
  --pwd "${sandbox}" \
  "$image_path" \
  python3 "${sandbox}/continuation_fppssu8/continuation_worker.py" \
