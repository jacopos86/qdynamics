#!/usr/bin/env bash
# Continuation variant of the sealed package's execute_job.sh: identical
# verification and container invocation, but stages the retrieved resume
# checkpoint and launches continuation_worker.py wrapping the sealed worker.
set -euo pipefail

if [[ "$#" -ne 10 ]]; then
  echo "usage: execute_continuation.sh PACKAGE JOB PROTOCOL AUTH SOURCE SOURCE_SHA IMAGE IMAGE_SHA EXECUTION_ID OUTPUT" >&2
  exit 64
fi
...
for path in "$job_path" "$protocol_path" "$authorization_path" "$source_archive" "$image_path" "${package_dir}/worker.py" "continuation_flpassu8/continuation_worker.py" "continuation_flpassu8/current.json.gz"; do
...
package_attempt() {
  ...
  trap - EXIT
  set +e
  # The staged resume inputs are inputs, not results; drop them from the
  # output archive so it matches the sealed layout.
  rm -rf "${attempt}/resume"
...
mkdir "${attempt}/source" "${attempt}/resume"
tar -xzf "$source_archive" -C "${attempt}/source"
gunzip -c continuation_flpassu8/current.json.gz > "${attempt}/resume/current.json"
for ledger_gz in continuation_flpassu8/current.estimator_call_ledger_checkpoint.*.json.gz; do
  [[ -f "$ledger_gz" ]] || continue
  base="$(basename "$ledger_gz" .gz)"
  gunzip -c "$ledger_gz" > "${attempt}/resume/${base}"
done
sandbox="$(pwd -P)"
"$runtime_bin" exec --cleanenv \
  ...
  --pwd "${sandbox}" \
  "$image_path" \
  python3 "${sandbox}/continuation_flpassu8/continuation_worker.py" \
    --package-worker "${sandbox}/${package_dir}/worker.py" \
    --resume-checkpoint "${sandbox}/${attempt}/resume/current.json" \
