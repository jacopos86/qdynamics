...
def _write_resume_archive(
    *, repo_root: Path, destination: Path
    ...
    route_profile = load_json(repo_root / SOURCE_PROTOCOL_INPUT)["route_contract"][
        "route_profile"
    ]
    if (
...
def main() -> int:
    ...
    bundled_protocol_path = PACKAGE_DIR / BUNDLED_SOURCE_PROTOCOL
    _exclusive_copy(
        repo_root / SOURCE_PROTOCOL_INPUT, bundled_protocol_path
    )
    bundled_protocol_binding = _package_binding(
        bundled_protocol_path, canonical=True
    )
    if (
        bundled_protocol_binding["sha256"]
        != SOURCE_PROTOCOL_FILE_SHA256
        or bundled_protocol_binding["canonical_sha256"]
        != SOURCE_PROTOCOL_CANONICAL_SHA256
    ):
        raise PackageContractError("Bundled source protocol drifted.")
    ...
    audit = digested(
        {
            ...
            "status": "passed",
            "source_protocol": bundled_protocol_binding,
            "source_protocol_portability": (
                "package_relative_no_external_lookup_v1"
            ),
