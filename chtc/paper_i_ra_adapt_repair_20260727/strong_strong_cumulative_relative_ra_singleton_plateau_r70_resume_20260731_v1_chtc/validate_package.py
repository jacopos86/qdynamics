...
def validate(*, deep_resume: bool) -> dict[str, Any]:
    ...
    source_payload = documents["source_protocol"]
    ...
    if (
        ...
        or manifest.get("source_protocol", {}).get("path")
        != BUNDLED_SOURCE_PROTOCOL.as_posix()
        or manifest.get("source_protocol", {}).get("sha256")
        != SOURCE_PROTOCOL_FILE_SHA256
        or manifest.get("source_protocol", {}).get("canonical_sha256")
        != SOURCE_PROTOCOL_CANONICAL_SHA256
        or job.get("source_protocol") != manifest.get("source_protocol")
        or audit.get("source_protocol") != manifest.get("source_protocol")
        or audit.get("source_protocol_portability")
        != "package_relative_no_external_lookup_v1"
        or plan.get("source_protocol_sha256")
        != SOURCE_PROTOCOL_CANONICAL_SHA256
