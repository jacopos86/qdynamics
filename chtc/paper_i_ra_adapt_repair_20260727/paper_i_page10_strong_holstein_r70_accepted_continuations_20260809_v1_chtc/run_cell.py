#!/usr/bin/env python3
"""Execute one authenticated Page-10 accepted-state continuation to round 70."""
...
def _load_job(
    ...
    return job, manifest, protocol_payload, source_locks, composition, resume_manifest
...
def _extract_source(
    *,
    ...
    if (
        ...
        raise PackageContractError("Accepted-state loader overlay drifted.")
    ...
    if sha256_file(controller) != CONTROLLER_AFTER_SHA256:
        raise PackageContractError("Accepted-state loader overlay failed.")
...
def _activate_source_root(source_root: Path) -> None:
    ...
    module = importlib.import_module("pipelines.static_adapt.sr_snake._controller")
    try:
        module_path = Path(str(module.__file__)).resolve()
        module_path.relative_to(root)
    except ValueError as exc:
        raise PackageContractError(
            "Runtime controller escaped the source archive."
        ) from exc
    if sha256_file(module_path) != CONTROLLER_AFTER_SHA256:
        raise PackageContractError("Runtime controller overlay is inactive.")
...
def _load_protocol(
    *,
    ...
    if (
        ...
        raise PackageContractError("Typed Page-10 continuation protocol drifted.")
...
def _prepare(
    ...
    *,
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix=f"paper-i-page10-r70-{job['regime_id']}."
    )
    try:
        ...
        temporary.cleanup()
        raise
    return job, manifest, protocol, problem, resume_manifest, temporary
...
def preflight(job_path: Path, *, verify_resume_bytes: bool) -> dict[str, Any]:
    ...
    return digested(
        {
            "schema": "paper_i_page10_strong_r70_worker_preflight_v1",
            "status": "passed",
...
def run_cell(
    *,
    ...
    job, manifest, protocol, problem, resume_manifest, temporary = _prepare(
    ...
    try:
        ...
        checkpoint = _extract_resume(
            job=job, manifest=resume_manifest, destination=resume_root
        )
        ...
        if result.run.paper_i_summary is None:
            raise PackageContractError("Page-10 continuation summary is absent.")
        _write_json(
        ...
        execution_manifest = digested(
            {
                "schema": "paper_i_page10_strong_r70_execution_manifest_v1",
                "status": "passed",
                "package_id": PACKAGE_ID,
        ...
        receipt = digested(
            {
                "schema": "paper_i_page10_strong_r70_worker_receipt_v1",
                "status": "passed",
                "package_id": PACKAGE_ID,
                "campaign_id": CAMPAIGN_ID,
                "execution_id": job["execution_id"],
