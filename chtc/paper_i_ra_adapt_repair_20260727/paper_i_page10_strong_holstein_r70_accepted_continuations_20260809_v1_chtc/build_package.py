...
def _materialize_resume(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if spec["source_kind"] == "local_recoverable_checkpoint_triplet":
        _build_local_resume_archive(spec, archive_path)
    elif spec["source_kind"] == "validated_chtc_attempt_archive":
        _build_attempt_resume_archive(spec, archive_path)
    else:
        raise PackageContractError(f"Unknown resume source: {spec['source_kind']}")
    members = [
        {
            "role": row["role"],
    ...
    manifest = digested(
        {
            ...
            "archive": file_binding(archive_path, root=PACKAGE_DIR),
        }
    )
    ...
    if int(validated["metadata"].get("S_alg", spec["source_s_alg"])) < 0:
...
transfer_input_files = {package_relative}/package_contract.py, {package_relative}/run_cell.py, {package_relative}/package_manifest.json, {package_relative}/source/source_locked.tar.gz, {package_relative}/source/source_archive_manifest.json, {package_relative}/source/source_composition.json, {package_relative}/source_overlay/{CONTROLLER_RELATIVE_PATH}, {package_relative}/bundle/bundle_manifest.json, {package_relative}/bundle/source_locks.json, {package_relative}/$(job_path), {package_relative}/$(protocol_path), {package_relative}/$(resume_archive), {package_relative}/$(resume_manifest), {package_relative}/activation/activation_request.json, {package_relative}/$(authorization_path), {REMOTE_IMAGE_PATH}
...
+JobBatchName = "paper-i-page10-strong-r70-cont-v1"
...
queue execution_id, job_path, protocol_path, authorization_path, resume_archive, resume_manifest, resume_archive_sha256, request_cpus, memory_mb, disk_mb, max_runtime_seconds from {package_relative}/queue.tsv
"""
