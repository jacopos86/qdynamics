...
def validate(*, full_resume: bool, worker_preflight: bool) -> dict[str, Any]:
    ...
    if (
        len(queue_rows) != 3
        or any(len(row) != 11 for row in queue_rows)
        or [row[0] for row in queue_rows] != expected_ids
        or any(row[7:] != ["4", "32768", "61440", "259200"] for row in queue_rows)
    ):
    ...
    if (
        ...
        not in descriptor
        or "queue execution_id, job_path, protocol_path, authorization_path, resume_archive, resume_manifest, resume_archive_sha256, request_cpus, memory_mb, disk_mb, max_runtime_seconds"
        not in descriptor
