#!/usr/bin/env python3
"""Preflight or execute one sealed fresh historical-average RA r100 row."""
...
def _load_closed_job(
    ...
    if (
        ...
        raise PackageContractError("Fresh r100 job contract drifted.")
...
def _load_protocol(
    *, job: Mapping[str, Any], payload: Mapping[str, Any], source_locks: Mapping[str, Any]
    ...
    if (
        ...
        raise PackageContractError("Typed historical-average protocol drifted.")
...
def _prepare(job_path: Path) -> tuple[dict[str, Any], dict[str, Any], Any, Any, tempfile.TemporaryDirectory[str]]:
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix=f"paper-i-ra-r100-{job['execution_id']}."
    )
    try:
...
def preflight(job_path: Path) -> dict[str, Any]:
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r100_worker_preflight_v1",
            "status": "passed",
    ...
    *, protocol: Any, problem: Any, staging: Path, maximum_rounds: int
    ...
    controls = RAAdaptOperationalControls(
        ...
        observation=SRObservationPolicy(
            ...
            resource_rounds=((1,) if maximum_rounds == 1 else (50, 70)),
        ),
    )
...
def smoke_one_round(job_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return digested(
            {
                "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r100_worker_smoke_v1",
...
def _publish_staging_directory(staging: Path, output_dir: Path) -> None:
    """Publish complete worker artifacts even when scratch filesystems differ."""
    ...
    *, job_path: Path, authorization_path: Path, output_dir: Path, receipt_path: Path
    ...
    try:
        ...
        execution_manifest = digested(
            {
                "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r100_execution_manifest_v1",
                "status": "passed",
        ...
        receipt = digested(
            {
                "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r100_worker_receipt_v1",
                "status": "passed",
                "package_id": PACKAGE_ID,
                "campaign_id": CAMPAIGN_ID,
                "execution_id": job["execution_id"],
