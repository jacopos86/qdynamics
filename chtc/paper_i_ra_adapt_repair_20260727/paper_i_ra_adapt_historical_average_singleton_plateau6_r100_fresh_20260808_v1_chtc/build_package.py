#!/usr/bin/env python3
"""Materialize the inert six-cell historical-average RA r100 package."""
...
def _target_cell(source_cell: Any, *, horizon: int) -> Any:
    return replace(
        ...
        stage="historical_average_r100_fresh",
        horizon=int(horizon),
    )
...
def _validate_source_to_target(
    *,
    ...
    if horizon_paths != HORIZON_DIFFERENCE_PATHS:
        raise PackageContractError(
            "Typed r100 derivation changed non-horizon fields: "
    ...
    source_lock_audit = digested(
        {
            ...
            "unchanged_scientific_settings": [
                ...
                "fresh_maximum_controller_rounds_100",
            ],
            "source_patch": source_patch,
            "preserved_settings": {
                "active_gradient_policy": ACTIVE_GRADIENT_POLICY,
