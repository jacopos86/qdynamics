#!/usr/bin/env python3
"""Materialize the worker-durable v2 package from immutable v1 science."""
...
def _load_source_package(repo_root: Path) -> dict[str, Any]:
    ...
    manifest = _load_bound_json(
        ...
        label="sealed v1 worker-predecessor package",
    )
...
def _apply_exact_source_patch(source_root: Path) -> list[dict[str, Any]]:
    ...
    if completed.returncode != 0:
        raise PackageContractError(
            "Exact v1-to-v2 source patch failed: "
...
def build() -> dict[str, Any]:
    ...
    temporary = tempfile.TemporaryDirectory(
        prefix="paper-i-phase3-on-plateau-v2-source-"
    )
    source_root = (
    ...
    if (
        ...
        raise PackageContractError(
            "v1-derived v2 implementation/source-lock closure drifted."
        )
    ...
    if (
        ...
        or patch_paths != {"pipelines/static_adapt/ra_adapt/engine.py"}
    ):
        raise PackageContractError(
            "v2 source inventory changed v1 archive membership."
        )
    members: list[dict[str, Any]] = []
    ...
    for regime_id, nph in REGIME_ROWS:
        ...
        if source_id not in source_rows:
            raise PackageContractError(
                f"Missing immutable-v1 source cell: {source_id}"
            )
        ...
        if (
            ...
            raise PackageContractError(
                f"Immutable-v1 source cell drifted: {source_id}"
            )
    ...
    for target_cell in target_cells:
        ...
        if not isinstance(source_payload, dict):
            raise PackageContractError(
                "Immutable-v1 protocol is not a mapping."
            )
        ...
        if source_protocol.sha256 != source_payload.get("sha256"):
            raise PackageContractError(
                "Immutable-v1 protocol digest drifted."
            )
    ...
    source_lock_audit = digested(
        {
            ...
            "scientific_changes": [],
            "operational_changes": [
                "accepted_winner_only_phase3_stabilization_receipt_collection_v1",
                "worker_owned_live_progress_and_atomic_success_publication_v2",
                "hash_bound_checkpoint_dynamic_sidecars_v2",
            ],
