...
if [[ "$#" -ne 4 ]]; then
  echo "usage: $0 ACTIVATION_DIR PACKAGE_DIR IMAGE OUTPUT_ARCHIVE" >&2
  exit 64
fi
...
job_spec="${package_dir}/job.json"
authorization="${activation_dir}/execution_authorization.json"
activation_manifest="${activation_dir}/activation_manifest.json"
activation_validator="${activation_dir}/validate_activation.py"
archive_builder="${activation_dir}/build_attempt_archive.py"
resume_archive="${package_dir}/resume_inputs/${execution_id}.tar.gz"
source_archive="${package_dir}/source/source_locked.tar.gz"
worker_root="worker_outputs"

expected_job_sha="a7bface6ae66eb6d3d4bfc06684c8c74463ab6013a26ab2ed8ccda75053e794e"
expected_authorization_sha="7e7796346162b3faab465b0941f2fe996ba8cf4d0f820806fa5addef41f823f0"
expected_resume_sha="3d788f25e5a71a9e09c367e05de9196d7c71aabd20b984b8fbb7a1ee352c4e64"
expected_source_sha="857961b79306494c4f8311c7c833d34a7674bc1c7c6cfa25d327fdd9e9f2a8a1"
...
for required in "$job_spec" "$authorization" "$activation_manifest" "$activation_validator" "$archive_builder" "$resume_archive" "$source_archive" "$image_path" "${package_dir}/run_cell.py"; do
...
python3 "$activation_validator" --repo-root "$(pwd -P)" --require-image >/dev/null
