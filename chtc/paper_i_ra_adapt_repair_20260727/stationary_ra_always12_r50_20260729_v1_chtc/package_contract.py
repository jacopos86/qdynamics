...
PACKAGE_ID = "paper_i_ra_adapt_stationary_ra_always12_r50_20260729_v1_chtc"
PACKAGE_RELATIVE_ROOT = (
    "chtc/paper_i_ra_adapt_repair_20260727/"
    "stationary_ra_always12_r50_20260729_v1_chtc"
)
...
BATCH_NAME = "paper-i-ra-adapt-stationary-ra-always12-r50-20260729-v1"

CORE_MATERIALIZATION_RELATIVE_ROOT = (
    "chtc/paper_i_ra_adapt_repair_20260727/bundles/materializations/"
    "ra_adapt_stationary_late_core_v12"
)
...
CORE_SINGLETON_ROUTES = (
    "append_singleton",
    "ra_singleton_append_only",
    "ra_singleton_plateau",
    "ra_singleton_always",
)
CORE_ROUTE_IDS = (*CORE_MACRO_ROUTES, *CORE_SINGLETON_ROUTES)
CORE_APPEND_ROUTES = frozenset({"append_macro", "append_singleton"})
SOURCE_CORE_CELL_COUNT = 48
MACRO_ROUTES = ("ra_macro_always",)
SINGLETON_ROUTES = ("ra_singleton_always",)
ROUTE_IDS = (*MACRO_ROUTES, *SINGLETON_ROUTES)
DIRECT_EXECUTION_COUNT = 12
APPEND_ROUTES = frozenset()
RA_ROUTES = frozenset(ROUTE_IDS)
INSERTION_CAPABLE_ROUTES = frozenset(ROUTE_IDS)
...
P4_EXECUTION_ID = "core__strong_weak_u8__nph3__ra_macro_always"
P4_SMOKE_ROUNDS = 2
...
def representation_for_route(route_id: str) -> str:
    if route_id in CORE_MACRO_ROUTES:
        return "macro_generator_v1"
    if route_id in CORE_SINGLETON_ROUTES:
...
def _execution_rows(
    ...
    for regime_id, nph in REGIME_CUTOFF_PAIRS:
        for route_id in route_ids:
            ...
            method = "append" if route_id in CORE_APPEND_ROUTES else "ra"
            ...
            rows.append(
                {
                    ...
                    "execution_entrypoint": (
                        "run_append_adapt"
                        if route_id in CORE_APPEND_ROUTES
                        else "run_ra_adapt"
                    ),
                    "resources": {
                        "request_cpus": cpus,
...
def core_authority_rows() -> tuple[dict[str, Any], ...]:
    rows = _execution_rows(CORE_ROUTE_IDS)
    if (
        len(rows) != SOURCE_CORE_CELL_COUNT
        or len({row["execution_id"] for row in rows})
        != SOURCE_CORE_CELL_COUNT
    ):
        raise AssertionError(
            "The immutable Paper-I source core must be exactly 48 rows."
        )
    return rows


def direct_execution_rows() -> tuple[dict[str, Any], ...]:
    rows = _execution_rows(ROUTE_IDS)
    if (
        ...
        raise AssertionError(
            "The RA-always successor must be exactly 12 direct rows."
        )
    return rows
...
def _validate_protocol(
    ...
    *,
    ...
    expected_schema = (
        APPEND_PROTOCOL_SCHEMA
        if route_id in CORE_APPEND_ROUTES
        else RA_PROTOCOL_SCHEMA
    )
...
def validate_core_authority(
    ...
    *,
    ...
    if (
        ...
        or matrix
        != {
            "cell_count": SOURCE_CORE_CELL_COUNT,
            "regime_cutoff_pairs": [
                {"regime_id": regime_id, "nph": nph}
                for regime_id, nph in REGIME_CUTOFF_PAIRS
            ],
            "semantic_route_ids": list(CORE_ROUTE_IDS),
            "horizon": 50,
            "direct_execution_cell_count": SOURCE_CORE_CELL_COUNT,
        }
        or stationarity
        != {
    ...
    expected_rows = {
        str(row["cell_id"]): row for row in core_authority_rows()
    }
    if (
    ...
    if (
        ...
        or int(source_locks.get("required_cell_lock_count", -1))
        != SOURCE_CORE_CELL_COUNT
        ...
        or int(expected_artifacts.get("cell_count", -1))
        != SOURCE_CORE_CELL_COUNT
    ...
    if (
        ...
        or len(protocol_paths) != SOURCE_CORE_CELL_COUNT
        or len(template_paths) != SOURCE_CORE_CELL_COUNT
    ):
        raise PackageContractError(
    ...
    for cell_id, row in expected_rows.items():
        ...
        expected_entrypoint = (
            ...
            if row["route_id"] in CORE_APPEND_ROUTES
...
def validate_p3_receipt(
    ...
    *,
    ...
    for raw in raw_routes:
        ...
        if route_id in INSERTION_CAPABLE_ROUTES:
            ...
            shared_witness_invalid = (
                ...
                or witness.get("phase_i_full_logical_positions_verified")
                is not True
                or int(
                    witness.get(
                        "phase_i_full_logical_position_round_count",
                        -1,
                    )
                )
                != witness_cap
            ...
            if route_id.endswith("plateau"):
                ...
                route_witness_invalid = (
                    ...
                    or witness.get("interior_witness_status")
                    != "observed"
                    or witness.get("limitation") is not None
                )
    ...
    if set(route_rows) != set(ROUTE_IDS):
        raise PackageContractError(
            "P3 does not observe both selected RA-always routes."
        )
    return {
