...
def validate_attempt(path: Path, *, relative_path: str) -> dict[str, Any]:
    ...
    if execution_id not in direct_execution_ids():
        raise PackageContractError(
            f"Attempt is outside the 12-cell matrix: {execution_id}"
        )
    ...
    with tarfile.open(path, "r:gz") as archive:
        ...
        if exit_status == 0:
            ...
            retained_worker_members = {
                name
                for name in members
                if name.startswith("worker_outputs/")
            }
            if (
                not expected_members.issubset(members)
                or set(members)
                != retained_worker_members | {job_name}
            ):
                raise PackageContractError(
                    "Attempt retained-file namespace drifted."
                )
            ...
            if (
                ...
                raise PackageContractError(
                    "Failed attempt retained-file namespace drifted."
                )
    return {
