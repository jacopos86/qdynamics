...
post-seal mutation: after validating an externally produced, two-round
...
def _validate_plan_and_jobs(
    *,
    ...
    if len(queue_rows) != DIRECT_EXECUTION_COUNT:
        raise PackageContractError(
            "Queue must contain exactly 12 rows."
        )
    ...
    if len(execution_by_id) != DIRECT_EXECUTION_COUNT:
...
def _validate_submit_surface() -> None:
    ...
    required = (
        ...
        "stationary_ra_always12_r50_20260729_v1_chtc/queue.tsv",
        "request_cpus = $(cpus)",
        ...
        "$(Cluster).$(Process)",
        "$(execution_id)",
        "stationary_ra_always12_r50_20260729_v1_chtc_runtime",
    )
...
def _validate_p4_smoke_payload(
    *,
    ...
    if (
        ...
        or int(smoke.get("maximum_controller_rounds", -1))
        != P4_SMOKE_ROUNDS
    ...
    full_insertion_witness = smoke.get("full_insertion_witness")
    if not isinstance(full_insertion_witness, Mapping):
        raise PackageContractError(
            "P4 smoke has no full-insertion witness."
        )
    full_insertion_witness_sha256 = verify_self_digest(
        full_insertion_witness,
        label="P4 full-insertion witness",
    )
    if (
        full_insertion_witness.get("schema")
        != "paper_i_ra_adapt_always_p4_full_insertion_witness_v1"
        or full_insertion_witness.get("typed_full_insertion_verified")
        is not True
        or full_insertion_witness.get(
            "phase_i_full_logical_positions_verified"
        )
        is not True
        or int(
            full_insertion_witness.get("controller_round_count", -1)
        )
        != P4_SMOKE_ROUNDS
        or int(
            full_insertion_witness.get("interior_scored_count", 0)
        )
        <= 0
        or full_insertion_witness.get("status") != "passed"
        or not isinstance(full_insertion_witness.get("rounds"), list)
        or len(full_insertion_witness["rounds"]) != P4_SMOKE_ROUNDS
        or smoke.get("full_insertion_witness_sha256")
        != full_insertion_witness_sha256
    ):
        raise PackageContractError(
            "P4 full-insertion witness contract drifted."
        )
    artifact_roles = [
        str(raw.get("role", ""))
...
def _validate_p4_artifact_semantics(
    *,
    ...
    if (
        ...
        or smoke_job.get("route_id") != "ra_macro_always"
        ...
        or len(trajectory) != P4_SMOKE_ROUNDS
        or not isinstance(transitions, list)
        or len(transitions) != P4_SMOKE_ROUNDS
        or not isinstance(replay, list)
        or len(replay) != P4_SMOKE_ROUNDS
        or run.get("final_state") != trajectory[-1]
        or run.get("problem") != problem
        or not isinstance(stop, Mapping)
        or int(stop.get("completed_controller_rounds", -1))
        != P4_SMOKE_ROUNDS
        or int(stop.get("accepted_operator_count", -1))
        != P4_SMOKE_ROUNDS
    ...
    accepted_rounds = scientific.get("accepted_round_receipts")
    if (
        not isinstance(insertion, Mapping)
        or insertion.get("kind") != "full_commutation"
        or not isinstance(accepted_rounds, list)
        or len(accepted_rounds) != P4_SMOKE_ROUNDS
    ):
        raise PackageContractError(
            "Retained P4 full-insertion authority/round receipts drifted."
        )
    interior_scored_count = 0
    for round_index, raw_round in enumerate(accepted_rounds, start=1):
        if not isinstance(raw_round, Mapping):
            raise PackageContractError(
                "Retained P4 accepted-round receipt is malformed."
            )
        population = raw_round.get(
            "scored_insertion_position_population"
        )
        if not isinstance(population, Mapping):
            raise PackageContractError(
                "Retained P4 accepted round lacks its scored population."
            )
        verify_self_digest(
            population,
            label=f"retained P4 scored population round {round_index}",
        )
        ...
        if (
            ...
            or phases[0].get("phase") != "phase_i"
            or not isinstance(phases[0].get("records"), list)
        ):
            raise PackageContractError(
                "Retained P4 Phase-I scored population drifted."
            )
        positions_by_generator: dict[tuple[int, str], set[int]] = {}
        for raw_record in phases[0]["records"]:
            if not isinstance(raw_record, Mapping):
                raise PackageContractError(
                    "Retained P4 Phase-I record is malformed."
                )
            key = (
                int(raw_record.get("pool_index", -1)),
                str(raw_record.get("generator_id", "")),
            )
            positions_by_generator.setdefault(key, set()).add(
                int(raw_record.get("insertion_position", -1))
            )
        expected_positions = set(range(append_position + 1))
        if (
            not positions_by_generator
            or any(
            ...
            raise PackageContractError(
                "Retained P4 full insertion did not score every Phase-I "
                "generator at every logical position."
            )
        interior_scored_count += int(
            population.get("interior_scored_count", 0)
        )
    if interior_scored_count <= 0:
        raise PackageContractError(
            "Retained P4 RA-always smoke has no interior scored position."
        )
    ...
    if (
        ...
        or int(checkpoint_adapt.get("ansatz_depth", -1))
        != P4_SMOKE_ROUNDS
        or int(checkpoint_adapt.get("history_count", -1))
        != P4_SMOKE_ROUNDS
    ...
    if (
        ...
        or len(accepted_error_trace) != P4_SMOKE_ROUNDS
...
def validate_package(
    *,
    ...
    if (
        ...
        or int(manifest.get("direct_execution_count", -1))
        != DIRECT_EXECUTION_COUNT
    ...
    if (
        ...
        or int(p2.get("direct_cell_count", -1))
        != DIRECT_EXECUTION_COUNT
    ...
    p4_execution_id = P4_EXECUTION_ID
    if (
        ...
        or int(smoke_spec.get("maximum_controller_rounds", -1))
        != P4_SMOKE_ROUNDS
    ...
    if require_p4:
        ...
        if (
            ...
            or int(p4.get("maximum_controller_rounds", -1))
            != P4_SMOKE_ROUNDS
            ...
            or int(preauth.get("direct_execution_count", -1))
            != DIRECT_EXECUTION_COUNT
