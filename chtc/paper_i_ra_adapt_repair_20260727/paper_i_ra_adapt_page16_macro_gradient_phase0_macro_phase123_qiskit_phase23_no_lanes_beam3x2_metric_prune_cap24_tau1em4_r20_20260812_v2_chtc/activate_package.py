...
def _render_submit_template(
    *,
    ...
    expected_batch_line = f'+JobBatchName = "{BATCH_NAME}"'
    if rendered.count(expected_batch_line) != 1:
        raise PackageContractError(
            "Rendered submit descriptor lost its unique batch-name binding."
        )
...
def activate(
    *,
    ...
    if (
        ...
        or package_manifest.get("batch_name") != BATCH_NAME
