#!/usr/bin/env python3
"""Preflight or locally execute one sealed threshold-sweep RA r50 row."""
...
def _load_closed_job(
    ...
    if (
        ...
        or manifest.get("row_count") != len(expected_ids)
        ...
        raise PackageContractError("Inert one-row package manifest drifted.")
    ...
    if len(matching) != 1:
        raise PackageContractError("Requested job is outside the one-row closure.")
...
def _validate_authorization(
    ...
    *,
    ...
    if (
        ...
        != (
            "single_cell_local_execution_only"
            if EXECUTION_TARGET == "local"
            else "single_cell_chtc_execution_only"
        )
        or authority.get("authorization_kind")
        != (
            "explicit_user_local_execution_authority"
            if EXECUTION_TARGET == "local"
            else "explicit_user_execution_and_submission_authority"
        )
        or authority.get("execution_authorized") is not True
        or authority.get("submission_authorized")
        != (EXECUTION_TARGET == "chtc")
        or authority.get("execution_target") != EXECUTION_TARGET
        or authority.get("remote_stage") is not False
        or authority.get("condor_submit") is not False
...
def preflight(job_path: Path) -> dict[str, Any]:
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_sweep_weak_weak_r50_worker_preflight_v1",
            "status": "passed",
...
def smoke_one_round(job_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return digested(
            {
                "schema": "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_sweep_weak_weak_r50_worker_smoke_v1",
    ...
    *, job_path: Path, authorization_path: Path, output_dir: Path, receipt_path: Path
    ...
    try:
        ...
        execution_manifest = digested(
            {
                "schema": "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_sweep_weak_weak_r50_execution_manifest_v1",
                "status": "passed",
        ...
        receipt = digested(
            {
                "schema": "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_sweep_weak_weak_r50_worker_receipt_v1",
                "status": "passed",
                "package_id": PACKAGE_ID,
                "campaign_id": CAMPAIGN_ID,
                "execution_id": job["execution_id"],
