#!/usr/bin/env python3
"""Fail-closed contract for historical-mean global-singleton RA activation."""
...
BATCH_NAME = (
    "paper-i-ra-historical-mean-global-singleton-plateau3-nph7-r50-"
    "20260802-v2-finalizer-repair"
)
...
def _load_package_contract(package_dir: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "historical_mean_global_singleton_ra_r50_sealed_package_contract", path
    )
...
def validate_sealed_package(repo_root: Path) -> dict[str, Any]:
    ...
    if not isinstance(manifest_jobs, list) or len(manifest_jobs) != 3:
