...
package_attempt() {
  ...
  if [[ -e "${worker_root}/checkpoint.json" \
     || -L "${worker_root}/checkpoint.json" ]]; then
    needs_compaction=1
  else
    for ledger_candidate in \
      "${worker_root}"/checkpoint.estimator_call_ledger_checkpoint.*.json
    do
  ...
  if [[ ! -e "$output_archive" ]]; then
    if [[ "$needs_compaction" -eq 1 ]]; then
      if [[ ! -f "$archive_builder" || -L "$archive_builder" ]]; then
        packaging_status=71
      else
        apptainer exec \
          --cleanenv \
      ...
      tar -czf "$output_archive" "$worker_root" "$job_spec" \
        || packaging_status=71
    fi
  else
    packaging_status=71
