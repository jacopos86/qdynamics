#!/usr/bin/env python3
"""Preflight or execute one gradient-Phase-0 Qiskit cell."""
...
def _load_closed_job(
    ...
    if (
        ...
        raise PackageContractError("Inert matched six-cell package manifest drifted.")
    ...
    expected_algorithm = (
        RA_ALGORITHM_ID if method == "ra_page12" else APPEND_ALGORITHM_ID
    )
    expected_policy = (
        ACTIVE_GRADIENT_RA if method == "ra_page12" else ACTIVE_GRADIENT_APPEND
    )
    expected_entrypoint = (
        "run_ra_adapt" if method == "ra_page12" else "run_append_adapt"
    )
    if (
        ...
        or job.get("regime_id") not in REGIMES
        or job.get("num_sites") != 3
        or job.get("nph") != 3
        ...
        raise PackageContractError("Sealed matched L3 job drifted.")
...
def _validate_authorization(
    ...
    *,
    ...
    if (
        ...
        != "prepare_single_cell_chtc_execution_and_submission_v1"
...
def _load_protocol(
    *,
    ...
    method = str(job.get("method", ""))
    expected_algorithm = RA_ALGORITHM_ID if method == "ra_page12" else APPEND_ALGORITHM_ID
    expected_policy = ACTIVE_GRADIENT_RA if method == "ra_page12" else ACTIVE_GRADIENT_APPEND
    common_invalid = (
        receipt is None or receipt.bundle_id != BUNDLE_ID
        or receipt.bundle_manifest_sha256 != job.get("bundle_manifest_sha256")
        ...
        or protocol.request.execution.stop.maximum_controller_rounds != 50
        or protocol.request.execution.resume.kind != "fresh_start"
        or protocol.request.adapter.adapter_id != CANDIDATE_ADAPTER_ID
        or protocol.active_gradient_policy != expected_policy
        ...
        or protocol.problem.num_sites != 3 or protocol.problem.n_ph_max != 3
        ...
        or protocol.optimizer != "powell" or protocol.optimizer_maxiter != 200
        or protocol.seeds.get("adapt") != 7
    )
    if method == "ra_page12":
        ...
        method_invalid = (
            ...
            or protocol.selector_identity
            != "append_adapt_largest_absolute_commutator_gradient_v1"
            or protocol.selector_scope
        ...
        method_invalid = True
    if common_invalid or method_invalid:
        raise PackageContractError("Typed matched L3 protocol drifted.")
    cell_lock_id = str(protocol.source_locks["cell_source_lock_id"])
    cell_lock = _mapping(
        _mapping(source_locks.get("cell_locks"), label="cell locks").get(
            cell_lock_id
        ),
        label="cell source lock",
    )
    authority_refs = {
        "source_locks_manifest_sha256": str(source_locks["sha256"]),
        "implementation_source_inventory_sha256": str(
            source_locks["implementation_sources"]["sha256"]
        ),
        "cell_source_lock_id": cell_lock_id,
        "cell_source_lock_sha256": str(cell_lock["sha256"]),
        "ed_cutoff_reference_sha256": str(
            cell_lock["same_cutoff_exact_reference_receipt_sha256"]
        ),
        APPLICATION_SOURCE_LOCK_KEY: str(
            job["application_source_contract_sha256"]
        ),
        "l3_route_contract_sha256": TARGET_ROUTE_CONTRACT_SHA256,
    }
    authority = _mint_bundle_protocol_materialization_authority(
        receipt,
        source_lock_refs=authority_refs,
    ...
    *,
    ...
    try:
        ...
        summary = (
            ...
            if job["method"] == "ra_page12"
            else result.paper_i_summary
        )
        if summary is None:
            raise PackageContractError(
