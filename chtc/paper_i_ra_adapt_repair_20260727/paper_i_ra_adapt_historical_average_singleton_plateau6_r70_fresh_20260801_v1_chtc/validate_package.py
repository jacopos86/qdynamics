#!/usr/bin/env python3
"""Fail-closed validation for the inert six-cell historical-average package."""
...
def validate_package(
    *, deep: bool = False, smoke_one_round: bool = False
    ...
    route_paths = {tuple(path) for path in ROUTE_DIFFERENCE_PATHS}
    horizon_paths = {tuple(path) for path in HORIZON_DIFFERENCE_PATHS}
    ...
    for row in audit_rows:
        if (
            ...
            or {tuple(item["path"]) for item in row.get("route_differences", [])}
            != route_paths
            or {tuple(item["path"]) for item in row.get("horizon_differences", [])}
            != horizon_paths
        ):
            raise PackageContractError("Route/horizon delta audit drifted.")
    ...
    for execution_id, (regime_id, nph) in zip(expected_ids, REGIME_ROWS, strict=True):
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_historical_average_singleton_plateau6_r70_package_validation_v1",
            "status": "passed",
