#!/usr/bin/env python3
"""Run one authorized cell from a local Qiskit-cost diagnostic.
...
RUNNER_SCHEMA = "paper_i_ra_adapt_local_qiskit_cost_runner_v2"
...
QISKIT_DIAGNOSTIC_SPECS = {
    (
        ...
        "insertion_kind": "plateau_commutation",
        "protocol_horizon": 50,
    },
    (
    ...
    (
        ...
        "candidate_representation": "macro_generator_v1",
        "insertion_kind": "always_commutation_reduced",
        "protocol_horizon": (50,),
    },
    (
        ...
        "candidate_representation": "macro_generator_v1",
        "insertion_kind": "always_commutation_reduced",
        # 13 is the original one-cell always13 diagnostic. 15 and 20 are the
        # six-regime always6 diagnostic horizons (20/20/15/20/20/15).
        "protocol_horizon": (13, 15, 20, 50),
    },
}
...
def _validate_protocol_and_inventory(
    ...
    expected = QISKIT_DIAGNOSTIC_SPECS.get(protocol.algorithm_id)
    if expected is None:
        raise LocalRunContractError(
            "The selected protocol is not an authorized Qiskit-cost "
            "diagnostic cell."
        )
    ...
    if (
        ...
        raise LocalRunContractError(
            "The selected protocol representation does not match its "
            "Qiskit-cost algorithm identity."
        )
    if (
        protocol.request.method.insertion.kind
        != expected["insertion_kind"]
    ):
        raise LocalRunContractError(
            "The selected protocol insertion policy does not match its "
            "Qiskit-cost algorithm identity."
        )
    ...
    allowed_horizons = expected["protocol_horizon"]
    if isinstance(allowed_horizons, int):
        allowed_horizons = (allowed_horizons,)
    allowed_horizons = tuple(int(value) for value in allowed_horizons)
    if int(protocol.horizon) not in allowed_horizons:
        raise LocalRunContractError(
            "The diagnostic protocol horizon does not match its algorithm "
            f"identity: expected one of {allowed_horizons}."
        )
    expected_horizon = int(protocol.horizon)
    if (
        int(
        ...
        raise LocalRunContractError(
            "The validated protocol lost its fixed-horizon fresh-start "
            "execution contract."
        )
    ...
    return _digested(
        {
            ...
            "authorization_basis": (
                "explicit_user_local_qiskit_cost_diagnostic_authorization_"
                "20260730"
            ),
    ...
    *,
    ...
    try:
        ...
        if result.run.paper_i_summary is None:
            raise LocalRunContractError(
                "The completed Qiskit-cost run has no Paper-I summary."
            )
        result_payload = result.to_dict()
        summary_payload = result.run.paper_i_summary.to_dict()
        receipts_payload = dict(result.scientific_receipts)
        _atomic_write_json_noreplace(paths["result"], result_payload)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run one validated local Qiskit-cost RA-ADAPT diagnostic."
        )
    )
    parser.add_argument(
        "--protocol",
