...
def _checkpoint_triplets(root: Path) -> list[dict[str, Any]]:
    """Return structurally closed checkpoint triplets available for recovery."""

    triplets: list[dict[str, Any]] = []
    for checkpoint in sorted(root.rglob("checkpoint.json")):
        if not checkpoint.is_file() or checkpoint.is_symlink():
            raise AttemptArchiveError("Checkpoint member is unsafe.")
        directory = checkpoint.parent
        ledgers = sorted(
            directory.glob("checkpoint.estimator_call_ledger_checkpoint.*.json")
        )
        sidecars = sorted(
            directory.glob("checkpoint.verified_singleton_resume.*.json")
        )
...
def build_archive(args: argparse.Namespace) -> dict[str, Any]:
    ...
    triplets = _checkpoint_triplets(args.worker_root)
