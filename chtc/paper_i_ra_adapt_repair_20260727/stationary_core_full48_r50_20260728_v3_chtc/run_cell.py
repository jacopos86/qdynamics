    ...
    return digested(
        {
            ...
            "typed_reporting_projection": {
                "source": (
                    "paper_i_run_summary_v1.provenance"
                    if not append
                    else "core_g8_post_controller_receipt_v1"
                ),
                "controller_decision_influence": False,
                "reporting_only": True,
            },
    ...
    *,
    ...
    if append:
        ...
        if (
            ...
            always_route and not full_insertion_policy_verified
        ) or (
        ...
        g5 = {
            ...
            "interior_witness_status": (
                ...
                if always_route and interior == 0
                else "observed"
                if interior > 0
                else "not_applicable"
            ),
    ...
    if append:
        ...
        for index, (raw, raw_replay) in enumerate(
            ...
            accepted_refit = _mapping(
                ...
                label=f"RA G6 accepted refit {index}",
            )
            supported_metric = _mapping(
                accepted_refit.get("supported_metric"),
                label=f"RA G6 supported metric {index}",
            )
            kappa = _finite(
            ...
            if (
                ...
                or stabilization.get(
                    "metric_inverse_sqrt_constructed"
                )
                is not False
                or not math.isclose(
                    total,
                    ...
                    rel_tol=0.0,
                    abs_tol=128.0
                    * math.ulp(max(1.0, abs(total))),
                )
                or bool(
                    stabilization.get("trust_boundary_active")
                )
                != bool(boundary > 0.0)
                or trust.get(
                ...
                raise PackageContractError(
                    f"G6 Phase-III integrity failed: "
                    f"{job['execution_id']} round {index}."
                )
        g6 = {
    ...
    *,
    ...
    try:
        ...
        if job["execution_entrypoint"] == "run_ra_adapt":
            ...
            if (
                len(primary_trajectory) != 50
                or primary_trajectory[:2] != first_trajectory
                or primary_trajectory[:2] != second_trajectory
