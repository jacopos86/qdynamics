#!/usr/bin/env python3
"""Seal the Page-12 strong-sector CHTC accepted-state continuations."""
...
from package_contract import (  # noqa: E402
    ...
    V1_PACKAGE_MANIFEST_SIZE_BYTES,
...
def _materialize_resume(
    ...
    if (
        ...
        or v1_manifest.get("package_id") != V1_PACKAGE_ID
        or v1_receipt.get("package_id") != V1_PACKAGE_ID
...
+JobBatchName = "paper-i-page12-strong-r70-cont-v2-128g"
...
def build() -> dict[str, Any]:
    if any((PACKAGE_DIR / path).exists() for path in GENERATED_TARGETS):
        raise FileExistsError("Refusing to overwrite the sealed CHTC package.")
    ...
    _verify_file(
        ...
        size_bytes=V1_PACKAGE_MANIFEST_SIZE_BYTES,
    )
    ...
    if (
        ...
        != V1_PACKAGE_MANIFEST_SHA256
        or v1_package_manifest.get("package_id") != V1_PACKAGE_ID
    ...
    composition = digested(
        {
            "schema": "paper_i_page12_r70_chtc_source_composition_v1",
            "status": "passed",
    ...
    bundle = digested(
        {
            ...
            "bundle_id": BUNDLE_ID,
            "cell_count": CONTINUATION_ROW_COUNT,
            "cells": [
                {
    ...
    plan = digested(
        {
            "schema": "paper_i_page12_strong_r70_chtc_plan_v2",
            "status": "passed_inert",
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "row_count": CONTINUATION_ROW_COUNT,
    ...
    manifest = digested(
        {
            ...
            "status": "passed_inert_two_authenticated_continuations",
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "bundle_id": BUNDLE_ID,
            "run_class": RUN_CLASS,
    ...
    request = digested(
        {
            "schema": "paper_i_page12_strong_r70_chtc_activation_request_v2",
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "package_manifest_sha256": manifest["sha256"],
            "requested_execution_ids": list(expected_execution_ids()),
            "authority_scope": "two_page12_strong_continuations_to_round_70",
            "authorization_kind": (
    ...
    activation = digested(
        {
            ...
            "authorization_count": CONTINUATION_ROW_COUNT,
