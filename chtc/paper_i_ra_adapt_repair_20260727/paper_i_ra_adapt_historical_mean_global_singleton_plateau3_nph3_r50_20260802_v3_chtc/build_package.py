#!/usr/bin/env python3
"""Materialize the repaired three-cell nph3 global-singleton package."""
...
def _load_source_package(repo_root: Path) -> dict[str, Any]:
    ...
    manifest = _load_bound_json(
        ...
        label="sealed v2 runtime reference package",
    )
    archive_path, archive_manifest = _package_bound_json(
        source_package,
        manifest.get("source_archive_manifest"),
        label="runtime reference source archive manifest",
    )
    if (
...
def _load_source_bundle(repo_root: Path) -> dict[str, Any]:
    bundle_root = repo_root / SOURCE_BUNDLE_RELATIVE
    manifest = _load_bound_json(
        ...
        label="global-singleton source bundle",
    )
    if manifest.get("bundle_id") != SOURCE_BUNDLE_ID:
        raise PackageContractError("Global-singleton source bundle identity drifted.")
    locks_binding = manifest.get("source_locks")
    if not isinstance(locks_binding, Mapping):
        raise PackageContractError("Global-singleton source-lock binding is absent.")
    locks_path = bundle_root / str(locks_binding.get("path", ""))
    source_locks = _load_bound_json(
        locks_path,
        file_sha256=SOURCE_LOCKS_FILE_SHA256,
        canonical_sha256=SOURCE_LOCKS_CANONICAL_SHA256,
        label="global-singleton source locks",
    )
    if source_locks.get("sha256") != locks_binding.get("sha256"):
        raise PackageContractError("Global-singleton source-lock pointer drifted.")
    return {
        "root": bundle_root,
        "manifest": manifest,
        "protocol_bundle": manifest,
...
def _apply_exact_source_patch(source_root: Path) -> list[dict[str, Any]]:
    ...
    if completed.returncode != 0:
        raise PackageContractError(
            "Exact reporting-identity source patch failed: "
            + completed.stderr.strip()
        )
    result: list[dict[str, Any]] = []
    for relative, row in expected.items():
        if sha256_file(source_root / relative) != row["after_sha256"]:
...
def _activate_source_root(source_root: Path) -> None:
    ...
    try:
        ...
        raise PackageContractError("Materializer escaped the sealed runtime archive.") from exc
...
def _target_cell(source_cell: Any, *, horizon: int) -> Any:
    return replace(
        ...
        stage="historical_mean_global_singleton_v3_nph3",
        horizon=int(horizon),
    )
...
def _validate_source_to_target(
    *,
    ...
    differences = scalar_differences(source_payload, target_payload)
    difference_paths = {path for path, _before, _after in differences}
    if difference_paths != ALLOWED_SOURCE_TO_TARGET_DIFF_PATHS:
        raise PackageContractError(
            "Historical-mean global-singleton derivation changed unexpected "
            f"fields: {sorted(difference_paths, key=str)}"
        )
    ...
    if (
        ...
        != "insertion_commutation_plateau_v2"
        or invariants["candidate_adapter_id"] != CANDIDATE_ADAPTER_ID
        or invariants["phase_i_candidate_supply"]
        != PHASE_I_CANDIDATE_SUPPLY
        or invariants["phase_i_candidate_visibility"]
        != PHASE_I_CANDIDATE_VISIBILITY
        or invariants["phase_ii_candidate_exposure"]
        != PHASE_II_CANDIDATE_EXPOSURE
        or invariants["admission_cardinality"]
        != PHASE_III_ADMISSION_CARDINALITY
        or route_execution["phase1_shortlist_size"]
        != PHASE_I_SHORTLIST_SIZE
        or route_execution["phase2_shortlist_size"]
        != PHASE_II_SHORTLIST_SIZE
        or route_execution["phase3_runtime_split_max_subset_size"]
        != PHASE_III_ADMISSION_CARDINALITY
        ...
        raise PackageContractError(
            "Target historical-mean global-singleton protocol drifted."
        )
    return {
        "source_to_target_differences": [
...
def build() -> dict[str, Any]:
    ...
    runtime_package = _load_source_package(repo_root)
    source_bundle = _load_source_bundle(repo_root)
    temporary = tempfile.TemporaryDirectory(
        prefix="paper-i-historical-mean-global-singleton-source-"
    )
    source_root = (
        Path(temporary.name) / "source_locked_checkout"
    ).resolve()
    source_members = _extract_predecessor_source(
    ...
    if (
        ...
        raise PackageContractError(
            "Extracted v2 runtime implementation inventory drifted."
        )
    source_locks = _normalize_source_locks(
        source_bundle["source_locks"],
    ...
    source_by_path = {str(row["path"]): row for row in source_members}
    ...
    for relative in sorted(source_by_path):
        ...
        if relative not in patch_paths and row != source_by_path[relative]:
            raise PackageContractError(
                f"Sealed runtime archive member drifted: {relative}"
            )
        members.append(row)
    ...
    source_rows = {
        str(row["cell_id"]): row
        for row in source_bundle["protocol_bundle"]["cells"]
    }
    ...
    for regime_id, nph in REGIME_ROWS:
        ...
        if source_id not in source_rows:
            raise PackageContractError(
                f"Missing global-singleton source cell: {source_id}"
            )
        ...
        if (
            ...
            raise PackageContractError(
                f"Global-singleton source cell drifted: {source_id}"
            )
    ...
    protocol_bundle_manifest = digested(
        {
            ...
            "source_bundle": {
                ...
                "bundle_manifest_sha256": source_bundle["manifest"]["sha256"],
                "source_locks_sha256": source_bundle["source_locks"]["sha256"],
            },
            "runtime_reference": {
                "path": SOURCE_PACKAGE_RELATIVE.as_posix(),
                "package_manifest_sha256": runtime_package["manifest"]["sha256"],
                "source_archive_sha256": RUNTIME_SOURCE_ARCHIVE_FILE_SHA256,
    ...
    for target_cell in target_cells:
        ...
        if not isinstance(source_payload, dict):
            raise PackageContractError(
                "Global-singleton source protocol is not a mapping."
            )
        ...
        if source_protocol.sha256 != source_payload.get("sha256"):
            raise PackageContractError(
                "Global-singleton source protocol digest drifted."
            )
        ...
        job = digested(
            {
                ...
                "phase_i_candidate_supply": PHASE_I_CANDIDATE_SUPPLY,
                "phase_i_candidate_visibility": PHASE_I_CANDIDATE_VISIBILITY,
                "phase_ii_candidate_exposure": PHASE_II_CANDIDATE_EXPOSURE,
                "phase_i_shortlist_size": PHASE_I_SHORTLIST_SIZE,
                "phase_ii_shortlist_size": PHASE_II_SHORTLIST_SIZE,
                "phase_iii_admission_cardinality": (
                    PHASE_III_ADMISSION_CARDINALITY
                ),
    ...
    source_archive_manifest = digested(
        {
            ...
            "predecessor_archive_manifest_sha256": (
                RUNTIME_SOURCE_ARCHIVE_MANIFEST_CANONICAL_SHA256
            ),
    ...
    source_lock_audit = digested(
        {
            ...
            "status": "passed",
            "source_bundle_manifest_sha256": source_bundle["manifest"]["sha256"],
            "runtime_reference_package_manifest_sha256": (
                runtime_package["manifest"]["sha256"]
            ),
            ...
            "unchanged_scientific_settings": [
                "global_guarded_singleton_phase_i_pool",
                "phase_i_shortlist_24_phase_ii_shortlist_12_phase_iii_admission_1",
                "stationary_source_response_v1",
                "all_phase_resource_weighting_v1",
                "plateau_prior_mean_ratio_threshold_1e-4",
                "fresh_maximum_controller_rounds_50",
    ...
    execution_plan = digested(
        {
            "schema": (
                ...
                "plateau3_nph3_r50_execution_plan_v2"
            ),
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "run_class": RUN_CLASS,
    ...
    manifest = digested(
        {
            ...
            "status": "passed_inert_three_rows",
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "row_count": len(job_payloads),
            "execution_ids": [job["execution_id"] for job in job_payloads],
