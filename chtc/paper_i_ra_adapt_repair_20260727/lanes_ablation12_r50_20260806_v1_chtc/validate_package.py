#!/usr/bin/env python3
"""Read-only validation for the repaired no-submit package."""
...
def _validate_archive(
    ...
    declared = {
        safe_relative_path(row.get("path"), label="archive member").as_posix():
        row
        for row in rows
...
def main() -> int:
    ...
    if (PACKAGE_DIR / "authority").exists():
        raise PackageContractError(
            "The v2 package contains a forbidden authority overlay."
        )
    manifest = load_json(
    ...
    if (
        ...
        or manifest["core_final_receipt"]
        != authority["final_binding"]
        or manifest["always_route_delta_receipt"]
        != authority["route_delta_binding"]
    ):
    ...
    for row, binding in zip(expected_rows, jobs):
        ...
        if (
            ...
            or job.get("protocol")
            != authority["protocol_bindings"][row["cell_id"]]
