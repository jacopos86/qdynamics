#!/usr/bin/env python3
"""Fail-closed contract for the inert conventional-Append round-70 package."""
...
CAMPAIGN_ID = (
    "paper_i_append_adapt_stationary_core_r70_fresh_v1"
)
...
PACKAGE_MANIFEST_SCHEMA = (
    "paper_i_append_adapt_stationary_core_r70_fresh_package_v1"
)
EXECUTION_PLAN_SCHEMA = (
    "paper_i_append_adapt_stationary_core_r70_fresh_execution_plan_v1"
)
JOB_SCHEMA = (
    "paper_i_append_adapt_stationary_core_r70_fresh_job_v1"
)
SOURCE_AUTHORITY_SCHEMA = (
    "paper_i_append_adapt_stationary_core_r70_source_authority_v1"
)
...
ROUTE_IDS = ("append_macro", "append_singleton")
CANDIDATE_REPRESENTATIONS = {
    "append_macro": "macro_generator_v1",
...
EXPECTED_EXECUTION_IDS = tuple(
    f"r70_fresh__{regime_id}__nph{nph}__{route_id}"
...
def _expected_source_for_execution(execution_id: str) -> str:
    ...
    return "core__" + execution_id.removeprefix("r70_fresh__")
...
def _expected_execution_semantics(
    ...
    for regime_id, nph in REGIME_CUTOFF_PAIRS:
        for route_id in ROUTE_IDS:
            expected = f"r70_fresh__{regime_id}__nph{nph}__{route_id}"
...
def _expected_artifact_paths(execution_id: str) -> dict[str, str]:
    root = (
        "raw_outputs/"
        "paper_i_append_adapt_stationary_core12_r70_fresh_20260731_v1/"
        f"{execution_id}"
    )
    return {
