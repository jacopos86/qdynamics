#!/usr/bin/env python3
"""Build one authenticated, failure-safe accepted-resume attempt archive."""
...
def _worker_files(
    ...
    imported_resume_root = (
        root / f".{execution_id}.resume_work_v1" / "resume_input"
    )
    files: list[Path] = []
    excluded: list[Path] = []
    for path in sorted(root.rglob("*")):
        ...
        try:
            path.relative_to(imported_resume_root)
        except ValueError:
            files.append(path)
        else:
...
def _checkpoint_triplets(
    ...
    """Return structurally closed checkpoint triplets available for recovery."""
    ...
    for checkpoint in sorted(
        ...
        if not checkpoint.is_file() or checkpoint.is_symlink():
            raise AttemptArchiveError("Checkpoint member is unsafe.")
...
def build_archive(args: argparse.Namespace) -> dict[str, Any]:
    ...
    worker_files, excluded_imported_resume_files = _worker_files(
    ...
    receipt: dict[str, Any] = {
        ...
        "excluded_imported_resume_file_count": len(
            excluded_imported_resume_files
        ),
        "excluded_imported_resume_bytes": sum(
            path.stat().st_size for path in excluded_imported_resume_files
        ),
        "source_resume_archive_retained_separately": True,
