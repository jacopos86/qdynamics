...
if [[ "$#" -ne 13 ]]; then
  echo "usage: $0 ACTIVATION_DIR PACKAGE_DIR JOB JOB_FILE_SHA AUTH AUTH_FILE_SHA SOURCE SOURCE_SHA IMAGE IMAGE_SHA RESUME RESUME_SHA OUTPUT_ARCHIVE" >&2
  exit 64
fi
...
source_archive="$7"
expected_source_sha256="$8"
image_path="$9"
expected_image_sha256="${10}"
resume_archive="${11}"
expected_resume_sha256="${12}"
output_archive="${13}"
...
if [[ "$(hash_file "$resume_archive")" != "$expected_resume_sha256" ]]; then
  echo "resume archive SHA-256 mismatch" >&2
  exit 66
fi
...
package_attempt() {
  ...
  "$runtime_bin" exec \
    ...
    python3 "$archive_builder" \
      ...
      --resume-archive-sha256 "$expected_resume_sha256" \
