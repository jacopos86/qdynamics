"""Atomically materialize and audit immutable Paper-I Study-1 v7 bundles.

This append-only command has no scientific execution or scheduler seam.  It:

* verifies the exact immutable v1-v6 materialization anchors;
* byte-copies v6's inherited source-materialization tree;
...
* atomically publishes v7 without replacing any existing path; and
...
REPO_ROOT = v6.REPO_ROOT
CAMPAIGN_ROOT = v6.CAMPAIGN_ROOT
MATERIALIZATIONS_ROOT = v6.MATERIALIZATIONS_ROOT
V6_ROOT = v6.V6_ROOT
V7_ROOT = (
    MATERIALIZATIONS_ROOT / "ra_adapt_unification_post_refactor_v7"
)
V6_SOURCE_ROOT = V6_ROOT / "source_materialization"
SOURCE_LOCKS_INPUT = V6_SOURCE_ROOT / "source_locks_input.json"
PROBLEM_BASELINES = V6_SOURCE_ROOT / "problem_baselines.json"
V6_FINAL_RECEIPT = V6_ROOT / "final_materialization_receipt.json"

PACKAGE_ID = "paper_i_ra_adapt_study1_minimal_20260728_v2_chtc"
AUTHORITY_NAME = "study1_objective_gate_authority_receipt.json"
AUTHORITY_SCHEMA = (
    "paper_i_ra_adapt_study1_objective_gate_authority_v2"
)
...
EXPECTED_V6_TREE_SHA256 = (
    "a5111a509af9c2cbe91f73481805abbc43065589857c36cc0669159d5a44aaf6"
)
EXPECTED_V6_FILE_COUNT = 373
EXPECTED_V6_TOTAL_SIZE_BYTES = 14_014_385
EXPECTED_V6_FINAL_RECEIPT_FILE_SHA256 = (
    "6fca31f9a9f4eb4860198aeda6300dfa9291244f02df06f8c47a8b818209a75f"
)
EXPECTED_V6_FINAL_RECEIPT_CANONICAL_SHA256 = (
    "03265f27d0e3b6db55a9ca80914ca6fef5780e73649ff254a8d0e6ca4f5f4e36"
)
EXPECTED_V6_SOURCE_TREE_SHA256 = (
    "08ac8303870ba206493859e2037e53f0e32f758f235c06084681b149073e7c26"
)
EXPECTED_V6_SOURCE_FILE_COUNT = 126
...
MaterializationAuditError = v6.MaterializationAuditError
...
def _load_canonical_digested(path: Path, *, label: str) -> dict[str, Any]:
    return v6._load_canonical_digested(path, label=label)


def _sha256_file(path: Path) -> str:
    return v6._hash_file(path)


def _summary(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    return v6._snapshot_summary(snapshot)


def _assert_equal(actual: Any, expected: Any, *, label: str) -> None:
    v6._assert_equal(actual, expected, label=label)


def _historical_snapshots() -> dict[str, dict[str, Any]]:
    snapshots = v6._snapshot_historical_materializations()
    snapshots["v6"] = v6._snapshot_roots((V6_ROOT,))
    return snapshots
...
def _assert_historical_anchors(
    ...
    v6._assert_historical_anchors(snapshots)
    observed = snapshots.get("v6")
    if not isinstance(observed, Mapping):
        raise MaterializationAuditError("Missing immutable v6 snapshot.")
    expected = {
        "file_count": EXPECTED_V6_FILE_COUNT,
        "total_size_bytes": EXPECTED_V6_TOTAL_SIZE_BYTES,
        "tree_sha256": EXPECTED_V6_TREE_SHA256,
    }
    for key, value in expected.items():
        _assert_equal(observed.get(key), value, label=f"v6 immutable {key}")
    _assert_equal(
        _sha256_file(V6_FINAL_RECEIPT),
        EXPECTED_V6_FINAL_RECEIPT_FILE_SHA256,
        label="v6 final receipt file SHA-256",
    )
    final = _load_canonical_digested(
        V6_FINAL_RECEIPT, label="v6 final materialization receipt"
    )
    _assert_equal(
        final["sha256"],
        EXPECTED_V6_FINAL_RECEIPT_CANONICAL_SHA256,
        label="v6 final receipt canonical SHA-256",
    )
...
def _copy_v6_source_materialization(
    staging_root: Path,
) -> tuple[dict[str, Any], dict[str, Any]]:
    source_before = v6._snapshot_roots(
        (V6_SOURCE_ROOT,), relative_to=V6_SOURCE_ROOT
    )
    _assert_equal(
        source_before["file_count"],
        EXPECTED_V6_SOURCE_FILE_COUNT,
        label="v6 source-materialization file count",
    )
    _assert_equal(
        source_before["tree_sha256"],
        EXPECTED_V6_SOURCE_TREE_SHA256,
        label="v6 source-materialization tree SHA-256",
    )
    ...
    source_after = v6._snapshot_roots(
        (destination,), relative_to=destination
    )
    _assert_equal(
        source_after,
        source_before,
        label="v6-to-v7 byte-identical source inheritance",
    )
...
def _validate_v2_gate_semantics(root: Path) -> dict[str, Any]:
    ...
    return _digested(
        {
            "schema": "ra_adapt_v6_v7_gate_semantics_receipt_v1",
            "status": "passed",
            "baseline_revision": "v6",
            "target_revision": "v7",
            "objective_gate_ids": expected_ids,
            "correction_class": (
...
def _build_g3_pool_receipt(
    ...
    *,
    ...
    resolver = v6._problem_resolver_from(baselines)
...
def _write_receipt(
    ...
    return v6._write_receipt(path, payload)
...
def main() -> int:
    if V7_ROOT.exists():
        raise FileExistsError(
            f"Refusing to overwrite immutable v7: {V7_ROOT}"
        )
    if not MATERIALIZATIONS_ROOT.is_dir():
        raise MaterializationAuditError(
            f"Materializations root is missing: {MATERIALIZATIONS_ROOT}"
        )
    v6._darwin_renameatx_np()

    captured_utc = v6._utc_now()
    repository_state = v6._repository_state()
    ...
    staging_root = Path(
        tempfile.mkdtemp(
            prefix=".ra_adapt_unification_post_refactor_v7.staging.",
            dir=MATERIALIZATIONS_ROOT,
        )
    )
    source_before, source_after = _copy_v6_source_materialization(
        staging_root
    )
    inheritance_receipt = _write_receipt(
        staging_root / "source_materialization_inheritance_receipt.json",
        {
            ...
            "source_revision": "v6",
            "target_revision": "v7",
            "source_path": V6_SOURCE_ROOT.relative_to(
                REPO_ROOT
            ).as_posix(),
            "target_path": (
                V7_ROOT / "source_materialization"
    ...
    preflight_receipt = _write_receipt(
        ...
        {
            "schema": "ra_adapt_v7_preflight_receipt_v1",
            "status": "passed",
    ...
    timestamp = v6._utc_now()
    materialized_receipts = materialize_study1_bundles(
        staging_root,
        problem_resolver=v6._problem_resolver_from(baselines),
    ...
    gate_semantics = _validate_v2_gate_semantics(staging_root)
    gate_semantics_receipt = _write_receipt(
        staging_root / "v6_v7_gate_semantics_receipt.json",
        {
            key: value
            for key, value in gate_semantics.items()
            if key != "sha256"
        },
    )
    ...
    _assert_historical_unchanged(
        ...
        label="Staged v7 materialization",
    )
    source_staged = v6._snapshot_roots(
    ...
    final_gate_semantics = _validate_v2_gate_semantics(staging_root)
    _assert_equal(
        final_gate_semantics,
        gate_semantics_receipt,
        label="revalidated staged v2 gate semantics",
    )
    ...
    v6_final = _load_canonical_digested(
        V6_FINAL_RECEIPT, label="v6 final receipt for supersession"
    )
    supersession_chain = list(v6_final.get("supersession_chain", ()))
    if (
        not supersession_chain
        or supersession_chain[-1].get("revision") != "v6"
    ):
        raise MaterializationAuditError(
            "The v6 supersession chain has no v6 terminus."
        )
    supersession_chain.append(
        {
            "revision": "v7",
            "path": V7_ROOT.relative_to(REPO_ROOT).as_posix(),
            "status": "passed",
        }
    )
    ...
    staged_receipt = _write_receipt(
        ...
        {
            "schema": "ra_adapt_v7_staged_materialization_receipt_v1",
            "status": "passed",
            "materialization_revision": "v7",
            "materialization_timestamp": timestamp,
            ...
            "gate_semantics_receipt_sha256": gate_semantics_receipt[
                "sha256"
            ],
    ...
    tree_before_final = v6._snapshot_roots(
    ...
    final_receipt = _write_receipt(
        staging_root / "final_materialization_receipt.json",
        {
            ...
            "corrected_gate_semantics": {
                "path": "v6_v7_gate_semantics_receipt.json",
                "canonical_sha256": gate_semantics_receipt["sha256"],
            ...
            "v7_tree_before_final_receipt": _summary(tree_before_final),
    ...
    complete_staged_tree = v6._snapshot_roots(
        (staging_root,), relative_to=staging_root
    )
    ...
    _assert_historical_unchanged(
        ...
        label="Complete staged v7 materialization",
    )
    ...
    v6._atomic_rename_no_replace(staging_root, V7_ROOT)

    # Everything below is read-only.  A successful publication never exposes
    # an incomplete canonical v7 and never requires cleanup to retry.
    source_post_publish = v6._snapshot_roots(
        (V7_ROOT / "source_materialization",),
        relative_to=V7_ROOT / "source_materialization",
    )
    _assert_equal(
        source_post_publish,
        source_before,
    ...
    published_final_receipt = _load_canonical_digested(
        V7_ROOT / "final_materialization_receipt.json",
    ...
    _assert_equal(
        _validate_v2_gate_semantics(V7_ROOT),
        gate_semantics_receipt,
        label="staged-to-published v2 gate semantics",
    )
    _assert_equal(
        _validate_g8_materialized_protocol_guard(V7_ROOT),
        g8_protocol_guard_receipt,
    ...
    _assert_historical_unchanged(
        historical_before,
        historical_after,
        label="Published v7 materialization",
    )
    ...
    published_tree = v6._snapshot_roots(
        (V7_ROOT,), relative_to=V7_ROOT
    )
    _assert_equal(
        published_tree,
    ...
    print(
        json.dumps(
            {
                "destination": V7_ROOT.relative_to(REPO_ROOT).as_posix(),
                "status": "passed",
