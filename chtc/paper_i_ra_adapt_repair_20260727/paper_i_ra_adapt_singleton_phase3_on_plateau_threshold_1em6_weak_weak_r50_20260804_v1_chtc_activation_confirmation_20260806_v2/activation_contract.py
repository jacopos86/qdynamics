#!/usr/bin/env python3
"""Fail-closed contract for the weak-weak tau=1e-6 CHTC confirmation."""
...
ACTIVATION_ID = (
    f"{PACKAGE_ID}_activation_confirmation_20260806_v2"
)
ACTIVATION_SCHEMA = (
    "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_1em6_"
    "weak_weak_r50_confirmation_activation_v1"
)
AUTHORIZATION_SCHEMA = (
    "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_sweep_"
    "weak_weak_r50_execution_authorization_v1"
)
BATCH_NAME = (
    "paper-i-ra-phase3-on-plateau-threshold-1em6-weak-weak-"
    "r50-confirmation-20260806-v2"
)
IMAGE_PATH = "chtc/phase3_optuna/image.sif"
IMAGE_SHA256 = (
    "fa5c4ea89a19ad2fa4c264f3bce65d9e83b86b91618ed97dfc80ee7be401239f"
)
PACKAGE_EXECUTION_COUNT = 1
DIRECT_EXECUTION_COUNT = 1
ROW_SELECTION = "weak_weak_tau_1e_minus_6_confirmation_only_v1"
ACTIVATION_EXECUTION_IDS = (
    "phase3_on_plateau_r50__weak_weak__nph3__ra_singleton_plateau",
)
RUNTIME_RELATIVE = (
    "chtc/paper_i_ra_adapt_repair_20260727/"
    f"{PACKAGE_ID}_runtime_confirmation_20260806_v2"
)
SWEEP_VALIDATION_RECEIPT_FILE_SHA256 = (
    "3f9f6c7be5845333865ed2852e3271749db91e069d3789b91c617919a572600e"
)
SWEEP_VALIDATION_RECEIPT_SHA256 = (
    "dedf77567e2f6ce7d34f5c01da1cadb15c425cf79c69b9a001db773c7e4462f1"
)
ANCHOR_COMPARISON_FILE_SHA256 = (
    "cc6be520ada557d9d61c27e86df919abfca3cb43e89c2770e3cfdf4b76f7a082"
)
ANCHOR_COMPARISON_SHA256 = (
    "b199074e6117aed3f9979c1724face5e3aa893cf865b8842d761fb16612d4309"
)
QUEUE_VARIABLES = (
    "execution_id",
    "job_path",
...
KNOWN_TRANSFER_LIMITATIONS = (
    "in_progress_attempt_payloads_are_diagnostic_not_resume_authority_v2",
    "retry_attempts_reuse_the_cluster_proc_fetched_archive_destination_v1",
)
...
def package_relative() -> PurePosixPath:
    return (
        PurePosixPath("chtc/paper_i_ra_adapt_repair_20260727")
        / "paper_i_ra_adapt_singleton_phase3_on_plateau_threshold_sweep_"
        "weak_weak_r50_20260804_v1_local"
        / "threshold_1em6"
    )
...
def _load_package_contract(package_dir: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "phase3_on_plateau_r50_sealed_package_contract", path
    )
...
def _run_package_validator(package_dir: Path) -> dict[str, Any]:
    receipt_path = package_dir.parent / "validation_receipt_v2.json"
    if sha256_file(receipt_path) != SWEEP_VALIDATION_RECEIPT_FILE_SHA256:
        raise ActivationContractError("Sweep validation receipt bytes drifted.")
    result = load_json(receipt_path, label="sweep validation receipt")
    verify_self_digest(result, label="sweep validation receipt")
    rows = result.get("rows") if isinstance(result, dict) else None
    matching = [
        row
        for row in rows or []
        if isinstance(row, Mapping) and row.get("package") == "threshold_1em6"
    ]
    anchor_path = package_dir.parent / "anchor_comparison.json"
    if sha256_file(anchor_path) != ANCHOR_COMPARISON_FILE_SHA256:
        raise ActivationContractError("CHTC anchor comparison bytes drifted.")
    anchor = load_json(anchor_path, label="CHTC anchor comparison")
    verify_self_digest(anchor, label="CHTC anchor comparison")
    if (
        not isinstance(result, dict)
        or result.get("status") != "passed"
        or result.get("sha256") != SWEEP_VALIDATION_RECEIPT_SHA256
        or len(matching) != 1
        or matching[0].get("status") != "passed"
        or matching[0].get("threshold") != 1e-6
        or matching[0].get("package_manifest_sha256")
        != "294cf5d56799a614d38c6bd2be742de7b3cb42bf16812e4d132392e9c7f3a6c4"
        or anchor.get("status") != "passed"
        or anchor.get("sha256") != ANCHOR_COMPARISON_SHA256
    ...
    return {
        "status": "passed",
        "sweep_validation_sha256": result["sha256"],
...
def validate_sealed_package(repo_root: Path) -> dict[str, Any]:
    ...
    package_ids = list(contract.expected_execution_ids())
    if (
        ...
        or manifest.get("row_count") != PACKAGE_EXECUTION_COUNT
        or manifest.get("execution_ids") != package_ids
        or manifest.get("source_horizon") != 50
        or manifest.get("target_horizon") != contract.TARGET_HORIZON
        or manifest.get("threshold") != 1e-6
        or manifest.get("fanout_execution_blocked_pending_chtc_anchor") is not True
        ...
        or plan.get("execution_ids") != package_ids
        or plan.get("row_count") != PACKAGE_EXECUTION_COUNT
        or plan.get("run_class") != "diagnostic"
        or plan.get("threshold") != 1e-6
        or plan.get("fanout_execution_blocked_pending_chtc_anchor") is not True
        or plan.get("execution_authorized") is not False
        or plan.get("submission_authorized") is not False
        or plan.get("submitted") is not False
        or audit.get("status") != "passed"
        or audit.get("row_value") != 1e-6
        or audit.get("resolved_threshold") != 1e-6
    ...
    if (
        ...
        or len(manifest_jobs) != PACKAGE_EXECUTION_COUNT
    ):
        raise ActivationContractError("Package job closure drifted.")
    if len(package_ids) != len(manifest_jobs):
        raise ActivationContractError("Package job cardinality drifted.")
    jobs: list[dict[str, Any]] = []
    for index, (execution_id, row) in enumerate(
        zip(package_ids, manifest_jobs)
    ):
    ...
    jobs_by_id = {row["execution_id"]: row for row in jobs}
    if (
        len(ACTIVATION_EXECUTION_IDS) != DIRECT_EXECUTION_COUNT
        or len(set(ACTIVATION_EXECUTION_IDS)) != DIRECT_EXECUTION_COUNT
        or any(
            execution_id not in jobs_by_id
            for execution_id in ACTIVATION_EXECUTION_IDS
        )
    ):
        raise ActivationContractError("Activation row selection drifted.")
    jobs = [jobs_by_id[execution_id] for execution_id in ACTIVATION_EXECUTION_IDS]
    return {
        "contract": contract,
        "manifest": manifest,
...
def validate_submit_text(text: str) -> None:
    ...
    if re.search(r"(?im)^\s*\+?HolsteinLifecycleMode\s*=", text):
        raise ActivationContractError(
            "Plain ordinary activation must not claim a reserved lifecycle mode."
        )
...
def authorization_payload(
    *,
    ...
    return {
        ...
        "authorization_source": "explicit_user_request_2026-08-05",
...
def _sealed_package_binding(package: Mapping[str, Any], repo_root: Path) -> dict[str, Any]:
    return {
        ...
        "source_anchor_comparison": json_binding(
            package["anchor_path"], relative_to=repo_root
        ),
...
def validate_activation(repo_root: str | Path) -> dict[str, Any]:
    ...
    if expected_ids != list(ACTIVATION_EXECUTION_IDS):
        raise ActivationContractError("Selected execution ids drifted.")
    ...
    if len(queue_rows) != len(package["jobs"]):
        raise ActivationContractError("Queue/job cardinality drifted.")
    expected_executions: list[dict[str, Any]] = []
    for index, (queue_row, job_row) in enumerate(
        zip(queue_rows, package["jobs"])
    ):
        job = job_row["payload"]
    ...
    fixed_manifest = {
        ...
        "package_execution_count": PACKAGE_EXECUTION_COUNT,
        "direct_execution_count": DIRECT_EXECUTION_COUNT,
        "activation_execution_ids": list(ACTIVATION_EXECUTION_IDS),
        "row_selection": ROW_SELECTION,
        "queue_variables": list(QUEUE_VARIABLES),
        "activation_control_plane_sha256": control_sha,
        "operational_mode": "ordinary_unheld_worker_durable_v2",
        "execution_authorized": True,
        "submission_authorized": True,
        "submission_state": "authorized_pending_remote_preflight",
        "remote_stage": False,
        ...
        "known_transfer_limitations": list(KNOWN_TRANSFER_LIMITATIONS),
    }
