#!/usr/bin/env python3
"""Read-only validation for the inert 48-cell factorial package."""
...
def _verify_binding(
    ...
    *,
    ...
    if (
        ...
        and payload.get("sha256") != binding.get("canonical_sha256")
    ):
...
def _validate_archive(manifest: Mapping[str, Any]) -> None:
    ...
    if observed != set(declared):
        raise PackageContractError("Source archive member closure failed.")
...
def main() -> int:
    ...
    manifest = load_json(
        PACKAGE_DIR / PACKAGE_MANIFEST_NAME, label="package manifest"
    )
    verify_self_digest(manifest, label="package manifest")
    smoke = load_json(
        PACKAGE_DIR / SMOKE_RECEIPT_NAME, label="two-round smoke receipt"
    )
    validate_smoke_receipt(smoke)
    plan = load_json(
        PACKAGE_DIR / EXECUTION_PLAN_NAME, label="execution plan"
    )
    ...
    if (
        ...
        != DIRECT_EXECUTION_COUNT
        or manifest.get("factorial_arm_count") != 4
    ...
    if (
        ...
        or manifest["factorial_materialization_receipt"]
        != authority["final_binding"]
        or manifest["factor_source_lock_delta_receipt"]
        != authority["factor_delta_binding"]
        ...
        raise PackageContractError("Package provenance binding drifted.")
    ...
    for row, binding in zip(expected_rows, jobs):
        path = _verify_binding(
            binding, base=PACKAGE_DIR, label=f"{row['execution_id']} job"
        )
        ...
        if (
            ...
            or any(job.get(key) != value for key, value in row.items())
            or job.get("protocol")
            != authority["protocol_bindings"][row["execution_id"]]
    ...
    if (
        ...
        or [line.split("\t")[0] for line in queue_rows] != expected_ids
    ...
    control_names = [row["path"] for row in manifest["control_plane"]]
    if control_names != list(CONTROL_FILES):
        raise PackageContractError("Control-plane allowlist drifted.")
    ...
    if any(path.is_symlink() for path in PACKAGE_DIR.rglob("*")):
        raise PackageContractError("Package contains a forbidden symlink.")
        ...
        canonical_json_bytes(
            {
                ...
                "source_archive_sha256": manifest["source_archive"][
                    "sha256"
                ],
