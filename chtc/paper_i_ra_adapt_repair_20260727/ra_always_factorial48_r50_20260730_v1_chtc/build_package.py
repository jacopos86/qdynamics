#!/usr/bin/env python3
"""Build the inert source-locked 48-cell factorial transfer package."""
...
def _source_members(
    *,
    ...
    if not isinstance(global_sources, Mapping):
        raise PackageContractError(
            "Factorial source lock has no global sources."
        )
...
def main() -> int:
    ...
    if (PACKAGE_DIR / "authority").exists():
        raise PackageContractError(
            "The inert factorial package must not contain authority."
        )
    for name in (
        SOURCE_ARCHIVE_NAME,
        SOURCE_ARCHIVE_MANIFEST_NAME,
    ...
    for row in rows:
        ...
        _write_json(jobs_dir / f"{row['execution_id']}.json", job)
    ...
    manifest = digested(
        {
            ...
            "factorial_materialization_receipt": authority[
                "final_binding"
            ],
            "factor_source_lock_delta_receipt": authority[
                "factor_delta_binding"
            ],
            "bundle_bindings": authority["bundle_bindings"],
            "cross_arm_equality_audit": authority["equality_audit"],
