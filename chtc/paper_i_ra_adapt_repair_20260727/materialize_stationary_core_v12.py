...
seam. It preserves v1-v11, reuses all 48 selected v11 scientific source locks
...
protocols, and publishes v12 with no-replace semantics. None of the CHTC
...
from chtc.paper_i_ra_adapt_repair_20260727 import (
    materialize_stationary_core_v11 as v11,
)
...
MATERIALIZATIONS_ROOT = v11.MATERIALIZATIONS_ROOT
support = v11.support
V11_ROOT = v11.V11_ROOT
V12_ROOT = (
    MATERIALIZATIONS_ROOT / "ra_adapt_stationary_late_core_v12"
)
V11_SOURCE_LOCKS_INPUT = (
    V11_ROOT / "source_materialization" / "source_locks_input.json"
)
V11_PROBLEM_BASELINES = (
    V11_ROOT / "source_materialization" / "problem_baselines.json"
)
V11_FINAL_RECEIPT = V11_ROOT / "final_publication_receipt.json"
APPEND_REGISTRY = (
    REPO_ROOT
...
ICM_RECEIPT = (
    ...
    "materialize-stationary-core-v12.json"
)
FINAL_RECEIPT_NAME = "final_publication_receipt.json"

EXPECTED_V11_FILE_COUNT = 111
EXPECTED_V11_TOTAL_SIZE_BYTES = 4_239_095
EXPECTED_V11_RELATIVE_TREE_SHA256 = (
    "52dbd811116d12970c210517369e662c8290fd916de0e9e68c9b68b1de77f0b3"
)
EXPECTED_V11_FINAL_RECEIPT_FILE_SHA256 = (
    "5ad78e90153738cfd6a01fb17f68a4a44fa3cdfb3f236b45e125a8beb1476870"
)
EXPECTED_V11_FINAL_RECEIPT_CANONICAL_SHA256 = (
    "33f1f8a690b800f4c077382340749da466ee01e26e999aa9c3204b9b2a6330f5"
)
EXPECTED_V11_SOURCE_LOCKS_FILE_SHA256 = (
    "e31a6a0814cdee2d1b4bce7b7fefd18612e0f5d93fe5010f7e48a4e6679acccc"
)
EXPECTED_V11_PROBLEM_BASELINES_FILE_SHA256 = (
...
V11_SOURCE_LOCKS_PATH = V11_SOURCE_LOCKS_INPUT.relative_to(REPO_ROOT).as_posix()
...
MaterializationAuditError = v11.MaterializationAuditError
...
def _load_mapping(path: Path, *, label: str) -> dict[str, Any]:
    return v11._load_mapping(path, label=label)


def _load_digested(path: Path, *, label: str) -> dict[str, Any]:
    return v11._load_digested(path, label=label)
...
def _write_receipt(path: Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    return v11._write_receipt(path, payload)
...
def _historical_snapshots() -> dict[str, dict[str, Any]]:
    snapshots = v11._historical_snapshots()
    snapshots["v11"] = support._snapshot_roots((V11_ROOT,))
    return snapshots
...
def _assert_historical_anchors(
    ...
    v11._assert_historical_anchors(snapshots)
    observed = snapshots.get("v11")
    if not isinstance(observed, Mapping):
        raise MaterializationAuditError("Missing immutable v11 snapshot.")
    relative = support._snapshot_roots((V11_ROOT,), relative_to=V11_ROOT)
    _assert_equal(
        int(relative["file_count"]),
        EXPECTED_V11_FILE_COUNT,
        label="v11 immutable file count",
    )
    _assert_equal(
        int(relative["total_size_bytes"]),
        EXPECTED_V11_TOTAL_SIZE_BYTES,
        label="v11 immutable total bytes",
    )
    _assert_equal(
        relative["tree_sha256"],
        EXPECTED_V11_RELATIVE_TREE_SHA256,
        label="v11 immutable relative tree SHA-256",
    )
    _assert_equal(
        support._hash_file(V11_FINAL_RECEIPT),
        EXPECTED_V11_FINAL_RECEIPT_FILE_SHA256,
        label="v11 final receipt file SHA-256",
    )
    final = _load_digested(
        V11_FINAL_RECEIPT, label="v11 final materialization receipt"
    )
    _assert_equal(
        final["sha256"],
        EXPECTED_V11_FINAL_RECEIPT_CANONICAL_SHA256,
        label="v11 final receipt canonical SHA-256",
    )
...
def _ra_singleton_locks(
    *,
    ...
    for regime_id, nph in CLAIM_FACING_REGIME_CUTOFF_PAIRS:
        ...
        archive_authority = (
            {
                "kind": "predecessor_v11_archive_lock_v1",
                "predecessor_source_locks_path": V11_SOURCE_LOCKS_PATH,
...
def _build_source_locks(
    *,
    v11_source_locks: Mapping[str, Any],
    registry: Mapping[str, Any],
    baselines: Mapping[str, Any],
) -> tuple[
    dict[str, Any],
    ...
    _assert_equal(
        v11_source_locks.get("schema"),
        SOURCE_LOCK_SCHEMA,
        label="v11 source-lock schema",
    )
    raw_v11_cells = v11_source_locks.get("cell_locks")
    if not isinstance(raw_v11_cells, Mapping):
        raise MaterializationAuditError(
            "v11 source locks have no cell mapping."
        )
    required_ids = {
    ...
    _assert_equal(
        set(raw_v11_cells),
        required_ids,
        label="v11 stationary-core source-lock id set",
    )
    global_sources = v11_source_locks.get("global_sources")
    if not isinstance(global_sources, Mapping) or not global_sources:
        raise MaterializationAuditError(
            "v11 source locks have no global source mapping."
        )
    source_locks = copy.deepcopy(dict(v11_source_locks))
    macro_locks = {
        lock_id: copy.deepcopy(raw_v11_cells[lock_id])
        for lock_id in sorted(raw_v11_cells)
        if str(raw_v11_cells[lock_id]["route_id"]) in MACRO_ROUTE_IDS
    }
    singleton_locks = {
        lock_id: copy.deepcopy(raw_v11_cells[lock_id])
        for lock_id in sorted(raw_v11_cells)
        if str(raw_v11_cells[lock_id]["route_id"])
        in SINGLETON_CORE_ROUTE_IDS
    }
    _assert_equal(
        (len(macro_locks), len(singleton_locks)),
        (24, 24),
        label="v11 macro/singleton source-lock counts",
    )
    macro_receipt = _digested(
        {
            ...
            "objects_equal_to_predecessor": all(
                macro_locks[lock_id] == raw_v11_cells[lock_id]
    ...
    singleton_receipt = _digested(
        {
            ...
            "status": "passed",
            "derivation_mode": (
                "predecessor_v11_exact_source_lock_reuse_v1"
            ),
            "predecessor_source_locks": {
                "path": V11_SOURCE_LOCKS_PATH,
                "file_sha256": (
            ...
            "problem_baselines": {
                "path": V11_PROBLEM_BASELINES.relative_to(
                    REPO_ROOT
                ).as_posix(),
                "file_sha256": (
                    EXPECTED_V11_PROBLEM_BASELINES_FILE_SHA256
            ...
            "objects_equal_to_predecessor": all(
                singleton_locks[lock_id] == raw_v11_cells[lock_id]
    ...
    ra_singleton_rows = [
        {
            ...
            "predecessor_v11_object_reused_without_edit": True,
            "scientific_result_anchor_claimed": False,
        }
    ...
    semantic_receipt = _digested(
        {
            ...
            "status": "passed",
            "source_role": (
                "predecessor_v11_source_authority_reuse_no_result_"
                "adoption_v1"
            ),
            "cluster_id": 9381198,
    ...
    return _digested(
        {
            ...
            "predecessor_v11": {
                ...
                "file_count": EXPECTED_V11_FILE_COUNT,
                "total_size_bytes": (
                    EXPECTED_V11_TOTAL_SIZE_BYTES
                ),
...
def _complete_missing_icm_receipt() -> int:
    """Safely complete the sole receipt outside an already-published v12."""

    if not V12_ROOT.is_dir() or ICM_RECEIPT.exists():
        raise MaterializationAuditError(
            "The missing-ICM recovery path requires published v12 and no "
            "existing ICM receipt."
        )
    support._darwin_renameatx_np()
    historical = _historical_snapshots()
    _assert_historical_anchors(historical)
    final_path = V12_ROOT / FINAL_RECEIPT_NAME
    ...
    source_locks = _load_digested(
        V12_ROOT / CORE_BUNDLE_ID / "source_locks.json",
    ...
    loader_receipt = _load_digested(
        V12_ROOT / "cross_file_loader_validation.json",
    ...
    persisted = _load_digested(
        ICM_RECEIPT,
        label="recovered stationary-core v12 ICM receipt",
    )
    _assert_equal(
        persisted,
        icm_payload,
        label="recovered stationary-core v12 ICM receipt payload",
    )
    print(
        json.dumps(
            {
                "destination": V12_ROOT.relative_to(
                    REPO_ROOT
                ).as_posix(),
                "status": "passed",
...
def main() -> int:
    if V12_ROOT.exists():
        ...
        raise FileExistsError(
            f"Refusing to overwrite immutable v12: {V12_ROOT}"
        )
    ...
    _assert_equal(
        support._hash_file(V11_SOURCE_LOCKS_INPUT),
        EXPECTED_V11_SOURCE_LOCKS_FILE_SHA256,
        label="v11 source-lock input file SHA-256",
    )
    _assert_equal(
        support._hash_file(V11_PROBLEM_BASELINES),
        EXPECTED_V11_PROBLEM_BASELINES_FILE_SHA256,
        label="v11 problem-baselines file SHA-256",
    )
    _assert_equal(
    ...
    v11_source_locks = _load_mapping(
        V11_SOURCE_LOCKS_INPUT, label="v11 source-lock input"
    )
    baselines = _load_mapping(
        V11_PROBLEM_BASELINES, label="v11 problem baselines"
    )
    ...
    staging_root = Path(
        tempfile.mkdtemp(
            prefix=".ra_adapt_stationary_late_core_v12.staging.",
    ...
    support._write_bytes_atomic_no_replace(
        ...
        V11_PROBLEM_BASELINES.read_bytes(),
    )
    _assert_equal(
        support._hash_file(source_root / "problem_baselines.json"),
        EXPECTED_V11_PROBLEM_BASELINES_FILE_SHA256,
        label="copied v12 problem-baselines SHA-256",
    )
    ...
    (
        ...
        v11_source_locks=v11_source_locks,
        registry=registry,
        baselines=baselines,
    )
    _write_plain_json(
    ...
    preflight_receipt = _write_receipt(
        ...
        {
            "schema": (
                "paper_i_ra_adapt_stationary_core_v12_preflight_v1"
            ),
            "status": "passed",
    ...
    _assert_historical_unchanged(
        ...
        label="Staged stationary-core v12 materialization",
    )
    ...
    staged_receipt = _write_receipt(
        ...
        {
            "schema": (
                "paper_i_ra_adapt_stationary_core_v12_staged_"
                "materialization_receipt_v1"
            ),
            "status": "passed",
            "campaign_id": CORE_CAMPAIGN_ID,
    ...
    final_receipt = _write_receipt(
        staging_root / FINAL_RECEIPT_NAME,
        {
            ...
            "materialization_id": "ra_adapt_stationary_late_core_v12",
            "campaign_id": CORE_CAMPAIGN_ID,
            ...
            "atomic_publish": {
                ...
                "post_publish_v12_write_count": 0,
            },
            "tree": {
                "scope": (
                    "materialization_tree_excluding_"
    ...
    _assert_historical_unchanged(
        ...
        label="Complete staged stationary-core v12 materialization",
    )
    ...
    support._atomic_rename_no_replace(staging_root, V12_ROOT)

    published_final = _load_digested(
        V12_ROOT / FINAL_RECEIPT_NAME,
    ...
    _assert_historical_unchanged(
        historical_before,
        historical_after,
        label="Published stationary-core v12 materialization",
    )
    ...
    _assert_equal(
        ...
        label="staged-to-published complete v12 tree",
    )
    ...
    persisted_icm = _load_digested(
        ICM_RECEIPT,
        label="stationary-core v12 ICM stage receipt",
    )
    _assert_equal(
        persisted_icm,
        icm_payload,
        label="stationary-core v12 ICM receipt payload",
    )
    print(
        json.dumps(
            {
