#!/usr/bin/env python3
"""Execute one global-singleton cell with external exact authorization."""
    ...
    *,
    ...
    if (
        ...
        raise PackageContractError(
            "External authorization does not bind this exact job."
        )
    return authority
...
def _extract_source_archive(destination: Path) -> None:
    ...
    if observed != set(declared):
        raise PackageContractError(
            "Extracted source member closure failed."
        )
...
def _activate_source_root(source_root: Path) -> None:
    ...
    try:
        ...
        raise PackageContractError(
            "Runtime escaped the source-locked archive."
        ) from exc
...
def main() -> int:
    ...
    if (
        ...
        raise PackageContractError(
            "Job is not the inert global-singleton contract."
        )
    ...
    with tempfile.TemporaryDirectory(
        prefix="paper-i-global-singleton-source."
    ) as temporary_name:
