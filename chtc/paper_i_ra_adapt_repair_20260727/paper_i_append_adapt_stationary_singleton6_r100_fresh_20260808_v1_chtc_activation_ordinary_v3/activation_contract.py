#!/usr/bin/env python3
"""Fail-closed contract for the Append-ADAPT r100 ordinary activation."""
...
ACTIVATION_ID = f"{PACKAGE_ID}_activation_ordinary_v3"
ACTIVATION_SCHEMA = (
    "paper_i_append_adapt_stationary_singleton6_r100_ordinary_activation_v3"
)
AUTHORIZATION_SCHEMA = (
    "paper_i_append_adapt_stationary_core_r100_execution_authorization_v1"
)
BATCH_NAME = (
    "paper-i-append-adapt-stationary-singleton6-r100-fresh-"
    "20260809-v3"
)
...
PACKAGE_MANIFEST_CANONICAL_SHA256 = (
    "7388c169c2158f44bb6bf93313b938826a5b7e778cbfe4ccd2416e732f42d274"
)
PACKAGE_MANIFEST_FILE_SHA256 = (
    "0b08f27a22029805abcc8ecd4f9761b30652e8d714271b8c5a6b9be67a7535cd"
)
EXECUTION_PLAN_CANONICAL_SHA256 = (
    "d3468c3cc250cb08282c01e68221d566cb42162849fb3a7c10b491a92bf2d8ba"
)
EXECUTION_PLAN_FILE_SHA256 = (
    "78e344f3806f2d0377b6b4a486e3739d25f3f5f986a0fca33dc0d12411035a5d"
)
HORIZON_AUDIT_CANONICAL_SHA256 = (
    "f5ed8e0c56df751f5abb29e855c904d01c03a53d78ba9f94eb085e859b3a933b"
)
SOURCE_ARCHIVE_SHA256 = (
    "1f949b0cc8b61dca63911832e8dc8bb32614174755ac476827956bb0812accee"
)
DIRECT_EXECUTION_COUNT = 6
APPEND_R100_NPH7_MEMORY_FLOOR_MB = 49_152
QUEUE_VARIABLES = (
    "execution_id",
    "job_path",
    "job_file_sha256",
...
def _load_package_contract(package_dir: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "append_r100_sealed_package_contract", path
    )
...
def validate_submit_text(text: str) -> None:
    required = (
        ...
        "hold = False",
        "transfer_output_files = transfer/$(execution_id)__$(ClusterId)__"
        "$(ProcId).tar.gz",
        "transfer_output_remaps = \"transfer/$(execution_id)__$(ClusterId)__"
        "$(ProcId).tar.gz=/staging/jsstrobel/"
    ...
    lowered = text.lower()
    if re.search(r"(?im)^\s*output_destination\s*=", text):
    ...
    if "max_materialize" in lowered or "max_idle" in lowered:
        raise ActivationContractError(
            "Ordinary activation must not be a factory."
        )
...
def quota_release_contract() -> dict[str, Any]:
    return {
        "initial_state": "all_rows_submitted_unheld",
        "automatic_release": False,
        "release_scope": "not_applicable",
        "output_destination": "chtc_staging_posix_explicit_archive_remap",
    }
...
def validate_activation(repo_root: str | Path) -> dict[str, Any]:
    ...
    for index, (queue_row, planned) in enumerate(
        ...
        expected_memory_mb = int(resources["request_memory_mb"])
        if "__nph7__" in execution_id:
            expected_memory_mb = max(
                expected_memory_mb, APPEND_R100_NPH7_MEMORY_FLOOR_MB
            )
        expected = [
            execution_id,
        ...
        expected_executions.append(
            {
                ...
                "resources": {
                    ...
                    "request_memory_mb": expected_memory_mb,
