#!/usr/bin/env python3
"""Materialize the authorized source-locked five-cell CHTC package."""
...
def main() -> int:
    ...
    source_manifest = digested(
        {
            "schema": "paper_i_ra_allphase_adaptive_append_position_all6_source_archive_v1",
    ...
    if len(jobs) != 6 or len({row["execution_id"] for row in jobs}) != 6:
        raise RuntimeError("matrix is not exactly six unique cells")
    ...
    plan = digested(
        {
            ...
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "matrix_shape": {"arms": 1, "regimes": 6, "cells": 6},
            "arms": list(ARMS),
