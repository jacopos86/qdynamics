#!/usr/bin/env python3
"""Build a deterministic authenticated worker-attempt archive."""
...
ATTEMPT_SCHEMA = (
    "paper_i_ra_adapt_cumulative_relative_ss_plateau_r70_"
    "worker_attempt_v1"
)


class AttemptArchiveError(ValueError):
    """Raised when an attempt cannot be archived safely."""
                ...
                with tarfile.open(
                    ...
                    if len(worker_files) != len(bindings):
                        raise AttemptArchiveError(
                            "Worker-output binding count drifted."
                        )
                    # Keep compatibility with the Python 3.9 login-node
                    # environment; the explicit count check is equivalent to
                    # ``zip(..., strict=True)`` here.
