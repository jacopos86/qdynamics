#!/usr/bin/env python3
"""Execute one authorized fresh 0→100 conventional Append-ADAPT row."""
...
def _write_artifacts(
    *,
    ...
    if (
        ...
        raise PackageContractError(
            "Fresh round-100 result/summary horizon closure failed."
        )
    checkpoint = digested(
        {
            "schema": (
...
def run_cell(
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"paper_i_append_r100_{execution_id}_"
    ) as raw:
        ...
        if (
            ...
            raise PackageContractError(
                "Runtime round-100 protocol derivation drifted."
            )
