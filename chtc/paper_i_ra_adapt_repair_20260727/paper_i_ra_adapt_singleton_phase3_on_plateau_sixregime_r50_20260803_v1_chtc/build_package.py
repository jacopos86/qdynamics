#!/usr/bin/env python3
"""Materialize the inert six-regime Phase-III-on-plateau RA package."""
...
def _load_source_package(repo_root: Path) -> dict[str, Any]:
    ...
    manifest = _load_bound_json(
        ...
        label="sealed v5 historical-mean predecessor package",
    )
...
def _apply_exact_source_patch(source_root: Path) -> list[dict[str, Any]]:
    ...
    if completed.returncode != 0:
        raise PackageContractError(
            "Exact v5-to-target source patch failed: "
...
def _target_cell(source_cell: Any, *, horizon: int) -> Any:
    return replace(
        ...
        stage="phase3_on_insertion_plateau_r50",
...
def _validate_source_to_target(
    *,
    ...
    if (
        ...
        != "phase2_winner_only_refit_geometry_v1"
        or invariants["phase3_activation_independent_latch"] is not False
        or invariants["phase3_activation_hysteresis_active"] is not False
    ):
        raise PackageContractError("Target Phase-III-on-plateau protocol drifted.")
    return {
...
def build() -> dict[str, Any]:
    ...
    source_locks = _load_bound_json(
        repo_root / TARGET_SOURCE_LOCKS_RELATIVE,
        file_sha256=TARGET_SOURCE_LOCKS_FILE_SHA256,
        canonical_sha256=TARGET_SOURCE_LOCKS_CANONICAL_SHA256,
        label="completed-route source-lock snapshot",
    )
    ...
    if (
        ...
        raise PackageContractError(
            "Patched implementation inventory missed the completed-route lock."
        )

    anchor_path = repo_root / TARGET_COMPLETED_RESULT_RELATIVE
    if (
        not anchor_path.is_file()
        or anchor_path.is_symlink()
        or sha256_file(anchor_path) != TARGET_COMPLETED_RESULT_FILE_SHA256
    ):
        raise PackageContractError("Completed route anchor exact bytes drifted.")
    anchor_payload = json.loads(anchor_path.read_text(encoding="utf-8"))
    anchor_protocol = anchor_payload.get("protocol", {})
    if (
        not isinstance(anchor_protocol, Mapping)
        or anchor_protocol.get("algorithm_id") != ALGORITHM_ID
        or anchor_protocol.get("route_contract", {}).get("sha256")
        != ROUTE_CONTRACT_SHA256
        or anchor_protocol.get("source_locks", {}).get(
            "implementation_source_inventory_sha256"
        )
        != TARGET_IMPLEMENTATION_INVENTORY_SHA256
    ):
        raise PackageContractError("Completed route anchor identity drifted.")
    ...
    if member_paths - set(predecessor_by_path) != {
        "pipelines/static_adapt/ra_adapt/h2o_application.py"
    }:
        raise PackageContractError(
            "Target source inventory changed unexpected archive membership."
        )
    members: list[dict[str, Any]] = []
    ...
    for regime_id, nph in REGIME_ROWS:
        ...
        if source_id not in source_rows:
            raise PackageContractError(
                f"Missing v5 historical-mean source cell: {source_id}"
            )
        ...
        if (
            ...
            raise PackageContractError(
                f"Historical-average v5 source cell drifted: {source_id}"
            )
    ...
    source_lock_audit = digested(
        {
            ...
            "target_horizon": TARGET_HORIZON,
            "scientific_changes": [
                "phase3_competitive_population_activates_on_same_round_insertion_plateau_v1",
            ],
            ...
            "completed_route_anchor": {
                "path": TARGET_COMPLETED_RESULT_RELATIVE.as_posix(),
                "sha256": TARGET_COMPLETED_RESULT_FILE_SHA256,
                "route_contract_sha256": ROUTE_CONTRACT_SHA256,
                "implementation_source_inventory_sha256": (
                    TARGET_IMPLEMENTATION_INVENTORY_SHA256
                ),
    ...
    execution_plan = digested(
        {
            "schema": (
                ...
                "sixregime_r50_execution_plan_v1"
            ),
            "package_id": PACKAGE_ID,
            "campaign_id": CAMPAIGN_ID,
            "run_class": "candidate",
