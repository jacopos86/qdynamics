...
def _load_closed_job(
    ...
    if (
        ...
        raise PackageContractError("Inert three-row package manifest drifted.")
    ...
    if len(matching) != 1:
        raise PackageContractError("Requested job is outside the three-row closure.")
...
def preflight(job_path: Path) -> dict[str, Any]:
    ...
    return digested(
        {
            "schema": "paper_i_ra_adapt_historical_mean_global_singleton_plateau3_nph7_r50_worker_preflight_v2",
            "status": "passed",
...
def smoke_one_round(job_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return digested(
            {
                "schema": "paper_i_ra_adapt_historical_mean_global_singleton_plateau3_nph7_r50_worker_smoke_v2",
...
def _publish_staging_directory(staging: Path, output_dir: Path) -> None:
    """Publish a temporary result directory across filesystems atomically."""
    ...
    *, job_path: Path, authorization_path: Path, output_dir: Path, receipt_path: Path
    ...
    try:
        ...
        execution_manifest = digested(
            {
                "schema": "paper_i_ra_adapt_historical_mean_global_singleton_plateau3_nph7_r50_execution_manifest_v2",
                "status": "passed",
        ...
        receipt = digested(
            {
                "schema": "paper_i_ra_adapt_historical_mean_global_singleton_plateau3_nph7_r50_worker_receipt_v2",
                "status": "passed",
                "package_id": PACKAGE_ID,
                "campaign_id": CAMPAIGN_ID,
                "execution_id": job["execution_id"],
