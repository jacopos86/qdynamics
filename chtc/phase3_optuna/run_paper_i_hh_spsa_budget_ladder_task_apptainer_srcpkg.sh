...
if [[ ! -d src ]]; then
  SRC_TARBALL="${STATIC_ADAPT_SOURCE_TARBALL:-}"
  if [[ -z "$SRC_TARBALL" ]]; then
    SRC_TARBALL="$(ls chtc/phase3_optuna/input/*/src_sanitized.tar.gz 2>/dev/null | head -n 1 || true)"
  fi
  if [[ -z "$SRC_TARBALL" || ! -f "$SRC_TARBALL" ]]; then
    echo "Missing src/ and no src_sanitized.tar.gz transfer bundle was found." >&2
    exit 2
  fi
  tar -xzf "$SRC_TARBALL"
fi
...
"$APPTAINER_BIN" exec --cleanenv --bind "$ROOT:/work" "$IMAGE" \
  bash -lc 'cd /work && python -u -m chtc.phase3_optuna.run_paper_i_hh_spsa_budget_ladder_cell "$@"' \
