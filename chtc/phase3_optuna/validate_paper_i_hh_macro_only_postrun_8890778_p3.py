#!/usr/bin/env python3
"""Fail-closed validator for one fetched macro-only/physical-lanes archive."""
...
POST_RUN_RECOVERY_SCHEMA = "paper_i_hh_sr_macro_post_run_recovery_v1"
POST_RUN_RECOVERY_CLUSTER = "8890778"
POST_RUN_RECOVERY_FAILURE = (
    "ValueError: Active operator 0 execution terms disagree with parameterization."
)
POST_RUN_RECOVERY_REPORTER_SHA256 = (
    "91edb3bba6e20d7f84e331d8428fd422c7b5ae272b63fd3f99ebb4c9be5f2dce"
)
POST_RUN_RECOVERY_SOURCE_ARCHIVE_SHA256 = (
    "a3a80c964ba2925de1c14843c75dd2c9b8440f4e27f3f713341c96c95e40e2fc"
)
POST_RUN_RECOVERY_PARENT_SOURCE_ARCHIVE_SHA256 = (
    "3a5ed36ebdf260357aa86b3a5ab3c7d8372072329a8fec2e1043e90b6f7c34c7"
)
POST_RUN_RECOVERY_RUN_JOB_SHA256 = (
    "9bdedc80fa4fba4edecaecab053344e19fcc7f33d7cdd39913b66cc4382614ce"
)
POST_RUN_RECOVERY_EVIDENCE_VALIDATION_SHA256 = (
    "567273813b5c89c94a0f2b459c0c16762a95bd537db4df426799110e5f37c3e3"
)
POST_RUN_RECOVERY_ALLOWLIST = {
    "weak_weak": {
        "proc_id": 0,
        "archive_sha256": "a6e29aa540ac552f39080940fe38a18547b51fe8a878527b692da0a7583e988d",
        "execution_sha256": "cd0e16ba59655a40db2c5a8c256bec8afbf8d261f05d4a1902711431537a6c85",
        "job_manifest_sha256": "d1b9e91594944fedea6129268d51368b52f876fb78df7850192d3699c8ac83e1",
    },
    "intermediate_weak": {
        "proc_id": 1,
        "archive_sha256": "52f2018f6ce34563d3153d32d88329f582900579b3bf340ba236fd5270819953",
        "execution_sha256": "e4b9cbd979d34ef7fd19879337f2703366d8f3eef57e333e5b4f4bcc73a81489",
        "job_manifest_sha256": "602ef4fc139a3c20c83b9d45a539943364f826c4f26c6d2930968e5a0c1408f8",
    },
    "strong_weak_u8": {
        "proc_id": 2,
        "archive_sha256": "5aa6178f222aba2db59c6d69647a52605525eacead1da82214e372604d6f14d7",
        "execution_sha256": "50fc609726a460f808eca29642c9e1572f3993ef329ef663884db4df5fadd65f",
        "job_manifest_sha256": "6ac4e06975d5da40dd3360b52d5a691c07a48b9061fb8157f5a179a2395a5983",
    },
    "weak_strong": {
        "proc_id": 3,
        "archive_sha256": "be5aa72c0371b2d3a3bc694b1527917be0cc0ef0800f1cb898c3b1785decdecb",
        "execution_sha256": "b22fe93967bc924a59f1b40944253fdf5e7f4a09a4901798151c637de1c0ae9d",
        "job_manifest_sha256": "5bf9964db66e67c0622a9ffd26b934ec2d9e834a31371481d2764c9ceb808e61",
    },
}
...
def validate_post_run_recovery_receipt(
    *, output: Path, execution: dict[str, Any], normalized: dict[str, Any]
) -> dict[str, Any] | None:
    """Validate the one authorized reporting-only recovery lineage.

    The original CHTC execution record deliberately remains failed.  Recovery
    is accepted only when that exact post-science permutation-validator failure
    and every original science-artifact hash are preserved, and when the four
    newly generated reporting artifacts are independently hash locked.
    """

    receipt_path = output / "post_run_recovery_receipt.json"
    if not receipt_path.is_file():
        return None
    receipt = load(receipt_path)
    source = receipt.get("source", {})
    safeguards = receipt.get("safeguards", {})
    reporting_runtime = receipt.get("reporting_runtime", {})
    allowlist = POST_RUN_RECOVERY_ALLOWLIST.get(output.name)
    if (
        allowlist is None
        or
        receipt.get("schema") != POST_RUN_RECOVERY_SCHEMA
        or receipt.get("status") != "pass"
        or receipt.get("classification")
        != "post_run_reporting_only_permutation_repair"
        or receipt.get("regime_slug") != output.name
        or source.get("cluster_id") != POST_RUN_RECOVERY_CLUSTER
        or source.get("bundle_id") != BUNDLE_ID
        or source.get("profile_contract_sha256")
        != normalized.get("route_identity", {}).get("profile_contract_sha256")
        or source.get("original_execution_sha256") != sha256(output / "execution.json")
        or source.get("original_execution_status") != "failed"
        or int(source.get("original_execution_exit_code", -1)) != 70
        or source.get("original_validation_error") != POST_RUN_RECOVERY_FAILURE
        or execution.get("status") != "failed"
        or int(execution.get("exit_code", -1)) != 70
        or execution.get("validation_error") != POST_RUN_RECOVERY_FAILURE
        or reporting_runtime.get("fidelity_reporter_sha256")
        != POST_RUN_RECOVERY_REPORTER_SHA256
        or reporting_runtime.get("source_archive_sha256")
        != POST_RUN_RECOVERY_SOURCE_ARCHIVE_SHA256
        or reporting_runtime.get("parent_scientific_source_archive_sha256")
        != POST_RUN_RECOVERY_PARENT_SOURCE_ARCHIVE_SHA256
        or reporting_runtime.get("original_run_job_sha256")
        != POST_RUN_RECOVERY_RUN_JOB_SHA256
        or reporting_runtime.get("original_evidence_validation_sha256")
        != POST_RUN_RECOVERY_EVIDENCE_VALIDATION_SHA256
        or safeguards.get("scientific_execution_replayed") is not False
        or safeguards.get("controller_replayed") is not False
        or safeguards.get("optimizer_replayed") is not False
        or safeguards.get("scientific_settings_changed") is not False
        or safeguards.get("original_execution_json_modified") is not False
        or safeguards.get("estimator_query_delta") != 0
    ):
        raise ValueError("post-run recovery identity/safeguard drift")
    proc_id = int(source.get("proc_id", -1))
    expected_proc = int(allowlist["proc_id"])
    expected_archive_name = (
        f"{POST_RUN_RECOVERY_CLUSTER}.{proc_id}__{output.name}_transfer.tar.gz"
    )
    original_archive = source.get("original_transfer_archive", {})
    if (
        proc_id != expected_proc
        or original_archive.get("name") != expected_archive_name
        or original_archive.get("sha256") != allowlist["archive_sha256"]
        or int(original_archive.get("size_bytes", -1)) <= 0
        or original_archive.get("untouched") is not True
        or source.get("original_execution_sha256")
        != allowlist["execution_sha256"]
        or source.get("original_job_manifest", {}).get("sha256")
        != allowlist["job_manifest_sha256"]
        or safeguards.get("original_archive_untouched") is not True
    ):
        raise ValueError("post-run recovery original-transfer identity drift")
    original_job_manifest = BUNDLE / "jobs" / f"{output.name}.json"
    if (
        not original_job_manifest.is_file()
        or sha256(original_job_manifest) != allowlist["job_manifest_sha256"]
        or source.get("original_job_manifest", {}).get("path")
        != f"chtc/phase3_optuna/input/{BUNDLE_ID}/jobs/{output.name}.json"
        or int(source.get("original_job_manifest", {}).get("size_bytes", -1))
        != original_job_manifest.stat().st_size
        or sha256(BUNDLE / "run_job.py") != POST_RUN_RECOVERY_RUN_JOB_SHA256
        or sha256(BUNDLE / "evidence_validation.py")
        != POST_RUN_RECOVERY_EVIDENCE_VALIDATION_SHA256
    ):
        raise ValueError("post-run recovery control-source identity drift")
    original_records = receipt.get("original_science_artifacts", {})
    execution_artifacts = execution.get("artifacts", {})
    for key, relative in ORIGINAL_EXECUTION_ARTIFACTS.items():
        path = output / relative
        record = original_records.get(key, {})
        execution_record = execution_artifacts.get(key, {})
        digest = sha256(path)
        if (
            not path.is_file()
            or record.get("sha256") != digest
            or int(record.get("size_bytes", -1)) != path.stat().st_size
            or execution_record.get("exists") is not True
            or execution_record.get("sha256") != digest
        ):
            raise ValueError(f"post-run recovery changed original artifact: {key}")
    recovered_records = receipt.get("recovered_reporting_artifacts", {})
    for key, relative in RECOVERED_REPORTING_ARTIFACTS.items():
        path = output / relative
        record = recovered_records.get(key, {})
        execution_record = execution_artifacts.get(key, {})
        if (
            not path.is_file()
            or record.get("sha256") != sha256(path)
            or int(record.get("size_bytes", -1)) != path.stat().st_size
            or execution_record.get("exists") is not False
            or execution_record.get("sha256") is not None
        ):
            raise ValueError(f"post-run recovery artifact provenance drift: {key}")
    ledger_digest = sha256(output / "json/estimator_call_ledger.json")
    if (
        safeguards.get("estimator_ledger_sha256_before") != ledger_digest
        or safeguards.get("estimator_ledger_sha256_after") != ledger_digest
    ):
        raise ValueError("post-run recovery estimator-ledger closure failed")
    return receipt
...
def validate(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or source_revision.get("dirty_live_source_lock") is not True
    ...
    recovery = validate_post_run_recovery_receipt(
        output=output,
        execution=execution,
    ...
    if recovery is not None:
        recovery_repair = recovery.get("checkpoint_repair", {})
        if (
            recovery_repair.get("status") != "repaired_permutation_only"
            or recovery_repair.get("substantive_term_changes") is not False
            or recovery_repair.get("source_checkpoint_sha256")
            != terminal_checkpoint.get("checkpoint_sha256")
            or recovery_repair.get("repaired_checkpoint_sha256")
            != repaired_digest
            or recovery.get("reporting_runtime", {}).get(
                "fidelity_state_replay_source"
            )
            != "exact_signed_prefix_checkpoint_permutation_repaired_replay"
            or fidelity.get("state_replay_source")
            != recovery.get("reporting_runtime", {}).get(
                "fidelity_state_replay_source"
            )
        ):
            raise ValueError("post-run recovery permutation-only repair drift")
    return {
        "schema": "paper_i_hh_sr_macro_only_physical_lanes_fetched_validation_v1",
        "status": "pass",
...
def main() -> int:
    ...
    if args.path.is_file():
        with tempfile.TemporaryDirectory(prefix="sr_macro_physical_fetch_") as tmp:
            root = Path(tmp)
