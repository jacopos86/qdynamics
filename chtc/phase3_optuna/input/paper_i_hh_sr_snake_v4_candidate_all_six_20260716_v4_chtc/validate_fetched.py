...
DIGEST = "835841f4a2d9b348ce9af6df7364d85a5e4b0696b84696252e73cc8d2f3eb02d"
...
def validate(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or source_revision.get("profile_contract_sha256") != DIGEST
        ...
        raise ValueError("local v4 bundle source authority is incomplete")
    expected_overlays = {
        relative: {
            "sha256": digest,
    ...
    if (
        ...
        raise ValueError("local v4 compatibility-overlay source authority drift")
    for relative, expected_hash in NONSCIENTIFIC_ARCHIVE_OVERLAYS.items():
        ...
        if (
            ...
            raise ValueError(f"local v4 compatibility overlay drift: {relative}")
    ...
    for key, expected in {
        ...
        if normalized_contract.get("execution_settings", {}).get(key) != expected:
            raise ValueError(f"fetched v4 profile {key} drift")
        if normalized_contract.get("semantic_invariants", {}).get(key) != expected:
            raise ValueError(f"fetched v4 semantic invariant {key} drift")
        if phase12.get(key) != expected:
            raise ValueError(f"fetched v4 job Phase-I/II contract {key} drift")
    if normalized_contract.get("semantic_invariants", {}).get(
        "phase1_phase2_lambda_f_proxy_active"
    ) is not False:
        raise ValueError("fetched v4 lambda-F proxy invariant is active")
    artifact_paths = {
    ...
    return {
        "schema": "paper_i_hh_sr_snake_v4_fetched_validation_v3",
        "status": "pass",
