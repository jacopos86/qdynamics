...
def validate_ledger(
    ...
    *,
    estimator_ledger_class: Any | None = None,
) -> dict[str, Any]:
    ...
    strict_validation = strict_validate_ledger_payload(
        ledger,
        estimator_ledger_class=estimator_ledger_class,
    )
    ...
    return {
        ...
        "strict_source_locked_deserialization": strict_validation,
    }


def strict_validate_ledger_payload(
    ledger_payload: Mapping[str, Any],
    *,
    estimator_ledger_class: Any | None,
) -> dict[str, Any]:
    """Strictly reconstruct a ledger with its source-locked implementation.

    Runtime validation may omit ``estimator_ledger_class`` because the worker
    has not yet fetched its own transfer archive.  Post-fetch validation must
    supply the class loaded from the authenticated ``source_locked.tar.gz``;
    that path calls the production ``EstimatorCallLedger.from_payload`` rather
    than trusting aggregate counts or a merely nonempty fingerprint.
    """

    ledger = _mapping(ledger_payload, field="raw estimator ledger")
    fingerprint = str(ledger.get("ledger_fingerprint", ""))
    if len(fingerprint) != 64:
        raise ValueError("raw estimator ledger fingerprint is missing or malformed")
    if estimator_ledger_class is None:
        return {
            "performed": False,
            "status": "deferred_to_source_locked_fetched_validator",
            "ledger_fingerprint": fingerprint,
        }
    from_payload = getattr(estimator_ledger_class, "from_payload", None)
    if not callable(from_payload):
        raise TypeError("source-locked estimator ledger class lacks from_payload")
    restored = from_payload(ledger)
    to_payload = getattr(restored, "to_payload", None)
    if not callable(to_payload):
        raise TypeError("source-locked estimator ledger instance lacks to_payload")
    round_trip = to_payload()
    if round_trip != dict(ledger):
        raise ValueError("source-locked estimator ledger round-trip mismatch")
    return {
        "performed": True,
        "status": "strict_source_locked_deserialization_pass",
        "ledger_fingerprint": fingerprint,
    }
...
def validate_parent_evidence(
    *,
    ...
    if adapt.get("adapt_beam_enabled") is not True:
        raise ValueError("historical 3x2 beam execution was not enabled")
    if adapt.get("finite_angle_fallback") is not False:
    ...
    expected_beam = {
        ...
        "terminal_archive_policy_version": "legacy_parent_stop_archive_cap_v1",
