...
def _validate_compact_controller_history(
    history_raw: Any,
    *,
    expected_rounds: int,
    fallback_rounds: Sequence[int],
) -> dict[str, Any]:
    history = list(_sequence(history_raw, field="compact controller history"))
    if len(history) != expected_rounds:
        raise ValueError("compact controller history length drift")
    checkpoints: list[Mapping[str, Any]] = []
    previous_depth = 0
    previous_radius: float | None = None
    max_sector = 0.0
    ...
    prune_executed = 0
    prune_accepted = 0
    previous_prune_state: tuple[float, float, int] | None = None
    for outer_iteration, raw in enumerate(history, start=1):
        row = _mapping(raw, field=f"compact controller round {outer_iteration}")
        if int(row.get("depth", -1)) != outer_iteration:
            raise ValueError(f"round {outer_iteration}: compact depth drift")
        if str(row.get("selected_op", "")).startswith(
            ("guarded_singleton::", "projected_singleton::")
        ):
            raise ValueError("macro-only route admitted a generated child")
        records = list(
            _sequence(
                row.get("selected_records"),
                field=f"round {outer_iteration} compact selected records",
            )
        )
        if len(records) != 1:
            raise ValueError(f"round {outer_iteration}: compact non-singleton admission")
        accepted = _mapping(
            row.get("accepted_refit"),
            field=f"round {outer_iteration} compact accepted refit",
        )
        invocation = _mapping(
            accepted.get("accepted_refit_invocation"),
            field=f"round {outer_iteration} compact refit invocation",
        )
        config = _mapping(
            invocation.get("config"),
            field=f"round {outer_iteration} compact refit config",
        )
        pre_support = previous_depth + 1
        expected_indices = list(range(pre_support))
        accepted_indices = [
            int(value)
            for value in _sequence(
                invocation.get("accepted_refit_logical_indices"),
                field=f"round {outer_iteration} compact accepted indices",
            )
        ]
        selector_indices = [
            int(value)
            for value in _sequence(
                invocation.get("selector_active_logical_indices"),
                field=f"round {outer_iteration} compact selector indices",
            )
        ]
        if (
            config.get("scope") != "full_ansatz_v1"
            ...
            or config.get("base_chart_policy")
            != "expanded_runtime_projected_logical_v1"
            or accepted_indices != expected_indices
            or not selector_indices
            or len(selector_indices) != len(set(selector_indices))
            or any(index not in expected_indices for index in selector_indices)
        ):
            raise ValueError(
                f"round {outer_iteration}: compact accepted-refit drift"
            )
        trust = _mapping(
            row.get("route_a_trust_region_update"),
            field=f"round {outer_iteration} compact trust update",
        )
        before = float(trust.get("radius_before", float("nan")))
        after = float(trust.get("radius_after", float("nan")))
        if (
            ...
            raise ValueError(f"round {outer_iteration}: compact trust-chain drift")
        previous_radius = after
        compact_prune = _mapping(
            row.get("post_admission_prune"),
            field=f"round {outer_iteration} compact live prune",
        )
        if (
            compact_prune.get("enabled") is not True
            or compact_prune.get("prune_mode") != "live"
        ):
            raise ValueError(f"round {outer_iteration}: compact prune policy drift")
        candidate_count = int(compact_prune.get("candidate_count", -1))
        probe_indices = [
            int(value)
            for value in _sequence(
                compact_prune.get("probe_indices"),
                field=f"round {outer_iteration} compact prune probes",
            )
        ]
        executed = compact_prune.get("executed") is True
        accepted_count = int(compact_prune.get("accepted_count", -1))
        if (
            candidate_count not in (0, 1)
            or len(probe_indices) != candidate_count
            or accepted_count not in (0, 1)
            or accepted_count > int(executed)
        ):
            raise ValueError(f"round {outer_iteration}: compact prune count drift")
        before_state = _mapping(
            compact_prune.get("phase1_prune_trust_state_before"),
            field=f"round {outer_iteration} compact prune state before",
        )
        after_state = _mapping(
            compact_prune.get("phase1_prune_trust_state_after"),
            field=f"round {outer_iteration} compact prune state after",
        )
        update = _mapping(
            compact_prune.get("phase1_prune_trust_update"),
            field=f"round {outer_iteration} compact prune trust update",
        )
        before_tuple = (
            float(before_state.get("radius", float("nan"))),
            float(before_state.get("metric_damping", float("nan"))),
            int(before_state.get("update_count", -1)),
        )
        after_tuple = (
            float(after_state.get("radius", float("nan"))),
            float(after_state.get("metric_damping", float("nan"))),
            int(after_state.get("update_count", -1)),
        )
        if (
            before_state.get("schema") != "affine_deletion_fs_trust_state_v1"
            or after_state.get("schema") != "affine_deletion_fs_trust_state_v1"
            or not all(math.isfinite(value) for value in before_tuple[:2])
            or not all(math.isfinite(value) for value in after_tuple[:2])
            or before_tuple[1] != 0.0
            or after_tuple[1] != 0.0
            or after_tuple[0] > before_tuple[0]
            or after_tuple[1] < before_tuple[1]
            or (previous_prune_state is not None and before_tuple != previous_prune_state)
            or update.get("schema") != "affine_deletion_fs_trust_state_update_v1"
            or int(update.get("classical_quantum_query_charge", -1)) != 0
        ):
            raise ValueError(f"round {outer_iteration}: compact prune trust drift")
        if executed:
            model = _mapping(
                compact_prune.get("phase1_prune_affine_deletion_model"),
                field=f"round {outer_iteration} compact affine model",
            )
            work = _mapping(
                compact_prune.get("phase1_prune_exact_refit_work_accounting"),
                field=f"round {outer_iteration} compact prune work",
            )
            trial = _mapping(
                compact_prune.get("phase1_prune_trial_receipt"),
                field=f"round {outer_iteration} compact prune trial",
            )
            ...
            if (
                candidate_count != 1
                or len(probe_indices) != 1
                or model.get("schema") != "full_logical_affine_deletion_fs_trust_v1"
                or model.get("solver_policy")
                != "full_logical_affine_deletion_fs_trust_v1"
                or model.get("feasible") is not True
                or int(model.get("pre_support_coordinate_count", -1)) != pre_support
                or int(model.get("metric_supported_rank", -1)) not in range(1, pre_support + 1)
                or model.get("all_logical_coordinates_entered_before_support_reduction")
                is not True
                or model.get("supported_rank_projection_after_full_coordinate_entry")
                is not True
                or int(model.get("classical_quantum_query_charge", -1)) != 0
                or int(model.get("deletion_index", -1)) != probe_indices[0]
                or int(compact_prune.get("selected_index", -1)) != probe_indices[0]
                or not str(compact_prune.get("selected_label") or "")
                or nfev < 0
                or committed < 0
                or discarded < 0
                or committed + discarded != nfev
                or work.get("included_in_total_nfev") is not True
                or work.get("included_in_total_estimator_ledger") is not True
                or work.get("classification")
                != ("committed_prune" if accepted_count else "discarded_prune")
                or trial.get("prediction_complete") is not True
                or trial.get("realization_complete") is not True
                or trial.get("energy_receipt_complete") is not True
                or trial.get("measured_delete_refit_is_acceptance_authority")
                is not True
                or trial.get("endpoint_overlap_measured") is not False
                or int(trial.get("added_endpoint_overlap_query_count", -1)) != 0
                or not trial_id
                or trial.get("prediction_trial_id") != trial_id
                or trial.get("realization_trial_id") != trial_id
                or after_tuple[2] != before_tuple[2] + 1
                or float(update.get("radius_before", float("nan"))) != before_tuple[0]
                or float(update.get("radius_after", float("nan"))) != after_tuple[0]
                or float(update.get("metric_damping_before", float("nan")))
                != before_tuple[1]
                or float(update.get("metric_damping_after", float("nan")))
                != after_tuple[1]
                or int(update.get("update_count_before", -1)) != before_tuple[2]
                or int(update.get("update_count_after", -1)) != after_tuple[2]
                or update.get("radius_never_increased") is not True
                or update.get("metric_damping_never_decreased") is not True
            ):
                raise ValueError(f"round {outer_iteration}: compact prune trial drift")
        else:
            no_feasible_raw = compact_prune.get("phase1_prune_no_feasible_model")
            if isinstance(no_feasible_raw, Mapping):
                no_feasible = _mapping(
                    no_feasible_raw,
                    field=f"round {outer_iteration} compact no-feasible prune",
                )
                no_feasible_before = _mapping(
                    no_feasible.get("trust_state_before"),
                    field=f"round {outer_iteration} compact no-feasible state before",
                )
                no_feasible_after = _mapping(
                    no_feasible.get("trust_state_after"),
                    field=f"round {outer_iteration} compact no-feasible state after",
                )
                no_feasible_update = _mapping(
                    no_feasible.get("trust_update"),
                    field=f"round {outer_iteration} compact no-feasible update",
                )
                if (
                    compact_prune.get("permission_reason")
                    != "all_affine_deletion_models_infeasible"
                    or no_feasible.get("schema")
                    != "sr_v4_no_feasible_affine_deletion_models_v1"
                    or no_feasible.get("status")
                    != "skipped_no_feasible_affine_deletion_models"
                    or int(no_feasible.get("model_count", -1)) < 1
                    or int(no_feasible.get("feasible_model_count", -1)) != 0
                    or int(no_feasible.get("exact_delete_refit_trial_count", -1)) != 0
                    or no_feasible.get("legacy_nomination_fallback_used") is not False
                    or int(no_feasible.get("classical_quantum_query_charge", -1)) != 0
                    or no_feasible_before != no_feasible_after
                    or no_feasible_update.get("status") != "held"
                    or int(no_feasible_update.get("classical_quantum_query_charge", -1))
                    != 0
                    or int(no_feasible_update.get("update_count_before", -1))
                    != int(no_feasible_update.get("update_count_after", -2))
                    or update.get("status") != "held"
                ):
                    raise ValueError(
                        f"round {outer_iteration}: compact no-feasible prune drift"
                    )
            elif no_feasible_raw is not None or update.get("status") != "not_evaluated":
                raise ValueError(f"round {outer_iteration}: compact prune hold drift")
            if (
                candidate_count != 0
                or accepted_count != 0
                or compact_prune.get("phase1_prune_affine_deletion_model") is not None
                or compact_prune.get("phase1_prune_exact_refit_work_accounting")
                is not None
                or compact_prune.get("phase1_prune_trial_receipt") is not None
                or after_tuple != before_tuple
            ):
                raise ValueError(f"round {outer_iteration}: compact prune hold drift")
        previous_prune_state = after_tuple
        prune = {"executed": executed, "accepted": bool(accepted_count)}
        prune_executed += int(prune["executed"] is True)
        prune_accepted += int(
            prune["executed"] is True and prune.get("accepted") is True
        )
        checkpoint = _mapping(
            row.get("active_prefix_checkpoint"),
            field=f"round {outer_iteration} compact active-prefix checkpoint",
        )
        summary = validate_checkpoint(
            checkpoint,
            outer_iteration=outer_iteration,
    ...
    return {
        ...
        "final_active_depth": previous_depth,
        "final_radius": previous_radius,
        "fallback_rounds": list(fallback_rounds),
        "max_fixed_sector_leakage": max_sector,
        "max_binary_padding_leakage": max_padding,
        "prune_rounds_executed": prune_executed,
        "prune_rounds_accepted": prune_accepted,
        "compact_controller_receipt": {
            "schema": "paper_i_sr_compact_controller_checkpoint_refit_prune_v1",
            "status": "pass",
            "rounds": expected_rounds,
            "phase3_response_execution_receipt_scope": (
                "source_locked_route_policy_only_in_compact_history_v1"
            ),
            "accepted_refit_execution_receipt_scope": "every_controller_round_v1",
        },
    }
...
def validate_beam_parent_evidence(
    *,
    ...
    if relationship.get("relationship_present") is True:
        fallback = _mapping(
            current_adapt.get(
        ...
        fallback_rounds = [
            int(value)
            for value in _sequence(
                fallback.get("controller_rounds"),
                field="controller-frontier fallback rounds",
            )
        ]
        controller_path = _validate_compact_controller_history(
            current_adapt.get("history"),
            expected_rounds=target_round,
            fallback_rounds=fallback_rounds,
        )
        compact_current_receipt = controller_path[
            "compact_controller_receipt"
        ]
    else:
        if (
            ...
            raise ValueError(
                "round-target selected winner/current frontier relationship drift"
            )
