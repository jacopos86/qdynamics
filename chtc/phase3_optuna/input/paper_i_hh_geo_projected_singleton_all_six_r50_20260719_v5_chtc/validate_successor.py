...
def main() -> int:
    ...
    queue_rows = [line.split("\t") for line in (BUNDLE / "queue.tsv").read_text(encoding="utf-8").splitlines() if line]
    if len(queue_rows) != 12:
        raise RuntimeError(f"Geo successor queue row count mismatch: {len(queue_rows)}")
    if queue_rows[0][0] == "job_id" or not (BUNDLE.parents[3] / queue_rows[0][1]).is_file():
        raise RuntimeError("Geo successor queue header was enqueued as a proc")
