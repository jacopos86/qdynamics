# Paper-I Geo-ADAPT completion comparator bundle (v4 queue repair)

This immutable v4 successor preserves v3's worker-path and true-fresh fixes and
changes only the Condor queue surface: `queue.tsv` is headerless because
`queue <vars> from <file>` treats every nonempty line as a process. This makes
the submission exactly twelve real comparator jobs rather than twelve jobs plus
a literal-header process.
