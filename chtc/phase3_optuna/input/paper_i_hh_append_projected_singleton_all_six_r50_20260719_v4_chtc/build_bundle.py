#!/usr/bin/env python3
"""Build the immutable Paper-I Append-ADAPT completion comparator bundle."""
...
SCHEMA = "paper_i_hh_append_completion_bundle_v1"
...
def _copy_visible_append_locks(repo: Path, bundle: Path) -> dict[str, Any]:
    source_dir = (
        repo
        / "chtc"
        / "phase3_optuna"
        / "input"
        / "paper_i_hh_fullmeta_singleton_symmetry_corrected_parent_iter50_20260710_v1"
        / "visible_source_locks"
    )
    target_dir = bundle / "visible_source_locks"
    target_dir.mkdir(parents=True, exist_ok=True)
    locks: dict[str, Any] = {}
    for regime in REGIMES:
        source = source_dir / f"{regime['label']}_append_adapt.json"
        if not source.is_file():
            raise RuntimeError(f"visible Append baseline lock missing: {source}")
...
def _write_submit(
    ...
    *,
    ...
    lines = [
        "universe = vanilla",
        "batch_name = paper-i-hh-append-projected-singleton-r50-20260719-v4",
...
def build() -> dict[str, Any]:
    ...
    visible_locks = _copy_visible_append_locks(repo, bundle)
    ...
    settings_audit = {
        "schema": "paper_i_hh_append_completion_settings_difference_audit_v1",
        "generated_utc": _utc_now(),
        "status": "pass",
        "visible_append_source_locks": visible_locks,
        "preserved_method_contract": {
            "algorithm_id": "static_full_meta_append_adapt_vqe",
            "selection": "full candidate pool with replacement",
            "insertion": "append",
            "optimizer": "Powell",
        ...
        "operational_repairs": [
            ...
            "manifest-selected fail-closed finite non-increasing Powell maxiter acceptance",
            "projected-singleton full-meta expansion cap raised to 9000 so the requested untruncated pool is constructible",
        ],
    }
