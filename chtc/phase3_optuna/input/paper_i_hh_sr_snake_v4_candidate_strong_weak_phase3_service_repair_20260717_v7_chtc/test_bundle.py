...
def isolated_env(root: Path) -> dict[str, str]:
    ...
    env.update({
        "HOME": str(home),
        "PYTHONPATH": os.pathsep.join((str(root), str(site.getusersitepackages()))),
...
class ScaffoldTests(unittest.TestCase):
    def test_scaffold_is_fail_closed_and_names_corrected_contract(self) -> None:
        ...
        if not BUILT_BUNDLE:
            self.assertIn('EXPECTED_HEAD = "b2400825d3279753e2f1b1a61d4447f0ccbb606c"', builder)
            self.assertIn('EXPECTED_TREE = "b7bd374befe8e564da1ab4d0c53250f41bfad3c4"', builder)
...
class BundleTests(unittest.TestCase):
    def test_submission_queue_contains_only_strong_weak_repair(self) -> None:
        rows = [
            line.split("\t")
        ...
        self.assertEqual(rows[0][0], "strong_weak_u8")
        manifest = load(BUNDLE / "bundle_manifest.json")
        self.assertEqual(
            manifest["submission_scope"]["regimes"], ["strong_weak_u8"]
        )
        self.assertEqual(manifest["submission_scope"]["scientific_settings_changed"], [])
