#!/usr/bin/env python3
"""Build the source-locked SR-SNAKE-v4 strong-weak repair bundle.
...
source, verifies all six same-cutoff physics anchors, and writes a single fresh
strong-weak round-0 -> round-30 submission plus a fail-closed preflight.
"""
...
BUNDLE_ID = (
    "paper_i_hh_sr_snake_v4_candidate_strong_weak_"
    "phase3_service_repair_20260717_v7_chtc"
)
BATCH_NAME = "paper-i-hh-sr-v4-sw-r30-phase3-service-repair-20260717-v7"
...
EXPECTED_HEAD = "b2400825d3279753e2f1b1a61d4447f0ccbb606c"
EXPECTED_TREE = "b7bd374befe8e564da1ab4d0c53250f41bfad3c4"
...
SOURCE_LOCK_STATE = "frozen_full_response_maturity_retirement_repair_commit"
...
SUBMISSION_REGIMES = frozenset({"strong_weak_u8"})
...
def job_command(row: dict[str, Any]) -> tuple[list[str], dict[str, str]]:
    ...
    argv = [
        ...
        "--adapt-segment-id", f"{slug}-sr-v4-r0-r30-20260717-v7",
...
# Generated from the frozen v4 strong-weak repair source lock. Matchmaking remains disabled until
...
def main() -> int:
    ...
    if len(queue_lines) != 1 or queue_lines[0].split("\t", 1)[0] != "strong_weak_u8":
        raise ValueError(
            "repair bundle must queue exactly the strong_weak_u8 row"
        )
    ...
    bundle_manifest = {
        ...
        "created_utc": utc_now(),
        "run_class": "candidate",
        "parent_stage": "fresh_round0_to_round30_strong_weak_repair_only",
        "submission_scope": {
            "regimes": sorted(SUBMISSION_REGIMES),
            "job_count": len(queue_lines),
            "supersedes_cluster": 8878503,
            "supersedes_proc": 2,
            "superseded_exit_code": 1,
            "repair_classification": "existing_full_response_contract_enforcement",
    ...
    preflight = {
        ...
        "checks": {
            ...
            "single_strong_weak_submission_queue": (
                len(queue_lines) == 1
                and queue_lines[0].split("\t", 1)[0] == "strong_weak_u8"
            ),
