...
def validate(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("local v3 bundle source authority is incomplete")
    expected_overlays = {
        relative: {
            "sha256": digest,
    ...
    if (
        ...
        raise ValueError("local v3 compatibility-overlay source authority drift")
    for relative, expected_hash in NONSCIENTIFIC_ARCHIVE_OVERLAYS.items():
        ...
        if (
            ...
            raise ValueError(f"local v3 compatibility overlay drift: {relative}")
