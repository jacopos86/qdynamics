#!/usr/bin/env python3
"""Fail-closed validator for one fetched guarded-singleton pool archive."""
...
def validate(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema": "paper_i_hh_sr_guarded_singleton_fetched_validation_v1",
        "status": "pass",
...
def main() -> int:
    ...
    if args.path.is_file():
        with tempfile.TemporaryDirectory(prefix="sr_guarded_singleton_fetch_") as tmp:
            root = Path(tmp)
