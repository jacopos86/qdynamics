#!/usr/bin/env python3
"""Fail-closed validator for one fetched historical-beam appendix archive."""
...
SOURCE_LOCK_STATE = (
    "frozen_main_archive_plus_exact_historical_beam_overlay_"
    "plus_s_alg_accounting_repair_v2"
)
...
S_ALG_OVERLAY_SHA256 = (
    "7dd0532449388d7c9359c579fda2f25f57153f83c7e3ebbb1aeba66aa0019652"
)
S_ALG_OVERLAY_FILE_HASHES = {
    "pipelines/static_adapt/adapt_pipeline.py": (
        "84f4f271f90a95285600a5354e2c1a94e34e2c90b7df54fcee807d4b27dc7f43"
    ),
    "pipelines/static_adapt/estimator_call_ledger.py": (
        "22aee41ebedb452c1fe97ffc667021cd745e82b21ea815e0a84aa4d011f68cdd"
    ),
    "pipelines/static_adapt/resume_scaffold.py": (
        "aa45f96e8cc92555fa4081725cd49d0393d9c2864433e934bef0b41b8b9ce2c2"
    ),
    "pipelines/scaffold/hh_continuation_scoring.py": (
        "0f8d401c782b0d60a02dc9ae0bb274978a70c653559728341b5539a8c8bfb713"
    ),
    "test/test_static_adapt_accepted_refit.py": (
        "f39493f68bbe26ef1ffefd6d0542eb11c6d43860844c56808d3f4b2cdd137813"
    ),
    "test/test_static_adapt_estimator_call_ledger.py": (
        "84ba8c8e4ade0ab9c5fcd4e743ad7a35f9204ae3606a27b9b9275fe1d34fdc89"
    ),
    "test/test_static_adapt_resume_scaffold.py": (
        "51094d07a776a0a5f7ea2dcb4f4a510515683f9cbdf88e83b2542960c59e7027"
    ),
    "test/test_hh_continuation_scoring.py": (
        "22487de565c16c4cf185fa7ba9a24f8b0e622bfb875d5a44d88d2b0750c0298f"
    ),
}
...
ESTIMATOR_LEDGER_SOURCE_PATH = (
    "pipelines/static_adapt/estimator_call_ledger.py"
)
ESTIMATOR_LEDGER_POINTER_SCHEMA = (
    "paper_i_estimator_call_ledger_sidecar_pointer_v1"
)
...
def load_source_locked_estimator_ledger_module(
    source_archive: Path,
    source_archive_manifest: dict[str, Any],
) -> types.ModuleType:
    """Load the exact ledger implementation authenticated by the archive.

    The fetched validator must not import this class from the live repository:
    doing so could accept or reject evidence under code different from the
    worker's source lock.  The module bytes are read directly from the frozen
    archive, checked against its complete file manifest, registered in
    ``sys.modules`` for dataclass construction, and only then executed.
    """

    record = source_archive_manifest.get("files", {}).get(
        ESTIMATOR_LEDGER_SOURCE_PATH
    )
    if not isinstance(record, dict):
        raise ValueError("source archive lacks estimator-ledger file record")
    expected_sha = str(record.get("sha256") or "")
    expected_size = int(record.get("size_bytes", -1))
    if len(expected_sha) != 64 or expected_size <= 0:
        raise ValueError("estimator-ledger source record is incomplete")
    with tarfile.open(source_archive, "r:gz") as handle:
        matches = [
            member
            for member in handle.getmembers()
            if member.name == ESTIMATOR_LEDGER_SOURCE_PATH
        ]
        if len(matches) != 1 or not matches[0].isfile():
            raise ValueError(
                "source archive must contain exactly one estimator-ledger module"
            )
        extracted = handle.extractfile(matches[0])
        if extracted is None:
            raise ValueError("cannot read source-locked estimator-ledger module")
        source_bytes = extracted.read()
    actual_sha = hashlib.sha256(source_bytes).hexdigest()
    if actual_sha != expected_sha or len(source_bytes) != expected_size:
        raise ValueError("source-locked estimator-ledger module hash/size drift")

    module_name = f"_source_locked_estimator_call_ledger_{actual_sha[:16]}"
    module = types.ModuleType(module_name)
    module.__file__ = (
        f"{source_archive}!/{ESTIMATOR_LEDGER_SOURCE_PATH}"
    )
    module.__package__ = ""
    module.__source_archive_sha256__ = sha256(source_archive)
    module.__source_file_sha256__ = actual_sha
    ...
    try:
        ...
        raise
    ledger_class = getattr(module, "EstimatorCallLedger", None)
    if ledger_class is None or not callable(getattr(ledger_class, "from_payload", None)):
        sys.modules.pop(module_name, None)
        raise TypeError("source-locked module lacks EstimatorCallLedger.from_payload")
    return module


def validate_ledger_sidecar_pointer(
    result: dict[str, Any],
    ledger_path: Path,
) -> dict[str, Any]:
    """Authenticate the fetched ledger bytes against the result pointer."""

    accounting = result.get("adapt_vqe", {}).get("estimator_call_accounting")
    if not isinstance(accounting, dict):
        raise ValueError("result estimator accounting is missing")
    pointer = accounting.get("sidecar")
    if not isinstance(pointer, dict):
        raise ValueError("result estimator-ledger sidecar pointer is missing")
    if pointer.get("schema") != ESTIMATOR_LEDGER_POINTER_SCHEMA:
        raise ValueError("result estimator-ledger sidecar pointer schema drift")
    pointer_path = str(pointer.get("path") or "")
    if not pointer_path or Path(pointer_path).name != ledger_path.name:
        raise ValueError("result estimator-ledger sidecar pointer path drift")
    actual_sha = sha256(ledger_path)
    actual_size = int(ledger_path.stat().st_size)
    if str(pointer.get("sha256") or "") != actual_sha:
        raise ValueError("result estimator-ledger sidecar pointer SHA-256 mismatch")
    if int(pointer.get("size_bytes", -1)) != actual_size:
        raise ValueError("result estimator-ledger sidecar pointer size mismatch")
    return {
        "schema": ESTIMATOR_LEDGER_POINTER_SCHEMA,
        "path_basename": ledger_path.name,
        "sha256": actual_sha,
...
def validate(root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != (
            "frozen_main_archive_plus_exact_historical_beam_overlay_plus_"
            "s_alg_accounting_overlay_inventory_v2"
        )
        or source_revision.get("immutable_parent_archive", {}).get("sha256")
        != PARENT_ARCHIVE_SHA256
        or source_revision.get("historical_beam_overlay", {}).get("overlay_sha256")
        != BEAM_OVERLAY_SHA256
        or source_revision.get("historical_beam_overlay", {}).get(
            "parent_route_contract_sha256"
        ) != PARENT_PROFILE_CONTRACT_SHA256
        or source_revision.get("s_alg_accounting_repair_overlay", {}).get(
            "overlay_sha256"
        ) != S_ALG_OVERLAY_SHA256
        or source_revision.get("s_alg_accounting_repair_overlay", {}).get(
            "derived_file_sha256"
    ...
    if (
        ...
        or source_archive_manifest.get("historical_beam_overlay", {}).get(
            "overlay_sha256"
        ) != BEAM_OVERLAY_SHA256
        or source_archive_manifest.get("historical_beam_overlay", {}).get(
            "parent_route_contract_sha256"
        ) != PARENT_PROFILE_CONTRACT_SHA256
        or source_archive_manifest.get("s_alg_accounting_repair_overlay", {}).get(
            "overlay_sha256"
        ) != S_ALG_OVERLAY_SHA256
        or source_archive_manifest.get("s_alg_accounting_repair_overlay", {}).get(
            "derived_file_sha256"
    ...
    for relative, expected_hash in S_ALG_OVERLAY_FILE_HASHES.items():
        record = source_archive_manifest.get("files", {}).get(relative, {})
        if record.get("sha256") != expected_hash:
            raise ValueError(
                f"local S_alg accounting repair overlay drift: {relative}"
            )
    ...
    source_locked_ledger_module = load_source_locked_estimator_ledger_module(
        source_archive,
    ...
    ledger_pointer = validate_ledger_sidecar_pointer(
        result,
        output / "json/estimator_call_ledger.json",
    )
    ...
    for key, expected in beam_execution.items():
        if normalized_contract.get("execution_settings", {}).get(key) != expected:
            raise ValueError(f"fetched historical beam setting drift: {key}")
    for key, expected in beam_semantics.items():
        if normalized_contract.get("semantic_invariants", {}).get(key) != expected:
            raise ValueError(f"fetched historical beam invariant drift: {key}")
    artifact_paths = {
    ...
    if target_round != 50 or target_new_admissions != 50:
        raise ValueError("historical-beam appendix requires exact round-50 horizon")
    ...
    evidence = validate_parent_evidence(
        ...
        estimator_ledger_class=(
            source_locked_ledger_module.EstimatorCallLedger
        ),
    )
    ...
    if not isinstance(runtime_evidence, dict):
        raise ValueError("runtime scientific-evidence validation is missing")
    fetched_strict = evidence.get("ledger", {}).get(
        "strict_source_locked_deserialization", {}
    )
    if (
        not isinstance(fetched_strict, dict)
        or fetched_strict.get("performed") is not True
        or fetched_strict.get("status")
        != "strict_source_locked_deserialization_pass"
    ):
        raise ValueError("fetched ledger lacked strict source-locked validation")
    # The worker-side report is generated before transfer and records strict
    # reconstruction as deferred.  Compare every scientific field except that
    # one fetched-only authentication receipt.
    runtime_comparable = json.loads(json.dumps(runtime_evidence))
    fetched_comparable = json.loads(json.dumps(evidence))
    runtime_comparable.get("ledger", {}).pop(
        "strict_source_locked_deserialization", None
    )
    fetched_comparable.get("ledger", {}).pop(
        "strict_source_locked_deserialization", None
    )
    if runtime_comparable != fetched_comparable:
    ...
    return {
        "schema": "paper_i_hh_sr_appendix_historical_beam3x2_fetched_validation_v1",
        "status": "pass",
        ...
        "ledger_sidecar_pointer_validation": ledger_pointer,
        "ledger_source_locked_module": {
            "path": ESTIMATOR_LEDGER_SOURCE_PATH,
            "sha256": getattr(
                source_locked_ledger_module,
                "__source_file_sha256__",
            ),
            "source_archive_sha256": getattr(
                source_locked_ledger_module,
                "__source_archive_sha256__",
            ),
        },
...
def main() -> int:
    ...
    if args.path.is_file():
        with tempfile.TemporaryDirectory(prefix="sr_historical_beam3x2_fetch_") as tmp:
            root = Path(tmp)
