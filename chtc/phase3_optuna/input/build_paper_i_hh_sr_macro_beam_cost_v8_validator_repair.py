#!/usr/bin/env python3
"""Build immutable v8 all-key validator/reporting successors from v6."""
...
CREATED_UTC = "2026-07-20T14:35:00Z"
REPAIR_SCHEMA = "paper_i_sr_macro_beam_cost_validator_reporting_repair_v2"
...
def _repair_receipt(arm: dict[str, Any], predecessor: str, successor: str) -> dict[str, Any]:
    return {
        ...
        "scientific_rerun_required_after_passing_v8_revalidation": False,
    }
...
def _build_arm(arm: dict[str, Any]) -> Path:
    ...
    successor_name = predecessor_name.replace("_v6_chtc", "_v8_chtc")
    ...
    new_batch = old_batch.removesuffix("-v6") + "-v8"
    BASE._replace_text_tree(
        successor,
        (
            (predecessor_name, successor_name),
            (old_batch, new_batch),
            ("20260719-v6", "20260719-v8"),
        ),
    )
    ...
    shutil.copy2(
        INPUT_ROOT / "paper_i_hh_sr_macro_beam_cost_v8_revalidate_v6_archive.py",
        successor / "revalidate_v6_archive.py",
    )
    shutil.copy2(
        INPUT_ROOT / "paper_i_hh_sr_macro_beam_cost_v8_validation_test.py",
    ...
    archive_preflight.update(
        {
            "validator_reporting_repair": repair,
            "v8_bundle_tests_passed": 0,
            "v8_archive_only_validate_rows_passed": 0,
            "v8_validator_repair_tests_passed": 0,
            "v8_frozen_archive_revalidations_passed": 0,
    ...
    if BASE._sha256_file(successor / "source_locked.tar.gz") != SOURCE_SHA256:
        raise RuntimeError("v8 scientific source archive changed")
    for path in (
    ...
    incomplete_v7 = INPUT_ROOT / successor_name.replace("_v8_chtc", "_v7_chtc")
    if incomplete_v7.is_dir():
        BASE._json_dump(
            incomplete_v7 / "SUPERSEDED_DO_NOT_SUBMIT.json",
            {
                "bundle_id": incomplete_v7.name,
        ...
        v7_submit = incomplete_v7 / "submit.sub"
        v7_submit_text = v7_submit.read_text(encoding="utf-8")
        if "requirements = False && TARGET.HasSIF" not in v7_submit_text:
            if "requirements = TARGET.HasSIF" not in v7_submit_text:
                raise RuntimeError("v7 submit requirement anchor drift")
            v7_submit.write_text(
                v7_submit_text.replace(
                    "requirements = TARGET.HasSIF",
                    "requirements = False && TARGET.HasSIF",
                    1,
                ),
