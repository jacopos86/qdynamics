...
bundle_id="paper_i_hh_geo_comparators_macro_projected_singleton_all_six_r50_20260718_v3_chtc"
output_dir="raw_outputs/${bundle_id}/${job_id}"
mkdir -p "${output_dir}"
...
apptainer exec \
  ...
  "${image_path}" \
  python "chtc/phase3_optuna/input/${bundle_id}/run_job.py" "${job_manifest}" "${output_dir}"

tar -czf "raw_outputs/${bundle_id}/${job_id}_transfer.tar.gz" \
  -C "raw_outputs/${bundle_id}" \
  "${job_id}"
