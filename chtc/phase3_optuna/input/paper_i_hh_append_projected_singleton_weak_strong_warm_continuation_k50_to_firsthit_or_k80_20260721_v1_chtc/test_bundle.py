#!/usr/bin/env python3
"""Focused local-only tests for the Append-S warm continuation bundle."""
...
def test_bundle_is_one_row_and_fail_closed_for_submission() -> None:
    ...
    assert "requirements = False" in submit
    assert "LOCAL-ONLY" in submit
    manifest = _load(BUNDLE / "bundle_manifest.json")
    assert manifest["row_count"] == 1
    assert manifest["submission_enabled"] is False
    assert manifest["remote_image_preflight"] == "pending"
...
def test_manifest_changes_are_authorized_and_baseline_science_is_identical() -> None:
    ...
    assert audit["submission_authorized"] is False
