...
_SPEC = importlib.util.spec_from_file_location("geo_completion_bundle_builder", BUNDLE / "build_bundle.py")
assert _SPEC is not None and _SPEC.loader is not None
builder = importlib.util.module_from_spec(_SPEC)
_SPEC.loader.exec_module(builder)
EXPECTED_SOURCE = "33f0e8ffba1d532e86037077e99d8578423fc7a52842e2479a40afea1588ed3d"
EXPECTED_TEST = "790fd3bea888c444883d7677ba0418b2112781fec65e8283fe6a88a9935d1c19"
EXPECTED_REGIMES = {
...
def test_queue_is_exact_two_by_six_matrix() -> None:
    ...
    assert job_ids[0] == "geo_macro__weak_weak__r50"
    ...
    for variant in ("macro", "projected_singleton"):
        assert {job.split("__")[1] for job in job_ids if job.startswith(f"geo_{variant}__")} == set(
            EXPECTED_REGIMES
        )
...
def test_normalized_manifest_contract(variant: str, regime: str) -> None:
    job_id = f"geo_{variant}__{regime}__r50"
    ...
    assert job["algorithm_id"] == "static_geo_adapt_vqe"
    ...
    assert controller["fresh_round_zero"] is True
    assert controller["max_adapt_iterations"] == 50
    assert controller["stop_policy"] == "fixed_horizon_no_target_v1"
    assert controller["energy_stop_target"] is None
    assert controller["gradient_threshold"] == 0.0
    assert controller["hh_preseed"] == "off"
    assert controller["repeat_policy"] == "with_replacement_except_immediate_repeat"
    assert controller["immediate_repeat_policy"] == "score_full_pool_then_skip_append"
    optimizer = job["optimizer"]
    assert optimizer["kind"] == "powell"
    assert optimizer["maxiter"] == 200
    assert optimizer["powell_maxiter_cap_policy"] == "strict_failure_v1"
    assert optimizer["position_policy"] == "append"
    assert optimizer["position_optimized_geo_adapt"] is False
    selector = job["geo_selector"]
    assert selector["metric_factorization"] == "exact_real_tangent_span_svd_v1"
    assert selector["solve_kind"] == "moore_penrose_pseudoinverse"
    assert selector["regularization_used_in_selector_solve"] is False
    assert selector["pre_screen"] == "full_candidate_set"
    assert selector["numerical_tie_policy"] == "machine_precision_collinear_alias_then_label_v1"
    assert selector["fixed_horizon_overrides_plateau_stop"] is True
    assert job["seed"] == 7
    ...
    if variant == "macro":
        ...
        assert job["variant"]["candidate_representation"] == "unsplit_full_meta_macro_parent"
    else:
        ...
        assert (
            job["variant"]["candidate_representation"]
            == "symmetry_and_padding_valid_projected_singleton_child"
        )
...
def test_source_archive_and_inventory_are_immutable_and_closed() -> None:
    ...
    assert manifest["files"]["test/test_generic_static_adapt_variants.py"]["sha256"] == EXPECTED_TEST
    for relative in (
...
def test_visible_geo_baselines_are_resolved_and_vendored_by_hash() -> None:
    audit = _load(BUNDLE / "settings_difference_audit.json")
    assert audit["status"] == "pass"
    assert audit["unapproved_drift"] == []
    assert audit["visible_source_map_path_repair"]["scientific_fields_changed"] == []
    assert set(audit["visible_geo_source_locks"]) == set(EXPECTED_REGIMES)
    for record in audit["visible_geo_source_locks"].values():
        ...
        assert lock["source_sha256_match"] is True


def test_worker_requires_exact_geo_span_receipts_fidelity_cost_and_accounting() -> None:
    worker = (BUNDLE / "run_job.py").read_text(encoding="utf-8")
    for token in (
        "expected 50 completed Geo rounds",
        "exact_real_tangent_span_svd_v1",
        "moore_penrose_pseudoinverse",
        "geo_metric_regularization_used_in_selector_solve",
...
def test_submission_artifact_hash_inventory_matches_current_files() -> None:
    payload = _load(BUNDLE / "submission_artifact_hashes.json")
    for relative, expected in payload["files"].items():
        assert _sha256(BUNDLE / relative) == expected


def test_worker_is_true_fresh_round_zero_and_wrapper_uses_immutable_path() -> None:
    worker = (BUNDLE / "run_job.py").read_text(encoding="utf-8")
    for forbidden in (
    ...
    assert 'python "chtc/phase3_optuna/input/${bundle_id}/run_job.py"' in wrapper
    assert "python run_job.py" not in wrapper
