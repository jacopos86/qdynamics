...
OUTPUT_PARENT="raw_outputs/${BUNDLE_ID}"
OUTPUT_ROOT="${OUTPUT_PARENT}/${JOB_ID}"
TRANSFER_ARCHIVE="${OUTPUT_PARENT}/${JOB_ID}_transfer.tar.gz"
...
package_outputs() {
  local status="$?"
  trap - EXIT
  mkdir -p "$OUTPUT_ROOT" "$OUTPUT_PARENT"
  tar -czf "${TRANSFER_ARCHIVE}.tmp" -C "$OUTPUT_PARENT" "$JOB_ID" || status=74
...
# The parent v5 wrapper packaged only after a successful Python return. These
# traps preserve diagnostics on Python/container failure and Condor eviction.
...
"$APPTAINER_BIN" exec --cleanenv --bind "$ROOT:/work" "$IMAGE" \
  bash -lc 'cd /work && export PYTHONPATH=/work/source_locked PYTHONNOUSERSITE=1 PYTHONDONTWRITEBYTECODE=1; python3 -c "$3"; python3 -u "$1" "$2" "$4"' \
