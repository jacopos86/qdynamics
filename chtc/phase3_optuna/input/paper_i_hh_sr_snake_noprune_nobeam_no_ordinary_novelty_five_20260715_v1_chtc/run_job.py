#!/usr/bin/env python3
"""Validate or execute one source-locked five-regime SR-SNAKE row."""
...
EXPECTED_SCHEMA = "paper_i_hh_sr_noprune_nobeam_nonovelty_chtc_job_v1"
BUNDLE_ID = (
...
ALLOWED_DIFF_FLAGS = REGIME_FLAGS | PATH_FLAGS
...
def validate(manifest_path: Path, manifest: dict[str, Any]) -> list[str]:
    ...
    if manifest.get("run_class") != "diagnostic_provisional_cross_regime_extension":
        raise ValueError("job must remain diagnostic/provisional")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    normalized = {
        "schema": "paper_i_hh_sr_noprune_nobeam_nonovelty_runtime_manifest_v1",
        "job_manifest": manifest_path.as_posix(),
        "job_manifest_sha256": sha256(manifest_path),
    ...
    execution = {
        **normalized,
        "schema": "paper_i_hh_sr_noprune_nobeam_nonovelty_execution_v1",
        "status": "running",
        "exit_code": None,
    }
