#!/usr/bin/env python3
"""Build the five-regime source-locked SR-SNAKE diagnostic bundle.

The successful local weak--weak run is the executable anchor.  This builder
changes only the Hamiltonian regime fields and isolated output/cache paths.
...
BATCH_NAME = "paper-i-hh-sr-noprune-nobeam-nonovelty-five-20260715-v1"
...
def route_signature(argv: Sequence[str]) -> dict[str, Any]:
    ...
    return {
        ...
        if key not in REGIME_FLAGS | PATH_FLAGS
    }
...
def build_job(
    ...
    unexpected = sorted(set(changed) - (REGIME_FLAGS | PATH_FLAGS))
    if unexpected:
    ...
    return {
        "schema": "paper_i_hh_sr_noprune_nobeam_nonovelty_chtc_job_v1",
        "bundle_id": BUNDLE_ID,
        "batch_name": BATCH_NAME,
        "regime_slug": slug,
        "created_utc": utc_now(),
        "run_class": "diagnostic_provisional_cross_regime_extension",
        "route_identity": {
            ...
            "status": "explicit_diagnostic_provisional",
        },
        "physics": {
            "problem": "hh",
            "L": 2,
        ...
        "command": {
            ...
            "allowed_changed_flags": sorted(REGIME_FLAGS | PATH_FLAGS),
            "unexpected_differences": unexpected,
            "all_non_regime_non_path_fields_identical": True,
        },
        "scientific_contract": {
            ...
            "ordinary_novelty_multipliers_enabled": False,
        ...
        "settings_difference": {
            "scientific_axis": "hh_interaction_regime_and_locked_same_cutoff",
            "regime_fields": {
                "--u": str(regime["u"]),
            ...
            "all_other_executable_fields_identical_to_successful_anchor": True,
        },
        "source_lock": {
...
def frozen_parse_check(job_paths: list[Path]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sr_five_parse_") as temporary:
    ...
    return {
        "schema": "paper_i_hh_sr_five_regime_local_parse_validation_v1",
        "created_utc": utc_now(),
        "status": "pass",
...
transfer_input_files = {base}/run_job.py, {base}/source_archive_manifest.json, {base}/source_lock_and_settings_diff.json, {base}/bundle_manifest.json, {base}/preflight.json, {base}/local_parse_validation.json, {base}/source_lock/anchor_command.json, {base}/source_lock/anchor_result_summary.json, {base}/source_lock/anchor_execution.json, {base}/source_lock/anchor_settings_diff.json, {base}/source_lock/source_revision_manifest.json, {base}/source_lock/base_source_manifest.json, $(job_manifest), {base}/source_locked.tar.gz, chtc/phase3_optuna/image.sif
...
def main() -> int:
    ...
    settings_diff = {
        "schema": "source_locked_cross_regime_extension_audit_v1",
        "bundle_id": BUNDLE_ID,
        "created_utc": utc_now(),
        "status": "pass",
        "run_class": "diagnostic_provisional_cross_regime_extension",
        ...
        "approved_scientific_axis": (
            "five_remaining_hh_interaction_regimes_with_locked_same_cutoff"
        ),
        ...
        "jobs": [
            {
                ...
                "non_regime_non_path_settings_diff": [],
            }
            for job in jobs
        ],
    ...
    bundle_manifest = {
        "schema": "paper_i_hh_sr_noprune_nobeam_nonovelty_five_bundle_v1",
        "bundle_id": BUNDLE_ID,
        "batch_name": BATCH_NAME,
        "created_utc": utc_now(),
        "run_class": "diagnostic_provisional_cross_regime_extension",
    ...
    preflight = {
        "schema": "paper_i_hh_sr_noprune_nobeam_nonovelty_five_preflight_v1",
        "created_utc": utc_now(),
        "status": "pass",
        "scientific_execution_performed": False,
        "checks": {
            ...
            "five_remaining_regimes_present": len(jobs) == 5,
            ...
            "ordinary_novelty_multipliers_disabled": True,
            "infeasible_model_novelty_fallback_retained": True,
    ...
    artifact_hashes = {
        "schema": "paper_i_hh_sr_five_submission_artifact_hashes_v1",
        "created_utc": utc_now(),
        "artifacts": {
