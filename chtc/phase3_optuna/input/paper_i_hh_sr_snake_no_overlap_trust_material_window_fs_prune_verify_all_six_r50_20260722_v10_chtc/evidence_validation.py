...
FALLBACK_MODE = "collective_span_novelty_over_cost_v1"
...
def validate_supported_rank_or_exact_fallback(
    row: Mapping[str, Any],
    *,
    pre_support: int,
    outer_iteration: int,
    require_supported_rank: bool,
    expected_fallback_fired: bool | None = None,
) -> dict[str, Any]:
    """Require a supported rank unless this exact admission used the fallback.

    The all-energy-models-infeasible admission bypasses the whitened quadratic
    solve, so it has no supported-rank receipt to serialize.  A missing rank is
    valid only when the same history row carries the complete, zero-query
    collective-novelty fallback receipt.  This keeps an ordinary missing rank
    fail-closed rather than turning absence into an implicit fallback.
    """

    # The accepted record is the source of truth for this safety admission.
    # In the effective-1x1 execution path, the top-level convenience fields can
    # be absent even though the accepted record and cumulative receipt retain
    # the exact marker.  Reconstruct only from those durable, explicit records;
    # never infer fallback merely from a missing supported rank.
    evidence: list[tuple[str, Mapping[str, Any], bool]] = [
        ("history_row", row, True),
    ]
    for collection_name in ("selected_feature_rows", "admitted_records"):
        values = row.get(collection_name, ())
        if isinstance(values, Sequence) and not isinstance(
            values, (str, bytes, bytearray)
        ):
            evidence.extend(
                (f"{collection_name}[{index}]", value, False)
                for index, value in enumerate(values)
                if isinstance(value, Mapping)
            )

    exact_sources: list[str] = []
    for source, record, top_level in evidence:
        if top_level:
            fired = (
                record.get(
            ...
            mode = str(
                ...
                or record.get("route_a_geometry_expansion_mode")
                or ""
            )
            reason = str(
                record.get(
                ...
                or record.get("route_a_geometry_expansion_reason")
                or ""
            )
            charge_raw = record.get(
            ...
            mode = str(
                record.get("route_a_geometry_expansion_mode")
                or record.get(
                    "all_energy_models_infeasible_novelty_fallback_mode"
                )
                or ""
            ...
            reason = str(
                record.get("route_a_geometry_expansion_reason")
                or record.get(
                    "all_energy_models_infeasible_novelty_fallback_reason"
                )
                or ""
            ...
            charge_raw = record.get(
                "route_a_geometry_expansion_novelty_query_charge",
                record.get(
                    "all_energy_models_infeasible_novelty_fallback_query_charge",
                    -1,
                ),
            )
        if not fired and not mode:
            continue
        try:
            query_charge = int(charge_raw)
        except (TypeError, ValueError) as exc:
            raise ValueError(
                f"round {outer_iteration}: malformed fallback query charge"
            ) from exc
        if (
            mode != FALLBACK_MODE
            ...
            raise ValueError(
                f"round {outer_iteration}: incomplete or nonzero-query fallback receipt"
            )
        exact_sources.append(source)

    fallback_fired = bool(exact_sources)
    if (
        expected_fallback_fired is not None
        and fallback_fired is not bool(expected_fallback_fired)
    ):
        raise ValueError(
            f"round {outer_iteration}: per-round/cumulative fallback telemetry mismatch"
        )

    supported_raw = row.get("phase3_response_supported_rank")
    if require_supported_rank and supported_raw is None and not fallback_fired:
    ...
    return {
        ...
        "fallback_receipt_sources": exact_sources,
        "supported_rank": supported_rank,
        "supported_rank_recorded": supported_rank is not None,
    }
...
def validate_parent_evidence(
    *,
    ...
    if int(fallback.get("query_charge_total", -1)) != 0:
        raise ValueError("fallback query charge must be exactly zero")
    ...
    for outer_iteration, (history_raw, checkpoint_raw) in enumerate(
        ...
        rank_receipt = validate_supported_rank_or_exact_fallback(
            row,
            pre_support=pre_support,
            outer_iteration=outer_iteration,
            require_supported_rank=require_supported_rank,
            expected_fallback_fired=(outer_iteration in fallback_rounds),
        )
        if not rank_receipt["supported_rank_recorded"]:
        ...
        expected_trust_update_policy = str(
            settings.get(
                "historical_singleton_trust_region_update_policy",
        ...
        if (
            ...
            or trust.get("policy") != expected_trust_update_policy
        ...
        row_fallback = bool(rank_receipt["fallback_fired"])
...
def validate_no_overlap_trust_evidence(
    *, result: dict[str, Any], target_round: int,
    ...
    initial_zero_active_holds = 0
    source_metric_transaction_failure_holds = 0
    source_metric_displacement_unresolved_holds = 0
    contractions = 0
    expansions = 0
    holds = 0
    for expected_round, row in enumerate(history, start=1):
        ...
        initial_zero_active_hold = (
            expected_round == 1
            and receipt.get("active_context_indices") == []
            and receipt.get("source_metric_trust_transaction") is None
            and receipt.get("source_metric_trust_transaction_failure")
            == "ValueError:Projected trust transaction is missing square G_AA_raw."
            and receipt.get("update_reason")
            == "source_metric_transaction_failure_hold"
            and float(receipt.get("radius_after"))
            == float(receipt.get("radius_before"))
        )
        source_metric_transaction_failure_hold = (
            not initial_zero_active_hold
            and receipt.get("source_metric_trust_transaction") is None
            and isinstance(receipt.get("source_metric_trust_transaction_failure"), str)
            and bool(receipt.get("source_metric_trust_transaction_failure"))
            and receipt.get("update_reason")
            == "source_metric_transaction_failure_hold"
            and receipt.get("predicted_source_metric_displacement") is None
            and receipt.get("realized_source_metric_displacement") is None
            and receipt.get("displacement_ratio") is None
            and float(receipt.get("radius_after"))
            == float(receipt.get("radius_before"))
        )
        source_metric_displacement_unresolved_hold = (
            receipt.get("update_reason")
            == "source_metric_displacement_unresolved_hold"
            and isinstance(receipt.get("source_metric_trust_transaction"), dict)
            and receipt["source_metric_trust_transaction"].get("transaction_complete")
            is True
            and receipt.get("displacement_ratio") is None
            and math.isfinite(
                float(receipt.get("predicted_source_metric_displacement"))
            )
            and float(receipt.get("predicted_source_metric_displacement")) >= 0.0
            and math.isfinite(
                float(receipt.get("realized_source_metric_displacement"))
            )
            and float(receipt.get("realized_source_metric_displacement")) >= 0.0
            and float(receipt.get("radius_after"))
            == float(receipt.get("radius_before"))
        )
        ...
        elif initial_zero_active_hold:
            initial_zero_active_holds += 1
        elif source_metric_transaction_failure_hold:
            source_metric_transaction_failure_holds += 1
        elif source_metric_displacement_unresolved_hold:
            source_metric_displacement_unresolved_holds += 1
        else:
            for key in (
        ...
        if after < before:
            ...
            holds += 1
    if (
        source_metric_receipts
        + geometry_expansion_holds
        + initial_zero_active_holds
        + source_metric_transaction_failure_holds
        + source_metric_displacement_unresolved_holds
        != int(target_round)
    ):
    ...
    return {
        ...
        "initial_zero_active_no_overlap_hold_count": initial_zero_active_holds,
        "source_metric_transaction_failure_hold_count": (
            source_metric_transaction_failure_holds
        ),
        "source_metric_displacement_unresolved_hold_count": (
            source_metric_displacement_unresolved_holds
        ),
