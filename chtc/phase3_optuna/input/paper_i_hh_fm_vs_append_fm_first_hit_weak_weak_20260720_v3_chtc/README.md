...
Both arms start from the same empty HF-reference ansatz. The automatic
Hubbard--Holstein seed scaffold is explicitly disabled and locked in the
normalized scientific settings. This v3 bundle supersedes the failed v2 launch:
v2 exposed a coordinate-order plumbing defect at FM round 3, and v3 applies the
required Phase-II-to-Phase-I permutation consistently to metric, Hessian, and
gradient blocks. The scientific settings are unchanged.
