#!/usr/bin/env python3
"""Non-destructively revalidate one completed v6 post-run-validator failure."""
...
V7_BUNDLE_ID = run_job.BUNDLE_ID
V6_BUNDLE_ID = V7_BUNDLE_ID.replace("_v7_chtc", "_v6_chtc")
KNOWN_FAILURE = (
...
def rebase_manifest(output: Path, generated: Path) -> dict[str, Any]:
    ...
    if not manifest_path.is_file():
        raise ValueError(f"v7 job manifest missing for regime {regime}")
...
def validate_original_failure(
    *, output: Path, manifest: dict[str, Any]
    ...
    if (
        ...
        raise ValueError("frozen v6 normalized manifest differs from v7 science lock")
...
def main() -> int:
    ...
    if not source_archive.is_file() or sha256(source_archive) != expected_source_sha:
        raise ValueError("v7 source archive lock is missing or changed")

    with tempfile.TemporaryDirectory(prefix="sr_macro_cost_v7_revalidate_") as tmp:
        workspace = Path(tmp)
        ...
        receipt = {
            "schema": "paper_i_sr_macro_beam_cost_v7_v6_archive_revalidation_v1",
            "status": "pass",
            ...
            "v7_validator_bundle_id": V7_BUNDLE_ID,
            ...
            "hysteresis_disabled_source_receipt": validation.get(
                "hysteresis_disabled_source_receipt"
            ),
