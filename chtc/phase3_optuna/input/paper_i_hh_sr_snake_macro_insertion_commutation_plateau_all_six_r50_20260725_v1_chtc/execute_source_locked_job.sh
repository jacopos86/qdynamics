...
package_outputs() {
  ...
  # Preserve the live checkpoint and its closed estimator-ledger sidecar while
  # omitting worker-local caches and duplicate multi-GB terminal ledgers.
  tar \
    --exclude="$OUTPUT_ROOT/cache" \
    --exclude="$OUTPUT_ROOT/json/result.json" \
    --exclude="$OUTPUT_ROOT/json/estimator_call_ledger.json" \
    -czf "${TRANSFER_ARCHIVE}.tmp" "$OUTPUT_ROOT" || status=74
...
"$APPTAINER_BIN" exec --cleanenv --bind "$ROOT:/work" "$IMAGE" \
  bash -lc 'cd /work && export PYTHONPATH=/work PYTHONNOUSERSITE=1 PYTHONDONTWRITEBYTECODE=1; python3 -c "$3"; python3 -u "$1" "$2"' \
