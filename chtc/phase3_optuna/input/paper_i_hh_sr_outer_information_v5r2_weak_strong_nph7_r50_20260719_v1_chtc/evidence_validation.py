...
def validate_resumed_segment_horizon(
    segment: Mapping[str, Any],
    *,
    target_round: int,
    target_new_admissions: int,
) -> dict[str, int]:
    """Validate cumulative depth while allowing an operational continuation."""

    source_round = int(segment.get("source_controller_round", -1))
    new_admissions = int(segment.get("new_admission_records", -1))
    if not (0 <= source_round <= target_round):
        raise ValueError("source controller round is outside the horizon")
    if int(segment.get("final_controller_round", -1)) != target_round:
        raise ValueError("segment did not finish the exact target controller round")
    if source_round + new_admissions != target_round:
        raise ValueError("resumed segment does not close the exact target horizon")
    ...
    return {
        ...
        "segment_new_admissions": new_admissions,
    }
...
def validate_parent_evidence(
    *,
    ...
    segment_closure = validate_resumed_segment_horizon(
        segment,
...
def validate_parent_evidence(
    *,
    ...
    return {
        "controller_rounds": target_round,
        "new_admissions": target_new_admissions,
        "resume_source_controller_round": segment_closure[
            "source_controller_round"
        ],
        "segment_new_admissions": segment_closure["segment_new_admissions"],
        "final_active_depth": final_depth,
        "terminal_checkpoint_sha256": str(last["checkpoint_sha256"]),
        "max_fixed_sector_leakage": max(
            row["fixed_sector_leakage"] for row in validated_checkpoints
