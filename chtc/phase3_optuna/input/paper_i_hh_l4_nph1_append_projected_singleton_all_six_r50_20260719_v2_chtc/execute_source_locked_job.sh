...
BUNDLE_ID="paper_i_hh_l4_nph1_append_projected_singleton_all_six_r50_20260719_v2_chtc"
OUTPUT_ROOT="raw_outputs/${BUNDLE_ID}/${JOB_ID}"
TRANSFER_ARCHIVE="raw_outputs/${BUNDLE_ID}/${JOB_ID}_transfer.tar.gz"

# Condor job identifiers in this bundle include the canonical case token `L4`.
# Keep the identifier path-safe while accepting both ASCII letter cases.
[[ "$JOB_ID" =~ ^[A-Za-z0-9_]+$ ]] || { echo "unsafe job id" >&2; exit 2; }
