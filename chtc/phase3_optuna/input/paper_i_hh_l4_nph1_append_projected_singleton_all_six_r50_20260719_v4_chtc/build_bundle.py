...
def _write_submit(
    ...
    *,
    ...
    lines = [
        "universe = vanilla",
        "batch_name = paper-i-hh-l4-nph1-append-projected-singleton-r50-20260719-v4",
...
def build() -> dict[str, Any]:
    ...
    settings_audit = {
        ...
        "operational_repairs": [
            (
                "wrapper-only job-id validation repair: accept ASCII uppercase letters so the "
                "canonical hh_L4_nph1 identifiers reach the immutable Append-v4 source archive"
            ),
            (
                "resource-gate repair: set GENERIC_STATIC_TABLE_RESOURCE_QUBIT_CAP=12, exactly "
                "matching the intended L=4,n_ph=1 canonical register instead of the default 10"
            ),
            (
                "manifest-lock GENERIC_STATIC_TABLE_EXACT_FIDELITY_MAX_QUBITS=12 so the "
                "required same-cutoff fidelity receipt uses the same declared register limit"
            ),
        ],
    }
    ...
    bundle_manifest = {
        ...
        "operational_successor": {
            "predecessor_bundle_id": (
                "paper_i_hh_l4_nph1_append_projected_singleton_all_six_"
                "r50_20260719_v3_chtc"
            ),
            "predecessor_cluster_id": 8890181,
            "predecessor_failure_stage": "pre-execution review; zero job starts",
            "repair_scope": "manifest-lock both runtime and exact-fidelity caps to 12",
