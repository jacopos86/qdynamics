...
def _refresh_operational_records(
    ...
    successor_receipt_path = BUNDLE_DIR / "operational_successor_receipt.json"
    created_utc = None
    if successor_receipt_path.is_file():
        created_utc = load(successor_receipt_path).get("created_utc")
    if not created_utc:
    ...
    _write_json(
        successor_receipt_path,
        {
            ...
            "repair_scope": [
                "complete_ledger_active_prefix_receipt_closure",
                "explicit_phase2_raw_full_response_contract_failure",
                "focused_bundle_tests",
                "operational_identity_and_artifact_hash_refresh",
            ],
            "known_scientific_blocker": (
                "missing_phase3_supported_rank_on_phase2_raw_rounds_requires_"
                "scientific_route_resolution"
            ),
            "validated_v2_failure_evidence": {
                "proc0_weak_weak": {
                    "raw_occurrences": 277793,
                    "all_branch_s_alg": 220815,
                    "winning_lineage_s_alg": 217387,
                    "discarded_only_s_alg": 3428,
                    "repair_class": "validator_complete_vs_winning_view",
                },
                "proc1_intermediate_weak": {
                    "first_missing_supported_rank_round": 39,
                    "last_missing_supported_rank_round": 50,
                    "fallback_fired": False,
                    "repair_class": "scientific_contract_blocker",
                },
                "proc2_strong_weak": {
                    "first_missing_supported_rank_round": 27,
                    "last_missing_supported_rank_round": 50,
                    "fallback_fired": False,
                    "repair_class": "scientific_contract_blocker",
                },
            },
            "scientific_source_archive_sha256": SOURCE_ARCHIVE_SHA256,
            "scientific_source_archive_unchanged": (
                sha256(BUNDLE_DIR / "source_locked.tar.gz")
                == SOURCE_ARCHIVE_SHA256
            ),
    ...
    manifest["operational_successor"] = {
        "path": successor_receipt_path.name,
        **_artifact_record(successor_receipt_path),
    }
    ...
    inventory[successor_receipt_path.name] = _artifact_record(
        successor_receipt_path
    )
    ...
    artifacts[successor_receipt_path.name] = _artifact_record(
        successor_receipt_path
    )
