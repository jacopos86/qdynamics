...
BUNDLE_ID="paper_i_hh_sr_snake_noprune_nobeam_no_ordinary_novelty_strong_weak_u8_r50_repair_20260716_v2_chtc"
...
if [[ "$(sha256_file "$SOURCE_ARCHIVE")" != "$EXPECTED_SOURCE_SHA256" ]]; then
  echo "Source archive hash mismatch" >&2
  exit 3
fi
if [[ "$(sha256_file "$IMAGE")" != "$EXPECTED_IMAGE_SHA256" ]]; then
