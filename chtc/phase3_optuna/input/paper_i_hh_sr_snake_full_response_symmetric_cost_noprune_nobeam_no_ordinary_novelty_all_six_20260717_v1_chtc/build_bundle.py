#!/usr/bin/env python3
"""Build the source-locked six-regime SR-SNAKE symmetric-cost bundle.

This builder performs no scientific calculation and never submits CHTC jobs.
It snapshots the exact locally tested source tree, resolves the explicit
no-prune/no-beam/no-ordinary-novelty profile from that isolated archive,
verifies all six same-cutoff physics anchors, and writes six fresh round-0
jobs plus a fail-closed submission preflight.
"""
...
BATCH_NAME = (
    "paper-i-hh-sr-fullresp-symcost-noprune-nobeam-"
    "nonovelty-six-20260717-v1"
)
BUNDLE_DIR = Path(__file__).resolve().parent
REPO = BUNDLE_DIR.parents[3]
# Git commit/tree identify ancestry only. The deliberately dirty, locally tested
# live source snapshot plus its complete per-file archive hashes are executable
# authority; worker validation never imports from the live tree.
...
SOURCE_LOCK_STATE = "hash_locked_locally_tested_worktree_snapshot_v1"
...
CRITICAL_SOURCE_SHA256 = {
    "pipelines/static_adapt/adapt_pipeline.py": "5249c8a97b7d8983a414af6bf28cb84a60e0856f3373b5e4b688e7acbac436e2",
    ...
    "test/test_static_adapt_historical_singleton_overlays.py": "da4eba7cf7d497967d1e2fb8e33aab5d2001e7f5f1ca7c22c0cb196cfe22fd03",
    ...
    "test/test_static_adapt_accepted_refit.py": "61daa36df4b805ab6ef68dc792f942934255677ba1bfe3b8be48a5e731922f34",
...
REGIMES: tuple[dict[str, Any], ...] = (
    {
        ...
        "memory_mb": 32768, "disk_mb": 61440, "target_round": 30,
    },
    {
...
def snapshot_file_paths() -> tuple[str, ...]:
    ...
    selected = {
        token.decode("utf-8") for token in raw.split(b"\0") if token
    }
...
def exact_source_tar_bytes() -> bytes:
    """Materialize a deterministic tar from the hash-locked tested worktree."""

    buffer = io.BytesIO()
    with tarfile.open(fileobj=buffer, mode="w") as handle:
        for relative in snapshot_file_paths():
            path = REPO / relative
...
def verify_source_lock() -> tuple[dict[str, Any], dict[str, Any]]:
    if len(EXPECTED_HEAD) != 40 or len(EXPECTED_TREE) != 40:
        raise ValueError(
            "source authority is not frozen: replace EXPECTED_HEAD and "
    ...
    required_untracked = verify_required_untracked_source_modules()
    critical = {}
    ...
    for path, expected in CRITICAL_SOURCE_SHA256.items():
        ...
        if actual != expected:
            raise ValueError(
                f"critical live-source drift: {path}: expected {expected}, got {actual}"
            )
    ...
    revision = {
        ...
        "executable_source_authority": (
            "hash_locked_live_worktree_snapshot_plus_complete_file_inventory_v1"
        ),
        "dirty_live_source_lock": True,
        ...
        "required_untracked_source_modules": {
            ...
            for relative, digest in required_untracked.items()
        },
...
def build_source_archive() -> dict[str, Any]:
    ...
    for path, expected in required_untracked.items():
        if members.get(path, {}).get("sha256") != expected:
            raise ValueError(
                f"required untracked source module missing/drifted in archive: {path}"
            )
    return {
        ...
        "git_role": "base_ancestry_metadata_only",
        "executable_source_authority": (
            "archive_sha256_plus_complete_per_file_sha256_inventory_v1"
        ),
        "worker_source_mode": "hash_locked_live_worktree_snapshot_v1",
...
def job_command(row: dict[str, Any]) -> tuple[list[str], dict[str, str]]:
    ...
    argv = [
        ...
        "--adapt-segment-id", (
            f"{slug}-sr-symcost-noprune-r0-r{target_round}-20260717-v1"
        ),
...
def build_job(
    ...
    return {
        "schema": "paper_i_hh_sr_symcost_noprune_candidate_job_v1",
        "bundle_id": BUNDLE_ID,
...
def _retired_cross_bundle_settings_diff_validator(
    *,
    ...
    return {
        "schema": "retired_candidate_parent_job_v1",
        "bundle_id": BUNDLE_ID,
        "batch_name": BATCH_NAME,
        "run_class": "candidate",
        "regime_slug": slug,
...
def main() -> int:
    ...
    for row in REGIMES:
        ...
        dump_json(normalized_path, {
            "schema": "paper_i_hh_sr_symcost_noprune_normalized_parent_manifest_v1",
            "bundle_id": BUNDLE_ID,
            "regime_slug": slug,
    ...
    if (
        ...
        raise ValueError("candidate bundle must queue exactly all six regimes")
    ...
    parity = {
        "schema": "paper_i_hh_sr_symcost_noprune_route_parity_v1",
        "status": "pass",
        "profile_request": PROFILE_REQUEST,
        "profile_resolved": PROFILE_RESOLVED,
        "profile_contract_sha256": PROFILE_CONTRACT_SHA256,
    ...
    bundle_manifest = {
        "schema": "paper_i_hh_sr_symcost_noprune_candidate_bundle_v1",
        "bundle_id": BUNDLE_ID,
        "batch_name": BATCH_NAME,
        "created_utc": utc_now(),
        "run_class": "candidate",
        "parent_stage": "fresh_round0_exact_per_regime_horizon_all_six",
        "submission_scope": {
    ...
    preflight = {
        "schema": "paper_i_hh_sr_symcost_noprune_candidate_preflight_v1",
        "created_utc": utc_now(),
        "status": (
        ...
        "checks": {
            ...
            "dirty_live_source_archive_is_executable_authority": True,
            "critical_source_hashes": True,
            "source_archive_safe_and_closed": True,
            "hash_locked_live_snapshot_plus_hashed_nonscientific_overlays": (
