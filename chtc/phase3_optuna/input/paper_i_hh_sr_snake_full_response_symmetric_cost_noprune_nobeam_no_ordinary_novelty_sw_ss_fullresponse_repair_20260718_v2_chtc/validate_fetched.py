...
def validate(root: Path) -> dict[str, Any]:
    ...
    if target_round not in {30, 50} or target_new_admissions != target_round:
        raise ValueError("fetched segment horizon drift")
...
def main() -> int:
    ...
    if args.path.is_file():
        with tempfile.TemporaryDirectory(prefix="sr_symcost_noprune_fetch_") as tmp:
            root = Path(tmp)
