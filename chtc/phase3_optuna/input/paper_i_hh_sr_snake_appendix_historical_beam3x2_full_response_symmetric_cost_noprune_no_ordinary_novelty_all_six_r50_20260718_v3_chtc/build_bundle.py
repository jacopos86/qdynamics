...
BUNDLE_ID = (
    ...
    "symmetric_cost_noprune_no_ordinary_novelty_all_six_r50_20260718_v3_chtc"
)
BATCH_NAME = (
    "paper-i-hh-sr-appendix-historical-beam3x2-fullresp-symcost-"
    "noprune-nonovelty-six-r50-20260718-v3"
)
...
S_ALG_OVERLAY_PATH = BUNDLE_DIR / "s_alg_accounting_repair_overlay.patch"
...
CRITICAL_SOURCE_SHA256 = {
    "pipelines/static_adapt/adapt_pipeline.py": S_ALG_OVERLAY_FILE_HASHES[
        "pipelines/static_adapt/adapt_pipeline.py"
    ],
    ...
    "pipelines/static_adapt/resume_scaffold.py": S_ALG_OVERLAY_FILE_HASHES[
        "pipelines/static_adapt/resume_scaffold.py"
    ],
    ...
    "pipelines/scaffold/hh_continuation_scoring.py": S_ALG_OVERLAY_FILE_HASHES[
        "pipelines/scaffold/hh_continuation_scoring.py"
    ],
    ...
    "test/test_hh_continuation_scoring.py": S_ALG_OVERLAY_FILE_HASHES[
        "test/test_hh_continuation_scoring.py"
    ],
    ...
    "test/test_static_adapt_resume_scaffold.py": S_ALG_OVERLAY_FILE_HASHES[
        "test/test_static_adapt_resume_scaffold.py"
    ],
    "test/test_ground_space_fidelity.py": (
        "55ff094aca73f59362886cb5b951c9ab4b70eff2733f9e4061970be88836a8bf"
...
def derived_source_files() -> dict[str, tuple[bytes, int]]:
    ...
    with tempfile.TemporaryDirectory(prefix="sr_historical_beam_overlay_") as tmp:
        ...
        if sha256(S_ALG_OVERLAY_PATH) != S_ALG_OVERLAY_SHA256:
            raise ValueError("S_alg accounting repair overlay hash drift")
        completed = subprocess.run(
            ["/usr/bin/patch", "-p1", "--fuzz=0", "--batch", "--forward"],
            cwd=root,
            input=S_ALG_OVERLAY_PATH.read_text(encoding="utf-8"),
            capture_output=True,
            text=True,
            check=False,
        )
        if completed.returncode != 0:
            raise ValueError(
                "S_alg accounting repair overlay failed to apply exactly: "
                + completed.stdout.strip()
                + " "
                + completed.stderr.strip()
            )
        for relative, expected in S_ALG_OVERLAY_FILE_HASHES.items():
            destination = root / relative
            if not destination.is_file() or destination.is_symlink():
                raise ValueError(f"S_alg accounting overlay omitted {relative}")
            data = destination.read_bytes()
            if bytes_sha256(data) != expected:
                raise ValueError(f"S_alg accounting overlay result drift: {relative}")
            prior_mode = files.get(relative, (b"", 0o644))[1]
            files[relative] = (data, int(prior_mode or 0o644))
    return files
...
def verify_source_lock() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    revision = {
        ...
        "s_alg_accounting_repair_overlay": {
            "classification": "non_scientific_query_accounting_repair_v2",
            "path": S_ALG_OVERLAY_PATH.relative_to(REPO).as_posix(),
            "overlay_sha256": S_ALG_OVERLAY_SHA256,
            "derived_file_sha256": dict(S_ALG_OVERLAY_FILE_HASHES),
            "scientific_route_digest_unchanged": True,
            "repairs": [
                "physical_tangent_identity_and_reuse",
                "phase3_active_gradient_query_receipts",
                "adaptive_trust_endpoint_overlap_receipts",
                "atomic_checkpoint_ledger_and_resume_cursor",
                "ordinary_novelty_off_null_zero_query_telemetry",
            ],
        },
...
def job_command(row: dict[str, Any]) -> tuple[list[str], dict[str, str]]:
    ...
    argv = [
        ...
        "--adapt-segment-id", (
            f"{slug}-sr-appendix-historical-beam3x2-r0-r{target_round}-20260718-v3"
        ),
...
def main() -> int:
    ...
    preflight_checks = {
            ...
            "immutable_parent_plus_hash_locked_overlay_inventory": (
                ...
                == BEAM_OVERLAY_FILE_HASHES
                and archive["s_alg_accounting_repair_overlay"]["overlay_sha256"]
                == S_ALG_OVERLAY_SHA256
                and archive["s_alg_accounting_repair_overlay"][
                    "derived_file_sha256"
                ]
                == S_ALG_OVERLAY_FILE_HASHES
            ...
            "historical_beam_overlay_result_hashes_closed": all(
                ...
                if relative not in S_ALG_OVERLAY_FILE_HASHES
            ),
            "s_alg_accounting_overlay_result_hashes_closed": all(
                archive["files"].get(relative, {}).get("sha256") == digest
                for relative, digest in S_ALG_OVERLAY_FILE_HASHES.items()
            ),
