#!/usr/bin/env python3
"""Fail-closed tests for the all-six historical-beam appendix bundle."""
...
class FrozenBundleTests(unittest.TestCase):
    def test_worker_wrapper_uses_this_successor_bundle_identity(self) -> None:
        ...
        self.assertNotIn(
            'BUNDLE_ID="paper_i_hh_sr_snake_main_full_response_', wrapper
        )
        self.assertIn(
    ...
    def test_source_freeze_is_complete_and_submission_is_enabled(self) -> None:
        ...
        self.assertEqual(
            archive_manifest["historical_beam_overlay"]["overlay_sha256"],
            spec.BEAM_OVERLAY_SHA256,
        )
        self.assertEqual(
            archive_manifest["historical_beam_overlay"]["derived_file_sha256"],
            spec.BEAM_OVERLAY_FILE_HASHES,
        )
        self.assertEqual(
            archive_manifest["s_alg_accounting_repair_overlay"]["overlay_sha256"],
            spec.S_ALG_OVERLAY_SHA256,
        )
        self.assertEqual(
            archive_manifest["s_alg_accounting_repair_overlay"][
                "derived_file_sha256"
            ],
            spec.S_ALG_OVERLAY_FILE_HASHES,
        )
    ...
    def test_route_is_exact_historical_3x2_no_prune_no_batch_no_novelty(self) -> None:
        for slug in EXPECTED_PHYSICS:
            job = load(BUNDLE / "jobs" / f"{slug}.json")
            ...
            self.assertEqual(execution["adapt_beam_live_branches"], 3)
            self.assertEqual(execution["adapt_beam_children_per_parent"], 2)
            self.assertEqual(execution["adapt_beam_terminated_keep"], 3)
            self.assertEqual(execution["adapt_beam_terminal_archive_mode"], "legacy")
            ...
            self.assertEqual(semantics["beam_shape"], "historical_3x2_v1")
            self.assertTrue(semantics["beam_parent_stop_terminal_also_materialized"])
            self.assertEqual(
                semantics["beam_structural_mode"], "stop_or_single_admission"
            )
            self.assertEqual(
                semantics["beam_terminal_archive_accumulation"],
                "cumulative_across_rounds",
            )
            self.assertEqual(semantics["beam_terminal_archive_cap"], 3)
    ...
    def test_source_locked_ledger_deserializer_rejects_payload_mutations(self) -> None:
        archive_manifest = load(BUNDLE / "source_archive_manifest.json")
        module = fetched_validator.load_source_locked_estimator_ledger_module(
            BUNDLE / "source_locked.tar.gz",
            archive_manifest,
        )
        source_record = archive_manifest["files"][
            fetched_validator.ESTIMATOR_LEDGER_SOURCE_PATH
        ]
        self.assertEqual(
            module.__source_file_sha256__,
            source_record["sha256"],
        )
        ledger = module.EstimatorCallLedger()
        identity = module.EstimatorCallKey(
            projective_state_fingerprint="projective-state:test",
            hamiltonian_fingerprint="hamiltonian:test",
            backend_fingerprint="backend:test",
            precision_contract="exact:test",
            primitive_kind="energy",
            observable_or_formula_identity="formula:test",
        )
        ledger.record_call(
            identity,
            component="N_H_outer",
            consumer_scope="bundle_mutation_test",
        )
        payload = ledger.to_payload()
        receipt = evidence_validation.strict_validate_ledger_payload(
            payload,
            estimator_ledger_class=module.EstimatorCallLedger,
        )
        self.assertTrue(receipt["performed"])

        bad_fingerprint = copy.deepcopy(payload)
        bad_fingerprint["ledger_fingerprint"] = "0" * 64
        with self.assertRaisesRegex(ValueError, "fingerprint"):
            evidence_validation.strict_validate_ledger_payload(
                bad_fingerprint,
                estimator_ledger_class=module.EstimatorCallLedger,
            )

        bad_identity = copy.deepcopy(payload)
        bad_identity["entries"][0]["identity"][
    ...
    def test_fetched_ledger_pointer_rejects_hash_and_size_mutations(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sr_ledger_pointer_test_") as tmp:
            ledger_path = Path(tmp) / "estimator_call_ledger.json"
            ledger_path.write_bytes(b'{"ledger":"test"}\n')
            actual_sha = fetched_validator.sha256(ledger_path)
            actual_size = ledger_path.stat().st_size
            result = {
                "adapt_vqe": {
                    "estimator_call_accounting": {
                        "sidecar": {
                            "schema": (
                                fetched_validator.ESTIMATOR_LEDGER_POINTER_SCHEMA
                            ),
                            "path": f"json/{ledger_path.name}",
                            "sha256": actual_sha,
            ...
            receipt = fetched_validator.validate_ledger_sidecar_pointer(
                result,
                ledger_path,
            )
            self.assertEqual(receipt["sha256"], actual_sha)
            self.assertEqual(receipt["size_bytes"], actual_size)

            bad_hash = copy.deepcopy(result)
            bad_hash["adapt_vqe"]["estimator_call_accounting"]["sidecar"][
                "sha256"
            ] = "0" * 64
            with self.assertRaisesRegex(ValueError, "SHA-256"):
                fetched_validator.validate_ledger_sidecar_pointer(
                    bad_hash,
                    ledger_path,
                )

            bad_size = copy.deepcopy(result)
            bad_size["adapt_vqe"]["estimator_call_accounting"]["sidecar"][
                "size_bytes"
            ] = actual_size + 1
    ...
    def test_derivation_changes_only_declared_overlay_paths(self) -> None:
        ...
        allowed = set(spec.BEAM_OVERLAY_FILE_HASHES) | set(
            spec.S_ALG_OVERLAY_FILE_HASHES
        )
        ...
        for relative, expected in spec.BEAM_OVERLAY_FILE_HASHES.items():
            if relative not in spec.S_ALG_OVERLAY_FILE_HASHES:
                self.assertEqual(spec.bytes_sha256(derived[relative][0]), expected)
        for relative, expected in spec.S_ALG_OVERLAY_FILE_HASHES.items():
            self.assertEqual(spec.bytes_sha256(derived[relative][0]), expected)
        self.assertEqual(spec.sha256(spec.PARENT_ARCHIVE_PATH), spec.PARENT_ARCHIVE_SHA256)
        self.assertEqual(
            spec.sha256(spec.BEAM_OVERLAY_PATH),
            spec.BEAM_OVERLAY_SHA256,
        )
        self.assertEqual(
            spec.sha256(spec.S_ALG_OVERLAY_PATH),
            spec.S_ALG_OVERLAY_SHA256,
        )
    ...
    def test_final_build_job_matches_worker_schema_and_argv(self) -> None:
        ...
        self.assertEqual(job["run_class"], "appendix_historical_beam_ablation_matrix")
