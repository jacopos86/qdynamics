...
two narrow, hash-locked overlays in order:

- `historical_beam_exact_semantics_overlay.patch` supplies the sole scientific
  route change, the historical 3x2 beam contract;
- `s_alg_accounting_repair_overlay.patch` supplies only physical-tangent query
  identity/reuse, active-gradient and endpoint-overlap receipts, atomic
  checkpoint-ledger resume state, strict ledger telemetry, and ordinary
  novelty-off null telemetry.

The derived archive never imports scientific modules from the live source tree,
and the parent, v1, and v2 appendix bundles are never modified. This v3
successor changes only its wrapper/output bundle identity so the transferred
worker invokes its own `run_job.py`. Its source archive is byte-identical to v2,
and its route digest remains exactly the v1/v2 historical-beam digest.
...
- ordinary novelty is not evaluated and is not represented by an exponent-zero
  multiplication: its values are null, query charge and classical solve count
  are zero, and only the explicitly retained all-models-infeasible fallback may
  compute deferred Gram novelty;
