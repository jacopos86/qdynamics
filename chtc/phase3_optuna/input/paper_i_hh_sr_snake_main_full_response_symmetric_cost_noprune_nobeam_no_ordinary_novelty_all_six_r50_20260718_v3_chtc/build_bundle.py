#!/usr/bin/env python3
"""Build the source-locked six-regime SR-SNAKE round-50 main bundle.

This builder performs no scientific calculation and never submits CHTC jobs.
It derives from an immutable, previously executed source archive, applies one
hash-locked telemetry-only estimator-receipt overlay, resolves the explicit
no-prune/no-beam/no-ordinary-novelty profile from that isolated archive,
verifies all six same-cutoff physics anchors, and writes six fresh round-0 to
round-50 rows plus a fail-closed submission preflight.
"""
...
BUNDLE_ID = (
    ...
    "no_ordinary_novelty_all_six_r50_20260718_v3_chtc"
)
BATCH_NAME = (
    "paper-i-hh-sr-main-fullresp-symcost-noprune-nobeam-"
    "nonovelty-six-r50-20260718-v3"
)
PREDECESSOR_BUNDLE_ID = (
    "paper_i_hh_sr_snake_main_full_response_symmetric_cost_noprune_nobeam_"
    "no_ordinary_novelty_all_six_r50_20260718_v2_chtc"
)
PREDECESSOR_BATCH_NAME = (
    "paper-i-hh-sr-main-fullresp-symcost-noprune-nobeam-"
    "nonovelty-six-r50-20260718-v2"
)
...
FINAL_JOB_SCHEMA = "paper_i_hh_sr_main_all_six_r50_job_v1"
...
PARENT_BUNDLE_DIR = (
    ...
    "no_ordinary_novelty_sw_ss_fullresponse_repair_20260718_v2_chtc"
)
...
ADAPT_PIPELINE_PATH = "pipelines/static_adapt/adapt_pipeline.py"
PARENT_ADAPT_PIPELINE_SHA256 = (
    "88b7de1e762db9f5d87a37228b9b166faf9a82d373d9f62edcc07bc194ce63a2"
)
DERIVED_ADAPT_PIPELINE_SHA256 = (
    "81723a773946ff0aabea3c57f3ea12fff2808e935a7dc38ad2c01b472c3fa7fc"
)
ADAPT_PIPELINE_OVERLAY_PATH = (
    BUNDLE_DIR / "active_prefix_estimator_receipts_overlay.patch"
)
ADAPT_PIPELINE_OVERLAY_SHA256 = (
    "df47bcc4d9aae9f4bbd2ac3d9901dd8fa14f8ba7f638dc87b4f97151572ea4e2"
)
ACCEPTED_REFIT_TEST_PATH = "test/test_static_adapt_accepted_refit.py"
PARENT_ACCEPTED_REFIT_TEST_SHA256 = (
    "72f7bd0f0b336399a7a0b860546ee27e09c00bddd8f6a7a1d86ec6431cd7f76d"
)
DERIVED_ACCEPTED_REFIT_TEST_SHA256 = (
    "ee4874747d3dbe11335f7544b84544a6d7294c070e81b72883566f20573849e4"
)
ACCEPTED_REFIT_TEST_OVERLAY_PATH = (
    BUNDLE_DIR / "no_beam_active_prefix_receipt_test.patch"
)
ACCEPTED_REFIT_TEST_OVERLAY_SHA256 = (
    "2bc688b288d15326e78e0beb617dc9b5180386446463a9762becda5dd1a37921"
)
SOURCE_LOCK_STATE = "immutable_parent_archive_plus_hash_locked_overlay_v1"
...
SCIENTIFIC_SUBMISSION_BLOCKER = (
    "frozen source can enter a phase2_raw/eps_grad_suppressed_continue path "
    "without executing the declared full-active-plus-singleton Phase-III "
    "response on every controller round; missing supported rank on those "
    "rounds is not an all-energy-models-infeasible fallback receipt and "
    "cannot be repaired by evidence-validation or telemetry plumbing"
)
...
CRITICAL_SOURCE_SHA256 = {
    "pipelines/static_adapt/adapt_pipeline.py": DERIVED_ADAPT_PIPELINE_SHA256,
    ...
    "pipelines/static_adapt/sr_snake_route_profile.py": "131c37f2ab06eb6911b57251e35471a7378e2935e501c0ff370d92741ecf45d5",
    ...
    "test/test_static_adapt_accepted_refit.py": DERIVED_ACCEPTED_REFIT_TEST_SHA256,
    "test/test_static_adapt_estimator_call_ledger.py": (
    ...
    "agent_guidance/static-adapt/route-identities.md": "e5a916b25e33f29ea491db9e01abf211261393e590d7f87cfa7077f74789340c",
    "agent_guidance/shared/run-guide.md": "5b69a0655af49e449b8ac9f6dc4c12a8ac027cda700ca8a3551eaea83cba4ff9",
    "MATH/paper_facing/paper_I_static_scaffold/paper_i_sr_snake_no_prune_symmetric_cost_candidate_runtime_settings_20260717.md": "bdee7843e9a990358cbd1d50c19567938d1e68b8c5ad161b66d1794d611686a2",
}
...
def _apply_adapt_pipeline_overlay(parent_data: bytes) -> bytes:
    """Apply only the approved estimator-receipt telemetry overlay."""

    if bytes_sha256(parent_data) != PARENT_ADAPT_PIPELINE_SHA256:
        raise ValueError("parent adapt_pipeline.py hash drift")
    if sha256(ADAPT_PIPELINE_OVERLAY_PATH) != ADAPT_PIPELINE_OVERLAY_SHA256:
        raise ValueError("active-prefix estimator overlay hash drift")
    with tempfile.TemporaryDirectory(prefix="sr_main_adapt_overlay_") as tmp:
        root = Path(tmp)
        destination = root / ADAPT_PIPELINE_PATH
        destination.parent.mkdir(parents=True)
        destination.write_bytes(parent_data)
        completed = subprocess.run(
            ["/usr/bin/patch", "-p1", "--fuzz=0", "--batch", "--forward"],
            cwd=root,
            input=ADAPT_PIPELINE_OVERLAY_PATH.read_text(encoding="utf-8"),
            capture_output=True,
            text=True,
            check=False,
        )
        if completed.returncode != 0:
            raise ValueError(
                "active-prefix estimator overlay failed to apply exactly: "
                + completed.stderr.strip()
            )
        derived = destination.read_bytes()
    if bytes_sha256(derived) != DERIVED_ADAPT_PIPELINE_SHA256:
        raise ValueError("derived adapt_pipeline.py hash drift")
    return derived


def _apply_no_beam_receipt_test_overlay(parent_data: bytes) -> bytes:
    """Add the focused no-beam receipt/terminal-closure regression only."""

    if bytes_sha256(parent_data) != PARENT_ACCEPTED_REFIT_TEST_SHA256:
        raise ValueError("parent accepted-refit test hash drift")
    if (
        sha256(ACCEPTED_REFIT_TEST_OVERLAY_PATH)
        != ACCEPTED_REFIT_TEST_OVERLAY_SHA256
    ):
        raise ValueError("no-beam estimator-receipt test overlay hash drift")
    with tempfile.TemporaryDirectory(prefix="sr_main_test_overlay_") as tmp:
        root = Path(tmp)
        destination = root / ACCEPTED_REFIT_TEST_PATH
        destination.parent.mkdir(parents=True)
        destination.write_bytes(parent_data)
        completed = subprocess.run(
            ["/usr/bin/patch", "-p1", "--fuzz=0", "--batch", "--forward"],
            cwd=root,
            input=ACCEPTED_REFIT_TEST_OVERLAY_PATH.read_text(encoding="utf-8"),
            capture_output=True,
            text=True,
            check=False,
        )
        if completed.returncode != 0:
            raise ValueError(
                "no-beam estimator-receipt test overlay failed to apply exactly: "
                + completed.stderr.strip()
            )
        derived = destination.read_bytes()
    if bytes_sha256(derived) != DERIVED_ACCEPTED_REFIT_TEST_SHA256:
        raise ValueError("derived accepted-refit test hash drift")
    return derived


def derived_source_files() -> dict[str, tuple[bytes, int]]:
    """Derive the exact worker source without reading live scientific modules."""

    files = _safe_parent_archive_files()
    parent_adapt, parent_mode = files[ADAPT_PIPELINE_PATH]
    files[ADAPT_PIPELINE_PATH] = (
        _apply_adapt_pipeline_overlay(parent_adapt),
        parent_mode,
    )
    parent_test, parent_test_mode = files[ACCEPTED_REFIT_TEST_PATH]
    files[ACCEPTED_REFIT_TEST_PATH] = (
        _apply_no_beam_receipt_test_overlay(parent_test),
        parent_test_mode,
    )
    overlays = {
        **REQUIRED_HASH_LOCKED_FIDELITY_FILES,
        "test/test_static_adapt_estimator_call_ledger.py": (
            CRITICAL_SOURCE_SHA256["test/test_static_adapt_estimator_call_ledger.py"]
        ),
    }
    for relative, expected in overlays.items():
        source = REPO / relative
        if not source.is_file() or source.is_symlink():
            raise ValueError(f"hash-locked reporting/test overlay missing: {relative}")
        data = source.read_bytes()
        if bytes_sha256(data) != expected:
            raise ValueError(f"hash-locked reporting/test overlay drift: {relative}")
        files[relative] = (data, 0o755 if os.access(source, os.X_OK) else 0o644)
    return files
...
def verify_source_lock() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    revision = {
        "schema": "paper_i_hh_sr_symcost_noprune_source_revision_v1",
        ...
        "adapt_pipeline_overlay": {
            "classification": "telemetry_only_non_scientific_v1",
            "path": ADAPT_PIPELINE_OVERLAY_PATH.relative_to(REPO).as_posix(),
            "overlay_sha256": ADAPT_PIPELINE_OVERLAY_SHA256,
            "parent_file_sha256": PARENT_ADAPT_PIPELINE_SHA256,
            "derived_file_sha256": DERIVED_ADAPT_PIPELINE_SHA256,
            "exact_hunk_anchors": [
                "active_prefix_estimator_ledger_receipt_definition",
                "active_prefix_checkpoint_estimator_receipt_attachment",
                "terminal_active_prefix_estimator_ledger_closure",
            ],
        },
    ...
    physics_lock = {
        "schema": "paper_i_hh_sr_symcost_noprune_physics_exact_reference_lock_v1",
...
def build_source_archive() -> dict[str, Any]:
    ...
    return {
        "schema": "paper_i_hh_sr_symcost_noprune_source_archive_manifest_v1",
...
"""
        ...
        return {
            "schema": "paper_i_hh_sr_symcost_noprune_archive_only_preflight_v1",
            "status": (
...
def job_command(row: dict[str, Any]) -> tuple[list[str], dict[str, str]]:
    ...
    argv = [
        ...
        "--adapt-segment-id", (
            f"{slug}-sr-main-symcost-noprune-r0-r{target_round}-20260718-v1"
        ),
...
def _retired_cross_bundle_settings_diff_validator(
    *,
    contract: dict[str, Any],
    jobs: list[dict[str, Any]],
) -> dict[str, Any]:
    baseline_contract = load_json(BUNDLE_DIR / "jobs/weak_weak.json")[
        "route_identity"
    ]["profile_contract"]
    ...
    if observed_contract_paths != ALLOWED_V2_TO_V3_CONTRACT_DIFF_PATHS:
        raise ValueError(
            "unexpected retired cross-bundle route-contract drift: "
    ...
    for job in jobs:
        ...
        baseline = load_json(BUNDLE_DIR / "jobs" / f"{slug}.json")
        ...
        if not all(checks.values()):
            raise ValueError(f"retired cross-bundle job drift for {slug}: {checks}")
        row_audits.append({
            "regime_slug": slug,
            "checks": checks,
    ...
    return {
        "schema": "retired_cross_bundle_scientific_settings_diff_v1",
        "baseline_bundle": BUNDLE_DIR.relative_to(REPO).as_posix(),
...
def validate_candidate_bundle_contract(
    *, contract: dict[str, Any], jobs: list[dict[str, Any]]
    ...
    return {
        "schema": "paper_i_hh_sr_symcost_noprune_scientific_settings_audit_v1",
        "status": "pass",
        "profile_request": PROFILE_REQUEST,
        "profile_resolved": PROFILE_RESOLVED,
        "profile_contract_sha256": PROFILE_CONTRACT_SHA256,
...
transfer_input_files = {base}/run_job.py, {base}/evidence_validation.py, {base}/validate_fetched.py, {base}/source_archive_manifest.json, {base}/source_revision_manifest.json, {base}/physics_and_exact_reference_lock.json, {base}/bundle_manifest.json, {base}/preflight.json, {base}/route_parity.json, {base}/scientific_settings_audit.json, {base}/operational_successor_receipt.json, $(job_manifest), $(normalized_manifest), {base}/source_locked.tar.gz, chtc/phase3_optuna/image.sif
...
def smoke_summary() -> dict[str, Any]:
    ...
    scientific_validation = {
        "status": "pre_overlay_route_smoke_reference_only",
        "note": (
            "The immutable depth-8 smoke predates active-prefix receipt telemetry; "
            "the archive-only no-beam regression is authoritative for that overlay."
        ),
    ...
    return {
        ...
        "note": (
            ...
            "no-prune, no-beam, and estimator-accounting health."
        ),
    }
...
def refresh_and_verify_operational_successor() -> dict[str, Any]:
    """Refresh only v2 operational provenance; never rebuild scientific source.

    The original builder depends on unrelated live-tree hash-locked reporting
    overlays.  This successor therefore reuses the already frozen v1 bundle
    byte-for-byte for scientific inputs and changes only the bundle identity,
    validator, focused tests, and their artifact hashes.
    """

    archive_path = BUNDLE_DIR / "source_locked.tar.gz"
    checks: dict[str, bool] = {
        "source_archive_byte_identical": sha256(archive_path)
        == "fa9014b9608ccb8a301df0429268482abf6dd10c91eb336111a15d00be256d35",
    }
    expected_slugs = {str(row["slug"]) for row in REGIMES}
    job_paths = sorted((BUNDLE_DIR / "jobs").glob("*.json"))
    normalized_paths = sorted((BUNDLE_DIR / "normalized_manifests").glob("*.json"))
    checks["six_jobs"] = {path.stem for path in job_paths} == expected_slugs
    checks["six_normalized_manifests"] = (
        {path.stem for path in normalized_paths} == expected_slugs
    )
    row_checks: list[dict[str, Any]] = []
    for path in job_paths + normalized_paths:
        payload = load_json(path)
        route = dict(payload.get("route_identity", {}))
        source = dict(payload.get("source_lock", {}))
        segment = dict(payload.get("segment", {}))
        ...
        row_checks.append(
            {
                ...
                "pass": (
                    ...
                    and BUNDLE_ID in encoded
                ),
            }
        )
    checks["row_identity_and_contract"] = all(row["pass"] for row in row_checks)
    submit_text = (BUNDLE_DIR / "submit.sub").read_text(encoding="utf-8")
    checks["submit_identity"] = (
        BUNDLE_ID in submit_text
        and BATCH_NAME in submit_text
        and PREDECESSOR_BUNDLE_ID not in submit_text
        and PREDECESSOR_BATCH_NAME not in submit_text
    )
    checks["submission_fail_closed"] = (
        not SUBMISSION_ENABLED and "requirements = False" in submit_text
    )

    receipt_path = BUNDLE_DIR / "operational_successor_receipt.json"
    created_utc = utc_now()
    if receipt_path.is_file():
        created_utc = str(load_json(receipt_path).get("created_utc") or created_utc)
    receipt = {
        "schema": "paper_i_hh_sr_operational_successor_receipt_v1",
        "created_utc": created_utc,
        "predecessor_bundle_id": PREDECESSOR_BUNDLE_ID,
        "predecessor_batch_name": PREDECESSOR_BATCH_NAME,
        "successor_bundle_id": BUNDLE_ID,
        "successor_batch_name": BATCH_NAME,
        "repair_scope": [
            "accepted_record_fallback_receipt_reconstruction",
            "fallback_aware_supported_rank_evidence_validation",
            "focused_bundle_tests",
            "operational_identity_and_artifact_hash_refresh",
        ],
        "scientific_source_archive_sha256": sha256(archive_path),
        "scientific_source_archive_unchanged": bool(
            checks["source_archive_byte_identical"]
        ),
        "route_contract_sha256": PROFILE_CONTRACT_SHA256,
        "route_contract_unchanged": True,
        "scientific_settings_changed": False,
        "scientific_validation_status": (
            "blocked_phase3_response_not_executed_every_round"
        ),
        "submission_blocked": True,
        "submission_blocker": SCIENTIFIC_SUBMISSION_BLOCKER,
        "submission_performed": False,
    }
    ...
    manifest["operational_successor"] = {
        "path": receipt_path.relative_to(REPO).as_posix(),
        "sha256": sha256(receipt_path),
    }
    manifest["submission_status"] = "blocked_scientific_contract_mismatch"
    manifest["scientific_blockers"] = [SCIENTIFIC_SUBMISSION_BLOCKER]
    manifest["failed_preflight_checks"] = ["submission_enabled"]
    dump_json(manifest_path, manifest)

    upload_path = BUNDLE_DIR / "upload_artifact_list.txt"
    upload_lines = [
        line for line in upload_path.read_text(encoding="utf-8").splitlines()
        if line.strip()
    ]
    receipt_relative = receipt_path.relative_to(REPO).as_posix()
    if receipt_relative not in upload_lines:
        upload_lines.append(receipt_relative)
    ...
    return {
        "schema": "paper_i_hh_sr_operational_successor_verification_v1",
...
def main() -> int:
    ...
    for row in REGIMES:
        ...
        dump_json(normalized_path, {
            "schema": "paper_i_hh_sr_main_all_six_r50_normalized_manifest_v1",
            "bundle_id": BUNDLE_ID,
            "regime_slug": slug,
    ...
    if (
        ...
        raise ValueError(
            "main bundle must queue exactly all six locked regimes"
        )
    ...
    parity = {
        "schema": "paper_i_hh_sr_main_all_six_r50_route_parity_v1",
        "status": "pass",
        "profile_request": PROFILE_REQUEST,
        "profile_resolved": PROFILE_RESOLVED,
        "profile_contract_sha256": PROFILE_CONTRACT_SHA256,
    ...
    bundle_manifest = {
        "schema": "paper_i_hh_sr_main_all_six_r50_bundle_v1",
        "bundle_id": BUNDLE_ID,
        "batch_name": BATCH_NAME,
        "created_utc": utc_now(),
        "run_class": "main_route_matrix",
        "parent_stage": "fresh_round0_to_round50_all_six_main_route_v1",
        "submission_scope": {
            ...
            "per_regime_horizons": expected_horizons,
            "scientific_contract": (
                "full_response_symmetric_cost_no_prune_no_beam_"
    ...
    preflight_checks = {
            ...
            "beam_disabled_effective_1x1": True,
    ...
    preflight = {
        "schema": "paper_i_hh_sr_main_all_six_r50_preflight_v1",
        "created_utc": utc_now(),
        "status": (
        ...
        "scientific_blockers": [SCIENTIFIC_SUBMISSION_BLOCKER],
...
if __name__ == "__main__":
    if "--verify-successor-only" in sys.argv[1:]:
        successor = refresh_and_verify_operational_successor()
        print(json.dumps(successor, indent=2, sort_keys=True))
        raise SystemExit(0 if successor["status"] == "pass" else 1)
    if "--archive-only-preflight-only" in sys.argv[1:]:
        archive = load_json(BUNDLE_DIR / "source_archive_manifest.json")
        job_paths = sorted((BUNDLE_DIR / "jobs").glob("*.json"))
        isolated = archive_only_preflight(
            archive=archive,
            job_paths=job_paths,
        )
        dump_json(BUNDLE_DIR / "archive_only_preflight.json", isolated)
        print(json.dumps(isolated, indent=2, sort_keys=True))
        raise SystemExit(0 if isolated.get("status") == "pass" else 1)
