...
> **DO NOT SUBMIT.** A completed-artifact audit found that the frozen source can
> enter a `phase2_raw` / `eps_grad_suppressed_continue` path.  On those rounds it
> does not construct the declared full active-plus-singleton Phase-III response,
> and the missing rank is not an all-energy-models-infeasible fallback receipt.
> This scientific route-contract mismatch cannot be repaired by loosening the
> evidence validator.  `SUBMISSION_ENABLED=False` and Condor matchmaking is
> deliberately impossible until the route semantics are resolved.
...
execution archive exists, but submission is scientifically blocked:
`SOURCE_FREEZE_COMPLETE=True`, `SUBMISSION_ENABLED=False`, and Condor
`requirements=False`.
