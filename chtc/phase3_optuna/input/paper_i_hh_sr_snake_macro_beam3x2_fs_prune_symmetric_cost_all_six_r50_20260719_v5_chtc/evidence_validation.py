...
def validate_supported_rank_or_exact_fallback(
    ...
    *,
    ...
    fallback_fired = (
        row.get("all_energy_models_infeasible_novelty_fallback_fired") is True
    )
    if fallback_fired:
        mode = str(
            row.get("all_energy_models_infeasible_novelty_fallback_mode") or ""
        )
        reason = str(
            row.get("all_energy_models_infeasible_novelty_fallback_reason") or ""
        )
        try:
            query_charge = int(
                row.get(
        ...
        if (
            ...
            or reason != "all_whitened_energy_models_infeasible"
            or query_charge != 0
        ):
            raise ValueError(
    ...
    if (
        require_supported_rank
        and supported_raw is None
        and not fallback_fired
        and selection_mode in {"phase2_raw", "eps_grad_suppressed_continue"}
    ):
        raise ValueError(
            f"round {outer_iteration}: supported rank missing because Phase III "
            f"did not provide a response receipt (selection_mode={selection_mode}); "
            "this violates the "
            "full-response-every-controller-round contract"
        )
...
def validate_post_prune_depth(
    *,
    previous_active_depth: int,
    pre_support: int,
    checkpoint_depth: int,
    prune_summary: Mapping[str, Any],
    outer_iteration: int,
) -> int:
    """Close one singleton admission against at most one accepted deletion."""

    if pre_support != previous_active_depth + 1:
        raise ValueError(
            f"round {outer_iteration}: pre-prune singleton depth does not close"
        )
    accepted_deletion = bool(
        prune_summary.get("executed") is True
        and prune_summary.get("accepted") is True
    )
    expected_depth = pre_support - int(accepted_deletion)
    if checkpoint_depth != expected_depth:
        raise ValueError(
            f"round {outer_iteration}: post-prune active depth does not close"
        )
...
def validate_active_prefix_estimator_receipts(
    *, adapt: Mapping[str, Any], ledger_summary: Mapping[str, Any], target_round: int,
    ...
    The runtime receipt snapshots ``EstimatorCallLedger.summary()`` without a
    branch filter, so its terminal unique total is the complete-search
    ``all_branch_search_work.S_alg``.  Under live pruning, rejected deletion
    probes are intentionally absent from the winning lineage even though they
    remain chargeable complete-search work.  The receipt must therefore close
    to all-work, while the winning-lineage total must be a nonnegative subset.
    """
    ...
    winning_lineage_s_alg = int(ledger_summary["winning_lineage_s_alg"])
    all_branch_s_alg = int(ledger_summary["all_branch_s_alg"])
    if (
        ...
        or winning_lineage_s_alg < 0
        or winning_lineage_s_alg > all_branch_s_alg
        or prior_unique_total != all_branch_s_alg
    ):
    ...
    return {
        ...
        "winning_lineage_S_alg": winning_lineage_s_alg,
        "discarded_only_S_alg": all_branch_s_alg - winning_lineage_s_alg,
        "closure_passed": True,
    }
...
def validate_live_prune_round(
    ...
    *,
    ...
    if mu_before != 0.0 or mu_after != 0.0:
        raise ValueError(
            f"round {outer_iteration}: undamped prune route recorded nonzero mu"
        )
...
# The public validator enforces live FS-trust pruning plus historical beam 3x2.  The
...
def validate_parent_evidence(
    *,
    ...
    for outer_iteration, (history_raw, checkpoint_raw) in enumerate(
        ...
        previous_active_depth = validate_post_prune_depth(
            previous_active_depth=previous_active_depth,
            pre_support=pre_support,
            checkpoint_depth=summary["active_ansatz_depth"],
    ...
    final_depth = int(segment.get("final_depth", -1))
    if final_depth != previous_active_depth or int(adapt.get("ansatz_depth", -1)) != final_depth:
        raise ValueError("final active depth does not match prune-adjusted checkpoint depth")
    route_state = _mapping(
    ...
    return {
        ...
        "prune_rounds_executed": sum(
            1 for prune in prune_round_summaries if prune["executed"]
        ),
        "prune_rounds_accepted": sum(
            1
            for prune in prune_round_summaries
            if prune["executed"] and prune.get("accepted") is True
        ),
        "terminal_state_unchanged_from_last_ordinary_round": True,
        "ledger": ledger,
