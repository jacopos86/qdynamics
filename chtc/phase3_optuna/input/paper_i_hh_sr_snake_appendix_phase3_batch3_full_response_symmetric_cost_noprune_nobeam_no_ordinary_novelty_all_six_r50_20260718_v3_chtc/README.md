...
one hash-locked overlay, `phase3_batch_appendix_overlay.patch`. Its v2 tail is
a non-scientific fail-closed route-identity repair: only this exact Phase-III
batch profile may accept the forward alias receipt
`phase3_enable_batching=True;phase2_enable_batching=False`; reverse conflicts,
other alias conflicts, and every other profile remain rejected. Its v3 tail is
the non-semantic plumbing repair for the failed v2 execution: the selector's
`FullScoreConfig` now receives the already-declared full residual Gram/Hessian,
full-ansatz context, and supported-metric global-trust solve. A runtime
invariant fails closed before selection if any of those three values drifts.
The derived archive never imports scientific modules from the live source
tree, and the parent, v1, and failed v2 bundles are never modified.
