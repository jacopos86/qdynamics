#!/usr/bin/env python3
"""Build the source-locked six-regime Phase-III batching appendix bundle.

This builder performs no scientific calculation and never submits CHTC jobs.
It derives byte-for-byte from the frozen main SR archive, applies the
hash-locked Phase-III batching overlay plus its non-semantic joint-context
plumbing repair, resolves the explicit appendix batch-three profile from that
isolated archive, verifies all six same-cutoff physics anchors, and writes six
fresh round-0 to round-50 rows plus a fail-closed submission preflight.  The
main/no-batch archive and failed v2 successor are never modified.
"""
...
BUNDLE_ID = (
    ...
    "noprune_nobeam_no_ordinary_novelty_all_six_r50_20260718_v3_chtc"
)
BATCH_NAME = (
    "paper-i-hh-sr-appendix-phase3-batch3-fullresp-symcost-noprune-"
    "nobeam-nonovelty-six-r50-20260718-v3"
)
FAILED_PREDECESSOR_BUNDLE_ID = (
    "paper_i_hh_sr_snake_appendix_phase3_batch3_full_response_symmetric_cost_"
    "noprune_nobeam_no_ordinary_novelty_all_six_r50_20260718_v2_chtc"
)
FAILED_PREDECESSOR_CLUSTER = 8887538
BUNDLE_DIR = Path(__file__).resolve().parent
REPO = Path(
    os.environ.get("HOLSTEIN_REPO_ROOT", str(BUNDLE_DIR.parents[3]))
).resolve()
...
PROFILE_REQUEST = "sr_snake_no_prune_symmetric_cost_phase3_batch_v1"
PROFILE_RESOLVED = (
    "supported_whitened_adaptive_trust_full_response_symmetric_cost_"
    "no_prune_phase3_batch_v1"
)
PROFILE_CONTRACT_SHA256 = (
    "04662940a5c9b6cf20afb5484cd7359de2a93a65d6dba2028e4412dcba0416c6"
)
PARENT_PROFILE_CONTRACT_SHA256 = (
    "69dcf6d5711a05b54cc143562bc6f437511d781164319b2705857b22deda1538"
)
FINAL_JOB_SCHEMA = "paper_i_hh_sr_appendix_phase3_batch3_all_six_r50_job_v1"
...
PARENT_SCIENTIFIC_SETTINGS_AUDIT_PATH = (
    PARENT_BUNDLE_DIR / "scientific_settings_audit.json"
)
PARENT_SCIENTIFIC_SETTINGS_AUDIT_SHA256 = (
...
BATCH_OVERLAY_PATH = BUNDLE_DIR / "phase3_batch_appendix_overlay.patch"
BATCH_OVERLAY_SHA256 = (
    "0c8dc5454a0badfc5844f46e03c2eeb689f2fabd05a0086b5668bc4c2d5743f9"
)
BATCH_OVERLAY_FILE_HASHES = {
    "pipelines/static_adapt/adapt_pipeline.py": (
        "4581bdfb5a01d29e4a98eef0441b38c1fea7bbb982d108e1999b9d137d6c2c7c"
    ),
    "pipelines/static_adapt/cli_config.py": (
        "3d40cfdc0e7ca46030397a01b46db6493904eaae542c0d3358c97f04adfc9d95"
    ),
    "pipelines/static_adapt/route_identity.py": (
        "14a1b2123aa0465b77bcd1d96392dc948d744a35a7d9dbea135299090238489a"
    ),
    "pipelines/static_adapt/sr_snake_route_profile.py": (
        "9b0dfcec339361fbd62cebc8cc720577f47bedb212fa47846eb4f7bc35bd22de"
    ),
    "test/test_static_adapt_sr_phase3_batch_appendix_profile.py": (
        "9aeb278b196a03114188f44e136fd3c6ccd2146646286b5ea3d0fa2eabc17157"
    ),
}
SOURCE_LOCK_STATE = (
    "frozen_main_archive_plus_exact_phase3_batch_overlay_"
    "plus_exact_route_alias_and_joint_context_validation_repair_v3"
)
...
CRITICAL_SOURCE_SHA256 = {
    "pipelines/static_adapt/adapt_pipeline.py": BATCH_OVERLAY_FILE_HASHES[
        "pipelines/static_adapt/adapt_pipeline.py"
    ],
    "pipelines/static_adapt/cli_config.py": BATCH_OVERLAY_FILE_HASHES[
        "pipelines/static_adapt/cli_config.py"
    ],
    "pipelines/static_adapt/estimator_call_ledger.py": (
    ...
    "pipelines/static_adapt/sr_snake_route_profile.py": BATCH_OVERLAY_FILE_HASHES[
        "pipelines/static_adapt/sr_snake_route_profile.py"
    ],
    "pipelines/static_adapt/route_identity.py": BATCH_OVERLAY_FILE_HASHES[
        "pipelines/static_adapt/route_identity.py"
    ],
    ...
    "test/test_static_adapt_sr_phase3_batch_appendix_profile.py": BATCH_OVERLAY_FILE_HASHES[
        "test/test_static_adapt_sr_phase3_batch_appendix_profile.py"
    ],
    ...
    "agent_guidance/static-adapt/route-identities.md": (
        "e5a916b25e33f29ea491db9e01abf211261393e590d7f87cfa7077f74789340c"
    ),
    "agent_guidance/shared/run-guide.md": "5b69a0655af49e449b8ac9f6dc4c12a8ac027cda700ca8a3551eaea83cba4ff9",
    "MATH/paper_facing/paper_I_static_scaffold/paper_i_sr_snake_no_prune_symmetric_cost_candidate_runtime_settings_20260717.md": (
        "bdee7843e9a990358cbd1d50c19567938d1e68b8c5ad161b66d1794d611686a2"
    ),
}
...
def derived_source_files() -> dict[str, tuple[bytes, int]]:
    ...
    if sha256(BATCH_OVERLAY_PATH) != BATCH_OVERLAY_SHA256:
        raise ValueError("Phase-III batching overlay hash drift")
    with tempfile.TemporaryDirectory(prefix="sr_phase3_batch_overlay_") as tmp:
        root = Path(tmp)
        ...
        completed = subprocess.run(
            ...
            cwd=root,
            input=BATCH_OVERLAY_PATH.read_text(encoding="utf-8"),
            capture_output=True,
            text=True,
            check=False,
        )
        if completed.returncode != 0:
            raise ValueError(
                "Phase-III batching overlay failed to apply exactly: "
                + completed.stdout.strip()
                + " "
                + completed.stderr.strip()
            )
        for relative, expected in BATCH_OVERLAY_FILE_HASHES.items():
            destination = root / relative
            if not destination.is_file() or destination.is_symlink():
                raise ValueError(f"Phase-III batching overlay omitted {relative}")
            data = destination.read_bytes()
            if bytes_sha256(data) != expected:
                raise ValueError(f"Phase-III batching overlay result drift: {relative}")
...
def verify_source_lock() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        not PARENT_SCIENTIFIC_SETTINGS_AUDIT_PATH.is_file()
        or sha256(PARENT_SCIENTIFIC_SETTINGS_AUDIT_PATH)
        != PARENT_SCIENTIFIC_SETTINGS_AUDIT_SHA256
    ):
        raise ValueError("immutable parent scientific-settings audit drift")
    ...
    revision = {
        "schema": "paper_i_hh_sr_phase3_batch3_source_revision_v1",
        "git_commit": EXPECTED_HEAD,
        "git_tree": EXPECTED_TREE,
        "git_role": "base_ancestry_metadata_only",
        "executable_source_authority": (
            "frozen_main_archive_plus_exact_phase3_batch_overlay_"
            "plus_exact_route_alias_and_joint_context_repair_inventory_v3"
        ),
        ...
        "phase3_batch_overlay": {
            "classification": (
                "appendix_one_factor_scientific_overlay_plus_nonsemantic_"
                "joint_context_plumbing_repair_v3"
            ),
            "path": BATCH_OVERLAY_PATH.relative_to(REPO).as_posix(),
            "overlay_sha256": BATCH_OVERLAY_SHA256,
            "derived_file_sha256": dict(BATCH_OVERLAY_FILE_HASHES),
            "parent_route_contract_sha256": PARENT_PROFILE_CONTRACT_SHA256,
            "exact_semantics": {
                "phase2_batching": False,
                "phase3_batching": True,
                "selection_mode": "combinatorial_reduced_plane",
                "target_size": 3,
                "size_cap": 3,
                "response_scope": "full_active_plus_batch_v1",
                "supported_fs_trust_receipt_required": True,
                "full_accepted_refit_required": True,
                "runtime_child_subset_cap": 1,
                "beam_shape": "effective_1x1_v1",
            },
        },
        "operational_repair": {
            "classification": "non_scientific_contract_activation_plumbing_v1",
            "failed_predecessor_bundle_id": FAILED_PREDECESSOR_BUNDLE_ID,
            "failed_predecessor_cluster": FAILED_PREDECESSOR_CLUSTER,
            "observed_failure": (
                "Phase-III batching appendix requires the full-ansatz joint "
                "response context."
            ),
            "repair": (
                "apply the already-declared full residual Gram/Hessian, "
                "full-ansatz context, and supported-metric global-trust "
                "solver to the selector FullScoreConfig"
            ),
    ...
    physics_lock = {
        "schema": "paper_i_hh_sr_phase3_batch3_physics_exact_reference_lock_v1",
...
def build_source_archive() -> dict[str, Any]:
    ...
    return {
        "schema": "paper_i_hh_sr_phase3_batch3_source_archive_manifest_v1",
...
"""
        ...
        return {
            "schema": "paper_i_hh_sr_phase3_batch3_archive_only_preflight_v1",
            "status": (
...
def job_command(row: dict[str, Any]) -> tuple[list[str], dict[str, str]]:
    ...
    argv = [
        ...
        "--adapt-max-depth", str(TARGET_DEPTH_CEILING),
        "--adapt-segment-id", (
            f"{slug}-sr-appendix-phase3-batch3-r0-r{target_round}-20260718-v3"
        ),
        "--adapt-segment-target-controller-round", str(target_round),
        "--adapt-segment-target-depth", str(TARGET_DEPTH_CEILING),
        "--adapt-segment-max-new-admissions", str(TARGET_DEPTH_CEILING),
...
def build_job(
    ...
    return {
        ...
        "run_class": "appendix_phase3_batching_ablation_matrix",
        "regime_slug": slug,
        "route_identity": {
        ...
        "segment": {
            ...
            "target_depth": int(TARGET_DEPTH_CEILING),
            "max_new_admissions": int(TARGET_DEPTH_CEILING),
        ...
        "evidence_requirements": {
            ...
            "full_active_plus_batch_response_each_round_required": True,
            "supported_fs_batch_trust_receipt_each_round_required": True,
            "phase2_batching_disabled_required": True,
            "phase3_batching_post_shortlist_only_required": True,
    ...
    *, contract: dict[str, Any], jobs: list[dict[str, Any]]
    ...
    expected_execution_diff = {
        "phase2_batch_selection_mode": (
            "<MISSING>", "combinatorial_reduced_plane"
        ),
        "phase2_batch_size_cap": ("<MISSING>", 3),
        "phase2_batch_target_size": ("<MISSING>", 3),
        "phase3_batch_selection_mode": (
            "<MISSING>", "combinatorial_reduced_plane"
        ),
        "phase3_batch_size_cap": ("<MISSING>", 3),
        "phase3_batch_target_size": ("<MISSING>", 3),
        "phase3_enable_batching": (False, True),
    }
    ...
    if observed_execution_diff != expected_execution_diff:
        raise ValueError(
            "appendix route is not the exact one-factor Phase-III batching "
    ...
    for job in jobs:
        ...
        checks = {
            ...
            "exact_horizon": (
                ...
                and int(segment["target_depth"]) == TARGET_DEPTH_CEILING
                and int(segment["max_new_admissions"])
                == TARGET_DEPTH_CEILING
            ),
            "horizon_explicit_in_argv": (
                "--adapt-max-depth" in argv
                and argv[argv.index("--adapt-max-depth") + 1]
                == str(TARGET_DEPTH_CEILING)
            ),
    ...
    return {
        "schema": "paper_i_hh_sr_appendix_phase3_batch3_settings_audit_v1",
        "status": "pass",
        "profile_request": PROFILE_REQUEST,
        "profile_resolved": PROFILE_RESOLVED,
        "profile_contract_sha256": PROFILE_CONTRACT_SHA256,
        ...
        "immutable_parent": {
            ...
            "scientific_settings_audit": (
                PARENT_SCIENTIFIC_SETTINGS_AUDIT_PATH.relative_to(REPO).as_posix()
            ),
            "scientific_settings_audit_sha256": (
                PARENT_SCIENTIFIC_SETTINGS_AUDIT_SHA256
            ),
            "bundle_manifest_used_as_provenance_anchor": False,
        },
        "one_factor_execution_diff": {
            key: {"parent": values[0], "appendix_phase3_batch": values[1]}
...
def smoke_summary() -> dict[str, Any]:
    ...
    scientific_validation = {
        ...
        "note": (
            ...
            "settings only. Exact Phase-III batching behavior is proved by the "
    ...
    return {
        ...
        "passed_gates": [
            ...
            "shadow_damping_off",
            "phase3_batch_exact_semantics_archive_regression",
        ...
        "note": (
            ...
            "this smoke is not evidence from the Phase-III batching variant."
        ),
    }
...
def main() -> int:
    ...
    for row in REGIMES:
        ...
        dump_json(normalized_path, {
            "schema": (
                "paper_i_hh_sr_appendix_phase3_batch3_all_six_r50_"
    ...
    if (
        ...
        raise ValueError(
            "appendix Phase-III batching bundle must queue exactly all six locked regimes"
        )
    ...
    parity = {
        "schema": "paper_i_hh_sr_appendix_phase3_batch3_route_parity_v1",
        "status": "pass",
        "profile_request": PROFILE_REQUEST,
        "profile_resolved": PROFILE_RESOLVED,
        "profile_contract_sha256": PROFILE_CONTRACT_SHA256,
        ...
        "controller_round_horizon": TARGET_CONTROLLER_ROUND,
        "active_depth_ceiling": TARGET_DEPTH_CEILING,
        "max_new_admissions": TARGET_DEPTH_CEILING,
    ...
    bundle_manifest = {
        "schema": "paper_i_hh_sr_appendix_phase3_batch3_bundle_v1",
        "bundle_id": BUNDLE_ID,
        "batch_name": BATCH_NAME,
        "created_utc": utc_now(),
        "run_class": "appendix_phase3_batching_ablation_matrix",
        "parent_stage": (
            "failed_phase3_batch_v2_nonsemantic_joint_context_repair_v3"
        ),
        "operational_repair": revision["operational_repair"],
        "submission_scope": {
            ...
            "per_regime_horizons": expected_horizons,
            "scientific_contract": (
                "full_response_symmetric_cost_no_prune_phase3_batch3_"
    ...
    preflight_checks = {
            ...
            "immutable_parent_plus_hash_locked_overlay_inventory": (
                ...
                == PARENT_ARCHIVE_SHA256
                and archive["phase3_batch_overlay"]["overlay_sha256"]
                == BATCH_OVERLAY_SHA256
                and archive["phase3_batch_overlay"]["derived_file_sha256"]
                == BATCH_OVERLAY_FILE_HASHES
                and set(archive["non_scientific_archive_overlays"])
                == set(NONSCIENTIFIC_ARCHIVE_OVERLAYS)
            ),
            "phase3_batch_overlay_result_hashes_closed": all(
                archive["files"].get(relative, {}).get("sha256") == digest
                for relative, digest in BATCH_OVERLAY_FILE_HASHES.items()
            ),
            ...
            "phase2_batching_disabled": True,
            "phase3_batching_enabled": True,
            "phase3_batching_post_shortlist_only": True,
            "phase3_batch_combinatorial_reduced_plane": True,
            "phase3_batch_target_and_cap_three": True,
            "phase3_batch_full_active_plus_batch_response": True,
            "phase3_batch_supported_fs_trust_receipt_required": True,
            "phase3_batch_full_accepted_refit_required": True,
            "phase3_runtime_child_subset_cap_one": True,
            "beam_effective_1x1": True,
    ...
    preflight = {
        "schema": "paper_i_hh_sr_appendix_phase3_batch3_preflight_v1",
        "created_utc": utc_now(),
        "status": (
