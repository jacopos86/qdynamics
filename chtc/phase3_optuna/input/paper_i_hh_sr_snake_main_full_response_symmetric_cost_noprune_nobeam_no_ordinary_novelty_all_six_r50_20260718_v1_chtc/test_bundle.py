...
class FrozenBundleTests(unittest.TestCase):
    ...
    def test_derivation_changes_only_declared_overlay_paths(self) -> None:
        ...
        allowed = {
            spec.ADAPT_PIPELINE_PATH,
            spec.ACCEPTED_REFIT_TEST_PATH,
            "test/test_static_adapt_estimator_call_ledger.py",
            *spec.REQUIRED_HASH_LOCKED_FIDELITY_FILES,
        }
        ...
        self.assertEqual(
            spec.bytes_sha256(derived[spec.ADAPT_PIPELINE_PATH][0]),
            spec.DERIVED_ADAPT_PIPELINE_SHA256,
        )
        self.assertEqual(
            spec.bytes_sha256(derived[spec.ACCEPTED_REFIT_TEST_PATH][0]),
            spec.DERIVED_ACCEPTED_REFIT_TEST_SHA256,
        )
        self.assertEqual(spec.sha256(spec.PARENT_ARCHIVE_PATH), spec.PARENT_ARCHIVE_SHA256)
        self.assertEqual(
            spec.sha256(spec.ADAPT_PIPELINE_OVERLAY_PATH),
            spec.ADAPT_PIPELINE_OVERLAY_SHA256,
        )
