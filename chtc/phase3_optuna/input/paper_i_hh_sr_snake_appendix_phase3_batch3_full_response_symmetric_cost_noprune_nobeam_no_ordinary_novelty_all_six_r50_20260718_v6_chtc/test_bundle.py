...
class BundleTest(unittest.TestCase):
    ...
    def test_batch_selectors_return_complete_workspace_summary(self):
        ...
        for function_name in (
            ...
            with self.subTest(function_name=function_name):
                ...
                returned_source = ast.unparse(returns[0].value)
                self.assertIn("dict(summary)", returned_source)
                self.assertNotIn("proposals[0].summary", returned_source)

    def test_combinatorial_builder_records_geometry_workspace(self):
        source = _archive_source(
            "pipelines/scaffold/hh_continuation_scoring.py"
        )
        tree = ast.parse(source)
        function = next(
            ...
            for node in tree.body
            if isinstance(node, ast.FunctionDef)
            and node.name == "combinatorial_reduced_plane_batch_proposals"
        )
        self.assertIn("geometry_workspace", ast.unparse(function))
