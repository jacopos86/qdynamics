...
OUT_ROOT="${2:-raw_outputs/paper_i_phase1_score_parity_20260604_v1/${CASE_ID}}"
...
"$APPTAINER_BIN" exec --cleanenv --bind "$ROOT:/work" "$IMAGE" \
  bash -lc 'cd /work && export TABLE_I_STATIC_SUITE_PROFILE=paper_i_three_model_hh_symmetric_20260527_v1 && python -u chtc/phase3_optuna/input/paper_i_phase1_score_parity_20260604_v1/run_phase1_score_parity.py "$@"' -- "$CASE_ID" --output-root "$OUT_ROOT"
