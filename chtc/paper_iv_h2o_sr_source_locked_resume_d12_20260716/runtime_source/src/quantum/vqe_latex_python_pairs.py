...
class AnsatzTerm:
    ...
    ``execution_mode`` is an optional runtime hint.  The default preserves the
    historical product-of-Pauli-rotations implementation.  Legal-subspace
    hardening may set ``"grouped_exact"`` for binary-boson generators whose
    grouped polynomial is legal but whose individual Pauli factors would leak
    under finite termwise-product execution.
    """
...
def vqe_minimize(
    ...
    *,
    ...
    objective_value_transform: Optional[Callable[[Dict[str, Any]], float]] = None,
) -> VQEResult:
    """
    ...
    def _objective_value_for_optimizer(
        energy_ideal: float,
        theta: np.ndarray,
        *,
        restart_index: int,
        nfev_restart: int,
        nfev_total_estimate: int,
    ) -> float:
        ideal_value = float(energy_ideal)
        if objective_value_transform is None:
            return ideal_value
        transformed = float(
            objective_value_transform(
                {
                    "energy_ideal": ideal_value,
                    "energy_exact": ideal_value,
                    "theta": np.array(theta, dtype=float, copy=True),
                    "surface": "vqe_objective",
                    "value_kind": "energy",
                    "phase": "optimizer",
                    "restart_index": int(restart_index),
                    "nfev_restart": int(nfev_restart),
                    "nfev_total_estimate": int(nfev_total_estimate),
                    "progress_label": str(progress_label),
                }
            )
        )
        if not math.isfinite(transformed):
            log.error("objective_value_transform must return a finite scalar")
        return transformed
    ...
    for r in range(int(restarts)):
        ...
        def _objective_with_progress(x: np.ndarray) -> float:
            ...
            energy_ideal = float(energy_fn(x_arr))
            restart_nfev += 1
            e_val = _objective_value_for_optimizer(
                energy_ideal,
                x_arr,
                restart_index=int(r + 1),
        ...
        if method_key == "spsa":
            ...
            try:
                ...
                if restart_best_theta is not None and np.isfinite(restart_best):
                    ...
                    restart_nfev += 1
                    energy = _objective_value_for_optimizer(
                        float(energy_fn(theta_opt)),
                        theta_opt,
                        restart_index=int(r + 1),
                        nfev_restart=int(restart_nfev),
                        nfev_total_estimate=int(total_nfev + restart_nfev),
