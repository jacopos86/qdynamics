...
from src.quantum.chemistry.psi4_adapter import (
    ...
    align_restricted_closed_shell_snapshot_to_center_mo,
...
def pauli_polynomial_to_jsonable(poly: PauliPolynomial) -> dict[str, Any]:
    """Public serializer for chemistry fixture/generator artifacts."""

    return _polynomial_to_jsonable(poly)


def pauli_polynomial_from_jsonable(payload: Any, *, expected_nq: int | None = None) -> PauliPolynomial:
    """Public loader for chemistry fixture/generator Pauli-polynomial payloads."""

    return _polynomial_from_jsonable(payload, expected_nq=expected_nq)
...
def _align_snapshot_to_center_mo(
    ...
    *,
    ...
    return align_restricted_closed_shell_snapshot_to_center_mo(
        snapshot,
...
def build_vibronic_h2_model_from_local_taylor_product(
    local_taylor: dict[str, Any] | Any,
    *,
    n_ph_max: int = 1,
    ...
    """Build the existing runtime model from an ab initio local-Taylor product.

    This is the compatibility bridge for the upgraded H2 fixture generator.  It
    intentionally emits the same ``molecular_vibronic_h2_fixture_v1`` model
    family as the checked-in cache and rejects larger active spaces rather than
    silently projecting them onto the current four-fermion-qubit runtime.
    """

    if not isinstance(local_taylor, dict):
        raise ValueError("local_taylor product must be a JSON object.")
    if str(local_taylor.get("schema")) != "h2_local_taylor_product_v1":
        raise ValueError(f"Unsupported local Taylor schema: {local_taylor.get('schema')!r}")
    active = local_taylor.get("active_space")
    if not isinstance(active, dict):
        raise ValueError("local_taylor product missing active_space object.")
    if (
        int(active.get("n_spatial_orbitals", -1)) != 2
        or int(active.get("n_spin_orbitals", -1)) != 4
        or int(active.get("n_alpha", -1)) != 1
        or int(active.get("n_beta", -1)) != 1
        or str(active.get("ordering", "")).strip().lower() != "blocked"
    ):
        raise ValueError(
            "local Taylor product is not compatible with molecular_vibronic_h2_fixture_v1: "
            "requires two spatial orbitals, four spin orbitals, (1,1) electrons, blocked ordering."
        )
    return _build_vibronic_h2_model_from_cached_components(
        h_electronic=_polynomial_from_jsonable(local_taylor.get("h_electronic"), expected_nq=4),
        dH_dR=_polynomial_from_jsonable(local_taylor.get("dH_dR"), expected_nq=4),
        bond_length_angstrom=float(local_taylor["center_R_angstrom"]),
        bond_step_angstrom=float(local_taylor["step_angstrom"]),
        basis=str(local_taylor.get("basis", "unknown")),
        omega_au=float(local_taylor["omega_au"]),
        reduced_mass_au=float(local_taylor["reduced_mass_au"]),
        x_zpf_bohr=float(local_taylor["x_zpf_bohr"]),
        curvature_au_per_bohr2=float(local_taylor["curvature_au_per_bohr2"]),
        electronic_exact_energy_minus=float(local_taylor["electronic_exact_energy_minus"]),
        electronic_exact_energy_center=float(local_taylor["electronic_exact_energy_center"]),
        electronic_exact_energy_plus=float(local_taylor["electronic_exact_energy_plus"]),
        n_ph_max=int(n_ph_max),
        boson_encoding=str(boson_encoding),
        coupling_scale=float(coupling_scale),
