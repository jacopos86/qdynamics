    ...
    for group in pauli_label_groups:
        ...
        for label in labels:
            ...
            if label == "e" * nq:
                continue
            try:
                append_pauli_rotation_exyz(qc, label_exyz=label, angle=float(cfg.structure_theta_value))
            except Exception as exc:
    ...
    *,
    ...
    for index, op in enumerate(tuple(ops)):
        ...
        if mode == "termwise_product" or commuting:
            ...
            runtime_rotation_count += 1
            row.update(exact_meta, commuting=False)
        operator_rows.append(row)
