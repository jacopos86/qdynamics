...
def allowed_method_ids() -> tuple[str, ...]:
    """Return the six visible non-SNAKE comparator method IDs."""
...
def full_method_target_records(
    *,
    ...
    """Build deterministic method-target record stubs for the full 6x6 matrix."""
...
def validate_full_method_target_records(records: Sequence[Mapping[str, object]]) -> tuple[dict[str, object], ...]:
    """Validate that ``records`` are exactly the full 36 method-target matrix."""
    ...
    if seen != expected:
        ...
        raise ValueError(
            f"Expected exactly 36 full method-target records for {PAPER_I_COMPARATOR_SPSA_CALIBRATION_PROFILE_ID}; "
...
def validate_native_spsa_adaptive_records(records: Sequence[Mapping[str, object]]) -> tuple[dict[str, object], ...]:
    """Validate exactly the 4x6 affected-adaptive native SPSA rerun matrix."""
    ...
    if seen != expected:
        ...
        extra = sorted(seen - expected)
        raise ValueError(
            f"Expected exactly 24 native SPSA adaptive rerun records; got {len(seen)} unique records, "
...
validate_target_cases()
if len(PAPER_I_COMPARATOR_SPSA_CALIBRATION_ALLOWED_METHOD_IDS) != 6:
    raise ValueError("Paper-I comparator SPSA calibration must expose exactly six allowed methods.")
...
if len(full_method_target_records()) != 36:
    raise ValueError("Paper-I comparator SPSA calibration full matrix must contain exactly 36 records.")
...
if len(native_spsa_adaptive_method_target_records()) != 24:
    raise ValueError("Native SPSA adaptive rerun matrix must contain exactly 24 records.")
