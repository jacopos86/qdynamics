...
_STATIC_ROUTE_TSV_FIELDS = ("static_route_id",)
...
def _static_route_env_name(field: str) -> str:
    return "GENERIC_STATIC_TABLE_" + str(field).upper()
...
def _static_route_env_value(field: str) -> str | None:
    for name in (_static_route_env_name(field), str(field).upper()):
        raw = os.environ.get(name)
        if raw not in {None, ""}:
            return str(raw).strip()
    return None
...
def _static_route_policy_overrides_from_env() -> dict[str, object]:
    raw_values = {field: _static_route_env_value(field) for field in _STATIC_ROUTE_TSV_FIELDS}
    raw = raw_values["static_route_id"]
    if raw in {None, ""}:
        return {}
    return {"static_route_id": normalize_static_route_id(raw)}


def _static_route_policy_overrides_requested(overrides: dict[str, object]) -> bool:
    return bool(overrides)
...
def _shared_pauli_pool_overrides_from_env() -> dict[str, object]:
    ...
    if mode not in {_SHARED_PAULI_POOL_MODE_OFF, _SHARED_PAULI_POOL_MODE_CHILD_SETS_V1}:
...
def _reject_invalid_append_only_static_route(*, algorithm_id: str, static_route_id: object) -> None:
    if str(algorithm_id) != "static_append_only_adapt_phase3":
        return
    if normalize_static_route_id(static_route_id, default=ROUTE_ID_UNSPECIFIED) != ROUTE_ID_UNSPECIFIED:
        raise ValueError("static_append_only_adapt_phase3 is diagnostic and must use static_route_id='unspecified'.")


def _enforce_phase3_policy_algorithm_route_contract(policy, *, algorithm_id: str):
    _reject_invalid_append_only_static_route(
        algorithm_id=str(algorithm_id),
...
def _phase3_policy_for_algorithm(
    *,
    ...
    static_route_overrides: dict[str, object] | None = None,
    ...
    route_kwargs = dict(static_route_overrides or {})
    ...
    if algorithm_id == "static_append_only_adapt_phase3":
        _reject_invalid_append_only_static_route(
            algorithm_id=str(algorithm_id),
            static_route_id=static_kwargs.get("static_route_id"),
        )
        append_static_kwargs = {**static_kwargs, "static_route_id": ROUTE_ID_UNSPECIFIED}
    ...
    return AlgorithmPolicy(pool=pool, static=StaticScaffoldPolicy(**static_kwargs), inner_optimizer=inner)
...
def _phase3_policy_with_env_overrides(
    ...
    *,
    ...
    static_route_overrides: Mapping[str, object] | None = None,
...
def _run_phase3_static_single(
    *,
    ...
    from pipelines.static_adapt.optimization.phase3_policy_optuna import run_static_benchmark
    ...
    if policy_json_env is not None:
        ...
        policy = _phase3_policy_with_env_overrides(
            ...
            static_route_overrides=static_route_overrides,
    ...
    payload = {
        ...
        "runner": "pipelines.static_adapt.optimization.phase3_policy_optuna.run_static_benchmark",
        ...
        "static_route_env_overlay": dict(static_route_overrides or {}),
...
def run_single(*, family: str, case_id: str, algorithm_id: str, output_dir: Path) -> dict:
    ...
    static_route_overrides = _static_route_policy_overrides_from_env()
    static_route_requested = _static_route_policy_overrides_requested(static_route_overrides)
    ...
    if (
        ...
        or static_route_requested
    ) and dispatch != "phase3_static_adapt":
        raise ValueError(
            "Phase3 oracle/value-noise, runtime, or static-route CHTC env overlay is only valid for "
    ...
    if dispatch == "phase3_static_adapt":
        payload = _run_phase3_static_single(
            ...
            static_route_overrides=static_route_overrides if static_route_requested else None,
