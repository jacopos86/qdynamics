...
from src.quantum.operator_pools.boson_chains import boson_chain_legal_basis_indices
...
def _fixed_count_qubits(context: ResolvedProblemContext, quantity: str, scope: str) -> tuple[int, ...]:
    quantity_key = str(quantity)
    request = context.request
    n_sites = int(request.num_sites)
    if quantity_key == "n_up":
        return tuple(mode_index(site, 0, n_sites=n_sites, indexing=str(request.ordering)) for site in range(n_sites))
    if quantity_key == "n_dn":
        return tuple(mode_index(site, 1, n_sites=n_sites, indexing=str(request.ordering)) for site in range(n_sites))
    if quantity_key == "n_f":
        block = context.layout.block("fermion")
        if block is not None:
            return tuple(range(int(block.start_qubit), int(block.stop_qubit)))
        return tuple(range(int(context.layout.fermion_qubits or context.layout.total_qubits)))
...
def _truncation_constraint_spec(context: ResolvedProblemContext, constraint: TruncationConstraint) -> dict[str, Any]:
    ...
    if block is None:
        if scope_key == "full":
            ...
            raise ValueError(f"No layout block found for truncation scope {scope!r}.")
    else:
        start_qubit = int(block.start_qubit)
        stop_qubit = int(block.stop_qubit)

    qubits = tuple(range(int(start_qubit), int(stop_qubit)))
    ...
    bits_per_site = int(boson_qubits_per_site(max_local_occupancy, encoding))
    if bits_per_site <= 0 or register_size % bits_per_site != 0:
        raise ValueError(
            "Truncation register size is incompatible with the boson encoding: "
            f"scope={scope!r}, register_size={register_size}, bits_per_site={bits_per_site}."
        )
    num_sites = int(register_size // bits_per_site)
    legal = frozenset(
        int(x)
        for x in boson_chain_legal_basis_indices(
            num_sites=num_sites,
    ...
    return {
        ...
        "qubits": list(qubits),
        "start_qubit": int(start_qubit),
        "stop_qubit": int(stop_qubit),
        "num_sites": num_sites,
        "boson_encoding": encoding,
        "bits_per_site": bits_per_site,
        "legal_basis_count": len(legal),
        "_register_mask": (1 << int(register_size)) - 1,
        "_legal_register_indices": legal,
    }


def sector_probability(context: ResolvedProblemContext, psi: np.ndarray) -> dict[str, Any]:
    """Return reporting-only sector/truncation diagnostics for an HEA statevector."""
    psi_arr = np.asarray(psi, dtype=complex).reshape(-1)
    norm = float(np.vdot(psi_arr, psi_arr).real)
    if norm <= 0.0:
        raise ValueError("Cannot compute sector diagnostics for a zero-norm state.")
    probs = np.abs(psi_arr) ** 2 / norm
    constraints = tuple(context.sector.constraints)
    fixed_constraints = [
        c for c in constraints if isinstance(c, (FixedCountConstraint, WeightedChargeConstraint))
    ]
    truncation_constraints = [c for c in constraints if isinstance(c, TruncationConstraint)]
    if not fixed_constraints and not truncation_constraints:
        return {
            ...
            "sector_leak_threshold": _SECTOR_LEAK_THRESHOLD,
    ...
    constraint_specs: list[tuple[tuple[int, ...], int, str, str]] = []
    for constraint in fixed_constraints:
        qubits = _fixed_count_qubits(context, constraint.quantity, constraint.scope)
        target = int(constraint.value)
        quantity = str(constraint.quantity)
        scope = str(constraint.scope)
        constraint_specs.append((qubits, target, quantity, scope))
        evaluated.append({"quantity": quantity, "scope": scope, "value": target, "qubits": list(qubits)})

    truncation_specs = [_truncation_constraint_spec(context, constraint) for constraint in truncation_constraints]
    ...
    for basis_index, prob in enumerate(probs):
        ...
        for qubits, target, _quantity, _scope in constraint_specs:
            count = sum((int(basis_index) >> int(q)) & 1 for q in qubits)
            if int(count) != int(target):
                ok = False
                break
        for idx, spec in enumerate(truncation_specs):
            register_index = (int(basis_index) >> int(spec["start_qubit"])) & int(spec["_register_mask"])
            if register_index in spec["_legal_register_indices"]:
                truncation_good_probs[idx] += prob_f
            else:
                ok = False
        if ok:
            good_prob += prob_f
    ...
    for spec, legal_prob in zip(truncation_specs, truncation_good_probs):
        legal = min(1.0, max(0.0, float(legal_prob)))
        illegal = min(1.0, max(0.0, float(1.0 - legal)))
        public = {key: value for key, value in spec.items() if not key.startswith("_")}
        public.update({"legal_probability": legal, "illegal_probability": illegal})
        truncation_evaluated.append(public)

    good_prob = min(1.0, max(0.0, float(good_prob)))
    leak = min(1.0, max(0.0, float(1.0 - good_prob)))
    boson_legal_probability_min = (
        min(float(item["legal_probability"]) for item in truncation_evaluated)
        if truncation_evaluated
        else None
    )
    boson_illegal_probability_max = (
        max(float(item["illegal_probability"]) for item in truncation_evaluated)
        if truncation_evaluated
        else None
    )
    boson_truncation_leak_flag = bool(
        boson_illegal_probability_max is not None
        and float(boson_illegal_probability_max) > _SECTOR_LEAK_THRESHOLD
    )
    if constraint_specs and truncation_specs:
        policy = "diagnostic_only_fixed_count_and_truncation_probability"
    ...
    if truncation_evaluated:
        boson_subspace_diagnostics = {
            ...
            "boson_illegal_probability_max": boson_illegal_probability_max,
            "constraints_evaluated": truncation_evaluated,
        }
    return {
        "sector_probability": good_prob,
        "sector_leak_probability": leak,
        "sector_leak_flag": bool(leak > _SECTOR_LEAK_THRESHOLD),
        "sector_leak_threshold": _SECTOR_LEAK_THRESHOLD,
        "constraints_evaluated": [*evaluated, *truncation_evaluated],
        ...
        "boson_legal_probability_min": boson_legal_probability_min,
        "boson_illegal_probability_max": boson_illegal_probability_max,
        "boson_truncation_leak_flag": boson_truncation_leak_flag,
        "policy": policy,
    }
