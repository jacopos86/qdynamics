...
Then audits noiseless dynamics (Suzuki-2, midpoint Magnus-2, exact)
    ...
    ideal/shots/aer_noise, with opt-in aer_density_matrix support, for static + drive profiles.
"""
    ...
    *,
    ...
    return {
        ...
        "noise_config": {
            ...
            "symmetry_mitigation": dict(symmetry_mitigation_config),
            "value_noise": {
                "enabled": bool(str(value_noise_model).strip().lower() != "off"),
                "model": str(value_noise_model),
                "std": float(value_noise_std),
                "seed": value_noise_seed,
...
def _build_equation_registry_and_contracts(payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    modes = ["ideal", "shots", "aer_noise", "aer_density_matrix"]
...
_NOISE_MODE_COLORS: dict[str, str] = {
    "ideal": "#ff7f0e",
    "shots": "#2ca02c",
    "aer_noise": "#d62728",
    "aer_density_matrix": "#9467bd",
}


def _noise_mode_color(mode: str) -> str:
    return _NOISE_MODE_COLORS.get(str(mode), "#8c564b")
...
def _noise_style_legend_lines() -> list[str]:
    return [
        f"noise ideal              : {_noise_mode_color('ideal')}",
        f"noise shots              : {_noise_mode_color('shots')}",
        f"noise aer shot           : {_noise_mode_color('aer_noise')}",
        f"noise aer density matrix : {_noise_mode_color('aer_density_matrix')}",
    ...
    with PdfPages(str(pdf_path)) as pdf:
        ...
        for profile_name in ("static", "drive"):
            ...
            for mode in available_modes:
                color = _noise_mode_color(str(mode))
