...
_DEFAULT_ALGORITHMS: tuple[BenchmarkAlgorithm, ...] = (
    ...
    BenchmarkAlgorithm(
        ...
        method_family="tetris_full_meta_adapt",
        supported_families=None,
        required_pool_key="full_meta",
        implemented_families=_GENERIC_STATIC_ADAPT_VARIANT_TABLE_I_FAMILIES,
        runner_module=_STATIC_HH_RUNNER,
        ...
        diagnostic=False,
        hamiltonian_generic=True,
        **_source_kwargs("static_tetris_qubit_adapt_vqe"),
        notes=(
            "Benchmark-local generic TETRIS-style row over the problem-local full_meta "
            "pool. Each ADAPT iteration greedily batches gradient-ranked candidates "
            "with disjoint qubit support. This "
            "is separate from the Hubbard-only public-code TETRIS adapter and does "
            "not emulate TETRIS through Phase3."
        ),
    ),
    BenchmarkAlgorithm(
    ...
    BenchmarkAlgorithm(
        ...
        diagnostic=True,
        hamiltonian_generic=True,
        notes="HH uses the legacy wrapper; non-HH static fixture cases use a repo-native exact-assisted AVQDS(T) target-tangent diagnostic comparator.",
    ),
    BenchmarkAlgorithm(
