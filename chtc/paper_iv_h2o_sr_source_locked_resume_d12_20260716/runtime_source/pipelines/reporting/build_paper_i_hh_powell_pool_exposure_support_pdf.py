...
from pipelines.exact_bench.hh_tableiii_prefix_resources import (
    ENERGY_KEYS,
    ERROR_KEYS,
    ...
    _first_present,
    _history as _generic_prefix_history,
    _num_qubits_from_groups,
    _pauli_map,
    _reference_state_from_source,
    _selected_labels,
    _selected_positions,
...
SNAKE_SELECTED_PREFIX_BY_REGIME = {
    "weak-weak": 10,
    "intermediate-weak": 9,
    "strong-weak": 17,
    "weak-strong": 13,
    "intermediate-strong": 30,
    "strong-strong": 10,
}
COMPARATOR_PLATEAU_REL_TOL = 0.10
...
def first_plateau_prefix(points: Sequence[Sequence[float]], rel_tol: float = COMPARATOR_PLATEAU_REL_TOL) -> tuple[int | None, dict[str, Any]]:
    clean: list[tuple[int, float]] = []
    for point in points:
        if len(point) < 2:
            continue
        k = as_int(point[0])
        err = as_float(point[1])
        if k is not None and err is not None and err > 0.0:
            clean.append((int(k), float(err)))
    if not clean:
        return None, {"status": "missing_positive_trajectory"}
    best = min(err for _k, err in clean)
    threshold = best * (1.0 + float(rel_tol))
    for k, err in clean:
        if err <= threshold:
            return k, {
                "status": "ok",
                "rule": "first_prefix_with_error_within_rel_tol_of_best_trajectory_error",
    ...
    k, err = clean[-1]
    return k, {
        "status": "fallback_terminal",
        "rule": "no_prefix_within_rel_tol_of_best_trajectory_error",
        "rel_tol": float(rel_tol),
        "best_error": best,
        "threshold": threshold,
        "selected_error": err,
...
def generic_prefix_s_alg(payload: Mapping[str, Any], selected_k: int) -> tuple[float | None, str, dict[str, Any]]:
    ...
    return float(outer + refit + grad + metric + other), "ok", {
        "N_H_outer_eval": outer,
...
def compile_generic_selected_prefix_row(
    *,
    ...
    """
    history = _generic_prefix_history(payload)
    consumed, reached_depth = _history_rows_through_depth(history, int(selected_k))
    if int(selected_k) < 1 or reached_depth != int(selected_k):
        raise ValueError(
    ...
    energy_key, energy = _first_present(selected_hist_row, ENERGY_KEYS)
    error_key, error = _first_present(selected_hist_row, ERROR_KEYS)
    return {
...
def selected_prefix_payload(raw: Mapping[str, Any], spec: RoleSpec, regime: str, points: Sequence[Sequence[float]]) -> dict[str, Any]:
    if spec.method == "snake":
        selected_k = SNAKE_SELECTED_PREFIX_BY_REGIME.get(regime)
        policy = "manual_snake_selected_prefix_by_regime"
        policy_meta: dict[str, Any] = {"manual_prefixes": dict(SNAKE_SELECTED_PREFIX_BY_REGIME)}
    else:
        selected_k, policy_meta = first_plateau_prefix(points)
        policy = "first_plateau_rel_tol"
    out: dict[str, Any] = {
    ...
    try:
        if spec.method == "snake":
            ...
            out.update(
                ...
                s_work_source="generic_result_adapt_history_prefix_components",
...
def write_tex(path: Path, rows: Sequence[DerivedRow], figures: Sequence[Mapping[str, Any]], stem: str) -> None:
    ...
    body.extend(
        [
            ...
            "% value_policy=selected-prefix diagnostics: SNAKE manual k by regime; Geo/append first prefix within 10 percent of best trajectory error; Qiskit costs compiled at selected prefix.",
            "% visible_policy=no standalone report title, repo provenance prose, or route legend block; method/pool exposure appears in plot legends and table rows; marker denotes selected prefix.",
    ...
    payload: dict[str, Any] = {
        ...
        "data_policy": {
            ...
            "selected_prefix_policy": {
                "snake": SNAKE_SELECTED_PREFIX_BY_REGIME,
                "comparators": {
                    "rule": "first_prefix_with_error_within_rel_tol_of_best_trajectory_error",
                    "rel_tol": COMPARATOR_PLATEAU_REL_TOL,
                },
            },
            ...
            "work_policy": "SNAKE S_alg uses snake_algorithmic_work_from_payload(scope=display_prefix); comparator S_alg is recovered from emitted adapt_history prefix components",
