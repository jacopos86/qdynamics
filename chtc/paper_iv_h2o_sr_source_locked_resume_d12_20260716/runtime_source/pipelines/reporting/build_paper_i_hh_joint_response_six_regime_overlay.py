...
SCHEMA = "paper_i_hh_joint_response_six_regime_overlay_v6"
...
def _stitched_winning_lineage_query_work(paths: Sequence[Path]) -> dict[str, Any]:
    ...
    for path in paths:
        ...
        if sidecar is not None:
            ...
            segment_total = float(sidecar["query_work_total"])
            if not math.isclose(
                segment_total,
                sum(mapped_components.values()),
                rel_tol=0.0,
                abs_tol=1.0e-9,
            ):
                raise ValueError(f"Query sidecar component closure failed: {sidecar_path}")
            audit_status = "validated_query_work_sidecar"
            ...
            if work.get("S_alg_status") != "ok" or work.get("S_alg") is None:
                raise ValueError(
                    f"Winning-lineage query work is unavailable for {path}: "
                    f"{work.get('S_alg_status')}"
                )
            mapped_components = {}
            for key in component_keys:
                value = work.get(key)
                if value is None:
                    raise ValueError(f"Missing {key} for stitched query work: {path}")
                mapped_components[key] = float(value)
            segment_total = float(work["S_alg"])
            audit_status = str(audit.get("status"))
            source_kind = "payload_formula_fallback_no_sidecar"
        for key, value in mapped_components.items():
        ...
        segments.append(
            {
                ...
                "source_kind": source_kind,
                "query_work_sidecar": (
                    _rel(sidecar_path) if sidecar_path.is_file() else None
                ),
            }
        )
...
def _live_l10_weak_weak_evidence(output_dir: Path) -> dict[str, Any] | None:
    ...
    resource = _supplemental_resource_row(
        ...
        s_override=float(query_work["S_alg"]),
        s_source_override="stitched_winning_lineage_query_work.S_alg",
...
def _load_jr_chtc_live_snapshot_manifest(
    ...
    for raw_entry in raw_entries:
        ...
        if raw_qiskit_sidecar:
            ...
            if query.get("schema") == "jr_snake_stitched_winning_lineage_query_work_v1":
                query_ok = (
                    query.get("status") == "ok"
            ...
            if not query_ok or query_total is None or float(query_total) < 0.0:
                raise ValueError(
            ...
            resource.update(
                ...
                D2q=int(qiskit["compiled_depth_2q_total"]),
                Dc=int(qiskit["compiled_depth_total"]),
                S=int(round(float(query_total))),
                S_source="validated_winning_lineage_query_sidecar",
                S_scope=str(query_scope),
                resource_status="validated_snapshot_sidecars",
                prefix_semantics=(
        ...
        if qiskit_sidecar_path is not None and query_sidecar_path is not None:
            provenance.update(
                resource_fields="validated_qiskit_and_winning_lineage_query_sidecars",
                qiskit_sidecar=_rel(qiskit_sidecar_path),
                qiskit_sidecar_sha256=_sha256(qiskit_sidecar_path),
                query_work_sidecar=_rel(query_sidecar_path),
                query_work_sidecar_sha256=_sha256(query_sidecar_path),
    ...
    source_comment = json.dumps(
        {
            ...
            "page_contract": [
                ...
                "original_paper_i_route_context",
            ],
...
def build(
    ...
    if _page_count(pdf_path) != 6:
        raise ValueError("Six-page model-comparison PDF contract failed")
    return {
        "pdf": str(pdf_path),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    optional_build_kwargs = (
        {"prior_report_json": args.prior_report_json}
