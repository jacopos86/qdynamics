...
def _build_hh_hamiltonian_from_settings(settings: Mapping[str, Any]) -> Any:
    ...
    return build_hubbard_holstein_hamiltonian(
        ...
        n_ph_max=n_ph_max,
        boson_encoding=boson_encoding,
        v_t=float(settings.get("dv", 0.0)),
        v0=None,
        t_eval=None,
        repr_mode="JW",
        indexing=str(settings.get("ordering", "blocked")),
        pbc=(str(settings.get("boundary", "open")) == "periodic"),
        include_zero_point=bool(settings.get("include_zero_point", True)),
    )


def _legacy_hh_fallback_allowed(settings: Mapping[str, Any]) -> bool:
    problem_key = str(settings.get("problem", "hh")).strip().lower().replace("-", "_")
    return problem_key in {"hh", "hubbard_holstein"}


def _reconstruct_imported_adapt_circuit_legacy(
    payload: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if not _legacy_hh_fallback_allowed(settings):
        raise ValueError(
            "Legacy ADAPT circuit reconstruction only supports Hubbard-Holstein; "
            f"refusing to substitute that Hamiltonian for problem={settings.get('problem')!r}."
        )
...
def reconstruct_imported_adapt_circuit(
    ...
    *,
    ...
    if settings_overrides:
        ...
        if not isinstance(settings, Mapping):
            raise ValueError("Imported payload settings must be a mapping before applying overrides.")
        data = dict(data)
        data["settings"] = {**dict(settings), **dict(settings_overrides)}
    if isinstance(data.get("ansatz_input_state", None), Mapping):
        try:
            runtime_input = load_scaffold_runtime_input_from_payload(data)
            ansatz_input_state, ansatz_input_state_meta = _resolve_ansatz_input_state_from_payload(data)
            if ansatz_input_state is None:
                ansatz_input_state = np.asarray(runtime_input.psi_ref, dtype=complex).reshape(-1)
                ansatz_input_state_meta = {
                    "available": True,
                    "reason": None,
                    "error": None,
                    "source": data.get("ansatz_input_state", {}).get("source", None),
                    "handoff_state_kind": data.get("ansatz_input_state", {}).get(
                        "handoff_state_kind",
                        None,
                    ),
                    "nq_total": int(runtime_input.resolved_problem.layout.total_qubits),
                }
            qc = _build_ansatz_circuit(
                runtime_input.base_layout,
                np.asarray(runtime_input.theta_runtime, dtype=float).reshape(-1),
                int(runtime_input.resolved_problem.layout.total_qubits),
                ref_state=np.asarray(ansatz_input_state, dtype=complex).reshape(-1),
            )
            return {
                "payload": data,
                "settings": dict(data.get("settings", {})),
                "adapt_vqe": dict(data.get("adapt_vqe", {})),
                ...
                "ansatz_input_state": np.asarray(ansatz_input_state, dtype=complex).reshape(-1),
                "ansatz_input_state_meta": dict(ansatz_input_state_meta),
                "circuit": qc,
                "runtime_input": runtime_input,
                "reconstruction_source": "runtime_loader",
            }
        except Exception as exc:
            settings = data.get("settings", {})
            if not isinstance(settings, Mapping) or not _legacy_hh_fallback_allowed(settings):
                problem_key = settings.get("problem") if isinstance(settings, Mapping) else None
                raise ValueError(
                    "Runtime-loader reconstruction failed for a non-Hubbard-Holstein "
                    f"problem={problem_key!r}; legacy Hamiltonian fallback is forbidden. "
                    f"Original error: {type(exc).__name__}: {exc}"
                ) from exc
            bundle = _reconstruct_imported_adapt_circuit_legacy(data)
            bundle["reconstruction_source"] = "legacy_fallback"
            bundle["runtime_loader_warning"] = f"{type(exc).__name__}: {exc}"
            return bundle
    return _reconstruct_imported_adapt_circuit_legacy(data)
...
def _resolve_runtime_layout_and_theta(
    ...
    if isinstance(parameterization, Mapping):
        ...
        if int(optimal_point.size) != int(layout.runtime_parameter_count):
            repaired_layout = _reconcile_pruned_checkpoint_layout(
                parameterization,
                final_operator_labels=adapt_vqe.get("operators", ()),
            )
            logical_point = np.asarray(
                adapt_vqe.get("logical_optimal_point", []),
                dtype=float,
            ).reshape(-1)
            if (
                repaired_layout is not None
                and int(optimal_point.size) == int(repaired_layout.runtime_parameter_count)
                and (
                    int(logical_point.size) == 0
                    or int(logical_point.size) == int(repaired_layout.logical_parameter_count)
                )
            ):
                return repaired_layout, optimal_point
            raise ValueError(
...
def _reconcile_pruned_checkpoint_layout(
    parameterization: Mapping[str, Any],
    *,
    final_operator_labels: Any,
) -> AnsatzParameterLayout | None:
    """Recover a final layout when a compact result retained a pre-prune checkpoint layout."""

    blocks_raw = parameterization.get("blocks", ())
    if not isinstance(blocks_raw, Sequence) or isinstance(
        blocks_raw,
        (str, bytes, bytearray),
    ):
        return None
    if not isinstance(final_operator_labels, Sequence) or isinstance(
        final_operator_labels,
        (str, bytes, bytearray),
    ):
        return None
    target_labels = [str(label) for label in final_operator_labels]
    if not target_labels or len(target_labels) >= len(blocks_raw):
        return None

    filtered_blocks: list[dict[str, Any]] = []
    source_cursor = 0
    runtime_start = 0
    for logical_index, target_label in enumerate(target_labels):
        matched: Mapping[str, Any] | None = None
        while source_cursor < len(blocks_raw):
            raw_block = blocks_raw[source_cursor]
            source_cursor += 1
            if not isinstance(raw_block, Mapping):
                return None
            if str(raw_block.get("candidate_label", "")) == target_label:
                matched = raw_block
                break
        if matched is None:
            return None
        terms_raw = matched.get("runtime_terms_exyz", ())
        if not isinstance(terms_raw, Sequence) or isinstance(
            terms_raw,
            (str, bytes, bytearray),
        ):
            return None
        ...
        block["runtime_start"] = int(runtime_start)
        block["runtime_count"] = int(len(terms_raw))
        filtered_blocks.append(block)
        runtime_start += int(len(terms_raw))

    repaired = dict(parameterization)
    repaired["blocks"] = filtered_blocks
    repaired["logical_operator_count"] = int(len(filtered_blocks))
    repaired["runtime_parameter_count"] = int(runtime_start)
    try:
        return deserialize_layout(repaired)
...
class CompileScoutConfig:
    ...
    molecular_vibronic_h2o_linear_fd_fixture_json: Path | None = None
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Explicit fixture override for historical H2O ADAPT JSON files that "
            "predate fixture-path serialization."
        ),
    )
    return parser
...
def resolve_compile_scout_config(args: argparse.Namespace) -> CompileScoutConfig:
    ...
    return CompileScoutConfig(
        ...
        molecular_vibronic_h2o_linear_fd_fixture_json=(
            ...
            else Path(args.molecular_vibronic_h2o_linear_fd_fixture_json)
        ),
    )
...
def run_compile_scout(cfg: CompileScoutConfig) -> dict[str, Any]:
    ...
    if cfg.molecular_vibronic_h2o_linear_fd_fixture_json is not None:
        settings_overrides["molecular_vibronic_h2o_linear_fd_fixture_json"] = str(
            cfg.molecular_vibronic_h2o_linear_fd_fixture_json
        )
    source_payload = _load_adapt_result(source_json)
    if settings_overrides:
        ...
        bundle = reconstruct_imported_adapt_circuit(source_payload)
    ...
    warnings: list[str] = []
    runtime_loader_warning = bundle.get("runtime_loader_warning")
    if runtime_loader_warning is not None:
        warnings.append(f"Runtime loader failed; Hubbard-Holstein legacy reconstruction used: {runtime_loader_warning}")
    ...
    logical_circuit = {
        ...
        "reconstruction_source": str(bundle.get("reconstruction_source", "unknown")),
    }
    measurement_cost = {
        ...
        "hamiltonian_source": str(bundle.get("reconstruction_source", "unknown")),
        "valid_for_declared_problem": True,
    }
    summary = {
