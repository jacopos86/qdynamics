...
class StaticPruneCurvatureCache:
    ...
    the remove-refit energy check in :func:`apply_pruning`.
    """
...
def _normalized_prune_policy(policy: str | None) -> str:
    raw = str(policy or PRUNE_POLICY_LEGACY_SMALL_ANGLE_V1).strip().lower()
    aliases = {
        "legacy": PRUNE_POLICY_LEGACY_SMALL_ANGLE_V1,
        "small_angle": PRUNE_POLICY_LEGACY_SMALL_ANGLE_V1,
        "small_angle_v1": PRUNE_POLICY_LEGACY_SMALL_ANGLE_V1,
    ...
    if raw not in {PRUNE_POLICY_LEGACY_SMALL_ANGLE_V1, PRUNE_POLICY_RECOVERABILITY_LADDER_V1}:
...
def resolve_prune_tolerance_mode(*, mode: str | None, prune_policy: str | None) -> str:
    ...
    ``auto`` follows the manuscript route: adaptive tolerance is active only for
    recoverability-ladder pruning; legacy small-angle pruning stays fixed unless
    ...
    if raw == PRUNE_TOLERANCE_AUTO:
        policy_eff = _normalized_prune_policy(prune_policy)
        return (
            PRUNE_TOLERANCE_ADAPTIVE_V1
            if policy_eff == PRUNE_POLICY_RECOVERABILITY_LADDER_V1
            else PRUNE_TOLERANCE_FIXED
        )
...
def compute_prune_regression_tolerance(
    *,
    ...
    safety check in :func:`apply_pruning` and
    :func:`recoverability_prune_ladder`.
    """
...
def evaluate_prune_permission(
    *,
    ...
    does not open the deletion cadence.  Legacy small-angle pruning keeps the
    old low-SNR/checkpoint cadence.
    """
    ...
    if not base_ok:
        for key, value in base_gates.items():
            if not bool(value):
                reason = str(key)
                break
    elif policy_eff == PRUNE_POLICY_RECOVERABILITY_LADDER_V1:
        ...
        if bool(checkpoint_due):
            permission_open = True
            reason = "checkpoint_due"
        elif bool(snr_low_enough):
            permission_open = True
            reason = "legacy_low_snr"
        else:
            reason = "legacy_admitted_gain_resolved"
...
def build_static_prune_surrogate_scores(
    *,
    ...
    ``surrogate_scores``; ``apply_pruning`` remains the commit boundary.
    """
...
def recoverability_prune_ladder(
    *,
    ...
    retained-gain/curvature guards. Amplitude-witness rows are diagnostic only.
    """
