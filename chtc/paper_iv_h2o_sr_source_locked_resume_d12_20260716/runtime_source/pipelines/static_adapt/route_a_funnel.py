...
def run_route_a_child_funnel(
    ...
    *,
    ...
    def _evaluate_full_records(
        records: Sequence[Mapping[str, Any]],
    ) -> list[dict[str, Any]]:
        if full_record_evaluator is None:
        ...
        for record in records:
            if _representation(record) == "parent_macro":
                ...
                continue
            evaluated.append(dict(full_record_evaluator(record)))
        return evaluated
    ...
    if hierarchical_child_stages:
        ...
        phase2_evaluated_records = _evaluate_full_records(phase1_records)
        if (
        ...
        phase2_records = _evaluate_full_records(phase1_input_records)
