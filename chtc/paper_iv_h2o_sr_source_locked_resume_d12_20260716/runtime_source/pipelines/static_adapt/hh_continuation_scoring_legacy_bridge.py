...
def _with_current_feature_defaults(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Fill current-scorer dataclass defaults absent from legacy features."""
    out = dict(payload)
    for field in dataclasses.fields(_CURRENT.CandidateFeatures):
...
def _coerce_current_cfg(cfg: FullScoreConfig) -> Any:
    return _CURRENT.FullScoreConfig(
        ...
        gamma_N_schedule_mode=str(getattr(cfg, "gamma_N_schedule_mode", "fixed")),
        gamma_N_schedule_start=(
            None
            if getattr(cfg, "gamma_N_schedule_start", None) is None
            else float(getattr(cfg, "gamma_N_schedule_start"))
        ),
        gamma_N_schedule_end=(
            None
            if getattr(cfg, "gamma_N_schedule_end", None) is None
            else float(getattr(cfg, "gamma_N_schedule_end"))
        ),
    ...
    score_payload.update(
        {
            ...
            "gamma_N_accepted_depth": (None if accepted_depth is None else int(accepted_depth)),
            "gamma_N_max_depth": (None if max_depth is None else int(max_depth)),
            "motif_bonus": float(getattr(base_feature, "motif_bonus", 0.0)),
            "phase3_duplicate_penalty": float(getattr(base_feature, "phase3_duplicate_penalty", 0.0)),
        }
    )
    score_feat = SimpleNamespace(**_with_current_feature_defaults(score_payload))
    exact_score, exact_fallback = _CURRENT.full_v2_score(score_feat, exact_cfg)
    gamma_context = _CURRENT.novelty_gamma_schedule_context(
        exact_cfg,
        accepted_depth=accepted_depth,
        max_depth=max_depth,
    )
    exact_n3 = float(
        max(0.0, min(1.0, float(score_feat.novelty)))
        ** max(0.0, float(gamma_context.get("gamma_N_effective", getattr(exact_cfg, "gamma_N", 1.0))))
    )
    return {
        ...
        "exact_reduced_N3": float(exact_n3),
        "exact_reduced_gamma_N_effective": float(gamma_context.get("gamma_N_effective", getattr(exact_cfg, "gamma_N", 1.0))),
        "exact_reduced_gamma_N_schedule_mode": str(gamma_context.get("gamma_N_schedule_mode", getattr(exact_cfg, "gamma_N_schedule_mode", "fixed"))),
        "exact_reduced_gamma_N_depth_fraction": gamma_context.get("gamma_N_depth_fraction"),
