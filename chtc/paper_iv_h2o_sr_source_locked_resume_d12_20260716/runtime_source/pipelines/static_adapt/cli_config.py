...
from pipelines.static_adapt.route_identity import ROUTE_ID_CHOICES, STATIC_META_FEATURE_PROFILE_CHOICES
...
_PHASE_BATCH_SELECTION_MODE_CHOICES = (
    "reduced_plane",
...
_PHASE3_BATCH_ORDER_SELECTION_MODE_CHOICES = (
    "finite_step_v1",
    "score_sorted",
)
...
def _build_adapt_arg_parser(*, adapt_gradient_parity_rtol: float) -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--static-route-id",
        choices=list(ROUTE_ID_CHOICES),
        default="unspecified",
        help=(
            "Static SNAKE route identity claim for ADAPT artifacts. route_a is the "
            "only first-class SNAKE route; unspecified remains available for "
            "diagnostic and historical artifacts. Optimizer choice remains separate."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        choices=list(STATIC_META_FEATURE_PROFILE_CHOICES),
        default="off",
        help=(
            "Static SNAKE feature-bundle contract. safe_core_v1 keeps the hard "
            "Route-A selector/novelty/pool identity fixed while allowing approved "
            "feature on/off choices such as batching and pruning. "
            "paper_i_production_v1 is the clean Paper-I/Table-I production "
            "contract: candidate-position SNAKE with Phase0/algebraic/prune "
            "enabled, reduced Phase-III geometry, phase liveness off, zero "
            "position-shift/family-repeat/motif score priors, and batching/repeats "
            "as the allowed feature toggles."
        ),
    )
    p.add_argument(
        "--static-lane-route",
        ...
        default="algebraic",
        help=(
            "Lane partition used by staged static ADAPT shortlisting. algebraic "
            "preserves the existing support/commutation route; physical_operator_type "
            "uses problem-specific physical families such as HH electronic/phonon "
            "lanes, Hubbard UCCSD singles/doubles, and structured spin-boson or "
            "Bose-Hubbard full-meta lanes."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            "Executable SR-SNAKE route profile. sr_snake_v1 and "
            "supported_whitened_adaptive_trust_v1 both materialize the full "
            "historical high-accuracy Paper-I policy and fail closed on any "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "supported-metric v1 changes only the retained singleton "
            "coordinate/Gram solve; global-trust v2 is reserved for an "
    ...
    p.add_argument(
        ...
        choices=list(GRAM_NOVELTY_POLICY_CHOICES),
        default=GRAM_NOVELTY_POLICY_ORDINARY_MULTIPLIER_V1,
        help=(
            "Whether Phase-II Gram novelty multiplies the ordinary SR score. "
            "fallback_only_v1 keeps the supported Gram for whitening/trust but "
            "uses DeltaE/(1+K) in ordinary ranking."
        ),
    )
    p.add_argument(
        ...
        default=GRAM_NOVELTY_POLICY_ORDINARY_MULTIPLIER_V1,
        help=(
            "Whether Phase-III Gram novelty multiplies the ordinary SR score. "
            "fallback_only_v1 uses DeltaE/(1+K) normally while retaining the "
            "explicit all-energy-models-infeasible novelty fallback."
        ),
    )
    p.add_argument(
        "--sr-controller-ablation-contract",
        choices=list(SR_CONTROLLER_ABLATION_CONTRACT_CHOICES),
        default=SR_CONTROLLER_ABLATION_CONTRACT_OFF,
        help=(
            "Explicit run-level authorization marker for narrow SR controller "
            "ablations. This field does not change scoring, pruning, or beam "
            "capacity by itself. novelty_prune_controls_v1 permits only the "
            "separately configured fallback-only Gram-novelty and live/disabled "
            "prune differences; novelty_prune_beam_controls_v1 additionally "
            "permits an explicitly configured one-branch, beam-disabled route."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "unbounded displacement-calibrated policy uses its source-defined "
            "parameters without enabling a JR funnel."
        ),
    )
    p.add_argument(
        "--sr-escape-mode",
    ...
    p.add_argument(
        ...
        choices=[FORMAL_MANIFOLD_WARM_START_OFF, FORMAL_MANIFOLD_ROUTE],
        default=FORMAL_MANIFOLD_WARM_START_OFF,
        help=(
            "Accepted-ansatz reoptimization route. The formal manifold route is an "
            "experimental exact-state, full-coordinate qBroyden-shadow/RBFGS warm "
            "start with query-closed formal Phase-I/II scoring; it keeps a separate "
            "route state and does not alter static_route_id."
        ),
    )
    p.add_argument(
        "--adapt-formal-manifold-route-profile",
        choices=list(FORMAL_MANIFOLD_ROUTE_PROFILE_CHOICES),
        default=FORMAL_MANIFOLD_ROUTE_PROFILE_OFF,
        help=(
            "Resolved Formal-Manifold SNAKE composition profile. The opt-in "
            "SR Phase-II+III profile keeps FM as the outer route, uses the "
            "singleton-response selector, retains measured novelty as "
            "telemetry without applying N2/N3 score multipliers, and selects "
            "the expanded-runtime projected-logical Powell chart."
        ),
    )
    p.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON object overriding typed FormalManifoldConfig fields. "
            "Valid only with --adapt-reoptimization-route="
            f"{FORMAL_MANIFOLD_ROUTE}."
        ),
    )
    p.add_argument(
        "--adapt-state-backend",
    ...
    p.add_argument(
        ...
        default=0,
        help=(
            "Opt-in fixed-local Phase-III geometry window W3 size. "
            "0 preserves legacy coupled nested-window behavior; N>=1 counts the candidate coordinate."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        choices=["append_only", "adaptive", "always"],
        default="append_only",
        help=(
            ...
            "stage-controller plateau/flat/repeated-family triggers; 'always' scores the bounded insertion surface every depth."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="Weak Phase-0 lane exposure pressure; 1 preserves live lanes when a user cap allows.",
    )
    p.add_argument(
        "--phase0-algebraic-lane-mode",
        choices=["weak", "off"],
        default="weak",
        help="Weak Phase-0 algebraic lane exposure mode; does not activate strict algebraic_nested_v1.",
    )
    ...
    p.add_argument(
        "--phase1-prune-policy",
        choices=["legacy_small_angle_v1", "recoverability_ladder_v1"],
        default="legacy_small_angle_v1",
        help=(
            "Static ADAPT prune policy. recoverability_ladder_v1 broadens candidate "
            "nomination/ranking but final deletion still requires remove-refit safety."
        ),
    )
    p.add_argument(
        "--phase1-prune-mode",
    ...
    p.add_argument(
        ...
        default="auto",
        help=(
            "Prune regression tolerance mode. auto keeps legacy small-angle fixed "
            "and uses adaptive_v1 for recoverability_ladder_v1."
        ),
    )
    ...
    p.set_defaults(phase1_prune_amplitude_witness_required=False)
    p.add_argument(
        "--phase1-prune-amplitude-witness-required",
        dest="phase1_prune_amplitude_witness_required",
        action="store_true",
        help=(
            "Compatibility no-op: record amplitude-collapse diagnostics without "
            "making them prune-acceptance authority."
        ),
    )
    p.add_argument(
        ...
        dest="phase1_prune_amplitude_witness_required",
        action="store_false",
        help="Record amplitude witness diagnostics; prune acceptance is not gated by this signal.",
    )
    p.add_argument("--phase1-prune-collapse-peak-abs-min", type=float, default=None)
    p.add_argument("--phase1-prune-collapse-current-abs-max", type=float, default=None)
    p.add_argument("--phase1-prune-collapse-ratio", type=float, default=0.25)
    p.add_argument("--phase1-prune-collapse-min-abs-drop", type=float, default=None)
    p.add_argument("--phase1-prune-collapse-min-observations", type=int, default=3)
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=0.10,
        help="Relative lane-health floor for algebraic nested Phase-2 shortlist survival.",
    )
    p.add_argument(
        "--algebraic-phase1-lane-quota-pressure",
        type=float,
        default=0.70,
        help="Quota pressure for algebraic Phase-1 lane budgets; 1 preserves all live lanes when cap allows.",
    )
    p.add_argument(
        "--algebraic-phase2-lane-quota-pressure",
        type=float,
        default=0.70,
        help="Quota pressure for algebraic Phase-2 lane-health budgets; lower values favor global score rank.",
    )
    ...
    p.set_defaults(phase_live_hysteresis_enabled=True)
    p.add_argument(
        "--phase-live-hysteresis-enabled",
        dest="phase_live_hysteresis_enabled",
        action="store_true",
    )
    p.add_argument(
    ...
    p.add_argument("--phase2-null-nrem-high-threshold", type=float, default=0.0)
    p.add_argument("--phase2-live-nrem-low-threshold", type=float, default=0.25)
    p.add_argument("--phase3-null-nrem-high-threshold", type=float, default=0.5)
    p.add_argument("--phase3-live-nrem-low-threshold", type=float, default=1.0)
    p.add_argument("--phase2-hysteresis-steps", type=int, default=2)
    p.add_argument("--phase3-hysteresis-steps", type=int, default=2)
    p.add_argument(
        "--phase2-lambda-H",
        type=float,
        default=1e-6,
    ...
    p.add_argument(
        ...
        choices=["fixed", "depth_linear_v1"],
        default="fixed",
        help="Optional depth-based schedule for gamma_N; fixed preserves the scalar exponent.",
    )
    p.add_argument("--phase2-gamma-N-schedule-start", type=float, default=None)
    p.add_argument("--phase2-gamma-N-schedule-end", type=float, default=None)
    p.add_argument(
    ...
    p.add_argument(
        "--phase2-novelty-mode",
        choices=_PHASE2_NOVELTY_MODE_CHOICES,
        default="collective_span_v1",
        help=(
            "Phase-II raw novelty mode. collective_span_v1 is the Math.md canonical "
            "inherited-window tangent-span residual; legacy_pairwise_v1 restores the old "
            "pairwise overlap proxy with its historical confidence multiplier."
        ),
    )
    p.add_argument(
    ...
    p.set_defaults(phase2_enable_batching=None, phase3_enable_batching=None)
    ...
    p.add_argument(
        ...
        dest="phase3_enable_batching",
        action="store_true",
        help="Preferred alias: enable Phase-III post-shortlist batch admission.",
    )
    p.add_argument(
        "--phase3-no-batching",
        dest="phase3_enable_batching",
        action="store_false",
        help="Preferred alias: disable Phase-III post-shortlist batch admission.",
    )
    p.add_argument(
        "--phase2-batch-selection-mode",
        choices=_PHASE_BATCH_SELECTION_MODE_CHOICES,
        default=None,
        help=(
            "Legacy alias for the Phase-III post-shortlist batch admission selector. Defaults to reduced-plane; "
            "tetris_disjoint_benchmark, overlap_orthogonal_benchmark, and "
            "ceo_commuting_benchmark are opt-in benchmark-only hard-filter selectors."
        ),
    )
    p.add_argument(
        ...
        default=None,
        help=(
            "Preferred Phase-III post-shortlist batch admission selector. Defaults to reduced_plane; "
            "hard-filter modes are diagnostic/benchmark controls, not canonical geometry-first Route A."
        ),
    )
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Legacy alias for the target number of near-degenerate candidates admitted into a Phase-III batch step.",
    )
    p.add_argument(
        "--phase3-batch-target-size",
        type=int,
        default=None,
        help="Preferred Phase-III batch target size.",
    )
    p.add_argument(
        "--phase2-batch-size-cap",
        type=int,
        default=None,
        help="Legacy alias for the hard cap on candidates admitted into a Phase-III batch step.",
    )
    p.add_argument(
        "--phase3-batch-size-cap",
        type=int,
        default=None,
        help="Preferred Phase-III batch size cap.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Preferred Phase-III near-degenerate shell ratio.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Preferred Phase-III reduced-plane batch rank-floor tolerance.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Preferred Phase-III reduced-plane batch additivity-defect tolerance.",
    )
    p.add_argument(
        "--phase3-batch-prefilter-mode",
        choices=_PHASE3_BATCH_PREFILTER_MODE_CHOICES,
        default="off",
        help=(
            "Optional Phase-III exact structural prefilter. Defaults to off; canonical Route A is "
            "geometry-first when this is disabled."
        ),
    )
    p.add_argument(
        "--phase2-batch-order-selection-mode",
        choices=_PHASE3_BATCH_ORDER_SELECTION_MODE_CHOICES,
        default=None,
        help=(
            "Legacy alias for ordered batch-word selection. finite_step_v1 is the default; "
            "score_sorted restores the old score-sorted batch admission order."
        ),
    )
    p.add_argument(
        ...
        default=None,
        help=(
            "Preferred ordered batch-word selector. finite_step_v1 scores candidate batch orderings "
            "by a finite-step energy proxy; score_sorted restores legacy score-sorted admission."
        ),
    )
    p.add_argument(
        ...
        type=int,
        default=24,
        help="Maximum batch orderings scored by the finite-step ordered-batch proxy.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "algebraic_nested_v1 opts into Slice-B algebraic lanes and nested post-admission refit."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        choices=["off", "no_phase2", "no_phase3", "all"],
        default="off",
        help=(
            "Ablation-only novelty bypass. 'off' preserves Route-A source scoring "
            "(Phase-II collective novelty active, no extra Phase-III multiplier); "
            "'no_phase2' removes the raw tangent-overlap novelty screen; "
            "'no_phase3' removes reduced-window novelty from full_v2 scoring; "
            "'all' removes both while preserving telemetry fields."
        ),
    )
    p.add_argument(
...
def _build_run_hardcoded_adapt_vqe_kwargs(
    ...
    *,
    ...
    # any batching aliases or runtime kwargs are derived.
    ...
    def _resolve_alias(
        ...
        *,
        ...
        if legacy_value is not None and phase3_value is not None and legacy_value != phase3_value:
            raise ValueError(
                f"Conflicting legacy {legacy_name}={legacy_value!r} and preferred "
                f"{phase3_name}={phase3_value!r} values."
            )
        if phase3_value is not None:
            return phase3_value
        if legacy_value is not None:
            return legacy_value
        return default

    phase2_enable_batching_effective = bool(
        _resolve_alias(
            getattr(args, "phase2_enable_batching", None),
            getattr(args, "phase3_enable_batching", None),
            default=True,
    ...
    phase2_batch_selection_mode_effective = str(
        _resolve_alias(
            ...
            getattr(args, "phase3_batch_selection_mode", None),
            default="reduced_plane",
            legacy_name="phase2_batch_selection_mode",
            phase3_name="phase3_batch_selection_mode",
        )
    )
    phase2_batch_target_size_effective = int(
        _resolve_alias(
            getattr(args, "phase2_batch_target_size", None),
            getattr(args, "phase3_batch_target_size", None),
            default=2,
            legacy_name="phase2_batch_target_size",
            phase3_name="phase3_batch_target_size",
        )
    )
    phase2_batch_size_cap_effective = int(
        _resolve_alias(
            getattr(args, "phase2_batch_size_cap", None),
            getattr(args, "phase3_batch_size_cap", None),
            default=3,
            legacy_name="phase2_batch_size_cap",
            phase3_name="phase3_batch_size_cap",
        )
    )
    phase2_batch_near_degenerate_ratio_effective = float(
        _resolve_alias(
            getattr(args, "phase2_batch_near_degenerate_ratio", None),
            getattr(args, "phase3_batch_near_degenerate_ratio", None),
            default=0.9,
            legacy_name="phase2_batch_near_degenerate_ratio",
            phase3_name="phase3_batch_near_degenerate_ratio",
        )
    )
    phase2_batch_rank_rel_tol_effective = float(
        _resolve_alias(
            getattr(args, "phase2_batch_rank_rel_tol", None),
            getattr(args, "phase3_batch_rank_rel_tol", None),
            default=1e-6,
            legacy_name="phase2_batch_rank_rel_tol",
            phase3_name="phase3_batch_rank_rel_tol",
        )
    )
    phase2_batch_additivity_tol_effective = float(
        _resolve_alias(
            getattr(args, "phase2_batch_additivity_tol", None),
            getattr(args, "phase3_batch_additivity_tol", None),
            default=0.25,
            legacy_name="phase2_batch_additivity_tol",
            phase3_name="phase3_batch_additivity_tol",
        )
    )
    phase3_batch_order_selection_mode_effective = str(
        _resolve_alias(
            getattr(args, "phase2_batch_order_selection_mode", None),
            getattr(args, "phase3_batch_order_selection_mode", None),
            default="finite_step_v1",
            legacy_name="phase2_batch_order_selection_mode",
            phase3_name="phase3_batch_order_selection_mode",
        )
    )
    formal_manifold_config = None
    formal_manifold_config_path = getattr(
        args, "adapt_formal_manifold_config_json", None
    )
    if formal_manifold_config_path is not None:
        formal_manifold_config = json.loads(
            Path(formal_manifold_config_path).read_text(encoding="utf-8")
        )
        if not isinstance(formal_manifold_config, dict):
            raise ValueError(
                "--adapt-formal-manifold-config-json must contain a JSON object."
            )
    ...
    return {
        ...
        "static_route_id": str(getattr(args, "static_route_id", "unspecified")),
        "static_meta_feature_profile": str(getattr(args, "static_meta_feature_profile", "off")),
        "static_lane_route": str(getattr(args, "static_lane_route", "algebraic")),
        ...
        "adapt_reoptimization_route": str(args.adapt_reoptimization_route),
        "adapt_formal_manifold_route_profile": str(
            getattr(
                args,
        ...
        "adapt_formal_manifold_config": formal_manifold_config,
        ...
        "phase0_lane_quota_pressure": float(args.phase0_lane_quota_pressure),
        "phase0_algebraic_lane_mode": str(args.phase0_algebraic_lane_mode),
        ...
        "phase1_prune_amplitude_witness_required": bool(args.phase1_prune_amplitude_witness_required),
        "phase1_prune_collapse_peak_abs_min": (
            None if args.phase1_prune_collapse_peak_abs_min is None else float(args.phase1_prune_collapse_peak_abs_min)
        ),
        "phase1_prune_collapse_current_abs_max": (
            None if args.phase1_prune_collapse_current_abs_max is None else float(args.phase1_prune_collapse_current_abs_max)
        ),
        "phase1_prune_collapse_ratio": float(args.phase1_prune_collapse_ratio),
        "phase1_prune_collapse_min_abs_drop": (
            None if args.phase1_prune_collapse_min_abs_drop is None else float(args.phase1_prune_collapse_min_abs_drop)
        ),
        "phase1_prune_collapse_min_observations": int(args.phase1_prune_collapse_min_observations),
        ...
        "algebraic_phase2_lane_rel_threshold": float(args.algebraic_phase2_lane_rel_threshold),
        "algebraic_phase1_lane_quota_pressure": float(args.algebraic_phase1_lane_quota_pressure),
        "algebraic_phase2_lane_quota_pressure": float(args.algebraic_phase2_lane_quota_pressure),
        ...
        "phase_live_hysteresis_enabled": bool(args.phase_live_hysteresis_enabled),
        "phase2_null_nrem_high_threshold": float(args.phase2_null_nrem_high_threshold),
        "phase2_live_nrem_low_threshold": float(args.phase2_live_nrem_low_threshold),
        "phase3_null_nrem_high_threshold": float(args.phase3_null_nrem_high_threshold),
        "phase3_live_nrem_low_threshold": float(args.phase3_live_nrem_low_threshold),
        "phase2_hysteresis_steps": int(args.phase2_hysteresis_steps),
        "phase3_hysteresis_steps": int(args.phase3_hysteresis_steps),
        "phase2_lambda_H": float(args.phase2_lambda_H),
        "phase2_rho": float(args.phase2_rho),
        "phase2_gamma_N": float(args.phase2_gamma_N),
        "phase2_gamma_N_schedule_mode": str(args.phase2_gamma_N_schedule_mode),
        "phase2_gamma_N_schedule_start": (
            float(args.phase2_gamma_N_schedule_start)
            if args.phase2_gamma_N_schedule_start is not None
            else None
        ),
        "phase2_gamma_N_schedule_end": (
            float(args.phase2_gamma_N_schedule_end)
            if args.phase2_gamma_N_schedule_end is not None
            else None
        ),
        ...
        "phase2_novelty_mode": str(args.phase2_novelty_mode),
        ...
        "phase3_batch_order_selection_mode": str(phase3_batch_order_selection_mode_effective),
        "phase3_batch_order_max_permutations": int(args.phase3_batch_order_max_permutations),
        "phase2_batch_target_size": int(phase2_batch_target_size_effective),
        "phase2_batch_size_cap": int(phase2_batch_size_cap_effective),
        "phase2_batch_near_degenerate_ratio": float(phase2_batch_near_degenerate_ratio_effective),
        "phase2_batch_rank_rel_tol": float(phase2_batch_rank_rel_tol_effective),
        "phase2_batch_additivity_tol": float(phase2_batch_additivity_tol_effective),
        ...
        "phase3_novelty_ablation_mode": str(args.phase3_novelty_ablation_mode),
