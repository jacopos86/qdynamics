...
_LEGACY_INCLUDE_ZERO_POINT_GLOBAL = "_HH_LEGACY_INCLUDE_ZERO_POINT"
_LEGACY_INCLUDE_ZERO_POINT_SOURCE_PATCHES: tuple[tuple[str, str], ...] = (
    (
        "include_zero_point=True,",
        f"include_zero_point=bool(globals().get('{_LEGACY_INCLUDE_ZERO_POINT_GLOBAL}', True)),",
    ),
)
_LEGACY_INCLUDE_ZERO_POINT_HELP = (
    "  --include-zero-point  Include the phonon zero-point identity contribution in HH Hamiltonians and exact anchors.\n"
    "  --no-include-zero-point\n"
    "                        Disable the phonon zero-point identity contribution in HH Hamiltonians and exact anchors."
)
...
def _extract_include_zero_point(argv: Sequence[str] | None) -> tuple[bool, list[str] | None]:
    if argv is None:
        raw_args = list(sys.argv[1:])
    else:
        raw_args = [str(arg) for arg in argv]
    ...
    for token in raw_args:
        ...
        if token == "--no-include-zero-point":
            ...
            continue
        stripped.append(token)
    return bool(include_zero_point), stripped


def _emit_legacy_zero_point_help(argv: Sequence[str] | None) -> None:
    raw_args = list(sys.argv[1:]) if argv is None else [str(arg) for arg in argv]
    if any(token in {"-h", "--help"} for token in raw_args):
        print(_LEGACY_INCLUDE_ZERO_POINT_HELP)
...
def _load_legacy_source() -> str:
    ...
    for old_snippet, new_snippet in _LEGACY_INCLUDE_ZERO_POINT_SOURCE_PATCHES:
        if old_snippet in source:
            source = source.replace(old_snippet, new_snippet)
    return source
...
def parse_args(argv: Sequence[str] | None = None) -> Any:
    _emit_legacy_zero_point_help(argv)
    include_zero_point, argv_wo_zero_point = _extract_include_zero_point(argv)
    geometry_mode, stripped_argv = _extract_legacy_geometry_mode(argv_wo_zero_point)
    module = _load_legacy_module(geometry_mode)
    setattr(module, _LEGACY_INCLUDE_ZERO_POINT_GLOBAL, bool(include_zero_point))
    args = module.parse_args(stripped_argv)
    setattr(args, "phase3_legacy_geometry_mode", str(geometry_mode))
    setattr(args, "include_zero_point", bool(include_zero_point))
    return args
