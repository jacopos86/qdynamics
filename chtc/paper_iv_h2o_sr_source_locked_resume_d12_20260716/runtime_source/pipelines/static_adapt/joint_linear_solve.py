...
def factor_supported_metric(
    ...
    *,
    ...
    """Factor a raw metric using the shared supported-whitening convention.

    This is the only public support/ridge convention used by the whitened
    joint solver.  It performs no estimator work and therefore has zero
    quantum-query charge.
    """
    ...
    negative_tolerance = float(
        64.0 * np.finfo(float).eps * max(1, dimension) * metric_abs_scale
    )
    ...
    if eigendecomposition_failed:
        ...
        feasible = False
    elif raw_eigenvalues.size and float(np.min(raw_eigenvalues)) < -negative_tolerance:
    ...
    if feasible:
        ...
        regularized_supported_metric = np.zeros((dimension, dimension), dtype=float)
        raw_metric_pseudoinverse = np.zeros((dimension, dimension), dtype=float)

    provenance_id = _whitening_provenance_id(
        raw_metric=G,
    ...
    return SupportedMetricWhitening(
        ...
        reason=str(reason),
        raw_metric=np.asarray(G, dtype=float).copy(),
