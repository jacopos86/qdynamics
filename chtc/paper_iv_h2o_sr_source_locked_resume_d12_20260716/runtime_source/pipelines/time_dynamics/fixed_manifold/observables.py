...
def _normalize_selected_runtime_indices(
    selected_runtime_indices: Sequence[int] | None,
    *,
    total_runtime: int,
) -> tuple[int, ...]:
    total = int(total_runtime)
    if total <= 0:
        raise ValueError("runtime selection requires at least one runtime rotation.")
    if selected_runtime_indices is None:
        return tuple(range(total))
    out = tuple(sorted({int(idx) for idx in selected_runtime_indices}))
    if not out:
        raise ValueError("selected_runtime_indices must be nonempty when provided.")
    bad = [int(idx) for idx in out if int(idx) < 0 or int(idx) >= total]
    if bad:
        raise ValueError(
            f"selected_runtime_indices out of range for {total} rotations: {bad}."
        )
    return out
...
def build_checkpoint_observable_plan_from_layout(
    ...
    *,
    ...
    selected_runtime_indices: Sequence[int] | None = None,
    ...
    selected_indices = _normalize_selected_runtime_indices(
        selected_runtime_indices,
        total_runtime=len(rotations),
    )
    selection_active = bool(tuple(selected_indices) != tuple(range(len(rotations))))
    selected_rotations = tuple(rotations[int(idx)] for idx in selected_indices)
    ...
    for runtime_idx in selected_indices:
        ...
        gen_spec = _make_observable_spec(
            name=f"A_{int(runtime_idx)}",
            kind="generator_mean",
            nq=int(nq),
            poly_map=a_map,
            runtime_index=int(runtime_idx),
            runtime_pair=None,
            drop_abs_tol=float(drop_abs_tol),
            hermiticity_tol=float(hermiticity_tol),
        )
    ...
    for i in range(len(selected_rotations)):
        for j in range(i + 1, len(selected_rotations)):
            ...
            runtime_i = int(selected_indices[int(i)])
            runtime_j = int(selected_indices[int(j)])
            pair_spec = _make_observable_spec(
                name=f"AAsym_{runtime_i}_{runtime_j}",
                kind="pair_anticommutator",
                nq=int(nq),
                poly_map=pair_map,
                runtime_index=None,
                runtime_pair=(runtime_i, runtime_j),
                drop_abs_tol=float(drop_abs_tol),
                hermiticity_tol=float(hermiticity_tol),
            )
    ...
    return CheckpointObservablePlan(
        circuit=circuit,
        runtime_rotations=tuple(selected_rotations),
        energy=energy,
        ...
        stats={
            "runtime_parameter_count": int(len(selected_rotations)),
            "total_runtime_parameter_count": int(len(rotations)),
            "observable_selection_mode": (
                "runtime_subset" if bool(selection_active) else "full"
            ),
            "selected_runtime_parameter_count": int(len(selected_rotations)),
            "selected_runtime_indices": [int(idx) for idx in selected_indices],
            ...
            "selected_generator_mean_count": int(len(generator_means)),
            "selected_pair_anticommutator_count": int(len(pair_anticommutators)),
            "selected_force_anticommutator_count": int(len(force_anticommutators)),
            "selected_observable_count_total": int(
                2
                + len(generator_means)
                + len(force_anticommutators)
                + len(pair_anticommutators)
            ),
