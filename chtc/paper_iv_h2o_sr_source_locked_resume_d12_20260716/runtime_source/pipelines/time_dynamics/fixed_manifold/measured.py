...
def run_fixed_manifold_measured(
    ...
    *,
    ...
    noise_mode = str(oracle_cfg.noise_mode).strip().lower()
    if noise_mode not in {"ideal", "shots", "aer_noise", "aer_density_matrix", "backend_scheduled", "runtime"}:
        raise ValueError(
            "Measured fixed-manifold v1 supports noise_mode in "
            "{'ideal','shots','aer_noise','aer_density_matrix','backend_scheduled','runtime'}."
        )
    if str(getattr(oracle_cfg, "execution_surface", "expectation_v1")).strip().lower() != "expectation_v1":
        raise ValueError("Measured fixed-manifold v1 requires execution_surface='expectation_v1'.")
    validate_oracle_execution_request(oracle_cfg)
    ...
    payload = {
        ...
        "oracle_config": {
            ...
            "value_noise": {
                "enabled": bool(str(getattr(oracle_cfg, "value_noise_model", "off")) != "off"),
                "model": str(getattr(oracle_cfg, "value_noise_model", "off")),
                "std": float(getattr(oracle_cfg, "value_noise_std", 0.0)),
                "seed": getattr(oracle_cfg, "value_noise_seed", None),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="off",
        help="Opt-in post-expectation value noise for measured fixed-manifold oracle evaluations; not physical shots.",
    )
    parser.add_argument("--value-noise-std", type=float, default=0.0)
    parser.add_argument("--value-noise-seed", type=int, default=None)
    parser.add_argument("--backend-name", type=str, default=None)
    parser.add_argument("--use-fake-backend", action="store_true")
    parser.add_argument("--seed-transpiler", type=int, default=None)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    oracle_cfg = OracleConfig(
        ...
        backend_name=(None if args.backend_name in {None, ""} else str(args.backend_name)),
        use_fake_backend=bool(args.use_fake_backend),
        seed_transpiler=(None if args.seed_transpiler is None else int(args.seed_transpiler)),
        transpile_optimization_level=int(args.transpile_optimization_level),
        value_noise_model=str(args.value_noise_model),
        value_noise_std=float(args.value_noise_std),
        value_noise_seed=(None if args.value_noise_seed is None else int(args.value_noise_seed)),
    )
    geom_cfg = FixedManifoldMeasuredConfig(
