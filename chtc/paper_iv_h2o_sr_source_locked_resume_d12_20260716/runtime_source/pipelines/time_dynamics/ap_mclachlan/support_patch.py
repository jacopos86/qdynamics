...
def build_support_patch_before_cache(
    *,
    ...
    rank_before = _matrix_rank(_apply_ridge(K_before, inverse_policy.ridge_lambda))
    before_inverse = None
...
def schur_novelty(
    *,
    ...
    if n == 0:
        ...
        inverse_w = _shared_supported_inverse(K_w, policy=inverse_policy)
        projected = np.asarray(K_wa.T @ inverse_w.inverse @ K_wa, dtype=float)
...
def score_support_patch(
    *,
    ...
    if cache is None:
        ...
        rank_before = _matrix_rank(
            _apply_ridge(K_before_raw, inverse_policy.ridge_lambda)
        )
    else:
    ...
    return SupportPatchScore(
        ...
        solve_damping=float(inverse_policy.solve_damping),
        rank_before=rank_before,
        rank_after=_matrix_rank(_apply_ridge(K_after_raw, inverse_policy.ridge_lambda)),
