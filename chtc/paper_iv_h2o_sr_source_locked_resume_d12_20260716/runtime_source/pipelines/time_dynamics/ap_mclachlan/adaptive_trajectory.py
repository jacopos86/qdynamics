...
class _PruneControllerRuntimeState:
    ...
    def update_after_support_change(
        ...
        *,
        ...
        active_atom_ids = {
            str(atom.atom_id)
            for atom in active_support_atoms(new_state, theta_runtime)
            if str(atom.atom_id) != ""
        }
    ...
    for index, time_value in enumerate(time_grid):
        ...
        evaluation = evaluate_mclachlan_geometry(
            ...
            include_tangent_matrix=(
                _needs_prune_patch_smoothness(support_patch_config)
...
def _integrate_interval_with_repair(
    *,
    ...
    def try_interval(local_theta: np.ndarray, local_t: float, local_dt: float) -> tuple[IntegrationStep, tuple[FixedMcLachlanStep, ...]]:
    ...
    try:
        full_step, fixed_steps = try_interval(theta0, float(time), float(dt))
        ...
        active_request = force_local_subdivision_request or inferred_request
        first_reason = (
    ...
    for depth in range(start_depth, max_depth + 1):
        ...
        try:
            for _ in range(substep_count):
                substep, fixed_steps = try_interval(local_theta, local_t, sub_dt)
    ...
    if float(base_step.residual_ratio) < float(support_config.residual_ratio_threshold):
        ...
        if not append_atoms:
            ...
            for branch in _top_patch_branches(
                ...
                branch_atoms = _atoms_from_candidate_metadata(
                    branch,
                    append_by_id,
                )
                finalists.append(
...
def _materialize_support_patch_atom_set(
    *,
    ...
    return patched_state, theta_patched, evaluation, step
...
def _evaluate_deletion_patch_safety(
    *,
    ...
    try:
        patched_state, theta_patched, evaluation, step = _materialize_support_patch_atom_set(
            state=state,
            hamiltonian=hamiltonian,
            theta_runtime=theta_runtime,
            time=float(time),
    ...
    *,
    ...
    try:
        ...
        geometry = SupportPatchGeometry(
            ...
            f_before=base_f,
            norm_b_sq=float(base_evaluation.geometry.norm_b_sq),
            K_insert_cross=K_aug[:n_before, n_before:],
            K_insert_insert=K_aug[n_before:, n_before:],
        ...
        delete_score = score_support_patch(
            geometry=SupportPatchGeometry(
                K_before=base_K,
                f_before=base_f,
                norm_b_sq=float(base_evaluation.geometry.norm_b_sq),
        ...
        append_score = score_support_patch(
            geometry=geometry,
            patch=SupportPatch(
                inserted_count=m_insert,
                inserted_labels=inserted_labels,
...
def _prune_atom_ids_from_score(candidate: PatchCandidateScore) -> tuple[str, ...]:
    atom_ids = candidate.metadata.get("atom_ids", ())
