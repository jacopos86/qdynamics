...
PROFILE_ALIAS = "sr_snake_no_novelty_metric_prune_beam_v1"
RECORD_ID = (
...
def profile_diff_audit(*, repo: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    baseline = dict(CANONICAL_SR_SNAKE_V3_EXECUTION_SETTINGS)
    target = dict(
        CANONICAL_SR_SNAKE_NO_NOVELTY_METRIC_PRUNE_BEAM_V1_EXECUTION_SETTINGS
    )
    differences = {
        field: {"paper_i_sr_v3": baseline.get(field), "h2o_target": target.get(field)}
    ...
    contract = canonical_sr_snake_no_novelty_metric_prune_beam_v1_contract()
    result_evidence_comparison = paper_i_result_evidence_comparison(
        repo=repo, target=target
    )
    audit = {
        "schema": "paper_iv_h2o_vs_paper_i_sr_v3_settings_audit_v1",
        ...
        "paper_i_result_evidence_comparison": result_evidence_comparison,
    }
    if result_evidence_comparison["status"] != "pass":
        audit["status"] = "blocked"
    if audit["status"] != "pass":
        raise SystemExit(
            "Paper-I SR-v3 difference audit blocked: "
...
def parser_preflight(repo: Path, argv: list[str]) -> dict[str, Any]:
    ...
    expected = CANONICAL_SR_SNAKE_NO_NOVELTY_METRIC_PRUNE_BEAM_V1_EXECUTION_SETTINGS
    mismatches = {
    ...
    return {
        "schema": "paper_iv_h2o_sr_command_preflight_v1",
        "status": "pass",
        "checks": checks,
...
def main() -> None:
    ...
    command = {
        "schema": "paper_iv_h2o_sr_depth50_command_v1",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "record_id": RECORD_ID,
    ...
    manifest = {
        "schema": "paper_iv_h2o_sr_depth50_input_manifest_v1",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "record_id": RECORD_ID,
        "files": files,
