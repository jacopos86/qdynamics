...
def build_deliverables(
    *, root: Path, outdir: Path, record_id: str, command: dict[str, Any]
) -> dict[str, Any]:
    ...
    for name in ("shell_status.json", "runtime_preflight.json", "result_validation.json"):
        ...
        if source.is_file():
            ...
            copied.append(name)
    compressed = []
    for name in ("current.json", "stdout.log", "stderr.log"):
        source = outdir / name
        if source.is_file():
    ...
    manifest = {
        "schema": "paper_iv_h2o_sr_depth50_deliverables_manifest_v1",
...
def latest_progress(stdout_path: Path, record_id: str) -> dict[str, Any]:
    payload: dict[str, Any] = {
        "schema": "paper_iv_h2o_sr_depth50_progress_v1",
        "timestamp_utc": utc_now(),
        "record_id": record_id,
    }
...
def main() -> None:
    ...
    if preflight.returncode != 0:
        write_json(
            status_path,
            {
                "schema": "paper_iv_h2o_sr_depth50_shell_status_v1",
                "timestamp_utc": utc_now(),
                "record_id": record_id,
