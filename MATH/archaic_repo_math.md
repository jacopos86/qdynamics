...
This multiplier changes the reference path only. It does **not** change the reported-trajectory macro-step count.

Code anchor: `pipelines/hardcoded/hubbard_pipeline.py`

## 12.5 Filtered exact manifold and subspace fidelity
...
3. propagated trajectory from the PAOP / ADAPT handoff state,
4. exact propagation from the hardcoded VQE branch,
5. propagated trajectory from the hardcoded VQE branch.
...
This appendix records material that appears in `notes/IMPLEMENT_SOON.md` or `MATH/IMPLEMENT_NEXT.md` but is **not** promoted into the main manuscript as an implemented claim unless a direct code anchor was located in the scoped audit above.
...
## 13.5 Filtered exact manifold and subspace fidelity
