...
For macro-generator records retained by the Phase-II shortlist, SNAKE may replace \(r=(m,p)\) by exposed records \(r_J=((m,J),p)\), where \(A_{m,J}=\sum_{\mu\in J}a_{m\mu}P_{m\mu}\) is a nonempty Pauli subexpansion of \(A_m\). These exposed records use the same \(S_{3,\ell}\) score and the same batch rule as ordinary records, but \(K_3(r_J)\) is evaluated on the exposed Pauli support and compiled-resource burden. With the fermionic hard guard enabled, only exposed records satisfying \(\Gamma_{\rm hard}(r_J)=\mathbf 1\{[A_{m,J},N]\approx0,\,[A_{m,J},N_\uparrow]\approx0,\,[A_{m,J},N_\downarrow]\approx0\}\) are admissible.
...
\]
and the operative state for recoverability eligibility is
\[
u_j(t)=(a_j,c_j(t)),
\]
where \(a_j\) is the admission step of \(d_j\) and \(c_j(t)\) is its current cooldown remaining.  The implementation also records \(f_j,\zeta_j,\beta_j,\ell_j\) as telemetry/bookkeeping fields, but the recoverability eligibility rule below uses only \(a_j\) and \(c_j(t)\).
...
\]
be the post-admission, post-refit child operator sequence, with optimized angles \(\theta_k^+\).  For a candidate deletion coordinate \(i\), form the survivor sequence
\[
S_i=(d_1,\ldots,d_{i-1},d_{i+1},\ldots,d_{n_k}),
\qquad
N_i:=n_k-1 .
\]
...
\]
\[
a_i:=\min\{A_i,N_i-L_i+1\}.
\]
Writing \(S_{i,r}\) for the \(r\)-th entry of \(S_i\), the local survivor window is
\[
W_i^{\mathrm{Schur}}
:=
(S_{i,a_i},\ldots,S_{i,a_i+L_i-1}),
\qquad
|W_i^{\mathrm{Schur}}|\le 4 .
\]
Thus an interior deletion uses
\(W_i^{\mathrm{Schur}}=(d_{i-1},d_{i+1},d_{i+2},d_{i+3})\); near the right boundary the slice shifts left, e.g. one right survivor and three available left survivors gives \(W_i^{\mathrm{Schur}}=(d_{i-3},d_{i-2},d_{i-1},d_{i+1})\).  The window shrinks only when fewer than four survivors exist.

For each eligible \(i\), the Schur surrogate computes the deletion-loss proxy
\[
\begin{aligned}
\Lambda_i^{\ominus}
&:=
\Lambda_i^{\ominus}(W_i^{\mathrm{Schur}})
\\
&\approx
\frac12(\theta_{k,i}^+)^2
\left(
...
\end{aligned}
\]
where \(W_i=W_i^{\mathrm{Schur}}\) and the local Schur ridge is \(\lambda=10^{-6}\).  The live recoverability prune ranking key is
\[
K_i=
\bigl(
\Lambda_i^{\ominus},
...
\bigr),
\]
with smaller \(K_i\) preferred lexicographically.  Here
\[
b_i:=\frac{q_i}{1+\max(0,\beta_i)}
\]
is the stored selector/admission score \(q_i\) discounted by the stored selector burden \(\beta_i\), and
\[
\bar\theta_i
:=
\operatorname{wrap}_{[-\pi,\pi)}(\theta_{k,i}^+).
\]
The stored small-angle stagnation score is
\[
s_i
:=
\max\left\{0,1-\frac{|\bar\theta_i|}{m_k^\theta}\right\},
\]
\[
m_k^\theta
:=
...
\right\},
\]
where the median is over the current scaffold coordinates.
The small-angle pool flag is
\[
\alpha_i
:=
\mathbf 1\{|\bar\theta_i|>\tau_{\theta,k}\},
\]
\[
m_{k,\mathcal E}^{\theta}
:=
...
\right\}.
\]
Thus \(\alpha_i=0\) marks small-angle-pool membership, \(s_i\) is the continuous smallness score, and \(\ell_i\) is the deterministic coordinate label.  The tested set is
\[
\begin{aligned}
\mathcal N_k^{\ominus}
&:=
...
\left(
\operatorname{sort}_{K_i}
\mathcal E_k^{\ominus}
\right)
:
\Lambda_i^{\ominus}\le \epsilon_k^{\ominus}
\right\},
\\
\mathcal C_k^{\ominus}
&:=
...
\end{aligned}
\]
Here \(\mathcal E_k^{\ominus}\) is the eligible deletion-coordinate set, \(K_i\) is the Schur-based ranking key, \(M_{\max}\) is the nomination-list cap, \(\Lambda_i^{\ominus}\) is the Schur deletion-loss proxy, and \(\mathcal C_k^{\ominus}\) is the set actually sent to the measured delete test.  For the Hubbard--Holstein rows in this path, \(M_{\max}=6\) and the exact delete-refit trial cap is \(1\).  Thus the tested generator is \(d_j\) with \(j\in\mathcal C_k^{\ominus}\subseteq\{1,\ldots,n_k\}\), not an arbitrary generator in the ansatz.  Equivalently, \(\mathcal C_k^{\ominus}=\operatorname{first}_{1}\mathcal N_k^{\ominus}\) is the first coordinate in the \(K_i\)-sorted eligible list satisfying the Schur-loss gate,
\[
\Lambda_i^{\ominus}\le \epsilon_k^{\ominus}.
\]
...
\operatorname{first}
\left\{
i\in \operatorname{sort}_{K_i}(\mathcal E_k^{\ominus})
:
\Lambda_i^{\ominus}\le \epsilon_k^{\ominus}
\right\}.
\]
