...
\newunicodechar{’}{'}
\usepackage{graphicx}
\graphicspath{{}{figures/}{output/pdf/}}
...
\structlabel{Motivation/Background Intro}
...
\structlabel{Modeled Problem (Condensed Many-body)}
...
\]
%
where \(A_m\in\mathcal{G}\) is the candidate generator and \(p\) is an admissible insertion position in the current circuit 
...
\]
and then apply a sequence of scoring phases indexed by \(t\in\{0,1,2,3\}\). Within each phase, records are partitioned into lanes \(\ell\in\mathcal L\), defined by qubit-support overlap and windowed commutation structures. Each phase maps the previous staged-recursion set \(\mathcal R_k^{(t-1)}\) to \(\mathcal R_k^{(t)}\) by lane-wise score thresholding
...
%
[
\textcolor{red}{CAREFUL HERE - IT IS NOT EXACTLY ANALOGOUS - THE ANALOGOUS WOULD BE THE EFFECTIVE CUMULATIVE SCORE\\
which is the analogue of the argument in Eq.~(\ref{eq:adapt_append_gradient})
}
]\\
...
\noindent
For a system of \(Q\) qubits, the compiled generator associated with a candidate record \(r=(m,p)\) can be expressed as a Pauli polynomial, 
\[
A_m=\sum_{\mu}a_{m\mu}P_{m\mu}, \qquad P_{m\mu}\in\{I,X,Y,Z\}^{\otimes Q}\,.
\]
%
...
\begin{aligned}
S_{r\mu}
&:=\operatorname{supp}(P_{r\mu}) \\
&=\{q\in\{1,\ldots,Q\}:(P_{r\mu})_q\ne I\}.
\end{aligned}
\label{eq:pauli_word_support}
\end{equation}
%
The cost estimates below are computed from these Pauli-word supports. Larger supports require longer parity-computation circuits, while supports spread across the hardware graph require larger entangling depth or routing overhead. For a Pauli exponential compiled by a CNOT ladder, a multi-qubit Pauli rotation is implemented by reducing it to a single-qubit rotation with additional entangling gates. A Pauli word acting on \(s=|S_{r\mu}|\) qubits requires \(s-1\) \textsc{CNOT}s before the \(R_z\) rotation and \(s-1\) \textsc{CNOT}s after it to restore the original qubit register. the two-qubit gate count is estimated by
%
\begin{equation}
\widehat C_{\twq}(r)=\sum_{\mu}2\,\max[|S_{m\mu}|-1, 0],
...
\end{equation}
Here \(G\) is the QPU connectivity graph, and $\operatorname{span}_{G}(S_{m \mu})$ is the number of edges in the shortest connected subgraph containing the support \(S_{m \mu}\). The quantity is set to zero when $|S_{m \mu}|\le1$. This term penalizes Pauli words whose active qubits are far apart in the connected graph.\\
\noindent
The same Pauli-rotation circuit also uses one-qubit basis-change gates. Let
\[
S_{m\mu}^{XY} = \{q\in S_{m\mu}:(P_{m\mu})_q = X, Y\}
\]
On each qubit in \(S_{m\mu}^{XY}\), we rotate into the \(Z\) basis before the Pauli rotation and rotate it back afterward. If the required basis rotation is $\beta_{m\mu q}$, its forward and back cost is 
\[
c_q^{\pm}(\beta_{m\mu q})=c_q(\beta_{m\mu q})+c_q(\beta^{-1}_{m\mu q})\,.
\]
The one-qubit cost of this Pauli word is then given by
%
\begin{equation}
\widehat{C}_{1q}^{(\mu)}(r) = \sum_{q\in S_{m\mu}^{XY}}c_q^\pm(\beta_{m\mu q}) + c_{\bar{q}_{m\mu}}(R_z)\,.
\end{equation}
%
...
%\end{equation}
Here \(\bar{q}_{m\mu}\) is the qubit where the central rotation \(R_z\) is applied. The total one-qubit cost then becomes \(\widehat{C}_{1q}(r)=\sum_\mu\widehat{C}_{1q}^{(\mu)}(r)\). The parameter cost is the number of new independent parameters added by candidate \(r\):
...
%
\noindent
For a candidate record $r$ at ADAPT iteration $k$, the magnitude of the candidate-gradient is always computed as 
\begin{equation}
\begin{aligned}
g
&=
...
%
\noindent
Phase 0 is the inexpensive first pass screening stage. It is obtained from Eq.~\eqref{eq:phase0_score} by using only the standard first-order ADAPT gradient and the phase-0 hardware-cost penalty. For a candidate record \(r=(m,p)\), let \(g_0(r)\) denote the magnitude of the first-order energy gradient associated with inserting generator \(A_m\) at position \(p\). The Phase-0 score is
%
\begin{equation}
S_{0}(r)=\frac{\alpha_0 g_0(r)}{K_0(r)}\,.
\label{eq:phase0_score}
\end{equation}
%
Here \(\alpha_0>0\) is a fixed pre-screening scale factor, and \(K_0(r)\) is the phase-0 cost penalty. This stage is intentionally non-aggressive: its purpose is not to make the final generator-selection decision, but to remove candidates with both small gradient signals and large estimated costs before more expensive evaluations are performed. Records are passed to Phase 1 when their lane-wise score exceeds the threshold \(\tau_{0,\ell}\). To avoid discarding all useful candidates in a low-signal iteration, a fallback rule also retains the top-ranked records in each lane.
...
$B\subseteq\mathcal R_3$ is the set of eligible batch candidates.  An optional conservative batch shortlist restricts \(B\) to records with disjoint qubit support and mutual commutation.
...
\end{equation}
with \(s_j^\star=\infty\) if no rung passes. A finite-\(s_j^\star\) candidate is tested on a deletion branch and refitted from \(\delta\theta_{s_j^\star}^\star\). The measured nonlinear deletion loss is
\begin{equation}
\Delta E(d_j)
:=
...
\end{equation}
where the first energy is evaluated after branch reoptimization. Let \(\chi(d_j)\in\{0,1\}\) denote the history/maturity eligibility indicator. The ablation-admissible set in Eq.~(\ref{eq:snake_ansatz_update}) is
\begin{equation}
\begin{aligned}
\mathcal D
=
...
\chi(d_j)=1,\;
s_j^\star<\infty,\;
\Delta E(d_j)\le\epsilon_{\rm del}\}.
\end{aligned}
...
\colrule
SNAKE & 10 & 3.968e-04 & -- & 34 & 28 & 148 & 6,531 \\
...
Method & \metrichead{$k_{\rm pl}$} & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{\twq}$} & \metrichead{$D_{\twq}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
SNAKE & 9 & 1.896e-04 & -- & 30 & 24 & 129 & 5,020 \\
...
Method & \metrichead{$k_{\rm pl}$} & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{\twq}$} & \metrichead{$D_{\twq}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
SNAKE & 17 & 1.393e-06 & -- & 62 & 53 & 272 & 9,976 \\
...
Method & \metrichead{$k_{\rm pl}$} & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{\twq}$} & \metrichead{$D_{\twq}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
SNAKE & 13 & 1.798e-02 & -- & 66 & 58 & 226 & 11,450 \\
...
Method & \metrichead{$k_{\rm pl}$} & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{\twq}$} & \metrichead{$D_{\twq}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
SNAKE & 30 & 3.056e-04 & 8.554e-05 & 150 & 122 & 602 & 189,465 \\
...
Method & \metrichead{$k_{\rm pl}$} & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{\twq}$} & \metrichead{$D_{\twq}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
SNAKE & 10 & 4.131e-05 & -- & 34 & 26 & 170 & 5,623 \\
...
Append-only ADAPT, Geo-ADAPT, and SNAKE use the same Hamiltonian-local
\(\Gset\) for a given benchmark.
...
\end{ruledtabular}
\end{center}
Append-only ADAPT, Geo-ADAPT, and SNAKE use the Hamiltonian-local operator pool
\(\Gset\) when that pool is available for the displayed benchmark. Append-only
ADAPT admits one appended generator at a time, and Geo-ADAPT uses Fubini--Study
metric scoring with a
...
is the optimization rule used for the displayed rows. The shared batched SNAKE
rows promoted on 2026-06-10 use the joint batch-2 cost-search trial-3 settings
recorded in \path{output/pdf/paper_i_joint_batch2_cost_search_promotion_20260610.json}.
Across the Hubbard and spin--boson/Rabi weak--strong rows, this setting keeps
batch admission active, disables target-hit stopping as a termination rule,
uses the compiled IBM Marrakesh resource target, and records zero admitted-record
leakage penalty. In the displayed SNAKE rows for which window provenance is available, the refit/geometry caps are at least the active ansatz size, so those windows are effectively full. The following settings ledger records the row-varying Hubbard--Holstein controls used by the current Schur warm-start SNAKE source rows.

\begin{table*}[t]
\noindent\textbf{Hubbard--Holstein SNAKE settings.}
Row-varying Schur warm-start SNAKE settings for the current six-regime SNAKE rows. \(P_1\) is the Phase-I shortlist size; \(P_2\) reports shortlist fraction/size; \(d_{\max}/d_{\rm drop}\) reports the accepted-iteration cap and drop-minimum depth; \(w_{\rm pos}/k_{\rm pos}\) reports the insertion-position window and top-\(k\) cap; \(f_{\rm pr}/p_{\rm pr}\) reports prune fraction/protect steps; the batch tuple is near-degeneracy ratio, rank relative tolerance, and additivity tolerance. The value \(999999\) denotes an effectively full window in the source record. All displayed rows use append--prune Schur warm starts.
\par\smallskip
\scriptsize
...
Regime & \(P_1\) & \(P_2\) & \(d_{\max}/d_{\rm drop}\) & \(w_{\rm pos}/k_{\rm pos}\) & \(\rho\) & \(w_{\rm shot}\) & \(f_{\rm pr}/p_{\rm pr}\) & batch tuple & \(n_{\rm SPSA}^{\parallel}\)\\
\colrule
weak--weak & 24 & \(0.25/12\) & \(30/0\) & \(4/4\) & 0.25 & 0.08 & \(0.410191/2\) & \((0.998241,1.366{\times}10^{-4},0.666313)\) & 2\\
intermediate--weak & 24 & \(0.25/12\) & \(30/0\) & \(4/4\) & 0.25 & 0.04 & \(0.193096/2\) & \((0.914354,7.703{\times}10^{-7},0.0102765)\) & 2\\
strong--weak & 24 & \(0.25/12\) & \(30/0\) & \(999999/999999\) & 0.50 & 0.15 & \(0.410191/2\) & \((0.998241,1.366{\times}10^{-4},0.666313)\) & 2\\
weak--strong & 24 & \(0.25/12\) & \(30/0\) & \(4/4\) & 0.25 & 0.02 & \(0.339229/2\) & \((0.98,1.910{\times}10^{-5},0.0999312)\) & 4\\
intermediate--strong & 24 & \(0.25/12\) & \(30/0\) & \(4/4\) & 0.25 & 0.02 & \(0.339229/2\) & \((0.98,1.910{\times}10^{-5},0.0999312)\) & 4\\
strong--strong & 24 & \(0.25/12\) & \(30/0\) & \(16/16\) & 0.50 & 0.08 & \(0.193096/2\) & \((0.914354,7.703{\times}10^{-7},0.0102765)\) & 8\\
\end{tabular}
\end{ruledtabular}}
\end{table*}

The common Hubbard--Holstein SNAKE weights not repeated in the ledger are Phase-I \((\lambda_F,\lambda_{\rm comp},\lambda_{\rm meas},\lambda_{\rm leak})=(1,0.05,0.02,0)\), Phase-I compile weights \((1,0.5,1,1,1)\) for two-qubit, single-qubit, rotation-step, position-shift, and active-refit proxies, and Phase-I measurement weights \((1,1,1)\) for shots, Pauli groups, and reuse. Phase-II uses \(\lambda_H=10^{-6}\), \(\gamma_N=1\), \(\eta_L=0\), compatibility weights \((w_{\rm comm},w_{\rm curv},w_{\rm meas},w_{\rm overlap},w_{\rm sched})=(0.2,0.2,0.2,0.4,0.2)\), and resource weights \((w_{\rm depth},w_{\rm group},w_{\rm reuse},w_{\rm optdim},w_{\rm lifetime})=(0.2,0.15,0.1,0.1,0.05)\), with \(w_{\rm shot}\) row-varying as listed. Batching is enabled in all six rows with target size \(8\) and cap \(16\). Generator ablation uses policy \(\texttt{recoverability\_ladder\_v1}\), mode \(\texttt{both}\), prune candidate min/max \(1/6\), local recoverability window \(4\), maturity threshold \(0.5\), maximum Schur-regression tolerance \(10^{-8}\), retained-gain ratio \(0.5\), and small-amplitude thresholds \(10^{-3}\) absolute and \(0.5\) relative.

The result tables use a common metric ledger: \(|\Delta E|\) is the absolute
energy error against the stated ED reference, \(1-F\) is the post-hoc
infidelity when reconstructible, \(N_{2q}\) is the compiled two-qubit count,
\(D_{2q}\) is the compiled two-qubit depth, \(D_c\) is the total compiled
circuit depth, and \(S\) is the normalized estimator/probe-work count for the
displayed ansatz prefix.
...
displayed diagnostic evaluates \(E_{\rm ED}^{(r)}(M)\) for \(M=1,\ldots,10\)
and plots \(S_r(M)\) for \(M=2,\ldots,9\).
...
\captionof{figure}{Hubbard--Holstein exact-diagonalization phonon-cutoff diagnostic for the four two-site regimes.  The upper panel shows the adjacent cutoff drift \(\delta_r(M)\); the lower panel shows the log-change sensitivity \(S_r(M)\).  Markers indicate the first and last plotted cutoff for each curve.}
\label{fig:hh_ed_cutoff_log_sensitivity}
\end{minipage}
\par\addvspace{1.0ex}

\subsection{Enlarged-continuation diagnostics}
\label{app:hh_enlarged_continuation_diagnostics}


To separate operator-pool expressivity from selection and inner-optimization effects, we also ran enlarged same-pool Hubbard--Holstein continuations with Powell inner optimization. Across the four Hubbard--Holstein regimes, these diagnostics reach \(10^{-7}\)-level energy error or better. The capped-budget rows in the main Hubbard--Holstein comparison should therefore be interpreted as budgeted ansatz-growth and optimizer-limited behavior, not as evidence that the Hamiltonian-local operator pool lacks expressive directions. These diagnostics are not used as replacement capped-budget rows because they change the continuation depth and inner-optimization protocol relative to the displayed comparisons.

% BEGIN_MACHINE_READABLE_HH_SECTOR_ERROR_DECOMPOSITION_20260614
% {"schema":"paper_i_hh_sector_error_decomposition_pointer_v1","updated_date":"2026-06-14","support_json":"output/pdf/paper_i_hh_sector_error_snake_geo_comparison_20260614.json","support_json_sha256":"6d196972aaad833560f9e8cef14cf354c45670d0818d3a82d1dba54f1999ef57","support_pdf":"output/pdf/paper_i_hh_sector_error_snake_geo_comparison_20260614.pdf","methods":["SNAKE","Geo-ADAPT"],"sectors":["Hubbard electronic","phonon","electron--phonon"],"absolute_sector_error":"|Delta E_x|","relative_sector_error":"|Delta E_x|/|E_ED|","reader_facing_role":"Appendix C qualitative interpretation only; no visible table insertion in this pass"}
% END_MACHINE_READABLE_HH_SECTOR_ERROR_DECOMPOSITION_20260614
...
\end{widetext}
\fi
\section{Additional Hamiltonian-family checks}
\label{app:additional_hamiltonian_checks}

A separate coverage pass checked that the same implementation extends beyond the three primary benchmark families.  The additional families retained for this appendix-level summary are extended Hubbard, ionic Hubbard, \(t\)-\(t'\) Hubbard, spinless \(t\)-\(V\), Bose--Hubbard, and molecular restricted-closed-shell instances; harmonic/Kerr-chain stress rows are not included here.

\subsection{Auxiliary comparator baselines}
\label{app:auxiliary_comparator_baselines}

These rows are not part of the core Append--Geo--SNAKE comparison.  Fixed-structure VQE denotes HEA VQE and family-informed VQE rows optimized at fixed circuit structure.  TETRIS-ADAPT batches mutually compatible high-gradient qubit generators in one adaptive step \cite{Anastasiou2024}.  Qubit/QEB-ADAPT uses a qubit-excitation pool \cite{Tang2021,Yordanov2021}: with \(X_q,Y_q\) Pauli matrices on qubit \(q\), \(\sigma_q^\pm=(X_q\mp\ii Y_q)/2\), and \(T_{abij}=\sigma_a^+\sigma_b^+\sigma_j^-\sigma_i^-\), the QEB generators are \(G_{ij}^{\rm qeb}=(X_iY_j-Y_iX_j)/2\) and \(G_{ij\to ab}^{\rm qeb}=\ii(T_{abij}-T_{abij}^{\dagger})\). Fixed-structure VQE rows use the fixed-ansatz optimizer/evaluation work recorded for the compiled ansatz.

\noindent\begin{minipage}{\columnwidth}
\captionof{table}{Auxiliary spin-boson/Rabi comparator rows, weak regime.  These rows are separated from the core Append--Geo--SNAKE comparison.}
\begin{center}
\scriptsize
\setlength{\tabcolsep}{1.1pt}
\renewcommand{\arraystretch}{1.02}
...
Method & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{2q}$} & \metrichead{$D_{2q}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
HEA VQE & 3.05e-8 & -- & 6 & 5 & 35 & 48000\\
family VQE & 1.67e-3 & -- & 31 & 29 & 245 & 48000\\
TETRIS-ADAPT & 6.16e-7 & -- & 119 & 107 & 750 & 173390\\
Qubit/QEB & 1.67e-3 & -- & 971 & 971 & 4477 & 300000\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
...
\noindent\begin{minipage}{\columnwidth}
\captionof{table}{Auxiliary spin-boson/Rabi comparator rows, strong regime.  These rows are separated from the core Append--Geo--SNAKE comparison.}
\begin{center}
\scriptsize
\setlength{\tabcolsep}{1.1pt}
\renewcommand{\arraystretch}{1.02}
...
Method & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{2q}$} & \metrichead{$D_{2q}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
HEA VQE & 4.87e-3 & -- & 10 & 7 & 37 & 48000\\
family VQE & 6.70e-3 & -- & 101 & 78 & 609 & 48000\\
TETRIS-ADAPT & 9.97e-6 & -- & 711 & 623 & 3544 & 5823\\
Qubit/QEB & 6.70e-3 & -- & 1017 & 1017 & 4753 & 301000\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
