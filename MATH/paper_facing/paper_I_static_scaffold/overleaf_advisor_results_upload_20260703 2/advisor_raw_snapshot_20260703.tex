...
We benchmark SNAKE against Qiskit-implemented append-only ADAPT-VQE and Geo-ADAPT on equally physically parameterized two-site lattice Hamiltonian models. Namely, we include results of Hubbard and Hubbard--Holstein weak through strong coupling regimes, and include the spin-boson/Rabi model in App.~\ref{app:spin_boson_rabi}. \cite{JavadiAbhari2024Qiskit,QiskitAlgorithmsAdaptVQE}. The Hubbard and Hubbard--Holstein lattice rows use \(L=2\), half filling, and open boundary conditions.

The reported ADAPT comparisons receive the same fermion-particle-number-preserving Hamiltonian-local operator pool defined in Appendix~\ref{app:pools}, along with equal inner-optimization budgets using the SPSA protocol defined in Appendix~\ref{app:results_discussion}; SPSA parameters were determined by using a method-invariant Optuna objective, with 20 trials for each, run in parallel over the six displayed Hubbard--Holstein regimes.

We report energy error as the absolute difference of the ADAPT-resolved lowest energy minus exact-diagonalized lowest energy \(|\Delta E|=|E_{\rm ADAPT}(M)-E_{\rm ED}|(M)\), using equal phonon mode truncations. For the Holstein weak correlation sector, we use phonon mode truncation $M=2$, whereas for the strong Holstein sector, we use $M=4$.
...
The plotted method marker, \(k_{\rm pl}\) marks the plateau iteration point at which we report error, fidelity, and resource cells of the ansatz. The effect of generator ablation causes SNAKE gate-counts to occasionally decrease after plateau point $k_{\rm{pl}}$, in which case we additionally report final circuit costs at depth 30. All methods are ran to 30 ADAPT outer-loop iterations.
...
For the Hubbard--Holstein comparison below, all displayed adaptive rows use the native SPSA optimizer path with an inner-optimizer budget of 200 iterations. Each row is evaluated at the first plateau prefix \(k_{\rm pl}\); circuit resource cells are Qiskit-compiled active-ansatz costs at the same prefix, and the displayed error is the same-cutoff quantity \(|E_{\rm ADAPT}(M)-E_{\rm ED}(M)|\). The weak-Holstein sectors use \(M=2\), and the strong-Holstein sectors use \(M=4\). Append and Geo retain the native200 evidence, while the SNAKE rows use the Schur warm-start SNAKE plateau evidence. Schur plateau-prefix \(1-F\) was not stored in the source artifacts, and SNAKE \(S\) is not displayed in this comparison.
