...
For CFQM methods, the key mathematical point is that the stage times are
fixed scheme nodes \(c_j\), so a macro-step from \(t_n\) to
\(t_{n+1}=t_n+\Delta t\) samples the Hamiltonian at \[
t_n+c_j\Delta t,
...
\hypertarget{a.5-cfqm-macro-step-mathematics}{%
\subsection{A.5 CFQM macro-step
mathematics}\label{a.5-cfqm-macro-step-mathematics}}
...
\] with method-specific coefficients \(a_r\) and nodes \(c_r\). The
formal order is achieved when the stage exponentials are themselves
treated at the intended accuracy.

\hypertarget{a.6-filtered-exact-manifold-and-subspace-fidelity}{%
\subsection{A.6 Filtered exact manifold and subspace
fidelity}\label{a.6-filtered-exact-manifold-and-subspace-fidelity}}
