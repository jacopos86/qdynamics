...
The results section follows a compact accuracy/resource order: mixed Hubbard--Holstein static spectral table over the four-regime Cartesian grid, QSE-selection ablation table, dynamics table, QSE-to-dynamics handoff ablation table, and then figures.  The static results focus on excitation spectra.  The dynamics results compare frozen QSE, adaptive QSE, and QSE-triggered live McLachlan refinement.

\subsection{Static excited-state results}

\structlabel{Static excited-state results}
Table~\ref{tab:qse_static_claims} gives the static mixed Hubbard--Holstein comparison over the weak--weak, strong--weak, weak--strong, and strong--strong regimes.  Each regime compares same-cutoff exact diagonalization, regular mixed Fullmeta QSE, and geometry-selected mixed Fullmeta QSE.  The same-cutoff columns test whether the QSE basis recovers the ED spectrum inside the working Hilbert space; the higher-cutoff columns test whether lower-cutoff QSE spectra remain physically stable after embedding.  Resource columns report effective basis size, overlap conditioning, final matrix-measurement proxy, and optional compiled two-qubit cost.
...
Table~\ref{tab:excited_dyn_claims} compares frozen QSE propagation, adaptive-QSE propagation, and QSE-triggered live McLachlan refinement against projected-variational and product-formula baselines.  Within each benchmark point all methods start from the same optimized ground-state ansatz and the same selected excitation root, use the same drive, and are evaluated on the same time grid.
...
\bibitem{UtamaDipojono2026ACASE}
G. Utama and H. K. Dipojono,
``Adaptive operator-generated subspaces for effective many-body Hamiltonians,''
arXiv:2608.00560 (2026), preprint.

\bibitem{UtamaDipojono2026DACASE}
G. Utama and H. K. Dipojono,
``DA-CASE: reusable measurements for adaptive quantum subspaces,''
arXiv:2608.08739 (2026), preprint.
