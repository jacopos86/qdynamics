...
As such, we define a geometry- and cost-aware excitation-record acquisition rule for QSE.  An optimized ground-state ADAPT ansatz supplies the seed state, with ADAPT and recent resource- or geometry-aware descendants providing the adaptive-ansatz lineage for that seed construction \cite{Grimsley2019ADAPT,Tang2021QubitADAPT,Anastasiou2024TETRIS,Ramoa2025CEO,Sohail2026GeoADAPT}.  Candidate excitation records are ranked by retained-frame metric novelty and eigen-residual capture under one normalized compiled-hardware cost discount.  Probe-transition visibility and explicit two-level Ritz gain are evaluated only as ablations and do not enter the production score.  The cited QSE literature motivates the ingredients---operator-generated response manifolds, regularized nonorthogonal metrics, and measurement-aware matrix construction---while the single-pass admission rule is introduced here.  The admitted records define a nonorthogonal excitation basis, whose overlap and Hamiltonian matrices are regularized and diagonalized to obtain approximate excited energies and transition matrix elements.
...
Before defining the ingredients, we separate the local geometric surrogate from the ranked score.  For a candidate excitation record $r$ at growth round $\ell$, define
\begin{align}
G_{\rm geo}(r;\ell)
&=
w_{\Ncal}\,\Ncal^{\qse}(r;\ell)
+w_{R}\,C^{\rm res}(r;\ell),
\label{eq:qse_geometric_surrogate}\\
\Scal(r;\ell)
&=
\frac{G_{\rm geo}(r;\ell)}
{\left[\max\!\left(\widehat K(r;\ell),K_{\rm floor}\right)\right]^{\alpha}}.
\label{eq:qse_score_topdown}
\end{align}
The two geometric ingredients are
\begin{itemize}
\item $\Ncal^{\qse}$, \emph{metric novelty}: the Schur-complement residual fraction of the record's image after projection onto the retained support, so a direction already expressible by the current basis scores zero;
\item $C^{\rm res}$, \emph{residual capture}: the fraction of the targeted roots' eigen-equation residual that the record's direction absorbs.
\end{itemize}
Here $C^{\rm res}$ denotes capture, not cost; cost is denoted by $K$ throughout.  The local surrogate $G_{\rm geo}$ enters additively, so a record strong in one geometric coordinate can still be admitted when the other is weak.  The veto behavior that a product would impose is instead supplied by the metric-novelty floor, below which a candidate is rejected outright as unrepresentable.  The denominator of Eq.~(\ref{eq:qse_score_topdown}) applies the hardware discount exactly once; its cost model is deferred to Sec.~\ref{sec:cost_weighting}.  Section~\ref{sec:ablations} shows that removing metric novelty, residual capture, or that single cost discount prevents convergence in most regimes.  Two conventions are load bearing throughout.  First, novelty and gain are always evaluated against the \emph{numerically retained} support $P_{\Acal,\tau}$ of Eq.~(\ref{eq:qse_supported_projector_objective}), never the exact span of accepted images: acquisition and diagonalization must share one stabilization cutoff $\tau$, or the selector will score as redundant a direction that the stabilized pencil has discarded and cannot represent.  Second, every score is discounted by the incremental compiled-hardware cost of the record, following the acquisition principle of the companion papers \cite{PaperI,PaperII}.

The distinction between utility and score is therefore explicit.  The support-level utility $U(\Acal)$ below is the ideal global objective; $G_{\rm geo}(r;\ell)$ is the cheap local geometric surrogate for a candidate's marginal residual closure; and $\Scal(r;\ell)$ is the final cost-discounted ranking score used by the selector.  Section~\ref{sec:acquisition_objective} defines the ideal utility and the two implemented geometric quantities; Sec.~\ref{sec:cost_weighting} defines the cost coordinate; Sec.~\ref{sec:stopping_rule} gives the stopping rule that makes support size an output rather than a budget; and Sec.~\ref{sec:exchange_maintenance} gives the certified exchange move that maintains an already-selected support.

\subsection{Acquisition rule}
\label{sec:acquisition_objective}

\structlabel{Acquisition rule}
The retained set $\Acal$ is grown greedily by a single-pass rule: at each round every remaining admissible candidate is scored, the best is admitted, and the round repeats.  There is no staged shortlist---the two geometric quantities of Eq.~(\ref{eq:qse_score_topdown}) are evaluated for every candidate in every round, and both are obtained from the same objects the pencil already needs.  We state the objective the rule approximates, then the two quantities as implemented.
...
\end{equation}
The first term rewards closure of the spectral-window eigen-equation residual and the second penalizes total support cost.  This global utility is not evaluated by the operating selector.  Direct greedy maximization of Eq.~(\ref{eq:qse_acquisition_utility}) would require the very matrix elements the selector is meant to avoid, so each candidate is ranked instead by the local score in Eq.~(\ref{eq:qse_score_topdown}), computed against the current \emph{retained} frame.

\structlabel{Metric novelty}
Let $\Acal_\ell$ be the records admitted so far, with overlap matrix and cross-overlap
\begin{equation}
...
\end{equation}
Equivalently $S_{ab}=\bra{\psi_0}\Omega_a^\dagger Q_{\mathfrak s,0}\Omega_b\ket{\psi_0}$, so $S_{\Acal_\ell}$ is the pullback of the sector-projective state metric onto the operator-generated manifold.  Writing $P_{\ell,\tau}$ for the projector onto the numerically retained principal directions of $S_{\Acal_\ell}$ under the solver's cutoff $\tau$, the residualized candidate image and its normalized direction are
\begin{equation}
...
\end{equation}
and the metric novelty is the Schur-complement residual fraction
\begin{equation}
\Ncal^{\qse}(r;\ell)=\frac{F_r^\perp}{\braket{\phi_r}{\phi_r}+\eps_F}\in[0,1].
\label{eq:qse_phase2_novelty}
\end{equation}
The projection in Eq.~(\ref{eq:qse_reduced_candidate_direction}) is applied twice (reorthogonalized Gram--Schmidt): a single pass lets numerical noise from near-dependent accepted directions masquerade as zero novelty and permanently exclude records that still carry rank.  Candidates with $\Ncal^{\qse}$ below a hard floor are rejected outright as unrepresentable rather than merely ranked low.

\structlabel{Residual capture}
Let $M_{\Acal_\ell}=(\bra{\phi_a}H_0\ket{\phi_b})_{a,b\in\Acal_\ell}$ and let $c^{(\nu)}$ solve the projected problem
\begin{equation}
M_{\Acal_\ell}c^{(\nu)}=\varepsilon_\nu S_{\Acal_\ell}c^{(\nu)},
\qquad
...
\end{equation}
with current root $\ket{\Psi_\nu^{(\ell)}}=\sum_{a\in\Acal_\ell}c_a^{(\nu)}\ket{\phi_a}$.  The eigen-equation residual on the same retained support is
\begin{equation}
\ket{R_\nu^{(\ell)}}=(I-P_{\ell,\tau})(H_0-\varepsilon_\nu)\ket{\Psi_\nu^{(\ell)}},
\label{eq:qse_eigen_residual}
\end{equation}
and residual capture is the fraction of that residual the candidate direction absorbs, maximized over the targeted roots:
\begin{equation}
C^{\rm res}(r;\ell)
=
\max_{\nu\le R}
...
\end{equation}
Because $\ket{u_r}$ is already orthogonal to the retained support, $\braket{u_r}{R_\nu^{(\ell)}}=\bra{u_r}(H_0-\varepsilon_\nu)\ket{\Psi_\nu^{(\ell)}}$, so residual capture needs only one additional application of $H_0$ per candidate.

\structlabel{Scope of this rule}
Two quantities that a staged screen would use are deliberately absent from the production rule: a probe-transition visibility term and an explicit two-level Ritz gain.  Both were implemented, evaluated, and removed because they did not improve selection (Sec.~\ref{sec:ablations}).  The rule therefore evaluates two geometric quantities per candidate per round and nothing else.
...
Equation~(\ref{eq:qse_score_topdown}) already discounts the geometric surrogate by incremental compiled-hardware cost exactly once, following the acquisition principle established for ansatz construction \cite{PaperI} and for structural patches in checkpoint dynamics \cite{PaperII}.  Write $K(r;\ell)$ for the raw incremental cost, let $\mathcal R_{\rm elig}$ be the eligible record alphabet, and define
\begin{equation}
K_{\max}=\max\!\left\{1,\max_{q\in\mathcal R_{\rm elig}}K(q;\ell)\right\},
\qquad
\widehat K(r;\ell)=\frac{K(r;\ell)}{K_{\max}}.
\label{eq:qse_cost_normalization}
\end{equation}
Thus $\widehat K\in[0,1]$ and Eq.~(\ref{eq:qse_score_topdown}) is the final ranked quantity; no second cost penalty is applied.  The bounded floor $K_{\rm floor}$ is not cosmetic: without it, records whose compiled cost rounds to zero acquire unbounded score and crowd out spectrally useful directions.  We use $\alpha=1$ and $K_{\rm floor}=0.05$ throughout; $\alpha=2$ is too greedy at small support in the strong-coupling regimes.
...
K_1(r;\ell)
&=
w_S\widehat K_S(r)+w_M\widehat K_M(r)
+w_{\obs}\widehat K_{\obs}(r)\\
&\quad
+w_\kappa\widehat K_\kappa(r)
+w_{\twq}\widehat K_{\twq}(r;\ell).
\end{aligned}
...
\end{equation}
where \(\mathcal G_{\rm meas}\) is the grouped-observable set used for all required overlap, Hamiltonian, transition, and probe matrix elements, and \(N_g\) is the allocated shot count for group \(g\).  Tables report \(M_{\rm eff}=\rank(S_\tau)\), \(\kappa(S_\tau)\), \(N_{\rm group}\), and \(S_{\rm mat}^{\rm proxy}\) whenever the corresponding artifacts are available.
The weights are nonnegative controller weights, and the hatted quantities are dimensionless normalized resource or risk predictors.  The term $\widehat K_S(r)$ estimates the additional grouped measurements needed for overlap entries involving $\phi_r$; $\widehat K_M(r)$ estimates the additional grouped measurements needed for projected Hamiltonian entries; $\widehat K_{\obs}(r)$ estimates additional observable or transition-matrix groups for the probe set; $\widehat K_\kappa(r)$ estimates numerical conditioning risk from adding a nearly dependent basis direction; and $\widehat K_{\twq}(r;\ell)$ estimates the incremental compiled two-qubit or controlled-preparation burden of measuring record $r$ at growth step $\ell$.  These are acquisition costs, not evidence that the candidate is physically unimportant.  If $\Acal_\ell$ denotes the current admitted record set, then a cheap Hamiltonian-matrix measurement proxy is
\begin{equation}
\widehat K_M(r)\propto
\sum_{a\in\Acal_\ell}
...
\end{equation}
with the analogous expression for $\widehat K_S$ obtained by replacing $H_0$ with $I$, and the analogous expression for $\widehat K_{\obs}$ obtained by replacing $H_0$ with the measured probe or observable $O$.
...
Greedy acquisition is monotone: a record admitted early on the evidence then available is never reconsidered, even after later records change what the support expresses.  We therefore maintain the selected support with the generalized support-patch algebra of Paper II \cite{PaperII}, specialized here to the QSE pencil.  At maintenance round $m$, let
\begin{equation}
\mathcal B_m=(B_m^{\ominus},B_m^{\oplus}),
\qquad
\Acal_m^{\mathcal B}
=
(\Acal_m\setminus B_m^{\ominus})\cup B_m^{\oplus},
\label{eq:qse_exchange_patch}
\end{equation}
where $B_m^{\ominus}\subseteq\Acal_m$ and $B_m^{\oplus}\cap\Acal_m=\emptyset$.  As in Paper II, the empty-component pattern classifies the action: two empty components give stay, $B_m^{\ominus}=\emptyset$ gives pure insertion, $B_m^{\oplus}=\emptyset$ gives pure deletion, and two nonempty components give true exchange.  These are faces of one patch family, not sequential actions; failure of a true exchange does not authorize either one-sided face.

Each patch is scored on a full re-solve of the patched pencil, not on an additive delete-plus-insert surrogate.  For $R$ targeted roots let $\varepsilon_1^{(\Acal)}\le\cdots\le\varepsilon_R^{(\Acal)}$ be the lowest retained Ritz values of the support and define the Ky Fan objective
\begin{equation}
\Fcal_R(\Acal)=\sum_{\nu=1}^{R}\varepsilon_\nu^{(\Acal)},
\label{eq:qse_kyfan}
\end{equation}
a variational upper bound on the sum of the true lowest $R$ sector excitations, so that decreasing $\Fcal_R$ improves the represented window as a whole rather than any single root.  Writing $K(\Acal)$ for the support's total compiled cost, the patch is certified only if it dominates the incumbent,
\begin{equation}
\Fcal_R(\Acal_m^{\mathcal B})\le\Fcal_R(\Acal_m)+\eta_{\rm acc}
\quad\text{and}\quad
K(\Acal_m^{\mathcal B})\le K(\Acal_m),
...
\begin{equation}
\begin{aligned}
\kappa\!\left(S_{\Acal_m^{\mathcal B},\tau}\right)
&\le
\gamma_\kappa\,
\kappa\!\left(S_{\Acal_m,\tau}\right),
\\
\kappa\!\left(S_{\Acal_m^{\mathcal B},\tau}\right)
&\le\kappa_{\max}
\quad\text{when an absolute cap is enabled},
\\
\operatorname{rank}_\tau S_{\Acal_m^{\mathcal B}}
&\ge
\left\lceil
\rho_{\rm rank}\operatorname{rank}_\tau S_{\Acal_m}
\right\rceil .
\end{aligned}
\label{eq:qse_exchange_guards}
\end{equation}
The reported traces use $\gamma_\kappa=10$, $\rho_{\rm rank}=1$, and no absolute condition cap, so retained rank cannot decrease and conditioning may worsen by at most one decade.
Let $\Gamma_{\rm ex}^{\qse}(\mathcal B_m\mid\Acal_m)=1$ exactly when Eqs.~(\ref{eq:qse_exchange_dominance}) and~(\ref{eq:qse_exchange_guards}), including the strict-margin condition, all hold.  The shared atomic event map is then
\begin{equation}
\Acal_{m+1}
=
\begin{cases}
\Acal_m^{\mathcal B},
& \Gamma_{\rm ex}^{\qse}(\mathcal B_m\mid\Acal_m)=1,\\
\Acal_m,
& \Gamma_{\rm ex}^{\qse}(\mathcal B_m\mid\Acal_m)=0.
\end{cases}
\label{eq:qse_exchange_event_map}
\end{equation}
Thus commit and rollback are properties of the whole pair $(B_m^{\ominus},B_m^{\oplus})$; neither component is ever applied provisionally.  An optional budgeted variant replaces the cost inequality in Eq.~(\ref{eq:qse_exchange_dominance}) by a declared envelope $K(\Acal_m^{\mathcal B})\le K_{\rm budget}$, which lets the move trade cost for accuracy up to a stated ceiling; budgeted numbers are always labeled with their envelope.

Three properties distinguish this from pruning followed by regrowth.  The deletion and insertion sets are evaluated \emph{jointly}, so a patch may be accepted whose benefit does not decompose into individually favorable single moves---a survivor record can absorb the deleted direction only once the replacement is present.  Acceptance is decided by a full re-solve of the projected pencil on $\Acal_m^{\mathcal B}$, so the objective, conditioning, retained rank, and compiled cost are all evaluated on the support that will actually be measured.  Equation~(\ref{eq:qse_exchange_event_map}) makes atomic rejection explicit: a patch failing any gate leaves $\Acal_m$ untouched.  In practice exchange acts both as a repair for supports where greedy acquisition stalls and as a cost compressor after convergence, and it reduces matrix-measurement work as well as circuit cost.

Only the patch algebra and atomic event map are inherited from Paper II.  Its realized captured-drift objective and ray- and velocity-continuity gates are dynamics-specific and are not transplanted here; Paper III replaces them by the Ky Fan objective and QSE overlap-metric guards above.  The implementation evaluated here restricts $|B_m^{\ominus}|\le 1$ and $|B_m^{\oplus}|\le 1$, so every nondegenerate patch is a one-record-for-one-record exchange.  Pure deletion is enabled as a degenerate patch; pure insertion is implemented but disabled in the reported growth trace.  Multi-record exchanges are not evaluated here.
...
\\
&\times
\mathbf 1[\widehat K_{\rm meas}(B)\le \tau_{\rm meas}^{B}(\ell)]
\\
&\times
...
\end{equation}
Thus spectral regressions are attributed to support truncation, root rematching, or sector changes rather than to exact Galerkin enlargement.
Finite-rank stability is enforced by supported metric whitening, following the quantum-subspace lesson that measured matrix pencils must be solved only on a numerically reliable overlap support \cite{Epperly2022QSD,Lee2024SamplingQKSD,Kwao2026GEPQSE}.  If
...
For an isolated whitened root, the corresponding first-order eigenvector sensitivity follows the usual eigengap dependence of Hermitian subspace perturbation bounds \cite{DavisKahan1970} and obeys
\begin{equation}
\|\delta y_\nu\|_2
\lesssim
\frac{
...
\subsection{Excited-state results}

\structlabel{Comparison method}
All comparisons use one construction.  Every method grows its subspace one direction at a time to a fixed dimension cap and is scored at each step against exact sector-restricted references on the same six-excitation window.  The candidate sets differ by method family, and the difference is load bearing.  Record-based methods---ours and the ordering controls---draw every candidate from an \emph{identical} fixed alphabet: each is the image $O_i\ket{\psi_0}$ of a pool record, so their comparison is of acquisition rules at fixed expressive reach.  The synthesized-basis methods are not so confined: quantum-Davidson growth builds each new direction from the preconditioned residual and real-time Krylov from time evolution, so neither is restricted to the alphabet and both can reach directions no record supplies.  That asymmetry favors them on reachability and costs them on resources, and both effects appear below: where our alphabet spans the window we resolve it an order of magnitude more cheaply, and where it does not they resolve windows we cannot.  The resource required to reach an accuracy target is read off that growth trace,
\begin{equation}
K^\star(\eps_E)=\min\Bigl\{K:\ \max_{\nu\le 6}|\Delta E_\nu|\le\eps_E\Bigr\},
\label{eq:qse_kstar}
\end{equation}
so no method's own stopping rule enters the comparison; our residual stop is reported as a marker on the trace rather than as the mechanism of comparison.  Resources are reported on four axes that are never conflated: the compiled two-qubit gate count $N_{2q}$, the two-qubit depth $D_{2q}$, the total circuit depth $D_c$---all three from a deterministic graph-span proxy, so no transpiler routing randomness enters---and the matrix-measurement estimator queries needed to build the projected pencil.  Alphabet identity is checkable rather than assumed: each run records an ordered digest of the record pool, since equal pool sizes do not prove equal pools.

\structlabel{Methods compared}
Three methods and one baseline are evaluated on identical Hamiltonians, sectors, and references.  \emph{Ours} is the cost-weighted geometric acquisition of Sec.~\ref{sec:acquisition_objective} with certified exchange maintenance.  \emph{Adaptive QSE} is standard quantum-Davidson subspace growth: each new direction is synthesized from the preconditioned residual of a target root rather than selected from the alphabet, and it carries no hardware-cost term.  \emph{Real-time Krylov} builds its subspace from time-evolved states of a seeded sector kick---a different construction family that likewise does not consume the alphabet---under conventions chosen to favor it, including best-matching-root scoring.  The \emph{standard QSE} baseline is the complete linear-response class: the physically obvious operator set, used whole and without adaptation.

\structlabel{Exchange activation}
The certified transaction is active in the completed $n_{\rm ph}^{\max}=1$ diagnostic rather than a dormant method option.  At the terminal prefix of each regime's exchange-maintained growth trace, 11 patches are committed across the six regimes: 10 are genuine one-record-for-one-record exchanges and one is a pure deletion.  Five regimes accept at least one patch; the intermediate--strong regime evaluates one maintenance round and leaves its support unchanged.

\begin{table}[t]
\caption{Activation of certified exchange at the terminal prefixes of the completed $n_{\rm ph}^{\max}=1$ growth trace.  Exchange is applied at each recorded support.  ``Rounds'' includes the final round that finds no admissible patch.  A joint commit has $|B^{\ominus}|=|B^{\oplus}|=1$; pure insertion is disabled in this diagnostic.}
\label{tab:qse_exchange_activation}
\scriptsize
\begin{ruledtabular}
\begin{tabular}{lrrrrr}
Regime & $k$ & rounds & commits & $1\leftrightarrow 1$ & delete only \\
\colrule
weak--weak & 15 & 2 & 1 & 1 & 0 \\
intermediate--weak & 16 & 4 & 3 & 3 & 0 \\
strong--weak ($U/t=8$) & 16 & 4 & 3 & 2 & 1 \\
weak--strong & 15 & 3 & 2 & 2 & 0 \\
intermediate--strong & 16 & 1 & 0 & 0 & 0 \\
strong--strong ($U/t=8$) & 15 & 3 & 2 & 2 & 0 \\
\end{tabular}
\end{ruledtabular}
\end{table}

\structlabel{Excitation gaps}
Figure~\ref{fig:qse_growth_gaps} shows the resolved gaps $\omega_\nu=E_\nu-E_0$ against the exact ladder in every regime.  It is the primary display because it separates the two ways a method fails: losing accuracy on a represented state, and omitting a state altogether.  The standard-QSE baseline does the latter---its class does not span the window, so it misplaces specific excitations and inserts spurious ones at any budget, a deficiency no additional measurement repairs.  The adaptive and Krylov constructions resolve the ladder, so the question they pose is one of cost rather than of reachability.

\begin{figure}[t]
\includegraphics[width=\columnwidth]{generated/paper_iii_growth_gaps_nph1.pdf}
\caption{Excitation gaps $\omega_\nu=E_\nu-E_0$ for the six lowest excitations of each regime at $n_{\rm ph}=1$: exact sector-restricted spectrum (bars) against the three compared methods and the standard-QSE baseline, each at its most accurate recorded subspace.  Values above the plotted window are drawn as carets with their value printed, so none are omitted.}
\label{fig:qse_growth_gaps}
\end{figure}

\begin{figure}[t]
\includegraphics[width=\columnwidth]{generated/paper_iii_growth_gaps_nph3.pdf}
\caption{As Fig.~\ref{fig:qse_growth_gaps}, at the larger phonon cutoff $n_{\rm ph}=3$.}
\label{fig:qse_growth_gaps_nph3}
\end{figure}

\structlabel{Resources at fixed accuracy}
The tables below give $K^\star(\eps_E)$ for each method at three accuracy targets in the four reported axes, at both phonon cutoffs.  Three readings matter.  First, the non-adaptive baseline reaches no target in any regime: its failure is one of representation, not of budget, so no resource ratio against it is meaningful.  Second, among the methods that do resolve the window, cost-weighted selection reaches each target with roughly an order of magnitude fewer compiled two-qubit gates than quantum-Davidson growth and one to two orders fewer than the Krylov construction, with a comparable reduction in estimator queries.  That reduction follows from support size: the projected pencil requires $k(k+1)/2$ elements of both the overlap and the Hamiltonian, so a method certifying the same window from fewer records pays proportionately less to build it.  We claim no reduction in selection-time measurement, because the acquisition rule of Sec.~\ref{sec:acquisition_objective} scores every admissible candidate each round rather than screening a shortlist.  Third, the advantage is bounded by the record alphabet.  At $n_{\rm ph}=3$ the selected manifold does not reach the tightest target in the weak--strong and intermediate--strong regimes, stalling near $10^{-2}$, while quantum-Davidson growth reaches it there.  The reason is structural rather than a matter of budget: our candidates are images of a fixed record alphabet under one reference, whereas the Davidson construction synthesizes each new direction from the residual itself and is therefore not confined to that alphabet.  Where the alphabet spans the window we are an order of magnitude cheaper; where it does not, no acquisition rule over that alphabet can close the gap, and enriching the pool---not reweighting the score---is the remedy.

\input{generated/paper_iii_growth_cost_tables_nph1}

\input{generated/paper_iii_growth_cost_tables_nph3}

\label{sec:ablations}
\structlabel{Score-term ablations}
Table~\ref{tab:qse_ablation_matrix} disables one ingredient of the production score at a time, at the production stop so that support size is an output: a redundant term then shows as equal accuracy at larger $k$, and a load-bearing term as failure to converge at all.  Both terms are load bearing, and in the same way: removing either the metric-novelty weight or the residual-capture term roughly doubles the converged support, multiplies compiled cost several-fold, and in most regimes prevents convergence outright, the run instead exhausting its pool at an error one to two orders of magnitude worse than the anchor.  The metric-novelty result differs from the companion ansatz-construction setting, where an analogous novelty term did not improve selection and was removed \cite{PaperI}.  Both settings invert a metric---the Fubini--Study metric of the variational manifold there, the overlap Gram matrix of the nonorthogonal excitation basis here---so the difference is not the presence of a metric but what happens to a redundant admission.  In ansatz construction the angles are re-optimized after every insertion, so a redundant generator is variationally absorbed and costs only depth.  In subspace expansion there is no such re-optimization: the basis is the answer, its retained rank fixes which spectral directions exist at all, and a direction the stabilized pencil cannot represent consumes measurement budget while contributing nothing to the window.

The metric-novelty \emph{floor} plays a distinct role from its weight.  Removing the floor while keeping the weight still converges, but at a systematically larger support, because candidates whose retained-frame-orthogonal component is numerically zero are admitted, consume budget, and contribute no representable direction.  The weight ranks useful directions; the floor excludes unrepresentable ones.

\input{generated/paper_iii_ablation_table}
...
\input{generated/paper_iii_driven_table}

\input{generated/paper_iii_adaptive_table}
