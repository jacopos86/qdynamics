...
\begin{itemize}
\item $\Ncal_2^{\qse}$, \emph{metric novelty}: the Schur-complement residual fraction of the record's image after projection onto the retained support, so a direction already expressible by the current basis scores zero (Sec.~\ref{sec:phase2});
\item $C^{\rm res}$, \emph{residual capture}: the fraction of the targeted roots' eigen-equation residual that the record's direction absorbs (Sec.~\ref{sec:phase3});
\item $T$, \emph{probe-transition visibility}: the largest overlap of the record's direction with the normalized images of the target observables (Sec.~\ref{sec:phase1}).
\end{itemize}
