...
\Ucal(r;\ell)
=
w_{\Ncal}\,\Ncal_2^{\qse}(r;\ell)
+w_{R}\,C^{\rm res}(r;\ell)
+w_{T}\,T(r;\ell),
\label{eq:qse_score_topdown}
\end{equation}
whose four ingredients are
\begin{itemize}
\item $\Ncal_2^{\qse}$, \emph{metric novelty}: the Schur-complement residual fraction of the record's image after projection onto the retained support, so a direction already expressible by the current basis scores zero;
