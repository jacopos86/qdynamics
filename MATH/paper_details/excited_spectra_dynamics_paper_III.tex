...
Ahead of the locked CHTC production matrix, Figs.~\ref{fig:qse_frontier} and~\ref{fig:qse_gaps} and Tables~\ref{tab:qse_multiroot_diag}--\ref{tab:qse_peierls_diag} report the completed statevector-diagnostic evidence sweep at the Paper-I regime conventions ($n_{\rm ph}=3$ weak-phonon and $n_{\rm ph}=7$ strong-phonon sectors, per-regime full-meta pools, exact sector-restricted references).  Three findings organize these tables.  First, over the lowest six excitations the geometry-selected support with certified exchange reaches (near-)manifold-limit accuracy in every regime, while the complete fixed linear-response class fails several higher roots in five of six regimes and, being complete, cannot be repaired by additional budget.  Figure~\ref{fig:qse_gaps} shows where those failures fall: the fixed class does not merely lose accuracy but misplaces specific states, overshooting the top of the window by a factor of four in the strong-coupling regimes and omitting the lowest excitation altogether at $u=8$, where that state is the sector spin triplet.  Second, Table~\ref{tab:qse_comparator_matrix} compares four method families on every benchmark Hamiltonian over the full six-root window.  Under conventions chosen to favor the baselines, the complete fixed linear-response class and the real-time Krylov envelope fail the window on every case, with maximum errors of $10^{-1}$--$10^{0}$.  The third column is an internal control rather than a competing method: it runs our own acquisition rule on the identical record pool with the hardware-cost discount switched off, so the comparison isolates the contribution of cost weighting alone.  That control is three to five orders of magnitude better than the static and Krylov baselines, confirming that adaptive acquisition over the richer alphabet is what separates this family from them---and it is nonetheless dominated on \emph{both} axes in eight of the nine cases, typically by one to two orders of magnitude in accuracy at roughly half the compiled two-qubit cost, while exhausting its pool without meeting the residual stop in six cases where the cost-weighted rule converges.  The single exception is the weak-coupling Peierls point, where the residual-guided arm reaches $2\times10^{-11}$ against our $2\times10^{-10}$ at $258$ against $188$ gates---a genuine accuracy-for-cost trade, with both arms four orders past the target tolerance.  The interpretation is that discounting by compiled cost changes \emph{which} directions are acquired rather than merely how many: cost-aware acquisition is not a resource concession made at the expense of accuracy but a different, and here better, search.  Third, the Peierls--Hubbard generality check shows the physically ``obvious'' fixed class collapsing for bond coupling---the required spin-structure directions are absent---while the same selection-plus-exchange machinery recovers all six excitations without hand redesign.  Two methodological requirements surfaced by this sweep apply to all production rows: sector references must come from the exact sector-restricted eigenproblem (expectation-based filtering silently drops states that are degenerate across number sectors), and the selection metric must measure candidate novelty against the \emph{numerically retained} subspace of the current support under the solver's overlap cutoff, since the exact span counts directions the pencil's stabilization will discard.  The operative requirement is that acquisition and diagonalization share one stabilization scale: a candidate lying inside the exact span but entirely within a discarded principal mode is redundant to the span and unavailable to the solver, so span-based novelty permanently floors the operators that would re-supply it.  All numbers below are regenerated directly from the committed evidence records by \texttt{pipelines/reporting/build\_paper\_iii\_tex\_fragments.py}.
...
\caption{Excitation gaps $\omega_\nu=E_\nu-E_0$ for the six lowest excitations of each Paper-I regime (ordered left to right within each group): exact sector-restricted spectrum (bars) against the complete fixed linear-response class (squares) and the geometry-selected support with certified exchange (circles).  The vertical range is clipped to the target window; fixed-class roots falling above it are drawn as carets at the top edge with their value printed, so none are omitted.  The selected support lies on the exact ladder in every regime; the fixed class tracks the low roots in the weak-coupling regimes but places its highest represented root far above the window---$8.3$ against $2.0$ at strong--weak and $7.5$ against $2.0$ at strong--strong---and in the two $u=8$ regimes it misses the lowest excitation entirely, its ladder beginning near $\omega=1$ where the exact spectrum begins near $\omega=0.5$.  Being complete, that class cannot be repaired by additional budget.  The maximum-error column of Table~\ref{tab:qse_multiroot_diag} is the scalar summary of these departures.}
\label{fig:qse_gaps}
\end{figure}

\input{generated/paper_iii_multiroot_table}

\begin{widetext}
\input{generated/paper_iii_comparator_table}
\end{widetext}

\input{generated/paper_iii_comparator_matrix_table}

\begin{widetext}
\input{generated/paper_iii_peierls_table}
\end{widetext}

\label{sec:ablations}
\structlabel{Score-term ablations}
Table~\ref{tab:qse_ablation_matrix} disables one ingredient of the production score at a time, at the production stop so that support size is an output: a redundant term then shows as equal accuracy at larger $k$, and a load-bearing term as failure to converge at all.  All three terms are load bearing, and in the same way.  Removing the metric-novelty weight, the residual-capture term, or the cost discount roughly doubles the converged support, multiplies compiled cost by three to eight, and in most regimes prevents convergence outright, the run instead exhausting its pool at an error one to two orders of magnitude worse than the anchor.  The metric-novelty result is worth stating explicitly because it differs from the companion ansatz-construction setting, where an analogous novelty term did not improve selection and was removed \cite{PaperI}.  Both settings invert a metric---the Fubini--Study metric of the variational manifold there, the overlap Gram matrix of the nonorthogonal excitation basis here---so the difference is not the presence of a metric but what happens to a redundant admission.  In ansatz construction the angles are re-optimized after every insertion, so a redundant generator is variationally absorbed and costs only depth.  In subspace expansion there is no such re-optimization: the basis is the answer, its retained rank fixes which spectral directions exist at all, and a direction the stabilized pencil cannot represent consumes measurement budget while contributing nothing to the window.

The metric-novelty \emph{floor} plays a distinct role from its weight.  Removing the floor while keeping the weight still converges, but at a systematically larger support---$92$ against $71$ records at weak--weak and $138$ against $84$ at strong--strong---because candidates whose retained-frame-orthogonal component is numerically zero are admitted, consume budget, and contribute no representable direction.  The weight ranks useful directions; the floor excludes unrepresentable ones.  Finally, the two strong-phonon regimes at $n_{\rm ph}=7$ and weak or intermediate $u$ are pool limited rather than score limited: there every arm exhausts the pool within ten percent of the same error, and the honest reading of those columns is that no acquisition rule repairs a manifold the record pool does not span.
