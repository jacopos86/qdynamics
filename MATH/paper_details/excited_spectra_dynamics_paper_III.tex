...
\left[\max\!\left(\widehat C(r;\ell),C_{\rm floor}\right)\right]^{\alpha}
},
\label{eq:qse_score_topdown}
\end{equation}
whose two geometric ingredients are
\begin{itemize}
...
\item $T$, \emph{probe-transition visibility}: the largest overlap of the record's direction with the normalized images of the target observables .
\end{itemize}
The numerator is the geometric part of the score and the denominator the hardware discount, whose cost model is deferred to Sec.~\ref{sec:cost_weighting}.  The ingredients enter additively rather than as a product, so a record strong in one coordinate can still be admitted when another is weak; the veto behavior that a product would impose is instead supplied by the metric-novelty floor, below which a candidate is rejected outright as unrepresentable.  Each term is load bearing: Sec.~\ref{sec:ablations} shows that removing any one of metric novelty, residual capture, or the cost discount prevents convergence in most regimes.  Two conventions are load bearing throughout.  First, novelty and gain are always evaluated against the \emph{numerically retained} support $P_{\Acal,\tau}$ of Eq.~(\ref{eq:qse_supported_projector_objective}), never the exact span of accepted images: acquisition and diagonalization must share one stabilization cutoff $\tau$, or the selector will score as redundant a direction that the stabilized pencil has discarded and cannot represent.  Second, every score is discounted by the incremental compiled-hardware cost of the record, following the acquisition principle of the companion papers \cite{PaperI,PaperII}; the cost term is deferred to Sec.~\ref{sec:cost_weighting} so that the geometry is defined independently of the hardware model.

The remainder of this section makes each factor precise.  Section~\ref{sec:acquisition_objective} states the spectral-window utility that Eq.~(\ref{eq:qse_score_topdown}) approximates and defines both quantities; Sec.~\ref{sec:acquisition_objective} gives the two geometric quantities as implemented; Sec.~\ref{sec:cost_weighting} attaches the hardware discount; Sec.~\ref{sec:stopping_rule} gives the stopping rule that makes support size an output rather than a budget; and Sec.~\ref{sec:exchange_maintenance} gives the certified exchange move that maintains an already-selected support.
...
\end{equation}
The first term rewards closure of the spectral-window eigen-equation residual and the second penalizes acquisition cost.  Direct greedy maximization of Eq.~(\ref{eq:qse_acquisition_utility}) would require the very matrix elements the selector is meant to avoid, so each candidate is scored instead by two cheap surrogates computed against the current \emph{retained} frame.
...
\end{equation}
In the present implementation $B^-$ and $B^+$ are single records, so the move is a one-for-one exchange together with its pure-deletion and pure-insertion degenerate cases; higher-cardinality patches are admitted by the same certification but are not enumerated.  Across the $n_{\rm ph}=1$ regimes the mechanism is exercised rather than dormant: ten of the eleven committed patches are genuine delete--insert pairs, one is a pure deletion, and none is a pure insertion.  An optional budgeted variant replaces the cost inequality in Eq.~(\ref{eq:qse_exchange_dominance}) by a declared envelope $C(\Acal')\le C_{\rm budget}$, which lets the move trade cost for accuracy up to a stated ceiling; budgeted numbers are always labeled with their envelope.
...
The transaction shares the simultaneous delete--add structure used for checkpoint-maintained variational manifolds \cite{PaperII}, while its acceptance objective and guards are specific to the nonorthogonal QSE pencil.  The implementation evaluated here restricts $|B^-|\le 1$ and $|B^+|\le 1$, so every nondegenerate patch is a one-record-for-one-record exchange.  Pure deletion is enabled as a degenerate patch; pure insertion is implemented but disabled in the reported growth trace.  Multi-record exchanges are not evaluated here.
...
\begin{table}[t]
\caption{Activation of certified exchange at the terminal prefixes of the completed $n_{\rm ph}^{\max}=1$ growth trace.  Exchange is applied at each recorded support.  ``Rounds'' includes the final round that finds no admissible patch.  A joint commit has $|B^-|=|B^+|=1$; pure insertion is disabled in this diagnostic.}
