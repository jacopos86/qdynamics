...
All comparisons use one construction.  Every method draws candidates from an identical record alphabet, grows its subspace one record at a time to a fixed dimension cap, and is scored at each step against exact sector-restricted references on the same six-excitation window.  The resource required to reach an accuracy target is read off that growth trace,
\begin{equation}
