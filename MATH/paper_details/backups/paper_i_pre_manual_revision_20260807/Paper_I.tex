...
unitary \(U(\boldsymbol{\theta})\). The insertion domain remains append-only
until an accepted full refit produces an energy decrease below a prescribed
opening threshold \(\epsilon_{\rm ins}>0\); the next selection round then opens
...
decrease is at least \(\epsilon_{\rm ins}\). Appendix~\ref{app:numerical_conventions}
...
The shared Phase-II/III candidate-amplitude convention is given by
Eq.~\eqref{eq:phase23_alpha_star} in
Appendix~\ref{app:numerical_conventions}, together with the scalar degenerate
cases.
...
fixed affine map used by the inner optimizer.
...
decisions. The Phase-I parent response is only a screening proxy:
\(g_A=\sum_\mu c_\mu g_{P_\mu}\) can be small by cancellation even when an
individual Pauli-word gradient is large. Parent shortlisting may therefore
discard a high-utility child. The resource-cost term may also demote a candidate whose
realized energy improvement would have justified its additional cost. Either
mechanism changes the accepted state and all subsequent candidate rankings.
The present trajectories do not isolate which mechanism produces the policy
dependence.
...
Two local approximations also delimit the method. The accepted-refit whitening
map is fixed from the source-point Gram matrix although the Fubini--Study metric
changes during inner optimization; hence
\(W^\top G(\boldsymbol\theta)W=I\) holds only at the source point. The trust
radius is also feedback-calibrated: \(\rho_k\) constrains the predicted step,
while its predicted-to-realized displacement ratio determines \(\rho_{k+1}\).
This one-step-lag rule is a heuristic consistency correction, not an
independent test of local-model validity.
...
plateau gate and commutation reduction are applied. The three
...
For \(t\in\{2,3\}\), let \(\mathbf z_t^\star(r)\) be the optimizer returned
by the corresponding trust-region solve, with
\(\mathbf z_2=(\alpha)\) in Phase II and
\(\mathbf z_3=(\alpha,\delta\boldsymbol\theta)^\top\) on the retained
tangent support in Phase III. In both phases, the optimized candidate
amplitude is defined by the single expression
\begin{equation}
\alpha_t^\star(r)
:=
\bigl[\mathbf z_t^\star(r)\bigr]_\alpha,
\qquad t\in\{2,3\}.
\label{eq:phase23_alpha_star}
\end{equation}
For Phase II, this definition reduces, when \(H_{\alpha\alpha}>0\), to
\begin{equation}
\alpha_2^\star(r)
...
supported joint solve below; candidate--active coupling therefore changes the
solve, but not the definition in Eq.~\eqref{eq:phase23_alpha_star}.
...
\sum_\nu b_{k,j,\nu}P_\nu.
\end{equation*}
At the coefficient cutoff \(\tau=10^{-12}\), the candidate \(A\) and active
...
\end{equation}
Thus the append cut alone is scored until a subthreshold accepted refit opens
...
retained tangent support, all quantities below denote their supported
...
\subsection{Expanded accepted-refit coordinates}
...
Any inner optimizer acts in \(\mathbf y\) coordinates by passing its trial
points and perturbations to the circuit objective through the same affine map
...
The \(S\) entries in the resource tuples of
Figs.~\ref{fig:hh_macro_iteration50_insertion}
and~\ref{fig:hh_singleton_iteration50_insertion} give the number of logical
scalar-estimator invocations required through each reported accepted prefix.
...
\end{equation}
The accepted endpoint objective from round \(j\) is reused as the source energy
for round \(j+1\), and the initial source energy is carried into round zero.
Thus no separate outer-loop energy evaluation or duplicate post-Powell endpoint
...
Phase-III response & Active ansatz plus candidate & full active-coordinate response under the generalized Fubini--Study trust region; no independent \(H+\mu G\) damping \\
