...
Across the presently displayed prefixes, SNAKE gives lower energy error than
Geo-ADAPT in all six regimes and than Append-ADAPT in five, while reducing each
compiled circuit metric relative to both comparators in every regime. The
strong--strong sector is therefore a resource--accuracy tradeoff rather than a
strict accuracy improvement: Append-ADAPT reaches the lower error, whereas
SNAKE uses the substantially smaller compiled circuit. Logical estimator-query
work follows a different pattern from compiled cost, with SNAKE lower than
Geo-ADAPT throughout and lower than Append-ADAPT in five regimes; strong--weak
is the exception.
\par\addvspace{1.0ex}
...
Lane live-test threshold & Eq.~\eqref{eq:phase_restriction_map}, \(\tau_t\) & a lane remains live when its representative score exceeds \(\tau_t\) times the global phase maximum; the Hubbard--Holstein rows use \(\tau_2=0.10\) \\
...
Phase-I normalization and trust region & \(\lambda_F,\rho\) & \(\lambda_F=1\) in natural energy units; \(\rho=0.25\); trust-region score mode \\
...
Beam retention & \(B_{\rm live},B_{\rm child},\Phi_b\) & \(B_{\rm live}=3\), \(B_{\rm child}=2\); strict Pareto filtering in realized energy and cumulative added burden, followed by \(\Phi_b=(E_{\rm root}-E_b)/(1+\Delta K_b)\) \\
...
Schur ablation surrogate & \(\eta\) & metric-regularized Schur surrogate with \(\eta=0.01\); stationary-survivor approximation \(g_D=0\); ansatz-entry denominator cost weighting \\
