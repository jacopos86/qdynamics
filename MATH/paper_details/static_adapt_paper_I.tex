...
% Intro scaffold label retained for agents: Limitations with current approaches.
...
All inverse solves below use the damped, supported version of the displayed block. For Gram blocks, \(G^+\) denotes the Moore--Penrose inverse on the retained tangent support. For Schur and Hessian blocks, the implemented solve uses a symmetrized damped block, e.g. \(\frac12(H+H^{\top})+\lambda I\), with nonnegative ridge increased as needed to obtain a positive-definite Cholesky solve on the retained support. Reduced metric norms are floored, reduced-metric collapse is rejected, negative effective curvature is clipped in trust-region gain scoring, and scored gradients use the lower-confidence estimate. We suppress these inverse-regularization terms in the displayed scoring equations.
...
For macro-generator records retained by the Phase-II shortlist, SNAKE may replace \(r=(m,p)\) by exposed records \(r_J=((m,J),p)\), where \(A_{m,J}=\sum_{\mu\in J}a_{m\mu}P_{m\mu}\) is a nonempty Pauli subexpansion of \(A_m\) \cite{Tang2021}. These exposed records use the same \(S_{3,\ell}\) score and the same batch rule as ordinary records, but \(K_3(r_J)\) is evaluated on the exposed Pauli support and compiled-resource burden. With the fermionic hard guard enabled, only exposed records satisfying \(\Gamma_{\rm hard}(r_J)=\mathbf 1\{[A_{m,J},N]\approx0,\,[A_{m,J},N_\uparrow]\approx0,\,[A_{m,J},N_\downarrow]\approx0\}\) are admissible.
...
\end{equation}
where $c$ couples the candidate to the \(W_r\)-coordinate response and \(\mathbf H\) is the corresponding Hessian block on support \(W_r\); entries are given by
\begin{equation}
\begin{aligned}
c_j&:=
\left.
...
\end{equation}
Equations~\eqref{eq:phase3_regularized_hessian}--\eqref{eq:phase3_schur_response} give the ideal Schur model; the inverse and damping convention is the one stated above and also applies to the batch Schur blocks below.
...
\]
Each active coordinate \(d_j\) carries an admission step \(a_j\) and a cooldown
counter \(c_j(k)\).  The deletion-eligible coordinates are
\begin{equation}
\mathcal E_k^\ominus
=
\left\{
...
c_j(k)\le 0
\right\}.
\label{eq:ablation_eligibility}
\end{equation}
Thus a generator can be considered for deletion only after it has persisted for
at least \(p_{\rm protect}\) adaptive steps and is not currently on cooldown.

For the metric-regularized Schur route, the deletion model is built at the fixed
post-admission, post-refit ansatz.  Let
\[
g_a
=
\partial_{\theta_a}E(\mathcal O_k,\boldsymbol\theta_k),
\qquad
H_{ab}
=
...
\]
be the projected tangent Gram block, where
\[
\tau_a
=
\partial_{\theta_a}\ket{\psi(\mathcal O_k,\boldsymbol\theta_k)} .
\]
The metric-regularized quadratic block is
\[
\mathcal Q^\ominus
:=
H+\eta_{\rm FS}G,
...
\eta_{\rm FS}\ge 0 .
\]
Thus the prune route still uses the post-refit energy Hessian \(H\), but the
Schur solve is regularized by tangent-state geometry through \(G\).

For deleting coordinate \(j\), the survivor set is
\[
W_j=\{1,\ldots,n_k\}\setminus\{j\}.
\]
Deleting \(d_j\) corresponds to the coordinate displacement \(-\theta_j\).
Define the survivor right-hand side
\[
r_j^\ominus
:=
\theta_j\mathcal Q^\ominus_{W_jj}-g_{W_j}.
\]
The Schur-compensating survivor response is
\begin{equation}
\delta\boldsymbol\theta_{W_j}^{\star}
=
\left(
\mathcal Q^\ominus_{W_jW_j}
+\lambda_{\rm Schur} I
\right)^+
r_j^\ominus .
\label{eq:ablation_schur_response}
\end{equation}
This is the gradient-corrected version.  The stationary post-refit
specialization is recovered by setting \(g_{W_j}=0\).

The corresponding metric-regularized Schur-predicted deletion loss is
\begin{equation}
\Lambda_{j}^{\ominus}
=
-g_j\theta_j
+\frac12
\theta_j^2
\mathcal Q^\ominus_{jj}
-
(r_j^\ominus)^{\top}
\delta\boldsymbol\theta_{W_j}^{\star}
+
\frac12
(\delta\boldsymbol\theta_{W_j}^{\star})^{\top}
\left(
\mathcal Q^\ominus_{W_jW_j}
+\lambda_{\rm Schur} I
\right)
...
\end{equation}
Equivalently, for the unconstrained Schur response,
\[
\Lambda_{j}^{\ominus}
=
-g_j\theta_j
...
r_j^\ominus .
\]
This quantity estimates the local energy loss after removing \(d_j\) while
allowing the remaining ansatz coordinates to absorb the deleted tangent
direction in the metric-regularized quadratic model.

To prefer deletions that save larger compiled and measurement burden, let
\(K_j^{\rm ans}\) denote the same normalized hardware/shot-cost denominator used
in the SNAKE candidate scores, evaluated for the existing ansatz entry \(d_j\)
relative to the current ansatz-entry population.  The prune nomination score is
\[
\widetilde\Lambda_j^\ominus
=
\frac{\Lambda_j^\ominus}{K_j^{\rm ans}} .
\]
Smaller \(\widetilde\Lambda_j^\ominus\) is preferred: a deletion is most
attractive when its predicted Schur loss is small and the saved compiled or
measurement burden is large.  The Schur calculation is a nomination screen: it
chooses which deletion to test, but it is not the acceptance authority.

The tested deletion coordinate is the first Schur-safe coordinate in the ranked
eligible list,
\begin{equation}
\begin{aligned}
\mathcal C_k^\ominus
...
\left(
\operatorname{sort}_{\widetilde\Lambda^\ominus}
\mathcal E_k^\ominus
\right)
:
{}\right.
...
\end{equation}
The ordering \(\operatorname{sort}_{\widetilde\Lambda^\ominus}\) sorts by
increasing cost-weighted Schur deletion loss.  Deterministic tie-breakers are
used only for reproducibility.  If no eligible coordinate passes the Schur gate,
no measured deletion test is performed.  For the tested coordinate, the measured
nonlinear deletion loss is
\begin{equation}
\Delta E_j^\ominus
:=
...
\end{equation}
where the first energy is evaluated on the deleted child branch after the
allowed survivor coordinates have been initialized from the Schur response and
reoptimized.  Let \(E_k^-\) denote the energy before the just-accepted
admission/refit step, and define
\[
G_k^{\rm adm}:=\max\{0,E_k^- - E_k\},
\qquad
G_j^{\rm ret}:=
E_k^- - E(\mathcal O_k\ominus d_j,\boldsymbol\theta_{k,-j}).
\]
The measured deletion is accepted only if
\begin{equation}
A_j^\ominus
=
...
\end{equation}
Here \(G_k^{\rm adm}\) is the gain from the accepted admission, \(G_j^{\rm ret}\)
is the gain retained after deleting \(d_j\), \(\rho_{\rm ret}\) is the retained
gain fraction, and \(C_j^{\rm curv}\) is the curvature-safety indicator.

The committed child branch is
\[
\Xi_k^{\rm out}
=
\begin{cases}
(\mathcal O_k\ominus d_j,\boldsymbol\theta_{k,-j},
E(\mathcal O_k\ominus d_j,\boldsymbol\theta_{k,-j})),
& A_j^\ominus=1,\\[2mm]
(\mathcal O_k,\boldsymbol\theta_k,E_k),
& A_j^\ominus=0 .
\end{cases}
\]
Thus accepted ablation commits the reduced operator sequence.  Rejected
ablation discards only the temporary delete trial: the post-admission child
branch remains available for ordinary beam selection, and the rejected
coordinate is placed on cooldown.
...
The displayed adaptive comparisons use the Hamiltonian-local operator pools defined in Appendix~\ref{app:pools}, along with the inner-optimization budgets and pool-exposure conditions stated in the corresponding result blocks and settings notes.
...
% {"schema":"paper_i_hh_powell_pool_exposure_main_update_v1","updated_date":"2026-07-02","scope":"visible main Hubbard--Holstein result blocks","source_pdf":"output/pdf/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support.pdf","source_pdf_sha256":"4fee45db46167eea19181fc5b8fc1e73f23240c25982e5bc766b0c21c8734a02","source_json":"output/pdf/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support.json","source_json_sha256":"cbc86df20c0f0b25116e898dc1af439a3c429eda67f88d1033a78c6f782ed243","source_csv":"output/pdf/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support.csv","source_csv_sha256":"6409dd077cb4ab0fceaa15d62e7570a70549a49f1ed9989abcea6435767c521c","optimizer":"POWELL","source_row_order":["SNAKE","Geo macro","Geo singleton","Append macro","Append singleton"],"visible_row_order":["SNAKE","Geo macro","Append macro","Append singleton"],"omitted_visible_rows":["Geo singleton"],"omitted_plot_role_keys":["geo_singleton_b1"],"omission_policy":"Geo singleton source rows retained in TeX comments and source CSV/JSON; omitted from rendered tables and iteration plots because the macro-generator Geo row is non-worse at displayed precision in weak-weak, more accurate in the other five regimes, and much lower S in all six regimes.","figure_dir":"MATH/paper_details/figures/powell_pool_exposure_20260702","visible_columns":["method","k_pl","abs_delta_e","one_minus_F","N2q","D2q","Dc","S"],"fidelity_policy":"prefix fidelity is displayed only where source-selected prefix equals terminal row and terminal fidelity is present; otherwise --"}
% END_MACHINE_READABLE_HH_POWELL_POOL_EXPOSURE_MAIN_UPDATE_20260702
% BEGIN_MACHINE_READABLE_HH_POWELL_POOL_EXPOSURE_SNAKE_RUNTIME_SPLIT_TRACE_20260705
% {"schema":"paper_i_hh_powell_pool_exposure_snake_runtime_split_trace_v1","updated_date":"2026-07-05","scope":"visible weak-weak SNAKE runtime-split provenance for recovery reruns","support_json":"output/pdf/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support.json","source_result_json":"output/chtc_retrievals/paper_i_hh_fullmeta_singleton_symmetry_20260630_current_fetch/raw_outputs/paper_i_hh_fullmeta_singleton_symmetry_20260630_schedfix_powell/paper_i_hh_fullmeta_singleton_symmetry_20260630_schedfix_powell__weak_weak__snake__A_native_staged_singleton_hard_guard__native_forced__powell200__depth30_noearlystop__fullmeta_singleton_symmetry/json/result.json","visible_row_layer":"visible_row","optimizer":"POWELL","pool":"full_meta_unfiltered_hva_included","phase3_runtime_split_mode":"shortlist_pauli_children_v1","phase3_runtime_split_selection_mode":"archival_child_set_forward_v1","phase3_runtime_split_max_subset_size":1,"row_label_symmetry_policy":"hard_guard","selected_record_representation":"child_set","selected_record_symmetry_spec":null,"selected_record_symmetry_gate_checked":false,"selected_record_symmetry_gate_passed":true,"selected_record_symmetry_gate_skipped_reason":"runtime_split_symmetry_spec_missing","recovery_note":"Do not infer true selected-child-set hard-guard enforcement from the row label; recovery must preserve the archival missing-spec child-set representative unless explicitly running a true-hard-guard ablation."}
% END_MACHINE_READABLE_HH_POWELL_POOL_EXPOSURE_SNAKE_RUNTIME_SPLIT_TRACE_20260705
...
For the Hubbard--Holstein comparison below, all displayed adaptive rows use Powell refits in the pool-exposure diagnostic. SNAKE uses the native staged hard-guard singleton replacement condition; Geo-ADAPT is shown in its macro-generator form, while append-only ADAPT is shown with macro-generator and common Phase-0 hard-guard singleton-pool exposure. Each row is evaluated at the plotted selected prefix \(k_{\rm pl}\); circuit resource cells are Qiskit-compiled active-ansatz costs at the same prefix, and the displayed error is the same-cutoff quantity \(|E_{\rm ADAPT}-E_{\rm ED}|\). The Geo-ADAPT singleton-pool rows are non-competitive in this diagnostic: the macro-generator Geo-ADAPT row has equal displayed accuracy in weak--weak, lower same-cutoff error in the other five regimes, and approximately one to two orders of magnitude lower \(S\) throughout. We therefore omit Geo-ADAPT singleton rows from the rendered comparison and retain them only in source comments.

\noindent\begin{minipage}{\columnwidth}
\begin{center}
\includegraphics[width=\columnwidth]{figures/powell_pool_exposure_20260702/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support__weak_weak.png}
\end{center}
\end{minipage}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\includegraphics[width=\columnwidth]{figures/powell_pool_exposure_20260702/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support__intermediate_weak.png}
\end{center}
\end{minipage}
\par\addvspace{0.35ex}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\includegraphics[width=\columnwidth]{figures/powell_pool_exposure_20260702/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support__strong_weak.png}
\end{center}
\end{minipage}
\par\addvspace{0.35ex}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\includegraphics[width=\columnwidth]{figures/powell_pool_exposure_20260702/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support__weak_strong.png}
\end{center}
\end{minipage}
\par\addvspace{0.35ex}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\includegraphics[width=\columnwidth]{figures/powell_pool_exposure_20260702/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support__intermediate_strong.png}
\end{center}
\end{minipage}
\par\addvspace{0.35ex}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\includegraphics[width=\columnwidth]{figures/powell_pool_exposure_20260702/paper_i_hh_fullmeta_singleton_symmetry_chtc_current_20260630_powell_pool_exposure_support__strong_strong.png}
\end{center}
\end{minipage}
\par\addvspace{0.35ex}
...
\noindent\begin{minipage}{\columnwidth}
\captionof{table}{\label{tab:weak_weak_snake_mechanism_ablation_terminal_metrics}Weak--weak Hubbard--Holstein SNAKE mechanism ablations at terminal depth. Entries are differences from the source anchor in the terminal-depth support table. The energy-error column is a percent change, while circuit-resource and estimator-work columns are absolute integer deltas. \(\Delta S\) is computed from terminal branch-local \(S_{\rm alg}\). Batch-cap rows are omitted pending a corrected batching audit.}
\centering
\scriptsize
...
\toprule
Variant & $k$ & $\Delta|\Delta E|$ (\%) & $\Delta d_{\rm ans}$ & $\Delta N_{\twq}$ & $\Delta D_{\twq}$ & $\Delta D_c$ & $\Delta S$ \\
\midrule
Source anchor & 30 & 0.0 & 0 & 0 & 0 & 0 & 0 \\
No batching reference & 30 & +1.0 & 0 & -2 & -12 & -67 & -271 \\
No prune & 30 & +1.3 & +10 & +34 & +23 & +116 & +706 \\
No cost term & 30 & +1.4 & -2 & -6 & -12 & -52 & +574 \\
No novelty & 30 & +1.1 & -2 & -2 & -3 & -58 & -3198 \\
Novelty only; no second order & 30 & +202.3 & -10 & +166 & +112 & +922 & +34119 \\
Second order only; no novelty & 30 & +202.3 & -8 & +562 & +499 & +2619 & +55576 \\
No Phase 3 & 30 & +202.3 & -10 & +114 & +59 & +550 & +40823 \\
Phase 1 only; macro pool & 30 & +202.3 & -12 & +182 & +117 & +883 & +26538 \\
Phase 1 only; singleton pool & 30 & +202.3 & -5 & +24 & +15 & +229 & +19949 \\
Full geometry window & 30 & +7.8 & -1 & -10 & -16 & -88 & -530 \\
\bottomrule
\end{tabular}%
}
\end{minipage}
...
Tables~\ref{tab:weak_weak_schur_ladder_ablation_plateau_percent} and~\ref{tab:weak_weak_snake_mechanism_ablation_terminal_metrics} are the current weak--weak Hubbard--Holstein SNAKE mechanism-ablation tables. The plateau table reports percent changes relative to the full-SNAKE anchor, while the terminal table reports terminal-depth differences from the source anchor. In the terminal table, \(\Delta|\Delta E|\) is a percent change and \(\Delta S\) is an absolute difference in terminal branch-local \(S_{\rm alg}\).

\begin{itemize}
\item \textbf{Current machine-readable table source.} The terminal-depth table traces to \path{output/pdf/paper_i_hh_weak_weak_snake_mechanism_ablation_20260708/paper_i_hh_weak_weak_snake_mechanism_ablation_20260708.json}, SHA256 \texttt{07304f5bec8b71f8bf3ce29b2f1613899d74937e889af78cfcdfaf859f1630f6}. The plateau percent-change table retains the older support source until its prefix metrics are fully refreshed.
\item \textbf{Table layout convention.} The plateau-prefix and terminal-depth profiles are rendered as separate one-column tables. Plateau \(\Delta\) columns are percentages relative to the full-SNAKE anchor; terminal-depth rows are differences from the source anchor in support PDF page 7. \(k_{\rm pl}\) is an accepted-prefix index, not a percent change.
...
\item \textbf{No-shortlisting row.} The no-shortlisting variant is omitted from the current human-facing mechanism-ablation tables until the route has corrected deterministic-shot evidence.
\end{itemize}
...
Snake & 9.97e-6 & -- & 115 & 111 & 723 & 3,754\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
...
Snake & 1.59e-11 & -- & 18 & 10 & 120 & 5,936 & 8e-16 & -- & 52 & 52 & 274 & 751\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
...
Snake & 6.16e-7 & -- & 19 & 17 & 123 & 685 & 9.97e-6 & -- & 115 & 111 & 723 & 3,754\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
