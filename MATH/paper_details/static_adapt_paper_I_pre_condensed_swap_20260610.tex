...
appropriate.  
We use \(E_T\) as a useful convergence target for selecting displayed adaptive prefixes and for identifying a practical energy-resolution scale. The tables themselves report raw absolute energy error against the relevant ED reference,
\[
|\Delta E|=\left|E_{\rm alg}(n_{\rm ph}^{\rm work})-E_{\rm ref}\right|.
\]

Here $E_{\rm alg}$ is the energy obtained from a VQE method and $E_{\rm ref}$ is the energy obtained from exact diagonalization. For Tables~\ref{tab:fixed_accuracy_claims} and~\ref{tab:fixed_accuracy_spin_boson}, phonon-bearing VQE methods use a working cutoff \(n_{\rm ph}^{\rm work}\), while the reference ED energy uses the higher cutoff \(n_{\rm ph}^{\rm ED}\) listed in each table caption. The working cutoff is chosen so that the ED cutoff gap \(|E_{\rm ref}(n_{\rm ph}^{\rm ED})-E_{\rm ref}(n_{\rm ph}^{\rm work})|\) is below \(E_T\). For the Hubbard--Holstein stress test, several adaptive histories enter persistent plateaus before or near the same reference scale, so Table~\ref{tab:hh_first_plateau_prefix_costs} reports the same-cutoff plateau-prefix error \(|E_{\rm alg}(n_{\rm ph}^{\rm work})-E_{\rm ref}(n_{\rm ph}^{\rm work})|\) and compiled resources at the first effective plateau prefix. The higher-cutoff ED gap is retained as a cutoff-resolution diagnostic; for weak--weak, strong--weak, weak--strong, and strong--strong, respectively, the gaps are \(2.83\times10^{-5}\), \(9.86\times10^{-6}\), \(1.41\times10^{-4}\), and \(2.92\times10^{-5}\). The fixed-prefix Hubbard--Holstein comparison is retained as the audit table in Appendix~\ref{app:results_discussion}.
...
\end{equation}
Entries with \(1-F<10^{-5}\) can be considered sufficient targets.
...
% {"schema":"paper_i_tables_i_ii_raw_absolute_error_redisplay_v1","updated_date":"2026-06-09","scope":"visible Tables I-II error columns and captions only; resource, fidelity, and S cells unchanged","display_error_policy":"raw absolute energy error |E_alg-E_ref| at the displayed ansatz/prefix","target_role":"E_T=2e-4 retained as useful convergence target and adaptive-prefix/resource rule, not subtracted from displayed error","source":"MATH/paper_facing/paper_I_static_scaffold/paper_i_tables_i_ii_spsa_optuna_current_best_promotion_20260601.json plus row-specific SNAKE provenance comments","hubbard_raw_errors":{"HEA":["3.10e-8","5.86e-8"],"family":["2.81e-8","5.92e-8"],"append":["1.56e-2","1.37e-1"],"TETRIS":["1.58e-2","1.36e-1"],"Geo":["4.72e-10","7.30e-9"],"Qubit/QEB":["1.56e-2","5.44e-9"],"SNAKE":["6.66e-16","2.31e-14"]},"spin_boson_raw_errors":{"HEA":["3.05e-8","4.87e-3"],"family":["1.67e-3","6.70e-3"],"append":["6.16e-7","9.97e-6"],"TETRIS":["6.16e-7","9.97e-6"],"Geo":["6.16e-7","9.97e-6"],"Qubit/QEB":["1.67e-3","6.70e-3"],"SNAKE":["2.01e-6","1.10e-4"]}}
% END_MACHINE_READABLE_TABLE_I_II_RAW_ERROR_REDISPLAY_20260609

\noindent\begin{minipage}{\textwidth}
\inlinetablecaption{tab:fixed_accuracy_claims}{Hubbard current-evidence comparison.  Tables~\ref{tab:fixed_accuracy_claims}--\ref{tab:fixed_accuracy_spin_boson} report raw absolute energy error \(|\Delta E|=|E_{\rm alg}-E_{\rm ref}|\) for the displayed ansatz.  The useful convergence target \(E_T=10^{-4}LE_0=2\times10^{-4}\) determines the adaptive prefix used for resource accounting when a method reaches that scale.  The weak and strong columns use $U/t=0.5$ and $U/t=1.5$, respectively, where $U$ is the onsite repulsion and $t$ is the nearest-neighbor hopping.  Resource columns report Qiskit-transpiled ansatz circuits using an IBM Marrakesh backend; \(S\) is the normalized estimator/work proxy evaluated for the same ansatz.  Fixed-structure VQE rows use the compiled fixed ansatz.  Fidelity columns report the corresponding post-hoc overlap diagnostic when the displayed ansatz is reconstructable.  Entries with \(1-F<10^{-5}\) can be considered sufficient targets.}
...
\colrule
HEA VQE & 3.10e-8 & -- & 6 & 5 & 35 & 4.80e4 & 5.86e-8 & -- & 6 & 5 & 35 & 4.80e4\\
family VQE & 2.81e-8 & -- & 56 & 52 & 278 & 4.80e4 & 5.92e-8 & -- & 56 & 52 & 278 & 4.80e4\\
append ADAPT & 1.56e-2 & -- & 56 & 52 & 274 & 1.96e4 & 1.37e-1 & -- & 56 & 52 & 274 & 1.43e4\\
TETRIS-ADAPT & 1.58e-2 & -- & 56 & 52 & 274 & 2.41e4 & 1.36e-1 & -- & 56 & 52 & 274 & 2.41e4\\
Geo-ADAPT & 4.72e-10 & -- & 1764 & 1699 & 8898 & 7.05e4 & 7.30e-9 & -- & 376 & 364 & 1862 & 8.80e4\\
Qubit/QEB & 1.56e-2 & -- & 56 & 52 & 274 & 4356 & 5.44e-9 & -- & 56 & 52 & 274 & 2.46e4\\
SNAKE & 6.66e-16 & 0 & 56 & 52 & 219 & 440 & 2.31e-14 & 0 & 56 & 52 & 219 & 350\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
\par\addvspace{1.0ex}
For the Hubbard chain, UCCSD-like structure is sufficient to reach near-machine-precision energy error, as shown by the family-informed VQE row. The HEA row is a useful caveat: despite the known tendency of hardware-efficient layers to leave conserved symmetry sectors, this small two-site instance remains accurate at the displayed precision. However, greedy first-order ADAPT selects the large-gradient singles before the double excitation; after the singles are optimized, the double is not promoted, producing a plateau. This persists for append-only ADAPT, TETRIS-ADAPT, and Qubit/QEB-ADAPT even at compiled depths above \(10^4\). Pos-Geo-ADAPT also converges in both Hubbard regimes, but we report Geo-ADAPT consistently across the displayed benchmark suite because the positional search incurs a runtime blow-up in the spin-boson/Rabi table and especially in the Hubbard--Holstein table. SNAKE avoids this failure through second-order Schur reranking and reaches near-machine-precision error in both regimes.

\noindent\begin{minipage}{\textwidth}
\inlinetablecaption{tab:fixed_accuracy_spin_boson}{Spin-boson/Rabi current-evidence comparison.  The weak column uses $g/\omega_0=0.05$ with $(n_{\rm ph}^{\rm work},n_{\rm ph}^{\rm ED})=(1,5)$; the strong column uses $g/\omega_0=0.1$ with $(n_{\rm ph}^{\rm work},n_{\rm ph}^{\rm ED})=(2,6)$.  Here $g$ is the spin-mode coupling and $\omega_0$ is the mode frequency.  The error column reports raw absolute external-reference error \(|\Delta E|=|E_{\rm alg}(n_{\rm ph}^{\rm work})-E_{\rm ED}(n_{\rm ph}^{\rm ED})|\), while resource columns use the same displayed-prefix convention as Table~\ref{tab:fixed_accuracy_claims}.  Entries with \(1-F<10^{-5}\) can be considered sufficient targets.}
...
\cline{2-7}\cline{8-13}
 & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{2q}$} & \metrichead{$D_{2q}$} & \metrichead{$D_c$} & \metrichead{$S$} & \metrichead{$|\Delta E|$} & \metrichead{$1-F$} & \metrichead{$N_{2q}$} & \metrichead{$D_{2q}$} & \metrichead{$D_c$} & \metrichead{$S$}\\
\colrule
HEA VQE & 3.05e-8 & -- & 6 & 5 & 35 & 4.80e4 & 4.87e-3 & -- & 10 & 7 & 37 & 4.80e4\\
family VQE & 1.67e-3 & -- & 31 & 29 & 245 & 4.80e4 & 6.70e-3 & -- & 101 & 78 & 609 & 4.80e4\\
append ADAPT & 6.16e-7 & -- & 23 & 21 & 177 & 4938 & 9.97e-6 & -- & 173 & 163 & 1061 & 6269\\
TETRIS-ADAPT & 6.16e-7 & -- & 23 & 21 & 177 & 4176 & 9.97e-6 & -- & 173 & 163 & 1061 & 4310\\
Geo-ADAPT & 6.16e-7 & -- & 23 & 21 & 177 & 5430 & 9.97e-6 & -- & 593 & 583 & 4301 & 5.85e4\\
Qubit/QEB & 1.67e-3 & -- & 11 & 11 & 72 & 10 & 6.70e-3 & -- & 57 & 57 & 348 & 61\\
SNAKE & 2.01e-6 & 0 & 28 & 28 & 113 & 15 & 1.10e-4 & 4.37e-05 & 174 & 167 & 576 & 29\\
\end{tabular}
\end{ruledtabular}
\end{center}
\end{minipage}
\par\addvspace{1.0ex}
The spin-boson/Rabi benchmark isolates bosonic displacement and cutoff sensitivity without fermionic correlation. In the weak regime, HEA VQE and the adaptive methods reach the useful convergence target, while family VQE and Qubit/QEB remain above that scale. In the strong regime, the adaptive methods and SNAKE remain below the useful convergence target, while HEA VQE, family VQE, and Qubit/QEB remain above it. Relative to the below-threshold adaptive rows, SNAKE trades a higher two-qubit count and two-qubit depth for lower measurement overhead and shorter total circuit depth in both regimes.
...
% displayed ansatz. Rows that reach E_T use the first accepted below-threshold ansatz; other adaptive rows use the final
...
\noindent\begin{minipage}{\textwidth}
\inlinetablecaption{tab:hh_first_plateau_prefix_costs}{Current-evidence Hubbard--Holstein prefix resource comparison at the first effective plateau in Fig.~\ref{fig:hh_tableiii_adapt_convergence}.  The plateau iteration $k_{\rm pl}$ is the first displayed-window accepted ADAPT iteration whose remaining displayed-window same-cutoff improvement is at most five percent of the current same-cutoff error.  The error column reports \(|E_{\rm alg}(n_{\rm ph}^{\rm work})-E_{\rm ED}(n_{\rm ph}^{\rm work})|\) at that prefix.  Resource columns report the Qiskit-transpiled prefix circuit under the same IBM Marrakesh convention as Tables~\ref{tab:fixed_accuracy_claims} and~\ref{tab:fixed_accuracy_spin_boson}.}
\begin{center}
\scriptsize
\begin{ruledtabular}
\begin{tabular}{p{0.13\textwidth}p{0.17\textwidth}rrrrr}
Regime & Method & $k_{\rm pl}$ & $|\Delta E|$ & $N_{\twq}$ & $D_{\twq}$ & $D_{\rm circ}$ \\
\colrule
weak--weak & append ADAPT & 7 & 1.14e-3 & 180 & 126 & 870 \\
 & TETRIS-ADAPT & 4 & 1.13e-3 & 168 & 96 & 719 \\
 & SNAKE & 11 & 4.23e-4 & 124 & 48 & 773 \\
 & Geo-ADAPT & 6 & 1.13e-3 & 100 & 69 & 533 \\
\colrule
strong--weak & append ADAPT & 4 & 7.81e-2 & 96 & 68 & 503 \\
 & TETRIS-ADAPT & 4 & 7.83e-2 & 176 & 96 & 719 \\
 & SNAKE & 11 & 1.78e-4 & 314 & 243 & 801 \\
 & Geo-ADAPT & 62 & 1.89e-4 & 2988 & 2719 & 19755 \\
\colrule
weak--strong & append ADAPT & 11 & 4.51e-2 & 2560 & 2207 & 9016 \\
 & TETRIS-ADAPT & 9 & 4.55e-2 & 9160 & 8549 & 27422 \\
 & SNAKE$^\ddagger$ & 42 & 1.92e-2 & -- & -- & -- \\
 & Geo-ADAPT & 11 & 4.27e-2 & 776 & 630 & 3535 \\
\colrule
strong--strong & append ADAPT & 10 & 2.95e-2 & 1216 & 883 & 5112 \\
 & TETRIS-ADAPT & 4 & 2.81e-2 & 632 & 296 & 1841 \\
 & SNAKE$^\dagger$ & 13 & 7.95e-3 & 976 & 938 & 5559 \\
...
The duplicated final-run benchmark table is retained here as supporting context for the raw-error resource comparison in Table~\ref{tab:fixed_accuracy_claims}.
...
The main benchmark comparison uses three two-site Hamiltonian classes
...
Table~\ref{tab:fixed_accuracy_hh_cartesian} is retained here as a Hubbard--Holstein fixed-prefix audit in the sense that the displayed ansatz prefixes and resource cells come from the earlier fixed-prefix comparison. The main Hubbard--Holstein comparison uses plateau prefixes because several adaptive histories enter persistent plateaus before or near the benchmark threshold. In this audit, the error column reports raw external-reference energy error, so the error values remain comparable to the plateau-prefix table while preserving the older prefix-cost audit.
...
% Historical note added 2026-06-09: this legacy block records the old
% fixed-prefix audit source state. Later raw-error redisplay comments govern
% the current rendered Table~\ref{tab:fixed_accuracy_hh_cartesian} values.
% {
...
%       "display_delta_e_source": "legacy_external_abs_delta_e_minus_threshold",
...
%       "display_delta_e_source": "legacy_external_abs_delta_e_inferred_minus_threshold",
...
%       "display_delta_e_source": "raw_external_abs_delta_e_inferred",
%       "display_delta_e": 0,
...
%       "display_delta_e_source": "raw_external_abs_delta_e",
%       "display_delta_e": 0,
...
%   "table_label": "tab:fixed_accuracy_hh_cartesian",
%   "display_delta_e_policy": "DeltaE column redisplayed as raw_external_abs_delta_e=abs(E_ref(n_ph_ED)-E_alg(n_ph_work)); resource and fidelity cells are preserved from the prior fixed-prefix audit except for the explicitly promoted strong-weak Geo-ADAPT k=62 prefix.",
%   "source_files": [
...
The current raw-error tables use the noiseless accepted-prefix proxy
