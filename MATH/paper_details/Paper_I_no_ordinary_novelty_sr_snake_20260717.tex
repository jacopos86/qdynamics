...
\large Evidence Copy: No-Ordinary-Novelty SR-SNAKE}
...
\begin{abstract}
Near-term ground-state quantum simulation requires ansätze that must simultaneously satisfy expressivity while minimizing circuit depth, entangling gates, and measurement overhead. ADAPT-VQE addresses this problem; however, its gate-selection mechanism is sensitive to parameter heterogeneities, local gradient plateaus, state-tangent redundancy, and is agnostic to generator-specific hardware costs. We introduce SNAKE, which leverages a shortlisting mechanism to rank generators according to a joint objective: a Fubini--Study trust-region-constrained second-order energy model coupling the candidate generator's gradient to the existing ansatz through Schur reduction, and a graph-aware, generator-variance-competition model penalizing inordinate hardware costs. We present results of SNAKE, append-only ADAPT-VQE, and Geo-ADAPT tested on the Hubbard, spin-boson/Rabi, and Hubbard--Holstein using matched encodings and inner-optimization protocols. In this evidence copy, the Hubbard--Holstein SNAKE rows use an undamped singleton-response route with the ordinary Phase-II and Phase-III novelty multipliers, beam continuation, and generator pruning disabled; the all-energy-models-infeasible novelty fallback remains available. The displayed SNAKE points use terminal endpoints for weak--weak, weak--strong, and intermediate--strong, and the last selected plateau checkpoints for intermediate--weak, strong--weak, and strong--strong. Their same-cutoff absolute errors are \(7.99\times10^{-15}\), \(3.44\times10^{-15}\), \(1.57\times10^{-6}\), \(3.05\times10^{-6}\), \(4.73\times10^{-5}\), and \(3.21\times10^{-7}\), respectively. These selected errors are below the currently displayed Geo-ADAPT and append-only ADAPT errors in all six regimes. Qiskit costs are reconstructed at all six displayed SNAKE endpoints; because cross-method endpoints remain unmatched, this copy makes no compiled-resource superiority claim.
...
Furthermore, among the possible batch candidates, \textsc{SNAKE} admits the highest-ranked distinct batches as parallel circuit branches using a Lowerre-style beam search \cite{Lowerre1976Beam}. Branch pruning is determined by a circuit-cost weighted energy objective.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/snake_staged_record_selection_ablation_flow_20260710.png}
...
%
To extend ADAPT selection from generator identity to circuit placement, SNAKE
scores generator--position records and restricts them through increasingly
expensive phases.
...
r=(A,p) .
\]
We define the candidate generator--position record set as the set of all such
records. At the current iteration \(k\), this gives
\begin{equation}
\mathcal R_k
=\mathcal G\times\Pcal_k .
\label{record_set}
\end{equation}
...
We then give Eq.~\eqref{record_set} explicit phase dependence. At a given ADAPT outer-loop iteration \(k\), index each phase by \(t\in\{0,1,2,3\}\), and initialize the recursion as \(\mathcal R_k^{(-1)}:=\mathcal R_k\). We then suppress the outer-iteration subscript \(k\) in the following phase-restriction definitions, writing \(\mathcal R^{(t)}\) for \(\mathcal R_k^{(t)}\).
...
\end{equation}
where \(\Delta E_t(r)\) is the phase-\(t\) state-gradient gain and
\(K_t(r)\geq 1\) is the resource-cost
...
ranking that combines estimated energy gain and cost normalization.
...
\(\mathcal L\) denote the fixed set of physical lane labels, and let
\(\lambda:\mathcal G\to\mathcal L\) map each generator to its physical
operator-type label. For \(r=(A,p)\), we write
\(\lambda(r):=\lambda(A)\); thus, each record inherits the lane label of its
generator.

For each lane represented in \(\mathcal R^{(t-1)}\), define its inherited
record component by
\begin{equation*}
\mathcal R_{\ell}^{(t-1)}
=
...
\end{equation*}
These lane-record components partition the inherited record set
\(\mathcal R^{(t-1)}\). For each represented lane, its representative record is
the within-lane maximizer
\[
r_{\ell}^{\star}
\in
\operatorname*{arg\,max}_{r\in\mathcal R_{\ell}^{(t-1)}}
S^{(t)}(r).
\]
Let \(\mathcal L^{(t)}\subseteq\mathcal L\) denote the lane labels that remain
live at phase \(t\). For each represented lane \(\ell\), the lane remains live
when its best inherited record is competitive with the best inherited record
over the full phase set:
\begin{equation}
\ell\in\mathcal L^{(t)}
...
\ge
\tau_t
\max_{r\in\mathcal R^{(t-1)}}S^{(t)}(r).
\label{eq:phase_restriction_map}
\end{equation}
Here \(\tau_t\in[0,1]\) is the phase-\(t\) relative lane-retention threshold. The
...
\end{equation*}
%
Here $\mathcal B$ is the batch-admissible set of retained Phase-III records, and $\mathcal D$ is the ablation-admissible set of existing ansatz entries. The symbol \(\oplus\) denotes admission of selected generator records into the ordered ansatz, and \(\ominus\) denotes removal of ablation-admissible existing records. Before discussing in detail the different stages of candidate selection, we define the resource-cost model and candidate gradient used by those stages.

\subsubsection{Resource-cost model}
%
\noindent
...
incremental proxies for two-qubit gate count, graph-aware two-qubit depth,
...
%
\noindent
All ranking phases use the same position-resolved first-order energy signal.
For a candidate record \(r=(A,p)\), the expectation of the previously defined
gradient observable \(O_r=i[\widetilde A_r,H]\) gives the derivative magnitude
used for screening, trust-region prediction, and amplitude initialization:
\begin{equation}
\begin{aligned}
g(r)
&=
...
\end{equation}
%
Because \(A\) and \(-A\) generate the same one-parameter unitary family, we
choose the sign of each candidate direction so that its local energy derivative
is \(-g(r)\). Accordingly, \(\alpha\ge0\) below denotes displacement along this
downhill-oriented direction.

\subsubsection{Phase 0}
%
\textcolor{red}{To address [the specific local deficiency], SNAKE [performs the precise construction]. This produces [the quantity, ranking, warm start, admissibility decision, or reduced model used downstream].}

\noindent
Phase 0 is the inexpensive first-pass screening stage. It is obtained from Eq.~\eqref{eq:phase0_score} by using only the standard first-order ADAPT gradient and the phase-0 resource-cost penalty. For the fixed candidate record \(r=(A,p)\), the Phase-0 score is
%
\begin{equation}
S^{(0)}(r)=\frac{\alpha_0 g(r)}{K_0(r)}\,.
\label{eq:phase0_score}
\end{equation}
%
Here \(\alpha_0>0\) is a fixed pre-screening scale factor, \(g(r)\) is the gradient magnitude from Eq.~\eqref{eq:record_gradient}, and \(K_0(r)\) is the phase-0 cost penalty. This stage is intentionally non-aggressive: its purpose is not to make the final generator-selection decision, but to remove candidates with both small gradient signals and large estimated costs before more expensive evaluations are performed. Records are passed to Phase I when their lane-wise score exceeds the threshold \(\tau_{0,\ell}\). To avoid discarding all useful candidates in a low-signal iteration, a fallback rule also retains the top-ranked records in each lane.
...
and Hamiltonian-block motion therefore remain exposed before geometric and
curvature measurements are incurred.
...
At phase \(t\), records are ordered by \(S^{(t)}\) and retained under
...
Gram and metric blocks may be rank deficient when local tangent directions are
redundant. We use \(G^+\) for the Moore--Penrose pseudoinverse after removing
numerically null directions. Hessian response blocks instead use a
positive-definite stabilized solve: the block is symmetrized, shifted by a
positive ridge, and the ridge is increased geometrically until a Cholesky
factorization succeeds on the retained solve subspace. Only supported
positive-definite response solves are used. The same Hessian stabilization is
used for the Phase-III and batch response blocks; the ablation block is defined
separately below.

\subsubsection{Phase I}
%
\noindent
Phase I reduces sensitivity to generator parameterization by measuring the
squared Fubini--Study norm of the candidate-induced tangent. Combining this
state-space scale with the downhill gradient inside a displacement trust region
yields a geometry-regularized gain and a trial candidate amplitude, without yet
using directional energy curvature \cite{Nocedal2006TrustRegion}. For a fixed
candidate record \(r\), we suppress the \(r\)-dependence of \(g\), \(F\),
\(\tau\), and \(\alpha^\star\) in this local surrogate:
%
\begin{equation}
\begin{split}
\Delta E_{1}(r)={}&\max_{\substack{\alpha\ge0\\\alpha^2F\le \rho^2}}
\left[g\alpha-\frac{\lambda_F}{2}F\alpha^2\right],
\end{split}
\label{eq:phase1_gain}
\end{equation}
%
where \(g\) is given by Eq.~\eqref{eq:record_gradient}, and \(\lambda_F>0\)
is the fixed energy-unit normalization multiplying the Fubini--Study
regularizer. It is not a tunable resource weight; the reported calculations use
\(\lambda_F=1\) in natural energy units. The factor \(F\) measures how strongly
candidate record \(r\) moves the current variational state. This scale is
invariant under reversal of the generator sign. Using an unoriented dummy
coordinate \(\beta\), let
\(\ket{\psi(\beta;r)}=e^{-i\beta\widetilde A_r}\ket{\psi}\). The candidate
tangent is
\(\tau=\left.\partial_\beta\ket{\psi(\beta;r)}\right|_{\beta=0}
=-i\widetilde A_r\ket\psi\). Let
\(\Pi=I-\ket\psi\!\bra\psi\) be the projector orthogonal to the current state
ray. The diagonal Fubini--Study scale is then given by
%
\begin{equation*}
F=\|\Pi\tau\|^2.
...
\end{equation}
Substituting \(\alpha^\star\) into Eq.~\eqref{eq:phase1_gain} gives
\(\Delta E_1(r)\) for the Phase-I score. If the record is ultimately admitted,
the downhill-oriented value \(\alpha^\star\) initializes its new coordinate
before joint inner reoptimization under Eq.~\eqref{eq:theta_opt}.

\subsubsection{Phase II}
%
\noindent
Phase II adds directional energy curvature to the Phase-I model. The measured
curvature \(h\) replaces the geometric quadratic regularizer in the local gain
model. This construction yields the curvature-aware gain \(\Delta E_2(r)\) and
trial amplitude used in the Phase-II ranking.

The trust-region constrained second-order energy estimate is given by
\begin{equation*}
\Delta E_{2}(r)
=\max_{\substack{\alpha\ge0\\\alpha\sqrt{F}\le\rho}}
\Bigl[g\alpha -\frac{[h]_+\alpha^2}{2}\Bigr],
...
and $\partial^2_{\alpha}\ket{\psi} =-(U_{>p}AU_{>p}^{\dagger})^2\ket{\psi}.$ Here \(>p\) denotes the ansatz generators to the left of position index \(p\), where the rightmost generator is index \(1\), and \([h]_+:=\max(h,0)\) is the positive directional-curvature contribution retained by the score. The trial-amplitude magnitude uses Eq.~\eqref{eq:phase1_alpha_star} with the first denominator \(\lambda_FF\) replaced by \([h]_+\), while \(\rho/\sqrt{F}\) remains unchanged; when \([h]_+=0\), the magnitude is the trust-region boundary. The warm start follows the downhill orientation defined after Eq.~\eqref{eq:record_gradient}.
...
records retained by the Phase-II shortlist \cite{Tang2021}. The resulting
children pass through the fermionic symmetry guard before Phase-III scoring.
For the record
...
%
\noindent
Phase III estimates candidate value after the active ansatz is allowed to
respond through local reoptimization. Schur elimination of the
candidate-specific window \(W_r\) from the coupled quadratic model yields the
relaxed curvature \(h^\star\), inherited-coordinate response
\(\delta\theta^\star\), and compensated tangent geometry
\((\tau^\star,F^\star)\). These quantities determine the final
candidate gain and enlarged-ansatz warm start.

The energy gain model is
\begin{equation}
\Delta E_{3}(r)=
\max_{\substack{\alpha\ge0\\\alpha\sqrt{F^\star}\le\rho}}
\left[g\alpha-\frac{[h^\star]_+ \alpha^2}{2}\right].
\label{eq:phase3_gain}
\end{equation}
Here \(h^\star\) and \(F^\star\) are the Schur-relaxed energy curvature and
Fubini--Study scale defined below.

Let $\delta\theta$ be the response parameter vector with indices in \(W_r\subseteq\{1,\ldots,|\boldsymbol{\theta}_k|\}\). Phase III uses a stabilized local quadratic surrogate in which the candidate operator's energy gain is coupled to this refit-window response after admitting $r$.
...
\left[\frac12(\mathbf H+\mathbf H^{\top})+\lambda_HI\right]
\delta\theta,
\end{split}
...
\end{equation}
Let \(\mathbf R\) denote the stabilized \(W_r\)-supported Hessian block used in the refit solve:
\begin{equation}
\mathbf R=\frac12(\mathbf H+\mathbf H^{\top})+\lambda_HI\succ0.
\label{eq:phase3_regularized_hessian}
\end{equation}
Here \(\lambda_H\) denotes the effective Hessian ridge, increased geometrically
from its configured initial value until the retained response block is positive
definite.
...
\end{equation}
Eqs.~\eqref{eq:phase3_window_derivatives}--\eqref{eq:phase3_regularized_hessian} give the stabilized Schur model. Dividing $\delta\theta^\star$ by $\alpha$ gives the inherited-coordinate compensation per unit candidate amplitude.
For \(r\), let \(W_r\) be the capped refit-coordinate window, with
\(i,j\in W_r\). The Fubini--Study Gram matrix on this coordinate block and the
candidate tangent-covariance vector are
\begin{equation*}
(G)_{ij}=\Re\langle\Pi\tau_i,\Pi\tau_j\rangle,
\qquad
(q)_i=\Re\langle\Pi\tau_i,\Pi\tau\rangle.
\end{equation*}
These geometric blocks convert the inherited-coordinate response into the
state-space displacement entering the trust-region boundary. The resulting
Schur-reduced residual tangent and Fubini--Study norm are
\begin{equation}
\begin{aligned}
\tau^\star
...
\end{equation}
The candidate amplitude is obtained from
Eq.~\eqref{eq:phase1_alpha_star} after replacing the unconstrained denominator
\(\lambda_FF\) by \([h^\star]_+\) and using \(F^\star\) in the trust-region
boundary. Evaluating Eq.~\eqref{eq:phase3_schur_response} at this amplitude
gives the inherited-coordinate warm start for inner reoptimization.
This completes Eq.~\eqref{eq:phase3_gain}.
...
For batch-local indices \(a,b=1,\ldots,s\) and capped batch refit-coordinate window \(W_B\), let $H_{BB}=[\partial_{\alpha_a}\partial_{\alpha_b}E]_{a,b=1}^s$ be the Hessian block among candidate amplitudes, and let $H_{W_BB}=[\partial_{\theta_i}\partial_{\alpha_a}E]_{i\in W_B,\,a=1,\ldots,s}$ be the mixed window-candidate block. Then, with $H_{W_BW_B}=[\partial_{\theta_i}\partial_{\theta_j}E]_{i,j\in W_B}$ and $\mathbf R_B=\frac12(H_{W_BW_B}+H_{W_BW_B}^{\top})+\lambda_HI\succ0$, using the same adaptive ridge rule as Eq.~\eqref{eq:phase3_regularized_hessian}, the Schur-refit batch curvature, residual tangents, and reduced Gram matrix are
\begin{equation*}
\begin{aligned}
H_B^\star
&=
...
\Delta E_3(B)
=
\max_{\substack{\alpha_B\succeq0\\\alpha_B^\top G_B^\star\alpha_B\le \rho_B^2}}
\left[
g_B^\top\alpha_B
-
...
remains favorable along a different growth path. \textsc{SNAKE} therefore uses
Lowerre-style beam search \cite{Lowerre1976Beam} to refit multiple competitive
child circuits within a fixed-width beam. Given a score-ordered top-$N$
candidate-batch list, a parent circuit can spawn $N$ child branches, with each
child differing by the batched candidate admitted to that branch. The cost of
this parallel beam universe is increased total hardware-graph and qubit support
across candidate child circuits. Each child branch then undergoes the
...
\end{equation*}
and the top \(B_{\rm live}\) branches are retained.


\subsubsection{Generator ablation}
%
After each admission and inner refit, \textsc{SNAKE} tests whether an active
...
\(\delta\boldsymbol\theta_D^\star\) and predicted deletion loss
\(\Lambda(A_j)\); candidates are nominated by predicted loss per relieved
...
Then, for \(j\in\mathcal E_k\), we predict the Schur response energy
...
\right|_{\boldsymbol\theta=\boldsymbol\theta_k^\star}.
\]
Because the surrogate is expanded at the refitted optimum, we impose the
stationary-survivor approximation \(g_D=0\); residual survivor-coordinate
gradients from incomplete inner optimization are neglected.

We define a metric-regularized generalization of the Phase-III Schur surrogate
for ablation as
\begin{equation}
\begin{aligned}
\Lambda(A_j)
...
&+
\frac{\lambda_H}{2}
\left\|\delta\theta_D\right\|^2
\Biggr].
\end{aligned}
\label{eq:schur_metric_prune}
\end{equation}
The parameter \(\eta\ge0\) controls the Fubini--Study regularization of this
ablation surrogate.
The final term applies the same positive ridge \(\lambda_H\) used in the Schur
solve, so the stabilized survivor block is \(\mathcal Q_{DD}+\lambda_HI\).
The local energy term is the deletion specialization of the Phase-III
quadratic surrogate in Eq.~\eqref{eq:local_quadratic}:
\begin{equation*}
\begin{aligned}
\Lambda_E(A_j,\delta\theta_D)
={}&
...
symmetrized and projected onto their positive-semidefinite parts, which we
continue to denote by \(H\) and \(G\). Define the metric-regularized quadratic
block
\[
\mathcal Q:=H+\eta G .
\]
...
&=
\theta_{k,j}^\star
(\mathcal Q_{DD}+\lambda_HI)^{+}\mathcal Q_{Dj}
\\
&=
\theta_{k,j}^\star
\left(H_{DD}+\eta G_{DD}+\lambda_HI\right)^+
...
j^\star
\in
\operatorname*{arg\,min}_{j\in\mathcal E_k}
\frac{\Lambda(A_j)}{K_j},
\qquad
...
reused in Eq.~\eqref{eq:phase3_star_geometry}; similarly, the Hessian block is reused in
...
\exp(-i\theta_{k,j^\star}^\star A_{j^\star})\), where \((\boldsymbol\theta_k^\star)_{\setminus j^\star}\) represents the optimized circuit parameterization after removing coordinate \(j^\star\).
...
phases zero through three, followed by batch, beam, and generator ablation,
...
We record $E_{\rm{ADAPT}}$ and $\ket{\psi_{\rm{ADAPT}}}$ at the displayed evaluation iteration $k_{\rm eval}$. For the replacement SNAKE rows in this evidence copy, $k_{\rm eval}$ is terminal for weak--weak ($30$), weak--strong ($50$), and intermediate--strong ($50$). It is the user-selected last plateau point for intermediate--weak ($23$), strong--weak ($9$), and strong--strong ($33$). The SNAKE history axis is zero based: these three visible indices correspond to one-based preserved checkpoint positions and active depths $24$, $10$, and $34$, respectively. The retained Geo-ADAPT and append-only ADAPT context rows remain at their previously displayed first-plateau prefixes $k_{\rm pl}$. Consequently, the comparison is descriptive rather than prefix matched.

Circuit-resource columns report two-qubit gate count (\(N_{2q}\)), two-qubit depth (\(D_{2q}\)), and total circuit depth (\(D_c\)) when a Qiskit transpilation sidecar exists \cite{JavadiAbhari2024Qiskit,QiskitAlgorithmsAdaptVQE}. All six SNAKE triples are compiled from their exact displayed endpoints using the backend-free \texttt{table\_i\_basis\_gate\_transpile\_v1} convention (optimization level 0, seed 7): weak--weak, weak--strong, and intermediate--strong use preserved terminal results after terminal refit, while intermediate--weak, strong--weak, and strong--strong use their preserved plateau checkpoints. For SNAKE, $S$ is the ledger-closed, state-keyed winning-lineage estimator-query total $S_{\rm alg}$ through the same displayed prefix. Terminal rows include the terminal refit, while selected plateau rows stop at their preserved post-admission checkpoint. Discarded-branch work is zero because beam is disabled. The comparator $S$ entries retain their previously displayed plateau-prefix accounting.
...
%
% BEGIN_MACHINE_READABLE_NO_ORDINARY_NOVELTY_SR_SNAKE_EVIDENCE_COPY_20260717
% {
%   "schema": "paper_i_no_ordinary_novelty_sr_snake_evidence_copy_v1",
%   "copy_status": "separate_evidence_copy_active_manuscript_unchanged",
%   "source_active_tex": "MATH/paper_details/Paper_I.tex",
%   "source_active_tex_sha256": "f28f98309f4d99011738ba011ad0247e64bc08a4a011c4bf9c9475d249046d4a",
%   "governing_tracker": "MATH/paper_facing/paper_I_static_scaffold/paper_i_sr_snake_selected_candidate_and_ablation_tracker_20260716.json",
%   "governing_tracker_sha256": "2251a037262f169f7ab30ca3772d4497fd7a3c254b237f354d3954c5831dadc1",
%   "copy_provenance_json": "MATH/paper_facing/paper_I_static_scaffold/paper_i_no_ordinary_novelty_sr_snake_evidence_copy_20260717.json",
%   "plot_provenance_json": "MATH/paper_details/figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717_provenance.json",
%   "endpoint_policy": "SNAKE terminal for WW/WS/IS; zero-based displayed plateau k=23/9/33 (checkpoint positions 24/10/34) for IW/SW/SS; retained Geo/Append context rows use historical first-plateau prefixes",
%   "qiskit_policy": "all six SNAKE rows compiled at the same displayed endpoints with table_i_basis_gate_transpile_v1; WW/WS/IS terminal and IW/SW/SS selected plateau",
%   "s_policy": "replacement SNAKE values are state-keyed ledger-closed winning-lineage S_alg through the same displayed endpoint; discarded branch work zero",
%   "rows": {
%     "weak-weak": {"k_eval": 30, "abs_delta_e": 7.993605777301127e-15, "N2q": 258, "D2q": 236, "Dc": 1234, "S_alg": 39947},
%     "intermediate-weak": {"k_eval": 23, "checkpoint_position": 24, "abs_delta_e": 3.4416913763379853e-15, "N2q": 196, "D2q": 170, "Dc": 901, "S_alg": 33814},
%     "strong-weak-u8": {"k_eval": 9, "checkpoint_position": 10, "abs_delta_e": 1.5711479179891796e-6, "N2q": 54, "D2q": 40, "Dc": 329, "S_alg": 7639},
%     "weak-strong": {"k_eval": 50, "abs_delta_e": 3.051703169054676e-6, "N2q": 822, "D2q": 757, "Dc": 3187, "S_alg": 182236},
%     "intermediate-strong": {"k_eval": 50, "abs_delta_e": 4.7338917443573436e-5, "N2q": 824, "D2q": 767, "Dc": 3320, "S_alg": 196013},
%     "strong-strong-u8": {"k_eval": 33, "checkpoint_position": 34, "abs_delta_e": 3.2083758372269955e-7, "N2q": 404, "D2q": 329, "Dc": 1687, "S_alg": 80909}
%   }
% }
% END_MACHINE_READABLE_NO_ORDINARY_NOVELTY_SR_SNAKE_EVIDENCE_COPY_20260717
%
\clearpage
\onecolumngrid
...
\centering
\includegraphics[width=\linewidth]{figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717__weak_weak.png}
\par\smallskip
{\tiny\setlength{\tabcolsep}{1.35pt}%
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & \(k\) & \(|\Delta E|\) & \(N_{\twq}/D_{\twq}/D_c\) & \(S\)\\
\midrule
SNAKE & 30 & 7.994e-15 & 258/236/1,234 & 39,947 \\
...
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717__intermediate_weak.png}
\par\smallskip
{\tiny\setlength{\tabcolsep}{1.35pt}%
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & \(k\) & \(|\Delta E|\) & \(N_{\twq}/D_{\twq}/D_c\) & \(S\)\\
\midrule
SNAKE & 23 & 3.442e-15 & 196/170/901 & 33,814 \\
...
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717__strong_weak.png}
\par\smallskip
{\tiny\setlength{\tabcolsep}{1.35pt}%
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & \(k\) & \(|\Delta E|\) & \(N_{\twq}/D_{\twq}/D_c\) & \(S\)\\
\midrule
SNAKE & 9 & 1.571e-06 & 54/40/329 & 7,639 \\
...
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717__weak_strong.png}
\par\smallskip
{\tiny\setlength{\tabcolsep}{1.35pt}%
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & \(k\) & \(|\Delta E|\) & \(N_{\twq}/D_{\twq}/D_c\) & \(S\)\\
\midrule
SNAKE & 50 & 3.052e-06 & 822/757/3,187 & 182,236 \\
...
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717__intermediate_strong.png}
\par\smallskip
{\tiny\setlength{\tabcolsep}{1.35pt}%
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & \(k\) & \(|\Delta E|\) & \(N_{\twq}/D_{\twq}/D_c\) & \(S\)\\
\midrule
SNAKE & 50 & 4.734e-05 & 824/767/3,320 & 196,013 \\
...
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/paper_i_no_ordinary_novelty_sr_snake_20260717/paper_i_no_ordinary_novelty_sr_snake_20260717__strong_strong.png}
\par\smallskip
{\tiny\setlength{\tabcolsep}{1.35pt}%
\begin{tabular}{@{}lrrrr@{}}
\toprule
Method & \(k\) & \(|\Delta E|\) & \(N_{\twq}/D_{\twq}/D_c\) & \(S\)\\
\midrule
SNAKE & 33 & 3.208e-07 & 404/329/1,687 & 80,909 \\
...
\end{tabular}}
\end{minipage}
\captionof{figure}{Hubbard--Holstein error-versus-iteration trajectories for the no-ordinary-novelty SR-SNAKE evidence copy, with the previously displayed Geo-ADAPT and append-only ADAPT curves retained as context. The top row shows weak-Holstein sectors with \(M=2\), and the bottom row shows strong-Holstein sectors with \(M=4\). The SNAKE star marks the terminal point for weak--weak, weak--strong, and intermediate--strong, and the selected last plateau for intermediate--weak (visible \(k=23\), checkpoint position 24), strong--weak (visible \(k=9\), checkpoint position 10), and strong--strong (visible \(k=33\), checkpoint position 34). Comparator markers retain their historical first-plateau prefixes. Mini-tables report the same-prefix evaluation index, same-cutoff energy error, Qiskit cost triple \(N_{2q}/D_{2q}/D_c\), and ledger-closed winning-lineage \(S_{\rm alg}\). All six SNAKE triples use \texttt{table\_i\_basis\_gate\_transpile\_v1}. Because cross-method endpoint rules differ, this is not a prefix-matched resource comparison.}
...
At the displayed evaluation points, the no-ordinary-novelty SNAKE errors are
lower than the retained Geo-ADAPT and append-only ADAPT errors in all six
regimes. This statement is descriptive: three SNAKE rows use terminal endpoints,
three use selected last-plateau checkpoints, and comparator rows retain their
historical first-plateau prefixes. Qiskit costs are reconstructed for all six
displayed SNAKE endpoints, but no cross-method compiled-cost or
estimator-work superiority claim is inferred from the mixed endpoint conventions.
\par\addvspace{1.0ex}
...
The replacement Hubbard--Holstein SNAKE rows in this evidence copy use one
...
complete executable payloads and artifact hashes are listed in the governing
machine-readable provenance sidecar; this copy does not redefine the
unqualified canonical SR-SNAKE alias.
...
the replacement rows in this copy.}
\centering
\scriptsize
...
\midrule
Route family & singleton-response SNAKE & undamped; no prune; no beam; ordinary novelty off; infeasible-model fallback retained \\
...
Cutoff and reference & same-cutoff ED & \(M=2\) for weak-Holstein sectors and \(M=4\) for strong-Holstein sectors; working and exact-reference cutoffs are identical \\
Phase 0 & off & no Phase-0 work \\
...
Phase-III coordinates & supported-metric eigenspace whitening & supported Fubini--Study whitening; coordinate-solve scope Phase III only \\
Trust update & displacement-calibrated adaptive radius & variable Fubini--Study radius with no fixed upper cap \\
...
Accepted refit & full accepted ansatz & supported-FS-whitened refit over the expanded-runtime/projected-logical base chart \\
...
Stopping and reporting & \(k_{\max},k_{\rm eval}\) & terminal for weak--weak/weak--strong/intermediate--strong; selected last plateau for intermediate--weak/strong--weak/strong--strong \\
Compiled resources & displayed-endpoint Qiskit sidecars & resolved for all six SNAKE rows using \texttt{table\_i\_basis\_gate\_transpile\_v1} \\
\bottomrule
\end{tabular}%
}
\end{table*}
