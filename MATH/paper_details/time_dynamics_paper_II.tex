...
Collecting the pieces, the rule evaluated at every checkpoint is compact.  Deletions are always eligible: a candidate deletion is a row and column selection of geometry already acquired for propagation, so its structural evaluation costs no additional measurement.  Insertions are eligible while the McLachlan distance exceeds its declared cut, because candidate insertion requires new tangent overlaps.  For every admitted patch define the signed normalized change in realized captured drift
\begin{equation}
\delta_k(\mathcal B)
=
...
\end{center}
\captionof{figure}{Doublon-fraction deviation from the exact trajectory under the strong fast drive, for the two frozen comparator variants of Table~\ref{tab:competitor_baseline}.  The raw observable cannot separate methods on this problem --- all of them track it --- so the deviation is plotted instead.  Panels for the remaining five drives are generated by the same script and overlay this work's route when a matched run is available.  \tentative{Comparator baseline; this work's overlay pending the source-locked matrix.}}
\label{fig:competitor_doublon_strongfast}
\end{minipage}
\par\addvspace{1.0ex}
...
\end{center}
\captionof{figure}{The same quantity under the fast fast drive, the cell where the two comparator variants separate most and where the published numerics are least favourable.  Reporting only one variant would misstate the baseline by roughly an order of magnitude here.  \tentative{Comparator baseline.}}
\label{fig:competitor_doublon_fastfast}
\end{minipage}
\par\addvspace{1.0ex}
