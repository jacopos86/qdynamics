...
\begin{table}[t]
\caption{Adaptive-append comparison on one driven Hubbard--Holstein seed ($L=2$, $n_{\rm ph}^{\max}=1$, static-ansatz energy error $7.0\times10^{-5}$), with all policies sharing the seed, drive, reporting grid, candidate pool (identical membership and order, capped at eight words), inverse policy, solve repair, and fourth-order integration; errors are taken against the exact time-ordered trajectory of the same truncated Hamiltonian.  At this pool cap the comparator's McLachlan-distance cut is not reachable at most checkpoints, so it appends to its per-checkpoint budget and the two comparator rows differ by that budget rather than by their thresholds; the comparison is therefore at matched pool and stated budget, not at matched threshold satisfaction.  \tentative{Preliminary short-horizon evidence ($t\le1$); to be regenerated at production horizon and at a pool cap that admits threshold satisfaction.}}
...
adaptive append, budget 2 \cite{Yao2021} & \tentative{$1.0\times10^{-3}$} & \tentative{$2.6\times10^{-3}$} & \tentative{76}\\
adaptive append, budget 8 \cite{Yao2021} & \tentative{$8.5\times10^{-4}$} & \tentative{$1.8\times10^{-3}$} & \tentative{148}\\
...
\end{center}
\captionof{figure}{Active support size under the three structural policies of Table~\ref{tab:comparator_hh_snake}.  The adaptive-append comparator grows monotonically because its rule contains no removal mechanism, while the exchange policy contracts by removing coordinates whose deletion loss is certified negligible; the two bound an accuracy-versus-support trade rather than one dominating the other.  \tentative{Preliminary short-horizon diagnostic.}}
\label{fig:comparator_params}
\end{minipage}
\par\addvspace{1.0ex}

The comparison is an accuracy-versus-support trade, not a dominance result.  Adaptive append is the most accurate policy here, reaching $1.0\times10^{-3}$ mean energy error at $76$ coordinates and $8.5\times10^{-4}$ at $148$; the exchange policy is the smallest, holding $3.5\times10^{-3}$ on $24$ coordinates, a factor of three to six fewer.  The append-only ablation of our own route sits between them on both axes.  Read together, growth buys accuracy at a roughly linear cost in support on this seed, and removal buys support at a proportionate cost in accuracy; which side is preferable depends on whether circuit size or trajectory error is the binding constraint.  Three qualifications bound the evidence: one seed, a short horizon, and a comparator whose threshold the capped pool cannot satisfy, so its growth is set by budget rather than by its own stopping rule.
