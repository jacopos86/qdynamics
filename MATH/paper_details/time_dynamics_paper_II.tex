...
\paragraph*{Operating rule.}  The operating response is a single rule: when \(\delta_{{\rm step},k}\) of Eq.~(\ref{eq:state_space_step}) exceeds \(\tau_\Delta\), the interval is subdivided, because damping is not the first cure for a physically accurate velocity evaluated over too large a step.  Subdivision refines the internal integration grid only, leaving the reporting grid and the accepted support unchanged.  Hard invalidity --- nonfinite or dimensionally inconsistent checkpoint data --- rejects the step outright.  A wider diagnostic mode that opens a bidirectional inverse search on the remaining defect channels, together with the release rule that returns a held repair to the base inverse, is specified in Appendix~\ref{app:technical_realization} with the measurements motivating its exclusion from the operating configuration.
...
adaptive append \cite{Yao2021} & \tentative{$1.8\times10^{-3}$} & \tentative{$8.5\times10^{-3}$} & \tentative{68}\\
...
Everything in this subsection is measured and retained for diagnosis of
pathological trajectories.  None of it is part of the method used for any
result in this paper: the operating numerical rule is the single
