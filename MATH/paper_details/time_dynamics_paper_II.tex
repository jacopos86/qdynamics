...
\end{center}
\captionof{figure}{Deviation of the doublon fraction from the exact trajectory, same runs as Fig.~\ref{fig:obs_doublon_strongfast}.  The raw observable cannot separate the policies because all of them track it; the deviation can.  The append-only ablation sits near $10^{-2}$ for the whole horizon, while the exchange route and the AVQDS comparator run an order of magnitude below it and interleave, neither dominating.  \tentative{One drive.}}
...
\end{center}
\captionof{figure}{Absolute energy error against the exact trajectory.  The discrete step in the exchange route near $t\approx8.6$ is the stabilization defect discussed in the text: it carries no structural edit, and it dominates that route's mean energy error while leaving the observables of Figs.~\ref{fig:obs_doublon_strongfast}--\ref{fig:obs_occupations_strongfast} essentially unchanged.  \tentative{One drive.}}
...
\end{center}
\captionof{figure}{Active support size under the three policies of Table~\ref{tab:comparator_hh_snake}.  The AVQDS comparator and the append-only ablation both grow monotonically, to $46$ and $44$ coordinates, because neither rule contains a removal mechanism; the exchange policy front-loads growth under the same append condition and then contracts to $37$ by removing coordinates whose deletion loss is certified negligible, including a late contraction near $t\approx8.5$.  \tentative{One drive.}}
