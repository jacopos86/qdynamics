...
Only one of the three responses is part of the operating method.  Across every trajectory reported here, candidate inverse conventions other than the base regularization were selected on two integration steps out of $885$, and step subdivision triggered by excessive projected state displacement was the mechanism that repeatedly changed a step (Appendix~\ref{app:technical_realization}).  The operating rule is therefore: evaluate the three diagnostics, and subdivide the interval when the state-space step of Eq.~(\ref{eq:state_space_step}) exceeds its bound.  The numerical-miss and temporal-kink diagnostics are reported but do not open an inverse search in the reported configuration.
...
The response follows the measured failure channel.  Excessive state displacement opens local time-step subdivision, because damping is not the first cure for a physically accurate velocity evaluated over too large a step; subdivision refines the internal integration grid only, leaving the reporting grid and the accepted support unchanged.  A diagnostic mode additionally opens a bidirectional inverse search on excessive numerical miss, since a base solve may be under- or over-regularized, and selects the least intervention among finite candidates satisfying the numerical-miss, state-motion, and temporal-continuity thresholds; that mode is not used for the results reported here.  If no finite candidate passes, a diagnostic trajectory may continue with the least-bad finite candidate and mark the checkpoint unsupported.  The supported inverse, whitening map, candidate construction, and release patience are given in Appendix~\ref{app:technical_realization}.
...
The closest adaptive-ansatz comparator is adaptive variational quantum dynamics simulation \cite{Yao2021AVQDS}, which grows the ansatz during evolution using the McLachlan distance
\begin{equation}
L_k^2(J)=\|\bar b_k\|_2^2-Q_k(J)
\label{eq:mclachlan_distance}
\end{equation}
as its trigger: while $L_k^2$ exceeds a fixed cut, it appends the pool operator whose zero-angle addition maximally reduces $L_k^2$, at the end of the circuit, and never deletes.  We implement that decision rule on the same checkpoint geometry, regularized inverse, solve repair, integrator, seed, drive, and time grid used by the exchange route, so the comparison isolates the structural rule rather than the numerical realization.  Two structural differences from Eq.~(\ref{eq:append_score}) remain by construction: the comparator ranks by drift reduction alone rather than drift reduction per compiled cost, and it does not position insertions at commutation-reduced cuts.

\begin{table}[t]
\caption{Adaptive-append comparison on one driven Hubbard--Holstein seed ($L=2$, $n_{\rm ph}^{\max}=1$, static-ansatz energy error $7.0\times10^{-5}$), with all methods sharing the seed, drive, reporting grid, candidate pool, inverse policy, solve repair, and fourth-order integration, and all errors taken against the exact time-ordered trajectory of the same truncated Hamiltonian.  \tentative{Preliminary short-horizon evidence ($t\le1$); to be regenerated at production horizon.}}
\label{tab:comparator_hh_snake}
\begin{ruledtabular}
\begin{tabular}{lccc}
Policy & $\overline{|\Delta E|}$ & $|\Delta E|_{t=T}$ & $N_{\rm param}^{\rm final}$\\
\colrule
append-only (ablation) & \tentative{$1.7\times10^{-3}$} & \tentative{$3.8\times10^{-3}$} & \tentative{46}\\
adaptive append \cite{Yao2021AVQDS} & \tentative{$1.8\times10^{-3}$} & \tentative{$8.5\times10^{-3}$} & \tentative{68}\\
exchange (this work) & \tentative{$3.5\times10^{-3}$} & \tentative{$6.3\times10^{-3}$} & \tentative{24}\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\IfFileExists{figures/paper_ii_comparator_params_hh_snake.pdf}{\includegraphics[width=0.98\columnwidth]{figures/paper_ii_comparator_params_hh_snake.pdf}}{}
\end{center}
\captionof{figure}{Active support size under the three structural policies of Table~\ref{tab:comparator_hh_snake}.  The adaptive-append comparator grows monotonically because its rule contains no removal mechanism; the exchange policy reaches a comparable accuracy at roughly one third of that support by removing coordinates whose deletion loss is certified negligible.  \tentative{Preliminary short-horizon diagnostic.}}
\label{fig:comparator_params}
\end{minipage}
\par\addvspace{1.0ex}

At this horizon the comparator's threshold is not the binding constraint on its growth: below a cut of $3\times10^{-4}$ it exhausts its per-checkpoint append budget at every checkpoint, so the reported configuration is its slowest-growing binding setting.  The exchange policy reaches its accuracy with $24$ coordinates against $68$, at a comparable count of measured candidate directions, while the append-only ablation of our own route sits between them.  Two qualifications bound this evidence: a single seed and a short horizon, and a comparator threshold that never enters its own asymptotic regime here.  Both are removed by the production-horizon batch.
