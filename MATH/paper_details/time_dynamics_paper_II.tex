...
Collecting the pieces, the rule evaluated at every checkpoint is compact.  Deletions are always eligible: a candidate deletion is a row and column selection of geometry already acquired for propagation, so its loss $\mathcal L_k^{\ominus}$ costs no additional measurement.  Insertions are eligible only when the structural-repair predicate is active, because a candidate insertion requires new tangent overlaps.  Every admitted candidate carries the single scalar score
\begin{equation}
S_k(\mathcal B)
=
\underbrace{\frac{\Delta_k^{\oplus}}{K_{t_k}^{\alpha_{\rm ins}}}}_{\text{insertion gain per cost}}
+\underbrace{\frac{K_{t_k}^{\alpha_{\rm del}}}{\mathcal L_k^{\ominus}+\eps_L}}_{\text{deletion cost per loss}}
+w_\delta\,\delta_k(\mathcal B),
\label{eq:operating_score}
\end{equation}
with $\delta_k$ the net change in realized captured drift.  Ranked candidates are certified one at a time against the ray and velocity-continuity tests of Eqs.~(\ref{eq:patch_continuity})--(\ref{eq:patch_velocity_continuity}) after a bounded local refit, and the first patch that passes commits atomically.  A finiteness and conditioning assertion protects every finalist but is not a tuned threshold: across the reported runs the largest committed retained condition number is five orders of magnitude below its bound.  The history and conditioning extensions of Appendix~\ref{app:technical_realization} are carried at zero weight, so Eq.~(\ref{eq:operating_score}) is the complete operating criterion.
...
\end{center}
\captionof{figure}{Preliminary mechanism diagnostic on the deliberately redundant conditioning-stress seed (Hubbard--Holstein, $L=2$, $n_{\rm ph}^{\max}=3$, same-cutoff exact reference, no drive; fourth-order integration with state-space solve repair, twenty-five reporting intervals to $t{=}1$): total energy under the append-only ablation and the full exchange policy at two deletion ray tolerances.  At $\tau_{\rm ray}{=}2\times10^{-3}$ the exchange policy conserves energy to $2.9\times10^{-4}$ --- $4.6\times$ better than the append-only ablation ($1.3\times10^{-3}$) --- while ending seventeen coordinates smaller at equal measured-candidate count, consistent with deletion of redundant coordinates improving the conditioning of the retained solve.  Over-constraining the gate ($\tau_{\rm ray}{=}5\times10^{-4}$) forces insert--delete churn and degrades both accuracy and measurement cost, so the tolerance has an interior optimum.  A loose gate ($5\times10^{-2}$, not shown) admits cumulative structural damage: per-patch continuity certificates do not compose over a trajectory.  \tentative{Preliminary stress-seed diagnostic; to be regenerated on a conventional fixed-structure VQE seed before quantitative claims.}}
...
\end{center}
\captionof{figure}{Active support size for the runs of Fig.~\ref{fig:mechanism_energy_stress_prelim}.  Insertion enumeration is residual-gated in every arm; the exchange arms additionally perform measurement-free pruning at quiet checkpoints.  At the optimal gate the redundant seed is compressed from 616 to 607 coordinates while the append-only ablation grows to 624; the over-constrained gate churns without net compression.  \tentative{Preliminary stress-seed diagnostic.}}
...
\noindent\begin{minipage}{\columnwidth}
\begin{center}
\IfFileExists{figures/paper_ii_prod_params_hh_fixedvqe_nph3.pdf}{\includegraphics[width=0.98\columnwidth]{figures/paper_ii_prod_params_hh_fixedvqe_nph3.pdf}}{}
\end{center}
\captionof{figure}{Active support on the redundant seed under the three policies of Table~\ref{tab:production_matrix}.  The comparator grows monotonically because its rule contains no removal mechanism; exchange holds and then reduces support while tracking the same trajectory.  \tentative{Preliminary horizon.}}
\label{fig:production_params}
\end{minipage}
...
or merely its scale.  Figures~\ref{fig:obs_doublon_fastweak}--\ref{fig:obs_doublon_slowstrong}
follow the doublon fraction on the driven Holstein seed to $t=10$ under both
drive conditions, against the exact time-ordered trajectory of the same
truncated Hamiltonian.

\noindent\begin{minipage}{\columnwidth}
\begin{center}
\IfFileExists{figures/paper_ii_obs_doublon_hh_fastweak_t10.pdf}{\includegraphics[width=0.98\columnwidth]{figures/paper_ii_obs_doublon_hh_fastweak_t10.pdf}}{}
\end{center}
\captionof{figure}{Doublon fraction to $t=10$ under the fast weak drive ($A=0.6$, $\omega=3$), Holstein seed.  Mean absolute deviations from the exact trajectory are \tentative{$8.5\times10^{-3}$} (append-only, $134$ coordinates), \tentative{$1.7\times10^{-2}$} (exchange, $13$), and \tentative{$2.5\times10^{-2}$} (adaptive append, $888$).  The comparator's support exceeds the exchange policy's by a factor near seventy while tracking the observable less accurately.  \tentative{Preliminary: drawn from runs continuing to $t=20$.}}
\label{fig:obs_doublon_fastweak}
\end{minipage}
\par\addvspace{1.0ex}

\noindent\begin{minipage}{\columnwidth}
\begin{center}
\IfFileExists{figures/paper_ii_obs_doublon_hh_slowstrong_t10.pdf}{\includegraphics[width=0.98\columnwidth]{figures/paper_ii_obs_doublon_hh_slowstrong_t10.pdf}}{}
\end{center}
\captionof{figure}{Doublon fraction to $t=10$ under the slow strong drive ($A=1.2$, $\omega=1$).  Here the adaptive-append comparator is the most accurate on energy (\tentative{$2.7\times10^{-3}$} against \tentative{$1.2\times10^{-2}$} for both other policies) and reaches it on $924$ coordinates against $12$; on the doublon fraction the three agree to within a factor of two.  Accuracy and support therefore trade differently under the two drives, and the exchange policy holds the smaller support in both.  \tentative{Preliminary.}}
\label{fig:obs_doublon_slowstrong}
\end{minipage}
\par\addvspace{1.0ex}

Two features are stable across both drives.  The exchange policy runs on one to
two orders of magnitude fewer coordinates than the growth policies while
tracking the same observable, and the growth policies pay for accuracy with
support that increases monotonically.  What is not stable is which policy is
most accurate: append-only leads under the fast weak drive and adaptive append
under the slow strong one, by margins comparable to the spread between them.
The evidence supports a cost claim across drives and does not yet support an
accuracy ordering.
...
\begin{equation}
L_k^2(J)=\|\bar b_k\|_2^2-Q_k(J)
\label{eq:mclachlan_distance}
\end{equation}
as its trigger: while $L_k^2$ exceeds a fixed cut, it appends the pool operator whose zero-angle addition maximally reduces $L_k^2$, at the end of the circuit, and never deletes.  We implement that decision rule on the same checkpoint geometry, regularized inverse, solve repair, integrator, seed, drive, and time grid used by the exchange route, so the comparison isolates the structural rule rather than the numerical realization.  We further adopt the comparator's append \emph{condition} for the exchange route's own insertions, since growth is not this work's contribution: insertions are considered when $L_k^2$ exceeds the cut and are repeated within one checkpoint until it is met or no candidate improves it.  The route's normalized residual-ratio gate is retained only as an ablation.  The reason is measured rather than stylistic: a normalized gate is small precisely while the state is still accurate, so it defers growth past the window in which growth is cheap, and on this drive the comparator completed its growth by $t\approx0.9$ and held $46$ coordinates thereafter while the residual-gated route was still adding coordinates at $t\approx2$ with $L_k^2$ four orders of magnitude larger and a state already off the exact trajectory.  Two structural differences from Eq.~(\ref{eq:append_score}) remain by construction: the comparator ranks by drift reduction alone rather than drift reduction per compiled cost, and it does not position insertions at commutation-reduced cuts.

Under this condition accuracy debt and cost relief are sequenced rather than scored against one another.  While $L_k^2$ exceeds the cut the checkpoint owes accuracy and only insertions are admitted; once it falls below, the geometry has already been paid for and removal is free.  Scoring them jointly inverts the rule, because a deletion always improves the cost-weighted objective: without the sequencing the within-checkpoint repeat spends its rounds shedding coordinates while $L_k^2$ climbs, measured on this seed as $26\rightarrow19$ coordinates with $L_k^2$ rising from $4.5\times10^{-4}$ to $2.2\times10^{-1}$.

\begin{table}[t]
\caption{Adaptive-append comparison on one driven Hubbard--Holstein seed ($L=2$, $n_{\rm ph}^{\max}=1$) under the strong fast drive ($A=2.4$, $\omega=3$) to $t=10$ over $251$ checkpoints.  Both policies share the seed, drive, reporting grid, inverse policy, solve repair, fourth-order integration, and the full deduplicated candidate pool of $125$ words; errors are against the exact time-ordered trajectory of the same truncated Hamiltonian.  The pool is no longer capped below its own size, so the comparator reaches its McLachlan-distance cut by its own stopping rule rather than by a per-checkpoint budget, and no budget is imposed on it.  The residual-gated rows are the superseded insertion condition, retained as ablations of the gate itself and marked as such.  \tentative{One drive and one seed; the remaining five drives are in progress.}}
...
exchange (this work) & \tentative{$2.1\times10^{-2}$} & \tentative{$3.3\times10^{-3}$} & \tentative{37}\\
\colrule
\multicolumn{4}{l}{\emph{superseded residual-ratio gate, retained as gate ablations}}\\
append-only & \tentative{$1.33\times10^{-1}$} & \tentative{$2.9\times10^{-2}$} & \tentative{74}\\
exchange & \tentative{$1.26\times10^{-1}$} & \tentative{$2.8\times10^{-2}$} & \tentative{67}\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\end{center}
\captionof{figure}{Active support size under the two structural policies of Table~\ref{tab:comparator_hh_snake}.  The adaptive-append comparator grows monotonically because its rule contains no removal mechanism and then holds $46$ coordinates; the exchange policy front-loads growth under the same append condition and then contracts to $37$ by removing coordinates whose deletion loss is certified negligible.  \tentative{One drive.}}
\label{fig:comparator_params}
\end{minipage}
\par\addvspace{1.0ex}

On the observables the two policies are equivalent: the exchange route tracks the doublon fraction to $3.3\times10^{-3}$ against the comparator's $2.7\times10^{-3}$, and both overlay the exact site occupations through the full horizon (Figs.~\ref{fig:obs_doublon_strongfast}--\ref{fig:obs_occupations_strongfast}), which oscillate strongly to $t=10$ and are the discriminating observables on this drive.  The exchange route holds $37$ coordinates against $46$, a $20\%$ smaller support, because it removes coordinates whose deletion loss is certified negligible while the comparator's rule contains no removal mechanism.  Matched observable accuracy at smaller support is the claim this comparison supports.

The append-only ablation, run under the same insertion condition and differing only in that removal is disabled, is worse on both axes: $9.8\times10^{-3}$ on the doublon fraction at $44$ coordinates, three times the exchange route's deviation on seven more coordinates.  Removal is therefore not merely a cost device on this drive; forbidding it costs accuracy as well as support, which is consistent with the conditioning relief that removal provides.

The energy is the one axis where the two separate, and the separation is not distributed over the trajectory: the exchange route holds $\sim\!1.5\times10^{-2}$ from $t\approx2$ to $t\approx8.6$ and then takes a single discrete step to $\sim\!8\times10^{-2}$, which dominates its mean.  That step carries no structural edit; the state-motion guard is active and escalating at it, to depth six and $64$ substeps with per-substep prospective motion $1.8\times10^{-3}$ against a $10^{-2}$ cap.  The guard bounds displacement per substep, so subdividing further shrinks each substep while permitting more of them and leaves the displacement accumulated across one checkpoint unbounded.  We report this as an open defect of the stabilization layer rather than of the structural rule, and note that it is nearly invisible in the observables the method targets: $\langle H\rangle$ resolves it because the truncated spectrum spans $[-0.4,\,10.4]$.  Three qualifications bound the evidence: one seed, a short horizon, and a comparator whose threshold the capped pool cannot satisfy, so its growth is set by budget rather than by its own stopping rule.
...
the operating structural rule is Eq.~(\ref{eq:operating_score}).  The material
...
\end{equation}
The conditioning and history terms in Eq.~(\ref{eq:app_prune_nomination}) are extensions carried at zero weight in every run reported here; the operating deletion score is the cost-over-loss ratio alone, and no result in this paper depends on a nonzero $\lambda$.  Conditioning terms change nomination only.  Current grouped deletion loss, current patched solve geometry, reduced-state refit, ray continuity, velocity continuity, persistence, cooldown, and rollback retain commit authority.  Atom-level persistence survives append-only support changes for atoms that remain active; support-conditional loss and smoothness histories are invalidated when the support changes.
