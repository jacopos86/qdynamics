...
\end{equation}
Cooldown and atomic finalist rejection prevent repeated unsafe deletion attempts.  The history and conditioning nomination score is given in Appendix~\ref{app:technical_realization}; no deletion-persistence gate is part of the operating selector.
...
This mathematical search is independent of how an ansatz or generator pool is represented.  The AP realization supplies the active coordinate identities, positioned insertion occurrences, measured checkpoint geometry, resource costs, and a materialization-and-certification map for finalists.  The generalized-exchange rule itself consumes only the patch pair $(D,A)$ and realized objective values.  Consequently a different ansatz family or generator representation changes the realization adapter, not the admissible faces or the debt-ordering rule.
...
The three diagnostics are formed at every checkpoint, but they do not all open the same response.  The operating rule subdivides the interval when the tangent-state step of Eq.~(\ref{eq:state_space_step}) or the temporal kink of Eq.~(\ref{eq:temporal_kink}) exceeds its bound; the numerical-miss channel opens no search.  \tentative{The earlier diagnostic audit counted $372$ subdivisions from a now-retired realized trial-state overlap guard and $254$ from the temporal-kink diagnostic.  Those counts motivate the controller ablation but are not results for the tangent-only operating rule.}  Candidate inverse conventions other than the base regularization were selected on two integration steps out of $885$, and six of $1.15\times10^{4}$ evaluated candidates were accepted (Appendix~\ref{app:diagnostic_only}).
...
The second diagnostic is the Gram-metric size of the proposed tangent-state step,
\begin{equation}
\delta_{{\rm step},k}(v)
=\Delta t_k\sqrt{v^\top G_kv}
=\Delta t_k\|\bar T_kv\|_2 .
\label{eq:state_space_step}
\end{equation}
Here \(v\) is the candidate parameter velocity, \(G_k=\bar T_k^\top\bar T_k\)
is the checkpoint Gram matrix, and \(\Delta t_k\) is the proposed time step.
All three objects are already available from the McLachlan solve.  Evaluating
Eq.~(\ref{eq:state_space_step}) therefore requires no preparation of a trial
state and no additional overlap measurement.
...
\(L^2_{\rm cut}\).  In adaptive variational quantum dynamics simulation
(AVQDS) the cut closes greedy appends; in the generalized-exchange route it
closes the insertion faces while the pure-deletion face remains eligible.  The
natural comparison is to set both cuts to a common value.  On this problem that
is not sound.
...
We instead fix an accuracy target \(\varepsilon\) and ask what each run
configuration costs to reach it.  Past trajectories determine only the first
threshold worklist: a target-reaching prior contributes its resource-cheapest
target-reaching cut, a prior that has not yet reached the target contributes
its next tighter cut, and one logarithmic rung is added on either side.  A
drive with no prior evidence receives the complete initial ladder.  Every new
configuration is nevertheless run; old trajectories do not fill new cells.
If the worklist does not reach \(\varepsilon\), it is extended to tighter cuts
for that cell and the interim statement is ``target not yet reached.''  The
minimum-final-support target hit is a rapid diagnostic.  For the final resource
comparison we compile every target-reaching terminal ansatz under the Paper-I
FakeMarrakesh convention (optimization level one and transpiler seed seven),
including reference-state preparation, and select the rung with the smallest
compiled two-qubit gate count; ties are broken by two-qubit depth and then mean
absolute energy error.  We report the compiled two-qubit gate count, total
circuit depth, and two-qubit depth for every target-reaching rung.

This is an exact-reference-tuned, per-cell resource diagnostic: it answers how
much final ansatz was required to reach \(\varepsilon\) on a known drive, but it
is not an online QPU stopping rule because an experimental run does not know its
exact trajectory error.  Selecting the best rung is also a minimum over a
ladder whose response is not smooth: writing the adjacent-rung ratio as the mean absolute energy error at one
cut divided by that at the next-looser cut, the largest observed value across
\tentative{42} adjacent pairs is \tentative{$19.1$}, the smallest
\tentative{$0.033$}, and \tentative{11 of 42} exceed unity, meaning that
tightening the cut by one rung degraded accuracy in roughly a quarter of cases.
We therefore report every evaluated rung, label the selected point as per-cell
exact-reference tuning, and do not quote best-rung ratios as headline figures.

The primary experiment is factorial.  Its two algorithmic methods are APM
generalized exchange and AVQDS greedy growth.  Each is paired with three
time-step controllers: the tangent-state bound of
Eq.~(\ref{eq:state_space_step}), the parameter bound
\(\max_\mu |\dot\theta_\mu|\Delta t\le5\times10^{-3}\), and their composition.
These are six run configurations, not six methods.  All six initially share
Euler integration, Tikhonov regularization \(\xi=10^{-6}\), zero additional
damping, the same pseudoinverse cutoff, seed, candidate pool, drive, and
reporting grid.  This design isolates the algorithmic-method effect, the
time-step-controller effect, and their interaction.  The full selected matrix
will later be repeated under fourth-order integration as a consistency check;
that repetition is not part of the initial factorial experiment.

The prior frontier reaches \(\varepsilon=10^{-4}\) for several historical
growth-only and AVQDS cells and identifies useful threshold neighborhoods, but
it does not contain the six configurations under the common numerical lock.
\tentative{Accordingly, the two-by-three factorial result is in progress, and
no fixed-accuracy statement about generalized exchange or controller choice is
made here in its absence.}

\subsection{Historical adaptive-append threshold prior}
\label{sec:competitor_baseline}

The following earlier measurements are retained as a threshold-planning prior,
not as the factorial comparison.  Two variants were recorded because they
answered different questions at that stage.  The
...
and a claim about structural policy must cite the second.  Neither variant
fills a cell of the new two-method by three-controller matrix.
...
\begin{table*}[t]
\caption{Historical insertion-only-debt exchange ablation against the frozen adaptive-append baseline at matched numerics, driven Hubbard--Holstein seed ($L=2$, $n_{\rm ph}^{\max}=1$) to $t=10$.  Errors are against the exact time-ordered trajectory of the same truncated Hamiltonian.  These values do not test the signed-drift operating rule of Eq.~(\ref{eq:debt_score}).  \tentative{One seed and six drives; the remaining historical cell is in progress.}}
\label{tab:matched_comparison}
\begin{ruledtabular}
\begin{tabular}{lcccccccc}
& \multicolumn{3}{c}{adaptive append} & \multicolumn{3}{c}{exchange ablation} &\\
\cline{2-4}\cline{5-7}
...
The energy is the one axis where the two separate, and the separation is not distributed over the trajectory: the insertion-only-debt exchange ablation holds $\sim\!1.5\times10^{-2}$ from $t\approx2$ to $t\approx8.6$ and then takes a single discrete step to $\sim\!8\times10^{-2}$, which dominates its mean.  That step carries no structural edit.  In the historical run, the now-retired trial-overlap guard escalated to depth six and $64$ substeps; the tangent-only controller requires a new matched trajectory before that event can be interpreted.  We report the step as an open defect of the stabilization layer rather than evidence about the revised signed-drift structural rule, and note that it is nearly invisible in the observables the method targets: $\langle H\rangle$ resolves it because the truncated spectrum spans $[-0.4,\,10.4]$.  Three qualifications bound the evidence: one seed, a short horizon, and an insertion-only-debt policy superseded at the method level pending matched validation of Eq.~(\ref{eq:debt_score}).

\subsection{Factorial interactions}

The complete mechanism study crosses the generalized-exchange restriction (insert face, delete face, or full domain) and finalist-refit ablation with state-space numerical repair while retaining the same nominal inverse and reporting grid.  Insertion changes the dimension and retained spectrum presented to the solve; deletion can remove redundant or conditioning-toxic modes; repair determines how the selected support is realized over a finite step.  The crossed comparison therefore reports main effects and policy--repair interactions for trajectory error, integrated structural miss, unsupported checkpoints, final support, retained conditioning, measured-candidate work, and compiled resources.  A deliberately disabled numerical stabilizer is treated as an ablation, not as the canonical operating condition.
...
paper: the operating numerical rule is the tangent-state subdivision of
Sec.~\ref{sec:numerical_solve_repair}, and
...
\paragraph*{Operating configuration.}  The candidate set above is the diagnostic superset.  Across the historical $885$-step audit, the inverse-candidate search left the base regularization in place on all but two steps, damping never moved from its base value, and of $1.15\times10^4$ evaluated candidates six were accepted; the conditioning, numerical-miss, and temporal-kink triggers opened searches that did not change the accepted solve.  The reduced operating configuration therefore retains local subdivision and the base inverse convention alone.  The integration controller now evaluates the tangent-state quantity in Eq.~(\ref{eq:state_space_step}) and the temporal kink; it does not prepare a trial state or measure a trial-state overlap.  \tentative{The former count of $372$ overlap-triggered subdivisions is retained only as historical motivation and must not be reported as current-controller activity.}  Substituting the reduced inverse configuration for the full candidate search on the driven seed of Table~\ref{tab:comparator_hh_snake} changed the mean energy error from $3.4735\times10^{-3}$ to $3.4738\times10^{-3}$ at identical final support, while removing a quarter of the candidate solves.  The wider set is retained for diagnosis of pathological checkpoints, not as the operating point.
...
\end{equation}
The conditioning and history terms in Eq.~(\ref{eq:app_prune_nomination}) are extensions carried at zero weight in every provisional run reported here, and no reported result depends on a nonzero $\lambda$.  During accuracy debt, Eq.~(\ref{eq:app_prune_nomination}) contributes only through the bounded secondary term of Eq.~(\ref{eq:debt_score}); the signed realized-drift change is primary.  After the distance cut is met, the cost-over-loss utility resumes.  Current patched solve geometry, reduced-state refit, ray continuity, velocity continuity, cooldown, and atomic rollback retain commit authority.  No persistence gate is part of the operating selector; support-conditional loss and conditioning histories are invalidated when the support changes.
