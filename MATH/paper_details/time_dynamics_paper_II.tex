...
Set \(B_k^{\ominus}=\varnothing\).  Let \(B_k^{\oplus}\) be a candidate append batch and \(I_{B,k}^{\oplus}\) its new coordinate indices.  Added coordinates enter at zero amplitude, so the checkpoint state is unchanged.  The normalized gain from refitting the old and new tangent coordinates together is
\begin{equation}
\Delta_k^{\oplus}(B_k^{\oplus})
=
\frac{
\left[
\Gamma_k(J_k\cup I_{B,k}^{\oplus})
-\Gamma_k(J_k)
\right]_+
}{
\|\bar b_k\|_2^2+\eps_{\rm norm}
...
\end{equation}
An append batch must retain the declared rank in \(\mathsf S_{B|J,k}\) and yield a finite augmented McLachlan solve.  The combinatorial ladder increases the tested batch cardinality only when smaller batches do not clear the gain and residual criteria.  Whitening, retained-mode compression, and failed-search reuse are implementation accelerations collected in Appendix~\ref{app:technical_realization}; they do not replace Eq.~(\ref{eq:append_gain}).
...
\left[
\Gamma_k(J_k)
-\Gamma_k(J_k\setminus I_{B,k}^{\ominus})
\right]_+
}{
\|\bar b_k\|_2^2+\eps_{\rm norm}
}.
...
Because Eq.~(\ref{eq:prune_loss}) uses submatrices of the active \(G_k\) and \(f_k\) already acquired for McLachlan propagation, prune nomination itself requires no new quantum measurements.  A deletion commit is stricter.  The candidate block is first set to the identity and, if necessary, the surviving parameters are locally refit to the pre-patch state.  This reduced-state fit or overlap certification can require additional circuits on a QPU.
...
\end{equation}
Persistence prevents a transiently redundant block from being removed after one checkpoint; cooldown and exact rollback prevent repeated unsafe attempts.  The history and conditioning nomination score is given in Appendix~\ref{app:technical_realization}.

\subsection{Bounded support-patch search used here}

The ideal support-patch problem is combinatorial.  Exhaustive enumeration, mixed-integer optimization, branch-and-bound, greedy forward--backward selection, and local exchange are all possible search strategies for related subset problems \cite{Natarajan1995SparseApprox,BertsimasKingMazumder2016,DasKempe2011,Elenberg2018WeakSubmodularity}.  AP-McLachlan uses a bounded two-pass search because candidate tangent measurements, not only classical subset enumeration, contribute to the checkpoint cost.

The first pass constructs separate append and prune scout lists.  Append scouts are ranked by explained-drift gain per hardware cost and must retain Schur novelty.  Prune scouts are ranked among low-loss deletions using recovered hardware cost, historical redundancy, and conditioning relief.  The default executable atoms are Pauli- or polynomial-child terms, and candidate batches are evaluated jointly through a combinatorial cardinality ladder rather than by summing singleton scores.

The second pass forms a finite frontier containing stay, independently admissible pure append and pure prune candidates, and true exchanges obtained by pairing retained append and prune scouts.  Every retained candidate is recomputed on its complete patched support.  Selection uses conditional append gain, conditional deletion loss, hardware cost, Schur and conditioning terms, refit effort, the full patched solve, and the state and velocity continuity tests.  Scout-list sizes and pair-count limits bound classical and measurement work; they do not force a scientific batch size.  Exact trajectories do not enter the frontier, ranking, or commit tests.  Further history, conditioning, and retry conventions are collected in Appendix~\ref{app:technical_realization}.
...
The structural comparison first holds numerical realization fixed and contrasts stay with child-resolved append, continuity-certified prune, and the joint support-patch policy.  It reports structural miss, accepted patch types, conditional gain and loss, same-checkpoint continuity, final support, and compiled cost.  The numerical-repair comparison then holds the support fixed and contrasts passive telemetry under the nominal solve with the intervening state-space policy.  It reports numerical miss, state-space displacement, temporal kink, inverse-policy changes, local subdivisions, unsupported checkpoints, and post-run energy and observable errors.  These comparisons separate support maintenance from numerical realization before their interaction is interpreted.
...
Append uses a cost-weighted combinatorial batch ladder.  Candidate tangent overlaps are measured once and reused to assemble singleton and joint-batch Gram blocks.  A failed append search may be reused only while current-support geometry and selected sentinel couplings remain within declared staleness bounds; this is a search-reuse certificate, not a cap on the total number of append events.
...
True exchange is formed from the retained append and prune scouts.  Every pair is recomputed on
\[
J_k^{\mathcal B}
=(J_k\setminus I_{B,k}^{\ominus})\cup I_{B,k}^{\oplus},
\]
using conditional append gain, conditional deletion loss, patched Schur novelty, the full patched solve, and Eqs.~(\ref{eq:patch_continuity})--(\ref{eq:patch_velocity_continuity}).  This joint recomputation is what distinguishes exchange from sequential append followed by prune.
